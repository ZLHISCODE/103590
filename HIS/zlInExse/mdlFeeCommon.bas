...
Public grsSquareType As ADODB.Recordset '消费卡类别目录
...
Public Function GetStockSet(ByVal lng药房ID As Long, ByVal lng药品ID As Long, Optional ByVal lng批次 As Long = 0) As Recordset
    Dim strSQL As String
    ...
    On Error GoTo errH
    Set GetStockSet = zlDatabase.OpenSQLRecord(strSQL, App.ProductName, lng药房ID, lng药品ID, lng批次)
...
Public Function Get时价药品应收金额(ByVal lng药房ID As Long, ByVal lng药品ID As Long, ByRef dblAllTime As Double, _
...
'      dblPriceSingle-只有一个批次时返回该批次的单价，避免根据数量先乘再除后因四舍五入引起不同的数量的单价不同
    ...
    Set rsPrice = GetStockSet(lng药房ID, lng药品ID, lng批次)
    '时价=总金额/总数量
    dblPrice = 0 '本笔总应收金额
    For i = 1 To rsPrice.RecordCount
        ...
        Else
            dblPriceSingle = 0
        End If
        dblPrice = dblPrice + dblCurTime * Format(rsPrice!时价, gstrFeePrecisionFmt)
        dblAllTime = dblAllTime - dblCurTime
        rsPrice.MoveNext
    Next
    
    Get时价药品应收金额 = Format(dblPrice, strDec)
End Function
...
Public Function BillExistInsure(ByVal strNO As String, Optional ByVal strTime As String, _
    Optional ByVal blnAuditing As Boolean, Optional ByVal bytFlag As Byte = 2, _
...
Public Function Check发生时间(ByVal varDate As Date, ByVal varPaitOrNO As Variant, Optional ByVal lngPage As Long) As String
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能：检查发生时间是否合法
    '参数：varDate=发生时间
    ...
    Else
        ...
        " Where A.病人ID=B.病人ID And A.主页ID=B.主页ID And A.病人ID=[2] " & IIf(lngPage = 0, "", " And A.主页ID = [3]")
    End If
    
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, App.ProductName, CStr(varPaitOrNO), Val(varPaitOrNO), lngPage)
    For i = 1 To rsTmp.RecordCount
...
Public Function zlGetSpecialItemFee(strClass As String, Optional ByVal strPriceGrade As String) As Recordset
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    strSQL = _
        ...
        strWherePriceGrade
    Set zlGetSpecialItemFee = zlDatabase.OpenSQLRecord(strSQL, "获取特定项目的费用集", strClass, strPriceGrade)
...
Public Function zlAddThreeSwapSQLToCollection(ByVal bln预交款 As Boolean, _
    ...
    Optional ByVal lng预交ID As Long) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:保存三方结算数据
    '入参: bln预交款-是否预交款
    '       lngID-如果是预交款,则是预交ID,否则结帐ID
        ...
        If Trim(varData(i)) <> "" Then
            ...
            If varTemp(0) <> "" Then
                ...
                If zlCommFun.ActualLen(str交易信息 & "||" & strTemp) > 2000 Then
                    ...
                    '预交款缴款_In Number := 0
                    strSQL = strSQL & IIf(bln预交款, "1", "0") & ",Null,"
...
errH:
        ...
        Call SaveErrLog
End Function
Public Function zlGetSquareTypeRecord() As ADODB.Recordset
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取消费卡类别目录集
    '返回:成功,返回类别集，失败返回Nothing
    '编制:刘兴洪
    '日期:2017-11-15 16:33:18
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim strSQL As String
    
    On Error GoTo errHandle
    If Not grsSquareType Is Nothing Then
        If grsSquareType.State = 1 Then
            Set zlGetSquareTypeRecord = grsSquareType
            Exit Function
        End If
    End If
    strSQL = "" & _
    " Select 编号, 名称, 系统, 结算方式, 部件, 启用, 自制卡, 前缀文本, 卡号长度, 是否密文, 是否退现,  " & _
    "       是否全退, 密码长度, 密码长度限制, 密码规则, 读卡性质, 键盘控制方式, 限制类别, 是否严格控制, " & _
    "        是否特定病人, 是否允许换卡, 是否允许补卡, 是否允许余额退款, 应用场合 " & _
    " From 消费卡类别目录"
    Set grsSquareType = zlDatabase.OpenSQLRecord(strSQL, "获取消费卡类别目录")
    Set zlGetSquareTypeRecord = grsSquareType
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
        Resume
    End If
    Set zlGetSquareTypeRecord = Nothing
End Function
Public Function zlSquareIsDelCash(ByVal str编号 As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:判断指定消费卡类别是否退现
    '入参:str编号
    '出参:
    '返回:允许退现,返回true,否则返回False
    '编制:刘兴洪
    '日期:2017-11-15 16:24:09
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim strSQL As String, rsTemp As ADODB.Recordset
    
    Set rsTemp = zlGetSquareTypeRecord
    If rsTemp Is Nothing Then Exit Function
    rsTemp.Filter = "编号=" & Val(str编号)
    If Not rsTemp.EOF Then zlSquareIsDelCash = Val(Nvl(rsTemp!是否退现)) = 1
    rsTemp.Filter = 0
    zlSquareIsDelCash = True
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
        Resume
...
Public Function GetRemainderMoney(ByVal lng病人ID As Long, Optional ByVal byt类型 As Byte) As Double
    '获取病人费用余额
    '入参:
    ...
    GetRemainderMoney = Val(Nvl(rsTemp!费用余额))
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
        Resume
