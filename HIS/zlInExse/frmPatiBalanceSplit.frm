...
Begin VB.Form frmPatiBalanceSplit 
   ...
   Begin VB.PictureBox picBalanceBack 
      ...
      Begin VB.PictureBox picCurBalance 
         ...
         Top             =   1890
         Width           =   4170
         Begin zlIDKind.IDKindNew IDKind找补 
            Height          =   375
            Left            =   15
            TabIndex        =   53
            TabStop         =   0   'False
            ...
            IDKindStr       =   "找|找补|0|0|0|0|0|;门|门诊预交|0|0|0|0|0|;住|住院预交|0|0|0|0|0|"
            CaptionAlignment=   1
            BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
               Name            =   "宋体"
               Size            =   12
   ...
   Begin VB.PictureBox picPati 
      ...
      Begin MSMask.MaskEdBox txtEnd 
         ...
         PromptChar      =   "_"
      End
      Begin zl9InExse.ComboxExpend cbo婴儿 
         Height          =   360
         Left            =   13095
         TabIndex        =   5
         Top             =   630
      ...
      Begin VB.OptionButton opt出院 
         ...
         Width           =   1275
      End
      Begin zl9InExse.txtEdit txt预交余额 
         Height          =   360
         Left            =   4110
         TabIndex        =   98
         Top             =   585
         ...
         text            =   ""
      End
      Begin zl9InExse.txtEdit txt未结费用 
         Height          =   360
         Left            =   6690
         TabIndex        =   99
         Top             =   585
         ...
         text            =   ""
      End
      Begin zl9InExse.txtEdit txt剩余款额 
         Height          =   360
         Left            =   9270
         TabIndex        =   101
         Top             =   585
...
Public Function ShowMe(ByVal frmMain As Object, ByVal EditType As gBalanceBill, _
    ...
    Optional bln门诊转住院 As Boolean, Optional strPepositDate As String, Optional blnCancel As Boolean) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:结帐程序入口
    '入参:EditType-编辑类型
    '     strPrivs-权限串
...
Private Sub zlInitModulePara()
    ...
    lbl自付合计.ForeColor = mty_ModulePara.str自付合计色
    lbl剩余自付.ForeColor = Mid(mty_ModulePara.str当前付款色, 1, InStr(mty_ModulePara.str当前付款色, "|") - 1)
    txtBalance(4).ForeColor = Mid(mty_ModulePara.str缴款色, 1, InStr(mty_ModulePara.str缴款色, "|") - 1)
    lblBalance(4).ForeColor = Mid(mty_ModulePara.str缴款色, 1, InStr(mty_ModulePara.str缴款色, "|") - 1)
...
Private Sub SetCurBalanceVisible()
    ...
    If (mEditType = g_Ed_结帐作废 Or mEditType = g_Ed_重新作废 Or chkCancel.Value = 1) Then
        If InStr(1, mstrPrivs, ";预交退现金;") > 0 Then
            'blnVisible = InStr(";" & mstrPrivsCard & ";", ";三方退款强制退现;") > 0
...
Private Function CheckIsDepositAllowCash() As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:判断是否存在三方卡不允许退现的结算信息
    '入参:
    '出参:
    '返回:允许退现，返回True,否则返Fale
    '编制:刘兴洪
    '日期:2018-02-08 15:34:32
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim lng卡类别ID As Long, i As Long
    
    On Error GoTo errHandle
    With vsDeposit
        For i = 1 To .Rows - 1
            If .ColIndex("卡类别ID") >= 0 Then
                lng卡类别ID = Val(.TextMatrix(i, .ColIndex("卡类别ID")))
                'bln消费卡 = Val(.TextMatrix(i, .ColIndex("是否消费卡"))) = 1
                If lng卡类别ID <> 0 And Val(.TextMatrix(i, .ColIndex("是否退现"))) = 0 Then Exit Function
            End If
        Next
    End With
...
Private Sub InitFace()
    ...
    'all SetPatiConsControlVisible '设置病人条件控件的显示
    'Call SetOperationCtrl(0) 'bytFun-0-结算前;1-医保虚拟结算后;2-已保存了结帐单;
...
Private Sub InitRedInvoice(Optional blnFact As Boolean = True)
    ...
    If mPatiInfor.int结帐类型 = 1 Then
        bytKind = IIf(mty_ModulePara.bytInvoiceKindMZ = 0, 13, 11)
    Else
        bytKind = IIf(mty_ModulePara.bytInvoiceKindZY = 0, 13, 11)
    End If
    
    Call mobjInvoice.zlGetInvoicePreperty(mlngModul, bytKind, lng病人ID, lng主页ID, intInsure, mobjRedProperty)
...
Private Sub RefreshRed()
    ...
    If mobjRedProperty.启用使用类别 Then Call zlCheckFactIsEnough(0, True)
End Sub
...
Private Sub InitCombox_Cons()
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    cboDept.Clear: cboDept.AddItem "R", "所有科室", True, True, True
    cboFeeItem.Clear: cboFeeItem.AddItem "R", "所有费用项目", True, True, True
    cboChargeType.Clear: cboChargeType.AddItem "R", "所有收费类别", True, True, True
    cboFeeType.Clear: cboFeeType.AddItem "R", "所有费用类型", True, True, True
    
'    cbo婴儿.Checkboxes = False
    cbo婴儿.Clear: cbo婴儿.AddItem "R", "病人及婴儿", True, True, True
    cbo婴儿.AddItem "0", "病人费用", False, True
    cboDiag.Clear
    ...
    cboPatiNums.Clear
    If mEditType = g_Ed_门诊结帐 Then
        cboPatiNums.AddItem "R", "所有门诊", True, True, True
    Else
        cboPatiNums.AddItem "R", "所有住院", True, True, True
    End If
End Sub
...
Private Sub cboChargeType_NodeCheck(ByVal Node As MSComctlLib.Node, strCaption As String)
    ...
    Dim strValue As String, blnExistOwner As Boolean '存在自费的
...
Private Function GetChargeTypeCheckDatas(Optional ByRef blnOutCurOwner As Boolean) As String
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取收费类别选中的数据
    '出参: blnOutCurOwner-当前正在是否自费结帐
    '返回:获取成功,返回选中的收费类别,格式为:'C','D',....
    '     如果本身没有自费类别,且全选时,则返回空
    '编制:刘兴洪
    '日期:2015-02-15 11:14:19
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Next
    If strValue <> "" Then strValue = Mid(strValue, 2)
    If Not blnExistOwner And blnAll Then '选中了所有收费类别
       strValue = ""
    End If
...
Private Sub cboPatiNums_NodeCheck(ByVal Node As MSComctlLib.Node, strCaption As String)
    ...
    End If
    Call LoadDefaultOutStatu(mPatiInfor.lng病人ID, intNum)
...
Private Sub SetPatiNums(ByVal intTime As Integer)
    ...
    If mEditType <> g_Ed_门诊结帐 Then
        '设置病人医保状态
        blnInsure = False
        mobjBalanceAll.rsAllTime.Filter = "主页ID=" & intTime
        mYBInFor.intInsure = 0
        If Not mobjBalanceAll.rsAllTime.EOF Then
            mYBInFor.intInsure = Val(mobjBalanceAll.rsAllTime!险类):  blnInsure = True
            mYBInFor.strBalance = ""
...
Private Sub cbo婴儿_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyReturn Then zlCommFun.PressKey vbKeyTab
End Sub
Private Sub cbo婴儿_NodeCheck(ByVal Node As MSComctlLib.Node, strCaption As String)
    If mblnConsChange Then Exit Sub
    mblnConsChange = mobjBalanceCon.strBaby <> cbo婴儿.GetNodesCheckedDatas
    Call ClearListData
End Sub
...
Private Sub cbsThis_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Edit_MoneyUseDeposit   '按结帐金额使用预交(L)
        ...
        bytSetFocus = 1
        Call RecalcDepositMoney(2, dblMoney): mbln已报价 = False: GoTo GoFullDeposit:
...
Private Sub chkCancel_Click()
    ...
    If chkCancel.Value = 1 Then
        ...
        lblCurPaymentInfor.Caption = "当前退款情况"
        txt预交余额.Visible = False
        txt未结费用.Visible = False
        txt剩余款额.Visible = False
        lbl预交余额.Visible = False
        lbl未结费用.Visible = False
        lbl剩余款额.Visible = False
    Else
        chkCancel.ForeColor = &H80000012
        lblCurPaymentInfor.Caption = "当前支付情况"
        txt预交余额.Visible = True
        txt未结费用.Visible = True
        txt剩余款额.Visible = True
        lbl预交余额.Visible = True
        lbl未结费用.Visible = True
        lbl剩余款额.Visible = True
    End If
    ...
    txtInvoice.Locked = Not blnNew Or Not (zlStr.IsHavePrivs(mstrPrivs, "修改票据号")) And gblnStrictCtrl '89302
    cboNO.Locked = blnNew
    ...
    If mty_ModulePara.bln结帐后不清信息 And mblnNotClearBill Then
        ...
        ElseIf mrsInfo.State <> 1 Then
            Call LoadBalanceBill     '其中的InitBalanceSet设置了一些控件状态
             mblnNotClearBill = False
        End If
    Else
        Call LoadBalanceBill     '其中的InitBalanceSet设置了一些控件状态
...
Private Sub chkDeposit_Click()
    ...
    Dim strTemp As String, str交易流水号 As String, str交易说明 As String, str卡号 As String
    Dim lngCardTypeID As Long, bln消费卡 As Boolean
    Dim j As Long, i As Long
    Dim dblMoney As Double, strXMLExpend As String, str操作员姓名 As String
    ...
    If chkDeposit.Value = 0 Then
        ...
        With vsDeposit
            strCardTypes = ""
            For i = 1 To .Rows - 1
                lngCardTypeID = Val(.TextMatrix(i, .ColIndex("卡类别ID"))): bln消费卡 = Val(.TextMatrix(i, .ColIndex("是否消费卡")))
                strTemp = lngCardTypeID & ":" & Val(.TextMatrix(i, .ColIndex("是否消费卡")))
                
                If InStr(strCardTypes & ",", "," & strTemp & ",") = 0 And Val(.TextMatrix(i, .ColIndex("是否退现"))) = 0 And lngCardTypeID <> 0 Then
                    
                   dblMoney = Val(.TextMatrix(i, .ColIndex("冲预交")))
                    If Not bln消费卡 Then
                        str交易流水号 = .TextMatrix(i, .ColIndex("交易流水号"))
                        str交易说明 = .TextMatrix(i, .ColIndex("交易说明"))
                        str卡号 = .TextMatrix(i, .ColIndex("卡号"))
                        If zlCallReturnCashCheckInterface(Me, mlngModul, lngCardTypeID, str卡号, "1|" & Val(.TextMatrix(i, .ColIndex("预交ID"))), dblMoney, str交易流水号, str交易说明) = False Then
                            mblnNotChange = True: chkDeposit.Value = 1: mblnNotChange = False
                            Exit Sub
                        End If
                    End If
                    For j = i + 1 To .Rows - 1
                        If strTemp = Val(.TextMatrix(j, .ColIndex("卡类别ID"))) & ":" & Val(.TextMatrix(j, .ColIndex("是否消费卡"))) Then
                            dblMoney = dblMoney + Val(.TextMatrix(j, .ColIndex("冲预交")))
                            If Not bln消费卡 Then
                                str交易流水号 = .TextMatrix(j, .ColIndex("交易流水号"))
                                str交易说明 = .TextMatrix(j, .ColIndex("交易说明"))
                                str卡号 = .TextMatrix(j, .ColIndex("卡号"))
                                
                                If zlCallReturnCashCheckInterface(Me, mlngModul, lngCardTypeID, str卡号, "1|" & Val(.TextMatrix(j, .ColIndex("预交ID"))), Val(.TextMatrix(j, .ColIndex("冲预交"))), str交易流水号, str交易说明) = False Then
                                    mblnNotChange = True: chkDeposit.Value = 1: mblnNotChange = False
                                    Exit Sub
                                End If
                            End If
                    ...
                    strMsg = strMsg & vbCrLf & .TextMatrix(i, .ColIndex("卡类别名称")) & ":" & Format(dblMoney, gstrDec) & "元"
                End If
            Next
        End With
        ...
        If strMsg <> "" And chkDeposit.Value = 0 Then
            strMsg = Mid(strMsg, 2)
            If MsgBox("以下三方帐户中不允许退现，是否强制将其退现？" & vbCrLf & strMsg, vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) <> vbYes Then
                mblnNotChange = True: chkDeposit.Value = 1: mblnNotChange = False
                 Exit Sub
            End If
            '卡不支持退现的情况
            If InStr(";" & mstrPrivsCard & ";", ";三方退款强制退现;") = 0 Then
                ...
                If str操作员姓名 = "" Then
                    ...
                    Exit Sub
                End If
                mstrForceNote = str操作员姓名 & "强制退现:" & Replace(strMsg, vbCrLf, ";")
            Else
                mstrForceNote = UserInfo.姓名 & "强制退现:" & Replace(strMsg, vbCrLf, ";")
            End If
    ...
    Call Set找补信息
    Call LoadCurOwnerPayInfor
    If txtBalance(Idx_缴款).Enabled And txtBalance(Idx_缴款).Visible Then
        txtBalance(Idx_缴款).SetFocus
        zlControl.TxtSelAll txtBalance(Idx_缴款)
    End If
...
Private Sub SetBalanceConInfor()
    ...
    Call InitPatiBalanceVariableCon
    With mobjBalanceCon
        .strTime = zlGetAllTims(cboPatiNums.GetNodesCheckedDatas())     '病人结帐次数(初始="",可以为"1,2,3...")
        .strDeptIDs = cboDept.GetNodesCheckedDatas()    '病人结帐科室ID串(初始="",可以为"0,1,2,3...",0表示开单部门ID为空)
        .strClass = cboFeeType.GetNodesCheckedDatas()   '费用类型=""-所有费用(含未设置),"'公费','比例',..."
        .strChargeType = GetChargeTypeCheckDatas(blnCurOwner)     '收费类别 '34260
        .strItem = cboFeeItem.GetNodesCheckedDatas()   '要结的收据费目
        .strBaby = cbo婴儿.GetNodesCheckedDatas()  '是否仅结算婴儿费用(0-所有费用,1-病人费用,2及以上-第mbytbaby-1个婴儿费用)
        If chkPatiType(CK_Idx_普通).Value = 1 And chkPatiType(CK_Idx_体检).Value <> 1 Then
            .bytKind = 0
        ElseIf chkPatiType(CK_Idx_普通).Value <> 1 And chkPatiType(CK_Idx_体检).Value = 1 Then
              .bytKind = 1
        Else
            .bytKind = 2  '0-仅普通费用,1-仅体检费用,2-普通费用和体检费用
        End If
        If IsDate(txtBegin.Text) Then   '病人结帐的开始时间
            .dtBeginDate = Format(CDate(txtBegin.Text & " 00:00:00"), "yyyy-mm-dd HH:MM:SS")
        End If
        If IsDate(txtEnd.Text) Then   '病人结帐的结束时间
...
Private Function CheckInputValied() As Boolean
    On Error GoTo errHandle
    If zlCommFun.ActualLen(txtBalance(Idx_摘要).Text) > 50 Then
        MsgBox "输入的摘要不能超过25个汉字或50个字符,请检查!", vbInformation + vbOKOnly, gstrSysName
        If txtBalance(Idx_摘要).Enabled And txtBalance(Idx_摘要).Visible Then txtBalance(Idx_摘要).SetFocus
        Exit Function
    End If
    
    If zlCommFun.ActualLen(txtBalance(Idx_结算号码).Text) > 30 Then
        MsgBox "输入的结算号码不能超过15个汉字或30个字符,请检查!", vbInformation + vbOKOnly, gstrSysName
        If txtBalance(Idx_结算号码).Enabled And txtBalance(Idx_结算号码).Visible Then txtBalance(Idx_结算号码).SetFocus
        Exit Function
    End If
...
Private Function SaveBalanceData() As Boolean
    ...
    If blnHaveFee = False And mEditType <> g_Ed_重新结帐 Then
        MsgBox "病人不存在需要结帐的费用,请点击『刷新未结费用』", vbInformation + vbOKOnly, gstrSysName
    ...
    If Not objCard Is Nothing Then
        If objCard.结算性质 = 1 And mty_ModulePara.byt缴款输入控制 = 1 Then
            If (Val(lbl剩余自付.Caption) <> 0 And Val(txtBalance(4).Text) = 0) Then
                If Not (InStr(IDKind找补.GetCurCard.名称, "预交") > 0 And Val(lbl剩余自付.Caption) - Val(txtBalance(5).Text) = 0) Then
                    MsgBox "未输入缴款金额，不能使用" & objCard.结算方式 & "进行结算！", vbInformation + vbOKOnly, gstrSysName
    ...
    If mBalanceInfor.bln预交验证 = False And Val(txtBalance(Idx_冲预交).Text) <> 0 Then
        If DepositMonyVerfy(True) = False Then Exit Function
       ' MsgBox "预交金额未验证,请重新输入缴款金额!", vbInformation + vbOKOnly, gstrSysName
'        blnCancel = False
'        Call txtBalance_Validate(Idx_冲预交, blnCancel)
'        If blnCancel Then Exit Function
'
'        If txtBalance(Idx_缴款).Visible And txtBalance(Idx_缴款).Enabled Then txtBalance(Idx_缴款).SetFocus
'        Exit Function
    End If
    
    If mBalanceInfor.bln预交刷卡 = False And Val(txtBalance(Idx_冲预交).Text) <> 0 And (mEditType = g_Ed_门诊结帐 Or mblnCurMzBalanceNo) Then
        If Not zlDatabase.PatiIdentify(Me, glngSys, mPatiInfor.lng病人ID, Val(txtBalance(Idx_冲预交).Text), _
                , , , IIf(-1 * gdbl预存款消费验卡 >= Val(txtBalance(Idx_冲预交).Text), False, True), , , (gdbl预存款消费验卡 <> 0), (gdbl预存款消费验卡 = 2)) Then
            Exit Function
        Else
    ...
    LockedScreen True
    blnSaved = SaveBalaceCharge(bln预交, tyBrushCard, blnChargeEnd, objSetFocus)
    LockedScreen False
    ...
    If blnChargeEnd Then
        ...
        If mbln连续结帐 Or mobjBalanceCon.blnCurBalanceOwnerFee Then
            ...
            If mobjBalanceCon.blnCurBalanceOwnerFee Then
                ...
                If (12 - Len(Format(mBalanceInfor.dbl当前结帐, "0.00"))) / 2 < 0 Then
                    lblPrevious.Caption = "上次自费结帐" & vbCrLf & Format(mBalanceInfor.dbl当前结帐, "0.00")
                Else
                    lblPrevious.Caption = "上次自费结帐" & vbCrLf & String((12 - Len(Format(mBalanceInfor.dbl当前结帐, "0.00"))) / 2, " ") & Format(mBalanceInfor.dbl当前结帐, "0.00")
                End If
    ...
    ElseIf blnSaved Then
        If bln预交 And txtBalance(Idx_冲预交).Enabled And txtBalance(Idx_冲预交).Visible Then
            ...
            zlControl.TxtSelAll txtBalance(Idx_冲预交)
        ElseIf txtBalance(Idx_缴款).Enabled And txtBalance(Idx_缴款).Visible Then
            txtBalance(Idx_缴款).SetFocus
            zlControl.TxtSelAll txtBalance(Idx_缴款)
        End If
...
'    txtBalance(Idx_缴款).Enabled = blnState
    txtBalance(Idx_找补).Enabled = blnState
    txtBalance(Idx_摘要).Enabled = blnState
    txtBalance(Idx_结算号码).Enabled = blnState
    txtBalance(Idx_结帐说明).Enabled = blnState
    txtBalance(Idx_冲预交).Enabled = blnState
    txtBalance(Idx_本次结帐).Enabled = blnState And InStr(mstrPrivs, ";结帐设置;") > 0
    ...
    cbo婴儿.Enabled = blnState And InStr(mstrPrivs, ";结帐设置;") > 0
    
    cboPatiNums.Enabled = blnState And InStr(mstrPrivs, ";结帐设置;") > 0
    cboFeeType.Enabled = blnState And InStr(mstrPrivs, ";结帐设置;") > 0
    cboFeeItem.Enabled = blnState And InStr(mstrPrivs, ";结帐设置;") > 0
    cboDept.Enabled = blnState And InStr(mstrPrivs, ";结帐设置;") > 0
    cboChargeType.Enabled = blnState And InStr(mstrPrivs, ";结帐设置;") > 0
    cboDiag.Enabled = blnState And InStr(mstrPrivs, ";结帐设置;") > 0
    
    
    cbo婴儿.BackColor = IIf(cbo婴儿.Enabled, &H80000005, &H8000000F)
    ...
    If blnState Then
        txtBalance(Idx_找补).BackColor = &H80000005
        txtBalance(Idx_摘要).BackColor = &H80000005
        txtBalance(Idx_结算号码).BackColor = &H80000005
        txtBalance(Idx_结帐说明).BackColor = &H80000005
        txtBalance(Idx_冲预交).BackColor = &H80000005
    Else
        txtBalance(Idx_找补).BackColor = &H8000000F
        txtBalance(Idx_摘要).BackColor = &H8000000F
        txtBalance(Idx_结算号码).BackColor = &H8000000F
...
Private Sub cmdRefresh_Click()
    ...
    If blnNotPati Then
        MsgBox "没有确定结帐病人,不能刷新未结费用信息！", vbExclamation, gstrSysName
        If txtPatient.Visible And txtPatient.Enabled Then txtPatient.SetFocus
        Exit Sub
    End If
    
    If mEditType = g_Ed_门诊结帐 Then
        If chkPatiType(CK_Idx_普通).Value = 0 And chkPatiType(CK_Idx_体检).Value = 0 Then
            MsgBox "普通费用和体检费用需要至少选择一项,不能刷新未结费用信息！", vbExclamation, gstrSysName
            Exit Sub
        End If
    End If
    ...
    If cboPatiNums.Text = "" Or cboDept.Text = "" Or cboFeeType.Text = "" Or cboFeeItem.Text = "" Or cboChargeType.Text = "" Then
        MsgBox "存在未设置的结帐条件，无法读取结帐信息！", vbExclamation, gstrSysName
        Exit Sub
    End If
    ...
    If mEditType <> g_Ed_门诊结帐 Then
        With mobjBalanceCon
            ...
            Set objCard = IDKIND.GetIDKindCard("姓名", CardTypeName)
        ...
        Call CheckPatiFromZyNumIsYB(mrsInfo!病人ID, intMaxTime, intInsure, strInsureName)
...
Private Sub cmdYBBalance_Click()
    ...
    '加载结帐信息
    lng病人ID = Val(NVL(mrsInfo!病人ID))
    Call LoadBalancePayData(lng病人ID, mBalanceInfor.lng结帐ID, , , True)
'    Call RecalcDepositMoney(1)  '重新按缺省计算预交
    ...
    Call LoadCurOwnerPayInfor '加载当前支付信息
...
'        If txtPatient.Enabled And txtPatient.Visible Then txtPatient.SetFocus
    ...
    If mEditType = g_Ed_门诊结帐 Or mEditType = g_Ed_住院结帐 Then
        If txtPatient.Enabled And txtPatient.Visible Then txtPatient.SetFocus
    End If
    '111222,焦博,2017/08/03,登记了新生儿的病人显示cbo婴儿
    Call picPati_Resize
End Sub
...
GoLocal:
    ...
    blnEnabled = txtBalance(Idx_缴款).Enabled '避免在输入病人后，光标移动造成语音报价
    
    txtBalance(Idx_缴款).Enabled = False
    ...
    Next
    txtBalance(Idx_缴款).Enabled = blnEnabled
    Exit Sub
errHandle:
    If ErrCenter() = 1 Then
        Resume
...
Private Sub IDKindPaymentsType_ItemClick(Index As Integer, objCard As zlIDKind.Card)
   ...
   If txtBalance(Idx_缴款).Enabled And txtBalance(Idx_缴款).Visible _
        And picCurBalance.Enabled And picBalanceBack.Enabled Then txtBalance(Idx_缴款).SetFocus
    mblnNotChange = True
    Call LoadCurOwnerPayInfor
    mblnNotChange = False
End Sub
Private Sub LoadDefaultMoney()
    ...
    On Error GoTo errHandle
    txtBalance(Idx_缴款).Text = "": txtBalance(Idx_缴款).Locked = False
    Set objCard = IDKindPaymentsType.GetCurCard
    If objCard Is Nothing Then Exit Sub
...
'                     txtBalance(Idx_缴款).Text = -1 * Format(GetOldBalanceMoney(3, objCard), "0.00")
'                 Else
'                     txtBalance(Idx_缴款).Text = -1 * Format(GetOldBalanceMoney(5, objCard), "0.00")
'                 End If
'            Else
                 If objCard.结算性质 <> 1 Then
                    txtBalance(Idx_缴款).Text = Format(Val(lbl剩余自付.Caption), "0.00")
                 End If
'            End If
       End If
       Exit Sub
    ElseIf mEditType = g_Ed_门诊结帐 Or mEditType = g_Ed_住院结帐 Or mEditType = g_Ed_重新结帐 Then
        If objCard.结算性质 = 2 Then
        ...
        End If
        If objCard.接口序号 > 0 And Not objCard.消费卡 Then '86853
            If mty_ModulePara.byt刷卡缺省金额操作 <> 0 Then
                txtBalance(Idx_缴款).Text = Format(Val(lbl剩余自付.Caption), "0.00")
                If mty_ModulePara.byt刷卡缺省金额操作 = 2 Then txtBalance(Idx_缴款).Locked = True
            End If
            Exit Sub
        End If
        If objCard.消费卡 Then
            ...
            Exit Sub
        End If
        If objCard.结算性质 = 2 And mPatiInfor.bln退款标志 Then
            txtBalance(Idx_缴款).Text = Format(Val(lbl剩余自付.Caption), "0.00")
            Exit Sub
        End If
        If objCard.结算性质 <> 1 Then
            txtBalance(Idx_缴款).Text = Format(Val(lbl剩余自付.Caption), "0.00")
        End If
        If objCard.结算性质 = 1 And mPatiInfor.bln退款标志 And mty_ModulePara.bln退款现金缺省金额 Then
            txtBalance(Idx_缴款).Text = Format(Val(lbl剩余自付.Caption), "0.00")
        End If
    End If
...
Private Sub IDKind找补_ItemClick(Index As Integer, objCard As zlIDKind.Card)
    Dim dblTemp As Double
    If mblnNotChange Then Exit Sub
    mblnNotChange = True
    If objCard.接口序号 = 1 Then
        Call Set找补信息
        Call LoadCurOwnerPayInfor
    Else
        dblTemp = RoundEx(Abs(mBalanceInfor.dbl未付合计 - mBalanceInfor.dbl冲预交合计), 5)
        dblTemp = GetCentMoney(dblTemp)
        If RoundEx(Val(txtBalance(Idx_缴款).Text) - dblTemp - mPatiInfor.dbl未付累计, 6) >= 0 Then
            dblTemp = RoundEx(Val(txtBalance(Idx_缴款).Text) - dblTemp - mPatiInfor.dbl未付累计, 6)
            txtBalance(Idx_找补).Text = Format(dblTemp, "0.00")
        Else
            txtBalance(Idx_缴款).Text = "0.00"
            txtBalance(Idx_找补).Text = Format(dblTemp, "0.00")
        End If
...
Private Sub IDKind找补_KeyPress(KeyAscii As Integer)
    Call MoveIDKindItem(IDKindPaymentsType, KeyAscii)
End Sub
...
Private Sub picBalanceBack_Resize()
    ...
    With picBalanceBack
        ...
        lbl误差额.Top = lblCurPaymentInfor.Top
        If mEditType <> g_Ed_单据查看 Then
            Call ShowYBMoneyOrInvioceFormatInfor
        End If
        lbl误差额.Left = lblCurPaymentInfor.Left + lblCurPaymentInfor.Width + 100
    End With
...
Private Sub ShowYBMoneyOrInvioceFormatInfor()
    ...
    With picBalanceBack
        ...
        lbl医保基金.Top = .ScaleHeight - lbl医保基金.Height - 100
        lbl医保基金.Left = picOwerFee.Left
        lblBalance(7).Top = lbl医保基金.Top
        txtDate.Top = lblBalance(7).Top - 60
'        lbl个人帐户.Top = lbl医保基金.Top
'        lbl个人帐户.Left = lbl医保基金.Left + lbl医保基金.Width + 1000
        lbl误差额.Top = lblCurPaymentInfor.Top
        
'        lblFormat.Top = lbl医保基金.Top
        vsBlance.Height = lbl医保基金.Top - vsBlance.Top - lngStep
    End With
End Sub
Private Sub picCurDeposit_Resize()
    Err = 0: On Error Resume Next
    With picCurDeposit
        txtBalance(Idx_冲预交).Width = .ScaleWidth - txtBalance(Idx_冲预交).Left - 100
    End With
...
Private Sub picBalanceInfor_Resize()
    ...
    With picBalanceInfor
        ...
        txtBalance(Idx_本次未结).Width = txtBalance(Idx_本次结帐).Width
    End With
...
Private Sub picCurBalance_Resize()
    Err = 0: On Error Resume Next
    With picCurBalance
        txtBalance(Idx_缴款).Width = .ScaleWidth - txtBalance(Idx_缴款).Left - 100
        txtBalance(Idx_找补).Width = .ScaleWidth - txtBalance(Idx_找补).Left - 100
        txtBalance(Idx_结算号码).Width = .ScaleWidth - txtBalance(Idx_结算号码).Left - 100
        txtBalance(Idx_摘要).Width = .ScaleWidth - txtBalance(Idx_摘要).Left - 100
    End With
...
Private Function InitGrid() As Boolean
    ...
    With vsDeposit
        ...
        For i = 0 To .Cols - 1
            ...
            If InStr(",是否消费卡,是否退现,是否全退,是否缺省退现,是否转帐及代扣,编辑状态,", "," & .ColKey(i) & ",") > 0 Or .ColKey(i) Like "*ID" Then
...
Private Sub InitTride_FeeList()
    ...
    With vsFeeList
        ...
        .AutoSize 0, .Cols - 1
        zl_vsGrid_Para_Restore mlngModul, vsFeeList, Me.Name, "费用列表" & mEditType
    End With
    Call SetFeeListColumnShow
    With vsDetailList
        .Clear
        ...
        .ColWidth(.ColIndex("结帐金额")) = 1400
    End With
    zl_vsGrid_Para_Restore mlngModul, vsDetailList, Me.Name, "明细列表" & mEditType
    Exit Sub
errHandle:
    If ErrCenter() = 1 Then
        Resume
...
Private Sub picPati_Resize()
    ...
    With picPati
        ...
        cbo婴儿.Left = cboPatiNums.Left + cboPatiNums.Width - cbo婴儿.Width
        If cbo婴儿.Visible Then
            cboPatiNums.Width = cbo婴儿.Left - cboPatiNums.Left - 30
        End If
...
'
'    lne费别.Y1 = txt费别.Top + txt费别.Height
'    lne费别.Y2 = txt费别.Top + txt费别.Height
'    lne费别.X1 = txt费别.Left
'    lne费别.X2 = txt费别.Left + txt费别.Width
'
'    lne标识号.Y1 = txt标识号.Top + txt标识号.Height
'    lne标识号.Y2 = txt标识号.Top + txt标识号.Height
'    lne标识号.X1 = txt标识号.Left
'    lne标识号.X2 = txt标识号.Left + txt标识号.Width
'
...
'
'    lne科室.Y1 = txt科室.Top + txt科室.Height
'    lne科室.Y2 = txt科室.Top + txt科室.Height
'    lne科室.X1 = txt科室.Left
'    lne科室.X2 = txt科室.Left + txt科室.Width
'
...
'
'    lne预交余额.Y1 = txt预交余额.Top + txt预交余额.Height
'    lne预交余额.Y2 = txt预交余额.Top + txt预交余额.Height
'    lne预交余额.X1 = txt预交余额.Left
'    lne预交余额.X2 = txt预交余额.Left + txt预交余额.Width
'
'    lne未结费用.Y1 = txt未结费用.Top + txt未结费用.Height
'    lne未结费用.Y2 = txt未结费用.Top + txt未结费用.Height
'    lne未结费用.X1 = txt未结费用.Left
'    lne未结费用.X2 = txt未结费用.Left + txt未结费用.Width
'
'    lne剩余款额.Y1 = txt剩余款额.Top + txt剩余款额.Height
'    lne剩余款额.Y2 = txt剩余款额.Top + txt剩余款额.Height
'    lne剩余款额.X1 = txt剩余款额.Left
'    lne剩余款额.X2 = txt剩余款额.Left + txt剩余款额.Width
'End Sub
...
Private Sub Set找补信息()
    ...
    Set objCard = IDKindPaymentsType.GetCurCard
    Set objBackCard = IDKind找补.GetCurCard
    With mBalanceInfor
        dbl当前未付 = RoundEx(.dbl当前结帐 - .dbl冲预交合计 - .dbl已付合计, 6)
    End With
    
    If mEditType = g_Ed_重新作废 Or mEditType = g_Ed_结帐作废 Or chkCancel.Value = 1 Then
        dbl当前未付 = -1 * dbl当前未付
    End If
    
    dbl当前未付 = RoundEx(dbl当前未付 + mPatiInfor.dbl未付累计, 6)
    ...
    If dblMoney = 0 Or objCard Is Nothing Then
        ...
        Else
            Call Load找补项(1, "应收款 ")
        End If
        txtBalance(Idx_找补).ForeColor = txtBalance(Idx_结帐说明).ForeColor
        If Not objBackCard Is Nothing Then
            If Not objBackCard.接口序号 <> 1 Then
    ...
    If dbl当前未付 < 0 Then
        '当前状态为退款
        dbl当前未付 = RoundEx(Val(lbl剩余自付.Caption), 6)
'        IDKind找补.IDKind = 0   '证明是按某种方式退款,因此不能存为预交款
        If objCard.结算性质 = 1 Then
...
'            MsgBox "当前退款项(" & objCard.名称 & ")不支持多退,请重新输入!", vbInformation, gstrSysName
'            mblnNotChange = True
'            txtBalance(Idx_缴款).Text = ""
'            mblnNotChange = False
            txtBalance(Idx_找补).Text = Format(Abs(dblMoney - Abs(dbl当前未付)), "0.00")
...
Private Sub txtBalance_Change(Index As Integer)
    ...
    Select Case Index
    Case Idx_缴款
        If Not IDKind找补.GetCurCard Is Nothing Then
           If IDKind找补.GetCurCard.接口序号 <> 1 Then
               txtBalance(Idx_找补).Text = "0.00"
        ...
        If mty_ModulePara.byt刷卡缺省金额操作 = 2 Then
            Dim objCard As Card
            Set objCard = IDKindPaymentsType.GetCurCard
            txtBalance(Idx_缴款).Locked = objCard.接口序号 > 0 And Not objCard.消费卡
        End If
    Case Idx_冲预交
...
Private Sub LedVoiceSpeak(ByVal blnGotFocus As Boolean)
    ...
    dbl找补 = Val(txtBalance(Idx_找补).Text)
    If intMark = 1 Then
        dbl找补 = Val(IIf(Replace(IDKind找补.GetCurCard.名称, " ", "") = "应退款", Val(txtBalance(Idx_找补).Text), 0))
        zl9LedVoice.DispCharge Format(curTotal, "0.00"), Val(txtBalance(Idx_缴款).Text), dbl找补
        zl9LedVoice.Speak "#22 " & Val(txtBalance(Idx_缴款).Text)
        zl9LedVoice.Speak "#23 " & dbl找补
        zl9LedVoice.Speak "#3"   '#3  --请当面点清, 谢谢!
    Else
        If Val(txtBalance(Idx_缴款).Text) > Val(curTotal) Then
            zl9LedVoice.DispCharge Format(intMark * curTotal, "0.00"), Val(txtBalance(Idx_找补).Text), Val(txtBalance(Idx_缴款).Text)
            zl9LedVoice.Speak "#22 " & dbl找补
            zl9LedVoice.Speak "#23 " & Val(txtBalance(Idx_缴款).Text)
            zl9LedVoice.Speak "#3"   '#3  --请当面点清, 谢谢!
        Else
        
            zl9LedVoice.DispCharge Format(intMark * curTotal, "0.00"), 0, Val(txtBalance(Idx_缴款).Text) + dbl找补
            zl9LedVoice.Speak "#22 " & 0 'intMark * Val(txtBalance(Idx_缴款).Text)
            zl9LedVoice.Speak "#23 " & Val(txtBalance(Idx_缴款).Text) + dbl找补
            zl9LedVoice.Speak "#3"   '#3  --请当面点清, 谢谢!
        End If
    End If
End Sub
...
Private Sub txtBalance_KeyPress(Index As Integer, KeyAscii As Integer)
    ...
    If KeyAscii <> 13 Then
        If mPatiInfor.dbl未付累计 <> 0 Then Exit Sub
        If Index = Idx_缴款 Then
             Set objKind = IDKindPaymentsType
        ElseIf Idx_找补 = Index Then
             Set objKind = IDKind找补
        Else
             Exit Sub
        End If
    ...
    Case Idx_缴款
        ...
        Case 2   '按病人缴款累计
            ...
            ElseIf objCard.接口序号 > 0 Then
                If dblMoney = 0 And Val(lbl剩余自付.Caption) <> 0 Then
        ...
        Case Else '0-不进行缴款控制,1-输入现金时,必须输入缴款金额
            If mty_ModulePara.byt缴款输入控制 = 1 And objCard.结算性质 = 1 And (Val(lbl剩余自付.Caption) <> 0 And Val(txtBalance(4).Text) = 0) Then
                MsgBox "未输入缴款金额,不能使用『" & objCard.结算方式 & "』进行结算", vbInformation, gstrSysName
                Exit Sub
            End If
        ...
        If objCard.接口序号 > 0 Then
            If dblMoney = 0 And RoundEx(mBalanceInfor.dbl未付合计 - mBalanceInfor.dbl冲预交合计, 5) <> 0 Then
                MsgBox "未输入缴款金额,不能使用『" & objCard.结算方式 & "』进行结算", vbInformation, gstrSysName
                Exit Sub
            End If
            If SaveBalanceData = False Then Exit Sub
    ...
    Case Idx_冲预交
        ...
        If (mEditType = g_Ed_门诊结帐 Or mblnCurMzBalanceNo) Then
            If zlDatabase.PatiIdentify(Me, glngSys, mPatiInfor.lng病人ID, dblMoney, _
                    , , , IIf(-1 * gdbl预存款消费验卡 >= dblMoney, False, True), , , (gdbl预存款消费验卡 <> 0), (gdbl预存款消费验卡 = 2)) Then
                mBalanceInfor.bln预交刷卡 = True
            End If
        End If
    Case Else
...
Private Sub txtBalance_Validate(Index As Integer, Cancel As Boolean)
    ...
    Case Idx_本次结帐
        ...
        If Val(txtBalance(Idx_本次结帐).Text) > Val(txtBalance(Idx_本次未结).Text) Then
        ...
        If RoundEx(Val(txtBalance(Idx_本次结帐).Text), 6) < 0 Then
            MsgBox "当前结帐金额小于零,不允许结帐!", vbInformation + vbOKOnly, gstrSysName
            zlControl.TxtSelAll txtBalance(Index)
            Cancel = True: Exit Sub
        End If
...
Private Sub txtBegin_Validate(Cancel As Boolean)
    If txtBegin.Text <> "____-__-__" And Not IsDate(txtBegin.Text) Then
        MsgBox "请输入正确的日期！", vbInformation, gstrSysName
        Cancel = True
    Else
...
Private Sub vsBlance_AfterEdit(ByVal Row As Long, ByVal Col As Long)
    ...
    With vsBlance
        ...
        Case .ColIndex("结算方式")
        Case .ColIndex("结算金额")
            If .RowData(Row) = 110 Then mbln已报价 = False: Exit Sub
            mBalanceInfor.dbl医保支付合计 = RoundEx(GetYBTotal(0, dbl个人帐户, dbl医保基金), 5)
            mBalanceInfor.dbl已付合计 = mBalanceInfor.dbl医保支付合计
            mBalanceInfor.dbl未付合计 = RoundEx(mBalanceInfor.dbl当前结帐 - mBalanceInfor.dbl已付合计, 5)
            Call LoadCurOwnerPayInfor
            mbln已报价 = False
        Case Else
...
Private Sub vsBlance_BeforeEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    With vsBlance
        If Val(.TextMatrix(Row, .ColIndex("结算状态"))) = 1 Then
            Cancel = True: Exit Sub
        End If
        .ComboList = ""
        ...
        Case 1
            If Col <> .ColIndex("结算金额") And Col <> .ColIndex("结算号码") Then Cancel = True: Exit Sub
        Case 2
            If Col = .ColIndex("结算方式") And .RowData(Row) <> 110 Then
                 .ComboList = "..."
...
Private Sub DeletePayInfor(ByVal lngDelRow As Long, Optional ByVal blnForceDel As Boolean = False)
    ...
    On Error GoTo errHandle
    With vsBlance
        If Val(.TextMatrix(lngDelRow, .ColIndex("类型"))) = 3 Then
        ...
        If Val(.TextMatrix(lngDelRow, .ColIndex("是否退现"))) = 0 And Val(.TextMatrix(lngDelRow, .ColIndex("类型"))) = 3 Then
            '卡不支持退现的情况
            If InStr(";" & mstrPrivsCard & ";", ";三方退款强制退现;") = 0 Then
                If mstrForceNote = "" Then
                    ...
                    mstrForceNote = str操作员姓名 & "强制退现:" & .TextMatrix(lngDelRow, .ColIndex("卡类别名称")) & Format(Abs(Val(.TextMatrix(lngDelRow, .ColIndex("结算金额")))), gstrDec) & "元" & ";"
                Else
                    mstrForceNote = mstrForceNote & .TextMatrix(lngDelRow, .ColIndex("卡类别名称")) & Format(Abs(Val(.TextMatrix(lngDelRow, .ColIndex("结算金额")))), gstrDec) & "元" & ";"
                End If
            Else
                ...
                mstrForceNote = mstrForceNote & IIf(mstrForceNote = "", UserInfo.姓名 & "强制退现:", ";") & .TextMatrix(lngDelRow, .ColIndex("卡类别名称")) & Format(Abs(Val(.TextMatrix(lngDelRow, .ColIndex("结算金额")))), gstrDec) & "元"
            End If
        End If
    
        dblMoney = Val(.TextMatrix(lngDelRow, .ColIndex("结算金额")))
        '编辑状态:0-禁止删除;1-允许编辑金额;2-允许删除
        If Val(.TextMatrix(lngDelRow, .ColIndex("编辑状态"))) <> 2 And blnForceDel = False Then Exit Sub
...
Private Sub vsBlance_ValidateEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    With vsBlance
        ...
        If vsBlance.RowData(Row) = 110 Then
            ...
            .EditText = strKey
            dblMoney = RoundEx(Val(strKey) - dbl原始金额, 6)
            mBalanceInfor.dbl已付合计 = RoundEx(mBalanceInfor.dbl已付合计 + dblMoney, 6)
            mBalanceInfor.dbl未付合计 = RoundEx(mBalanceInfor.dbl未付合计 - dblMoney, 6)
'            Call LoadIntendBalance
            Call LoadDefaultMoney
...
'            Call Set找补信息
        ...
        Else
            ...
            Case 3 '个人帐户
                If Val(strKey) > dbl原始金额 And Val(strKey) <> 0 And dbl原始金额 <> 0 Then
                ...
                End If
                 '不允许超过允许透支金额
                If mYBInFor.cur个帐余额 + mYBInFor.cur个帐透支 - Val(strKey) < 0 Then
            ...
            '不允许超出单据剩余可结算金额
            dblTotal = RoundEx(mBalanceInfor.dbl当前结帐 - GetYBTotal(.Row) - mBalanceInfor.dbl已付合计, 5)
...
'    lbl个人帐户.Caption = "帐户余额:" & Format(mYBInFor.cur个帐余额, "0.00")
    staThis.Panels(5).Text = Format(mYBInFor.cur个帐余额, "0.00")
    staThis.Panels(5).Visible = True
    lbl医保基金.Caption = "统筹支付:" & Format(dbl医保基金, "0.00")
...
Private Sub vsDeposit_AfterEdit(ByVal Row As Long, ByVal Col As Long)
    ...
    With vsDeposit
        ...
        If Val(dbl预交余额) < Val(dbl冲预交) Then
            ...
            Exit Sub
        End If
        .TextMatrix(Row, .ColIndex("冲预交")) = Format(Val(.TextMatrix(Row, .ColIndex("冲预交"))), "0.00")
        mblnManualEdit = True
        ...
        If Not mBalanceInfor.bln预交验证 Then
...
Private Sub vsDeposit_AfterSort(ByVal Col As Long, Order As Integer)
    ...
    If mstrNoSort <> "" Then
        ...
        End With
    End If
    If mEditType = g_Ed_单据查看 Or mEditType = g_Ed_结帐作废 Or chkCancel.Value = 1 Then Exit Sub
...
Private Sub SetPatiConsControlVisible()
    ...
    cbo婴儿.Visible = Not blnMzBalance And cbo婴儿.ListCount > 2
    If cbo婴儿.Visible Then
        cboPatiNums.Width = cbo婴儿.Left - cboPatiNums.Left - 30
    End If
    ...
    lblBed.Visible = Not blnMzBalance
    chkPatiType(CK_Idx_普通).Visible = blnMzBalance
    chkPatiType(CK_Idx_体检).Visible = blnMzBalance
    ...
    If (mEditType <> g_Ed_门诊结帐 And mEditType <> g_Ed_住院结帐) _
        ...
        chkPatiType(CK_Idx_普通).Visible = False
        chkPatiType(CK_Idx_体检).Visible = False
...
Private Sub MovePatiConsControl()
    ...
    If mEditType = g_Ed_住院结帐 And chkCancel.Value <> 1 Then
        ...
        txt天数.Top = txtBegin.Top
        opt出院.Top = txtBegin.Top + (txtBegin.Height - opt出院.Height) \ 2
        opt中途.Top = opt出院.Top
        ...
        txt天数.Left = txtPatiEnd.Left + txtPatiEnd.Width + 20
        lblDayName.Left = txt天数.Left + txt天数.Width + 20
        Exit Sub
    End If
    ...
    If mEditType = g_Ed_门诊结帐 And chkCancel.Value <> 1 Then
        ...
        lbl费别.Left = txtOld.Left + txtOld.Width + lngStep
        txt费别.Left = lbl费别.Left + lbl费别.Width + 10
            
        lbl标识号.Left = txt费别.Left + txt费别.Width + lngStep
        txt标识号.Left = lbl标识号.Left + lbl标识号.Width + 10
...
'        txtEnd.Left = lblFsTimeRange.Left + lblFsTimeRange.Width + 10
        ...
        chkPatiType(CK_Idx_普通).Top = lblFsTime.Top
        chkPatiType(CK_Idx_体检).Top = chkPatiType(CK_Idx_普通).Top
        Exit Sub
    End If
    ...
    lblBed.Left = txt标识号.Left + txt标识号.Width + lngStep
    txtBed.Left = lblBed.Left + lblBed.Width + 10
        
    lbl科室.Left = txtBed.Left + txtBed.Width + lngStep
    txt科室.Left = lbl科室.Left + lbl科室.Width + 10
         
         
    txtBegin.Top = cbo婴儿.Top
    txtEnd.Top = txtBegin.Top
    txtPatiBegin.Top = txtBegin.Top
    txtPatiEnd.Top = txtBegin.Top
    txt天数.Top = txtBegin.Top
...
Private Sub SetControlEnabled(blnEanbled As Boolean)
    ...
    Case g_Ed_门诊结帐
        ...
        cboDept.Enabled = blnEanbled And InStr(mstrPrivs, ";结帐设置;") > 0
        cboFeeType.Enabled = blnEanbled And InStr(mstrPrivs, ";结帐设置;") > 0
        cboFeeItem.Enabled = blnEanbled And InStr(mstrPrivs, ";结帐设置;") > 0
        cboChargeType.Enabled = blnEanbled And InStr(mstrPrivs, ";结帐设置;") > 0
        txtInvoice.Enabled = blnEanbled
        cboPatiNums.Enabled = blnEanbled And InStr(mstrPrivs, ";结帐设置;") > 0
        cboDiag.Enabled = blnEanbled And InStr(mstrPrivs, ";结帐设置;") > 0
    ...
    Case g_Ed_住院结帐
        ...
        cbo婴儿.Enabled = blnEanbled And InStr(mstrPrivs, ";结帐设置;") > 0
        cboDept.Enabled = blnEanbled And InStr(mstrPrivs, ";结帐设置;") > 0
        cboFeeType.Enabled = blnEanbled And InStr(mstrPrivs, ";结帐设置;") > 0
        cboFeeItem.Enabled = blnEanbled And InStr(mstrPrivs, ";结帐设置;") > 0
        cboChargeType.Enabled = blnEanbled And InStr(mstrPrivs, ";结帐设置;") > 0
...
'            If InStr("," & mobjBalanceCon.strTime & ",", "," & mrsInfo!主页ID & ",") = 0 Then Call cmdRefresh_Click
'        End If
'    End If
End Sub
...
Private Function GetPatient(ByVal objCard As Card, ByVal strInput As String, _
    ...
    Dim lng病人ID As Long, blnHavePassWord As Boolean, rsIn As ADODB.Recordset, strInSQL As String
    ...
    If mlngPatientID <> 0 And mblnFirst Then
        ...
        If Val(mstr主页Id) = 0 Then '门诊
            ...
            If mEditType = g_Ed_住院结帐 Then bytMzMode = 2    '住院的:44022
        Else    '指定次数
...
'    mYBInFor.intInsure = Val(NVL(mrsInfo!险类))
    ...
    txtPatiType.Text = NVL(mrsInfo!病人类型)
    ...
    ElseIf txtBalance(Idx_冲预交).Enabled And txtBalance(Idx_冲预交).Visible Then
        txtBalance(Idx_冲预交).SetFocus
    ElseIf txtBalance(Idx_缴款).Enabled And txtBalance(Idx_缴款).Visible Then
       If mrsInfo Is Nothing Then
         If txtPatient.Enabled And txtPatient.Visible Then txtPatient.SetFocus
...
Private Function LoadDefaultOutStatu(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:加载默认的出院状态
    '编制:刘兴洪
    ...
    Else
        ...
        If lng主页ID < Val(NVL(mrsInfo!主页ID)) Then
            opt出院.Value = True: LoadDefaultOutStatu = True: Exit Function
        End If
    End If
...
Private Function Load余额信息(ByVal lng病人ID As Long, ByVal byt类型 As Byte) As Boolean
    ...
    If rsTemp.RecordCount <> 0 Then
        ...
        If mEditType <> g_Ed_门诊结帐 And mEditType <> g_Ed_住院结帐 Or ((mEditType = g_Ed_门诊结帐 Or mEditType = g_Ed_住院结帐) And chkCancel.Value = 1) Then
        ...
        Else
            ...
            lbl未结费用.Visible = True
            lbl剩余款额.Visible = True
            txt预交余额.Text = Format(mPatiInfor.dbl预交余额, "0.00")
            txt未结费用.Text = Format(mPatiInfor.dbl费用余额, "0.00")
            txt剩余款额.Text = Format(mPatiInfor.dbl剩余合计, "0.00")
        End If
    Else
        If mEditType <> g_Ed_门诊结帐 And mEditType <> g_Ed_住院结帐 Then
            txt预交余额.Visible = False
            txt未结费用.Visible = False
            txt剩余款额.Visible = False
            lbl预交余额.Visible = False
...
Private Sub ClearAdjustBalance()
    ...
    With vsBlance
        ...
        Else
            For i = 1 To .Rows - 1
                If .RowData(i) <> 110 Then .TextMatrix(i, .ColIndex("结算方式")) = ""
                .TextMatrix(i, .ColIndex("结算金额")) = ""
                .TextMatrix(i, .ColIndex("结算号码")) = ""
                .TextMatrix(i, .ColIndex("结算性质")) = 2
                .TextMatrix(i, .ColIndex("结算状态")) = 0
                .TextMatrix(i, .ColIndex("编辑状态")) = 1
                .RowData(i) = 110
            Next i
        End If
        .Redraw = intRedraw
...
Private Function ShowBalance(Optional ByVal blnInputPatiAfterID As Boolean, _
    ...
    If mbln连续结帐 Then
        ...
        If mEditType = g_Ed_住院结帐 Then
            Call CheckPatiFromZyNumIsYB(Val(NVL(mrsInfo!病人ID)), Val(Split(mobjBalanceCon.strTime, ",")(0)), intInsure, strInsureName)
             mYBInFor.intInsure = intInsure
        End If
    End If
    ...
    If mEditType <> g_Ed_门诊结帐 And mobjBalanceCon.blnCurBalanceOwnerFee = False _
        ...
        If intInsure <> 0 Then
            If intInsure <> mYBInFor.intInsure Then Call InitInsurePara(Val(NVL(mrsInfo!病人ID)), intInsure)
            mYBInFor.intInsure = intInsure
...
Private Sub LoadIntendBalance(Optional ByVal dblSum As Double = 0, Optional objCard As Card)
    Dim strSQL As String, rsTmp As ADODB.Recordset
    Dim str结算方式 As String, lngCount As Long
    ...
    If objCard Is Nothing Then
        ...
        mBalanceInfor.dbl未付合计 = RoundEx(mBalanceInfor.dbl当前结帐 - dblAlr, 5)
    Else
        With vsBlance
            For i = 1 To .Rows - 1
                If .TextMatrix(i, .ColIndex("组合信息")) <> "" And Val(.TextMatrix(i, .ColIndex("卡类别ID"))) = objCard.接口序号 Then
                    dblTotal = RoundEx(dblTotal + Val(.TextMatrix(i, .ColIndex("结算金额"))), 5)
                    lngCount = lngCount + 1
                Else
                    dblAlr = RoundEx(dblAlr + Val(.TextMatrix(i, .ColIndex("结算金额"))), 5)
                End If
            Next i
        End With
    End If
    ...
    dblBalanceSum = RoundEx(dblBalanceSum + -1 * Get脱机金额, 5)
...
Private Function Get脱机金额() As Double
    Dim dblMoney As Double
    Dim i As Long
    dblMoney = 0
    With vsBlance
        For i = 1 To .Rows - 1
            If Val(.RowData(i)) = 110 And Val(.TextMatrix(i, .ColIndex("结算金额"))) <> 0 Then
...
Private Function InsureBudgeting(ByVal blnOnlyUpload As Boolean) As Boolean
    ...
    With vsBlance
        ...
        For i = 0 To UBound(varData)
            ...
            byt状态 = 0: bytEditSta = IIf(Val(varTemp(2)) = 1, "1", "0")
...
'            If lngRow > 1 Then .Rows = .Rows + 1
            ...
            .TextMatrix(lngRow, .ColIndex("编辑状态")) = bytEditSta   '0-禁止删除;1-允许编辑金额;2-允许删除
...
Private Sub SetOperationCtrl(ByVal bytFun As Byte)
    ...
    cmdYB.Enabled = True
    txtBalance(Idx_找补).Locked = True
    txtBalance(Idx_本次未结).Enabled = False
    'IDKindPaymentsType.Enabled = Not mPatiInfor.bln连续结帐 '不允许选择其他结算方式
    ...
    Case 0  '结算前
        ...
        txtBalance(Idx_缴款).Enabled = True
        txtBalance(Idx_找补).Enabled = True
        txtBalance(Idx_摘要).Enabled = True
        txtBalance(Idx_结算号码).Enabled = True
        txtBalance(Idx_结帐说明).BackColor = &H80000005
        txtBalance(Idx_本次结帐).Enabled = InStr(mstrPrivs, ";结帐设置;") > 0
        txtBalance(Idx_本次结帐).Locked = InStr(mstrPrivs, ";结帐设置;") = 0
        cboPatiNums.Enabled = InStr(mstrPrivs, ";结帐设置;") > 0
        cbo婴儿.Enabled = InStr(mstrPrivs, ";结帐设置;") > 0
        cboDept.Enabled = InStr(mstrPrivs, ";结帐设置;") > 0
        cboFeeType.Enabled = InStr(mstrPrivs, ";结帐设置;") > 0
        cboFeeItem.Enabled = InStr(mstrPrivs, ";结帐设置;") > 0
        cboDiag.Enabled = InStr(mstrPrivs, ";结帐设置;") > 0
        cboChargeType.Enabled = InStr(mstrPrivs, ";结帐设置;") > 0
        
        txtBalance(Idx_本次结帐).BackColor = &H80000005
        If Not mBalanceInfor.bln预交验证 Then
            txtBalance(Idx_冲预交).BackColor = IIf(txtBalance(Idx_冲预交).Enabled, &H80000005, &H8000000F)
        End If
        txtBalance(Idx_缴款).BackColor = &H80000005
        txtBalance(Idx_找补).BackColor = &H80000005
        txtBalance(Idx_摘要).BackColor = &H80000005
        txtBalance(Idx_结算号码).BackColor = &H80000005
        txtBalance(Idx_本次未结).Enabled = False
    ...
    Case 1, 3 '医保虚拟结算后 或未设置过滤条件时
        ...
        txtBalance(Idx_结帐说明).Enabled = bytFun <> 3 ' True
        txtBalance(Idx_本次未结).Enabled = False
        txtBalance(Idx_本次结帐).Enabled = False
        
        txtBalance(Idx_冲预交).Enabled = False
        txtBalance(Idx_缴款).Enabled = False
        txtBalance(Idx_找补).Enabled = False
        txtBalance(Idx_摘要).Enabled = False
        txtBalance(Idx_结算号码).Enabled = False
        ...
        txtBalance(Idx_冲预交).BackColor = &H8000000F
        txtBalance(Idx_缴款).BackColor = &H8000000F
        txtBalance(Idx_找补).BackColor = &H8000000F
        txtBalance(Idx_摘要).BackColor = &H8000000F
        txtBalance(Idx_结算号码).BackColor = &H8000000F
        txtBalance(Idx_本次结帐).BackColor = IIf(txtBalance(Idx_本次结帐).Enabled, &H80000005, &H8000000F)
    ...
    Case Else   '已保存了结帐单
        ...
        txtBalance(Idx_缴款).Enabled = blnEnabled
        txtBalance(Idx_找补).Enabled = blnEnabled
        txtBalance(Idx_摘要).Enabled = blnEnabled
        txtBalance(Idx_结算号码).Enabled = blnEnabled
        ...
        txtBalance(Idx_缴款).BackColor = lngColor
        txtBalance(Idx_找补).BackColor = lngColor
        txtBalance(Idx_摘要).BackColor = lngColor
        txtBalance(Idx_结算号码).BackColor = lngColor
        
        IDKindPaymentsType.Enabled = blnEnabled
        IDKind找补.Enabled = blnEnabled
...
Private Sub SetNextBalanceCmdVisible()
    ...
    blnTemp = blnTemp And Val(txtBalance(Idx_缴款).Text) = 0
    ...
    If blnTemp Then
        ...
        Set objCard = IDKindPaymentsType.GetCurCard
        If Not objCard Is Nothing Then
            blnTemp = blnTemp And objCard.接口序号 <= 0 And Not objCard.消费卡
        Else
            blnTemp = False
        End If
    End If
    ...
    For i = 1 To vsBlance.Rows - 1
        If Val(vsBlance.TextMatrix(i, vsBlance.ColIndex("结算金额"))) <> 0 Then
            blnTemp = False
        End If
    Next i
...
Private Function LoadDefaultFilterCons() As Boolean
        ...
        Do While Not .EOF
            ...
            If Val(NVL(mrsFeeList!开单部门ID)) <> 0 Then
                If InStr(mobjBalanceAll.strAllDeptIDs & ",", "," & Val(NVL(!开单部门ID)) & ",") = 0 Then
                    ...
                    cboDept.AddItem Val(NVL(!开单部门ID)), NVL(!科室, "未知"), False, True
                End If
            End If
            ...
            If Trim(NVL(!费目, "")) <> "" Then
                If InStr(mobjBalanceAll.strAllItem & ",", ",'" & !费目 & "',") = 0 Then
                    ...
                    cboFeeItem.AddItem "'" & mrsFeeList!费目 & "'", !费目, False, True
                End If
            End If
            ...
            If InStr("," & mobjBalanceAll.strAllClass & ",", ",'" & NVL(!类型, "无") & "',") = 0 Then
                ...
                cboFeeType.AddItem "'" & NVL(!类型, "未知") & "'", NVL(!类型, "未知"), False, True
            End If
            ...
            If InStr("," & mobjBalanceAll.strAllBabys & ",", "," & Val(NVL(!婴儿费)) & ",") = 0 And Val(NVL(!婴儿费)) <> 0 Then
                ...
                cbo婴儿.AddItem Val(NVL(!婴儿费)), "第" & Val(NVL(!婴儿费)) & "个婴儿", False, True
            End If
    ...
    If mEditType = g_Ed_门诊结帐 Then
        ...
        If bln普通 Then chkPatiType(0).Value = 1
        If bln体检 Then chkPatiType(1).Value = 1
        If Not bln普通 And Not bln体检 Then
    ...
    If cllOwnerFeeType.Count <> 0 Then
        Set objNode = cboChargeType.Nodes.Add(, , "RootOwner", "所有自费类别")
        objNode.ForeColor = vbRed
        objNode.Bold = True
        objNode.Expanded = True
        objNode.Tag = "'RootOwner'"
        objNode.Checked = True
        cboChargeType.Text = "所有自费类别"
        lblFeeStyle.ForeColor = vbRed
        cboChargeType.ForeColor = vbRed
    End If
    ...
    Set objNode = cboChargeType.Nodes.Add(, , "RootBalance", "所有收费类别")
        ...
        With mobjBalanceCon
            If .strChargeType = "" And .blnCurBalanceOwnerFee = False And blnInputAfterPati = False Then
                Set rsBalance = GetMzBalanceData(lng病人ID, .strDeptIDs, _
                       ...
                       mty_ModulePara.blnZero, mblnDateMoved, .bytKind, .strChargeType, mty_ModulePara.int费用时间 = 1, .strTime, mty_ModulePara.strOwnerPayFeeType, cboDiag.Text)
            Else
                Set rsBalance = GetMzBalanceData(lng病人ID, .strDeptIDs, _
                        .strClass, .dtBeginDate, .dtEndDate, .strItem, blnUpload, _
                       mty_ModulePara.blnZero, mblnDateMoved, .bytKind, .strChargeType, mty_ModulePara.int费用时间 = 1, .strTime, , cboDiag.Text)
            End If
        End With
    ...
    With mobjBalanceCon
        If .strChargeType = "" And .blnCurBalanceOwnerFee = False And blnInputAfterPati = False Then
            Set rsBalance = GetZYBalanceData(lng病人ID, .strTime, .strDeptIDs, .strClass, _
                ...
                mblnDateMoved, .strChargeType, mty_ModulePara.int费用时间 = 1, mty_ModulePara.strOwnerPayFeeType, cboDiag.Text)
        Else
            Set rsBalance = GetZYBalanceData(lng病人ID, .strTime, .strDeptIDs, .strClass, _
                .dtBeginDate, .dtEndDate, .strBaby, .strItem, blnUpload, mty_ModulePara.blnZero, _
                mblnDateMoved, .strChargeType, mty_ModulePara.int费用时间 = 1, , cboDiag.Text)
        End If
    End With
    ReadBalanceData = True
    Exit Function
...
Private Function LoadDetailListFromBalanceID(ByVal lng结帐ID As Long) As Boolean
    ...
    " Where A.结帐ID=[1] And  Mod(A.记录性质,10)=Mod(B.记录性质,10)  " & _
    "       And B.收费细目ID=C.ID And A.NO=B.NO And A.序号=B.序号 And A.记录状态 = B.记录状态 " & _
...
Private Sub LoadCurOwnerPayInfor()
    ...
    Dim dblTemp As Double
    Dim dbl剩余自付 As Double
    
    With mBalanceInfor
        dblMoney = RoundEx(.dbl未付合计 - .dbl冲预交合计, 5)
        lbl自付合计.Caption = Format(.dbl当前结帐 - .dbl医保支付合计, mstrDec)
        If Val(lbl自付合计.Caption) < 0 Then
            stcTittleTotal.Caption = "退款合计"
        Else
            stcTittleTotal.Caption = "自付合计"
        End If
        lbl自付合计.Caption = Format(Abs(.dbl当前结帐 - .dbl医保支付合计), mstrDec)
        dblTemp = GetCentMoney(dblMoney)
            
        lbl剩余自付.Caption = Format(RoundEx(dblTemp + mPatiInfor.dbl未付累计, 6), mstrDec)
        lbl剩余自付.Tag = Format(dblTemp, mstrDec)
        
        If dblMoney < 0 Then
            Set objCard = IDKind找补.GetCurCard
            If Not objCard Is Nothing Then
                If objCard.接口序号 <> 1 Then
                    '存预存
                    dblTemp = Val(lbl剩余自付.Caption) - Val(txtBalance(Idx_找补).Text)
                    dblTemp = GetCentMoney(Abs(dblTemp))
                    lbl剩余自付.Caption = Format(dblTemp, mstrDec)
                    lbl剩余自付.Tag = dblTemp
                End If
            End If
        End If
        ...
        Case g_Ed_取消结帐, g_Ed_结帐作废
            stcTittleTotal.Caption = "退款合计"
            stcTittile.Caption = IIf(dblMoney >= 0, "当前未退", "当前未付")
            mPatiInfor.bln退款标志 = dblMoney >= 0
            lblBalance(4).Caption = IIf(dblMoney >= 0, "退    款", "收    款")
            If dblMoney >= 0 Then
                '退款
                lbl剩余自付.ForeColor = Mid(mty_ModulePara.str当前付款色, InStr(mty_ModulePara.str当前付款色, "|") + 1)
                txtBalance(4).ForeColor = Mid(mty_ModulePara.str缴款色, InStr(mty_ModulePara.str缴款色, "|") + 1)
                lblBalance(4).ForeColor = Mid(mty_ModulePara.str缴款色, InStr(mty_ModulePara.str缴款色, "|") + 1)
                IDKindPaymentsType.ForeColor = Mid(mty_ModulePara.str缴款色, InStr(mty_ModulePara.str缴款色, "|") + 1)
            Else
                '收款
                lbl剩余自付.ForeColor = Mid(mty_ModulePara.str当前付款色, 1, InStr(mty_ModulePara.str当前付款色, "|") - 1)
                txtBalance(4).ForeColor = Mid(mty_ModulePara.str缴款色, 1, InStr(mty_ModulePara.str缴款色, "|") - 1)
            ...
            Set objCard = IDKind找补.GetCurCard
            If Not objCard Is Nothing Then
                If objCard.接口序号 <> 1 And dblMoney > 0 Then
                    '存门诊预存
                    dblTemp = Val(lbl剩余自付.Caption) - Val(txtBalance(Idx_找补).Text)
                    dblTemp = GetCentMoney(Abs(dblTemp))
                    lbl剩余自付.Caption = Format(dblTemp, mstrDec)
        ...
        Case Else
            If chkCancel.Value = 1 Then
                ...
                lblCurPaymentInfor.Caption = "当前退款情况"
            Else
                stcTittile.Caption = IIf(Val(lbl剩余自付.Caption) < 0, "当前未退", "当前未付")
                lblBalance(4).Caption = IIf(Val(lbl剩余自付.Caption) < 0, "退    款", "收    款")
                mPatiInfor.bln退款标志 = dblMoney < 0
                If Val(lbl剩余自付.Caption) < 0 Then
                    '退款
                    lbl剩余自付.ForeColor = Mid(mty_ModulePara.str当前付款色, InStr(mty_ModulePara.str当前付款色, "|") + 1)
                    txtBalance(4).ForeColor = Mid(mty_ModulePara.str缴款色, InStr(mty_ModulePara.str缴款色, "|") + 1)
                    lblBalance(4).ForeColor = Mid(mty_ModulePara.str缴款色, InStr(mty_ModulePara.str缴款色, "|") + 1)
        ...
        End Select
    End With
    If Val(lbl剩余自付.Caption) < 0 Then lbl剩余自付.Caption = Format(Abs(RoundEx(lbl剩余自付.Caption, 6)), mstrDec)
    Set objCard = IDKind找补.GetCurCard
    If Not objCard Is Nothing Then
        If objCard.接口序号 <> 2 And objCard.接口序号 <> 3 Then
            Call LoadDefaultMoney
        End If
    Else
        Call LoadDefaultMoney
...
Private Function LoadFeeList() As Boolean
    ...
    With vsFeeList
        ...
        Do While Not mrsFeeList.EOF
          ...
          dblMoney(0) = RoundEx(dblMoney(0) + Val(NVL(mrsFeeList!应收金额)), 5)
          dblMoney(1) = RoundEx(dblMoney(1) + Val(NVL(mrsFeeList!实收金额)), 5)
          dblMoney(2) = RoundEx(dblMoney(2) + Val(NVL(mrsFeeList!未结金额)), 5)
    ...
    With vsDeposit
        ...
        Do While Not mrsDeposit.EOF
            ...
            .TextMatrix(i, .ColIndex("预交ID")) = NVL(mrsDeposit!预交ID)
             ...
             lng卡类别ID = Val(NVL(mrsDeposit!卡类别ID))
             If lng卡类别ID = 0 Then lng卡类别ID = Val(NVL(mrsDeposit!结算卡序号))

            .TextMatrix(i, .ColIndex("卡类别ID")) = lng卡类别ID
            .TextMatrix(i, .ColIndex("是否消费卡")) = Val(NVL(mrsDeposit!是否消费卡))
            .TextMatrix(i, .ColIndex("卡号")) = NVL(mrsDeposit!卡号)
            .TextMatrix(i, .ColIndex("卡类别名称")) = NVL(mrsDeposit!卡类别名称)
            .TextMatrix(i, .ColIndex("交易流水号")) = NVL(mrsDeposit!交易流水号)
            .TextMatrix(i, .ColIndex("交易说明")) = NVL(mrsDeposit!交易说明)
            .TextMatrix(i, .ColIndex("是否退现")) = Val(NVL(mrsDeposit!是否退现))
            .TextMatrix(i, .ColIndex("是否全退")) = Val(NVL(mrsDeposit!是否全退))
            .TextMatrix(i, .ColIndex("是否缺省退现")) = Val(NVL(mrsDeposit!是否缺省退现))
            .TextMatrix(i, .ColIndex("是否转帐及代扣")) = Val(NVL(mrsDeposit!转帐及代扣))
...
Private Function CheckDepositFactValied() As Boolean
    ...
    If objCard.接口序号 <= 1 Then CheckDepositFactValied = True: Exit Function
    '不存在找补
    If Val(txtBalance(Idx_找补).Text) = 0 Then CheckDepositFactValied = True: Exit Function
...
Private Function zlGetRedGroupUseID(ByRef lng领用ID As Long, _
    ...
    If mobjInvoice.zlGetInvoiceGroupID(mlngModul, UserInfo.姓名, bytKind, _
    ...
    With mrs结算方式
        ...
        Do While Not .EOF
            If (InStr(",3,4,", "," & Val(NVL(!性质)) & ",") = 0) And Val(NVL(!应付款)) <> 1 Then
                If InStr("|" & mty_ModulePara.str脱机医保结算方式 & "|", "|" & !名称 & "|") > 0 Then
                    '脱机医保结算方式
                    With vsBlance
                        If .TextMatrix(.Rows - 1, .ColIndex("结算方式")) <> "" Then
                            .Rows = .Rows + 1
                        End If
                        .TextMatrix(.Rows - 1, .ColIndex("结算方式")) = NVL(mrs结算方式!名称)
                        .TextMatrix(.Rows - 1, .ColIndex("结算性质")) = 2
                        .TextMatrix(.Rows - 1, .ColIndex("结算状态")) = 0
                        .TextMatrix(.Rows - 1, .ColIndex("编辑状态")) = 1
                        .RowData(.Rows - 1) = 110
                    End With
...
'        vsBlance.Clear 1: vsBlance.Rows = 2
        ...
        If Not mEditType = g_Ed_重新作废 And blnInsure = False Then
            mBalanceInfor.dbl冲预交合计 = 0
        End If
                ...
                With vsBlance
                    ...
                    If blnUnload = False Then
                        ...
                        If (mEditType = g_Ed_结帐作废 Or chkCancel.Value = 1) And mEditType <> g_Ed_重新作废 And mEditType <> g_Ed_取消结帐 Then
                            If Val(NVL(mrsBalance!类型)) = 3 Then     '一卡通
                                bytEdit = 2
                            End If
                            If Val(NVL(mrsBalance!类型)) = 5 Then bytEdit = 2
                        End If
                        If byt结算状态 <> 1 Then
                            If mEditType = g_Ed_结帐作废 Or chkCancel.Value = 1 Then
                                If mEditType = g_Ed_重新作废 Then
                        ...
                        .TextMatrix(i, .ColIndex("结算金额")) = FormatEx(dblMoney, 6, , , 2)
            ...
            With vsBlance
                If NVL(mrsBalance!结算方式) <> "" And (NVL(mrsBalance!类型) <> 6 Or mEditType = g_Ed_单据查看) And blnAdd Then
                    ...
                    Next j
                     ...
                     .TextMatrix(i, .ColIndex("结算方式")) = NVL(mrsBalance!结算方式)
                     .TextMatrix(i, .ColIndex("结算金额")) = FormatEx(intSign * Val(NVL(mrsBalance!冲预交)), 6, , , 2)
                     .TextMatrix(i, .ColIndex("结算号码")) = NVL(mrsBalance!结算号码)
                     .TextMatrix(i, .ColIndex("备注")) = NVL(mrsBalance!摘要)
                     .TextMatrix(i, .ColIndex("交易流水号")) = NVL(mrsBalance!交易流水号)
                     .TextMatrix(i, .ColIndex("交易说明")) = NVL(mrsBalance!交易说明)
...
'        panThis.Tag = mConPans.Pan_Deposit
    ...
    dkpMain.RecalcLayout
    'zlRestoreDockPanceToReg  Me, dkpMain, "区域"
End Sub
...
Private Function CheckDepositValied(Optional blnCurBrushDeposit As Boolean) As Boolean
    ...
    '刷过预交款的，需要重新刷卡
    If mBalanceInfor.bln预交验证 Then CheckDepositValied = True: Exit Function
...
Private Function CheckCurBalanceIsValied(ByRef tyBrushCard As TY_BrushCard, _
    ...
    If InStr(txtBalance(Idx_结算号码).Text, "'") > 0 Then
        MsgBox "结算号码含有非法字符单引号,不允许结帐", vbInformation + vbOKOnly, gstrSysName
         Set objSetFocus = txtBalance(Idx_结算号码)
         Exit Function
    End If
    ...
    If InStr(txtBalance(Idx_摘要).Text, "'") > 0 Then
        MsgBox "摘要含有非法字符单引号,不允许结帐", vbInformation + vbOKOnly, gstrSysName
         Set objSetFocus = txtBalance(Idx_摘要)
         Exit Function
    End If
    ...
    If objCard Is Nothing Then
        Screen.MousePointer = 0
        MsgBox "当前不存在有效的结算方式，请选择有效的结算方式!", vbInformation + vbOKOnly, gstrSysName
    ...
    With vsBlance
        ...
        If blnFind Then
            ...
            Exit Function
        End If
        If InStr("34", objCard.结算性质) > 0 And mbln连续结帐 Then
            MsgBox "连续结帐模式下,不允许使用:" & objCard.结算方式 & "进行结帐!", vbInformation + vbOKOnly, gstrSysName
            Exit Function
        End If
            
        If InStr(",1,2,", "," & objCard.结算性质 & ",") > 0 Then
            intCount = intCount + 1
        End If
        If intCount > 1 And mbln连续结帐 Then
            MsgBox "连续结帐模式下,不允许使用多种结算方式进行结帐!", vbInformation + vbOKOnly, gstrSysName
            Exit Function
        End If
    End With
...
Private Function CheckCashValied(ByVal objCard As Card, Optional ByVal bln退款 As Boolean) As Boolean
    ...
    If Not bln退款 Then
        If RoundEx(dblMoney, 6) <> 0 Then
            If Val(dblMoney) < Val(lbl剩余自付.Caption) Then
                If mPatiInfor.bln退款标志 Then
                    Screen.MousePointer = 0
                    MsgBox "退款金额不足,请补足应退金额！" & vbCrLf & "本次应退:" & lbl剩余自付.Caption & vbCrLf & "当前退款" & Format(dblMoney, "0.00"), vbInformation, gstrSysName
                    Exit Function
                Else
                    Screen.MousePointer = 0
    ...
    '退款处理
    If dblMoney < Abs(Val(lbl剩余自付.Caption)) And RoundEx(dblMoney, 6) <> 0 Then
        Screen.MousePointer = 0
...
Private Function SaveBalaceCharge(ByVal bln预交 As Boolean, ByRef tyBrushCard As TY_BrushCard, _
    ...
    With mBalanceInfor
        dbl未付金额 = RoundEx(.dbl未付合计 - .dbl冲预交合计, 6)
    End With
    ...
    ElseIf objCard.结算性质 = 1 Then '当前现金支付
        dblMoney = RoundEx(intSign * Val(txtBalance(Idx_缴款).Text), 6)
        If dblMoney <> 0 Then
            mBalanceInfor.dbl缴款 = dblMoney
            If Not bln存预交 Then
                mBalanceInfor.dbl找补 = RoundEx(IIf(IDKind找补.GetCurCard.名称 = "收款", 1, -1) * Val(txtBalance(Idx_找补).Text), 5)
            End If
        End If
    ...
    ElseIf objCard.名称 Like "*支票*" Then
        If mbln连续结帐 Then
            dblMoney = dbl未付金额: dbl剩余金额 = 0
        Else
            dblMoney = RoundEx(intSign * (Val(txtBalance(Idx_缴款).Text) - mPatiInfor.dbl未付累计), 6)
            If dbl未付金额 < 0 Then
                '支票退款的情况
                If Val(dbl未付金额) > Val(dblMoney) Then
                    MsgBox "退款支票的情况不能超过当前退款金额!", vbInformation, gstrSysName
                    Exit Function
                End If
                dblMoney = RoundEx(intSign * Val(txtBalance(Idx_缴款).Text) - mPatiInfor.dbl未付累计, 6)
                dbl剩余金额 = RoundEx(dbl未付金额 - dblMoney, 6)
            Else
                dbl剩余金额 = RoundEx(dbl未付金额 - dblMoney, 6)
                If Not bln存预交 Then
                    If dbl剩余金额 < 0 Then
                        dbl退支票额 = RoundEx(-1 * Val(txtBalance(Idx_找补).Text), 5)
                        dbl剩余金额 = 0
                    End If
                Else
                    dbl剩余金额 = RoundEx(dbl剩余金额 + Val(txtBalance(Idx_找补).Text), 6)
                End If
            End If
        End If
    Else  '其他结算方式支付
    ...
    Set cllThreeSwap = New Collection
    If Not (InStr(",1,2,", "," & objCard.结算性质 & ",") > 0 Or bln预交) Or dbl剩余金额 = 0 Then
                    ...
                    If Val(.TextMatrix(i, .ColIndex("是否转账"))) = 0 Then
                        If Val(.Cell(flexcpData, i, .ColIndex("组合信息"))) = 1 Then
                            If ExecuteThreeSwapDelSingle(IDKindPaymentsType.GetIDKindCard(.TextMatrix(i, .ColIndex("卡类别ID")), CardTypeID), _
                                                    RoundEx(-1 * Val(.TextMatrix(i, .ColIndex("结算金额"))), 2), .Cell(flexcpData, i, .ColIndex("卡号")), _
...
'                    .RowPosition(.Rows - 1) = 1
                ...
                .TextMatrix(.Rows - 1, .ColIndex("是否密文")) = 0
                '类型:0-普通结算;.rows-1-预交款;2-医保,3-一卡通;4-一卡通(老);5-消费卡
                If objCard.结算性质 = 7 And objCard.接口序号 < 0 Then
                    .TextMatrix(.Rows - 1, .ColIndex("类型")) = 4
                    .TextMatrix(.Rows - 1, .ColIndex("编辑状态")) = 0 '0-禁止删除;1-允许编辑金额;2-允许删除
                    .TextMatrix(.Rows - 1, .ColIndex("结算状态")) = 1 '是否已结算:1-已结算;0-未结算
                ElseIf objCard.接口序号 > 0 Then
                    .TextMatrix(.Rows - 1, .ColIndex("类型")) = 3
                    ...
                    .TextMatrix(.Rows - 1, .ColIndex("编辑状态")) = 0 '0-禁止删除;1-允许编辑金额;2-允许删除
                    .TextMatrix(.Rows - 1, .ColIndex("结算状态")) = 1 '是否已结算:1-已结算;0-未结算
                    .TextMatrix(.Rows - 1, .ColIndex("是否密文")) = IIf(objCard.卡号密文规则 <> "", 1, 0)
                Else
                    .TextMatrix(.Rows - 1, .ColIndex("类型")) = 0
                    .TextMatrix(.Rows - 1, .ColIndex("编辑状态")) = 2 '0-禁止删除;1-允许编辑金额;2-允许删除
                    .TextMatrix(.Rows - 1, .ColIndex("结算状态")) = 0 '是否已结算:1-已结算;0-未结算
                End If
                ...
                .TextMatrix(.Rows - 1, .ColIndex("校对标志")) = 2
                
                .TextMatrix(.Rows - 1, .ColIndex("结算方式")) = objCard.结算方式
                .TextMatrix(.Rows - 1, .ColIndex("结算方式")) = objCard.结算方式
                .TextMatrix(.Rows - 1, .ColIndex("结算金额")) = Format(dblMoney, "0.00")
                .TextMatrix(.Rows - 1, .ColIndex("结算号码")) = IIf(txtBalance(Idx_结算号码).Visible, Trim(txtBalance(Idx_结算号码).Text), "")
                .TextMatrix(.Rows - 1, .ColIndex("备注")) = Trim(txtBalance(Idx_摘要).Text)
                .TextMatrix(.Rows - 1, .ColIndex("卡号")) = IIf(objCard.卡号密文规则 <> "", String(Len(strCardNo), "*"), strCardNo)
                .Cell(flexcpData, .Rows - 1, .ColIndex("卡号")) = tyBrushCard.str卡号
                .TextMatrix(.Rows - 1, .ColIndex("交易流水号")) = tyBrushCard.str交易流水号
                .TextMatrix(.Rows - 1, .ColIndex("交易说明")) = tyBrushCard.str交易说明
                mBalanceInfor.dbl已付合计 = RoundEx(mBalanceInfor.dbl已付合计 + dblMoney, 6)
                mBalanceInfor.dbl未付合计 = RoundEx(mBalanceInfor.dbl未付合计 - dblMoney, 6)
            End If
            If Not mbln连续结帐 Then
...
Public Function Get消费卡结算方式() As String
    ...
    lngCardTypeID = 0
    If Not objCard Is Nothing Then
        If objCard.接口序号 > 0 And objCard.消费卡 And Val(txtBalance(Idx_缴款).Text) <> 0 Then
            '当前刷的消费卡,需使用内部记录集
            lngCardTypeID = objCard.接口序号
        End If
    End If
    ...
    With vsBlance
       For i = 1 To .Rows - 1
           ...
           If Val(.TextMatrix(i, .ColIndex("类型"))) = 5 _
                And Val(.TextMatrix(i, .ColIndex("结算状态"))) = 0 _
                And lngCardTypeID <> Val(.TextMatrix(i, .ColIndex("卡类别ID"))) Then
...
Public Function Get普通结算方式() As String
    ...
    With vsBlance
        ...
        If objCard.接口序号 <= 0 Then
            If mbln连续结帐 Then
                dblMoney = Format(mBalanceInfor.dbl未付合计 - mBalanceInfor.dbl冲预交合计, mstrDec)
            Else
                If mPatiInfor.bln退款标志 Then
                    dblMoney = Format(-1 * Val(txtBalance(Idx_缴款).Text) - mPatiInfor.dbl未付累计, mstrDec)
                Else
                    dblMoney = Format(Val(txtBalance(Idx_缴款).Text) - mPatiInfor.dbl未付累计, mstrDec)
                End If
            End If
            If objCard.结算性质 = 1 Then
                dblMoney = mBalanceInfor.dbl现金
            ElseIf bln存预交 Then
                dblMoney = RoundEx(Val(lbl剩余自付.Tag), 6)
            End If
            
            If dblMoney <> 0 Then
                strBalance = strBalance & "||" & objCard.结算方式
                If objCard.结算性质 = 1 Then
                    '现金
                    strBalance = strBalance & "|" & dblMoney
                    strBalance = strBalance & "| "
                    strBalance = strBalance & "| " & IIf(Trim(txtBalance(Idx_摘要).Text) = "", " ", Trim(txtBalance(Idx_摘要).Text))
                Else
                    strBalance = strBalance & "|" & dblMoney
                    strBalance = strBalance & "|" & IIf(Trim(txtBalance(Idx_结算号码).Text) = "", " ", Trim(Idx_结算号码))
                    strBalance = strBalance & "| " & IIf(Trim(txtBalance(Idx_摘要).Text) = "", " ", Trim(txtBalance(Idx_摘要).Text))
                End If
            End If
        End If
...
Private Function ExcuteBalanceEnd(ByVal dbl退支票 As Double, _
    ...
    If mbln连续结帐 Then
        mPatiInfor.dbl未付累计 = RoundEx(mPatiInfor.dbl未付累计 + Val(lbl剩余自付.Tag), 6)
        mPatiInfor.bln连续结帐 = mbln连续结帐
...
Private Function GetSaveAddDepositSQL(ByVal lng病人ID As Long, lng主页ID As Long, _
    ...
    '存为预交款
    dblMoney = RoundEx(Val(txtBalance(Idx_找补).Text), 6)
...
Private Sub Show误差金额(ByVal bln预交 As Boolean)
    ...
    With mBalanceInfor
        .dbl误差额 = 0
        dbl未付款 = RoundEx(.dbl未付合计 - .dbl冲预交合计, 6)
        intSign = IIf(dbl未付款 < 0, -1, 1)
    End With
    
    dblMoney = IIf(bln预交, Val(txtBalance(Idx_冲预交).Text), intSign * Val(txtBalance(Idx_缴款).Text))
...
Show误差:
    lbl误差额.Visible = mBalanceInfor.dbl误差额 <> 0 And mEditType <> g_Ed_取消结帐
    lbl误差额.Caption = "误差:" & RoundEx(mBalanceInfor.dbl误差额, 6)
End Sub
Private Function CheckSquareBalanceValied(ByVal objCard As Card, ByRef tyBrushCard As TY_BrushCard) As Boolean
    ...
    If RoundEx(dblMoney, 6) <> Val(txtBalance(Idx_缴款).Text) Then
...
Private Function CheckSquareDelValied(ByVal objCard As Card, _
    ...
    If dblDelMoney = 0 Then
        If Val(txtBalance(Idx_缴款).Text) = 0 Then
    ...
    If dblDelMoney = 0 Then
        txtBalance(Idx_缴款).Text = Format(dblTemp, "0.00")
    End If
...
Private Function CancelIsValied(ByVal objCard As Card, ByRef tyBrushCard As TY_BrushCard) As Boolean
    ...
    dblCurMoney = Val(txtBalance(Idx_缴款).Text)
               ...
               End If
                ...
                Case 3 '一卡通
                    ...
                    If Val(.TextMatrix(i, .ColIndex("结算状态"))) = 0 Then
                        Set objTemp = GetLocalePayCard(Val(.TextMatrix(i, .ColIndex("卡类别ID"))), False)
                        If objTemp Is Nothing Then
                            MsgBox "当前站点不支持" & strBalance & "方式支付!", vbInformation + vbOKOnly, gstrSysName
                            Exit Function
                        End If
                        If Val(.TextMatrix(i, .ColIndex("结算金额"))) > 0 And (mEditType = g_Ed_结帐作废 Or mEditType = g_Ed_重新作废) Then bln退款 = True
...
Private Function ExecuteBalaceCancel() As Boolean
    ...
    dblCurMoney = IIf(bln退款, 1, -1) * Val(txtBalance(Idx_缴款).Text)
...
'
    ...
    If objCard.结算性质 = 1 Then
        ...
        mBalanceInfor.dbl找补 = RoundEx(IIf(bln退款, 1, -1) * Val(txtBalance(Idx_找补).Text), 5)
        dblMoney = GetCentMoney(dblTemp)
        mBalanceInfor.dbl现金 = dblMoney
    Else
        dblTemp = dblCurMoney
        If Not objBackCard Is Nothing And dblCurMoney = 0 Then
            If objBackCard.接口序号 <> 1 And Val(txtBalance(Idx_找补).Text) <> 0 Then
               dblTemp = RoundEx(Val(txtBalance(Idx_找补).Text), 6)
            End If
        End If
        dblMoney = GetCentMoney(dblTemp)
        dbl剩余金额 = dbl剩余金额 - dblCurMoney - mBalanceInfor.dbl误差额
    End If
...
Private Function ExecuteOverBalanceCancel(ByVal objCard As Card, _
    ...
    If objCard.接口序号 <= 0 And InStr(",1,2,", "," & objCard.结算性质 & ",") > 0 Then
        ...
        strPTBalance = strPTBalance & "|" & IIf(txtBalance(Idx_结算号码).Text = "", " ", txtBalance(Idx_结算号码).Text)
        strPTBalance = strPTBalance & "|" & IIf(txtBalance(Idx_摘要).Text = "", " ", txtBalance(Idx_摘要).Text)
...
'    strSQL = "Zl_三方退款信息_Insert(" & mBalanceInfor.lng结帐ID & "," & objCard.接口序号 & "," & -1 * dblMoney & ",'" & strCardNo & "'," & "'" & strSwapNO & "'," & "'" & strSwapMemo & "',0)"
...
'                If (Not (mEditType = g_Ed_结帐作废 Or chkCancel.Value = 1)) Or ((mEditType = g_Ed_结帐作废 Or chkCancel.Value = 1) And mBalanceInfor.blnSaveBill = False) Then
                mBalanceInfor.dbl已付合计 = RoundEx(mBalanceInfor.dbl已付合计 + dblMoney, 6)
                mBalanceInfor.dbl未付合计 = RoundEx(mBalanceInfor.dbl未付合计 - dblMoney, 6)
'                End If
...
Private Sub vsDetailList_AfterEdit(ByVal Row As Long, ByVal Col As Long)
    ...
    With vsDetailList
        ...
        For i = 1 To .Rows - 1
            dblMoney = dblMoney + Val(.TextMatrix(i, .ColIndex("结帐金额")))
        Next i
    End With
    mblnNotChange = True
...
Private Sub vsFeeList_AfterMoveColumn(ByVal Col As Long, Position As Long)
    zl_vsGrid_Para_Save mlngModul, vsFeeList, Me.Name, "费用列表" & mEditType
End Sub
...
Private Sub vsDetailList_AfterMoveColumn(ByVal Col As Long, Position As Long)
    zl_vsGrid_Para_Save mlngModul, vsDetailList, Me.Name, "明细列表" & mEditType
End Sub
...
Private Function GetOldBalanceMoney(ByVal int类型 As Integer, _
    ...
    If rsBalance.EOF Then
        If objCard.是否转帐及代扣 Then
           GetOldBalanceMoney = RoundEx(Val(lbl剩余自付.Caption), 6)
        End If
...
Private Function DeleteBalance(Optional blnDelBalance As Boolean) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:结帐作废处理(异常作废)
    '入参:blnDelBalance-true-结算作废;false-异常作废
    ...
    If mYBInFor.intInsure > 0 Then
        If mYBInFor.bytMCMode <> 1 Then
            ...
            If MCPAR.住院结算作废 = False Then
                MsgBox "该医保不支持住院结帐作废，不能取消该结帐单！", vbInformation, gstrSysName: Exit Function
            End If
...
Private Function DepositMonyVerfy(Optional blnSaveCheck As Boolean) As Boolean
    ...
    '操作类型(0-清除所有冲预交;1-按缺省使用预交款;2-按结帐金额来冲预交(按时间先后来分摊）;3-全冲
    If dblMoney <> mBalanceInfor.dbl冲预交合计 And mBalanceInfor.bln预交验证 = False Then
        If dblMoney = 0 Then
            Call RecalcDepositMoney(0)
        Else
            Call RecalcDepositMoney(2, dblMoney)
