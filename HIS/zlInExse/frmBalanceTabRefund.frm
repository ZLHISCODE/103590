...
Begin VB.Form frmBalanceTabRefund 
   BorderStyle     =   0  'None
   Caption         =   "frmBalanceTabRefund"
   ClientHeight    =   6960
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   10200
   ...
   Begin VB.PictureBox picMain 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsfMain 
         ...
         FormatString    =   $"frmBalanceTabRefund.frx":0000
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmBalanceTabRefund"
...
Public Sub ReadData()
    ...
    End Select
    strFilter = " And A.记录状态 = 2 And A.收费时间 Between [1] And [2] And A.操作员姓名 = [3] "
    strTable = "" & _
            ...
            "   Where A.ID=B.结帐ID and  B.病人ID=C.病人ID And A.病人ID =D.病人ID(+) And A.主页ID = D.主页ID(+) And A.结算状态 = 1 " & strFilter & _
    ...
    If rsMain.RecordCount <> 0 Then
        frmManageBalance.tabMain.Item(2).Caption = "异常退费记录(" & rsMain.RecordCount & ")"
        frmManageBalance.stbThis.Panels(2).Text = "当前共有" & rsMain.RecordCount & "条异常退费记录,合计:" & Format(GetTotal, gstrDec) & "元"
    Else
        frmManageBalance.tabMain.Item(2).Caption = "异常退费记录"
...
Private Sub ReadBalance(Optional ByVal lngBalanceID As Long)
    ...
    strSql = _
        " Select Nvl(A.结算方式,'未结金额') As 结算方式," & _
                IIf(blnDel, " -1 *", "") & " Sum(A.冲预交) As 冲预交," & _
...
Private Sub vsfMain_DblClick()
    Call frmManageBalance.ViewBalance(2)
End Sub
...
Public Sub zlRptPrint(ByVal bytFunc As Integer)
    ...
    lngRow = vsfMain.Row
    Set vsBill = vsfMain: strTittle = GetUnitName & "异常销帐记录信息"
    mblnPrint = True
