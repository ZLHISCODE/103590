...
Private Function GetBoundData() As Boolean
    ...
    If UBound(marrInput) >= 0 Then
        Set mrsBindings = mobjDataBase.OpenSQLRecordByArray(mstrSql, mstrWinTittle & "选择", marrInput)
    Else
        Set mrsBindings = mobjDataBase.OpenSQLRecord(mstrSql, mstrWinTittle & "选择", mstrSql)
    End If
