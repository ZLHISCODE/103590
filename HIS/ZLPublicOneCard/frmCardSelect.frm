...
Private mcnOralce As ADODB.Connection
Private mobjDataBase As clsDataBase
Private mobjOneDataObject As clsOneCardDataObject
Private mobjGrid As clsGridObject

Public Function SelectCard(ByVal frmMain As Object, ByRef cnOracle As ADODB.Connection, Optional bytCardType As Byte = 0) As Long
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:选择卡
    '入参:frmMain-调用的主窗体
    ...
    Set mcnOralce = cnOracle
    If Not zlGetOneDataBase(cnOracle, mobjDataBase) = False Then Exit Function
    If Not zlGetOneCardDataObject(cnOracle, mobjOneDataObject) Then Exit Function
    Set mobjGrid = New clsGridObject
   ...
   Exit Function
errHandle:
    Call mobjDataBase.WritLog("CardSelect.SelectCard", "", Err.Description)
End Function
Public Function LoadData() As Boolean
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim objCard As Card, blnIsCardType As Boolean '是否该对接口
    Dim strPreCode As String, I As Long
    Dim strRegSection  As String
    Dim objYLCards As Cards
    '59760
    If mobjOneDataObject.zlGetCards_YL(objYLCards) = False Then Exit Function
    ...
    Exit Function
errHandle:
    Call mobjDataBase.WritLog("CardSelect.LoadData", "", Err.Description)
End Function
    ...
    Select Case Index
        ...
        Case 1 '-设置
            ...
            If lngCardTypeID > 0 Then
                Set mobjCard = mobjOneDataObject.zlGetComponentObject(lngCardTypeID, bln消费卡)
                ...
                Call mobjOneDataObject.zlClearCache '88185,将这两个全局变量置为nothing，以便其它地方更新医疗卡缓存数据
            End If
        Case 2 '-启用,停用
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If Not mcnOralce Is Nothing Then Set mcnOralce = Nothing
    If Not mobjDataBase Is Nothing Then Set mobjDataBase = Nothing
    If Not mobjGrid Is Nothing Then Set mobjGrid = Nothing
End Sub
 ...
 Private Sub vfgList_AfterRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long)
    ...
    mobjGrid.zl_VsGridRowChange vfgList, OldRow, NewRow, OldCol, NewCol, gSysColor.lngGridColorSel
    With vfgList
...
Private Sub vfgList_GotFocus()
  mobjGrid.zl_VsGridGotFocus vfgList, gSysColor.lngGridColorSel
End Sub

Private Sub vfgList_LostFocus()
  mobjGrid.zl_VsGridLostFocus vfgList, gSysColor.lngGridColorLost
End Sub
