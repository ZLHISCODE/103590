...
Private Sub txt卡号_GotFocus()
    ...
    If mobjSquare Is Nothing Then
        Set mobjSquare = CreateObject("zl9CardSquare.CardSquare")
        '初始化射频卡对象
    ...
    End If
    If mcurCardObject.是否非接触式读卡 Then mobjSquare.SetEnabled True
    ...
    Call mobjSquare.zlSetBrushCardObject(mcurCardObject.接口序号, txt卡号, strExpend, _
                                        mcurCardObject.消费卡)
    If mobjCommEvents Is Nothing Then Set mobjCommEvents = New zl9CommEvents.clsCommEvents
...
Public Function CheckBrushSquareCard(ByVal lngCardTypeID As Long, _
   ...
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If str名称 = "" Then
        Set rsTemp = mobjOneDataObject.zlGet消费卡接口
    ...
    If mobjOneDataObject.zlGet_SquareRecordFromCardNo(lngCardTypeID, strCardNo, rsTemp) = False Then
    ...
    dbl失效面额 = 0
    '检查效期
    If NVL(rsTemp!有效期, "3000-01-01 00:00:00") < Format(mobjDataBase.Currentdate, "yyyy-mm-dd HH:MM:SS") Then
        '到了有效期
        If Val(NVL(rsTemp!可否充值)) = 1 Then
            '允许允值的,到期的,不能消费卡面金额,只能消费允值部分
            dbl失效面额 = mobjOneDataObject.zlGet失效面额(Val(NVL(rsTemp!id)))
            dbl余额 = dbl余额 - dbl失效面额
    ...
    str限制类别 = mobjOneDataObject.zlGetFeeTypeFromNameToCode(NVL(rsTemp!限制类别))
