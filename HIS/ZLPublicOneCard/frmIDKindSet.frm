...
Private Sub SaveParaSet()
    ...
    If mblnConn Then Call objDatabase.SetPara("向前滚动-功能键", strValue, glngSys, 1153, True)
    Call SaveRegInFor(mRegType, "医疗卡类别", "向前滚动-功能键", strValue)
    strValue = Trim(cboFastkey(0).Text)
    If mblnConn Then Call objDatabase.SetPara("向前滚动-快键", strValue, glngSys, 1153, True)
    Call SaveRegInFor(mRegType, "医疗卡类别", "向前滚动-快键", strValue)
    
    strValue = Trim(cboFun(1).Text)
    If mblnConn Then Call objDatabase.SetPara("向后滚动-功能键", strValue, glngSys, 1153, True)
    Call SaveRegInFor(mRegType, "医疗卡类别", "向后滚动-功能键", strValue)
     strValue = Trim(cboFastkey(1).Text)
     If mblnConn Then Call objDatabase.SetPara("向后滚动-快键", strValue, glngSys, 1153, True)
    Call SaveRegInFor(mRegType, "医疗卡类别", "向后滚动-快键", strValue)
    
    strValue = Trim(cboFun(2).Text)
    If mblnConn Then Call objDatabase.SetPara("读卡-功能键", strValue, glngSys, 1153, True)
    Call SaveRegInFor(mRegType, "医疗卡类别", "读卡-功能键", strValue)
    strValue = Trim(cboFastkey(2).Text)
    If mblnConn Then Call objDatabase.SetPara("读卡-快键", strValue, glngSys, 1153, True)
    ...
    With cboDefaultCard
        If InStr(1, mstrPrivs, ";参数设置;") > 0 Then
            ...
            Call objDatabase.SetPara("缺省读卡类别", .ItemData(.ListIndex), glngSys, 1153, True)
...
Private Function InitData(Optional blnRestoreDefault As Boolean = False) As Boolean
    ...
    If Not blnRestoreDefault Then
        '78768:李南春,2014/11/26,将快键保存到参数表
        If mblnConn Then
            strA1 = objDatabase.GetPara("向前滚动-功能键", glngSys, 1153)
...
Private Sub Form_Load()
    ...
    Call InitFace
    If mblnConn Then mstrPrivs = ";" & gobjComLib.GetPrivFunc(glngSys, 1153) & ";"
End Sub
...
'将缺省读卡类型保存在参数表和注册表
    ...
    If objDatabase Is Nothing Then Set objDatabase = gobjDatabase
    If mblnConn Then
        strValue = objDatabase.GetPara("缺省读卡类别", glngSys, 1153, 0, Array(lblDefaultCard, cboDefaultCard), InStr(1, mstrPrivs, ";参数设置;") > 0)
    Else
