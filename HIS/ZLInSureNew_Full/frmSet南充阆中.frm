...
Public Function 参数设置() As Boolean
    ...
    frmSet南充阆中.Show vbModal, frm医保类别
    参数设置 = mblnOK
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
...
'    If 医保初始化_南充阆中 = False Then Exit Sub
'
    zlCommFun.ShowFlash "正在下载社保机构,请稍后..."

'   无门诊功能，直接下载住院的社保机构
'    If MsgBox("下载门诊的社保机构吗?", vbQuestion + vbDefaultButton1 + vbYesNo) = vbYes Then
'        If 业务请求_南充阆中(获得社保机构_旺苍, strInput, strOutPut) = False Then
'            zlCommFun.StopFlash
'            Exit Sub
'        End If
'    Else
        If 业务请求_南充阆中(获得社保机构_住院_旺苍, StrInput, strOutput) = False Then
            zlCommFun.StopFlash
            Exit Sub
        End If
...
'    End If
    ...
    For i = 0 To UBound(strArr)
        ...
        gstrSQL = "Select 序号 From 保险中心目录 where 险类=" & TYPE_南充阆中 & "  and 编码 ='" & strArr1(0) & "'"
        zlDatabase.OpenRecordset rsTemp, gstrSQL, Me.Caption
        If rsTemp.EOF Then
            '新增:
            gstrSQL = "Select nvl(Max(序号),0)+1 as 序号 from 保险中心目录 where 险类=" & TYPE_南充阆中
            zlDatabase.OpenRecordset rsTemp, gstrSQL, Me.Caption
            If rsTemp.EOF Then
                lng序号 = 1
            Else
            ...
            End If
              gstrSQL = "zl_保险中心目录_Insert(" & TYPE_南充阆中 & "," & lng序号 & ",'" & strArr1(0) & "','" & strArr1(1) & "')"
        Else
              gstrSQL = "zl_保险中心目录_Update(" & TYPE_南充阆中 & "," & Nvl(rsTemp!序号, 0) & ",'" & strArr1(0) & "','" & strArr1(1) & "')"
        End If
...
Private Sub cmd下载_Click()
    ...
    If 业务请求_南充阆中(下载交易记录_旺苍, StrInput, strOutput) Then Exit Sub
    ShowMsgbox "下载成功!"
...
Private Sub Form_Activate()
    ...
    mblnFirst = False
    Call LoadCbo
    gstrSQL = "Select * From 保险参数 where 险类=" & TYPE_南充阆中
    zlDatabase.OpenRecordset rsTemp, gstrSQL, Me.Caption
    With rsTemp
        Do While Not .EOF
            Select Case Nvl(!参数名)
...
Private Function SaveData() As Boolean
    ...
    '删除已经数据
    gstrSQL = "zl_保险参数_Delete(" & TYPE_南充阆中 & ",null)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    
    '新增参数数据
    gstrSQL = "zl_保险参数_Insert(" & TYPE_南充阆中 & ",null,'医保用户名','" & txtEdit(text医保用户).Text & "',1)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    gstrSQL = "zl_保险参数_Insert(" & TYPE_南充阆中 & ",null,'医保用户密码','" & txtEdit(Text医保密码).Tag & "',2)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    gstrSQL = "zl_保险参数_Insert(" & TYPE_南充阆中 & ",null,'医保服务器','" & txtEdit(Text医保服务器).Text & "',3)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    
    gstrSQL = "zl_保险参数_Insert(" & TYPE_南充阆中 & ",null,'明细时实上传','" & IIf(chk明细.Value = 1, 1, 0) & "',4)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    
   gstrSQL = "zl_保险参数_Insert(" & TYPE_南充阆中 & ",null,'操作员直下个人帐户','" & IIf(chk个人帐户.Value = 1, 1, 0) & "',5)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
   
   gstrSQL = "zl_保险参数_Insert(" & TYPE_南充阆中 & ",null,'比较结算数据','" & IIf(CHK比较.Value = 1, 1, 0) & "',6)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
...
Private Sub LoadCbo()
        ...
        zlDatabase.OpenRecordset rsTemp, gstrSQL, "获取社保机构目录" '  rsTemp.Open gstrSQL, gcnOracle_南充阆中
