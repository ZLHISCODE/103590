...
Private Sub cmd病种_Click()
    ...
    gstrSQL = " Select A.ID,A.编码,A.名称,A.简码,decode(A.类别,1,'慢性病',2,'特种病','普通病') as 类别 " & _
            " From 保险病种 A where A.险类=[1]" & IIf(mstr病种ID = "", " And 1=2", " And A.ID In ([2])")
    Set rsSelected = zlDatabase.OpenSQLRecord(gstrSQL, "提取上次已选择的病种", TYPE_泸州市, Replace(mstr病种ID, "|", ","))
    
    '门诊选择慢特病
    gstrSQL = " Select A.ID,A.编码,A.名称,A.简码,decode(A.类别,1,'慢性病',2,'特种病','普通病') as 类别 " & _
            " From 保险病种 A where A.险类=[1] And A.类别 IN ([2])"
    Set rs病种 = New ADODB.Recordset
    Set rs病种 = zlDatabase.OpenSQLRecord(gstrSQL, "身份验证", TYPE_泸州市, IIf(mint场合 = 0, "1,2", "0"))
...
Private Sub txt病种_KeyPress(KeyAscii As Integer)
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, TYPE_泸州市, IIf(mint场合 = 0, "1,2", "0"), strText)
