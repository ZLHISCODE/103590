...
'返回：
    ...
    cmb中心.Clear
    gstrSQL = "select A.具有中心,B.序号,B.编码,B.名称 from 保险类别 A,保险中心目录 B where A.序号=" & TYPE_重庆松藻 & " and A.序号=b.险类"
    Call OpenRecordset(rsTemp, Me.Caption)
...
Private Sub cmb中心_Click()
    ...
    gstrSQL = "select 参数名,参数值 from 保险参数 where 险类=" & TYPE_重庆松藻 & " and 中心=" & cmb中心.ItemData(cmb中心.ListIndex)
    Call OpenRecordset(rsTemp, Me.Caption)
...
Private Sub cmdOK_Click()
    ...
    '检查病人状态
    gstrSQL = "select nvl(当前状态,0) as 状态 from 保险帐户 where 险类=" & TYPE_重庆松藻 & " and 中心=" & lng中心 & " and 医保号='" & Trim(txtEdit(Text医保号).Text) & "'"
    Call OpenRecordset(rsTemp, Me.Caption)
    ...
    Select Case Index
        Case Select卡号
            gstrSQL = " Select A.病人ID as ID,A.卡号,A.医保号,B.姓名,B.性别,B.出生日期,B.身份证号,C.序号 as 中心ID " & _
                    ...
                    "  where A.病人ID=B.病人ID and A.险类=" & TYPE_重庆松藻 & _
        ...
        Case Select病种
            gstrSQL = " Select A.ID,A.编码,A.名称,A.简码,decode(A.类别,1,'慢性病',2,'特种病','普通病') as 类别 " & _
                    " From 保险病种 A where A.险类=" & TYPE_重庆松藻
...
'从已经存在的记录中读出帐户信息
    ...
    If Not rs帐户 Is Nothing Then
        ...
        '再读出帐户年度信息
        gstrSQL = "select * from 帐户年度信息 where 险类=" & TYPE_重庆松藻 & _
