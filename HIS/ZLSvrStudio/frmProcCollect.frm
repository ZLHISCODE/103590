...
Private Sub LoadFiles(ByVal strSystem As String, Optional ByVal strCurInitPath As String)
    ...
    With vsfFiles
        ...
        For i = 0 To UBound(arrSys)
            ...
            Else
                strCurInitPath = strInitPath & "\" & GetSysNameByCode(lngSysNum) & "\应用脚本\ZLSETUP.INI"
            End If
            
            '1.首先加载当前版本的安装脚本和升级脚本
            '2.加载目标版本的升级脚本,并合并
            Set rsTmpCur = GetUpgradeFiles(rsTmpCur, lngSysNum, GetPrimaryVer(strCur), strCurInitPath, , , strCur, , , True, False)
            Set rsTmpTarget = GetUpgradeFiles(rsTmpTarget, lngSysNum, strCur, strTargetInitPath, , , strTarget, , , True, True)
            ...
            If rsTmpTarget.RecordCount <> 0 Then
                ...
                '兼容zlthird文件
                If gobjFile.FileExists(Left(strCurInitPath, InStrRev(UCase(strCurInitPath), "ZLSETUP.INI") - 1) & "ZLTHIRD.SQL") Then
                    .Rows = .Rows + 1
                    j = .Rows - 1
                    .TextMatrix(j, 0) = j
                    .TextMatrix(j, .ColIndex("系统编号")) = lngSysNum
                    .TextMatrix(j, .ColIndex("系统名称")) = strSysName
                    .TextMatrix(j, .ColIndex("版本")) = GetPrimaryVer(strCur)
                    .TextMatrix(j, .ColIndex("文件")) = Left(strCurInitPath, InStrRev(UCase(strCurInitPath), "ZLSETUP.INI") - 1) & "ZLTHIRD.SQL"
                    .TextMatrix(j, .ColIndex("脚本类型")) = "当前"
                End If
            ...
            '释放对象
            Set rsTmpTarget = Nothing
            Set rsTmpCur = Nothing
        Next
        .Redraw = flexRDDirect
    End With
End Sub
...
Private Sub GetChangedProc(ByVal lngRow As Long)
    '循环表格获取变动过程
    Dim i As Long, j As Long, lngSysNum As Long, lngTmp As Long
    Dim blnCheck As Boolean
    Dim rsCurrent As ADODB.Recordset, rsUpdate As ADODB.Recordset
    Dim strProcTxt As String, strSysOwner As String, strProcNc As String, strErr As String
    ...
    With vsfFiles
        ...
        For i = lngRow To .Rows - 1
            ...
            '遍历表格,根据表格中的文件获取过程
            If i <= .Rows - 1 Then
                lngSysNum = val(.TextMatrix(i, .ColIndex("系统编号")))
                strSysOwner = GetOwnerName(.TextMatrix(i, .ColIndex("系统编号")), gcnOracle)
                
                If .TextMatrix(i, .ColIndex("脚本类型")) = "当前" Then
                    '获取同一系统下的当前版本脚本
                    GetProceduresByFile .TextMatrix(i, .ColIndex("文件")), rsCurrent, .TextMatrix(i, .ColIndex("版本")) _
                        , lngSysNum, .TextMatrix(i, .ColIndex("系统名称")), strSysOwner
                Else
                    '升级脚本
                ...
                If i = .Rows - 1 Then   '最后一行
                    blnCheck = True
                Else
                    If lngSysNum = val(.TextMatrix(i + 1, .ColIndex("系统编号"))) Then
                        blnCheck = False
                    Else
                        blnCheck = True
                    End If
            ...
            If blnCheck Or i = .Rows - 1 Then
                ...
                Else
                    If rsCurrent.RecordCount <= 0 Then
                        pgsBar.Max = 1
                    Else
                        pgsBar.Max = rsCurrent.RecordCount
                    End If
                    ...
                    Do While Not rsCurrent.EOF
                        ...
                        ElseIf IsProcCollected(rsCurrent!P_Name, rsCurrent!P_Ver) Then
                            '已经收集的过程不需要收集
                            rsCurrent.Delete adAffectCurrent
                        ElseIf ConvertStr(rsCurrent!P_DefineNC) = ConvertStr(strProcNc) Or ConvertStr(rsCurrent!P_Define) = ConvertStr(strProcTxt) Then
                            '相同就删除
                ...
                If Not rsUpdate Is Nothing Then
                    If rsCurrent Is Nothing Then
                        lngTmp = 0
                    Else
                        lngTmp = rsCurrent.RecordCount
                    End If
                    If lngTmp > 0 And rsUpdate.RecordCount > 0 Then
                        ...
                        Do While Not rsCurrent.EOF
                            ...
                            If rsUpdate.RecordCount <> 0 Then
                                rsUpdate.Update Array("P_System", "P_Owner", "P_SysNum") _
                                              , Array(.TextMatrix(i, .ColIndex("系统名称")) _
                                                    , strSysOwner _
                                                    , .TextMatrix(i, .ColIndex("系统编号")))
                            End If
                            rsCurrent.MoveNext
                            j = j + 1
                        Loop
