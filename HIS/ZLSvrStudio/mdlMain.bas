...
Public gstrLoginPwd As String               '用户登录数据库时输入的密码
...
'返回：是否可以进行操作
    ...
    Dim dateNow As Date
    Dim strNote As String
    Dim datBegin As Date, datEnd As Date, blnDefault As Boolean
    ...
    On Error GoTo errH
    If blnDefault Then
        datBegin = CDate("8:00:00")
        datEnd = CDate("12:00:00")
        strNote = "当前时段处于业务高峰期，使用此功能可能会对系统使用造成一定影响"
        intOperate = 1
        blnLimit = dateNow >= datBegin And dateNow <= datEnd
    Else
        strNote = Nvl(rsTemp!限时原因, "当前时段处于业务高峰期，使用此功能可能会对系统使用造成一定影响")
        intOperate = Nvl(rsTemp!操作选项, 0)
        With rsTemp
            Do While Not .EOF
    ...
    If blnLimit = True Then
        If intOperate = 0 Then  '弹出提示，并禁止用户进行后续操作
            MsgBox strNote & "，故在时间范围：" & vbNewLine & Mid(strTime, 2) & vbNewLine & "内禁止使用此功能！", vbInformation, gstrSysName
        Else   '弹出提示，但不禁止用户进行后续操作
            If MsgBox(strNote & "，确定要继续吗？", vbInformation + vbOKCancel, gstrSysName) = vbOK Then
                blnLimit = False
            Else
                blnLimit = True
            End If
    ...
    With objVsf
        ...
        For i = .Row + 1 To .Rows - .FixedRows
            For j = 0 To UBound(strFiels)   '循环每个列,有一个满足就记为当前行符合条件
                If UCase(.TextMatrix(i, .ColIndex(strFiels(j)))) Like "*" & UCase(strTxt) & "*" Or UCase(.RowData(i)) Like "*" & UCase(strTxt) & "*" Then
                    blnResult = True
                    Exit For
                End If
            Next
        ...
        If intRow = 0 Then
            For j = 0 To UBound(strFiels)   '检查当前行
                If UCase(.TextMatrix(.Row, .ColIndex(strFiels(j)))) Like "*" & UCase(strTxt) & "*" Or UCase(.RowData(.Row)) Like "*" & UCase(strTxt) & "*" Then
                    blnResult = True
                    Exit For
                End If
            Next
...
Public Function FindUser(ByVal strUser, Optional rsUsers As ADODB.Recordset) As String
'功能:根据传入的值模糊查询用户名,如果有多条记录,返回第一条,若无记录,返回空.
'新增传入记录集,从记录集中 通过 用户名、帐号、简码进行查找
    Dim strSQL As String, rstmp As ADODB.Recordset
    Dim strResult As String
    
    On Error GoTo errH
    
    '如果记录集不为空,直接从记录集中进行查找
    If Not rsUsers Is Nothing Then
        rsUsers.MoveFirst
        Do While Not rsUsers.EOF
            With rsUsers
                If UCase(!用户名) Like "*" & UCase(strUser) & "*" Or UCase(!姓名) Like "*" & UCase(strUser) & "*" _
                    Or UCase(!简码) Like "*" & UCase(strUser) & "*" Then
                    strResult = !用户名
                    Exit Do
                End If
                .MoveNext
...
'      lngMaxWidth=窗口的最大窗度，缺省为0表示按设计状态的窗体最大宽度为准
    ...
    Call frmTipInfo.ShowTipInfo(lnghwnd, strInfo, blnMultiRow, blnOutline, lngMaxWidth)
End Sub
