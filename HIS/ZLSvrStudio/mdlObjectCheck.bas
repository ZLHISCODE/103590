...
'功能：初始化解析本地SQL文件数据保存的记录集
    ...
    With InitProDataRecordset
        ...
        .Fields.Append "约束字段", adVarChar, 200, adFldIsNullable
        .CursorLocation = adUseClient
        .LockType = adLockOptimistic
        .CursorType = adOpenStatic
        .Open
...
'    lngTime = GetTickCount / 1000
    Call CheckSequence
''    Debug.Print strSysName & "序列检查耗时" & GetTickCount / 1000 - lngTime
''    lngTime = GetTickCount / 1000
    Call CheckView
''    Debug.Print strSysName & "视图检查耗时" & GetTickCount / 1000 - lngTime
''    lngTime = GetTickCount / 1000
    Call CheckPackage
''    Debug.Print strSysName & "包检查耗时" & GetTickCount / 1000 - lngTime
''    lngTime = GetTickCount / 1000
    Call CheckTable
'    Debug.Print strSysName & "表检查耗时" & GetTickCount / 1000 - lngTime
'    lngTime = GetTickCount / 1000
    Call CheckConstraint
'    Debug.Print strSysName & "约束检查耗时" & GetTickCount / 1000 - lngTime
'    lngTime = GetTickCount / 1000
    Call CheckIndex
''    Debug.Print strSysName & "索引检查耗时" & GetTickCount / 1000 - lngTime
''    lngTime = GetTickCount / 1000
    Call CheckProcedure
''    Debug.Print strSysName & "过程检查耗时" & GetTickCount / 1000 - lngTime
''    lngTime = GetTickCount / 1000
    Call CheckBasicData
'    Debug.Print strSysName & "基础数据检查耗时" & GetTickCount / 1000 - lngTime
    ...
    While Not mrsFildFromFile.EOF
        ...
        If mrsFildFromDB.RecordCount = 0 Then
            ...
            While strTableName = mrsFildFromFile!表名
                ...
                If mrsFildFromFile.EOF Then Exit Sub '可能在里面就循环完了，故需要判断
            Wend
            ...
            Do While strTableName = mrsFildFromFile!表名
                ...
                Else
                    ...
                    If mrsFildFromFile!字段类型 <> mrsFildFromDB!字段类型 And mrsFildFromFile!字段类型 <> "VARCHAR" Then
                        If mrsFildFromFile!字段长度 <> "" And mrsFildFromDB!字段类型 <> "" Then
                    ...
                    Else
                        ...
                        If mrsFildFromFile!字段类型 = "NUMBER" Then
                            If mrsFildFromFile!字段长度 <> "" And mrsFildFromDB!字段实际长度 <> "" Then
                                varTemp = Split(mrsFildFromFile!字段长度, ",")
                                strTemp = val(varTemp(0))
                                strFildLength = val(mrsFildFromDB!字段实际长度)
                                If strFildLength <> 0 Then
                                    If strTemp > strFildLength Then
                                        strSQL = "Alter Table " & strTableName & " Modify " & mrsFildFromFile!字段 & " " & mrsFildFromFile!字段类型 & "(" & mrsFildFromFile!字段长度 & ")"
                                        mrsProData.AddNew Array("系统名称", "修正SQL", "类别", "严重程度", "问题描述", "修正说明", "对象名"), Array(mstrSysName, strSQL, "字段", "较重", _
                                                    "数据库中该字段长度(" & strFildLength & ")比标准产品(" & strTemp & ")短，可能导致数据的不完整。", "将数据库字段长度调整为与产品标准脚本一致", strTableName & "：" & strFild)
                                    ElseIf strTemp < strFildLength Then
                        ...
                        ElseIf mrsFildFromFile!字段类型 = "VARCHAR2" Then
                            If mrsFildFromFile!字段长度 <> "" And mrsFildFromDB!字段实际长度 <> "" Then
                                strTemp = val(mrsFildFromFile!字段长度)
                                strFildLength = val(mrsFildFromDB!字段长度)
                                If strFildLength <> 0 Then
                                    If strTemp > strFildLength Then
                                        strSQL = "Alter Table " & strTableName & " Modify " & mrsFildFromFile!字段 & " " & mrsFildFromFile!字段类型 & "(" & mrsFildFromFile!字段长度 & ")"
                                        mrsProData.AddNew Array("系统名称", "修正SQL", "类别", "严重程度", "问题描述", "修正说明", "对象名"), Array(mstrSysName, strSQL, "字段", "较重", _
        ...
        If mrsConstraintFromDB.RecordCount = 0 Then
            If strName Like "*_PK" Or strName Like "*_UQ_*" Then
                ...
                If mrsIndexFromDB.RecordCount > 0 Then
                    ...
                    mrsProData.AddNew Array("系统名称", "修正SQL", "类别", "严重程度", "问题描述", "修正说明", "对象名", "约束字段"), Array(mstrSysName, mrsConstraintFromFile!SQL, "索引", "较重", _
                        "数据库中该约束不存在，但对应索引存在，可能影响产品升级数据", "删除索引再添加该约束", strName, mrsConstraintFromFile!字段)
                Else
                ...
                End If
            Else
                '35.100上面删除了多余的外键(125816),故需排除这些
                If InStr(",库房ID,使用部门ID,部门ID,发送部门ID,维修单位ID,执行科室ID,体检部门ID,采集科室ID,执行部门ID,科室ID,病区ID,所在库房ID,对方库房ID,药房ID," & vbNewLine & _
                    "使用小组ID,开单科室ID,虚拟库房ID,领用部门ID,适用科室ID,入院科室ID,婴儿科室ID,婴儿病区ID,出院科室ID,当前病区ID,入院病区ID,转诊科室ID," & vbNewLine & _
                    "续诊科室ID,报告科室ID,确认科室ID,开嘱科室ID,病人科室ID,当前科室ID,提交科室ID,发药药房ID,原病区ID,目标病区ID,登记科室ID,送检科室ID," & vbNewLine & _
                    "开单部门ID,收款部门ID,领卡部门ID,对方部门ID,病人病区ID,就诊病区ID,就诊科室ID,阅读部门ID,原科室ID,现科室ID,执行库房ID,申请科室ID,洗涤科室ID,维修部门ID,", "," & mrsConstraintFromFile!字段 & ",") = 0 Then
                        mrsProData.AddNew Array("系统名称", "修正SQL", "类别", "严重程度", "问题描述", "修正说明", "对象名", "约束字段"), Array(mstrSysName, mrsConstraintFromFile!SQL, "约束", "较重", _
                            "数据库中该约束不存在，可能影响产品相关功能的正常使用", "添加该约束", strName, mrsConstraintFromFile!字段)
                End If
            End If
        ...
        For i = 1 To mrsConstraintFromDB.RecordCount
            ...
            If mrsDataFormDB.RecordCount = 1 Then
                If mrsDataFormDB!系统编号 = mlngNum Then
                    Call frmAppCheck.ShowProgress(mstrSysName, lngTotal, lngLocal + i, "约束", strName)
    ...
    For i = 1 To mrsIndexFromFile.RecordCount
        ...
        If mrsIndexFromDB.RecordCount = 0 Then
            If Not (strName Like "*PK" Or strName Like "*UQ*") Then
                '35.100上面删除了多余的索引(125816),故需排除这些
                If InStr(",病区ID,部门ID,采集科室ID,出院科室ID,当前病区ID,发送部门ID,发药药房ID,科室ID,库房ID,目标病区ID,使用部门ID,提交科室ID," & vbNewLine & _
                    "体检部门ID,维修单位ID,婴儿病区ID,婴儿科室ID,原病区ID,执行部门ID,执行科室ID,部门ID,开单科室ID,科室ID," & vbNewLine & _
                    "库房ID,使用小组ID,执行科室ID,", "," & Mid(strName, InStrRev(strName, "_") + 1) & ",") = 0 Then
        ...
        Else
            ...
            Else
                If mrsIndexFromDB!字段 <> mrsIndexFromFile!字段 Then
                    strSQL = "drop index " & strName
                    strSQL = strSQL & "{JM|SQL分隔符}" & vbNewLine & mrsIndexFromFile!SQL
                    mrsProData.AddNew Array("系统名称", "修正SQL", "类别", "严重程度", "问题描述", "修正说明", "对象名"), Array(mstrSysName, strSQL, "索引", "较重", _
                            ...
                            Else
                                If mblnIndex Then
                                    ...
                                    Do While Not mrsIndexFromFile.EOF
                                        ...
                                        mrsIndexFromFile.MoveNext
                                    Loop
                                    If blnSpace = False And mrsIndexFromDB!表空间 <> "" And strSpace <> "" And mrsIndexFromDB!表名 <> "输血性质" Then
