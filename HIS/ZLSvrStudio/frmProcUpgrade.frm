    ...
    With vsfModule
        For i = 1 To .Rows - 1
            If .Cell(flexcpChecked, i, 0) = flexChecked Then
                '获取当前系统的配置文件
                strInitFile = lblTargetPath.Caption & "\" & GetSysNameByCode(val(.TextMatrix(i, .ColIndex("编号")))) & "\应用脚本\ZLSETUP.INI"
...
Private Sub lblVisable_Click()
    ...
    With vsfModule
        .Top = lblVisable.Top + lblVisable.Height
        .Visible = Not .Visible
        If .Visible Then .SetFocus  '可见就获取焦点
        For i = 1 To .Rows - 1
            If .Cell(flexcpChecked, i, 0) = flexChecked Then
...
Private Sub LoadUpdateSystem(ByVal strPath As String)
    '获取各个系统的升级目标版本
    Dim i As Long
    Dim strInitFile As String, strTarget As String
    Dim blnStep As Boolean
    Dim intNum As Integer
    
    With vsfModule
        For i = 1 To .Rows - 1
            strInitFile = strPath & "\" & GetSysNameByCode(val(.TextMatrix(i, .ColIndex("编号")))) & "\应用脚本\ZLSETUP.INI"
            If gobjFile.FileExists(strInitFile) Then
                If GetUpgradeFiles(Nothing, val(.TextMatrix(i, .ColIndex("编号"))), .TextMatrix(i, .ColIndex("当前版本号")) _
                    , strInitFile, , , , strTarget, , True, True) Is Nothing Then
                    '未得到目标版本信息
