...
Private Sub cboUserName_Change()
    ...
    If cboUserName.Tag = "1" Then Exit Sub
...
Private Sub cboWorkStation_Change()
    ...
    If cboWorkStation.Tag = "1" Then Exit Sub
...
Private Sub FillAuditLog()
    ...
    strSQL = "Select a.用户名, d.姓名, e.名称 部门, a.工作站, To_char(a.操作时间,'yyyy-MM-dd hh24:mi:ss') 操作时间, Decode(a.操作类型, 1, '增加', 2, '修改', '删除') 操作类型, 0 系统, '服务器管理工具' 所属系统, f.模块," & vbNewLine & _
            ...
            "Select a.用户名, d.姓名, e.名称 部门, a.工作站, To_char(a.操作时间,'yyyy-MM-dd hh24:mi:ss') 操作时间, Decode(a.操作类型, 1, '增加', 2, '修改', '删除') 操作类型, f.系统, h.名称 所属系统, f.模块," & vbNewLine & _
            "       g.标题 操作模块, f.功能, a.操作内容, a.操作说明" & vbNewLine & _
...
Private Sub lisShowList_Click()
    '单击数据，则更改cbo中的文字
    If lisShowList.Tag = "1" Then
        cboWorkStation.Tag = "1"  '使用tag过滤隐式调用cboWorkStation_Change方法
...
Private Sub lisShowList_DblClick()
    '鼠标双击击数据，则将该数据选中
    If lisShowList.Tag = "1" Then
        cboWorkStation.Text = lisShowList.List(lisShowList.ListIndex)
        lisShowList.Visible = False
