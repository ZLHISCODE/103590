...
Private rsM                     As New ADODB.Recordset
...
'    gstrSql = Replace(gstrSql, "[2]", mstrType)
    ...
    End If
     ...
     gstrSQL = Replace(gstrSQL, "[1]", "'" & mintItemFile & "'")
     gstrSQL = Replace(gstrSQL, "[2]", "'" & mintStrFile & "'")
'    Set rsM = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, mintItemFile, mintStrFile)
    
     Call OpenRecordset(rsM, gstrSQL, Me.Caption)
