...
Private Const COL_计价特性 = 32
Private Const COL_操作类型 = 33
Private Const COL_试管编码 = 34
Private Const COL_执行性质ID = 35
Private Const COL_执行科室ID = 36
Private Const COL_次数 = 37
Private Const COL_分解时间 = 38
Private Const COL_标本部位 = 39
Private Const COL_检查方法 = 40
Private Const COL_执行标记 = 41
Private Const COL_是否变价 = 42
Private Const COL_跟踪在用 = 43
...
Private Sub Form_Load()
    ...
    mblnAutoExe = Val(zlDatabase.GetPara("本科执行自动完成", glngSys, p住院医嘱发送)) <> 0
    mbln医技 = Val(zlDatabase.GetPara("医技医嘱后续处理", glngSys, p住院医嘱发送)) <> 0
    mblnSend = False
    mblnRefresh = False
    mblnFirst = True
...
'功能：重置发送条件
    With frmAdviceSendOtherCond
        ...
        If .mblnOK Then
            ...
            mstrEnd = .mstrEnd
            mint期效 = .mint期效
            mint婴儿 = .mint婴儿 '这个条件没有保存,用变量记录
            Call LoadAdviceSend(.mstrEnd, .mint期效, .mlng执行科室ID, .mstr病人IDs, .mstr类别s, .mint婴儿)
        End If
        ResetSend = .mblnOK
    End With
End Function
...
'功能：显示当前发送医嘱行的记帐费用信息(按费别打折)
    ...
    With vsAdvice
        ...
        If .TextMatrix(lngRow, COL_诊疗类别) = "4" Then
            strTmp = "卫生材料-" & Replace(.Cell(flexcpData, lngRow, COL_医嘱内容), "'", "''")
            strSQL = _
                "Select " & lngRow & " as 行号,'" & strTmp & "' as 计价医嘱," & _
                " B.ID,B.类别,B.名称||Decode(B.规格,NULL,NULL,' '||B.规格) as 名称,B.计算单位 as 单位," & _
                " 0 as 附加手术,B.是否变价,B.加班加价,C.单价," & Val(.TextMatrix(lngRow, COL_总量)) & " as 数量," & _
                Val(.TextMatrix(lngRow, COL_执行科室ID)) & " as 执行科室ID,B.费用类型,B.屏蔽费别,0 as 从项,0 as 费用性质,0 as 收费方式 " & _
                " From 病人医嘱记录 A,收费项目目录 B,病人医嘱计价 C" & _
                " Where A.收费细目ID=B.ID And A.ID=C.医嘱ID(+) And A.ID=" & Val(.TextMatrix(lngRow, COL_ID))
        ElseIf .TextMatrix(lngRow, COL_诊疗类别) = "E" And Val(.TextMatrix(lngRow - 1, COL_相关ID)) = Val(.TextMatrix(lngRow, COL_ID)) Then
            '检验组合
            If Val(.TextMatrix(lngRow, COL_计价特性)) = 0 And InStr(",0,5,", Val(.TextMatrix(lngRow, COL_执行性质ID))) = 0 Then
                ...
                strSQL = _
                    ...
                    " B.费用类型,B.屏蔽费别,Nvl(A.从项,0) as 从项,Nvl(A.费用性质,0) as 费用性质," & _
                    " Nvl(A.收费方式,0) as 收费方式 From 病人医嘱计价 A,收费项目目录 B" & _
        ...
        Else
            If Val(.TextMatrix(lngRow, COL_计价特性)) = 0 And InStr(",0,5,", Val(.TextMatrix(lngRow, COL_执行性质ID))) = 0 Then
                ...
                '包含基础费用及加收费用
                strSQL = _
                    "Select " & lngRow & " as 行号," & _
                    " '" & strTmp & "'||Decode(A.费用性质,1,Decode(" & Val(.TextMatrix(lngRow, COL_执行标记)) & ",1,'(床旁加收)',2,'(术中加收)','(加收)'),Null) as 计价医嘱," & _
                    " B.ID,B.类别,B.名称,B.计算单位 as 单位,0 as 附加手术,B.是否变价,B.加班加价,A.单价," & _
                    " Nvl(A.数量,0)*" & Val(.TextMatrix(lngRow, COL_总量)) & " as 数量," & _
                    " Nvl(A.执行科室ID," & Val(.TextMatrix(lngRow, COL_执行科室ID)) & ") as 执行科室ID," & _
                    " B.费用类型,B.屏蔽费别,Nvl(A.从项,0) as 从项,Nvl(A.费用性质,0) as 费用性质," & _
                ...
                If Val(.TextMatrix(i, COL_相关ID)) = Val(.TextMatrix(lngRow, COL_ID)) Then
                    If Val(.TextMatrix(i, COL_计价特性)) = 0 And InStr(",0,5,", Val(.TextMatrix(i, COL_执行性质ID))) = 0 Then
                        ...
                        ElseIf .TextMatrix(i, COL_诊疗类别) = "D" Then
                            '检查部位及方法
                            strTmp = "检查部位-" & Replace(.TextMatrix(i, COL_标本部位) & "(" & .TextMatrix(i, COL_检查方法) & ")", "'", "''")
                        End If
    ...
    With vsPrice
        ...
        If strSQL <> "" Then
            '以最新价格计算
            strSQL = "Select A.行号,A.费用性质,A.收费方式," & _
                ...
                " Order by A.行号,A.费用性质,A.从项,A.ID"
            Call zlDatabase.OpenRecordset(rsTmp, strSQL, Me.Caption)
            ...
            For i = 1 To rsTmp.RecordCount
                If str行号 <> rsTmp!行号 & "_" & rsTmp!费用性质 & "_" & rsTmp!ID Then
                    If str行号 <> "" Then
                    ...
                    End If
                    str行号 = rsTmp!行号 & "_" & rsTmp!费用性质 & "_" & rsTmp!ID
                    ...
                    .TextMatrix(.Rows - 1, COLP_收费方式) = Decode(Nvl(rsTmp!收费方式, 0), 0, "正常收取", 1, "检验试管费用", 2, "一次发送只收取一次", 3, "当天只收取一次", 4, "当天未执行收取一次", 5, "当天只收取一次,排斥其他项目", 6, "当天未执行收取一次,排斥其他项目")
...
'返回：str类别=计价类别
    ...
    With vsAdvice
        ...
        If Val(.TextMatrix(lngRow, COL_计价特性)) = 0 And InStr(",0,5,", Val(.TextMatrix(lngRow, COL_执行性质ID))) = 0 Then
            If .TextMatrix(lngRow, COL_诊疗类别) = "4" And Not (Val(.TextMatrix(lngRow, COL_跟踪在用)) = 0 And Val(.TextMatrix(lngRow, COL_是否变价)) = 1) Then
                '非跟踪在用的变价卫材医嘱，单价存放在医嘱计价表中；其他的取定价或时价
                dbl数量 = Format(Val(.TextMatrix(lngRow, COL_总量)), "0.00000")
                
                If str费别 = "" Then
                    If Val(.TextMatrix(lngRow, COL_是否变价)) = 1 Then
                        dbl单价 = CalcDrugPrice(Val(.TextMatrix(lngRow, COL_收费细目ID)), Val(.TextMatrix(lngRow, COL_执行科室ID)), dbl数量)
                    Else
                        dbl单价 = CalcPrice(Val(.TextMatrix(lngRow, COL_收费细目ID)))
                    End If
                    cur金额 = Format(dbl数量 * Format(dbl单价, "0.00000"), gstrDec)
                Else
                    If Val(.TextMatrix(lngRow, COL_是否变价)) = 1 Then
                        cur金额 = Format(CalcDrugPrice(Val(.TextMatrix(lngRow, COL_收费细目ID)), Val(.TextMatrix(lngRow, COL_执行科室ID)), dbl数量, str费别), gstrDec)
                    Else
                        cur金额 = Format(CalcPrice(Val(.TextMatrix(lngRow, COL_收费细目ID)), str费别, dbl数量, , Val(.TextMatrix(lngRow, COL_执行科室ID))), gstrDec)
                    End If
                End If
            Else
...
'功能：初始化清单格式
    ...
    strHead = ",300,4;科室,850,1;姓名,750,1;住院号,750,1;床号,500,4;费别,750,1;" & _
        ...
        "频率,1000,1;医生嘱托,1500,1;执行科室,1500,1;附加执行,1500,1;执行时间,1000,1;首次时间,1530,1;末次时间,1530,1;" & _
        "病人ID;主页ID;性别;年龄;ID;相关ID;病人科室ID;开嘱科室ID;开嘱医生;诊疗类别;诊疗项目ID;收费细目ID;" & _
        "计价特性;操作类型;试管编码;执行性质ID;执行科室ID;次数;分解时间;标本部位;检查方法;执行标记;是否变价;跟踪在用;紧急标志"
...
'功能：初始化计价清单格式
    ...
    strHead = "计价医嘱,2000,1;类别,650,1;收费项目,2000,1;数量,900,7;单位,500,1;单价,1000,7;" & _
        "应收金额,1200,7;实收金额,1200,7;执行科室,1000,1;费用类型,850,1;从项,450,4;收费方式,1500,1"
    arrHead = Split(strHead, ";")
    With vsPrice
        .Clear
        .FixedRows = 1
...
Private Function LoadAdviceSend(ByVal strEnd As String, ByVal int期效 As Integer, _
    ByVal lng执行科室ID As Long, ByVal str病人IDs As String, ByVal str类别s As String, ByVal int婴儿 As Integer) As Boolean
...
'   COL_执行科室ID：是否更改了执行科室
    ...
    Dim strSQL As String, str主要条件 As String
    ...
    Else
        str主要条件 = " And Nvl(A.医嘱状态,0) Not IN(1,2,4,8,9) And Nvl(A.医嘱期效,0)=1"
    End If
    '发送的医嘱范围限制
    If InStr(GetInsidePrivs(p住院医嘱发送), "全院医嘱发送") = 0 Then
        str主要条件 = str主要条件 & " And Substr(A.开嘱医生,Instr(A.开嘱医生,'/')+1) In(" & _
    ...
    '婴儿的处理
    If int婴儿 <> -1 Then
        str主要条件 = str主要条件 & " And Nvl(A.婴儿,0)" & IIF(int婴儿 = 0, "=0", ">0")
    End If
    ...
    For k = 0 To UBound(Split(str病人IDs, ","))
        ...
        '读取发送明细:叮嘱不发送(手术,检查,检验不允许为叮嘱,采集方法可能为叮嘱),持续护理等级,病重/危医嘱,术后医嘱不发送,记录入出量医嘱不发送
        strSQL = "Select A.ID,A.相关ID,Nvl(A.相关ID,A.ID) as 组ID,Nvl(X.序号,A.序号) as 组号," & _
            ...
            " A.婴儿,A.病人ID,A.主页ID,B.住院号,B.出院病床 as 床号,D.名称 as 科室,C.姓名,C.性别,C.年龄,B.费别,B.险类," & _
            " A.开始执行时间,A.上次执行时间,A.医嘱内容,A.总给予量,A.单次用量,E.计算规则,Decode(A.诊疗类别,'4',H.计算单位,E.计算单位) as 计算单位," & _
            " A.执行终止时间,A.执行频次,A.频率次数,A.频率间隔,A.间隔单位,A.医生嘱托,A.执行时间方案,A.病人科室ID,A.开嘱科室ID,A.开嘱医生," & _
            " A.标本部位,A.检查方法,A.执行标记,A.计价特性,E.操作类型,E.试管编码,A.执行性质,A.执行科室ID,F.名称 as 执行科室,H.是否变价,I.跟踪在用,a.紧急标志 " & _
            " From 病人医嘱记录 A,病案主页 B,病人信息 C,部门表 D,诊疗项目目录 E,部门表 F,诊疗项目类别 G,收费项目目录 H,材料特性 I,病人医嘱记录 X" & _
            " Where A.病人ID=[2] And A.病人ID=C.病人ID And B.出院科室ID=D.ID And A.收费细目ID=H.ID(+) And A.收费细目ID=I.材料ID(+)" & _
            ...
            " And A.诊疗类别 Not IN('5','6','7')" & str主要条件 & str执行科室 & str诊疗类别 & _
            " And (Nvl(A.执行性质,0)<>0 Or A.诊疗类别='E' And E.操作类型='6')" & _
            " And Not(A.诊疗类别='H' And E.操作类型='1' And E.执行频率=2)" & _
            " And Not(A.诊疗类别='Z' And E.操作类型 IN('4','9','10','12'))" & _
            " And A.开始执行时间 is Not NULL And Nvl(A.医嘱状态,0)<>-1" & _
            " And Nvl(A.执行标记,0)<>-1 And A.病人来源<>3" & IIF(Not mbln医技, " And A.前提ID is Null", "") & _
                    ...
                    If int期效 = 0 Then
                        ...
                        If (IsNull(rsSend!相关ID) And Not bln采集方法) _
                            ...
                            If IsNull(rsSend!执行时间方案) And (Nvl(rsSend!频率次数, 0) = 0 Or Nvl(rsSend!频率间隔, 0) = 0 Or IsNull(rsSend!间隔单位)) Then
                                ...
                                str分解时间 = "" '不需要
                                If str首次时间 < Format(rsSend!开始执行时间, "yyyy-MM-dd HH:mm:ss") Then
                                    str首次时间 = Format(rsSend!开始执行时间, "yyyy-MM-dd HH:mm:ss")
                                End If
                                If str末次时间 < Format(rsSend!开始执行时间, "yyyy-MM-dd HH:mm:ss") Then
                                    str末次时间 = Format(rsSend!开始执行时间, "yyyy-MM-dd HH:mm:ss")
                                End If
                                .TextMatrix(lngRow, COL_首次时间) = str首次时间 '也显示出来
                                .TextMatrix(lngRow, COL_末次时间) = str末次时间
                            ...
                            Else
                                '执行频率为"可选频率"的项目
                                str分解时间 = Calc段内分解时间(datBegin, datEnd, strPause, Nvl(rsSend!执行时间方案), rsSend!频率次数, rsSend!频率间隔, rsSend!间隔单位, rsSend!入院日期)
...
Private Function CompletePatiSend(rsPati As ADODB.Recordset, rsSQL As ADODB.Recordset, _
    rsUpload As ADODB.Recordset, rsItems As ADODB.Recordset, ByVal cur合计 As Currency, ByVal str类别 As String, _
    ByVal str类别名称 As String, strWarn As String, intWarn As Integer, blnTran As Boolean) As Boolean
...
'说明：如果出错,则在调用函数中处理,blnTran返回是否启用了事务
    ...
    If cur合计 > 0 Then
        ...
        Set rsWarn = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, Val(rsPati!当前病区ID), CStr(Nvl(rsPati!适用病人)))
        If Not rsWarn.EOF Then
            If rsWarn!报警方法 = 2 Then cur当日 = GetPatiDayMoney(rsPati!病人ID)
            str类别名称 = Mid(str类别名称, 2)
            For i = 1 To Len(str类别)
                intR = BillingWarn(Me, GetInsidePrivs(p住院医嘱发送), rsWarn, rsPati!姓名, Nvl(rsPati!剩余款, 0), cur当日, cur合计, Nvl(rsPati!担保额, 0), Mid(str类别, i, 1), Split(str类别名称, ",")(i - 1), strWarn, intWarn)
...
Private Function CheckWaitExecute(rsPati As ADODB.Recordset, ByVal lngRow As Long, ByVal byt检查方式 As Byte) As Boolean
...
'返回：是否继续
    ...
    With vsAdvice
        strTmp = ExistWaitExe(rsPati!病人ID, Val(.TextMatrix(lngRow, COL_主页ID)), .Cell(flexcpData, lngRow, COL_婴儿))
        If strTmp <> "" Then
            Call .ShowCell(lngRow, COL_医嘱内容): .Refresh
            If byt检查方式 = 1 Then
                If MsgBox("发现病人""" & rsPati!姓名 & IIF(.Cell(flexcpData, lngRow, COL_婴儿) <> 0, "(婴儿" & .Cell(flexcpData, lngRow, COL_婴儿) & ")", "") & """存在尚未执行完成的内容：" & _
                ...
                End If
            Else
                MsgBox "发现病人""" & rsPati!姓名 & IIF(.Cell(flexcpData, lngRow, COL_婴儿) <> 0, "(婴儿" & .Cell(flexcpData, lngRow, COL_婴儿) & ")", "") & """存在尚未执行完成的内容：" & _
                    vbCrLf & vbCrLf & strTmp & vbCrLf & vbCrLf & "医嘱""" & .TextMatrix(lngRow, COL_医嘱内容) & """将不被发送。", vbInformation, gstrSysName
                Exit Function
            End If
        End If
        
        strTmp = ExistWaitDrug(rsPati!病人ID, Val(.TextMatrix(lngRow, COL_主页ID)), .Cell(flexcpData, lngRow, COL_婴儿))
        If strTmp <> "" Then
            Call .ShowCell(lngRow, COL_医嘱内容): .Refresh
            If byt检查方式 = 1 Then
                If MsgBox("发现病人""" & rsPati!姓名 & IIF(.Cell(flexcpData, lngRow, COL_婴儿) <> 0, "(婴儿" & .Cell(flexcpData, lngRow, COL_婴儿) & ")", "") & """" & _
                    strTmp & vbCrLf & vbCrLf & "确实要发送""" & .TextMatrix(lngRow, COL_医嘱内容) & """吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                    Exit Function
                End If
            Else
                MsgBox "发现病人""" & rsPati!姓名 & IIF(.Cell(flexcpData, lngRow, COL_婴儿) <> 0, "(婴儿" & .Cell(flexcpData, lngRow, COL_婴儿) & ")", "") & """" & _
...
'返回：如果成功则返回发送号
    ...
    Dim str发生时间 As String, str登记时间 As String, dat发生时间 As Date
            ...
            If .Cell(flexcpData, i, COL_选择) = 0 And Not .Cell(flexcpPicture, i, COL_选择) Is Nothing Then
                ...
                If Val(.TextMatrix(i, COL_病人ID)) <> lng病人ID Then
                    ...
                    If lng病人ID <> 0 Then
                        ...
                        If Not CompletePatiSend(rsPati, rsSQL, rsUpload, rsItems, cur合计, str类别, str类别名称, strWarn, intWarn, blnTran) Then GoTo errH
                    ...
                    Call InitRecordSet(rsSQL, rsTotal, rsUpload, rsNumber, rsMoneyNow, rsItems) '重置SQL数组
                ...
                If .TextMatrix(i, COL_诊疗类别) = "Z" Then
                    ...
                    If InStr(",3,", .TextMatrix(i, COL_操作类型)) > 0 And gbyt转科检查未执行 <> 0 Then
                        If Not CheckWaitExecute(rsPati, i, gbyt转科检查未执行) Then
                            Set .Cell(flexcpPicture, i, COL_选择) = Nothing
                            GoTo NextLoop
                        End If
                    End If
                    If InStr(",5,6,", .TextMatrix(i, COL_操作类型)) > 0 And gbyt出院检查未执行 <> 0 Then
                        If Not CheckWaitExecute(rsPati, i, gbyt出院检查未执行) Then
                            Set .Cell(flexcpPicture, i, COL_选择) = Nothing
                            GoTo NextLoop
                        End If
                    End If
                ...
                '产生单据号分配关键字
                '-----------------------------------------------------------------------------------------
                If InStr(",4,M,", .TextMatrix(i, COL_诊疗类别)) > 0 Then
                ...
                If Val(.TextMatrix(i, COL_计价特性)) = 0 And InStr(",0,5,", Val(.TextMatrix(i, COL_执行性质ID))) = 0 Then
                    ...
                    If .TextMatrix(i, COL_诊疗类别) = "4" Then
                        strSQL = _
                            " Select A.ID,A.类别,D.名称 as 类别名称,A.名称,A.计算单位,B.收入项目ID,C.收据费目," & _
                            " A.计算单位 as 住院单位,1 as 住院包装,1 as 数量,Decode(A.是否变价,1,X.单价,B.现价) as 单价,A.加班加价," & _
                            " B.加班加价率,B.附术收费率,A.是否变价,E.在用分批 as 分批,E.跟踪在用,[2] as 执行科室ID,A.屏蔽费别," & _
                            " A.费用确认,I.要求审批,0 as 从项,0 as 费用性质,0 as 收费方式" & _
                            " From 收费项目目录 A,收费价目 B,收入项目 C,收费项目类别 D,材料特性 E," & _
                                " 病人医嘱记录 R,病人医嘱计价 X,保险支付项目 I" & _
                            " Where R.ID=[1] And R.ID=X.医嘱ID(+) And R.收费细目ID=A.ID" & _
                            " And A.ID=B.收费细目ID And B.收入项目ID=C.ID And A.类别=D.编码" & _
                            " And ((Sysdate Between B.执行日期 and B.终止日期) or (Sysdate>=B.执行日期 And B.终止日期 is NULL))" & _
                            " And A.ID=E.材料ID(+) And A.ID=I.收费细目ID(+) And I.险类(+)=[3]" & _
                            " Order by 费用性质,从项,A.ID"
                    Else
                        strSQL = _
                            " Select A.ID,A.类别,D.名称 as 类别名称,A.名称,A.计算单位,B.收入项目ID,C.收据费目," & _
                            " Y.住院单位,Y.住院包装,X.数量,Decode(A.是否变价,1,X.单价,B.现价) as 单价,A.加班加价," & _
                            " B.加班加价率,B.附术收费率,A.是否变价,Decode(A.类别,'4',E.在用分批,Y.药房分批) as 分批," & _
                            " E.跟踪在用,Nvl(X.执行科室ID,[2]) as 执行科室ID,A.屏蔽费别,A.费用确认,I.要求审批," & _
                            " Nvl(X.从项,0) as 从项,Nvl(X.费用性质,0) as 费用性质,Nvl(X.收费方式,0) as 收费方式" & _
...
MoneyItemBegin:
                        ...
                        If .TextMatrix(i, COL_分解时间) <> "" Then
                            dat发生时间 = CDate(Split(.TextMatrix(i, COL_分解时间), ",")(0))
                        Else
                            dat发生时间 = CDate(.Cell(flexcpData, i, COL_分解时间))
                        End If
                        
                        '----------------------------------------
                        '根据收费方式，确定当前收费项目是否应收费
                        If rsPrice!费用性质 & "_" & rsPrice!ID <> str收费项目 Then
                            If Not AdviceMoneyMake(lng病人ID, Val(.TextMatrix(i, COL_主页ID)), rsMoneyNow, rsMoneyDay, _
                                IIF(Val(.TextMatrix(i, COL_相关ID)) <> 0, Val(.TextMatrix(i, COL_相关ID)), Val(.TextMatrix(i, COL_ID))), _
                                Val(.TextMatrix(i, COL_诊疗项目ID)), rsPrice!ID, lng执行科室ID, .TextMatrix(i, COL_试管编码), _
                                rsPrice!类别, Nvl(rsPrice!收费方式, 0), dat发生时间) Then
                                '跳过当前收费项目(多个收入项目)
                        ...
                        If Not IsNull(rsPati!险类) Then
                            strTmp = gclsInsure.GetItemInsure(lng病人ID, rsPrice!ID, cur实收, False, rsPati!险类, .Cell(flexcpData, i, COL_医生嘱托) & "||" & dbl数量)
                            If strTmp <> "" Then
                        ...
                        If Not IsNull(rsPati!险类) And bln实时监控 Then
                            ...
                            Else
                                ...
                                If rsPrice!费用性质 & "_" & rsPrice!ID <> str收费项目 Then
                                    rsItems!数量 = Nvl(rsItems!数量, 0) + dbl数量
                                End If
                                '单价：同一收费项目的不同收入项目累加
                ...
                If Val(.TextMatrix(i, COL_执行性质ID)) <> 0 Then '叮嘱不发送(采集方法可能为)
                    ...
                    '本科执行的自动执行：特殊医嘱不处理，跟踪在用卫材不处理
                    int执行状态 = 0
                    If mblnAutoExe And Val(.TextMatrix(i, COL_开嘱科室ID)) = Val(.TextMatrix(i, COL_执行科室ID)) _
                        And Not (.TextMatrix(i, COL_诊疗类别) = "Z" And Val(.TextMatrix(i, COL_操作类型)) <> 0) Then
                        'And Not (.TextMatrix(i, COL_诊疗类别) = "4" And Val(.TextMatrix(i, COL_跟踪在用)) = 1)
                        int执行状态 = 1
                    End If
