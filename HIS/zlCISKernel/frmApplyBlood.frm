...
Public Function ShowMe(frmParent As Object, ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal lng病人性质 As Long, ByVal intType As Integer, Optional ByRef lngUpdateAdvice As Long, _
    ...
    Optional ByRef rsDefine As Recordset, Optional ByRef objMip As Object, Optional ByVal int场合 As Integer, Optional ByVal str挂号单 As String, _
    Optional ByVal lng项目id As Long, Optional ByRef rsCard As ADODB.Recordset, Optional ByVal bytBaby As Byte, Optional ByVal int调用场合 As Integer, Optional ByVal lng前提ID As Long) As Boolean
...
'功能：缓存数据
    ...
    If mstr诊断IDs <> "" Then
        ...
        str诊断关联信息SQL = str诊断关联信息SQL & "<splitSQL>" & "Zl_病人医嘱附件_Insert([相关ID],'申请单诊断',null,null,null,'" & txtInfo(txt诊断信息).Text & "',0)"
    End If
...
'功能：保存数据
    ...
    Else
        ...
        str项目名称 = Sys.RowValue("诊疗项目目录", mlng输血项目ID, "名称")
        ...
        arrSQL(UBound(arrSQL)) = "ZL_病人医嘱记录_Insert(" & lng医嘱ID & ",NULL," & lng医嘱序号 & "," & int病人来源 & "," & mlng病人ID & "," & strTmp主页ID & "," & mbytBaby & ",1,1,'K'," & mlng输血项目ID & _
                                 ...
                                 strTmp挂号单 & "," & ZVal(mlng前提ID) & ",Null,0,NULL," & IIF(mstr摘要输血 = "", "null", "'" & mstr摘要输血 & "'") & ",'" & UserInfo.姓名 & "',Null,NULL,'" & cboInfo(cbo输血目的).Text & "'," & str审核状态 & "," & lng申请序号 & ")"
...
Private Sub cbsMain_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Select Case Control.ID
        ...
        Case conMenu_File_PrintSet, conMenu_File_Print, conMenu_File_Preview
            blnVisible = (mint申请单打印模式 = 0 And InStr(GetInsidePrivs(p住院医嘱下达), ";输血申请单;") > 0) Or mint场合 = 1
...
'功能：初始下拉菜单
    ...
    '部分固定内容的下拉框
    Call Cbo.LoadFromList(cboInfo(cbo输血血型), Array(" ", "A", "B", "O", "AB"), 0)
    Call Cbo.LoadFromList(cboInfo(cbo用血安排), Array("普通", "急诊"), 0)
...
'功能：读取申请单信息
    ...
    ElseIf (mintType = 1 Or mintType = 3 Or mintType = 2) And mlngUpdateAdvice <> 0 Then
        ...
        If rsTmp.RecordCount > 0 Then
            ...
            cboInfo(cbo输血血型).ListIndex = Val(rsTmp!血型 & "")
        ...
        strSQL = "Select A.ID,A.相关ID,a.紧急标志,a.用药理由,NVL(to_char(a.手术时间,'yyyy-MM-dd hh24:mi'),a.标本部位) as 预定输血时间,a.开始执行时间,a.诊疗项目ID," & _
                " a.执行科室ID,a.执行性质,a.总给予量,B.计算单位,B.名称 as 项目名称,A.申请序号,A.审核状态,a.医生嘱托" & vbNewLine & _
...
'功能：通过缓存数据加载界面
    ...
    If blnDo Then
        With mrsCard
            ...
            If !输血目的 & "" <> "" Then
                Call Cbo.Locate(cboInfo(cbo输血目的), !输血目的 & "", False)
            End If
    ...
    Else
        ...
        If lng执行科室ID = 0 Then
            If cboInfo(cbo执行科室).ListIndex = -1 And cboInfo(cbo执行科室).ListCount = 1 Then
                If cboInfo(cbo执行科室).List(0) = "[其它...]" Then
                    blnTmp = True
                Else
                    cboInfo(cbo执行科室).ListIndex = 0
                End If
        ...
        End If
        If lng执行科室ID <> 0 Then
            Call Cbo.Locate(cboInfo(cbo执行科室), lng执行科室ID, True)
        End If
    End If
    mlng执行科室性质 = lng执行科室
    If cboInfo(cbo执行科室).ListCount = 1 And blnTmp = False Then cboInfo(cbo执行科室).Enabled = False
...
'参数：intType =0 KeyPress调用，=1 下拉按钮调用
    ...
    strSQL = "Select Distinct A.ID,A.编码,A.名称,A.执行分类 as 执行分类ID,A.计算单位,A.执行科室 as 执行科室ID,A.录入限量 as 录入限量ID" & _
        ...
        strSQL & "  And A.服务对象 IN(" & IIF(lng病人性质 = 1, "1,2", 2) & ",3)" & _
        " And (A.撤档时间=To_Date('3000-01-01','YYYY-MM-DD') Or A.撤档时间 IS NULL)" & _
        " And (A.站点='" & gstrNodeNo & "' Or A.站点 is Null)" & _
        IIF(intType = 0, " And (A.编码 Like [1] Or B.名称 Like [2] Or B.简码 Like [2])", "") & _
        IIF(lng病人性质 = 1, "", " And (Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID And 科室ID=[4]) Or Not Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID))") & _
...
Private Sub Form_Unload(Cancel As Integer)
    If mblnChange And mint调用场合 = 0 Then
