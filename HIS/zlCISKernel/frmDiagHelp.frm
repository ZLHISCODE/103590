...
Begin VB.Form frmDiagHelp 
   ...
   Begin TabDlg.SSTab stbCatalog 
      ...
      Tab(2).Control(0)=   "cmdShow(1)"
      Tab(2).Control(1)=   "cmdDel"
      Tab(2).Control(2)=   "cmdMark"
      Tab(2).Control(3)=   "lvwMark"
...
Private Sub cmdDel_Click()
    ...
    Call zlDatabase.SetPara("疾病诊断书签", strTemp, glngSys, p疾病诊断参考)
End Sub
...
Private Sub cmdFind_Click()
    ...
    mstrSQL = "select distinct I.ID,I.编码,I.名称,decode(I.类别,1,'西医','中医') as 类别" & _
...
Private Sub cmdMark_Click()
    ...
    mstrSQL = "select I.ID,I.编码,I.名称,decode(I.类别,1,'西医','中医') as 类别" & _
            " from 疾病诊断目录 I" & _
            " where I.ID=[1]"
    err = 0: On Error GoTo Errhand
...
Private Sub Form_Load()
    ...
    '提取书签显示
    strTemp = zlDatabase.GetPara("疾病诊断书签", glngSys, p疾病诊断参考)
...
Private Sub zlRefList()
    ...
    err = 0: On Error GoTo Errhand
    Set rsTemp = zlDatabase.OpenSQLRecord(mstrSQL, Me.Caption, IIF(Val(Me.tvwList.Tag) = 0, 1, 2))
    With rsTemp
        Do While Not .EOF
            If IsNull(!上级ID) Then
        ...
        Loop
    End With
    mstrSQL = "select R.分类ID,I.ID,I.编码,I.名称" & _
            " from 疾病诊断目录 I,疾病诊断属类 R" & _
            " where I.ID=R.诊断ID and I.类别=[1]" & _
            " order by I.编码"
    err = 0: On Error GoTo Errhand
    Set rsTemp = zlDatabase.OpenSQLRecord(mstrSQL, Me.Caption, IIF(Val(Me.tvwList.Tag) = 0, 1, 2))
    With rsTemp
...
Private Sub zlShowRef(lngItemID As Long)
    ...
    mstrSQL = "select distinct I.ID,I.编码,N.性质,N.名称||decode(N.性质,1,'',2,'(英文名)','(别名)') as 名称,I.类别" & _
            " from 疾病诊断目录 I,疾病诊断别名 N" & _
            " where I.ID=N.诊断ID and I.ID=[1]"
    err = 0: On Error GoTo Errhand
    ...
    mstrSQL = "select 项目层次,项目序号,nvl(证候序号,0) as 证候序号,0 as 内容行号,decode(nvl(证候名称,''),'',参考项目,证候名称) as 内容" & _
            " from 疾病诊断参考 " & _
            " where 诊断id=[1]" & _
            " union" & _
