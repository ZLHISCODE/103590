...
Private Const COL_医生嘱托 = 16 'Data用于存放摘要(医保)
...
Private Const COL_执行标记 = 37
Private Const COL_药品ID = 38
Private Const COL_剂量系数 = 39
Private Const COL_住院包装 = 40
Private Const COL_住院单位 = 41
Private Const COL_可否分零 = 42
Private Const COL_药房分批 = 43
Private Const COL_是否变价 = 44
Private Const COL_库存 = 45
Private Const COL_次数 = 46
Private Const COL_分解时间 = 47
'-------------------------------------------------
...
Private Const COLP_收费方式 = 12
Private Const COLP_行号 = 13
...
'功能：重置发送条件
    With frmAdviceSendDrugCond
        ...
        If .mblnOK Then
            ...
            Call LoadAdviceSend(.mstrEnd, .mint期效, .mlng药房ID, .mstr病人IDs, .mstr给药IDs, .mint婴儿)
        End If
        ResetSend = .mblnOK
    End With
End Function
        ...
        If InStr(",5,6,", .TextMatrix(lngRow, COL_诊疗类别)) > 0 Then
            ...
            If Val(.TextMatrix(lngRow, COL_执行性质ID)) <> 5 Then
                strSQL = "Select '药品医嘱-" & Replace(.Cell(flexcpData, lngRow, COL_医嘱内容), "'", "''") & "' as 计价医嘱," & _
                    ...
                    " A.屏蔽费别,A.费用类型,0 as 从项,0 as 收费方式" & _
            ...
            If Val(.TextMatrix(lngRow - 1, COL_相关ID)) <> Val(.TextMatrix(lngRow, COL_相关ID)) Then
                ...
                If Val(.TextMatrix(lngTmp, COL_计价特性)) = 0 _
                    ...
                    strSQL = strSQL & IIF(strSQL = "", "", " Union ALL ") & _
                        ...
                        " B.屏蔽费别,B.费用类型,Nvl(A.从项,0) as 从项,Nvl(A.收费方式,0) as 收费方式" & _
    ...
    With vsPrice
        ...
        If strSQL <> "" Then
            ...
            strSQL = _
                ...
                " A.是否变价,E.跟踪在用,A.从项,A.收费方式,A.加班加价,B.加班加价率,Decode(Nvl(A.是否变价,0),1,A.单价,B.现价) as 单价" & _
...
'功能：更改成药规格
    ...
    ElseIf Col = COL_规格 Then
        With vsAdvice
            ...
            '医嘱相关信息
            strSQL = _
                " Select A.ID,A.开始执行时间,A.上次执行时间,A.执行终止时间,A.执行时间方案," & _
                " A.频率次数,A.频率间隔,A.间隔单位,A.单次用量,A.可否分零,B.入院日期" & _
...
'功能：初始化清单格式
    ...
    strHead = ",300,4;科室,850,1;姓名,750,1;住院号,750,1;床号,500,4;费别,750,1;" & _
        ...
        "病人ID;主页ID;性别;年龄;ID;相关ID;病人科室ID;开嘱科室ID;开嘱医生;诊疗类别;诊疗项目ID;计价特性;执行性质ID;执行科室ID;执行标记;" & _
...
'功能：初始化计价清单格式
    ...
    strHead = "计价医嘱,2000,1;类别,650,1;收费项目,2500,1;付数,450,4;数量,900,7;" & _
        "单位,500,1;单价,1000,7;应收金额,1200,7;实收金额,1200,7;执行科室,1000,1;费用类型,850,1;从项,450,4;收费方式,1500,1;行号"
    arrHead = Split(strHead, ";")
    With vsPrice
        .Clear
        .FixedRows = 1
...
'      str分解时间=具体的执行时间分解
    ...
    dbl总量 = Calc发送药品总量( _
        ...
        rsSend!频率次数, rsSend!频率间隔, rsSend!间隔单位, mblnLimit, rsSend!入院日期)
...
Private Function LoadAdviceSend(ByVal strEnd As String, ByVal int期效 As Integer, _
    ByVal lng药房ID As Long, ByVal str病人IDs As String, ByVal str给药IDs As String, ByVal int婴儿 As Integer) As Boolean
...
'   COL_分解时间:临嘱无分解时间时,存放费用发生时间
    ...
    Dim bln品种药品 As Boolean, blnReCalc As Boolean
    ...
    For k = 0 To UBound(Split(str病人IDs, ","))
        ...
        strSQL = "Select A.ID,A.相关ID,Nvl(A.相关ID,A.ID) as 组ID,Nvl(X.序号,A.序号) as 组号," & _
            ...
            " A.病人ID,A.主页ID,B.住院号,B.出院病床 as 床号,D.名称 as 科室,C.姓名,C.性别,C.年龄,B.费别,B.险类," & _
            ...
            " A.可否分零,A.计价特性,A.执行性质,A.执行标记," & str药房置换 & " as 执行科室ID,F.名称 as 执行科室,A.摘要" & _
            ...
            " And A.诊疗类别 IN('5','6','7','E') And Nvl(A.执行标记,0)<>-1 And A.病人来源<>3" & _
            " And A.开始执行时间 is Not NULL And Nvl(A.医嘱状态,0)<>-1" & _
            IIF(Not mbln医技, " And A.前提ID is Null", "") & str主要条件 & str药房条件 & str给药途径 & _
                ...
                For i = 1 To rsSend.RecordCount
                    ...
                    If InStr(",5,6,7", rsSend!诊疗类别) > 0 Then
                        ...
                        Else
                            ...
                            If Not (int期效 = 0 And InStr(",5,6,", rsSend!诊疗类别) > 0) _
                    ...
                    If int期效 = 0 Then
                        ...
                        If rsSend!诊疗类别 = "7" Then
                            ...
                            .TextMatrix(lngRow, COL_分解时间) = Calc次数分解时间(rsSend!总给予量, rsSend!开始执行时间, rsSend!执行终止时间, "", Nvl(rsSend!执行时间方案), rsSend!频率次数, rsSend!频率间隔, rsSend!间隔单位, rsSend!入院日期)
...
Private Sub InitRecordSet(rsSQL As ADODB.Recordset, rsTotal As ADODB.Recordset, _
    rsUpload As ADODB.Recordset, rsMoneyNow As ADODB.Recordset, rsItems As ADODB.Recordset)
'初始化记录集
    'SQL记录集
...
'返回：如果成功，则返回发送号
    ...
    Dim lng父收入ID As Long, lng父项目ID As Long, str实收 As String
                ...
                If .Cell(flexcpData, i, COL_选择) = 0 And Not .Cell(flexcpPicture, i, COL_选择) Is Nothing Then
                    ...
                    If InStr(",5,6,7,", .TextMatrix(i, COL_诊疗类别)) > 0 Then
                        ...
                        If Val(.TextMatrix(i, COL_执行性质ID)) <> 5 Then
                            strSQL = _
                                ...
                                " 0 as 从项,[3] as 执行科室ID,A.屏蔽费别,A.费用确认,0 as 收费方式,I.要求审批" & _
                    ...
                    ElseIf Val(.TextMatrix(i, COL_计价特性)) = 0 And InStr(",0,5,", Val(.TextMatrix(i, COL_执行性质ID))) = 0 Then
                        ...
                        strSQL = _
                            ...
                            " Nvl(X.执行科室ID,[3]) as 执行科室ID,A.屏蔽费别,A.费用确认,Nvl(X.收费方式,0) as 收费方式,I.要求审批" & _
                    ...
                    cur医嘱合计 = 0: lng父收入ID = 0: lng父项目ID = 0
...
MoneyItemBegin:
                            ...
                            If CStr(rsPrice!ID) <> str收费项目 Then
                                If Not AdviceMoneyMake(lng病人ID, Val(.TextMatrix(i, COL_主页ID)), rsMoneyNow, rsMoneyDay, _
                                    ...
                                    Val(.TextMatrix(i, COL_诊疗项目ID)), rsPrice!ID, lng执行科室ID, "", rsPrice!类别, Nvl(rsPrice!收费方式, 0), dat发生时间) Then
                                    '跳过当前收费项目(多个收入项目)
                                    str收费项目 = CStr(rsPrice!ID)
                                    Do While CStr(rsPrice!ID) = str收费项目
                                        rsPrice.MoveNext
                            ...
                            rsSQL!SQL = "ZL_住院记帐记录_Insert(" & _
                                ...
                                IIF(int药品性质 <> 0, int药品性质, Val(.TextMatrix(i, COL_计价特性))) & ",Null,'" & str费用类型 & "')"
                            rsSQL.Update
                    ...
                    If Val(.TextMatrix(i, COL_执行性质ID)) <> 0 Then '叮嘱不发送(给药途径，配方煎法、用法可能为)
                        ...
                        '执行状态
                        int执行状态 = 0
                        If mblnAutoExe And InStr(",5,6,7,", .TextMatrix(i, COL_诊疗类别)) = 0 _
                            And Val(.TextMatrix(i, COL_开嘱科室ID)) = Val(.TextMatrix(i, COL_执行科室ID)) Then
                            int执行状态 = 1
                        End If
                        ...
                        rsSQL!SQL = "ZL_病人医嘱发送_Insert(" & _
                            ...
                            IIF(blnFirst, 1, 0) & ",NULL,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "')"
                        rsSQL.Update
                    End If
