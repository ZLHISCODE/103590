...
Private mrsDetail As ADODB.Recordset '叶子级明细SQL
Private mlngMaxPar As Long '扩展后的参数号最被替换的参数号
'程序变量
...
Private Sub cmdOK_Click()
    ...
    If mbln末级 And mbytStyle = 1 Then
        If mrsDetail!末级 <> 1 Then Exit Sub
    End If
    ...
    ElseIf Not lvw.SelectedItem Is Nothing Then
        If mbytStyle = 2 Then
            mrsDetail.Filter = "ID='" & Split(lvw.SelectedItem.Key, "_")(1) & "' And 末级=1"
        Else
...
Private Sub tvw_s_NodeClick(ByVal Node As MSComctlLib.Node)
    ...
    ElseIf mbytStyle = 2 Then
        ...
        Else
            '只有两处情况,=id,instr(id
            '注意区分了药品和卫材两种情况，关于上级ID,药品的是负数，其它的是正数
            If mblnShowSub Then
                strKeys = GetSubTree(Node) '数据量大时很慢
            Else
            ...
            If strIDs <> "" Then
                If UBound(varTmp) = 0 Then
                    If bln药品 Then
                        strSQL = Replace(mstrDetail, "[选择替换的过条件1]", " and (a.分类id=[" & mlngMaxPar & "] or e.分类id=0)")
                        strSQL = Replace(strSQL, "[选择替换的过条件2]", " and e.分类id=0")
                    Else
                        strSQL = Replace(mstrDetail, "[选择替换的过条件1]", " and (a.分类id=0 or e.分类id=[" & mlngMaxPar & "])")
                        strSQL = Replace(strSQL, "[选择替换的过条件2]", " and e.分类id=[" & mlngMaxPar & "]")
                    End If
                ...
                Else
                    ...
                    End If
                    If bln药品 Then
                        strSQL = Replace(mstrDetail, "[选择替换的过条件1]", "  and (a.分类id in ( " & strParTable & " ) or e.分类id=0)")
                        strSQL = Replace(strSQL, "[选择替换的过条件2]", " and e.分类id=0")
                    Else
                        strSQL = Replace(mstrDetail, "[选择替换的过条件1]", " and (a.分类id=0 or e.分类id in (" & strParTable & " ))")
                        strSQL = Replace(strSQL, "[选择替换的过条件2]", "  and e.分类id in (" & strParTable & " )")
                    End If
                End If
...
'功能：装入ListView数据
    ...
    For i = 1 To mrsDetail.RecordCount
        For j = 0 To mrsDetail.Fields.Count - 1
            If (Not mrsDetail.Fields(j).Name Like "*ID" Or mrsDetail.Fields(j).Name = "病人ID") And mrsDetail.Fields(j).Name <> "末级" And mrsDetail.Fields(j).Name <> mstrCheck Then
