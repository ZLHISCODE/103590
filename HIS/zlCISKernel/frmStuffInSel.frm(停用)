...
Private Sub Form_Load()
    ...
    dtpEnd.MaxDate = zlDatabase.Currentdate
...
Private Function FillData(Optional blnDefault As Boolean = True) As Boolean
    ...
    "           A.实际数量  as 入库数量,LTrim(To_Char(A.零售价,'999999" & gstrDecPrice & "'))  as 入库零售价,A.零售金额 as 入库零售金额, " & _
    "           to_char(nvl(D.可用数量,0),'9999990.00000') as 本次记帐数量,D.商品条码,D.内部条码, " & _
    "           Decode(B.是否变价,1,'时价',LTrim(To_Char(C1.现价,'999999" & gstrDecPrice & "'))) as 单价," & _
    ...
    "  Order By No,序号"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, mlng虚拟库房ID, CStr(Trim(txtNO.Text)), _
...
Private Sub MakeNO(ByVal intBillID As Integer, ByVal lng科室id As Long, ByRef strNo As String)
    ...
    On Error GoTo errH
    Set rsTemp = zlDatabase.OpenSQLRecord("Select 编号规则 From 号码控制表 Where 项目序号=[1]", "获取单据规则", intBillID)
    ...
    If Nvl(rsTemp!编号规则, 0) = 2 And lng科室id <> 0 Then
        ...
        Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, "获取部门性质", lng科室id)
        If rsTmp.EOF Then
            bln年度 = True
        Else
            bln年度 = False
    ...
    ElseIf rsTemp!编号规则 = 2 Then
        If rsTemp.State = 1 Then rsTemp.Close
        Set rsTemp = zlDatabase.OpenSQLRecord("Select 编号 From  科室号码表 Where 项目序号=[1] and nvl(科室ID,0)=[2]", "获取科室编号", intBillID, lng科室id)
