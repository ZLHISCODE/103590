...
Private mcurCardObject As Card
...
Private mobjThirdInterface As clsThirdSwapInterface '三方接口
Private mcnOracle As ADODB.Connection
Private mobjDataBase As clsDataBase
Private mobjOneDataObject As clsOneCardDataObject   '一卡通数据对象

'---------------------------------------------------------------------
Public Function zlBrushPay(ByVal cnOracle As ADODB.Connection, frmMain As Object, _
    ByVal lngModule As Long, _
    ByVal objCardObject As Card, _
    ...
    Call zlGetOneDataBase(cnOracle, mobjDataBase)
    Call zlGetOneCardDataObject(cnOracle, mobjOneDataObject)
...
Private Sub InitFace()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If Not mcurCardObject Is Nothing Then
        chk退现.Visible = mbln退费 And mcurCardObject.是否退现 = 1
        lblType.Caption = "请刷" & mcurCardObject.名称
    Else
    ...
    mblnPosPass = Val(mobjDataBase.GetPara(Val("276-消费卡刷卡消费须定位到密码框"), glngSys, , "1")) = 1
End Sub
...
Private Sub lbl卡号_Click()
    ...
    If Not mcurCardObject.是否接触式读卡 Then Exit Sub
...
'    End If
    ...
    If mcurCardObject.CardObject.zlReadCard(Me, mlngModule, False, strExpand, strCardNo, strOutXml) = False Then Exit Sub
...
Private Function SetBrushObject() As Boolean
    ...
    End If
    mblnPassInputCardNo = mcurCardObject.卡号密文规则 <> "" And mcurCardObject.卡号密文规则 <> "0"
    Me.Caption = mcurCardObject.名称
    With mcurCardObject
    ...
    If mcurCardObject.是否接触式读卡 Then
        lbl卡号.BorderStyle = 1
    Else
        lbl卡号.BorderStyle = 0
    End If
    txt卡号.Locked = Not (mcurCardObject.是否刷卡 Or mcurCardObject.是否扫描)
...
Private Function isValied() As Boolean
    ...
    If zlStringEncode(txtPass.Text) <> txtPass.Tag Then
...
Private Sub txt卡号_GotFocus()
    ...
    On Error GoTo errHand
    Call TxtSelAll(txt卡号)
    If Not mobjICCard Is Nothing And mlngCardTypeID = 0 Then Call mobjICCard.SetEnabled(True)
    tmrMain.Enabled = Val(tmrMain.Tag) <> 0 And Trim(txt卡号.Text) = ""
    
    If mobjThirdInterface Is Nothing Then
        Set mobjThirdInterface = New clsThirdSwapInterface ' CreateObject("zl9CardSquare.CardSquare")
        '初始化射频卡对象
        If Err <> 0 Then Exit Sub
        mobjThirdInterface.zlInitComponents Me, mlngModule, glngSys, gstrDBUser, gcnOracle
        If mobjCommEvents Is Nothing Then Set mobjCommEvents = New zl9CommEvents.clsCommEvents
    End If
    
    If mcurCardObject.是否非接触式读卡 Then mobjThirdInterface.SetEnabled True
    '85565:李南春,2015/7/21,调用刷卡接口
    Err = 0: On Error Resume Next
    
    If mcurCardObject.接口序号 = 0 Or mcurCardObject.接口程序名 = "" Then Exit Sub
    If Not (mcurCardObject.是否刷卡 Or mcurCardObject.是否扫描) Then Exit Sub
    
    Call mobjThirdInterface.zlSetBrushCardObject(mcurCardObject.接口序号, txt卡号, strExpend, _
                                        mcurCardObject.消费卡)
    If mobjCommEvents Is Nothing Then Set mobjCommEvents = New zl9CommEvents.clsCommEvents
    mobjThirdInterface.zlInitEvents Me.hWnd, mobjCommEvents
    Exit Sub
errHand:
    If mobjDataBase.ErrCenter() = 1 Then
        Resume
...
Private Sub txt卡号_KeyPress(KeyAscii As Integer)
    ...
    blnCard = InputIsCard(txt卡号, KeyAscii, mblnPassInputCardNo)
    txt卡号.PasswordChar = IIf(mblnPassInputCardNo, "*", "")
'
'    If lbl卡号.BorderStyle = 1 Then
'        '只能读卡，不能接收输入
...
'    End If
    ...
    If CheckBrush消费卡(strCardNo) = False Then
        ...
        Exit Sub
    End If
    txt卡号.Text = strCardNo ' GetCardNODencode(strCardNo, mlngCardTypeID, mcurCardObject.卡号密文规则, mbln消费卡)
    txt卡号.Tag = strCardNo
...
Public Function CheckBrushSquareCard(ByVal lngCardTypeID As Long, _
   ...
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Dim varData As Variant, dblMoney As Double, bln特定病人 As Boolean
    Dim str业务场合 As String, var费用来源 As Variant, strErrMsg As String
    Dim lng病人ID As Long
    dbl实际支付 = dbl本次支付额
    ...
    If zl_Exsesvr_GetConsumerCardInfo(lngCardTypeID, strCardNo, dbl余额, lng消费卡ID, strPassWord, str限制类别, str业务场合, lng病人ID, bln特定病人, strErrMsg) = False Then Exit Function
    ...
    End If
    '刷多次时,不能存在不同类别的刷卡情况
    If Not CheckTypeValied(lngCardTypeID, strCardNo, str限制类别) Then Exit Function

    '检查限制刷卡额
    Set rs收费类别 = mobjOneDataObject.zlGet收费类别
    str限制类别 = mobjOneDataObject.zlGetFeeTypeFromNameToCode(str限制类别)
...
Private Function Get收费限制类别_名称() As String
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Set rsTemp = mobjOneDataObject.zlGet收费类别
    rsTemp.Filter = 0
