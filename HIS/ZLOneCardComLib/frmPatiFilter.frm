...
Private mbytType As Byte '入:病人清单类型0-所有,1-在院,2-出院,3-门诊,4-留观
Private mstrFilter As String '出:条件
Private mbytInFun As Byte '0-普通调用,1-特殊病人过滤调用

Private Const mstrIDKind = "1-姓名;2-就诊卡号,3-门诊号;4-医保号;5-身份证号;6-IC卡号"
...
Public Function zlShowCard(ByVal frmMain As Object, ByVal bytType As Byte, ByRef cllFilter As Collection, ByVal cnOracle As ADODB.Connection, _
    Optional bytInFun As Byte = 0) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:显示过滤界面
    '入参:objPati-病人信息集
    '     cllFilter-当前查询的过滤条件
    '     bytType-入:病人清单类型0-所有,1-在院,2-出院,3-门诊,4-留观
    '     bytInFun-'0-普通调用,1-特殊病人过滤调用
    '出参:cllFilter-返回过滤条件
    ...
    mblnOK = False
    mbytInFun = bytInFun: mbytType = bytType
...
Private Sub InitFilter()
    ...
    '功能:初始化过滤条件
    '编制:刘兴洪
    '日期:2019-10-30 16:29:58
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim i As Long, j As Long
    
    On Error GoTo errHandle
    
    If mcllFilter Is Nothing Then Exit Sub
    For i = 1 To mcllFilter.count
        
        Select Case mcllFilter(i)(0)
        Case "登记时间"
            chk登记.value = 1
            dtp登记B.value = CDate(mcllFilter(i)(1))
            dtp登记E.value = CDate(mcllFilter(i)(2))
        Case "出生日期"
            chk出生.value = 1
            dtp出生B.value = CDate(mcllFilter(i)(1))
            dtp出生E.value = CDate(mcllFilter(i)(2))
        Case "入院日期"
            chk入院.value = 1
            dtp入院B.value = CDate(mcllFilter(i)(1))
            dtp入院E.value = CDate(mcllFilter(i)(2))
        Case "出院日期"
            chk入院.value = 1
            dtp出院B.value = CDate(mcllFilter(i)(1))
            dtp出院E.value = CDate(mcllFilter(i)(2))
        Case "住院号"
            txt住院号.Text = mcllFilter(i)(1)
        Case "性别"
             For j = 0 To cbo性别.ListCount
                If NeedName(cbo性别.Text) = mcllFilter(i)(1) Then
                    cbo性别.ListIndex = j: Exit For
                End If
             Next
        Case "区域"
            txt区域.Text = mcllFilter(i)(1)
        Case "特殊病人编号"
           txt编号.Text = mcllFilter(i)(1)
        Case "姓名"
             txtIdentity.Text = Replace(mcllFilter(i)(1), "%", "")
             cboIDKind.ListIndex = 0
        Case "就诊卡号"
            txtIdentity.Text = mcllFilter(i)(1)
            cboIDKind.ListIndex = 1
        Case "医保号"
            txtIdentity.Text = mcllFilter(i)(1)
            cboIDKind.ListIndex = 2
        Case "身份证号"
            txtIdentity.Text = mcllFilter(i)(1)
            cboIDKind.ListIndex = 3
        Case "IC卡号"
            txtIdentity.Text = mcllFilter(i)(1)
            cboIDKind.ListIndex = 3
        End Select
    ...
    Exit Sub
errHandle:
    If gobjOneDataObject.ErrCenter() = 1 Then
        Resume
    End If
End Sub
...
Private Sub Form_Load()
    ...
    Call InitFilter '初始化过滤条件
End Sub

Public Function MakeFilter(ByRef cllFilter As Collection) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:构建查询条件
    '出参:cllFilter-返回的过滤条件
    '返回:获取成功，返回true,否则返回False
    '编制:刘兴洪
    '日期:2019-10-30 15:56:26
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Set cllFilter = New Collection
     If chk登记.value = 1 Then
        cllFilter.Add Array("登记时间", Format(dtp登记B.value, "yyyy-mm-dd HH:MM:SS"), Format(dtp登记E.value, "yyyy-mm-dd HH:MM:SS")), "登记时间"
    End If
    If chk出生.value = 1 Then
        cllFilter.Add Array("出生日期", Format(dtp出生B.value, "yyyy-mm-dd HH:MM:SS"), Format(dtp出生E.value, "yyyy-mm-dd HH:MM:SS")), "出生日期"
    End If
    If chk入院.value = 1 Then
        cllFilter.Add Array("入院日期", Format(dtp入院B.value, "yyyy-mm-dd HH:MM:SS"), Format(dtp入院E.value, "yyyy-mm-dd HH:MM:SS")), "入院日期"
    End If
    
    If chk出院.value = 1 Then
        cllFilter.Add Array("出院日期", Format(dtp出院B.value, "yyyy-mm-dd HH:MM:SS"), Format(dtp出院E.value, "yyyy-mm-dd HH:MM:SS")), "出院日期"
    End If
    
    If txt住院号.Text <> "" Then
        Set cllTemp = New Collection
        cllTemp.Add Array("住院号", txt住院号.Text)
        lng病人Id = zl_CisSvr_GetPatiID(cllTemp)
        If lng病人Id = 0 Then
            MsgBox "未找到符合条件的病人信息，请检查！", vbInformation + vbOKOnly, gstrSysName
            Exit Function
        End If
        cllFilter.Add Array("病人ID", lng病人Id), "病人ID"
    End If
    
    If cbo性别.ListIndex <> 0 Then
        cllFilter.Add Array("性别", NeedName(cbo性别.Text)), "性别"
    End If
    If Trim(txt区域.Text) <> "" Then
        cllFilter.Add Array("区域", NeedName(txt区域.Text)), "区域"
    End If
    
    '该条件仅用于特殊病人过滤
    If txt编号.Visible Then
        If txt编号.Text <> "" Then
            cllFilter.Add Array("特殊病人编号", txt编号.Text), "特殊病人编号"
        End If
    Else '不同的查看范围时条件不同
        If cbo费别.ListIndex <> 0 Then
            cllFilter.Add Array("费别", NeedName(cbo费别.Text)), "费别"
        End If
    End If
        ...
        Select Case Val(cboIDKind.Text) '"1-姓名;2-就诊卡;3-门诊号;4-医保号;5-身份证号;6-IC卡号"
            Case 1
                If chk登记.value = 1 Or chk入院.value = 1 Or chk出院.value = 1 Then
                    cllFilter.Add Array("姓名", txtIdentity.Text & "%"), "姓名"
                Else
                    cllFilter.Add Array("姓名", txtIdentity.Text), "姓名"
                End If
            Case 2
                cllFilter.Add Array("就诊卡号", txtIdentity.Text), "就诊卡号"
            Case 3
                cllFilter.Add Array("门诊号", txtIdentity.Text), "门诊号"
            Case 4
                cllFilter.Add Array("医保号", txtIdentity.Text), "医保号"
            Case 5
                cllFilter.Add Array("身份证号", txtIdentity.Text), "身份证号"
            Case 6
                cllFilter.Add Array("IC卡号", txtIdentity.Text), "IC卡号"
        End Select
    End If
    MakeFilter = True
End Function
