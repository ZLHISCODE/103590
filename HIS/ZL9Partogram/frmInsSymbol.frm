...
Private Sub tabCard_Click()
    ...
    Case "标点符号", "单位符号", "数字序号", "数学符号", "特殊符号"
        ...
        Case "数学符号"
            strTemp = CON数学符号
        Case "特殊符号"
            strTemp = CON特殊符号 + IIf(blnEstopMedi, "", CON医学符号)
        End Select
...
Public Function ShowMe(Optional EstopMedi As Boolean, Optional ByVal bytSex As Byte = 0) As String
    '功能：显示本对话框
    '参数：
    '   EstopMedi,是否禁止医学符号
    ...
    If blnEstopMedi Then
    
        For intLoop = 1 To Me.tabCard.Tabs.Count
            If Me.tabCard.Tabs(intLoop).Key = "恒牙标注" Then
                Me.tabCard.Tabs.Remove "恒牙标注"
                Exit For
            End If
        Next
        
        For intLoop = 1 To Me.tabCard.Tabs.Count
            If Me.tabCard.Tabs(intLoop).Key = "乳牙标注" Then
                Me.tabCard.Tabs.Remove "乳牙标注"
                Exit For
            End If
        Next

        Me.tabCard.TabFixedWidth = 1800
    End If
