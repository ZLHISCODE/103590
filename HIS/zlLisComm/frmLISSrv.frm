...
'    Next
    ...
    Exit Sub
errH:
    WriteLog "frmLISSrv", "mnuReLoad_Click", LOG_错误日志, Err.Description
End Sub
...
'    If gobjComLib.ErrCenter() = 1 Then Resume
    ...
    Me.stbThis.Panels(3).Text = "错误：" & Format(lngErrCounts, "@@@@@@") & "条"
    Call WriteLog("frmLISSrv", "ListSeq", LOG_错误日志, Err.Description)
End Sub
...
DBError:
    lngErrCounts = lngErrCounts + 1
    Me.stbThis.Panels(3).Text = "错误：" & Format(lngErrCounts, "@@@@@@") & "条"
    Call WriteLog("frmLISSrv", "ListResult", LOG_错误日志, Err.Description)
End Sub
...
Private Sub timConn_Timer()
    ...
    Exit Sub
errH:
    WriteLog "frmLISSrv", "timConn_Timer", LOG_错误日志, Err.Description
    i = 0
    Do While i <= 30
        ...
        Else
            WriteLog "frmLISSrv", "timConn_Timer", LOG_错误日志, "本次重试次数=" & i
            Exit Do
        End If
        i = i + 1
    Loop
