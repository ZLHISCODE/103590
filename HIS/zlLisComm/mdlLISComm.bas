...
Public Sub WriteLog(ByVal ModuleName As String, ByVal strFunction As String, ByVal ErrorType As Integer, ByVal ErrorDesc As String)
    'Module:模块名称
    'strFunction:过程或函数名
    'ErrorType:日志类型
    'errorDesc:错误信息或日志信息
    
    Call WriteTxtLog(ErrorType, ModuleName, strFunction, ErrorDesc)
...
132     If objDevice Is Nothing Then Call WriteLog("mdlLISComm", "ResultFromFile", LOG_错误日志, "解析程序:" & strDevice & "创建失败!" & vbNewLine & Err.Description)
    
134     Call WriteLog("mdlLISComm", "ResultFromFile", LOG_通讯日志, "strFile:" & strFile & vbNewLine & "strSampleNO:" & strSampleNO & vbNewLine & "dtStart:" & CStr(dtStart) & vbNewLine & "dtEnd:" & CStr(dtEnd))
...
142             Call WriteLog("mdlLISComm", "ResultFromFile", LOG_通讯日志, "记录" & i & ":" & aRecord(i))
...
150                     Call WriteLog("mdlLISComm", "ResultFromFile", LOG_错误日志, strErr & vbCrLf & gstrSQL)
                    End If
                End If
            Next
        End If
...
errH:
    If CStr(Erl()) = 138 And Err.Number = 9 Then
        Call WriteLog("mdlLISComm", "ResultFromFile", LOG_错误日志, CStr(Erl()) & "行出现错误：  没有返回解码结果")
    Else
152     Call WriteLog("mdlLISComm", "ResultFromFile", LOG_错误日志, CStr(Erl()) & "行出现错误：  " & Err.Description)
    End If
End Sub
...
Private Sub WriteTxtLog(ByVal lng类型 As String, ByVal ModuleName As String, ByVal strFunction As String, ByVal str内容 As String)
    ...
    '调用公共方法写入日志
    Call gobjComLib.LogWrite("LIS老版通讯程序调试日志", ModuleName, strFunction, strType & str内容)
...
172                         WriteLog "mdlLISComm", "SaveImg", LOG_通讯日志, "开始时间"
...
180                                 gobjDatabase.ExecuteProcedure Replace(strSQL(IntCount), "Call", ""), "保存图像数据"
                                End If
                            Next
182                         WriteLog "mdlLISComm", "SaveImg", LOG_通讯日志, "结束时间"
                        End If
                    Else
                        '保存到FTP
...
194                         gobjDatabase.ExecuteProcedure gstrSQL, "保存检验图形数据"
                        Else
196                         WriteLog "mdlLISComm", "SaveImg", LOG_通讯日志, strUploadOk
                        End If
                    End If
...
210                         Call WriteLog("mdlLISComm", "SaveImg", LOG_错误日志, "文件" & strPicPath & "估计还有其他程序在用，未能删除，需事后手工删除！")
                        End If
                    End If
                End If
            Else
...
254                     gobjDatabase.ExecuteProcedure gstrSQL, "保存检验图形数据"
                    Else
256                     WriteLog "mdlLISComm", "SaveImg", LOG_通讯日志, strUploadOk
                    End If
258                 If gobjFSO.FileExists(strBMPFile) Then gobjFSO.DeleteFile strBMPFile
        ...
        Exit Sub
ErrHandle:
260     Call WriteLog("mdlLISComm", "SaveImg", LOG_错误日志, Err.Description)
...
Private Function zlLisBlobSql(ByVal Action As Long, ByVal KeyWord As String, ByVal strFile As String, ByVal layOut As Integer, ByRef arySql() As String) As Boolean
    ...
    lngFileNum = FreeFile
    WriteLog "mdlLISComm", "zlLisBlobSql", LOG_通讯日志, "开始时间"
    ...
    Next
    Close lngFileNum
    WriteLog "mdlLISComm", "zlLisBlobSql", LOG_通讯日志, "结束时间"
    zlLisBlobSql = True
    Exit Function
...
172         gobjDatabase.ExecuteProcedure gstrSQL, "生成条码标本"
        ...
        Exit Function
DBErr:
174     Call WriteLog("mdlLISComm", "CreateSample", LOG_错误日志, CStr(Erl()) & "行," & Err.Description)
End Function
...
296                               strItemsInfo = strItemsInfo & "," & strItemCode & "(未对码)=" & aItem(j + 1)
    ...
    '                            mcnAccess.Execute strSql
                           ...
                           End If
                       Next
298                    If str未知项 <> "" Then Call WriteLog("mdlLISComm", "SaveToDataBase", LOG_未知项, str未知项)
...
434    strlogs = strLog
       Exit Function
DBError:
436   Call WriteLog("mdlLISComm", "SaveToDataBase", LOG_错误日志, CStr(Erl()) & "行出现错误：  " & Err.Description)
...
126        strRetuInfo = strReturn
        ...
        Exit Sub
errH:
128    WriteLog "mdlLISComm", "AutoQCCompute", LOG_错误日志, CStr(Erl()) & "行出现错误：  " & Err.Description
End Sub
...
146         If strErrMessage <> "" Then VerifyAuditingRule = 1
        ...
        Exit Function
errH:
148     WriteLog "mdlLISComm", "VerifyAuditingRule", LOG_错误日志, CStr(Erl()) & "行出现错误：  " & Err.Description
End Function
...
104             WriteLog "mdlLISComm", "CheckSample", LOG_错误日志, "单据未收费，不能进行审核！"
                Exit Function
            End If
        End If
...
112         WriteLog "mdlLISComm", "CheckSample", LOG_错误日志, "病人本次住院的病案已提交审查，不能进行审核！"
            Exit Function
        End If
114     If CheckExesState(lngID) = False Then
116         WriteLog "mdlLISComm", "CheckSample", LOG_错误日志, "当前住院病人还有划价单未审核，但已出院或预出院！"
            Exit Function
        End If
...
120     CheckSample = True
        Exit Function
errH:
122     Call WriteLog("mdlLISComm", "CheckSample", LOG_错误日志, CStr(Erl()) & "行出现错误：  " & Err.Description)
End Function
...
146     CheckChargeState = True
        ...
        Exit Function
errH:
148     Call WriteLog("mdlLISComm", "CheckChargeState", LOG_错误日志, CStr(Erl()) & "行出现错误：  " & Err.Description)
End Function
...
108     CheckExesState = rsTmp.EOF
        Exit Function
errH:
110     Call WriteLog("mdlLISComm", "CheckExesState", LOG_错误日志, CStr(Erl()) & "行出现错误：  " & Err.Description)
End Function
...
128             WriteLog "mdlLISComm", "CheckPatientInfo", LOG_错误日志, "发现检验信息中的病人信息和病人信息中病人信息不一致!"
...
138     CheckPatientInfo = False
        ...
        Exit Function
errH:
140     Call WriteLog("mdlLISComm", "CheckPatientInfo", LOG_错误日志, CStr(Erl()) & "行出现错误：  " & Err.Description)
End Function
