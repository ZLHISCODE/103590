...
Public Function SendSample(ByVal lngDeviceID As Long, ByVal strSampleDate As String, ByVal strSampleNO As String, Optional strAdviceIDs As String = "", Optional ByVal blnUndo As Boolean = False, Optional ByVal iType As Integer = 0) As Boolean
    ...
    Exit Function
errH:
    WriteLog "ctrlComm", "SendSample", LOG_错误日志, Err.Description
End Function
...
240             timInData.Enabled = True
        ...
        Exit Sub
errH:
242     WriteLog "ctrlComm", "initcontrl", LOG_错误日志, CStr(Erl()) & "行," & Err.Description
End Sub
...
errH:
    ...
    If Err.Number <> 9 Then
        WriteLog "ctrlComm", "timInData_Timer", LOG_错误日志, Err.Description
    End If
End Sub
...
'                    ReadResultDirFile = strLine
                '将读取的文件内容保存到数据库
                If strLine <> "" Then
                    WriteLog "ctrlComm", "ReadResultDirFileIQ", LOG_通讯日志, strLine
    ...
    Exit Function
errH:
    WriteLog "ctrlComm", "ReadResultDirFileIQ", LOG_错误日志, Err.Description
End Function
...
Private Function ReadResultDirFileRE(ByVal blnDelete As Boolean) As String
        ...
        If objOneFile.Name Like "RE" & Format(Now, "yyyyMMdd") & "_*.txt" And objOneFile.DateCreated < Format(Now - 0.00002, "yyyy-mm-dd hh:mm:ss") Then
            ...
            If mfsoTmp.FileExists(strFileName) Then
                ...
                If strLine <> "" Then
                    ...
                    WriteLog "ctrlComm", "ReadResultDirFileRE", LOG_通讯日志, strLine
    ...
    Exit Function
errH:
    WriteLog "ctrlComm", "ReadResultDirFileRE", LOG_错误日志, Err.Description
End Function
...
Private Function WriteSampleInfo(ByVal strResult As String) As Boolean
        ...
        For i = 0 To UBound(aSamples)
            ...
            If UBound(aSampleInfo) > 0 Then
                ...
                If strSampleInfo <> "" Then
                    ...
                    WriteLog "ctrlComm", "WriteSampleInfo", LOG_通讯日志, gstrSQL
                    gobjDatabase.ExecuteProcedure gstrSQL, "打传送标志"
                End If
    ...
    Exit Function
errH:
    WriteLog "ctrlComm", "WriteSampleInfo", LOG_错误日志, Err.Description
End Function
...
Private Sub WriteToSendDir(ByVal strInput As String, Optional ByVal strFileType As String)
    ...
    Exit Sub
errH:
    WriteLog "ctrlComm", "WriteToSendDir", LOG_错误日志, Err.Description
End Sub
...
124             Call WriteLog("ctrlComm", "GetSampleInfo", LOG_通讯日志, "按标本查:" & lngDeviceID & "," & strSampleNO & "," & strSampleDate & "," & iType & "," & mlng允许发送已核收标本)
...
194                     rsTmp.MoveNext
            ...
            Next
        End If
196     Call WriteLog("ctrlComm", "getSampleInfo", LOG_通讯日志, GetSampleInfo)
        Exit Function
DBErr:
198     GetSampleInfo = ""
200     Call WriteLog("ctrlComm", "GetSampleInfo", LOG_错误日志, CStr(Erl()) & "行出现错误：  " & Err.Description)
End Function
...
128             Call WriteToSendDir(strlogs, "SaveDataLog")
        ...
        Exit Function
hErr:
130     Call WriteLog("ctrlComm", "GetSampleInfo", LOG_错误日志, CStr(Erl()) & "行出现错误：  " & Err.Description)
End Function
