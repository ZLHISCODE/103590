...
Private Sub cmdDel_Click()
    ...
    gstrSQL = "zl_医技执行房间_Delete(" & Val(Me.lblDept.Tag) & ",'" & Trim(Me.txtName.Text) & "')"
...
Private Sub cmdSave_Click()
    ...
    If Me.lblName.Tag = "" Then
        gstrSQL = "zl_医技执行房间_Insert(" & Val(Me.lblDept.Tag) & ",'" & Trim(Me.txtName.Text) & "')"
    Else
        gstrSQL = "zl_医技执行房间_Update(" & Val(Me.lblDept.Tag) & ",'" & Trim(Me.lblName.Tag) & "','" & Trim(Me.txtName.Text) & "')"
    End If
...
Private Sub zlRoomRef()
    ...
    gstrSQL = "Select 执行间 From 医技执行房间 where 科室id=[1]"
    Set rsTemp = OpenSQLRecord(gstrSQL, Me.Caption, Val(Me.lblDept.Tag))
    Me.lvwRoom.ListItems.Clear
    With rsTemp
        Do While Not .EOF
