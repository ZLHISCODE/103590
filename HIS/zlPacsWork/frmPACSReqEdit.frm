...
Private mblnCheck As Boolean '是否处理开始检查
Private aDevices() As String
Private strImageType As String '影像类别
Private lng标识号 As Long
...
Private Sub cbo医生_Validate(Cancel As Boolean)
    ...
    If Not rsTmp Is Nothing Then
        cbo医生.Text = Nvl(rsTmp!简码) & "-" & rsTmp!姓名
    Else
...
Private Sub chkUnicode_Click()
    If mblnCheck Then
        txtItem(7).Text = Next检查号(lngClinicID, PatientID, chkUnicode.Value = 1)
    End If
End Sub
...
'        Call SQLTest
    ...
    If mblnCheck Then
        ...
        strSQL = "ZL_影像检查_BEGIN('" & cboRoom.Text & "'," & txtItem(7).Text & "," & lngAdviceID & "," & lngSendNO & ",'" & strImageType & "','" & _
            Me.txt姓名 & "','" & Trim(txtItem(3)) & "','" & Trim(Me.cbo性别.Text) & "','" & _
            Me.txt年龄 & "'," & IIf(IsNull(DTBirth.Value), "Null", "to_Date('" & Format(DTBirth.Value, "yyyy-MM-dd") & "','YYYY-MM-DD')") & ",'" & txtItem(5) & "','" & txtItem(6) & "'," & _
        ...
        If optMatch(1).Value Then '门诊/住院号
            Set rsTmp = OpenSQLRecord(strSQL, Me.Caption, lng标识号, strImageType)
        End If
        ...
        If rsTmp.RecordCount = 1 Then '将图像和检查自动匹配
            strSQL = "ZL_影像检查_SET(" & lngAdviceID & "," & lngSendNO & ",'" & _
                rsTmp("ID") & "')"
            ExecuteProc strSQL, Me.Caption
        End If
    End If
...
'返回：本次录入是否有效
    ...
    If cbo医生.ListCount <= 0 Then
        MsgBox "当前开单科室无医生,请检查!", vbInformation, gstrSysName
        Exit Function
    End If
...
Private Sub txt姓名_Validate(Cancel As Boolean)
    ...
    If rsTmp.EOF Then
        ...
        Me.txt年龄 = ""
        Me.txt身份证号 = ""
        If InStr("+-*.", Left(Me.txt姓名.Text, 1)) > 0 Then Cancel = True 'Me.txt姓名.Text = "": Me.txt姓名.SetFocus
        
        PatientID = 0: PatientType = 1: CheckID = "": lng标识号 = 0
    ...
    End If
    '英文名
    If mblnCheck Then txtItem(3).Text = UCase(Replace(zlCommFun.mGetFullPY(Trim(txt姓名)), vbCrLf, ""))
    If mblnCheck Then txtItem(7).Text = Next检查号(lngClinicID, PatientID, chkUnicode.Value = 1)
...
'功能：读取病人信息，并显示该病人存在的医嘱时间
    ...
    If iInputType = 0 Then '刷卡
        strSQL = "Select Decode(A.当前科室id,Null,1,2) As PatientType,Nvl(A.住院次数,0) As 主页ID,Decode(A.当前科室id,Null,Nvl(B.执行部门ID,0),A.当前科室id) As 病人科室," & _
            "Decode(A.当前科室id,Null,A.门诊号,A.住院号) As 标识号,A.*" & _
    ...
    ElseIf iInputType = 1 Then '病人ID
        strSQL = "Select Decode(A.当前科室id,Null,1,2) As PatientType,Nvl(A.住院次数,0) As 主页ID,Nvl(A.当前科室id,0) As 病人科室," & _
            "Decode(A.当前科室id,Null,A.门诊号,A.住院号) As 标识号,A.*" & _
    ...
    ElseIf iInputType = 4 Then '挂号单
        ...
        strSQL = "Select Decode(B.主页ID,Null,1,2) As PatientType,Nvl(B.主页ID,0) As 主页ID,Nvl(B.执行部门ID,0) As 病人科室," & _
            "Decode(B.主页ID,Null,A.门诊号,A.住院号) As 标识号,A.*" & _
    ...
    ElseIf iInputType = 5 Then '收费单据号
        ...
        strSQL = "Select Decode(B.主页ID,Null,1,2) As PatientType,Nvl(B.主页ID,0) As 主页ID,B.开单部门ID As 病人科室,B.开单人 As 医生,B.姓名,B.性别,B.年龄," & _
            ...
            "A.国籍,A.婚姻状况,A.民族,A.职业,Decode(B.主页ID,Null,A.门诊号,A.住院号) As 标识号 From 病人信息 A,病人费用记录 B" & _
...
Public Function Next检查号(ByVal lngItemId As Long, ByVal lngPatientID As Long, Optional ByVal blnUnicode As Boolean = False) As Double
...
ReStart:
    ...
    If Not blnUnicode Then '按类别编号
        strSQL = "Select 检查号 From 影像检查记录 A,病人医嘱记录 B,影像检查项目 C" & _
            " Where A.医嘱ID=B.ID And B.病人ID=[1] And A.影像类别+0=C.影像类别 And C.诊疗项目ID=[2] Order By B.停嘱时间 Desc"
        Set rsCtrl = OpenSQLRecord(strSQL, "获取检查号", lngPatientID, lngItemId)
        If Not rsCtrl.EOF Then
            lngNO = Val(Nvl(rsCtrl("检查号"), 0))
        Else
            strSQL = "Select A.最大号码 From 影像检查类别 A,影像检查项目 B Where A.编码=B.影像类别 And B.诊疗项目ID=[1]"
            Set rsCtrl = OpenSQLRecord(strSQL, "获取检查号", lngItemId)
    ...
    Else '按执行科室编号
        ...
        Set rsCtrl = OpenSQLRecord(strSQL, "获取检查号", lngExeDept, lngPatientID)
        If rsCtrl.EOF Then '取类别的最大号码
            ...
            Set rsCtrl = OpenSQLRecord(strSQL, "获取检查号", lngExeDept)
            If rsCtrl.EOF Then
                Next检查号 = 0
                Exit Function
            End If
...
Private Sub ReadCheckData()
    ...
    strSQL = "Select Nvl(可行病检,0) As 可行病检,Nvl(可发胶片,0) As 可发胶片,Nvl(影像类别,'CT') As 影像类别 From 影像检查项目" & _
        " Where 诊疗项目ID=[1]"
