...
Private Function ReadCard(ByVal lng病人id As Long) As Boolean
    ...
    Set rsTmp = OpenSQLRecord(strSql, Me.Caption, lng病人id)
    ...
    txt户口邮编.Text = zlCommFun.NVL(rsTmp("户口邮编"))
    ...
    If zlCommFun.NVL(rsTmp("当前科室id"), 0) > 0 Then
        ...
        strSql = "Select B.费别 AS 住院费别 From 病人信息 A,病案主页 B " & _
                "Where A.病人ID=B.病人ID And Nvl(A.住院次数,0)=B.主页ID And A.病人ID=[1]"
        Set rsTmp = OpenSQLRecord(strSql, Me.Caption, lng病人id)
    ...
    strSql = "Select * From 病人余额 Where 性质=1 And 病人ID= [1] "
    Set rsTmp = OpenSQLRecord(strSql, Me.Caption, lng病人id)
