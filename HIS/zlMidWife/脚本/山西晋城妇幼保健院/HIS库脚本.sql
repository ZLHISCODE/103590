...
Select b.病人id, b.主页id, a.姓名, a.年龄, a.当前床号, a.住院号, c.诊断描述, 1 As Status, Null 
    ...
    If 操作类型_In = 1 Then
      ...
      If n_Count = 0 Then
        Insert Into Sol_Inf_Puerpera@ZLSOL_DBL
          (Pid, Tid, Name, Old, Bedno, Pno, Diagnosis,status)
          Select a.病人id, a.主页id, a.姓名, a.年龄, a.当前床号, a.住院号, c.诊断描述,0
...
Begin
  ...
  If n_Count = 1 Then
    If 操作类型_In = 1 Then
      Update Sol_Inf_PuerperaZLSOL_DBL Set Diagnosis = 诊断描述_In Where Pid = 病人id_In And Tid = 主页id_In;
    Elsif 操作类型_In = 2 Then
      Update Sol_Inf_PuerperaZLSOL_DBL Set Diagnosis = Null Where Pid = 病人id_In And Tid = 主页id_In;
    End If;
  End If;
