    ...
    Case Command.删除日志
        ...
        With vsf(0)
            ...
            If blnMuliSelect = True Then
                If MsgBox("您确认要删除已经勾选的接收日志吗？", vbQuestion + vbYesNo + vbDefaultButton2, ParamInfo.系统名称) = vbYes Then
                    ...
                    If gclsBusiness.SendLogEdit("Delete", rsPara) Then
    ...
    Case Command.标记重发
       ...
       Set rsPara = zlCommFun.CreateParameter
        ...
        With vsf(0)
            ...
            If blnMuliSelect = True Then
                If MsgBox("您确认要标记已经勾选的为需要重发吗？", vbQuestion + vbYesNo + vbDefaultButton2, ParamInfo.系统名称) = vbYes Then
                    ...
                    End If
                    Call zlCommFun.SetParameter(rsPara, "SendLog_SegmentCount", lngCount)
                    If gclsBusiness.SendLogEdit("Repeat", rsPara) Then
                        Call ExecuteCommand(Command.刷新日志)
                    End If
                End If
            ElseIf .TextMatrix(.Row, .ColIndex("id")) <> "" Then
                
                If MsgBox("您确认要标记当前选中消息为需要重发吗？", vbQuestion + vbYesNo + vbDefaultButton2, ParamInfo.系统名称) = vbYes Then
                    Call zlCommFun.SetParameter(rsPara, "SendLog_1", .TextMatrix(.Row, .ColIndex("id")))
                    Call zlCommFun.SetParameter(rsPara, "SendLog_SegmentCount", 1)
                    If gclsBusiness.SendLogEdit("Repeat", rsPara) Then
                        Call ExecuteCommand(Command.刷新日志)
    ...
    Case Command.刷新日志
        ...
        With vsf(0)
            ...
            If Trim(txtLocation.Text) = "" Then
                ExecuteCommand = mclsVsf(0).LoadDataSource(gclsBusiness.SendLogRead("FilterData", rsCondition))
            Else
    ...
    Case Command.刷新内容
        ...
        With vsf(0)
            ...
            If Trim(.TextMatrix(.Row, .ColIndex("id"))) <> "" Then
                 strTemp = gclsBusiness.GetSendLogContent(Trim(.TextMatrix(.Row, .ColIndex("id"))))
...
Private Function InitGrid() As Boolean
    ...
    With mclsVsf(0)
        ...
        Call .AppendColumn("发送时间", 1830, flexAlignLeftCenter, flexDTString, , "send_date", True)
        Call .AppendColumn("消息用户", 900, flexAlignLeftCenter, flexDTString, , "send_mipuser", True)
        Call .AppendColumn("产品用户", 1500, flexAlignLeftCenter, flexDTString, , "send_user", True)
        Call .AppendColumn("发送站点", 2400, flexAlignLeftCenter, flexDTString, , "send_station", True)
        Call .AppendColumn("发送模块", 1500, flexAlignLeftCenter, flexDTString, , "send_module", True)
        Call .AppendColumn("发送消息", 1500, flexAlignLeftCenter, flexDTString, , "item_identity", True)
        Call .AppendColumn("重发策略", 990, flexAlignLeftCenter, flexDTString, , "send_again", True)
        Call .AppendColumn("重发时间", 1800, flexAlignLeftCenter, flexDTString, , "send_again_date", True)
        Call .AppendColumn("重发次数", 900, flexAlignCenterCenter, flexDTString, , "send_again_number", True)
        Call .AppendColumn("发送状态", 900, flexAlignCenterCenter, flexDTString, , "send_state", True)
        Call .AppendColumn("说明", 1500, flexAlignLeftCenter, flexDTString, , "send_note", True)
    ...
    With mclsVsfXML
        ...
        Call .AppendColumn("节点名称", 3000, flexAlignLeftCenter, flexDTString, , "nodename", True)
...
Private Function InitCommandBar() As Boolean
    ...
    Set objControl = zlCommFun.NewToolBar(objBar, xtpControlButton, conMenu_Edit_Reuse, "标记重发", True)
    ...
    mstrFindKey = zlDataBase.GetPara("定位依据", ParamInfo.系统号, mlngModualCode, "发送站点")
    If mstrFindKey = "" Then mstrFindKey = "发送站点"
    ...
    objControl.IconId = 99999999
    Set objControl = mobjFindKey.CommandBar.Controls.Add(xtpControlButton, conMenu_View_LocationItem, "&3.发送站点"): objControl.Parameter = "发送站点"
    objControl.IconId = 99999999
    Set objControl = mobjFindKey.CommandBar.Controls.Add(xtpControlButton, conMenu_View_LocationItem, "&4.发送模块"): objControl.Parameter = "发送模块"
    objControl.IconId = 99999999
    Set objControl = mobjFindKey.CommandBar.Controls.Add(xtpControlButton, conMenu_View_LocationItem, "&5.发送消息"): objControl.Parameter = "发送消息"
    objControl.IconId = 99999999
...
Private Sub InitDockPannel()
    ...
    Set objPane = dkpMain.CreatePane(1, 100, 200, DockTopOf, Nothing)
    objPane.Title = "记录"
...
Private Sub cbsMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_File_Parameter
        Call frmSendLogPara.ShowConfigDialog(Me)
    '--------------------------------------------------------------------------------------------------------------
    ...
    Case conMenu_Edit_Reuse
        ...
        Call ExecuteCommand(Command.标记重发)
