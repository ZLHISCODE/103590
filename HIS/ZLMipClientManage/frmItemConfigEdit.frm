...
Private Function InitGrid() As Boolean
    ...
    With mclsVsf
        ...
        Call .AppendColumn("节点类型", 0, flexAlignLeftCenter, flexDTString, , "", True, , , True)
        ...
        vsf(1).OutlineCol = .ColIndex("节点标题")
        .IndicatorMode = 2
    ...
    If rsTmp.BOF = False Then
        ...
        If rsTmp.BOF = False Then
            Do While Not rsTmp.EOF
                mrsTrigger.AddNew
                mrsTrigger("序号").Value = zlCommFun.NVL(rsTmp("fld_order").Value)
                mrsTrigger("名称").Value = zlCommFun.NVL(rsTmp("fld_title").Value)
                rsTmp.MoveNext
            Loop
'            If mrsTrigger.RecordCount > 0 Then
'                mclsVsf.DropColData(mclsVsf.ColIndex("数据重复")) = mclsVsf.DropColData(mclsVsf.ColIndex("数据重复")) & "|[T.触发条件.Count]"
'            End If
        End If
...
Private Function InitCommandBar() As Boolean
    ...
    Set objControl = zlCommFun.NewToolBar(mobjToolBar, xtpControlButton, conMenu_Edit_Append, "格式")
    objControl.IconId = conMenu_Edit_NewItem
    Set objControl = zlCommFun.NewToolBar(mobjToolBar, xtpControlButton, conMenu_Edit_Transf_Save, "保存")
    
    Set objControl = zlCommFun.NewToolBar(mobjToolBar, xtpControlButton, conMenu_File_Exit, "退出")
    ...
    Set objControl = zlCommFun.NewToolBar(mobjToolBar, xtpControlButton, conMenu_View_Filter, "搜索")
    Set objControl = zlCommFun.NewToolBar(mobjToolBar, xtpControlButton, conMenu_View_Forward, "上一条")
    Set objControl = zlCommFun.NewToolBar(mobjToolBar, xtpControlButton, conMenu_View_Backward, "下一条")
    
    Set objControl = zlCommFun.NewToolBar(mobjToolBar, xtpControlLabel, 2, "层次:", True, , xtpButtonCaption)
    For intLoop = 1 To 10
    ...
    Set objControl = zlCommFun.NewToolBar(mobjToolBar, xtpControlButton, conMenu_View_Option, IIf(mbytMode = 1, "保存后继续新增", "保存后继续配置"))
        ...
        For lngLoop = 1 To .Rows - 1
            ...
            If InStr(UCase(.TextMatrix(lngLoop, .ColIndex("数据重复"))), UCase(".Count")) > 0 Then
                ...
                If InStr(strTemp, "[S.") > 0 Then
                    ...
                    End If
                Else
                    .TextMatrix(lngLoop, .ColIndex("数据重复_Key")) = strTemp
                End If
...
Private Sub cbo_Click()
    ...
    '------------------------------------------------------------------------------------------------------------------
    Case 1
        tvw.Nodes.Add , , "K6", "项目标识", "file", "file"
    ...
    Case 3          '触发条件
        If mrsTrigger.RecordCount > 0 Then
            ...
            Do While Not mrsTrigger.EOF
                ...
                tvw.Nodes.Add , , "K" & mrsTrigger("序号").Value, mrsTrigger("名称").Value, "file", "file"
