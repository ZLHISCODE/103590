...
Private Sub Form_Load()
    ...
    Else
        ...
        Else
            '无数据,提示,非取消退出
            If mbln输入内部条码 Or mbln条码扫描 Then
...
Private Function FillList(Optional ByVal blnClass As Boolean, Optional strIDs As String) As Boolean
    ...
    If mstr输入 <> "" Then
        ...
        If mbln输入内部条码 Then
            '备货卫材只能输内部条码
            strInput = " And M.内部条码=[4] "
        Else
            strInput = "   (A.编码 Like [6] Or B.名称 Like [7] Or B.简码 Like [7]) And B.码类=[8] " & strCheckBarCode
            If gobjCommFun.ActualLen(mstr输入) > 10 And gobjCommFun.IsCharChinese(mstr输入) = False Then
                '加入条码混合识别
                strInput = " And ((" & strInput & ") Or M.商品条码=[4] Or M.内部条码=[4])"
            Else
                strInput = " And " & strInput
            End If
        
            If mbln条码扫描 Then
                strInput = " And (M.商品条码=[4] Or M.内部条码=[4] )"
        ...
        If mint险类 = 0 Then
            ...
            "       M.批次,M.上次批号 As 批号,M.商品条码,M.内部条码,nvl(M.可用数量,0) as 可用库存, M.上次供应商ID " & _
    ...
    "               A.规格,A.产地,A.费用类型,A.医保大类,A.说明,A.批次,A.批号,A.商品条码,A.内部条码,A.是否变价,A.计算单位,A.可用库存, To_Char(A.可用库存,'9999990.00000'),F.名称"
    ...
    For i = 1 To vsItem.Cols - 1
        ...
        If vsItem.TextMatrix(0, i) Like "*ID" Or vsItem.TextMatrix(0, i) = "可用库存" _
            Or vsItem.ColKey(i) = "商品名" Or vsItem.ColKey(i) = "类别" Or vsItem.ColKey(i) = "批次" Then
