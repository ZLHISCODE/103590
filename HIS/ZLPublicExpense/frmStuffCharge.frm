                ...
                If mint记录性质 = 2 Then
                    If Not IsNull(mrsInfo!险类) And mobjBill.Details(Bill.Row).数次 <> 0 And MCPAR.实时监控 Then
                        If gclsInsure.CheckItem(mrsInfo!险类, 1, 0, MakeDetailRecord(mobjBill, NeedName(cbo开单人.Text), NeedName(cbo开单科室.Text), 2, 0, Bill.Row)) = False Then
                            ...
                            Bill.Text = "": Bill.TxtVisible = False: Exit Sub
                        End If
                    End If
                End If
    ...
    If Not mrsInfo.EOF Then
        ...
        If mint记录性质 = 2 Then
            ...
            If mbln费用登记 = False Then    '30504
                ...
                mbytWarn = BillingWarn(Me, mstrPrivs, mrsWarn, IIf(Nvl(mrsInfo!住院号) = "", "", mrsInfo!姓名 & "(住院号:" & mrsInfo!住院号 & " 床号:" & mrsInfo!床号 & ")"), cur余额, Val(Nvl(mrsInfo!当日额)) - mcurModiMoney, curTotal, _
    ...
    If Not rsTmp.EOF Then
        With mobjBill.Details(lngRow)
            ...
            If dblAllTime <> 0 Or Not .Detail.变价 Then
                ...
                End If
                Set rsPrice = gobjDatabase.OpenSQLRecord(gstrSQL, Me.Caption, .收费细目ID, mlng虚拟库房ID, dblAllTime, 0, .Detail.批次)
                If rsPrice.EOF Then
                    '获取价格失败
