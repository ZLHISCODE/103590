...
Private mbln分配票号 As Boolean, mblnSharedInvoice As Boolean
...
Private Sub mobjReport_BeforePrint(ByVal ReportNum As String, ByVal TotalPages As Integer, Cancel As Boolean, arrInvoice As Variant)
    ...
    End Select
    Call gobjDatabase.ExecuteProcedure(strSQL, "票据数据生成")
    mblnPrinted = True
    ...
    gobjDatabase.ExecuteProcedure strSQL, "凭条打印记录"

    '不严格控制票据时保存到注册表
    '更新本地票据
    If Not gblnBill挂号 Then
        If mblnSharedInvoice Then
            gobjDatabase.SetPara "当前收费票据号", mstrInvoice, glngSys, 1121
        Else
            gobjDatabase.SetPara "当前挂号票据号", mstrInvoice, glngSys, 1111
        End If
    End If
...
Private Function CheckInvoiceValied(Optional int张数 As Integer = 1, _
   ...
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    mlng领用ID = GetInvoiceGroupID(IIf(mblnSharedInvoice, 1, 4), int张数, mlng领用ID, mlngShareUseID, mstrInvoice, mstrUseType)
    '数据合法
...
ClearInvoice:
    ...
    gcnOracle.BeginTrans: blnTrans = True
        For i = 0 To UBound(Split(strClearNOs, ","))
            ...
            strSQL = "Zl_票据起始号_Update('" & strPrintNO & "',''," & IIf(mblnSharedInvoice, 1, 4) & ")"
