...
'LED语音报价控制
Public mblnLED As Boolean '是否使用Led显示
...
Public Function zlSquareAffirm(ByVal frmMain As Object, _
   ...
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    bln门诊留观使用预交款 = Val(zlDatabase.GetPara(323, glngSys)) <> 1
    If zlCheckCurPatiIsMzLg(lngPatiID) Then     '门诊留观病人使用预交款
       bln使用预交 = bln门诊留观使用预交款
    End If
    mbln使用预交 = bln使用预交
...
Private Function GetFeeData(ByVal lng病人ID As Long) As ADODB.Recordset
    ...
    If mstr医嘱IDs <> "" Then
        If zlGetVarBoundSQL(0, mstr医嘱IDs, strTableIDs, varPara(), 2) = False Then Exit Function
    End If
    If mstrInNos <> "" Then
    ...
    If mbytBillType <= 1 Then
        ...
        "       b.编码||'-'||Decode(a.是否保密, 1, '***', B.名称) As 项目," & vbNewLine & _
        ...
        "From 门诊费用记录 A,收费项目目录 B" & IIf(mstrInNos <> "", " ,单据 C", "") & vbNewLine & _
        "Where a.收费细目ID=b.ID And a.记录性质=1 And a.病人ID=[1] And (a.记录状态=0 Or a.记录状态=1 And a.结帐ID Is Null) " & _
        "      "
        If mstr医嘱IDs <> "" And mstrInNos <> "" Then
            '问题:49593
            strSfTable = strSfTable & " And (A.NO= C.Column_Value  or  A.NO in (Select Distinct NO From 门诊费用记录 J,医嘱 P Where J.病人ID=[1] and J.医嘱序号=P.Column_Value And J.记录性质=1  ))"
        ElseIf mstr医嘱IDs <> "" Then
            strSfTable = strSfTable & " And  A.NO in (Select Distinct NO From 门诊费用记录 J,医嘱 P Where J.病人ID=[1] and J.医嘱序号=P.Column_Value And J.记录性质=1)"
        ElseIf strTableNos <> "" Then
    ...
    If mbytBillType = 2 Or mbytBillType = 0 Then
        ...
        If mstr医嘱IDs <> "" And mstrInNos <> "" Then
            '问题:49593
            strJzTable = strJzTable & " And (A.NO= C.Column_Value  or  A.NO in (Select Distinct NO From 门诊费用记录 J,医嘱 P Where J.病人ID=[1] and J.医嘱序号=P.Column_Value And J.记录性质=2  ))"
        ElseIf mstr医嘱IDs <> "" Then
            strJzTable = strJzTable & " And   A.NO in (Select Distinct NO From 门诊费用记录 J,医嘱 P Where J.病人ID=[1] and J.医嘱序号=P.Column_Value And J.记录性质=2  ) "
        ElseIf strTableNos <> "" Then
...
Private Sub cmdYB_Click()
    ...
    Call MCPatientProcess(mobjPati.病人ID)
End Sub
...
Private Sub MCPatientProcess(ByVal lngCur病人ID As Long)
    ...
    If mobjPati.病人类型 <> "" Then
        lbl(Lbl_Index.姓名).ForeColor = GetPatiColor(mobjPati.病人类型, vbRed)
    End If
    ...
    For i = 0 To UBound(varNos)
        strSql = "Zl_门诊划价记录_Update_S(" & mInsure.intInsure & "," & lng病人ID & ",'" & varNos(i) & "',0)"
...
IdentifyCancel:
    ...
    lbl(Lbl_Index.姓名).ForeColor = GetPatiColor(mobjPati.病人类型, &HFF0000)
    staThis.Panels(Pan.C3个人帐户).Text = ""
    staThis.Panels(Pan.C3个人帐户).Visible = False
    Exit Sub
ErrHandler:
...
Private Function SaveFeeBill() As Boolean
    ...
    For p = 1 To UBound(varNos) + 1
        ...
        If mrsFeeData.RecordCount <> 0 Then
            ...
            'Zl_病人划价收费_Insert_S
            strSql = "Zl_病人划价收费_Insert_S("
            '  No_In         门诊费用记录.NO%Type,
            strSql = strSql & "'" & strNO & "',"
            '  病人id_In     门诊费用记录.病人id%Type,
            strSql = strSql & "" & ZVal(mlngPatiID) & ","
            '  付款方式_In   门诊费用记录.付款方式%Type,
            If mobjPati.医疗付款方式编码 <> "" Then
               strSql = strSql & "'" & mobjPati.医疗付款方式编码 & "',"
            Else
...
Private Function ExecuteInsureSwap(objPati As clsPatientInfo) As Boolean
    ...
    mInsure.strAllNos = ""
    If zlExecuteInsureSwap(bytMode, objPati, mInsure.intInsure, mstr个人帐户, _
...
Private Sub Form_Activate()
    ...
    If mbytCurType = 1 And zlstr.IsHavePrivs(mstrPrivs, "保险收费") And Not mobjPati Is Nothing Then '136681
        mInsure.intInsure = GetCustomPatiInsure(mobjPati.病人ID)
        If mInsure.intInsure <> 0 Then
            Call MCPatientProcess(mobjPati.病人ID)
        End If
    End If
...
Private Function GetMoneyInfo(lng病人ID As Long, Optional int类型 As Integer = -1, _
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim rsTmp As ADODB.Recordset, lng主页ID As Long
    ...
    strSql = "Select " & IIf(blnFamilyMoney, "0 As 家属,", "") & _
            ...
            " From 病人余额" & _
            " Where 性质=1 And 病人ID=[1] " & IIf(int类型 = -1, "", " And 类型=[2]")
    '79868,读取病人家属余额
    If blnFamilyMoney Then
        ...
        If strFamilyPatiIDs <> "" Then
            strSql = strSql & " Union All " & _
                    ...
                    " From 病人余额 A, Table(f_num2list([3])) B" & _
                    " Where a.病人id = b.Column_Value And a.性质 = 1" & _
                    IIf(int类型 = -1, "", " And a.类型=[2]")
        End If
    End If
    ...
    Set GetMoneyInfo = zlDatabase.OpenSQLRecord(strSql, App.ProductName, lng病人ID, int类型, strFamilyPatiIDs)
    Exit Function
ErrHandler:
    If ErrCenter() = 1 Then
        Resume
...
Private Sub Form_Load()
    ...
    End If
    If Load预交余额(mobjPati.病人ID) = False Then Unload Me: Exit Sub
...
Private Function LoadPatient() As Boolean
    ...
    lbl(Lbl_Index.姓名).Caption = mobjPati.姓名
    lbl(Lbl_Index.性别).Caption = "性别:" & mobjPati.性别
    lbl(Lbl_Index.门诊号).Caption = "门诊号:" & mobjPati.门诊号
...
Private Function BrushcardStrikePrepay(ByVal blnOKClick As Boolean) As Boolean
    ...
    '刷卡确认
    If PatiIdentify(mlngModule, Me, mlngPatiID, dblMoney, False, 1, mlngCardTypeID, True, , mstr家属IDs) Then
                    
        txt冲预交.Tag = "1" '标记已验证
        
        '修正病人预交记录关联交易ID数据
        If UpgradeHistoryData(mlngModule, mlngPatiID, mstr家属IDs) = False Then Exit Function
        '59412
...
Private Sub cmdOK_Click()
    ...
    Call LoadPatient
    If Load预交余额(mobjPati.病人ID) = False Then
        mblnOk = True
        Unload Me: Exit Sub
    End If
...
Private Function VerifyFee(ByRef strPrintNo As String) As Boolean
    ...
    '卫材已收费状态确认
    Call mclsExpenceSvr.zlStuffOutBillAffirm(strNos, 1, 2, mbln门诊自动发料)
...
Private Function SaveCharge(ByRef strPrintNo As String, Optional ByRef blnPartialSaved As Boolean) As Boolean
    ...
    If mblnAsyncCharge Then '费用结算异步控制，先提交数据
    ...
    If mobjThreeSwap.ExecutePay(mCurCardPay.lng卡类别ID, mCurCardPay.bln消费卡, _
        ...
        Else
            ...
            '2.删除费用结算对照,恢复为划价单
            Call CancelBillBalance(lng结帐ID)
            gcnOracle.CommitTrans: blnTrans = False
        End If
...
Private Sub ShowLedInfor()
    ...
    strInfo = mobjPati.姓名 & " " & mobjPati.性别 & " " & mobjPati.年龄
    zl9LedVoice.DisplayPatient strInfo, mobjPati.病人ID
...
Private Function InitThreeSwap(frmMain As Object) As Boolean
    ...
    mobjThreeSwap.Init mobjOneCardComLib, frmMain, mlngModule, mobjPati.病人ID, mobjPati.姓名, mobjPati.性别, mobjPati.年龄
    InitThreeSwap = True
    Exit Function
ErrHandler:
    If ErrCenter() = 1 Then
...
Private Function WriteInforToCard(frmMain As Object, ByVal lngModul As Long, ByVal strPrivs As String, _
    ...
    Do While Not rsTemp.EOF
        ...
        If lng病人ID <> 0 And lng结算序号 <> 0 Then
            Call mobjOneCardComLib.zlMzInforWriteToCard(frmMain, lngModul, lngCardTypeID, lng病人ID, lng结算序号)
        End If
        rsTemp.MoveNext
    Loop
