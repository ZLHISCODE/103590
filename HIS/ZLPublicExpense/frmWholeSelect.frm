...
Private Const mstrLvwWholeSet As String = "名称,3500,0,1;编码,800,0,2;拼音,1400,0,0;五笔,1400,0,0;使用范围,1000,0,0;所属分类,2400,0,0"
...
Private Sub cmdALL_Click()
    Dim i As Long, lng收费细目ID As Long
    Dim strAllSelItemIds As String
    With vsWholeSet
        ...
        For i = 1 To .Rows - 1
            ...
            If lng收费细目ID <> 0 Then
                If InStr(strAllSelItemIds & ",", "," & lng收费细目ID & ",") > 0 Then
                    .TextMatrix(i, .ColIndex("选择")) = 0
                Else
                    strAllSelItemIds = strAllSelItemIds & "," & lng收费细目ID
                End If
                ...
                Else
                    ...
                    .Cell(flexcpForeColor, i, 1, i, .Cols - 1) = .ForeColor
                End If
            End If
        Next
    End With
...
Private Function InitPanel()
    ...
    lngWidth = (Me.ScaleWidth * 2 / 3) \ Screen.TwipsPerPixelX
    ...
    With dkpMan
        ...
        Set objPane = .CreatePane(pan_Tree, 200, 100, DockTopOf, objPane)
        ...
        objPane.Handle = picTree.hWnd: objPane.Tag = pan_Tree
         ...
         Set objPane = .CreatePane(pan_WholeSet, lngWidth, 400, DockRightOf, objPane)
        objPane.Title = "成套项目"
        ...
        objPane.Handle = picList.hWnd: objPane.Tag = pan_WholeSet
         ...
         Set objPane = .CreatePane(pan_WholeItems, lngWidth, 400, DockBottomOf, objPane)
...
Private Function BulidingRecord() As Boolean
    ...
    Dim strAllSelItemed As String, lng收费细目ID As Long
    ...
    With vsWholeSet
        ...
        For i = 1 To .Rows - 1
            lng收费细目ID = Val(.Cell(flexcpData, i, .ColIndex("收费项目")))
            If lng收费细目ID <> 0 And GetVsGridBoolColVal(vsWholeSet, i, .ColIndex("选择")) Then
                If InStr(strAllSelItemed & ",", "," & lng收费细目ID & ",") = 0 Then
                    mrsOutSel.AddNew
                    mrsOutSel!成套ID = Val(Mid(lvwWholeSetItem.SelectedItem.Key, 2))
                    mrsOutSel!收费细目ID = lng收费细目ID
                    ...
                    mrsOutSel.Update
                    strAllSelItemed = strAllSelItemed & "," & lng收费细目ID
                End If
            End If
        Next
    End With
...
ReDO:
            ...
            If chk缺省显示.Value = 1 Then
                .TextMatrix(i, .ColIndex("选择")) = IIf(CheckItemSelected(i, Val(Nvl(rsTemp!收费细目ID)), , , False) = False, -1, 0)
            Else
                .TextMatrix(i, .ColIndex("选择")) = 0
            End If
...
Private Sub Form_Load()
    ...
    gobjComlib.RestoreWinState Me, App.ProductName, Me.Name
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    gobjComlib.SaveWinState Me, App.ProductName, Me.Name
    ...
    With vsWholeSet
        ...
        Case .ColIndex("选择")
            ...
            If Val(.Cell(flexcpData, Row, .ColIndex("收费项目"))) <> 0 Then
                If blnCheck Then
                    Call ClearItemAllSelected(Row, Val(.Cell(flexcpData, Row, .ColIndex("收费项目"))), False)
            ...
            If Val(.Cell(flexcpData, Row, .ColIndex("收费项目"))) = 0 Then
                ...
                For i = Row + 1 To .Rows - 1
                    If Val(.TextMatrix(i, .ColIndex("成套ID"))) = Val(.TextMatrix(Row, .ColIndex("成套ID"))) And Val(.Cell(flexcpData, i, .ColIndex("收费项目"))) <> 0 Then
                         If blnCheck Then
                            Call ClearItemAllSelected(i, Val(.Cell(flexcpData, i, .ColIndex("收费项目"))), False)
...
Public Function CheckIsValied() As Boolean
    ...
    Dim bln中药 As Boolean, strInfor As String
    Dim intMsgbox As VbMsgBoxResult
    Dim blnChecked As Boolean '已经检查了重复项目
    Dim blnChecked_ZY As Boolean '已经检查了中药项目
    Dim strOptSel As String '选择的操作
    
    blnChecked = False: blnChecked_ZY = False
    With vsWholeSet
        For i = 1 To .Rows - 1
            If GetVsGridBoolColVal(vsWholeSet, i, .ColIndex("选择")) Then
                 lng收费细目ID = Val(.Cell(flexcpData, i, .ColIndex("收费项目")))
                            ...
                            If GetVsGridBoolColVal(vsWholeSet, j, .ColIndex("选择")) Then
                                If lng收费细目ID = Val(.Cell(flexcpData, j, .ColIndex("收费项目"))) Then
                                    If blnChecked = False Then
                                        strInfor = "" & _
                                        "收费项目『" & .TextMatrix(j, .ColIndex("收费项目")) & " 』在第" & .TextMatrix(j, .ColIndex("序号")) & "行" & vbCrLf & vbCrLf & _
                                        "中已经存在,是否只取第一个选择项目？" & vbCrLf
                                        intMsgbox = ShowMsgBox_Custom(Me, strInfor, False, 0)
                                        Select Case intMsgbox
                                        Case vbIgnore: blnChecked = True: '不再检查
                                        Case vbYes  '继续检查
                                              '清除相同的选择项
                                             Call ClearItemAllSelected(i, lng收费细目ID)
                                        Case Else
                                            .Row = j: .SetFocus: Exit Function
                                        End Select
                                    End If
                                End If
                                If bln中药 And .TextMatrix(j, .ColIndex("类别")) <> "7" And Not blnChecked_ZY Then
                                    strInfor = "" & _
                                    "在第" & .TextMatrix(j, .ColIndex("序号")) & "行中的收费项目『" & .TextMatrix(j, .ColIndex("收费项目")) & " 』" & vbCrLf & vbCrLf & _
                                    "为非中草药项目,你只能选择一种操作:"
                                    If gobjCommFun.ShowMsgBox(gstrSysName, strInfor, "!确定(&O),?取消(&C)", Me, vbQuestion, "", , , ":只提取中草药项目|1,只提取非草药项目|0", strOptSel) = "" Then
                                        .Row = j: .SetFocus
                                        Exit Function
                                    End If
                                    If strOptSel = "只提取中草药项目" Then
                                        Call ReSelectedAllItem(True)    '重新选择中药项目
                                    Else
                                        Call ReSelectedAllItem(False)    '重新选择中药项目
                                    End If
                                     blnChecked_ZY = True
                                End If
                                If Not blnChecked_ZY Then
                                    If (bln中药 = False And .TextMatrix(j, .ColIndex("类别")) = "7") Then
                                        strInfor = "" & _
                                        "在第" & .TextMatrix(j, .ColIndex("序号")) & "行中的收费项目『" & .TextMatrix(j, .ColIndex("收费项目")) & " 』" & vbCrLf & vbCrLf & _
                                        "为中草药项目,你只能选择一种操作:"
                                        If gobjCommFun.ShowMsgBox(gstrSysName, strInfor, "!确定(&O),?取消(&C)", Me, vbQuestion, "", , , ":只提取中草药项目|0,只提取非草药项目|1", strOptSel) = "" Then
                                            .Row = j: .SetFocus: Exit Function
                                        End If
                                        If strOptSel = "只提取中草药项目" Then
                                            Call ReSelectedAllItem(True)     '重新选择中药项目
...
Private Function CheckItemSelected(ByVal lngCurRow As Long, ByVal lng项目id As Long, _
    Optional ByRef lngSelRow As Long, Optional ByRef lngSelNo As Long, Optional ByVal blnCurRowAfter As Boolean = True) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:判断指定项目是否已经在存套项目中选择
    '入参:lng项目ID-指定的项目ID
    '     lngCurRow-当前行
    '     blnCurRowAfter-blnCurRowAfter-true:只针对当前行后面的数据进行判断,False-从第一行开始判断
    '出参:lngSelRow-返回项目被选择的行
    '     lngSelNo-选中行的序号
    '返回:被选择true,否则返回False
    '编制:刘兴洪
    '日期:2017-11-07 20:33:17
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim i As Long, lng收费细目ID As Long
    Dim j As Long
    lngSelRow = -1: lngSelNo = -1
    With vsWholeSet
        j = IIf(blnCurRowAfter, lngCurRow, 0) + 1
        For i = j To .Rows - 1
            lng收费细目ID = Val(.Cell(flexcpData, i, .ColIndex("收费项目")))
            If lng收费细目ID <> 0 And lng项目id = lng收费细目ID And i <> lngCurRow Then
                If GetVsGridBoolColVal(vsWholeSet, i, .ColIndex("选择")) Then
                    lngSelRow = i: lngSelNo = Val(.TextMatrix(j, .ColIndex("序号")))
...
Private Sub ClearItemAllSelected(ByVal lngCurRow As Long, ByVal lng项目id As Long, Optional blnCurRowAfter As Boolean = True)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:清空指定项目的所有被选择的行(但不包护lngCurRow)
    '入参:lng项目ID-指定的项目ID
    '     lngCurRow-当前行
    '     blnCurRowAfter-true:只针对当前行后面的数据进行清除;False-所有项目
    '编制:刘兴洪
    '日期:2017-11-07 20:33:17
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim i As Long, lng收费细目ID As Long
    ...
    With vsWholeSet
        ...
        For i = j To .Rows - 1
            ...
            If lng收费细目ID <> 0 And lng项目id = lng收费细目ID And i <> lngCurRow Then
                If GetVsGridBoolColVal(vsWholeSet, i, .ColIndex("选择")) Then
                    .Cell(flexcpChecked, i, .ColIndex("选择")) = 0
                    .Cell(flexcpForeColor, i, 1, i, .Cols - 1) = .ForeColor
                End If
            End If
        Next
    End With
End Sub
Private Sub ReSelectedAllItem(Optional ByVal bln中草药 As Boolean = False)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:重新设置选择项目
    '入参:bln中草药-只选择中草药
    '编制:刘兴洪
    ...
    With vsWholeSet
        For i = 1 To .Rows - 1
            ...
            If lng收费细目ID <> 0 Then
                If bln中草药 Then
                    If .TextMatrix(i, .ColIndex("类别")) = "7" Then
                        If GetVsGridBoolColVal(vsWholeSet, i, .ColIndex("选择")) Then
                            '清除相同的选择项
                            Call ClearItemAllSelected(i, lng收费细目ID)
                        End If
