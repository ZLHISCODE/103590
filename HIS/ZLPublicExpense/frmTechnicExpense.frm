...
Private Const STR_HEAD = "行,450,4;类别,750,1;项目,2175,1;商品名,2000,1;规格,1105,1;单位,520,4;付数,520,1;数次,570,1;单价,1055,7;" & _
                        "应收金额,1030,7;实收金额,1080,7;执行科室,1255,1;标志,520,4;类型,520,1"
Private mblnOK As Boolean
Private mrsMainOperation As ADODB.Recordset
Private mbln诊间支付 As Boolean
Private mobjPublicDrug As Object '药品公共部件,105872
        ...
        If mobjBill.Details.Count >= Bill.Row Then
            With mobjBill.Details(Bill.Row)
                If .执行部门ID <> Bill.ItemData(Bill.ListIndex) Then
                    lng执行科室 = .执行部门ID
                    str执行科室 = Bill.TextMatrix(Bill.Row, Bill.Col)
                    ...
                    If mint记录性质 = 2 Then
                        If Not IsNull(mrsInfo!险类) And mobjBill.Details(Bill.Row).数次 <> 0 And MCPAR.实时监控 Then
                            If gclsInsure.CheckItem(mrsInfo!险类, 1, 0, MakeDetailRecord(mobjBill, NeedName(cbo开单人.Text), NeedName(cbo开单科室.Text), 2, 0, Bill.Row)) = False Then
                    ...
                    If mobjBill.Details(Bill.Row).数次 <> 0 Then
                        If CheckChargeItemByPlugIn(gobjPlugIn, glngSys, p医嘱附费管理, IIf(mint病人来源 = 2, 1, 0), 0, _
                            MakeDetailRecord(mobjBill, NeedName(cbo开单人.Text), NeedName(cbo开单科室.Text), mint记录性质, IIf(mint记录性质 = 1, 1, 0), Bill.Row)) = False Then
                            Bill.Text = "": Bill.TxtVisible = False
                            Bill.cboObj.Text = str执行科室
                             .执行部门ID = lng执行科室: Exit Sub
                        End If
...
'说明：可以全部为主要手术,但不能全部为附加手术
    ...
    If bytTime = 0 Then
        '114157，当前单据中不存在主手术，则检查该医嘱中是否存在主手术
        If HaveMainOperation(mint病人来源, mlng医嘱ID, mstrInNO) Then bytTime = 1
    End If
    ...
    ElseIf strCheck <> "" Then
        ...
        MsgBox "当前医嘱费用中还没有主要手术，不能添加附加手术！", vbInformation, gstrSysName
    End If
End Sub

Private Function HaveMainOperation(ByVal int病人来源 As Integer, ByVal lng医嘱ID As Long, _
    ByVal strModifyNo As String) As Boolean
    '当前医嘱(含相关医嘱)是否包含主医嘱费用
    '入参：
    '   int病人来源 1-门诊病人,2-住院病人
    '   strModifyNo 修改时要排除当前单据
    '说明：
    ...
    strSql = _
        "Select Nvl(Sum(c.数次), 0) As 数次" & vbNewLine & _
        "From " & strTable & " C, 病人医嘱记录 B, 病人医嘱记录 A" & vbNewLine & _
        "Where c.医嘱序号 = b.Id And (b.相关id = a.相关id Or b.相关id = a.Id Or b.Id = a.相关id Or b.Id = a.Id)" & vbNewLine & _
        "      And c.收费类别 = 'F' And Nvl(c.附加标志, 0) = 0 And a.Id = [1]" & strWhere & vbNewLine & _
        "Having Nvl(Sum(c.数次), 0) <> 0"
    If mrsMainOperation Is Nothing Then
        Set mrsMainOperation = gobjDatabase.OpenSQLRecord(strSql, Me.Caption, lng医嘱ID, strModifyNo)
...
Private Function CheckMainOperation() As Boolean
    ...
    For i = 1 To mobjBill.Details.Count
        ...
        If mobjBill.Details(i).收费类别 = "F" Then
           If mobjBill.Details(i).附加标志 = 0 Then lngCount = 0: Exit For  '存在主要手术,则不检查,直接返回true
    ...
    If lngCount <> 0 Then
        If HaveMainOperation(mint病人来源, mlng医嘱ID, mstrInNO) Then
            '114157，当前单据中不存在主手术，则检查该医嘱中是否存在主手术
        Else
            MsgBox "当前医嘱费用中还没有主要手术，不能添加附加手术！", vbInformation, gstrSysName
            Bill.Row = lngRow '定位行
...
Private Sub bill_CommandClick()
    ...
    End If
    mlng批次 = -1
    lng项目id = frmItemSelect.ShowSelect(Me, mstrPrivs, mint病人来源, int险类, True, str类别, , , str特准项目, _
        zl获取中药形态(Bill.Row), IIf(mbln费用登记, True, False), mbln条码刷卡, mlng批次, _
            ...
            Case "项目"
                ...
                If Bill.Text <> "" Then
                    ...
                    Else
                        ...
                        End If
                        mlng批次 = -1
                        lng项目id = frmItemSelect.ShowSelect(Me, mstrPrivs, mint病人来源, int险类, True, str类别, Bill.Text, _
                            Bill.TxtHwnd, str特准项目, zl获取中药形态(Bill.Row), IIf(mbln费用登记, True, False), _
                            mbln条码刷卡, mlng批次, mstr药品价格等级, mstr卫材价格等级, mstr普通价格等级)
                        If lng项目id <> 0 Then
                    ...
                    Else
                        If mrsInfo.State = 1 Then    '问题:主要是大连一院要求,由于BH不能登记,所以没有BugNo  '90304
                    ...
                    If mint记录性质 = 2 And mrsWarn.State = 1 And mobjBill.Details.Count = Bill.Row Then
                        ...
                        If curTotal > 0 And mbln费用登记 = False Then
                            ...
                            mbytWarn = BillingWarn(Me, mstrPrivs, mrsWarn, mrsInfo!姓名 & IIf(Nvl(mrsInfo!住院号) = "", "", "(住院号:" & mrsInfo!住院号 & " 床号:" & mrsInfo!床号 & ")"), cur余额, mrsInfo!当日额 - mcurModiMoney, curTotal, _
    ...
    For i = 1 To mcolDetails.Count
        ...
        If mrsInfo.State = 1 Then
            If Not IsNull(mrsInfo!险类) Then
                ...
                End If
                If zlCheck定价零价格对码(mcolDetails(i).ID, Not mcolDetails(i).变价, strPriceGrade) Then
                    '问题:27286
                Else
        ...
        str摘要 = mobjBill.Details(Bill.Rows - 1).摘要  '90304
        If mrsInfo.State = 1 Then
...
Private Sub Bill_EnterCell(Row As Long, Col As Long)
    ...
    If mbytInState = 3 Then
        ...
        Exit Sub
    End If
    If mlngPreRow <> Row Then sta.Panels(2).Text = "" '切换行时才清空,70160
     '--------------------------------------------------------------------------
    '1.行改变的相关数据处理和设置
        ...
        With mobjBill.Details(Bill.Row)
            ...
            If InStr(",5,6,7,", .收费类别) > 0 And .收费细目ID <> 0 Then
                If mbln其它药房 Or mbln其它药库 Then
                    ...
                    If strStock <> "" Then
                        If InStr(1, mstrPrivs, "显示库存") > 0 Then
                            sta.Panels(Pan.C2提示信息) = "[" & .Detail.名称 & "]可用库存:" & strStock
                        Else
                            sta.Panels(Pan.C2提示信息) = "[" & .Detail.名称 & "]有库存."
                        End If
                    End If
                End If
                If strStock = "" Then
...
'        End If
        ...
        If mint记录性质 = 2 And mrsWarn.State = 1 And mstrWarn <> "-" Then
            ...
            If curTotal > 0 Then
                ...
                If mbln费用登记 = False Then    '30504
                    For i = 1 To mobjBill.Details.Count
                        mbytWarn = BillingWarn(Me, mstrPrivs, mrsWarn, mrsInfo!姓名 & IIf(Nvl(mrsInfo!住院号) = "", "", "(住院号:" & mrsInfo!住院号 & " 床号:" & mrsInfo!床号 & ")"), cur余额, cur当日额 - mcurModiMoney, curTotal, IIf(IsNull(mrsInfo!担保额), 0, mrsInfo!担保额), mobjBill.Details(i).收费类别, mobjBill.Details(i).Detail.类别名称, mstrWarn, mintWarn)
        ...
        If mint记录性质 = 2 And Not IsNull(mrsInfo!险类) And MCPAR.实时监控 Then
            If gclsInsure.CheckItem(mrsInfo!险类, 1, 2, MakeDetailRecord(mobjBill, NeedName(cbo开单人.Text), NeedName(cbo开单科室.Text), 2, 0)) = False Then
                Exit Sub
            End If
        End If
        
        If CheckChargeItemByPlugIn(gobjPlugIn, glngSys, p医嘱附费管理, IIf(mint病人来源 = 2, 1, 0), 1, _
            MakeDetailRecord(mobjBill, NeedName(cbo开单人.Text), NeedName(cbo开单科室.Text), mint记录性质, IIf(mint记录性质 = 1, 1, 0))) = False Then
            Exit Sub
        End If
        
        '检查单据和医嘱中是否只有附加手术,如果只有附加手术,直接退出
        ...
        '刷卡消费验卡
        If mint病人来源 = 1 And mint记录性质 = 2 And gSysPara.dbl预存款消费验卡 <> 0 Then
            curTotal = CalcGridToTal
            If curTotal > 0 Then
                If Not gobjDatabase.PatiIdentify(Me, glngSys, mobjBill.病人ID, curTotal, _
                    , , , IIf(-1 * gSysPara.dbl预存款消费验卡 >= curTotal, False, True), , , , (gSysPara.dbl预存款消费验卡 = 2)) Then Exit Sub
            End If
        End If
        '74231,冉俊明,2014-6-24,项目开单后立即收费或记帐审核
        If mobjSquareCard Is Nothing Then
            If mint病人来源 = 1 And (gSysPara.bln开单后立即结算 Or mbln诊间支付) Then
        ...
        If zlChargeSaveValied_Plugin(p医嘱附费管理, mint记录性质, False, mint记录性质 = 1, "", rsItems) = False Then Exit Sub
        
        If Not SaveBill(strNos) Then Exit Sub
        
        Call zlChargeSaveAfter_Plugin(p医嘱附费管理, mlng病人ID, mlng主页ID, mint病人来源 = 1, mint记录性质, strNos)
        '74231,冉俊明,2014-6-24,项目开单后立即收费或记帐审核
        If mint病人来源 = 1 And (gSysPara.bln开单后立即结算 Or mbln诊间支付) And strNos <> "" Then
...
Private Sub Form_Load()
    ...
    If mbytInState = 0 Then
        ...
        If GetPriceGrade(gstrNodeNo, mlng病人ID, mlng主页ID, "", mstr药品价格等级, mstr卫材价格等级, mstr普通价格等级) = False Then
            mstr药品价格等级 = "": mstr卫材价格等级 = "": mstr普通价格等级 = ""
        End If
    End If
    ...
    Else
        ...
        If mstrInNO <> "" Then '修改单据
            Set mobjBill = ImportBill(mint病人来源, mstrInNO, mint记录性质, mbln费用登记, _
                mstr药品价格等级, mstr卫材价格等级, mstr普通价格等级)
            If mobjBill.NO = "" Then
...
Private Function InitData() As Boolean
    ...
    strSql = "Select A.ID, A.编码, A.名称, A.简码, 0 As 缺省, B.工作性质, D.优先级," & vbNewLine & _
            "        Row_Number() Over(Partition By ID Order By 工作性质) As R" & vbNewLine & _
            ...
            "       And B.部门id = D.部门id And (B.服务对象 IN(1,2,3) AND B.工作性质 IN('临床','手术') Or b.工作性质='产科')"
    '87562,每一个部门只查询出一条记录，其中"工作性质"合并显示,如:妇产科,有两个工作性质"临床"和"产科",则查询出的工作性质字段值为"临床,产科"
    strSql = "Select ID, 编码, 名称, 简码, 缺省, 优先级, 工作性质" & vbNewLine & _
            " From (Select Row_Number() Over(Partition By ID Order By Lvl Desc) As Rn, ID, 编码, 名称, 简码, 缺省, 优先级, 工作性质" & vbNewLine & _
            "        From (Select ID, 编码, 名称, 简码, 缺省, 优先级, Level Lvl, LTrim(Sys_Connect_By_Path(工作性质, ','), ',') As 工作性质" & vbNewLine & _
                                ...
                                'If Not (SendMessage(Bill.CboHwnd, CB_FINDSTRING, -1, ByVal strTmp) >= 0) Then
                                    ...
                                    If Not blnEnter Then '进入该列时保持已确定值不变
                                        ...
                                        ElseIf lngRow > 1 Then
                                            '与上一行非药品相同
                                            If rsUnit!ID = mobjBill.Details(lngRow - 1).执行部门ID And mobjBill.Details(lngRow - 1).Detail.执行科室 = .Detail.执行科室 _
...
'    mobjCard.卡号有效字符 = "0" '输入类型(0-所有字符,1-数字,2-字母;3-数字或字母;4-指定字符)|Ascii码1，Ascii码2....
    ...
    Select Case mbytInState
        Case 0 '执行
            ...
            '记帐单保存时就直接审核了的，无需再审核
            If gSysPara.bln开单后立即结算 = False And mint记录性质 = 1 Then
                If IsCan诊间支付(mobjSquareCard, mlng病人ID, mlng主页ID) Then
    ...
    If Not mrsInfo.EOF Then
        ...
        If mint记录性质 = 2 Then
            ...
            If mbln费用登记 = False Then    '30504
                ...
                mbytWarn = BillingWarn(Me, mstrPrivs, mrsWarn, mrsInfo!姓名 & IIf(Nvl(mrsInfo!住院号) = "", "", "(住院号:" & mrsInfo!住院号 & " 床号:" & mrsInfo!床号 & ")"), cur余额, Val(Nvl(mrsInfo!当日额)) - mcurModiMoney, curTotal, _
...
'      4.如果变价细目已经计算出收入项目(按第2步),并手动更改(也可能未改)了单价,则按该单价计算。
    ...
    gstrSQL = _
        ...
        "       And A.ID=[1]" & vbNewLine & _
                strWherePriceGrade
    Set rsTmp = gobjDatabase.OpenSQLRecord(gstrSQL, Me.Caption, mobjBill.Details(lngRow).收费细目ID, strPriceGrade)
    If Not rsTmp.EOF Then
        With mobjBill.Details(lngRow)
            If InStr(",5,6,7,", .收费类别) > 0 Or (.收费类别 = "4" And .Detail.跟踪在用) Then
                ...
                If dblAllTime <> 0 Or Not .Detail.变价 Then
                    ...
                    End If
                    Set rsPrice = gobjDatabase.OpenSQLRecord(gstrSQL, Me.Caption, .收费细目ID, .执行部门ID, dblAllTime, 0, .Detail.批次)
                    If rsPrice.EOF Then
                        '获取价格失败
                        If InStr(",5,6,7,", .收费类别) > 0 Then
            ...
            Else
                If .Detail.变价 Then
                    ...
                    Else                        '获取操作员以前输入的变价金额
                        ...
                        If Abs(dblMoney) > Abs(Val(Nvl(rsTmp!现价))) Then
                            dblMoney = Val(Nvl(rsTmp!缺省价格))
                        End If
                    End If
                End If
...
'出口:保存是否成功
    ...
    '                End If
                ...
                With mobjBillDetail
                    Select Case .收费类别
                    Case "4"    '卫材
                        If (mint病人来源 = 1 And mint记录性质 = 2 And gSysPara.bln门诊自动发料 Or mint病人来源 = 2 And gSysPara.int住院自动发料 <> 0) And int划价 = 0 Then
                ...
                Else
                    If mint记录性质 = 2 Then
                        ...
                        gstrSQL = gstrSQL & "" & ZVal(mobjBill.主页ID) & ","
                        '  病人病区id_In 门诊费用记录.病人病区id%Type := Null
                        gstrSQL = gstrSQL & "" & ZVal(lng病区ID) & ")"
                    Else
                        ...
                        gstrSQL = gstrSQL & "'" & mobjBillDetail.保险编码 & "',"
                        '  费用类型_In   门诊费用记录.费用类型%Type := Null,
                        gstrSQL = gstrSQL & "'" & mobjBillDetail.Detail.类型 & "',"
                        '  保险项目否_In 门诊费用记录.保险项目否%Type := Null,
                        gstrSQL = gstrSQL & "" & IIf(mobjBillDetail.保险项目否, 1, 0) & ","
                        '  保险大类id_In 门诊费用记录.保险大类id%Type := Null,
                        gstrSQL = gstrSQL & "" & ZVal(mobjBillDetail.保险大类ID) & ","
                        '  中药形态_In   门诊费用记录.结论%Type := Null,
                        gstrSQL = gstrSQL & "" & "NULL" & ","
                        '  备货材料_In   Number := 0,
                        gstrSQL = gstrSQL & "" & "0" & ","
...
'功能：取所有可供选择的药房
    ...
    If str类别 = "4" Then
        strSql = _
            ...
            "Where a.执行科室id + 0 = b.部门id And b.工作性质 = '发料部门' And b.服务对象 IN([1],3) And b.部门id = c.Id And" & vbNewLine & _
...
Public Function zlCheck定价零价格对码(ByVal lng收费细目ID As Long, bln定价 As Boolean, _
   ...
   '刘兴洪 问题:27286 定价的价格为零的不进行检查对码 日期:2010-01-07 15:13:45
    ...
    If bln定价 Then
        ...
        "       And B.收费细目ID=[1]" & vbNewLine & _
                strWherePriceGrade
        Set rs价格 = gobjDatabase.OpenSQLRecord(strSql, "获取当前价格", lng收费细目ID, strPriceGrade)
...
Private Sub cmdSelWholeSet_Click()
    ...
    If SelectWholeItems(Me, p医嘱附费管理, mstrPrivs, rsSel) = False Then Exit Sub
    ...
    Set tmpBill = ImportWholeSet(Me, intInsure, rsSel, lng病人ID, lng开单部门ID, byt婴儿费, IIf(mint病人来源 = 2 And mint记录性质 = 2, 2, 0), chk加班.Value = 1, _
        0, mint病人来源, UserInfo.姓名, NeedName(cbo开单人.Text), , mstr药品价格等级, mstr卫材价格等级, mstr普通价格等级)
    '处理数据
    '清除导入的病人信息
    '问题:37500
    With tmpBill
...
Public Function ImportWholeSet(frmParent As Object, ByVal intInsure As Integer, rsSel As ADODB.Recordset, Optional lng病人ID As Long = 0, _
    ...
    Set rsItems = gobjDatabase.OpenSQLRecord(gstrSQL, "mdlExse", strValue(0), strValue(1), strValue(2), strValue(3), _
        ...
        IIf(gSysPara.byt药品名称显示 = 1, 3, 1), lng病人ID, str药品价格等级, str卫材价格等级, str普通价格等级)
    '没有记录就是空单子
...
Private Sub Find开单科室(ByVal lng科室id As Long)
    ...
    If Not rsTmp.EOF Then
        '111904:李南春,2017/8/13,根据权限判断是否增加所有科室
...
Private Function IsCan诊间支付(objSquareCard As Object, _
    ByVal lng病人ID As Long, ByVal lng主页Id As Long) As Boolean
    '功能：判断是否能够显示"诊间支付"按钮
    '说明：
    '   如果当前没有启用存在账户的三方卡支付方式，且该病人也没有预交余额时不显示"诊间支付"按钮
    ...
    If objCards.Count > 0 Then IsCan诊间支付 = True: Exit Function
    
    '2.是否有预交余额
    Set rsTemp = GetMoneyInfo(lng病人ID, lng主页Id)
    If rsTemp Is Nothing Then Exit Function
    Do While Not rsTemp.EOF
        curMoney = curMoney + (Val(rsTemp!预交余额) - Val(rsTemp!费用余额))
        rsTemp.MoveNext
    Loop
    IsCan诊间支付 = curMoney > 0
    Exit Function
Errhand:
    If gobjComlib.ErrCenter() = 1 Then
        Resume
...
Private Function CheckYBFeeItemVerfiy(ByVal lng病ID As Long, ByVal intInsure As Integer, objDetail As Detail) As Boolean
    ...
    '1.保险支付项目对应检查
    If Not zlCheck定价零价格对码(objDetail.ID, Not objDetail.变价, strPriceGrade) Then
...
Private Function ReadDrugAndStuffStock(ByVal lng库房ID As Long, ByRef objDetail As Detail) As Boolean
    ...
    If objDetail.类别 = "4" And objDetail.跟踪在用 Then
        dblStock = GetStock(objDetail.ID, lng库房ID, 0, , , objDetail.批次)  ''29680
        objDetail.库存 = dblStock
    ...
    If InStr(",5,6,7,", objDetail.类别) > 0 Then
        '当前行药品库存
        dblStock = GetStock(objDetail.ID, lng库房ID, 0)  '29680
        If mbln药房单位 Then
            dblStock = dblStock / objDetail.药房包装
        End If
        objDetail.库存 = dblStock
