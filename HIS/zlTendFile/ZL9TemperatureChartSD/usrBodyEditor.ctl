   ...
   Begin VB.PictureBox picBack 
      ...
      Begin VB.PictureBox picMain 
         ...
         Begin VSFlex8Ctl.VSFlexGrid mshDownTab 
            ...
            AccessibleRole  =   24
         End
         Begin zl9TemperatureChartSD.VsfGrid vsf 
            Height          =   255
            Left            =   120
            TabIndex        =   28
            Top             =   3480
...
'功能:获取病人信息
    ...
    If T_Patient.lng婴儿 > 0 Then
        ...
        If rsTmp.BOF = False Then
            txtCard(3).Text = Format(Nvl(rsTmp("出生时间").Value), "yyyy-MM-dd")
        End If
    End If
    ...
    If rsTmp.BOF = False Then
        If txtCard(3).Text = "" And bln入科显示入院 = True Then txtCard(3).Text = Format(Nvl(rsTmp("开始时间").Value), "yyyy-MM-dd")
    End If
    ...
    If rsTmp.BOF = False Then
        ...
        If txtCard(3).Text = "" Then txtCard(3).Text = Format(Nvl(rsTmp("入院时间").Value), "yyyy-MM-dd")
    End If
...
'----------------------------------------------------------------
    ...
    Dim lngDays   As Long, intOpEndDays As Integer, arrOpTime
    ...
    With mshUpTab
        ...
        strTmp = ""
        '77056,根据《山东省中医病历书写基本规范》，体温单的手术时间标注应该连续填写14天，如在14天内又做手术，应连续写至末次手术14天。
        '显示但前段的手术标记
        gstrSQL = "Select B.发生时间 时间" & _
            ...
            "   And B.发生时间 <=[5] order by 发生时间 DESC "
        If mblnMoved Then
            gstrSQL = Replace(gstrSQL, "病人护理文件", "H病人护理文件")
            gstrSQL = Replace(gstrSQL, "病人护理数据", "H病人护理数据")
            gstrSQL = Replace(gstrSQL, "病人护理明细", "H病人护理明细")
        End If
        Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, "提取手术标记", Val(T_Patient.lng文件ID), T_Patient.lng病人ID, T_Patient.lng主页ID, Val(T_Patient.lng婴儿), CDate(mstr结束时间))
        
        strTmp = "": strTmp1 = ""
        Do While Not rsTmp.EOF
            str时间 = Format(rsTmp("时间"), "YYYY-MM-DD HH:mm:ss")
            If rsTmp.AbsolutePosition = 1 Then
                '最后一次手术则需要检查是否在可显示的天数内
                If str时间 >= Format(Int(CDate(mstr开始时间) - mintOpDays), "YYYY-MM-DD") Then
                    strTmp = str时间
                    strTmp1 = str时间
                Else
                    Exit Do
                End If
            Else
                If str时间 >= Format(Int(CDate(strTmp) - mintOpDays), "YYYY-MM-DD") Then
                    strTmp1 = strTmp1 & "'" & str时间
                Else
                    Exit Do
                End If
            End If
        ...
        If strTmp1 <> "" Then
            j = 0
            ReDim Preserve arrOpTime(UBound(Split(strTmp1, "'")))
            For i = UBound(arrOpTime) To LBound(arrOpTime) Step -1
                arrOpTime(j) = Split(strTmp1, "'")(i)
                j = j + 1
            Next i
        End If
        
        str结束时间 = mstr结束时间
        For j = LBound(arrOpTime) To UBound(arrOpTime)
            str时间 = Format(arrOpTime(j), "YYYY-MM-DD")
            ...
            End If
            If j = UBound(arrOpTime) Then
                intOpEndDays = mintOpDays
            Else
                If Format(str结束时间, "YYYY-MM-DD") > Format(CDate(arrOpTime(UBound(arrOpTime))) + mintOpDays, "YYYY-MM-DD") Then
                    intOpEndDays = DateDiff("D", CDate(Format(str时间, "YYYY-MM-DD")), CDate(arrOpTime(UBound(arrOpTime))) + mintOpDays)
                Else
                    intOpEndDays = DateDiff("D", CDate(Format(str时间, "YYYY-MM-DD")), CDate(str结束时间))
                End If
            End If
            For i = 1 To lng天数
...
Private Sub ReadBoyData(ByVal blnAutoAdjust As Boolean)
    ...
    gstrFields = "时间," & adLongVarChar & ",20|项目序号," & adDouble & ",18|类型," & adDouble & ",2|" & _
        ...
        "禁用," & adInteger & ",1|显示," & adDouble & ",1|标记时间," & adVarChar & ",10"
...
Private Sub SaveMemory(ByVal blnAutoAdjust As Boolean)
    ...
    Dim str时间 As String, str内容 As String, str标记时间 As String
    ...
    '读取手术、上下标信息
    '-----------------------------------------------------------------------
    gstrFields = "时间|项目序号|类型|内容|颜色|X坐标|Y坐标|高度|打印X坐标|禁用|标记时间"  '入出转缺省是红色,上下标及未记说明缺省是蓝色
    gstrSQL = "" & _
        ...
        Do While Not .EOF
            ...
            If !记录类型 = 4 Then
                ...
                If bytShow = 2 And Not blnAutoAdjust Then
                    str内容 = str内容 & gstrCaveSplit
                    str标记时间 = ConvertTimeToChinese(Format(!时间, "HH:mm"), 0)
                Else
                    str内容 = !项目名称
                    str标记时间 = ""
                End If
            ...
            If bytShow > 0 Then
                ...
                If mrsNote.BOF Then
                    gstrValues = Format(!时间, "yyyy-MM-dd HH:mm:ss") & "|" & lng行号 & "|" & !记录类型 & "|" & _
                        str内容 & "|" & lngColor & "|" & SinX & "|0|0|0|0|" & str标记时间
                    Call Record_Add(mrsNote, gstrFields, gstrValues)
                Else
                    mrsNote!时间 = Format(!时间, "yyyy-MM-dd HH:mm:ss")
                    mrsNote!内容 = str内容
                    mrsNote!标记时间 = str标记时间
                    mrsNote.Update
                End If
            End If
            mrsNote.Filter = 0
        ...
        Do While Not .EOF
            If Trim(Nvl(!内容)) <> "" Then
                ...
                If bytShow > 0 Then
                    ...
                    str标记时间 = ""
                    If bytShow = 2 Then
                        str内容 = str内容 & IIf(blnAutoAdjust = False, gstrCaveSplit, "")
                        str标记时间 = IIf(blnAutoAdjust = False, ConvertTimeToChinese(Format(!时间, "HH:mm"), 0), "")
                    ElseIf bytShow = 3 Then
                        str内容 = str内容 & IIf(blnAutoAdjust = False, gstrCaveSplit & Nvl(!科室), "")
                    ElseIf bytShow = 4 Then
                        str内容 = str内容 & IIf(blnAutoAdjust = False, gstrCaveSplit & Nvl(!科室) & gstrCaveSplit, "")
                        str标记时间 = IIf(blnAutoAdjust = False, ConvertTimeToChinese(Format(!时间, "HH:mm"), 0), "")
                    ...
                    If mrsNote.BOF Then
                        gstrValues = Format(!时间, "yyyy-MM-dd HH:mm:ss") & "|" & lng行号 & "|3|" & _
                            str内容 & "|" & lngSignColor & "|" & SinX & "|0|0|0|0|" & str标记时间
                        Call Record_Add(mrsNote, gstrFields, gstrValues)
                    Else
                        mrsNote!时间 = Format(!时间, "yyyy-MM-dd HH:mm:ss")
                        mrsNote!内容 = str内容
