...
Public Function GetFontHeight(ByVal objDraw As Object, ByVal strComment As String, Optional ByVal strTime As String = "") As Double
    '------------------------------------------------------------------------------------
    '功能:得到字符串高度
    'strTime：不为空格式必须为XX时XX分,XX必须为阿拉伯数字
    '------------------------------------------------------------------------------------
    Dim intAscCharNum As Integer
    Dim Y             As Double
    ...
    If strTime <> "" And strTime Like "*时*分" Then
        T_Size.H = objDraw.TextHeight("你") / T_TwipsPerPixel.Y
        Y = Y + T_Size.H * 4
    End If
    GetFontHeight = Y
...
'            If RS.BOF = False Then lng诊疗项目id = zlCommFun.Nvl(RS("ID").Value)
            ...
            strSQL = strSQL & vbNewLine & _
               "    Select '' As 科室, DECODE(Trunc(b.开始执行时间),Trunc(a.出院日期),b.开始执行时间,a.出院日期) As 时间," & strWay & "8 As 行号" & vbNewLine & _
...
'               "    Where A.病人id = [1] And A.主页id = [2] And A.出院日期 Between [4] And [5] And a.病人id=b.病人id(+) And a.主页id=b.主页id(+) "
            ...
            If lng婴儿 <> 0 Then
                ...
                strNewSql = _
                    ...
                    "       And  Instr(',3,5,11,', ',' || c.操作类型 || ',') > 0 order by B.开始执行时间 DESC" & vbNewLine & _
                    "   ) where Rownum<2"
...
Public Function ConvertTimeToChinese(ByVal strTime As String, Optional ByVal bytMode As Byte = 1) As String
            ...
            If int项目表示 = 4 Then '汇总项目
                ...
                If lngID <> 0 Then
                    If bln录入小时 = True And int记录频次 = 1 Then
                        strValues1 = IIf(dblData = 0, "", IIf(strValues1 = "", "", "(" & strValues1 & "h)") & IIf(Left(dblData, 1) = ".", "0", "") & dblData)
                    Else
                        intHour1 = 0
                        strValues1 = IIf(dblData = 0, "", IIf(Left(dblData, 1) = ".", "0", "") & dblData)
                    End If
