...
Public Function PrintOrPreviewBodyStateNew(objOut As Object, _
    ...
    Dim blnColor As Boolean, lngColor As Long
    ...
    Dim intDays As Integer, intOpEndDays As Integer   '手术天数
    Dim strOpdays() As String
    Dim strOpValue() As String
    Dim arrOperDay, arrOpTime
    ...
    Dim bln术后显示 As Boolean '病人术后不足手术天数出院标记显示
    ...
    For lngPage = 1 To lngCountPage
        ...
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "提取手术标记", lng文件ID, intBaby, lng病人ID, lng主页ID, CDate(strEndDay))
        strTmp = "": strTmp1 = ""
        Do While Not rsTmp.EOF
            strTime = Format(rsTmp("时间"), "YYYY-MM-DD HH:mm:ss")
            If rsTmp.AbsolutePosition = 1 Then
                '最后一次手术则需要检查是否在可显示的天数内
                If strTime >= Format(Int(CDate(Format(strTmpDay, "yyyy-mm-dd hh:mm:ss")) - intOpDays), "YYYY-MM-DD") Then
                    strTmp = strTime
                    strTmp1 = strTime
                Else
                    Exit Do
                End If
            Else
                If strTime >= Format(Int(CDate(strTmp) - intOpDays), "YYYY-MM-DD") Then
                    strTmp1 = strTmp1 & "'" & strTime
                Else
                    Exit Do
                End If
            End If
        ...
        For j = LBound(arrOpTime) To UBound(arrOpTime)
            strTime = Format(arrOpTime(j), "YYYY-MM-DD")
            ...
            If j = UBound(arrOpTime) Then
                intOpEndDays = intOpDays
            Else
                If Format(str结束时间, "YYYY-MM-DD") > Format(CDate(arrOpTime(UBound(arrOpTime))) + intOpDays, "YYYY-MM-DD") Then
                    intOpEndDays = DateDiff("D", CDate(Format(strTime, "YYYY-MM-DD")), CDate(arrOpTime(UBound(arrOpTime))) + intOpDays)
                Else
                    intOpEndDays = DateDiff("D", CDate(Format(strTime, "YYYY-MM-DD")), CDate(str结束时间))
                End If
            End If
                ...
                If DateDiff("d", strTmpDay, str结束时间) + 1 >= i Then
                    ...
                    Select Case intDays
                        Case 0 '当前区域内的手术开始时间
                            ...
                            End If
                        Case Else
                            If intDays >= 1 And intDays <= intOpEndDays Then '手术开始天数
...
'        '处理体温不升 体温为不升需要在35度下纵向输出体温不升二字
        ...
        '--提取入出院,手术等标记说明
        Call GetAutomatically(lng文件ID, lng病人ID, lng主页ID, intBaby, lng护理等级, strTmpDay, strEndDay, lngSignColor, bln入科显示入院, _
                bln入科不转入院, blnMoved, rsNotes)
...
Public Sub OutPutTextNew(ByVal objDraw As Object, ByVal rsDrawItems As ADODB.Recordset, ByVal lngDC As Long, ByVal rsNote As ADODB.Recordset, _
    ...
    Dim strComment    As String, strText As String, str标记时间 As String
    Dim intAscCharNum As Integer
        ...
        Do While Not .EOF
            If Trim(zlCommFun.Nvl(!内容)) <> "" Then
                If Not (!类型 = 2 Or !类型 = 99) Then
                    ...
                    !高度 = GetFontHeight(objDraw, Nvl(!内容), Nvl(!标记时间))
                    .Update
        ...
        Do While Not .EOF
            If Not (!类型 = 2 Or !类型 = 99) Then
                ...
                If blnBigSize = True And blnLessenSize = True Then
                    If GetFontHeight(objDraw, Nvl(!内容), Nvl(!标记时间)) > dblCurveHeight Then
                        blnBigSize = False
                    End If
                End If
                ...
                If blnBigSize = False Then
                    ...
                    lngFont = CreateFontIndirect(T_Font)
                    lngOldFont = SelectObject(lngDC, lngFont)
                    dblHeight = GetFontHeight(objDraw, zlCommFun.Nvl(!内容), Nvl(!标记时间))
                    Call SelectObject(lngDC, lngOldFont)
                    Call DeleteObject(lngFont)
                    '还原字体
                    gstdSet.Name = "宋体"
        ...
        Do While Not .EOF
            ...
            If strComment <> "" Then
                ...
                If blnBigSize = True And blnLessenSize = True And Not (!类型 = 2 Or !类型 = 99) Then
                    If GetFontHeight(objDraw, strComment, str标记时间) > dblCurveHeight Then
                        blnBigSize = False
                    End If
                End If
                ...
                For i = 1 To Len(strComment)
                    ...
                    End If
                Next i
                If str标记时间 Like "*时*分" Then
                    For i = 1 To 4
                        If Y < T_DrawClient.体温区域.Bottom Then
                            Select Case i
                                Case 1
                                    strText = Mid(str标记时间, 1, InStr(1, str标记时间, "时") - 1)
                                    str标记时间 = Mid(str标记时间, InStr(1, str标记时间, "时"))
                                Case 2
                                    strText = Mid(str标记时间, 1, 1)
                                    str标记时间 = Mid(str标记时间, 2)
                                Case 3
                                    strText = Mid(str标记时间, 1, InStr(1, str标记时间, "分") - 1)
                                    str标记时间 = Mid(str标记时间, InStr(1, str标记时间, "分"))
                                Case 4
                                    strText = str标记时间
                            End Select
                            Call SetTextColor(lngDC, Val(!颜色))
                            Call GetTextRect(objDraw, X, Y, strText, IIf(blnCenter = True, T_DrawClient.列单位, objDraw.TextWidth("刘") / T_TwipsPerPixel.X), False, , sngScale)
    ...
    Select Case strText
        ...
        Case "提取当前手术信息"
            strSQL = "Select B.发生时间 时间" & vbNewLine & _
                ...
                " And B.发生时间 <= [5] order by B.发生时间 DESC"
            strSQLText = strSQL
        Case "提取14天之前的手术信息"
...
Private Sub GetAutomatically(ByVal lng文件ID As Long, ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal intBaby As Integer, _
    ByVal lng护理等级 As Long, ByVal strTmpDay As String, ByVal strEndDay As String, ByVal lngSignColor As Long, _
    ByVal bln入科显示入院 As Boolean, ByVal bln入科不转入院 As Boolean, ByVal blnMoved As Boolean, ByRef rsNotes As ADODB.Recordset)
'功能：获取体温自动标志信息
'       strTmpDay 本页开始时间,strEndDay 本页终止时间

    Dim rsTmp As New ADODB.Recordset
    Dim bytShow As Byte
    Dim str内容 As String, str标记时间 As String
    Dim lng行号 As Long, lngColor As Long
    Dim SinX As Single, strSQL As String
    Dim bln婴儿体温单显示出院 As Boolean
            ...
            If Trim(zlCommFun.Nvl(!内容)) <> "" Then
                ...
                If bytShow > 0 Then
                    ...
                    ElseIf bytShow = 4 Then
                        str内容 = str内容 & gstrCaveSplit & zlCommFun.Nvl(!科室) & gstrCaveSplit
                        str标记时间 = ConvertTimeToChinese(Format(!时间, "HH:mm"), 0)
                    ...
                    If bytShow > 0 Then
                        Select Case bytShow
                            Case 1
                                ...
                                str标记时间 = ""
                            Case 2
                                str内容 = zlCommFun.Nvl(!内容) & gstrCaveSplit
                                str标记时间 = ConvertTimeToChinese(Format(!时间, "HH:mm"), 0)
                        End Select
