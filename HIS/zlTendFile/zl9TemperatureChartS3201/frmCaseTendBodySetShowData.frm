...
Private Function ChekPatientOut(ByVal lng病人id As Long, ByVal lng主页id As Long, ByVal lng婴儿ID As Long) As Boolean
    ...
    strSQL = _
            ...
            "                AND Instr(',3,5,11,', ',' || c.操作类型 || ',') > 0 And  B.病人ID = [1] AND B.主页ID = [2] AND B.婴儿(+) = [3]) B" & vbNewLine & _
...
'--------------------------------------------------------------------------------
  ...
  '------------------------------------------------------------------------------------------------------------------
    ...
    With mcbrToolBar.Controls
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_Edit_Adjust, "设置记录")
        cbrControl.BeginGroup = True
...
'            Call zlRefreshData
        ...
        Case conMenu_Edit_Adjust '设置记录
            ...
            '调用显示编辑窗体
            Call gobjTendEditor.BodyEditCur(0, strParam)
    ...
    With vfgShow
        If .Rows - 1 = .Row And .Col >= .FixedCols Then
            ...
            If .TextMatrix(.Row, .Col) = "√" Then
                ...
                mrsPoint.Update
                .TextMatrix(.Row, .Col) = ""
                mrsPoint.Filter = "列号=" & intCOl & " And 序号<>" & Val(.ColData(.Col))
                Do While Not mrsPoint.EOF
                    mrsPoint!显示 = 0
                    mrsPoint!标注 = 0
                    mrsPoint.Update
