...
Begin VB.Form frmImportOrder 
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsfList 
      ...
      Begin MSComctlLib.ImageList imgList 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            NumListImages   =   2
            BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmImportOrder.frx":7469
               Key             =   "未导入"
            EndProperty
            BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmImportOrder.frx":DCCB
               Key             =   "已导入"
            EndProperty
         EndProperty
      End
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsfItem 
      ...
      ColWidthMax     =   0
      ExtendLastCol   =   0   'False
      FormatString    =   $"frmImportOrder.frx":1452D
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Private mblnImportName As Boolean
Private mlngCollectNo As Long
...
Public Function ShowMe(frmParent As Object, ByVal lng文件ID As Long, ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal int婴儿 As Integer, _
        ByVal blnImportName As Boolean, ByVal lngCollectNo As Long, Optional ByVal strDate As String = "") As Recordset
    'lngCollectNo:汇总项目序号(用于查找该项目是否是医嘱导入)
    mlng文件ID = lng文件ID
    mlng病人ID = lng病人ID
    mlng主页ID = lng主页ID
    mint婴儿 = int婴儿
    mblnImportName = blnImportName
    mlngCollectNo = lngCollectNo
...
'保存选择的信息
    ...
    '初始化记录集
    strFileds = "key," & adLongVarChar & ",50|名称," & adLongVarChar & ",100|用量," & adDouble & ",16"
    ...
    With vsfItem
        For lngRow = .FixedRows To .Rows - 1
            If Trim(.Cell(flexcpData, lngRow, .ColIndex("项目"))) <> "" Then
                ...
                mrsItems.Fields("名称").Value = .TextMatrix(lngRow, .ColIndex("项目"))
                mrsItems.Fields("用量").Value = .TextMatrix(lngRow, .ColIndex("用量"))
                mrsItems.Update
            End If
        Next
    End With
...
Private Function ShowTree() As Boolean
    ...
    strSQL = " Select Rownum 序号,首次时间" & vbNewLine & _
        ...
        "             a.婴儿 = [3] And c.诊疗类别 = 'E' And c.执行性质 In (1, 2) And d.Id = c.诊疗项目id And a.相关id = c.Id And" & vbNewLine & _
        "             Nvl(d.执行分类, 0) In (1, 2)" & vbNewLine & _
...
' 功能:加载医嘱信息
    ...
    If mrsFileData Is Nothing Then
        gstrSQL = " Select b.Id, b.未记说明" & vbNewLine & _
            ...
            " Where a.Id = b.记录id And b.项目序号 = [1] And b.未记说明 Is Not Null And a.文件id = [2]"
        Set mrsFileData = zlDatabase.OpenSQLRecord(gstrSQL, "提取已经导入的数据", mlngCollectNo, mlng文件ID)
    End If
    ...
    gstrSQL = " Select a.Id, e.发送号, a.开嘱医生, a.医嘱内容, b.名称, a.单次用量, b.计算单位 单位, c.医嘱内容 用药途径, e.首次时间" & vbNewLine & _
            " From 病人医嘱发送 e, 病人医嘱记录 a, 诊疗项目目录 b, 病人医嘱记录 c, 诊疗项目目录 d" & vbNewLine & _
            " Where e.医嘱id = a.Id And a.诊疗类别 In ('5', '6', '7') And a.诊疗项目id = b.Id And a.病人id = [1] And a.主页id = [2] And a.婴儿 = [3] And" & vbNewLine & _
            "      e.首次时间 Between [5] And [6] And c.诊疗类别 = 'E' And c.执行性质 In (1, 2) And d.Id = c.诊疗项目id And a.相关id = c.Id And" & vbNewLine & _
    ...
    If Not rsTmp.EOF Then
        ...
        For i = 1 To rsTmp.RecordCount
            ...
            vsfList.TextMatrix(i, vsfList.ColIndex("用药途径")) = NVL(rsTmp!用药途径)
            vsfList.TextMatrix(i, vsfList.ColIndex("执行时间")) = Format(NVL(rsTmp!首次时间), "YYYY-MM-DD HH:mm")
...
Private Sub Form_Load()
    If mblnImportName = False Then
        lblDetail.Caption = "说明:由于未在样式构造中设置汇总项目和名称列的对照关系，本次将无法导入名称列."
    End If
    chkDay.Value = 1
End Sub
