...
'        '处理体温不升 体温为不升需要在35度下纵向输出体温不升二字
        ...
        Set rsPoints = ProcessPoint(int心率应用, rsPoints, rsNotes, strEditors)
...
Public Function ProcessPoint(ByVal int心率应用 As Integer, ByVal rsPoint As ADODB.Recordset, ByVal rsNotes As ADODB.Recordset, strEditor() As Variant) As ADODB.Recordset
    Dim lngValue As Long
    Dim lng项目序号 As Long
    Dim lng疼痛强度  As Long  '历史疼痛强度
    ...
    With rsNotes
        Do While Not .EOF
            If int心率应用 = 2 And !项目序号 = -1 Then
                rsPoint.Filter = "项目序号=" & gint脉搏 & " And X坐标<=" & !X坐标
            Else
                If Val(!项目序号) = gint体温 Or Val(!项目序号) = gint疼痛强度 Then
                    rsPoint.Filter = "项目序号=" & !项目序号 & " And  标记<>1 And X坐标<" & !X坐标
                Else
                    rsPoint.Filter = "项目序号=" & !项目序号 & " And X坐标<" & !X坐标
                End If
            End If
            rsPoint.Sort = "时间"
            If rsPoint.RecordCount <> 0 Then
                rsPoint.MoveLast
                Call Record_Update(rsPoint, gstrFields, gstrValues, "序号|" & rsPoint!序号)
            End If
            ...
            If Not IsNull(!序号) Then
                If Not ((Val(!项目序号) = gint体温 Or Val(!项目序号) = gint疼痛强度) And Val(!标记) = 1) Then
                    ...
                    If strTime <> "" Then
                        ...
                        End If
                    End If
                    strTime = Format(rsPoint!时间, "YYYY-MM-DD")
                    lngValue = Val(rsPoint!序号)
                End If
            End If
            .MoveNext
        Loop
    ...
    For i = 0 To UBound(Split(strTmp, ","))
        Call Record_Update(rsPoint, gstrFields, gstrValues, "序号|" & Split(strTmp, ",")(i))
    Next i
    
    '处理体温不升的.把前一个点的断开标志设置为1
    rsPoint.Filter = ""
    rsPoint.Filter = "项目序号=" & gint体温 & " and 标记<>1"
    rsPoint.Sort = "时间,标记"
    With rsPoint
        Do While Not .EOF
            If !数值 = "不升" And .AbsolutePosition <> 1 Then
                ...
                If Val(!断开) <> 1 Then
                    lngValue = !序号
                    Call Record_Update(rsPoint, gstrFields, gstrValues, "序号|" & lngValue)
                End If
                .MoveNext
            End If
        .MoveNext
   ...
   '处理疼痛不升不降.把前一个点的断开标志设置为1
    rsPoint.Filter = ""
    rsPoint.Filter = "项目序号=" & gint疼痛强度 & " and 标记<>1"
    rsPoint.Sort = "时间,标记"
    lng疼痛强度 = 0
    With rsPoint
        If rsPoint.RecordCount > 1 Then
            Do While Not .EOF
                If !数值 = lng疼痛强度 And .AbsolutePosition <> 1 Then
                    .MovePrevious '更新上一行断开标记
                    If Val(!断开) <> 1 Then
                        lngValue = !序号
                        Call Record_Update(rsPoint, gstrFields, gstrValues, "序号|" & lngValue)
...
'                    Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), Format(sin刻度, "#0"), zlCommFun.Nvl(!记录色, RGB_BLACK))
                ...
                If Val(Format(sin刻度, "#0.00")) <= Val(Format(zlCommFun.Nvl(!最小值), "#0.00")) Or Format(lngCurY, "#0") > T_DrawClient.刻度区域.Bottom Then
                    ...
                    If blnDoubleRow = False And (sinAlertness < Val(Nvl(!最大值)) And sinAlertness > Val(Nvl(!最小值))) Then
                        ...
                        '自贡地区要求疼痛项目需在刻度4再画蓝色警戒线
                        If Val(Nvl(!项目序号)) = gint疼痛强度 And gint疼痛强度 <> 0 Then
                            lngCurAlerY = Val(GetYCoordinate(objDraw, rsDrawItems, Val(Nvl(!项目序号)), 4))
                            Call DrawLine(lngDC, T_DrawClient.体温区域.Left, lngCurAlerY, lngMaxX, lngCurAlerY, PS_SOLID, intBold, RGB_BLUE)
                        End If
                    End If
...
'                        Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), Val(Format(sin刻度, "#0.0")), Nvl(!记录色, RGB_BLACK))
                    ...
                    If Val(Format(sin刻度, "#0.00")) <= Val(Format(Nvl(!最小值), "#0.00")) Or Format(lngCurY, "#0") > lngMaxY Then
                        ...
                        If blnDoubleRow = False And sinAlertness > Val(Nvl(!最小值)) And sinAlertness < Val(Nvl(!最大值)) Then
                            ...
                            If Val(Nvl(!项目序号)) = gint疼痛强度 And gint疼痛强度 <> 0 Then
                                lngCurAlerY = Val(GetYCoordinate(objDraw, rsDrawItems, Val(Nvl(!项目序号)), 4))
                                Call DrawLine(lngDC, lngX + lng刻度宽度, lngCurAlerY, lngMaxX, lngCurAlerY, PS_SOLID, 1, RGB_BLUE)
                            End If
                        End If
                        Exit Do
                    End If
...
'                        Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), "v", lngRGB)
            ...
            Else
                ...
                If Val(!数值) > Val(dblvalue) Then
                    ...
                    Else
                        Call DrawLine(lngDC, !X坐标 + T_DrawClient.列单位 / 2, !Y坐标 + (T_Size.H / 4), SinX + T_DrawClient.列单位 / 2, sinY, IIf(blnPrinter, PS_DASH, PS_DOT), 1, RGB_BLUE, False)
                    End If
                ElseIf Val(!数值) < Val(dblvalue) Then
                    ...
                    Else
                        Call DrawLine(lngDC, !X坐标 + T_DrawClient.列单位 / 2, !Y坐标 - (T_Size.H / 2), SinX + T_DrawClient.列单位 / 2, sinY, IIf(blnPrinter, PS_DASH, PS_DOT), 1, RGB_BLUE, False)
                    End If
