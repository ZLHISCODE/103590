            ...
            Do While True
                ...
                If Val(Format(sin刻度, "#0.00")) <= Val(Format(!最小值, "#0.00")) Or Format(lngCurY, "#0") > T_DrawClient.刻度区域.Bottom Then
                    ...
                    If (sinAlertness < Val(Nvl(!最大值)) And sinAlertness > Val(Nvl(!最小值))) Then
                        ...
                        '自贡地区要求疼痛项目在刻度4画蓝色警戒线
                        If Val(Nvl(!项目序号)) = gint疼痛强度 And gint疼痛强度 <> 0 Then
                            lngCurAlerY = Val(GetYCoordinate(mobjDraw, mrsDrawItems, Val(Nvl(!项目序号)), 4))
                            Call DrawLine(mlngMemDC, T_DrawClient.体温区域.Left, lngCurAlerY, lngMaxX, lngCurAlerY, PS_SOLID, 1, RGB_BLUE)
                        End If
                    End If
                    Exit Do
                End If
                ...
                Do While True
                    ...
                    If Val(Format(sin刻度, "#0.00")) <= Val(Format(Nvl(!最小值), "#0.00")) Or Format(lngCurY, "#0") > lngMaxY Then
                        ...
                        If blnDoubleRow = False And sinAlertness > Val(Nvl(!最小值)) And sinAlertness < Val(Nvl(!最大值)) Then
                            ...
                            '自贡地区要求疼痛项目在刻度4画蓝色警戒线
                            If Val(Nvl(!项目序号)) = gint疼痛强度 And gint疼痛强度 <> 0 Then
                                sinCurAlerY = Val(GetYCoordinate(mobjDraw, mrsDrawItems, Val(Nvl(!项目序号)), 4))
                                Call DrawLine(mlngMemDC, T_DrawClient.体温区域.Left, CLng(sinCurAlerY), lngMaxX, CLng(sinCurAlerY), PS_SOLID, 1, RGB_BLUE)
                            End If
            ...
            If Val(zlCommFun.Nvl(!状态)) <> 3 Then
                If (!项目序号 = gint体温 Or !项目序号 = gint疼痛强度) And !标记 = 1 Then
                    ...
                    If Val(!数值) > Val(str原值) Then
                        ...
                        Else
                            Call DrawLine(mlngMemDC, !X坐标 + T_DrawClient.列单位 / 2, !Y坐标 + (T_Size.H / 4), sinX坐标原 + T_DrawClient.列单位 / 2, sinY坐标原, PS_DOT, 1, RGB_BLUE, False)
                        End If
                    ElseIf Val(!数值) < Val(str原值) Then
                        ...
                        Else
                            Call DrawLine(mlngMemDC, !X坐标 + T_DrawClient.列单位 / 2, !Y坐标 - (T_Size.H / 2), sinX坐标原 + T_DrawClient.列单位 / 2, sinY坐标原, PS_DOT, 1, RGB_BLUE, False)
                        End If
                    End If
                Else
...
Private Sub ProcessPoint(ByVal strBeginDate As String)
    ...
    mrsPoint.Sort = "时间,标记"
    With mrsPoint
        If mrsPoint.RecordCount > 1 Then
            Do While Not .EOF
    ...
    '处理疼痛强度不升的,把前一个点的端开标志设置为1
    mrsPoint.Filter = ""
    mrsPoint.Filter = "项目序号=" & gint疼痛强度 & " and 标记<>1"
    mrsPoint.Sort = "时间,标记"
    lng疼痛强度 = 0
    With mrsPoint
        Do While Not .EOF
            If Val(!数值) = lng疼痛强度 And .AbsolutePosition <> 1 Then
                .MovePrevious '更新上一行断开标记
                If Val(zlCommFun.Nvl(!断开)) <> 1 Then
                    lngOrder = !序号
                    Call Record_Update(mrsPoint, gstrFields, gstrValues, strPrimary & lngOrder)
