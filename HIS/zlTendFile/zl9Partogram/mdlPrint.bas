...
'----------------------------------------------------------------------
    ...
    With rsCurData
        Do While Not .EOF
            ...
            '93083,陈刘,
            If !时间 = strBirthTime And strBirthTime <> "" And !项目序号 = IIf(int标志内容 = 1, T_Partogram.lng先露高低, T_Partogram.lng宫口扩大) Then
                Call SetTextColor(mlngDC, vbRed)
                Call DrawText(mlngDC, "☉", -1, T_LableRect, DT_CENTER)
            Else
...
'-------------------------------------------------------------------------
    ...
    Dim i As Integer, int标志内容 As Integer
    Dim strtmp As String
...
ErrEnd:
    ...
    If int标志内容 = 3 Then
        ...
        lngCurY = T_DrawClient.表格区域.Top + ((lngCurMaxY - T_DrawClient.表格区域.Top - ((Len(strtmp) - 1) * mobjDraw.TextHeight("1") / sinTwipsPerPixelY)) / 2)
        For i = 1 To Len(strtmp)
            strConnect = Mid(strtmp, i, 1)
            Call DrawRotateText(mobjDraw, mlngDC, lngCurX, lngCurY, Trim(strConnect), T_DrawClient.刻度单位 / 2, IIf(i = 1, RGB_RED, RGB_BLUE))
