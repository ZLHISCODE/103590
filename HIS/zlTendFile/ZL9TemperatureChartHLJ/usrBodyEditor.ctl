   ...
   Begin VB.PictureBox picBack 
      ...
      Begin VB.PictureBox picMain 
         ...
         Begin VSFlex8Ctl.VSFlexGrid mshDownTab 
            ...
            AccessibleRole  =   24
         End
         Begin zl9TemperatureChartHLJ.VsfGrid vsf 
            Height          =   255
            Left            =   120
            TabIndex        =   27
            Top             =   3480
...
'---------------------------------------------------------------------
    ...
    With mshUpTab
        ...
        If UBound(Split(T_BodyStyle.str列头名称, "@")) > 1 Then
            .Cell(flexcpFontSize, 1, 0, 1, 0) = IIf(mobjDraw.TextWidth(Split(T_BodyStyle.str列头名称, "@")(2)) > .ColWidth(0), 7.5, 9)
            .TextMatrix(1, 0) = Split(T_BodyStyle.str列头名称, "@")(2)
        Else
            .TextMatrix(1, 0) = "手术、产后日数"
        End If
        If UBound(Split(T_BodyStyle.str列头名称, "@")) > 0 Then
            .Cell(flexcpFontSize, 2, 0, 2, 0) = IIf(mobjDraw.TextWidth(Split(T_BodyStyle.str列头名称, "@")(1)) > .ColWidth(0), 7.5, 9)
            .TextMatrix(2, 0) = Split(T_BodyStyle.str列头名称, "@")(1)
        Else
            .TextMatrix(2, 0) = IIf(T_Patient.lng婴儿 = 0, "住 院 天 数", "出 生 天 数")
        End If
        
        If UBound(Split(T_BodyStyle.str列头名称, "@")) > 2 Then
            .Cell(flexcpFontSize, 3, 0, 3, 0) = IIf(mobjDraw.TextWidth(Split(T_BodyStyle.str列头名称, "@")(3)) > .ColWidth(0), 7.5, 9)
...
'----------------------------------------------------------------
    ...
    With mshUpTab
        ...
        For i = 1 To T_BodyStyle.lng天数
            mstrOpValue(i) = .TextMatrix(1, ((i - 1) * T_BodyStyle.lng监测次数 + 1))
            mstrOpdays(i) = .TextMatrix(1, ((i - 1) * T_BodyStyle.lng监测次数 + 1))
        Next i
                ...
                If Trim(mstrOpdays(i)) <> "" And lngValue + UBound(arrOperDay) < 12 Then
                    ...
                    If mblnStopFlag Then '手术标注后天数在次手术时停止前一次标注
                        Select Case mintOpFormat
                            Case 1 '--显示0
                                mstrOpValue(i) = .TextMatrix(1, ((i - 1) * T_BodyStyle.lng监测次数 + 1)) & "0" & .TextMatrix(1, ((i - 1) * T_BodyStyle.lng监测次数 + 1))
                            Case 2 '--显示次数
                                If strTmp = "Ⅰ" Then
                                    mstrOpValue(i) = 0
                                Else
        ...
        Next i
        '设定日期，住院天数文本颜色
        mshUpTab.Cell(flexcpForeColor, 0, mshUpTab.FixedCols, 0, mshUpTab.Cols - 1) = 16711680
        mshUpTab.Cell(flexcpForeColor, 2, mshUpTab.FixedCols, 2, mshUpTab.Cols - 1) = 16711680
        '设定手术 分娩文本颜色
        '51283,刘鹏飞,2012-07-11
        lngColor = Val(zlDatabase.GetPara("手术天数显示颜色", glngSys, 1255, "255"))
        mshUpTab.Cell(flexcpForeColor, 1, mshUpTab.FixedCols, 1, mshUpTab.Cols - 1) = lngColor
...
'-------------------------------------------------------------------------------------------
    ...
    Else '标准体温单
        ...
        T_BodyStyle.str列头名称 = "日       期@" & IIf(T_Patient.lng婴儿 = 0, "住 院 天 数", "出 生 天 数") & "@手术、产后日数@时       间"
