   ...
   Begin VB.PictureBox picBack 
      ...
      Begin VB.PictureBox picMain 
         ...
         Begin VSFlex8Ctl.VSFlexGrid mshDownTab 
            ...
            AccessibleRole  =   24
         End
         Begin zl9TemperatureChartJX.VsfGrid vsf 
            Height          =   255
            Left            =   120
            TabIndex        =   27
            Top             =   3480
...
Private mlngStopFlag As Long '手术停止标志
Private mintOpFormat As Integer '手术当天缺省格式 0-不显示;1-显示0;2-显示手术次数
Private mintRepairRows As Integer '体温表格固定输出行数
Private mintBreatheShowType As Integer '呼吸输出方式:0-呼吸值以R代替(默认),1-上下箭头标识,2-呼吸机以A+呼吸值表示
    ...
    Select Case strMenuItem
        ...
        Case "体温数据显示设置"
            ...
            If mstr开始时间 <> "" Then
                ...
                If frmCaseTendBodyOper.ShowEditor(UserControl.Extender.ParentForm, strInput, CDate(strTime), CDate(Format(mstrEndDate, "YYYY-MM-DD HH:mm:ss")), mint心率应用, mblnMoved, FontSize) = True Then
                    '保存成功后刷新体温单显示
                    strParam = mstrParam2
                    picMain.Tag = ""
        ...
        Case "体温数据编辑"
            ...
            If picMain.Tag = "" Then picMain.Tag = mstr开始时间 & ";" & mstr结束时间
                ...
                End If
                strInput = T_Patient.lng病人ID & ";" & T_Patient.lng主页ID & ";" & T_Patient.lng文件ID & ";" & T_Patient.lng婴儿 & ";" & T_Patient.lng科室ID & ";" & T_Patient.lng护理等级
                If frmCaseTendBodyData.ShowEditor(UserControl.Extender.ParentForm, strInput, strTime, mstr开始时间, mint心率应用, mblnMoved, FontSize) = True Then
                    '保存成功后刷新体温单显示
...
'----------------------------------------------------------------
    ...
    Dim bln出院后继续标注日期和住院天数 As Boolean
    ...
    With mshUpTab
        ...
        For intCOl = 1 To lng天数
            ...
            '91960,陈刘,2016-1-13,出院后停止标注日期和天数
            If strTmp > mstrEndDate And Not bln出院后继续标注日期和住院天数 And mbln出院 Then
                For i = 1 To lng频次
        ...
        mlngStopFlag = Val(zlDatabase.GetPara("再次手术停止前次标注", glngSys, 1255, "0"))
            ...
            If DateDiff("d", mstr开始时间, str结束时间) + 1 >= i Then
                ...
                If Trim(mstrOpdays(i)) <> "" And lngValue + UBound(arrOperDay) < 12 Then
                    ...
                    For j = UBound(arrOperDay) + 1 To 1 Step -1
                        ...
                        End If
                        If mlngStopFlag = 1 Then Exit For
                    Next j
...
Private Function ShowDowntab() As Boolean
    ...
    '2----开始输出呼吸数据 呼吸机为图形输出
    mintBreatheShowType = Val(zlDatabase.GetPara("呼吸表格呼吸机输出方式", glngSys, 1255, 0))
...
ErrNext:
    ...
    For i = 1 To T_BodyStyle.lng监测次数 * T_BodyStyle.lng天数
        ...
        If IsNumeric(str结果) Then
            ...
            If mintBreatheShowType = 2 And strPart = "呼吸机" Then vsf.TextMatrix(1, intCOl) = "A" & str结果
...
Public Sub Paint_CanvasNew(Optional ByVal blnAdjust As Boolean = False)
    ...
    If mbln呼吸曲线 = True Then
        ...
        With rsTemp
            Do While Not .EOF
                lng最高行 = Val(zlCommFun.Nvl(!最高行))
                If lng最高行 < 0 Then lng最高行 = 0
                lngMaxRows = lng最高行 + ((Val(Nvl(!最大值, 0)) - Val(Nvl(!最小值, 0))) / Val(Nvl(!单位值)))
                lngMaxRows = lngMaxRows
                T_DrawClient.总列数 = T_DrawClient.总列数 + lngMaxRows
                .MoveNext
            Loop
        End With
    ...
    With rsTemp
        Do While Not .EOF
            ...
            lngY = lngMaxY - T_DrawClient.行单位 * ((rsTemp!最大值 - rsTemp!最小值) / rsTemp!单位值)
            lngCurY = lngY
            lngCurX = T_DrawClient.偏移量X
            lngCurveRows = ((Val(Nvl(!最大值, 0)) - Val(Nvl(!最小值, 0))) / Val(Nvl(!单位值)))
            If lngCurveRows > 0 Then
                ...
                For lngRow = 1 To Len(Nvl(!记录名 & " " & !单位))
                    Call GetTextRect(mobjDraw, lngCurX + Fix((int体温) * lng刻度宽度 / (intLables)), lngCurY, Mid(Nvl(!记录名 & " " & !单位), lngRow, 1))
                    Call DrawText(mlngMemDC, Mid(Nvl(!记录名 & " " & !单位), lngRow, 1), -1, T_LableRect, DT_RIGHT)
                ...
                Do While True
                    ...
                    If blnFirst = False Then     '刚进入循环，此时取的最大值
                        sin刻度 = Nvl(!最大值, 0)
                        sinBegin刻度 = sin刻度
                        str最大值坐标 = Fix(T_DrawClient.体温区域.Left / intLables) * 2 & "," & lngCurY
                        blnFirst = True
                    Else                    '计算得到每个刻度的值
                        sin刻度 = sin刻度 - dbl单位值     '如果目前显示模式为双倍，则按双倍累计
                    End If
                    ...
                    If bln显示刻度 Then
                        '控制最大值不与曲线单位重复
                        lngCurX = Fix((T_DrawClient.体温区域.Left - T_DrawClient.偏移量X) / intLables) * (int体温 + 1) - mobjDraw.ScaleX(mobjDraw.TextWidth(Val(Format(sin刻度, "#0.0"))), vbTwips, vbPixels) + T_DrawClient.偏移量X
                        lngCurX = lngCurX - (mobjDraw.ScaleY(mobjDraw.TextHeight("1"), vbTwips, vbPixels) \ 3)
                        If lngCurY >= lngMaxY Then
                        ...
                        End If
                        Call DrawText(mlngMemDC, Val(Format(sin刻度, "#0.0")), -1, T_LableRect, DT_RIGHT)
                    End If
...
'-----------------------------------
    ...
    If Val(vsf.ColData(Col)) <> 0 Then
        ...
        End If
        '绘制呼吸值或图形
        If mintBreatheShowType = 2 Then
            Set objDraw = picBack
            If vsf.TextMatrix(1, Col) <> "" And InStr(vsf.TextMatrix(1, Col), "A") > 0 Then
                Set stdSet = New stdFont
                stdSet.Name = "宋体"
                stdSet.Size = 7
                Call SetFontIndirect(stdSet, hDC, objDraw)
                lngFont = CreateFontIndirect(T_Font)
                lngOldFont = SelectObject(hDC, lngFont)
                Call SetTextColor(hDC, Val(vsf.Tag))
                T_Size.H = objDraw.TextHeight("A") / T_TwipsPerPixel.Y
                T_Size.W = objDraw.TextWidth("A") / T_TwipsPerPixel.X
                If vsf.Cell(flexcpAlignment, 1, Col, 1, Col) = flexAlignCenterBottom Then Top = Top + Bottom - T_Size.H - 2
                '画出呼吸值,由于GetTextRect函数默认给X+1所以此处-1
                strPart = vsf.TextMatrix(1, Col)
                Call GetTextRect(objDraw, Left, Top + 4, strPart, Right - Left - 3, , False)
                Call DrawText(hDC, strPart, -1, T_LableRect, DT_CENTER)
            End If
        Else
