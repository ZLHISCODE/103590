        ...
        If blnAdd = True Then
            ...
            For i = LBound(arrTmp) To UBound(arrTmp)
                ...
                If Val(varCode(2)) = lngItemCode And varCode(4) = strItemName Then
                    mobjVsf.Rows = mobjVsf.Rows + 2
                    arrStr = Split(varCode(1), ",")
                    If UBound(arrStr) > 6 Then arrStr(7) = strPart
                    varCode(1) = Join(arrStr, ",")
                    mobjVsf.RowData(intRow) = Split(varCode(1), ",")(3) & ";" & IIf(IsWaveItem(varCode(2)), 2, 0)
                    If IsTatle(varCode(2)) Then mobjVsf.RowData(intRow) = Split(varCode(1), ",")(3) & ";" & "3"
                    mobjVsf.RowData(intRow + 1) = mobjVsf.RowData(intRow)
                    ...
                    mobjVsf.TextMatrix(intRow, col_tab原始时间) = Replace(Space(Split(varCode(1), ",")(3) - 1), " ", "'")
                    mobjVsf.TextMatrix(intRow, COL_tabDirect) = "时间"
                    mobjVsf.TextMatrix(intRow + 1, COL_tab字符串) = varCode(1)
                    mobjVsf.TextMatrix(intRow + 1, COL_tab项目序号) = lngItemCode
                    mobjVsf.TextMatrix(intRow + 1, COL_tab项目名) = strItemName
                    mobjVsf.TextMatrix(intRow + 1, COL_TabNull) = ""
                    mobjVsf.TextMatrix(intRow + 1, COL_tab项目名称) = varCode(3)
                    mobjVsf.TextMatrix(intRow + 1, col_tab原始时间) = Replace(Space(Split(varCode(1), ",")(3) - 1), " ", "'")
                    mobjVsf.TextMatrix(intRow + 1, COL_tabDirect) = "数据"
                    For intRow = mobjVsf.Rows - 3 To mobjVsf.Rows - 1
                        '设置竖边框
                        For intCOl = 0 To (Val(Split(mobjVsf.RowData(intRow), ";")(0))) - 1
                            mobjVsf.CellBorderRange intRow, mobjVsf.FixedCols + intCOl, intRow, mobjVsf.FixedCols + intCOl, mobjVsf.GridColor, 0, IIf(intCOl + 1 > lngPreNum And intRow > mobjVsf.FixedRows, 1, 0), 1, IIf(mobjVsf.FixedCols + intCOl < mobjVsf.Cols, 1, 0), 0, 0
                        Next intCOl
                        lngPreNum = (Val(Split(mobjVsf.RowData(intRow), ";")(0)))
                    Next intRow
                    
                    '根据背景频次决定背景色
                    For intRow = mobjVsf.Rows - 3 To mobjVsf.Rows - 1
                        If mobjVsf.FixedCols + (Val(Split(mobjVsf.RowData(intRow), ";")(0))) < mobjVsf.Cols Then
                           mobjVsf.Cell(flexcpBackColor, intRow, mobjVsf.FixedCols + (Val(Split(mobjVsf.RowData(intRow), ";")(0))), intRow, mobjVsf.Cols - 1) = &H8000000F
                        End If
                    Next intRow
                    
                    mobjVsf.Cell(flexcpAlignment, 0, 0, mobjVsf.Rows - 1, mobjVsf.Cols - 1) = 4
                    '定位到新添加的行
    ...
    For intRow = mobjVsf.FixedRows To mobjVsf.Rows - 1
        ...
        If Val(Split(mobjVsf.TextMatrix(intRow, COL_tab字符串), ",")(5)) = 2 Then
            ...
            If blnDelete = True Then
                mblnChage = True
                If mobjVsf.Rows = mobjVsf.FixedRows + 2 And intRow = mobjVsf.FixedRows Then
                    '主要处理可能没有绑定固定项目的情况
                    ...
                    mobjVsf.TextMatrix(intRow + 1, COL_tab字符串) = varCode(1)
                    mobjVsf.TextMatrix(intRow + 1, COL_tab项目序号) = varCode(2)
                    mobjVsf.TextMatrix(intRow + 1, COL_tab项目名) = varCode(4)
                    mobjVsf.TextMatrix(intRow + 1, COL_TabNull) = ""
                    mobjVsf.TextMatrix(intRow + 1, COL_tab项目名称) = varCode(3)
                    mobjVsf.Tag = "NO"
                    '定位到新添加的行
...
'---------------------------------------------------------------------
    ...
    mstrSQL = "" & _
            ...
            "            Where a.项目序号 = b.项目序号 And b.项目序号 = c.项目序号(+) And A.记录法=2 And b.项目性质 = 2 And Nvl(b.应用方式, 0) = 1 And NVL(B.应用场合,0)<>2 And" & vbNewLine & _
