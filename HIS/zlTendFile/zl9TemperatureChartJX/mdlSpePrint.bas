...
Public Function PrintOrPreviewBodyStateNew(objOut As Object, _
    ...
    Dim bln出院后继续标注日期和住院天数 As Boolean  '出院后是否继续标注日期和住院天数
    '其他绘图变量
    ...
    Dim int心率应用 As Integer, str心率信息 As String
    ...
    Dim int项目性质 As Integer, int项目类型 As Integer, strTabItemTemp As String
    ...
    Dim ArrComTable() As Variant '记录所有的表下表格项目 (项目序号||部位+项目名称|项目单位||项目值域||记录频次||项目性质||项目表示||入院首测||部位||记录色))
    ...
    bln出院后继续标注日期和住院天数 = (Val(zlDatabase.GetPara("出院后继续标注日期和住院天数", glngSys, 1255, "1")) = 1)
    ...
    lng心率颜色 = RGB_RED
    str心率信息 = ""
    strSQL = "Select a.应用方式,a.项目名称,b.记录符,b.记录色 From 护理记录项目 a,体温记录项目 b Where a.项目序号=-1 And a.项目序号=b.项目序号"
    ...
    If rsTemp.BOF = False Then
        ...
        str心率信息 = int心率应用 & "'" & rsTemp!项目名称 & "'" & str心率符号 & "'" & lng心率颜色
    ...
    If rsTemp.RecordCount > 0 Then
        rsTemp.MoveFirst
        rsTemp.Filter = "记录法=1 and 项目序号 <> " & gint呼吸
        ...
        If rsTemp.RecordCount > 0 Then mbln呼吸曲线 = True
        rsTemp.Filter = 0
    Else
        CloseRs rsTemp
    ...
    For lngPage = 1 To lngCountPage
        ...
        For i = 0 To T_BodyStyle.lng天数 - 1
            ...
            End If
            '91960,陈刘,2016-1-16,出院后继续标注日期和天数
            If strTmp > strEndDay And Not bln出院后继续标注日期和住院天数 And bln出院 Then
                strTmpString0 = strTmpString0 & "'" & ""
        ...
        '2.提取手术时间和次数
        strTmpString1 = GetOperationTag(blnMoved, lng病人ID, lng主页ID, lng文件ID, intBaby, lngValue, strEndDay, strTmpDay)
        ...
        '----------------------------------------------------------------------------------------------
        '提取所有特殊项目信息包括活动项目（活动项目可能存在一个项目多个部位也要提取）
        If intRepairRows <> T_BodyStyle.lng表格空行 Then
            ArrComTable = Array()
            Set rsItems = GetAppendGridItemNew(lng文件ID, lng病人ID, lng主页ID, lng护理等级, intBaby, CDate(strTmpDay), CDate(strEndDate), IIf(intBaby = 0, 1, 2), lngSectID, T_BodyItem.str表格项目, blnMoved)
            bln呼吸 = False
            With rsItems
                Do While Not .EOF
                    If Val(Nvl(!项目序号)) <> 5 Then
                        If Val(Nvl(!项目序号)) = 4 Then
                            str项目名称 = "血压"
                        Else
                            str项目名称 = Nvl(!记录名)
                        End If
                        ...
                        strTmp = zlCommFun.Nvl(!项目序号) & "||" & Replace(str项目名称, ";", ":") & "||" & zlCommFun.Nvl(!单位) & "||" & _
                            zlCommFun.Nvl(!项目值域) & "||" & int频次 & "||" & zlCommFun.Nvl(!项目性质, 1) & "||" & _
                            zlCommFun.Nvl(!项目表示) & "||" & zlCommFun.Nvl(!项目类型) & "||" & zlCommFun.Nvl(!入院首测, 0) & "||" & Nvl(!体温部位) & "||" & Nvl(!记录色, RGB_BLUE)
            ...
            '此处的计算请和DrawBodyPageFooterNew保持一致
             sinY = 0
             If (Val(zlDatabase.GetPara("体温单不打印曲线说明", glngSys, 1255, "0")) = 1) = False Then
                sinY = 14 * msngTwips
             Else
                sinY = 6 * msngTwips
             End If
             If (Val(zlDatabase.GetPara("打印页号", glngSys, 1255, "1")) = 1) Or (Val(zlDatabase.GetPara("打印周数", glngSys, 1255, "0")) = 1) Or _
                (Val(zlDatabase.GetPara("打印打印人", glngSys, 1255, "0")) = 1) Then
                sinY = 12 * msngTwips
            End If
            ...
            If T_DrawClient.行单位 < 5 Then T_DrawClient.行单位 = 5
            ...
            '为了避免空余太多的内容，此处可根据表格行数以及纸张信息重新计算表格高度
            '说明：由于画线单位只能是整数，曲线部分内容自动缩放可能会导致出现多余空白部分(有时+1可能导致表格输出不玩,不+有留下部分空白)
            sngHPrint = sngHTab - (intRepairRows * T_DrawClient.时间列单位 + IIf(bln呼吸 = True, mlngBreatheHeight - T_DrawClient.时间列单位, 0)) '空白部分
            If sngHPrint > 0 Then
                If bln呼吸 = True Then
                    If intRepairRows - 1 > 0 And sngHPrint - mlngBreatheHeight > 0 Then
                        T_DrawClient.时间列单位 = T_DrawClient.时间列单位 + (sngHPrint - mlngBreatheHeight) \ (intRepairRows - 1)
                    End If
                Else
                    T_DrawClient.时间列单位 = T_DrawClient.时间列单位 + sngHPrint \ intRepairRows
                End If
            End If
            
            If T_DrawClient.时间列单位 > 20 * msngTwips Then T_DrawClient.时间列单位 = 20 * msngTwips
        ...
        rsTemp.Sort = "排列序号"
        rsTemp.MoveFirst
        Call DrawCanvasNew(lngDC, objDraw, rsTemp, rsDrawItems, bln不打印心率列, str心率信息, sngScale)
        Call SelectObject(lngDC, lngOldFont)
        Call DeleteObject(lngFont)
        Call ReleaseFontIndirect(objDraw)
        ...
        '删除显示为2的数据
        Call DeleteRsTemp(rsPoints)
        Call DeleteRsTemp(rsNotes)
...
'        '处理体温不升 体温为不升需要在35度下纵向输出体温不升二字
        ...
        Set stdSet = New stdFont
        stdSet.Name = "宋体"
        stdSet.Size = ((T_DrawClient.行单位 * T_TwipsPerPixel.Y * 2) / objDraw.TextHeight("1")) * 9
        stdSet.Bold = False
        If stdSet.Size > 9 Then stdSet.Size = 9
        If stdSet.Size < 7.5 Then stdSet.Size = 7.5
        Call SetFontIndirect(stdSet, lngDC, objDraw)
        lngFont = CreateFontIndirect(T_Font)
        lngOldFont = SelectObject(lngDC, lngFont)
        ...
        If intRepairRows = T_BodyStyle.lng表格空行 Then GoTo ZeroLine
        '开始处理表下表格（特殊项目栏）
...
ErrNext:
            ...
            Else
                ...
                For j = 0 To T_BodyStyle.lng天数 - 1
                    ...
                    Set rsDownTab = ReturnItemRecord(rsTmp, Int(CDate(strBegin)), CDate(strBeginDate), lng项目序号 & ";" & str项目名称 & ";" & _
                                    int频次 & ";" & Val(strTmp) & ";" & int项目性质 & ";" & strTabItemTemp, bln汇总当天, bln录入小时)
        ...
        '项目序号'部位+项目名称'记录频次'项目性质'记录色'项目内容
        For i = 0 To UBound(ArrComTable)
            strTmpString0 = ""
        ...
        If bln显示皮试 = True Then
            ...
            ArrNewString(UBound(ArrNewString)) = "-999'皮试结果'1'1'0" & "'" & strNewTmpString
        End If
        
        lngCurX = X
ZeroLine:
        '开始绘画表格项目并展示数据
        If intRepairRows = T_BodyStyle.lng表格空行 Then ReDim ArrNewString(0): ArrNewString(0) = ""
        Set stdSet = New stdFont
        stdSet.Name = "宋体"
        stdSet.Size = 9 * sngScale
        stdSet.Bold = False
        ...
        lngFont = CreateFontIndirect(T_Font)
        lngOldFont = SelectObject(lngDC, lngFont)
        Call DrawBodyRecordItemNew(lngDC, objDraw, ArrNewString, lngCurX, T_DrawClient.曲线总区域.Bottom, T_DrawClient.体温区域.Right, intRepairRows, lngCurY, sngScale)
        Call SelectObject(lngDC, lngOldFont)
        Call DeleteObject(lngFont)
        Call ReleaseFontIndirect(objDraw)
        ...
        Call DrawBodyPageFooterNew(lngDC, objDraw, lngCurX, lngCurY, T_DrawClient.体温区域.Right, intPageNo, intEndPage, lngButtom, sngScale)
        '页脚图形输出
...
ErrExit:
    ...
    GoTo ErrClare
ErrClare:
    Call ClearData(M_DrawClient, lng刻度宽度)
    Call ErrEmpty
    Set stdSet = Nothing
End Function
...
Private Function DeleteRsTemp(ByRef rsTmp As ADODB.Recordset)
    '92278,陈刘
    '删除显示为2的
    rsTmp.Filter = ""
    rsTmp.Filter = "显示=2"
...
Private Function GetOperationTag(ByVal blnMoved As Boolean, _
                                        ByVal lng病人ID As Long, _
                                        ByVal lng主页ID As Long, _
                                        ByVal lng文件ID As Long, _
                                        ByVal intBaby As Integer, _
                                        ByVal lngValue As Long, _
                                        ByVal strEndDay As String, _
                                        ByVal strTmpDay As String) As String
    Dim strSQL As String
    Dim strTime As String
    Dim strTmp As String, strTmp1 As String
    Dim arrOpdays() As String
    Dim arrOpValue() As String
    Dim arrOperDay
    Dim str结束时间 As String
    ...
    Dim lngStopFlag As Long '再次手术停止前次标注
    ...
    lngStopFlag = Val(zlDatabase.GetPara("再次手术停止前次标注", glngSys, 1255, "0"))
    ...
    End If
     ...
     strSQL = getSQLString("提取当前手术信息", blnMoved)
        ...
        ReDim arrOpdays(1 To T_BodyStyle.lng天数) As String
        ReDim arrOpValue(1 To T_BodyStyle.lng天数) As String
        ...
        Do While Not rsTmp.EOF
            ...
            If Not rsTmp.EOF Then
                If bln术后显示 And DateDiff("d", CDate(Format(strTime, "YYYY-MM-DD")), str结束时间) < intOpDays Then
                    str结束时间 = Format(DateAdd("D", intOpDays, CDate(Format(strTime, "YYYY-MM-DD"))), "YYYY-MM-DD") & " 23:59:59"
                End If
            End If
                ...
                If DateDiff("d", strTmpDay, str结束时间) + 1 >= i Then
                    ...
                    Select Case intDays
                        Case 0 '当前区域内的手术开始时间
                            ...
                            If Trim(arrOpdays(i)) <> "" Then
                                arrOpdays(i) = strTime & "/" & arrOpdays(i)
                            Else
                                arrOpdays(i) = strTime
                            End If
                        Case Else
                            If intDays >= 1 And intDays <= intOpDays Then '手术开始天数
                                Select Case lngStopFlag  '手术标注后天数在次手术时停止前一次标注
                                
                                Case 0
                                    If Trim(arrOpValue(i)) <> "" Then
                                        arrOpValue(i) = intDays & "/" & arrOpValue(i)
                                    Else
                                        arrOpValue(i) = intDays
                                    End If
                                Case 1
                                    arrOpValue(i) = intDays
                                ...
                                Case 2
                                    If Trim(arrOpValue(i)) <> "" Then
                                        arrOpValue(i) = arrOpValue(i) & "/" & intDays
                                    Else
                                        arrOpValue(i) = intDays
                                    End If
                                End Select
        ...
        For i = 1 To T_BodyStyle.lng天数
            If DateDiff("d", Int(CDate(Format(strTmpDay, "yyyy-mm-dd hh:mm:ss"))), Int(CDate(Format(str结束时间, "yyyy-mm-dd hh:mm:ss")))) + 1 >= i Then
                If Trim(arrOpdays(i)) <> "" Then
                    arrOperDay = Split(arrOpdays(i), "/")
                Else
                    arrOperDay = Split("1", "/")
                End If
                lngValue = lng次数
                If Trim(arrOpdays(i)) <> "" And lngValue + UBound(arrOperDay) < 12 Then
                    strTmp = "": strTmp1 = ""
                    For j = UBound(arrOperDay) + 1 To 1 Step -1
                        ...
                        End If
                        If lngStopFlag = 1 Then Exit For
                    Next j
                    ...
                    Select Case lngStopFlag  '手术标注后天数在次手术时停止前一次标注
                        Case 0
                            Select Case intOpFormat
                                Case 1 '显示0
                                    If Trim(arrOpValue(i)) <> "" Then
                                        arrOpValue(i) = 0 & "/" & arrOpValue(i)
                                    Else
                                        arrOpValue(i) = 0
                                    End If
                                Case 2 '显示手术次数
                                    If Trim(arrOpValue(i)) <> "" Then
                                        arrOpValue(i) = strTmp & "/" & arrOpValue(i)
                                    Else
                                        arrOpValue(i) = strTmp
                                    End If
                                Case 3
                        ...
                        Case 1
                            Select Case intOpFormat
                                Case 1 '显示0
                                    If strTmp = "Ⅰ" Or arrOpValue(i) = "" Then
                                        arrOpValue(i) = 0
                                    Else
                                        arrOpValue(i) = strTmp & "/" & lng次数
                                    End If
                                Case 2 '显示手术次数
                                    If strTmp = "Ⅰ" Then
                                        arrOpValue(i) = 0
                        ...
                        Case 2
                            Select Case intOpFormat
                                Case 1 '显示0
                                    If Trim(arrOpValue(i)) <> "" Then
                                        arrOpValue(i) = arrOpValue(i) & "/" & 0
                                    Else
                                        arrOpValue(i) = 0
                                    End If
                                Case 2 '显示手术次数
                                    If Trim(arrOpValue(i)) <> "" Then
                                        arrOpValue(i) = arrOpValue(i) & "/" & strTmp
                                    Else
                                        arrOpValue(i) = strTmp
                                    End If
                                Case 3
            ...
            End If
        Next i
    GetOperationTag = Join(arrOpValue, "'")
End Function
...
Public Function DrawCanvasNew(ByVal lngDC As Long, ByVal objDraw As Object, ByVal rsTemp As ADODB.Recordset, rsDrawItems As ADODB.Recordset, _
    Optional ByVal bln不打印心率列 As Boolean = False, Optional ByVal str心率信息 As String = "", Optional sngScale As Single = 1) As Boolean
'------------------------------------------------------------------------------------------------------
...
'返回说明信息(项目的符号)
'-------------------------------------------------------------------------------------------------------
    Dim ArrCode, int心率应用 As Integer
    ...
    Dim int体温 As Integer, intCount As Integer                  '体温排列位置,呼吸为曲线时适用
    ...
    ArrCode = Split(str心率信息, "'")
    If UBound(ArrCode) = -1 Then
        int心率应用 = 0
    Else
        int心率应用 = Val(ArrCode(0))
    End If
    ...
    sngHPrint = T_DrawClient.行单位 * (T_DrawClient.总列数 + T_DrawClient.独立曲线总行数 + 2)
    ...
    With rsTemp
        ...
        Do While Not .EOF
            If Not (bln不打印心率列 = True And !项目序号 = gint心率) Then
                ...
                If !项目序号 <> gint体温 Then
                    intCount = intCount + 1
                    T_DrawClient.行单位 = (sngHPrint - 2 * T_DrawClient.行单位) / ((Val(Nvl(!最大值, 0)) - Val(Nvl(!最小值, 0))) / IIf(!项目序号 = gint脉搏, 2, Val(!单位值)) + T_DrawClient.独立曲线总行数)
                Else
                    int体温 = intCount
                    T_DrawClient.行单位 = (sngHPrint - 2 * T_DrawClient.行单位) / (T_DrawClient.总列数 + T_DrawClient.独立曲线总行数)
                End If
...
'                Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), Trim(zlCommFun.Nvl(!单位, 0)), zlCommFun.Nvl(!记录色, RGB_BLACK))
            ...
            If !项目序号 = gint体温 Then
                ...
                mobjPart.Add Array("" & !项目序号, Nvl(!记录名), "耳温", arrTemp(3), Nvl(!记录色, RGB_BLACK), "D"), "D" & !项目序号
            ElseIf !项目序号 = gint脉搏 Then
                ...
                If int心率应用 = 2 Then
                    mobjPart.Add Array("" & gint心率, CStr(ArrCode(1)), "", CStr(ArrCode(2)), Val(ArrCode(3)), "A"), "A" & gint心率
                End If
            Else
            ...
            Do While True
                ...
                If bln显示刻度 And Not (bln不打印心率列 = True And !项目序号 = gint心率) Then
                    ...
                    ElseIf lngCurY >= T_DrawClient.刻度区域.Bottom Then
                        lngCurY = T_DrawClient.刻度区域.Bottom
...
'                    Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), Format(sin刻度, "#0"), zlCommFun.Nvl(!记录色, RGB_BLACK))
    ...
    With rsTemp
        Do While Not .EOF
            T_DrawClient.行单位 = (sngHPrint) / (T_DrawClient.总列数 + T_DrawClient.独立曲线总行数)
            lngY = lngMaxY - T_DrawClient.行单位 * ((IIf(bln双行, 2, 1)) * (20 - 1))
            lngCurY = lngY
            lngCurX = T_DrawClient.偏移量X
            ...
            If lngCurveRows > 0 Then
                lngMaxY = (lngCurveRows - 2) * sinRowStep + lngCurY
                '完成刻度区域的绘制
                Call DrawLine(lngDC, lngCurX, lngCurY, lngCurX, lngMaxY, PS_SOLID, 1, RGB_BLACK)
                Call DrawLine(lngDC, lngCurX + lng刻度宽度, lngCurY, lngCurX + lng刻度宽度, lngMaxY, PS_SOLID, 1, RGB_BLACK)
                Call DrawLine(lngDC, lngCurX, lngMaxY, lngCurX + lng刻度宽度, lngMaxY, PS_SOLID, 1, RGB_BLACK)
                
                '画体温区域底线
                Call DrawLine(lngDC, T_DrawClient.偏移量X, lngMaxY, lngMaxX, lngMaxY, PS_SOLID, 1, RGB_BLACK)
                '完成项目名称和刻度的输出
                lngCurX = T_DrawClient.偏移量X: lngCurY = lngY
                '输出体温项目的名称
                '创建字体
                Set gstdSet = New stdFont
                gstdSet.Name = "宋体"
                ...
                If T_Size.H * Len(Nvl(!记录名 & " " & !单位)) >= lngCurveRows * T_DrawClient.行单位 Then
                    lngCurY = lngY
                Else
                    lngCurY = lngY + ((lngCurveRows * T_DrawClient.行单位) - (T_Size.H * Len(Nvl(!记录名 & " " & !单位)))) \ 2
                End If
                For lngRow = 1 To Len(Nvl(!记录名 & " " & !单位))
                    Call GetTextRect(objDraw, lngCurX + Fix((int体温) * lng刻度宽度 / (intLables - 1)), lngCurY, Mid(Nvl(!记录名 & " " & !单位), lngRow, 1))
                    Call DrawText(lngDC, Mid(Nvl(!记录名 & " " & !单位), lngRow, 1), -1, T_LableRect, DT_RIGHT)
                ...
                Do While True
                    ...
                    If blnFirst = False Then     '刚进入循环，此时取的最大值
                        sin刻度 = Nvl(!最大值, 0)
                        sinBegin刻度 = sin刻度
                        str最大值坐标 = Fix(T_DrawClient.体温区域.Left / (intLables - 1)) * 2 & "," & lngCurY
                        blnFirst = True
                    Else                    '计算得到每个刻度的值
                        sin刻度 = sin刻度 - dbl单位值     '如果目前显示模式为双倍，则按双倍累计
                    End If
                    ...
                    If bln显示刻度 Then
                        '控制最大值不与曲线单位重复
                        lngCurX = Fix((T_DrawClient.体温区域.Left - T_DrawClient.偏移量X) / (intLables - 1)) * (int体温 + 1) - objDraw.ScaleX(objDraw.TextWidth(Val(Format(sin刻度, "#0.0"))), vbTwips, vbPixels) + T_DrawClient.偏移量X
                        ...
                        End If
                        Call DrawText(lngDC, Val(Format(sin刻度, "#0.0")), -1, T_LableRect, DT_RIGHT)
                    End If
                    If Val(Format(sin刻度, "#0.00")) <= Val(Format(Nvl(!最小值), "#0.00")) Or Format(lngCurY, "#0") > lngMaxY Then
                        ...
                        gstrValues = Nvl(!项目序号) & "|" & Nvl(!最大值) & "|" & Nvl(!最小值) & "|" & dbl单位值 & "|" & _
                            str最大值坐标 & "|" & str最小值坐标 & "|" & T_DrawClient.行单位 & "," & T_DrawClient.列单位 & "|" & sin刻度间隔 & "|" & !记录色
                        Call Record_Add(rsDrawItems, gstrFields, gstrValues)
                        '输出警戒线
                        If blnDoubleRow = False And sinAlertness > Val(Nvl(!最小值)) And sinAlertness < Val(Nvl(!最大值)) Then
                            ...
                            sinCurAlerY = Val(GetYCoordinate(objDraw, rsDrawItems, Val(Nvl(!项目序号)), sinAlertness))
                            Call DrawLine(lngDC, T_DrawClient.体温区域.Left, CLng(sinCurAlerY), lngMaxX, CLng(sinCurAlerY), PS_SOLID, 1, RGB_RED)
                        End If
                        Exit Do
                    End If
                    lngCurY = lngCurY + T_DrawClient.行单位
...
'-------------------------------------------------------------------------------------
    ...
    Dim strWaveReview As String, lngWaveReviewColor As Long '参数:体温复试合格的符号、颜色
    On Error GoTo Errhand
            ...
            If Not ((zlCommFun.Nvl(!项目序号) = gint体温 Or zlCommFun.Nvl(!项目序号) = gint疼痛强度) And Val(zlCommFun.Nvl(!标记)) = 1) Then
                ...
                If !项目序号 = gint体温 Then
                    If zlCommFun.Nvl(!复查) = 1 Then '复试合格
                        Call SetTextColor(lngDC, lngWaveReviewColor)
...
'                        Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), strWaveReview, lngRGB)
                ...
                If Not ((Val(Nvl(!项目序号)) = gint体温 And Trim(Nvl(!数值)) = "不升") Or Val(Nvl(!项目序号)) = gint呼吸 Or Val(Nvl(!项目序号)) = gint脉搏) Then
...
'-----------------------------------------------------------------------------------------------------------------------
    ...
    Dim intBreatheShowType As Integer
    ...
    '2----开始输出呼吸数据 呼吸机为图形输出
    intBreatheShowType = Val(zlDatabase.GetPara("呼吸表格呼吸机输出方式", glngSys, 1255, 0))
                    ...
                    For intCOl = 0 To UBound(arrTmpString1)
                        ...
                        Else
                            ...
                            If IsNumeric(strTmp) Then
                                If strPart <> "自主呼吸" And intBreatheShowType = 2 Then strTmp = "A" & strTmp
                                    
                                If strPart = "自主呼吸" Or intBreatheShowType <> 0 Then
                                    Set stdFont = GetDrawFont(objDraw, strTmp, T_DrawClient.列单位, True, sngScale)
                                    Call SetTextColor(lngDC, lngColor)
                                    ...
                                    If intCOl Mod 2 = intValue Then
                                        If strPic = "" Then
                                            ...
                                            If int呼吸表格输出格式 = 0 Then
                                                Call DrawTabTextNew(lngDC, objDraw, strTmp, -1, DT_CENTER, LPoint, sngScale, 1, 9, stdFont, False)
                                            Else
                                                Call DrawTabTextNew(lngDC, objDraw, strTmp, -1, DT_CENTER, LPoint, sngScale, 3, 9, stdFont, False)
                                            End If
                                        Else
                                            If int呼吸表格输出格式 = 0 Then
...
Private Sub DrawBodyPageFooterNew(ByVal lngDC As Long, objDraw As Object, X As Long, Y As Long, ByVal LeftX As Long, ByVal intPageNo As Integer, _
    ByVal intBeginPage As Integer, ByVal lngButtom As Long, Optional ByVal sngScale As Single = 1)
    '--------------------------------------------------------------------------------------------------------------------------------
    '功能：画出最底部说明
    '参数:intPageNO=页码
    '--------------------------------------------------------------------------------------------------------------------------------
    ...
    If blnPrintCurveInfo = False Then
        ...
        PicRect.Top = Y
        PicRect.Left = X
        PicRect.Right = objDraw.Width \ T_TwipsPerPixel.X
        PicRect.Bottom = Y + T_Size.H + 2 * IIf(blnPrinter = True, msngTwips, 1)
        Call PrintCurveInfo(objDraw, PicRect)
        Y = Y + IIf(blnPrinter = True, msngTwips, 1) * 14
    Else
        Y = Y + IIf(blnPrinter = True, msngTwips, 1) * 6
    End If
    
    '如果将要输出的页码坐标小于页边距，则页码输出位置以页边距为准，否则以实际输出的位置为准
    If Val(objDraw.Height / T_TwipsPerPixel.Y - lngButtom - T_Size.H) > Y Then
        Y = objDraw.Height / T_TwipsPerPixel.X - lngButtom - T_Size.H
    End If
...
Private Sub DrawTabTextNew(ByVal lngDC As Long, ByVal objDraw As Object, ByVal strTmp As String, ByVal nCount As Long, ByVal wFormat As Long, LPoint As T_LPoint, Optional sngScale As Single = 1, Optional ByVal bytCenterType As Byte = 2, Optional sgnFontSize As Single = 9, Optional ByVal stdSet As stdFont = Nothing, Optional ByVal blnCrLf As Boolean = True)
'---------------------------------------------------
'功能 处理表下表格字体输出
'---------------------------------------------------
    Dim lngFont As Long, lngOldFont As Long, sgnSize As Single
    ...
    On Error GoTo Errhand
    If stdSet Is Nothing Then Set stdSet = GetDrawFont(objDraw, strTmp, LPoint.W, False, sngScale)
    blnChage = stdSet.Size <> 9
    
    arrData = Array()
    If blnChage = True And blnCrLf = True Then
        With frmTendFileRead.txtLength
            .Width = Val(Format(LPoint.W * T_TwipsPerPixel.X, "#0")) + IIf(TypeName(objDraw) = "Printer", 12, 0)
            .Text = Replace(Replace(Replace(strTmp, Chr(10), ""), Chr(13), ""), Chr(1), "")
            .FontName = "宋体"
            .FontSize = stdSet.Size
            .FontBold = False
            .FontItalic = False
        End With
        arrData = GetData(frmTendFileRead.txtLength.Text, frmTendFileRead.txtLength)
...
Private Function GetDrawFont(ByVal objDraw As Object, ByVal strTmp As String, ByVal lngWidth As Long, Optional ByVal blnSetFont As Boolean = True, Optional sngScale As Single = 1, Optional sgnFontSize As Single = 9) As stdFont
    '通过表格来计算需缩放字体大小
    ...
    objDraw.Font.Size = sgnFontSize
    objDraw.Font.Name = "宋体"
    objDraw.Font.Bold = False
    objDraw.Font.Italic = False
    
    If objDraw.TextWidth(strTmp) / T_TwipsPerPixel.X > lngWidth Then
ErrGoTo:
        sngD = Round((objDraw.TextWidth(strTmp) / T_TwipsPerPixel.X - lngWidth) / lngWidth, 4)
        If sngD > 0 Then
            sgnSize = Int(Round((1 - sngD), 2) * sgnSize - 0.5)
            If sgnSize < 6 Then sgnSize = 6
            objDraw.Font.Size = sgnSize * sngScale
            If (objDraw.TextWidth(strTmp) / T_TwipsPerPixel.X) > lngWidth And sgnSize > 6 Then GoTo ErrGoTo
            objDraw.Font.Size = 9 * sngScale
        End If
    Else
        sgnSize = sgnFontSize
    ...
    stdSet.Bold = False
    stdSet.Italic = False
    If blnSetFont = True Then Call ReleaseFontIndirect(objDraw, stdSet)
    Set GetDrawFont = stdSet
End Function
    ...
    With rsNote
        ...
        Do While Not .EOF
            ...
            If strComment <> "" Then
                ...
                gstdSet.Name = "宋体"
                gstdSet.Size = IIf(blnBigSize = True, lngFontSize, 7.5)
                Call SetFontIndirect(gstdSet, lngDC, objDraw)
                lngFont = CreateFontIndirect(T_Font)
                lngOldFont = SelectObject(lngDC, lngFont)
...
Public Function GetAppendGridItemNew(ByVal lng文件ID As Long, ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal int护理等级 As Integer, ByVal int婴儿 As Long, dt开始时间 As Date, dt结束时间 As Date, ByVal byt适用病人 As Byte, ByVal lng科室ID As Long, ByVal str表格项目 As String, Optional blnMove As Boolean = False) As ADODB.Recordset
    ...
    For i = 0 To UBound(ArrCode)
     ...
     strItemInfo = Mid(strItemInfo, 2)
    '提取固定表格项目
    strSQL = strSQL & "Select A.排列序号,A.项目序号,'' 体温部位,A.记录名,A.记录法,A.记录符,A.记录色,A.最大值,A.最小值,A.单位值,nvl(D.C2,2) 记录频次,A.入院首测,B.项目性质," & _
    ...
    If strItemInfo <> "" Then
        strSQL = strSQL & vbCrLf & " UNION ALL " & vbCrLf & _
            ...
            "   B.分组名,B.项目值域,B.项目表示,B.项目类型,B.项目长度,B.项目小数,B.项目单位 单位" & _
            "   From 体温记录项目 A,护理记录项目 B,诊治所见项目 C,TABLE(CAST(F_str2LIST2([2]) AS ZLTOOLS.T_strLIST2)) D" & _
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "提取表格项目", str表格项目, strItemInfo)
...
Private Function getSQLString(ByVal strText As String, ByVal blnMoved As Boolean, Optional ByVal strItems As String) As String
    ...
    Select Case strText
        ...
        Case "提取所有表格项目数据信息"
            ...
            strSQL = strSQL & vbNewLine & _
                ...
                "       WHERE A.项目序号=B.序号 And 父序号 Is Not Null  " & vbNewLine & _
    ...
    If blnMoved Then
        ...
        strSQLText = Replace(strSQLText, "病人医嘱记录", "H病人医嘱记录")
    End If
...
Private Sub ClearData(M_DrawClient As DrawClient, ByVal lng刻度宽度 As Long)
...
Private Function Get总行数(ByVal dbl数值 As Double, ByVal lngCurveRow As Long) As Integer
    ...
    If mbln呼吸曲线 = True Then
        rsTmp.Filter = "项目序号=" & gint呼吸
        '计算打印输出的行数
        With rsTmp
            Do While Not .EOF
                intRows = IIf(Val(rsTmp!最高行) < 0, 0, Val(rsTmp!最高行)) + ((Val(Nvl(!最大值, 0)) - Val(Nvl(!最小值, 0))) / Val(Nvl(!单位值)))
                intDrawLineRows = intDrawLineRows + intRows
                .MoveNext
            Loop
        End With
...
'在下表格左侧区域输入曲线部位符号信息
    Dim lngDC As Long
    Dim i As Integer, j As Integer
    Dim arrPar, strName As String, strPart As String, strSymbol As String, lngColor As Long, lngItemNO As Long, lngPreItemNo As Long
    Dim LPoint As T_LPoint
    ...
    If mobjPart Is Nothing Then Exit Sub
    '将体温移动到前面
    For j = 0 To 1
        For i = 1 To mobjPart.Count
            arrPar = mobjPart.Item(i)
            If (Val(arrPar(0)) = gint体温 And j = 0) Or (Val(arrPar(0)) <> gint体温 And j = 1) Then
                objHeat.Add arrPar, CStr(arrPar(5)) & Val(arrPar(0))
            End If
        Next i
    Next j
    ...
    LPoint.Y = T_RECT.Top + (lngHeight - objDraw.TextHeight("刘") / T_TwipsPerPixel.Y) \ 2
    LPoint.X = T_RECT.Left
    
    lngDC = objDraw.hDC
    For i = 1 To objHeat.Count
        arrPar = objHeat.Item(i)
        lngItemNO = Val(arrPar(0))
        strName = CStr(arrPar(1))
        strPart = CStr(arrPar(2))
        strSymbol = CStr(arrPar(3))
        lngColor = Val(arrPar(4))
        
        If i = 1 Then
            strTmp = "说明：" & strName & "(" & strPart
        ElseIf lngPreItemNo <> lngItemNO Then
            strTmp = ")、" & strName & "(" & strPart
        Else
            strTmp = "," & strPart
        End If
        Call SetTextColor(lngDC, RGB_BLACK)
        Call GetTextRect(objDraw, LPoint.X, LPoint.Y, strTmp, , False)
        Call DrawText(lngDC, strTmp, -1, T_LableRect, DT_CENTER)
        LPoint.X = LPoint.X + Format(objDraw.TextWidth(strTmp) / T_TwipsPerPixel.X, "#0")
        
        strTmp = ""
        If lngItemNO = gint脉搏 And strPart = "起搏器" Then
            strTmp = "PACEMAKER"
        ElseIf lngItemNO = gint呼吸 And strPart = "呼吸机" Then
            strTmp = "BREATH"
        End If
        If strTmp = "" Then
            Call SetTextColor(lngDC, lngColor)
            ...
            LPoint.X = LPoint.X + Format(objDraw.TextWidth(strSymbol) / T_TwipsPerPixel.X, "#0")
        Else
            Call DrawPicture(objDraw, strTmp, objDraw.ScaleX(LPoint.X, vbPixels, vbTwips), objDraw.ScaleY(LPoint.Y, vbPixels, vbTwips), _
                        objDraw.ScaleX(LPoint.X + Format(objDraw.TextHeight("@") / T_TwipsPerPixel.X, "#0"), vbPixels, vbTwips), objDraw.ScaleY(LPoint.Y + Format(objDraw.TextHeight("@") / T_TwipsPerPixel.X, "#0"), vbPixels, vbTwips), True)
             LPoint.X = LPoint.X + Format(objDraw.TextHeight("@") / T_TwipsPerPixel.X, "#0")
        End If
        lngPreItemNo = lngItemNO
        
        '最后一个项目则首尾
        If i = objHeat.Count Then
            Call SetTextColor(lngDC, RGB_BLACK)
            Call GetTextRect(objDraw, LPoint.X, LPoint.Y, ")", , False)
