...
Public Sub Paint_Canvas(Optional ByVal blnAdjust As Boolean = False)
    ...
    lngMaxX = lng刻度宽度 + (7 * 6 * lngColStep) + T_DrawClient.偏移量X   '刻度+总列数*宽度 +T_DrawClient.偏移量X
    ...
    If Val(vsf.ColData(Col)) <> 0 Then
        ...
        If Val(vsf.ColData(Col)) = 0 Then
            ...
            If vsf.TextMatrix(1, Col) <> "" Then
                ...
                T_Size.H = objDraw.TextHeight("刘") / T_TwipsPerPixel.Y
                T_Size.W = objDraw.TextWidth("刘") / T_TwipsPerPixel.X
