...
'------------------------------------------------------------------------
    ...
    ElseIf intType = 2 Then '手术上下标处理
        ...
        Else
            ...
            If strValue <> "" Then
                gstrFields = "序号|项目序号|时间|记录类型|内容|项目名称|未记说明|记录组号|数据来源|显示|来源ID|共用|状态"
                gstrValues = GetMaxID(mrsNote) & "|" & 0 & "|" & strTime & "|" & lngNO & "|" & strValue & "|" & strName & "|" & IIf(lngNO = 4, "", vsfCurve.Cell(flexcpBackColor, intRow, col_颜色, intRow, col_颜色)) & "|0|0|0|0|0|1"
                Call Record_Add(mrsNote, gstrFields, gstrValues)
            End If
        End If
    End If
...
'--------------------------------------------------------
    ...
    Dim i As Integer, int项目首次 As Integer, int检查科室 As Integer
...
'-------------------------------------------------------------------------
    ...
    Dim blnAllow As Boolean
    Dim strValue As String, strErrMsg As String
    Dim intIndex As Integer
    Dim strTime As String
        ...
        If strValue <> vsfTab.TextMatrix(intRow, intCOl) Then
            ...
            Else '不存在记录就新增数据
                If Trim(vsfTab.TextMatrix(intRow, intCOl)) <> "" Then
                    ...
                    gstrFields = "序号|分组名|数值|部位|标记|时间|项目序号|项目名称|复查|未记说明|数据来源|修改|显示|来源ID|共用|状态|列号"
                    gstrValues = GetMaxID(mrsCurve) & "|2)体温表格项目|" & vsfTab.TextMatrix(intRow, intCOl) & "|" & strPart & "|" & _
