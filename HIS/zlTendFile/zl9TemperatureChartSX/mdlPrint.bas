...
Public Function PrintOrPreviewBodyState(objOut As Object, _
    ...
    Dim arrUnitName() As Variant '体温单标题信息
    ...
    strTmp = zlDatabase.GetPara("打印医院名称", glngSys, 1255, "1")
    bln打印医院名称 = (Val(strTmp) = 1): bln标题分开显示 = False
    If InStr(1, strTmp, ",") > 0 Then bln标题分开显示 = (Val(Split(strTmp, ",")(1)) = 1)
    ...
    bln出院 = False
    strSQL = GetDataSQL("提取文件时间范围", blnMoved)
    ...
    strSQL = GetDataSQL("提取所有曲线项目", blnMoved)
    ...
    End If
    '求曲线总行数
    rsTemp.Filter = "记录法=1 And 项目序号=" & gint体温
    If rsTemp.RecordCount > 0 Then
        '修改问题：51442
        dbl数值 = Val(zlCommFun.Nvl(rsTemp!最小值, 0))
        intDrawLineRows = (Val(rsTemp!最大值) - IIf(dbl数值 > 34, 35, dbl数值)) / 0.1 + IIf(Val(rsTemp!最高行) < 0, 0, Val(rsTemp!最高行)) + IIf(dbl数值 > 34, 10, 0)
    Else
        intDrawLineRows = glngMaxRows
    End If
    ...
    If rsTemp.RecordCount > 0 Then
        Do While Not rsTemp.EOF
            intDrawLineRows = (Val(rsTemp!最大值) - Val(zlCommFun.Nvl(rsTemp!最小值, 0))) / Val(zlCommFun.Nvl(rsTemp!单位值, 0)) + IIf(Val(rsTemp!最高行) < 0, 0, Val(rsTemp!最高行))
            If intDrawLineRows Mod 2 = 1 Then intDrawLineRows = intDrawLineRows + 1
            T_DrawClient.独立曲线总行数 = T_DrawClient.独立曲线总行数 + intDrawLineRows
            rsTemp.MoveNext
        Loop
    End If
    ...
    strSQL = GetDataSQL("提取所有特殊项目", blnMoved)
    ...
    If mlngBreatheHeight < T_DrawClient.时间列单位 Then mlngBreatheHeight = T_DrawClient.时间列单位
    '------------------------------------------------------------------------------------------------------------------
    '5、循环：按总页数循环
    intCurOpt = 0
    intAllOpt = 100
    ...
    For lngPage = 1 To lngCountPage
        ...
        If bln打印医院名称 = True Then
            '获取体温单标题信息，此处至获取内容不输出(山西要求标题输出在体温单中间，由于受编剧和体温单实际大小的影响，放在最后输出页码函数中进行输出)
            stdset.Name = "宋体"
            stdset.Size = 18 * sngScale
            stdset.Bold = True
            ReDim Preserve arrUnitName(UBound(arrUnitName) + 1)
            arrUnitName(UBound(arrUnitName)) = stdset.Name & "'" & stdset.Size & "'" & IIf(stdset.Bold = True, 1, 0)
            Call SetFontIndirect(stdset, lngDC, objDraw)
            lngFont = CreateFontIndirect(T_Font)
            lngOldFont = SelectObject(lngDC, lngFont)
            strTmp = IIf(GetUnitName = "-", "", GetUnitName) & IIf(intBaby <> 0, "婴儿", "") & IIf(bln标题分开显示 = True, "", "体温单")
            Call GetTextExtentPoint32(lngDC, strTmp, Len(strTmp), T_Size)
            lngCurY = T_Size.H + lngCurY
            ReDim Preserve arrUnitName(UBound(arrUnitName) + 1)
            arrUnitName(UBound(arrUnitName)) = strTmp & "'" & lngCurY
            If bln标题分开显示 = True Then '是否另起一行输出体温单
                strTmp = "体温单"
                Call GetTextExtentPoint32(lngDC, strTmp, Len(strTmp), T_Size)
                lngCurY = T_Size.H + lngCurY + 6 * msngTwips
                ReDim Preserve arrUnitName(UBound(arrUnitName) + 1)
                arrUnitName(UBound(arrUnitName)) = strTmp & "'" & lngCurY
            End If
            Call SelectObject(lngDC, lngOldFont)
        ...
        If blnMoved Then strSQL = ReplaceSQL(strSQL)
        ...
        '----------------------------------------------------------------------------------------------
         ...
         j = 0
         If (Val(zlDatabase.GetPara("体温单不打印曲线说明", glngSys, 1255, "0")) = 1) = False Then j = j + 1
         If (Val(zlDatabase.GetPara("打印页号", glngSys, 1255, "1")) = 1) Or (Val(zlDatabase.GetPara("打印周数", glngSys, 1255, "0")) = 1) Then j = j + 1
        ...
        sngHTab = sngHTab + msngTwips * (14 * j)
        sngHPrint = Format(objDraw.Height / T_TwipsPerPixel.Y - lngCurY - lngButtom - sngHTab, "#0.00;-#0.00;0.00")
        T_DrawClient.行单位 = (sngHPrint - 4 * T_DrawClient.行单位) / (T_DrawClient.总列数 + T_DrawClient.独立曲线总行数)
        T_DrawClient.行单位 = Format(Round(T_DrawClient.行单位 - 0.05, 1), "#0") * sngScale
        ...
        '计算行高后在计算体温单可打印的表格行数
        If intRepairRows > 0 Then
            sngHPrint = (T_DrawClient.总列数 + T_DrawClient.独立曲线总行数) * T_DrawClient.行单位 + 4 * T_DrawClient.行单位
            sngHTab = objDraw.Height / T_TwipsPerPixel.Y - lngCurY - lngButtom - sngHPrint - (msngTwips * (14 * j))
        ...
        strSQL = "SELECT C.ID 序号, a.发生时间 As 时间,C.显示,C.记录内容 As 数值,C.体温部位,c.复试合格,D.记录名,E.保留项目,D.项目序号,DECODE(D.项目序号,-1,1,C.记录标记) 记录标记,C.未记说明 " & _
                    ...
                    "Where B.ID=A.文件ID  " & _
                        ...
                        "AND Instr(',1,3,', ',' || D.记录法 || ',')>0 AND (nvl(E.应用方式,0)=1 OR ( -1=[10] and nvl(E.应用方式,0)=2)) " & _
...
'        '处理体温不升 体温为不升需要在35度下纵向输出体温不升二字
        ...
        End With
        Call GetInOutInfo(rsNotes, blnMoved)
                        ...
                        If byt未记显示位置 = 1 Then '显示在体温的下面
                            ...
                            For i = 1 To Len(strTime)
                                If sinY < T_DrawClient.刻度区域.Bottom Then
                                    ...
                                    If T_DrawClient.刻度区域.Bottom - sinY >= T_Size.H - (1 * msngTwips) Then
                                        Call DrawRotateText(objDraw, lngDC, SinX, sinY, strText, Val(!颜色))
                                    End If
                                    If Asc(strText) < 0 Then
                                        sinY = sinY + IIf(T_DrawClient.行单位 * 2 > T_Size.H, T_DrawClient.行单位 * 2, T_Size.H)
                                    Else
                                        sinY = sinY + IIf(T_DrawClient.行单位 * 2 > T_Size.H, T_DrawClient.行单位 * 2, T_Size.H) / 2
                                    End If
                                End If
                            Next i
                            rsNotes!禁用 = 1
                    ...
                    ElseIf zlCommFun.Nvl(!类型) = 6 Then
                        ...
                        For i = 1 To Len(strTime)
                            ...
                            If sinY < T_DrawClient.刻度区域.Bottom Then
                                ...
                                If Asc(strText) < 0 Then
                                    If intAscCharNum Mod 2 = 1 Then sinY = sinY + IIf(T_DrawClient.行单位 * 2 > T_Size.H, T_DrawClient.行单位 * 2, T_Size.H) / 2
                                End If
                                '输出字体信息
                                If T_DrawClient.刻度区域.Bottom - sinY >= T_Size.H - (1 * msngTwips) Then
...
ErrNext:
        ...
        If bln显示皮试 = True Then
            strSQL = _
               "SELECT 时间,F_LIST2STR(CAST(COLLECT(药物名) AS T_STRLIST)) 药物名 FROM (" & vbNewLine & _
                "   SELECT TO_CHAR(开始执行时间,'YYYY-MM-DD') 时间,DECODE(皮试结果,'(+)',255,'(阳性)',255,0) || '-#' || REPLACE(REPLACE(医嘱内容,',',''),'-#','') || 皮试结果  药物名" & vbNewLine & _
        ...
        lngCurX = X
        '开始绘画表格项目并展示数据
        Call DrawBodyRecordItem(lngDC, objDraw, ArrNewString, rsItems, lngCurX, T_DrawClient.曲线总区域.Bottom, T_DrawClient.体温区域.Right, intRepairRows, lngCurY, sngScale)
        lngCurX = X
        lngCurY = lngCurY
        ...
        Call DrawBodyPageFooter(lngDC, objDraw, lngCurX, lngCurY, T_DrawClient.体温区域.Right, intPageNo, intEndPage, lngButtom, arrUnitName, str体温说明, sngScale)
        Call SelectObject(lngDC, lngOldFont)
        Call DeleteObject(lngFont)
        Call ReleaseFontIndirect(objDraw)
        '页脚图形输出
                ...
                If arrTmpString0(0) = "3" Then '呼吸项目
                    ...
                    For intCOl = 0 To UBound(arrTmpString1)
                        If intCOl = 0 Then '表头
                            ...
                            Call GetTextRect(objDraw, lngX, lngY + mlngBreatheHeight / 2, arrTmpString0(intCOl + 1), _
                                T_DrawClient.刻度区域.Right - lngX, True, , sngScale)
                            'Call DrawText(lngDC, arrTmpString0(intCOl + 1), -1, T_LableRect, DT_CENTER)
                            LPoint.X = lngX
                            LPoint.Y = lngY + mlngBreatheHeight / 2
                            ...
                            If IsNumeric(strTmp) Then
                                ...
                                If int呼吸表格输出格式 = 0 Then '顺序上下显示
                                    ...
                                    Else
                                        If strPic = "" Then
                                            LPoint.X = lngX1
                                            LPoint.Y = lngY + (mlngBreatheHeight - T_Size.H)
                                            Call GetTextRect(objDraw, lngX1, lngY + (mlngBreatheHeight - T_Size.H), _
                    ...
                    For intCOl = 1 To 42
                        If intCOl Mod 6 = 0 Then
                            Call DrawLine(lngDC, lngX1, lngY, lngX1, lngY1, PS_SOLID, intBold, IIf(intCOl = 42, RGB_BLACK, RGB_RED))
                        Else
                    ...
                    If blnDataTrue = True Then
                        ...
                        If InStr(1, ",2,3,4,", "," & int频次 & ",") = 0 Then
                            ...
                            For intCOl = 1 To int频次 * 7
                                Call DrawLine(lngDC, lngX1, lngCurY, lngX1, lngY1, PS_SOLID, IIf(intCOl Mod int频次 = 0, intBold, intFine), IIf(intCOl Mod int频次 = 0, IIf(intCOl = int频次 * 7, RGB_BLACK, RGB_RED), RGB_BLACK))
                                lngX1 = lngX1 + T_DrawClient.列单位 * (6 / int频次)
                            Next intCOl
                            Call DrawLine(lngDC, T_DrawClient.刻度区域.Right, lngY1, T_DrawClient.体温区域.Right, lngY1, PS_SOLID, IIf(intRowCount = intRepairRows, intBold, intFine), RGB_BLACK)
                        ElseIf int频次 = 3 Then
                            ...
                            For i = 1 To 3 - intNum
                                ...
                                lngY1 = lngCurY + T_DrawClient.时间列单位
                                For intCOl = 1 To 7
                                    Call DrawLine(lngDC, lngX1, lngCurY, lngX1, lngY1, PS_SOLID, intBold, IIf(intCOl = 7, RGB_BLACK, RGB_RED))
                                    lngX1 = lngX1 + T_DrawClient.列单位 * 6
                                Next intCOl
                        ...
                        ElseIf InStr(1, ",2,4,", "," & int频次 & ",") <> 0 Then
                            ...
                            For i = 1 To (int频次 / 2 - intNum)
                                ...
                                For intCOl = 1 To 14
                                    Call DrawLine(lngDC, lngX1, lngCurY, lngX1, lngY1, PS_SOLID, IIf(intCOl Mod 2 = 0, intBold, intFine), IIf(intCOl Mod 2 = 0, IIf(intCOl = 14, RGB_BLACK, RGB_RED), RGB_BLACK))
                                    lngX1 = lngX1 + T_DrawClient.列单位 * 3
                                Next intCOl
                                intRowCount = intValue + i
                                Call DrawLine(lngDC, T_DrawClient.刻度区域.Right, lngY1, T_DrawClient.体温区域.Right, lngY1, PS_SOLID, IIf(intRowCount = intRepairRows, intBold, intFine), RGB_BLACK)
...
'                Next intCOl
                ...
                For intCOl = 0 To 7
                    ...
                    Else
                        ...
                        Call DrawLine(lngDC, lngX1, lngCurY, lngX1, lngY1, PS_SOLID, intBold, IIf(intCOl = 7, RGB_BLACK, RGB_RED))
                        If intCOl = 7 Then
                            Call DrawLine(lngDC, T_DrawClient.刻度区域.Right, lngY1, T_DrawClient.体温区域.Right, lngY1, PS_SOLID, IIf(intRow = intRepairRows, intBold, intFine), IIf(intCOl = intRepairRows, IIf(intCOl = 7, RGB_BLACK, RGB_RED), RGB_BLACK))
                        End If
                    End If
                Next intCOl
                lngCurY = lngY1
...
Private Sub DrawBodyPageFooter(ByVal lngDC As Long, objDraw As Object, X As Long, Y As Long, ByVal LeftX As Long, ByVal intPageNo As Integer, _
    ByVal intBeginPage As Integer, ByVal lngButtom As Long, arrUnitName() As Variant, Optional ByVal strInfo As String, Optional ByVal sngScale As Single = 1)
    '--------------------------------------------------------------------------------------------------------------------------------
    '功能：画出最底部说明,以及医院名称
    '参数:intPageNO=页码
    '--------------------------------------------------------------------------------------------------------------------------------
    ...
    Dim blnPrinter As Boolean
    Dim i As Integer, lngFont As Long, lngOldFont As Long
    Dim lngCurY As Long, strTmp As String
    ...
    '打印页码
    If Val(objDraw.Height / T_TwipsPerPixel.Y - lngButtom - T_Size.H \ 2) < Y Then
        Y = Y + T_Size.H / 2
    Else
        Y = objDraw.Height / T_TwipsPerPixel.X - lngButtom - T_Size.H \ 2
    End If
    '------------------------------------------------------------------------------------------------------------------
    ...
    '山西要求居中体温单
    Call GetTextRect(objDraw, T_DrawClient.刻度区域.Left, Y, strNOPage, T_DrawClient.体温区域.Right - T_DrawClient.刻度区域.Left, True, , sngScale)
...
'    Call DrawText(lngDc, strNOPage, -1, T_LableRect, DT_CENTER)
    ...
    '--------------------------------------------------------------------------------------------------------------------------------
    '输出体温单标题
    If UBound(arrUnitName) >= 0 Then
            stdset.Name = Split(arrUnitName(0), "'")(0)
            stdset.Size = Val(Split(arrUnitName(0), "'")(1))
            stdset.Bold = IIf(Val(Split(arrUnitName(0), "'")(2)) = 0, False, True)
            Call SetFontIndirect(stdset, lngDC, objDraw)
            lngFont = CreateFontIndirect(T_Font)
            lngOldFont = SelectObject(lngDC, lngFont)
            For i = 1 To UBound(arrUnitName)
                strTmp = Split(arrUnitName(i), "'")(0)
                Call GetTextExtentPoint32(lngDC, strTmp, Len(strTmp), T_Size)
                lngCurY = Val(Split(arrUnitName(i), "'")(1))
                Call GetTextRect(objDraw, T_DrawClient.刻度区域.Left, lngCurY, strTmp, T_DrawClient.体温区域.Right - T_DrawClient.刻度区域.Left, True, T_Size.H, sngScale)
                Call DrawText(lngDC, strTmp, -1, T_LableRect, DT_CENTER)
            Next i
            Call SelectObject(lngDC, lngOldFont)
            Call DeleteObject(lngFont)
    ...
    With rsPoint
        Do While Not .EOF
            For i = 0 To UBound(strEditors)
                ...
                End If
            Next i
            If Not ((Nvl(!项目序号) = gint体温 Or Nvl(!项目序号) = gint疼痛强度) And Val(zlCommFun.Nvl(!标记)) = 1) Then
...
'-------------------------------------------------------------------------------------------------------
    ...
    lngInitRowStep = glngInitRowStep * IIf(blnPrinter = True, msngTwips, 1) '主要是顶行2行空行的单位
    sinRowStep = T_DrawClient.行单位
    lngLableStep = T_DrawClient.刻度单位
    lng刻度宽度 = T_DrawClient.刻度单位 * intLables
    
    lngCurX = T_DrawClient.偏移量X
    lngCurY = T_DrawClient.偏移量Y
    lngMaxX = lng刻度宽度 + (7 * 6 * lngColStep) + T_DrawClient.偏移量X  '刻度+7*宽度+偏移量X
    ...
    For lngRow = 1 To 6 * 7
        ...
        Call DrawLine(lngDC, lngCurX, lngCurY, lngCurX, lngMaxY, PS_SOLID, IIf(lngRow Mod 6 = 0, intBold, intFine), IIf(lngRow Mod 6 = 0, IIf(lngRow = 42, RGB_BLACK, RGB_RED), RGB_BLACK))
    Next
            ...
            Do While True
                ...
                If bln显示刻度 And Not (bln不打印心率列 = True And !项目序号 = gint心率) Then
                    ...
                    If sin刻度 = Val(Nvl(!最大值, 0)) And lngCurY < sinY单位 Then
                        Call GetTextRect(objDraw, lngCurX, sinY单位 + 1 * msngTwips, Format(sin刻度, "#0"), lngLableStep, , , sngScale)
...
'-----------------------------------------------------------------------------------------------------------------------
    ...
    strDay = IIf(mintBaby = 0, "住院日数", "出生日数")
    ...
    For i = 0 To 6
        ...
        Call DrawLine(lngDC, lngCurX, lngStartY, lngCurX, lngCurY, PS_SOLID, intBold, IIf(i = 6, RGB_BLACK, RGB_RED))
    Next i
...
Public Function GetAppendGridItem(ByVal lng文件ID As Long, ByVal lng病人id As Long, ByVal lng主页id As Long, ByVal int护理等级 As Integer, ByVal int婴儿 As Long, dt开始时间 As Date, dt结束时间 As Date, ByVal byt适用病人 As Byte, ByVal lng科室ID As Long, Optional blnMove As Boolean = False) As ADODB.Recordset
    ...
    If blnMove Then gstrSQL = ReplaceSQL(gstrSQL)
                ...
                For i = 0 To UBound(ArrString)
                    If Y < T_DrawClient.刻度区域.Bottom Then
                        ...
                        If Asc(ArrString(i)) < 0 Or IsNumeric(ArrString(i)) Then
                            If intAscCharNum Mod 2 = 1 Then Y = Y + IIf(T_DrawClient.行单位 * 2 > T_Size.H, T_DrawClient.行单位 * 2, T_Size.H) / 2
                            '根据坐标得到数值
                        ...
                        If Asc(ArrString(i)) < 0 Then
                            Y = Y + IIf(T_DrawClient.行单位 * 2 > T_Size.H, T_DrawClient.行单位 * 2, T_Size.H)
                            intAscCharNum = 0
                        ElseIf Asc(ArrString(i)) > 0 Then
                            Y = Y + IIf(T_DrawClient.行单位 * 2 > T_Size.H, T_DrawClient.行单位 * 2, T_Size.H)
                            intAscCharNum = 0
                        Else
                            Y = Y + IIf(T_DrawClient.行单位 * 2 > T_Size.H, T_DrawClient.行单位 * 2, T_Size.H) / 2
                            intAscCharNum = intAscCharNum + 1
                        End If
                    End If
                Next i
...
Public Function GetDataFromHis(ByVal lng病人id As Long, ByVal lng主页id As Long, ByVal lng婴儿 As Long, ByVal dtFrom As Date, ByVal dtTo As Date, ByVal blnMoved As Boolean, Optional ByVal bytMode As Byte = 1) As ADODB.Recordset
    ...
    Select Case bytMode
        ...
        Case 1              '从医嘱记录提取手术、分娩数据
            ...
            If blnMoved = True Then strSQL = ReplaceSQL(strSQL)
        ...
        Do While Not rsConVerPoint.EOF
                ...
                Else
                    gstrSQL = " select  C.序号,C.上级序号,C.项目序号,C.体温部位 from 体温重叠标记 C," & _
                        ...
                        "   where C.上级序号=D.序号 and C.项目序号 is not null order by C.序号"
                End If
                Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "提取重叠项目", Val(rsConVerPoint!重叠数目), Nvl(rsConVerPoint!体温部位))
...
Public Function GetInOutInfo(ByRef rsNotes As ADODB.Recordset, ByVal blnMoved As Boolean)
        Dim bytShow As Byte
        Dim str内容 As String
        ...
        If mIn.blnMoved Then strSQL = ReplaceSQL(strSQL)
        ...
        '1-入院；2-入科；3-转科；4-换床
        gstrFields = "时间|项目序号|类型|内容|颜色|X坐标|Y坐标|高度|打印X坐标|禁用"  '入出转缺省是红色,上下标及未记说明缺省是蓝色
        Set rsTmp = GetDataFromHis(mIn.lng病人id, mIn.lng主页id, mIn.intBaby, CDate(mIn.strTmpDay), CDate(mIn.strEndDay), blnMoved, 2)
        With rsTmp
            Do While Not .EOF
                If Trim(zlCommFun.Nvl(!内容)) <> "" Then
                    bytShow = 0
        ...
        If mIn.intBaby > 0 Then
            gstrFields = "时间|项目序号|类型|内容|颜色|X坐标|Y坐标|高度|打印X坐标|禁用"  '入出转缺省是红色,上下标及未记说明缺省是蓝色
            Set rsTmp = GetDataFromHis(mIn.lng病人id, mIn.lng主页id, mIn.intBaby, CDate(mIn.strTmpDay), CDate(mIn.strEndDay), blnMoved, 3)
            With rsTmp
                Do While Not .EOF
                    bytShow = 0
                    If Trim(zlCommFun.Nvl(!内容)) <> "" Then
...
Private Function GetDataSQL(ByVal strName As String, ByVal blnMoved As Boolean) As String
    Dim strSQL As String, strNewSql As String
    ...
    Select Case strName
        ...
        Case "提取所有曲线项目"
             strSQL = " Select A.项目序号,A.排列序号,A.记录名,C.项目值域,A.记录法,A.记录符,A.记录色,nvl(A.最大值,0) 最大值 ,nvl(A.最小值,0) 最小值,A.临界值," & _
                    ...
                    " And  Instr(',1,3,', ',' || A.记录法 || ',')>0  And A.项目序号=C.项目序号 and nvl(C.应用方式,0)=1 and C.护理等级>=[1]" & _
        ...
        Case "提取所有特殊项目"
            ...
            strSQLText = strSQL
    End Select
    If blnMoved Then strSQLText = ReplaceSQL(strSQLText)
    GetDataSQL = strSQLText
End Function
...
Public Function GetAdviceOutTime(ByVal lng病人id As Long, ByVal lng主页id As Long, ByVal lng婴儿 As Long, ByVal blnMoved As Boolean) As String
'功能:获取病人或婴儿的医嘱出院时间
    Dim rsTemp As New ADODB.Recordset
    ...
    If blnMoved Then gstrSQL = ReplaceSQL(gstrSQL)
...
Public Function ReplaceSQL(ByVal strSQL As String) As String
    strSQL = Replace(strSQL, "病人护理文件", "H病人护理文件")
    strSQL = Replace(strSQL, "病人护理数据", "H病人护理数据")
    strSQL = Replace(strSQL, "病人护理明细", "H病人护理明细")
