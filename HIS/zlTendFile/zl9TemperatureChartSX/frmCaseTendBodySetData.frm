...
'-----------------------------------------------------------------------------------------------
    ...
    If mblnMove = True Then strSQL = ReplaceSQL(strSQL)
...
'------------------------------------------------------------------------------
    ...
    strCurrDate = Format(strCurDate, "yyyy-MM-dd HH:mm")
...
Private Function IsAllowInput(ByVal lng病人id As Long, ByVal lng主页id As Long, lng婴儿 As Long, ByVal strTime As String, ByVal strCurTime As String) As Boolean
    ...
    If lng婴儿 <> 0 And mbln出院 = True Then
        strBabyOutTime = GetAdviceOutTime(lng病人id, lng主页id, lng婴儿, mblnMove)
                ...
                If !分组名 = vsfCurve.TextMatrix(intRow, COL_分组名) And !项目序号 = lngNO Then
                    ...
                    If Val(zlCommFun.Nvl(!数据来源)) <> 0 Then
                        If zlCommFun.Nvl(!数值) = "不升" And lngNO = gint体温 Then
                            lngColor = 255
                        ElseIf lngNO = gint体温 Or lngNO = gint疼痛强度 Or (lngNO = gint脉搏 And mint心率应用 = 2 And mblnEdit心率 = True) Then
...
Private Function GetTableRowName() As String
    ...
    mstrSQL = " Select A.排列序号,A.记录名 项目名,A.项目序号 as 项目号,A.记录法,A.记录色,A.入院首测," & _
            ...
            " Where c.项目ID=B.ID(+) And A.项目序号=C.项目序号 And (Instr(',1,3,', ',' || A.记录法 || ',')>0 OR (A.记录法=2 And A.项目序号=3)) And Nvl(C.应用方式,0)=1 AND C.护理等级>=[1] And Nvl(C.适用病人,0) In (0,[3]) " & _
...
'-----------------------------------------------------------------------------------------------------------------
    ...
    If blnCurve = True Then
        ...
        "                        AND (Instr(',1,3,', ',' || NVL(D.记录法,1) || ',') > 0 or (NVL(D.记录法,1)=2 And D.项目序号=3))" & _
        ...
        If mblnMove Then mstrSQL = ReplaceSQL(mstrSQL)
                        ...
                        If !分组名 = vsfCurve.TextMatrix(intRow, COL_分组名) And !项目序号 = lng项目序号 Then
                            ...
                            If InStr(1, ",0,9,", Val(zlCommFun.Nvl(!数据来源))) = 0 Then
                                If zlCommFun.Nvl(!数值) = "不升" And lng项目序号 = gint体温 Then
                                    lngColor = 255
                                ElseIf lng项目序号 = gint体温 Or lng项目序号 = gint疼痛强度 Or (lng项目序号 = gint脉搏 And mint心率应用 = 2 And mblnEdit心率 = True) Then
                                    If InStr(1, zlCommFun.Nvl(!数值), "/") = 0 Then
                                        lngColor = RGB(0, 0, 255)
                                    Else
                                        If Val(!修改) = 0 Then
        ...
        If NewCol = col_数据 Or NewCol = Col_未记说明 Then
            ...
            If InStr(1, ",0,9,", "," & Val(marrDate(NewRow)) & ",") = 0 Then
                If NewCol = col_数据 Then
                    ...
                    If lng项目序号 = gint体温 Or lng项目序号 = gint疼痛强度 Or (lng项目序号 = gint脉搏 And mint心率应用 = 2 And mblnEdit心率 = True) Then
    ...
    If KeyAscii <> vbKeyReturn Then
        If lngNO <> 0 Then
            If vsfCurve.TextMatrix(Row, COL_分组名) = "1)体温曲线项目" Then
                If Col <> Col_未记说明 Then
                    If lngNO = gint脉搏 And mint心率应用 = 2 And mblnEdit心率 = True Then
...
NotEdit:
            ...
            ElseIf lng项目序号 = gint脉搏 And mint心率应用 = 2 And mblnEdit心率 = True Then
...
CheckPoint:
    If Trim(vsfCurve.EditText) <> "" And str值域 <> "" Then
        ...
        If lngNO <> gint体温 And lngNO <> gint疼痛强度 And Not (lngNO = gint脉搏 And mint心率应用 = 2 And mblnEdit心率 = True) Then
...
Private Sub vsfOper_ValidateEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    With vsfOper
        ...
        If Col = Col_OperTime Then
            ...
            If mblnMove Then gstrSQL = ReplaceSQL(gstrSQL)
