...
Begin VB.Form frmCaseTendBodyPara 
   ...
   Begin VB.Frame fra 
      ...
      Begin VB.CheckBox chk 
         Caption         =   "且标题信息中的医院名称和""体温单""分两行显示"
         Height          =   180
         Index           =   19
         Left            =   480
         TabIndex        =   41
...
Public Function ShowPara(ByVal frmMain As Object, ByVal strPrivs As String) As Boolean
    ...
    strTmp = zlDatabase.GetPara("打印医院名称", glngSys, 1255, "1", Array(chk(1), chk(19)), InStr(mstrPrivs, "护理选项设置") > 0)
    chk(1).Value = Val(strTmp)
    If InStr(1, strTmp, ",") > 0 Then
        chk(19).Value = Val(Split(strTmp, ",")(1))
    End If
    Call chk_Click(1)
...
Private Sub chk_Click(Index As Integer)
    If Index = 1 Then
        chk(19).Enabled = (chk(Index).Value = 1)
    End If
End Sub
...
Private Sub cmdOK_Click()
    ...
    Call zlDatabase.SetPara("打印医院名称", chk(1).Value & "," & chk(19).Value, glngSys, 1255, InStr(mstrPrivs, "护理选项设置") > 0)
    '51282,刘鹏飞,2012-08-06,DYEY要求汇总项目可以录入、显示汇总小时
