...
Private Function InitBody(ByVal lng文件ID As Long, ByVal lng病人id As Long, ByVal lng主页id As Long, ByVal int婴儿 As Integer) As Boolean
    ...
    If mblnMoved = True Then strSQL = ReplaceSQL(strSQL)
    ...
    '读取此病人的体温单总页数
    '------------------------------------------------------------------------------------------------------------------
    strSQL = " SELECT  入院时间,实际入院时间,出院时间,1 + TRUNC((TO_DATE(TO_CHAR(出院时间,'yyyy-MM-dd'),'yyyy-MM-dd') -TO_DATE(TO_CHAR(入院时间,'yyyy-MM-dd'),'yyyy-MM-dd')) / 7) AS 页数,发生时间,记录 " & _
             "  From (" & _
    ...
    strSQL = " SELECT /*+ RULE */ 1 + TRUNC((TO_DATE(TO_CHAR(DECODE(D.开始时间,NULL,A.开始时间, DECODE(SIGN(D.开始时间 - A.开始时间), 1, D.开始时间, A.开始时间)),'YYYY-MM-DD'),'YYYY-MM-DD') -" & vbNewLine & _
            "                  TO_DATE(TO_CHAR(DECODE(D.开始时间, NULL, B.入院时间, D.开始时间), 'YYYY-MM-DD'), 'YYYY-MM-DD')) / 7) AS 开始页码," & vbNewLine & _
            ...
            "                           'YYYY-MM-DD') - TO_DATE(TO_CHAR(DECODE(D.开始时间, NULL, B.入院时间, D.开始时间), 'YYYY-MM-DD'), 'YYYY-MM-DD')) / 7) AS 结束页码," & vbNewLine & _
    ...
    Select Case strMenuItem
        Case "初始化"
            ...
            mstrSQL = "SELECT A.序号,A.姓名 FROM(" & vbNewLine & _
                        ...
                        "ORDER BY A.序号"
            If mblnMoved = True Then mstrSQL = ReplaceSQL(mstrSQL)
...
'---------------------------------------------------------------------
    ...
    With mshUpTab
        ...
        .TextMatrix(0, 0) = "日       期"
        .TextMatrix(1, 0) = IIf(T_Patient.lng婴儿 = 0, "住 院 日 数", "出 生 日 数")
        .TextMatrix(2, 0) = "手术后日数"
        .TextMatrix(3, 0) = "时       间"
    ...
    mshDownTab.RowHeightMin = 255
    mshDownTab.RowHeight(-1) = 255
    '补充完剩下的空行
...
'----------------------------------------------------------------
    ...
    With mshUpTab
        ...
        For i = 2 To .Cols - 2
                If (i - 1) Mod 6 = 0 Then
                Call .CellBorderRange(0, i - 1, .Rows - 1, i - 1, RGB_RED, 0, 0, 1, 0, 1, 0)
            End If
        Next
...
'        Next i
        ...
        If mblnMoved = True Then gstrSQL = ReplaceSQL(gstrSQL)
...
ErrNext:
    ...
    '体温框竖线和日期栏竖线颜色相同,第一条和最后一条竖线为黑色所以从2开始,最后减1
    For i = 2 To 40
        If (i - 1) Mod 6 = 0 Then
            Call vsf.Body.CellBorderRange(0, i, vsf.Rows - 1, i, RGB_RED, 0, 0, 1, 0, 1, 0)
        End If
    Next
    ...
    With mshDownTab
        ...
        lngItemCode = 0
        str项目名称 = ""
        '江西体温部件,体温框竖线和日期栏竖线颜色相同,第一条和最后一条竖线为黑色所以从2开始,最后减1
        For intRow = .FixedRows To .Rows - 1
            For i = 2 To .Cols - 6 - 1
                If (i - 1) Mod 2 = 0 Then
                    Call .CellBorderRange(intRow, i, intRow, i, RGB_RED, 0, 0, 1, 0, 1, 0)
                End If
            Next
        Next
        
        For intRow = .FixedRows To .Tag - 1
'            '山西体温部件,体温框竖线和日期栏竖线颜色相同,第一条和最后一条竖线为黑色所以从2开始,最后减1
'            For i = 2 To .Cols - 2
'                If (i - 1) Mod 2 = 0 Then
'                    Call .CellBorderRange(intRow, i - 1, intRow, i - 1, RGB_RED, 0, 0, 1, 0, 1, 0)
'                End If
'            Next
        ...
        If mbln显示皮试 = True And mintRepairRows > Val(mshDownTab.Tag) And mintRepairRows > 0 Then
            ...
            For intRow = Val(mshDownTab.Tag) To mintRepairRows - 1
                Call mshDownTab.CellBorderRange(intRow, 1, intRow, mshDownTab.Cols - 1 - 6, RGB_RED, 0, 0, 1, 0, 1, 0)
            Next intRow
        End If
    End With
...
Public Sub Paint_Canvas(Optional ByVal blnAdjust As Boolean = False)
    ...
    Dim lngMaxX     As Long, lngMaxY As Single, lngAllMaxY As Long  '边界
    ...
    If intLables <= 3 Then
        lng刻度宽度 = glngLableWith
    Else
        lng刻度宽度 = glngLableStep * intLables
    End If
    ...
    lngMaxX = lng刻度宽度 + (7 * 6 * lngColStep) + T_DrawClient.偏移量X '刻度+7*宽度 +T_DrawClient.偏移量X
...
'    Next
    ...
    For lngRow = 1 To 6 * 7
        ...
        Call DrawLine(mlngMemDC, lngCurX, lngCurY, lngCurX, lngMaxY, PS_SOLID, IIf(lngRow Mod 6 = 0, 2, 1), IIf(lngRow Mod 6 = 0, IIf(lngRow = 42, RGB_GRAY, RGB_RED), RGB_GRAY))
    Next
        ...
        Do While Not .EOF
            ...
            Select Case !项目序号
                ...
                Case gint体温  '体温整数时输出刻度
                    ...
                    lblCommText.Caption = lblCommText.Caption & "、" & zlCommFun.Nvl(!记录名) & "(口温" & arrTemp(0) & ",腋温" & arrTemp(1) & ",肛温" & arrTemp(2) & ",耳温" & arrTemp(3) & ")"
            ...
            Do While True
                ...
                If bln显示刻度 Then
                    ...
                    If sin刻度 = Val(Nvl(!最大值, 0)) And lngCurY < sinY单位 Then
                        Call GetTextRect(mobjDraw, lngCurX, sinY单位 + 1, Format(sin刻度, "#0"), lngLableStep)
                    ...
                    If strComment <> "" Then
                        ...
                        For i = 1 To Len(strComment)
                            ...
                            If Y < T_DrawClient.刻度区域.Bottom Then
                                ...
                                '输出字体信息
                                If T_DrawClient.刻度区域.Bottom - Y >= T_Size.H - 1 Then
            ...
            ElseIf !类型 = 6 Then '输出下标说明
                ...
                For i = 1 To Len(strComment)
                    If Y < T_DrawClient.刻度区域.Bottom Then
                        ...
                        If Asc(strText) < 0 Then
                            If (intAscCharNum - intNum) Mod 2 = 1 Then Y = Y + IIf(T_DrawClient.行单位 * 2 > T_Size.H, T_DrawClient.行单位 * 2, T_Size.H) / 2
                        End If
...
Private Sub SaveMemory(ByVal blnAutoAdjust As Boolean)
    ...
    gstrSQL = "SELECT C.ID 序号,a.发生时间 As 时间,C.显示,C.记录内容 As 数值,C.体温部位,C.复试合格,D.记录名,E.保留项目,D.项目序号,DECODE(D.项目序号,-1,1,C.记录标记) 记录标记,C.未记说明 " & _
                ...
                "Where B.ID=A.文件ID " & _
                    ...
                    "AND Instr(',1,3,', ',' || D.记录法 || ',') > 0 " & _
                "Order By A.发生时间,DECODE(C.项目序号,-1,1,0),DECODE(D.项目序号,-1,1,C.记录标记)"
    If mblnMoved Then gstrSQL = ReplaceSQL(gstrSQL)
    ...
    '1-入院；2-入科；3-转科；4-换床
    gstrFields = "时间|项目序号|类型|内容|颜色|X坐标|Y坐标|高度|打印X坐标|禁用"  '入出转缺省是红色,上下标及未记说明缺省是蓝色
    Set rsData = GetDataFromHis(T_Patient.lng病人id, T_Patient.lng主页id, T_Patient.lng婴儿, CDate(mstr开始时间), CDate(mstr结束时间), mblnMoved, 2) ' Int(cdate(mstr开始时间))
    With rsData
        Do While Not .EOF
    ...
    If T_Patient.lng婴儿 > 0 Then
        gstrFields = "时间|项目序号|类型|内容|颜色|X坐标|Y坐标|高度|打印X坐标|禁用"  '入出转缺省是红色,上下标及未记说明缺省是蓝色
        Set rsData = GetDataFromHis(T_Patient.lng病人id, T_Patient.lng主页id, T_Patient.lng婴儿, CDate(mstr开始时间), CDate(mstr结束时间), mblnMoved, 3)
        With rsData
            Do While Not .EOF
                bytShow = 0
                If Trim(zlCommFun.Nvl(!内容)) <> "" Then
...
Public Function PrintState(ByVal intPrintRange As Integer, ByVal blnPrint As Boolean, Optional lngBeginY As Long, _
    ...
    If mblnMoved Then strSQL = ReplaceSQL(strSQL)
    ...
    '读取此病人的体温单总页数
    '------------------------------------------------------------------------------------------------------------------
    strSQL = " SELECT  入院时间,出院时间,1 + TRUNC((TO_DATE(TO_CHAR(出院时间,'yyyy-MM-dd'),'yyyy-MM-dd') -TO_DATE(TO_CHAR(入院时间,'yyyy-MM-dd'),'yyyy-MM-dd')) / " & 7 & ") AS 页数,发生时间 " & _
            "  From (" & _
                " SELECT DECODE(D.开始时间,NULL,DECODE(C.出生时间,NULL," & IIf(strMarkDate = "", "B.入院时间", strMarkDate) & ",C.出生时间)," & vbNewLine & _
                "               DECODE(SIGN(D.开始时间 - DECODE(C.出生时间,NULL," & IIf(strMarkDate = "", "B.入院时间", strMarkDate) & ",C.出生时间))," & vbNewLine & _
                "                      1," & vbNewLine & _
    ...
    Case 0                  '打印当前页
        ...
        If PrintOrPreviewBodyState(objPrint, lng病人id, lng主页id, lng文件ID, intBaby, _
