   ...
   Begin VB.PictureBox picBack 
      ...
      Begin VB.PictureBox picMain 
         ...
         Begin VSFlex8Ctl.VSFlexGrid mshDownTab 
            ...
            AccessibleRole  =   24
         End
         Begin zl9TemperatureChartNJ.VsfGrid vsf 
            Height          =   255
            Left            =   120
            TabIndex        =   28
            Top             =   3480
...
Private mblnShowbreatheTab As Boolean  '是否显示呼吸表格
    ...
    Select Case strMenuItem
        Case "初始化"
            ...
            If RS.BOF = False Then
                T_Patient.lng科室ID = Val(Nvl(RS("出院科室ID").Value))
        ...
        Case "显示病人信息"
            ...
            Else
                ...
                If RS.BOF = False Then
                    ...
                    txtCard(8).Text = Format(RS("出生时间").Value, "yyyy-MM-dd")
                End If
            End If
...
Private Function InitData(ByVal lng病人ID As Long, ByVal lng主页ID As Long, _
    ...
    If rsTemp.BOF = False Then T_Patient.lng护理等级 = Nvl(rsTemp("护理等级"), 3)
...
'功能:获取病人信息
    Dim rsTmp As New ADODB.Recordset
    Dim strTmp As String, strTime As String, strStart As String, strTo As String, strUnit As String
    Dim intCOl As Integer
    ...
    If rsTmp.BOF = False Then
        txtCard(0).Text = Nvl(rsTmp("姓名").Value)
        txtCard(0).Tag = Nvl(rsTmp("姓名").Value)
        txtCard(1).Text = Nvl(rsTmp("住院号").Value)
        txtCard(5).Text = Nvl(rsTmp("性别").Value)
        txtCard(6).Text = Nvl(rsTmp("年龄").Value)
    End If
    ...
    If rsTmp.BOF = False Then
        Do While Not rsTmp.EOF
            If Nvl(rsTmp("病区").Value) <> strUnit And Nvl(rsTmp("病区").Value) <> "" Then
                strUnit = Nvl(rsTmp("病区").Value)
                
                If txtCard(3).Text = "" Then
                    txtCard(3).Text = strUnit
                Else
                    txtCard(3).Text = txtCard(3).Text & "->" & strUnit
                End If
            End If
            
            If Nvl(rsTmp("科室").Value) <> strTmp And Nvl(rsTmp("科室").Value) <> "" Then
                strTmp = Nvl(rsTmp("科室").Value)
            ...
            If Nvl(rsTmp("床号").Value) <> strTime And Nvl(rsTmp("床号").Value) <> "" Then
                strTime = Nvl(rsTmp("床号").Value)
        ...
        If Left(txtCard(3).Text, 2) = "->" Then txtCard(3).Text = Mid(txtCard(3).Text, 3)
    ...
    If bln显示诊断 = True Then
        ...
        If rsTmp.BOF = False Then
            If T_Patient.lng婴儿 = 0 Then
                txtCard(7).Text = Nvl(rsTmp("最后诊断").Value)
            Else
                txtCard(7).Text = ""
            End If
        Else
...
'---------------------------------------------------------------------
    ...
    '重新整理表格位置
    mblnShowbreatheTab = mItemNO.呼吸 <> 0
    ...
    PartRect.Top = 0
    PartRect.Left = 1
    PartRect.Right = picRect(1).Width \ Screen.TwipsPerPixelX
    PartRect.Bottom = picRect(1).Height \ Screen.TwipsPerPixelY
    picRect(1).Cls
    Call PrintCurveInfo(picRect(1), PartRect)
    Call SetVisible
    Exit Sub
Errhand:
    If ErrCenter = 1 Then
...
Private Function ShowDowntab() As Boolean
    ...
    If mblnShowbreatheTab = True Then strItems = strItems & ",'呼吸'"
    ...
    With rsTemp
        Do While Not .EOF
            If CDate(Format(!时间, "YYYY-MM-DD HH:mm:ss")) >= CDate(Format(mstr开始时间, "YYYY-MM-DD HH:mm:ss")) Then
                ...
                str结果 = Nvl(rsTemp!结果) & ";" & Nvl(rsTemp!体温部位)
...
ErrNext:
    ...
    For i = 1 To T_BodyStyle.lng监测次数 * T_BodyStyle.lng天数
        ...
        End If
        '整理每段呼吸机的一个范围
        If mblnShowbreatheTab = True Then
    ...
    With mshDownTab
            ...
            If str项目名称1 & ";" & .RowData(intRow) <> str项目名称 & ";" & lngItemCode Then
                ...
                For intDay = 0 To T_BodyStyle.lng天数 - 1
                    ...
                    Do While Not rsDownTab.EOF
                        str结果 = Nvl(rsDownTab!记录内容, "")
                        lngColor = 0
                        If blnColor Then lngColor = Val(Nvl(rsDownTab!未记说明, 0))
        ...
        If mbln显示皮试 = True And mintRepairRows > Val(mshDownTab.Tag) And mintRepairRows > 0 Then
            ...
            '89797:LPF,皮试内容和结果之间以空格区分,内容为黑色，结果以颜色区分，且结果不包含括号(为了便于组装内容此处先用'替代空格)
            strSQL = _
                ...
                "               Replace(Replace(Replace(Decode(b.试管编码, Null, a.医嘱内容, b.试管编码), ',', ''), '-#', ''), '皮试', '') || '$'  || Replace(Replace(a.皮试结果,'(',''),')','') 药物名" & vbNewLine & _
            ...
            Select Case !记录名
                ...
                Case Else
                    If mintRepairRows > 0 And mintRepairRows > intRowCount Then
                        ...
                        '汇总项目或波动项目频次最大为2
                        If Val(Nvl(!项目表示)) = 4 Or IsWaveItem(Val(Nvl(!项目序号))) Then
                        ...
                        For i = 1 To intRowNum
                            ...
                            mshDownTab.TextMatrix(intRow, 1) = Nvl(!项目类型) & "," & Nvl(!项目小数) & "," & _
                                int频次 & "," & Nvl(!项目表示) & "," & Nvl(!项目性质) & "," & Nvl(!项目长度) & "," & Nvl(!入院首测, 0) & "," & Nvl(!体温部位)
                            mshDownTab.TextMatrix(intRow, 2) = Nvl(!最小值, "") & ";" & Nvl(!最大值, "")
...
Private Sub mshDownTab_DrawCell(ByVal hDC As Long, ByVal Row As Long, ByVal Col As Long, ByVal Left As Long, ByVal Top As Long, ByVal Right As Long, ByVal Bottom As Long, Done As Boolean)
    ...
    Dim lngColor As Long, blnRed As Boolean
    Dim strTmp As String
    ...
    If mbln显示皮试 = True And mintRepairRows > Val(mshDownTab.Tag) And mintRepairRows > 0 And Col >= mshDownTab.FixedCols And Row >= Val(mshDownTab.Tag) Then
        If (Col - mshDownTab.FixedCols) Mod T_BodyStyle.lng监测次数 = 0 And UBound(mstrNewString) >= (Row - Val(mshDownTab.Tag)) Then
            ...
            '89797:LPF,皮试内容和结果之间以空格区分,且只有结果以颜色区分(内容和结果以'展示区分)
            '开始输出内容
            Call SetTextColor(lngDc, 0)
            blnRed = False
            For i = 0 To UBound(arrData)
                '居中显示:
                lngMoveX = Format((txtLength.Width - objDraw.TextWidth(CStr(arrData(i)))) \ T_TwipsPerPixel.X \ 2, "#0") - 1
                If lngMoveX < 0 Then lngMoveX = 0
                arrItem = Split(CStr(arrData(i)), "$")
                For j = 0 To UBound(arrItem)
                    If j > 0 And blnRed = False Then
                        Call SetTextColor(lngDc, lngColor)
                        blnRed = True
                    End If
                    strTmp = CStr(arrItem(j))
                    '排除最后一个字符(第一行)或第一个字符(第二行)为'的情况
                    If j > 0 Then
                        If arrItem(j - 1) <> "" And arrItem(j) <> "" Then strTmp = " " & strTmp
                    End If
                    If strTmp <> "" Then
                        Call GetTextRect(objDraw, LPoint.X + lngMoveX, LPoint.Y, strTmp, , False)
...
Private Sub picBack_Resize()
    ...
    picCard(0).Tag = ""
    picCard(0).Height = 740
    If T_Patient.lng婴儿 <> 0 And Not (Val(zlDatabase.GetPara("体温单显示诊断", glngSys, 1255, 1)) = 1) Then picCard(0).Height = picCard(0).Height / 2 + 5
    ...
    With mshUpTab
        ...
        .ColWidthMin = T_DrawClient.列单位 * Screen.TwipsPerPixelX
         picCard(0).Width = .Width + IIf(gbln呼吸曲线 = True, T_BodyStyle.lngBreashLabelStep * Screen.TwipsPerPixelX, 0)
         .Refresh
    End With
    
    picDraw.Move 0, mshUpTab.Top + mshUpTab.Height, (T_DrawClient.体温区域.Right + IIf(gbln呼吸曲线 = True, T_BodyStyle.lngBreashLabelStep, 0) + 1) * T_TwipsPerPixel.X, _
    ...
    With vsf
        ...
        .Visible = mblnShowbreatheTab
    End With
        
    With mshDownTab
        .ColWidth(0) = mshUpTab.ColWidth(0)
        .Left = lngLeft
        .Top = mshUpTab.Top + mshUpTab.Height + (IIf(mblnShowbreatheTab = True, vsf.Height, 0)) + (T_DrawClient.曲线总区域.Bottom - T_DrawClient.曲线总区域.Top + 1) * Screen.TwipsPerPixelY - Screen.TwipsPerPixelY
        .Width = mshUpTab.Width
        lngHeight = 0
    ...
    With picRect(0)
        .Top = mshUpTab.Top
        .Left = mshUpTab.Left + mshUpTab.Width - Screen.TwipsPerPixelX
        .Width = (T_BodyStyle.lngBreashLabelStep + 1) * Screen.TwipsPerPixelX
        .Height = mshUpTab.Height + Screen.TwipsPerPixelY
        .ZOrder 1
        .Visible = gbln呼吸曲线
    End With
    ...
    With picRect(1)
        .Top = mshDownTab.Top
        .Left = picRect(0).Left
        .Width = picRect(0).Width
        .Height = mshDownTab.Height
        .ZOrder 0
        .Visible = gbln呼吸曲线
    End With
    ...
    picMain.Width = mshUpTab.Width + mshUpTab.Left + IIf(gbln呼吸曲线 = True, T_BodyStyle.lngBreashLabelStep * Screen.TwipsPerPixelX, 0)
    picMain.Height = lblCommText.Top + lblCommText.Height
    
    If picCard(0).Tag = "" Then Call picCard_Resize(0)
    '计算滚动条
    Call CalcScrollBarSize
    
    '计算体温单的可画区域大小
    mlng高度 = (picBack.Height - mshUpTab.Top - mshUpTab.Height - mshDownTab.Height - lblCommText.Height - _
        IIf(mblnShowbreatheTab = True, vsf.Height, 0) - IIf(hsb.Max > 0, hsb.Height, 0)) / Screen.TwipsPerPixelY
...
Private Sub picCard_Resize(Index As Integer)
    On Error Resume Next
    If T_Patient.lng婴儿 <> 0 Then
        lblCard(8).Visible = True
        txtCard(8).Visible = True
        lblCard(2).Visible = False
        txtCard(2).Visible = False
        lblCard(5).Visible = False
        txtCard(3).Visible = False
        lblCard(8).Left = lblCard(2).Left
        lblCard(8).Top = lblCard(2).Top
        txtCard(8).Top = txtCard(2).Top
        txtCard(8).Left = lblCard(8).Left + lblCard(8).Width
        lblCard(3).Caption = "母亲床号:"
        lblCard(3).Top = lblCard(8).Top
        lblCard(3).Left = txtCard(8).Left + txtCard(8).Width + 90
        txtCard(4).Top = lblCard(3).Top
        txtCard(4).Left = lblCard(3).Left + lblCard(3).Width
        lblCard(1).Caption = "母亲住院号:"
        lblCard(1).Top = lblCard(3).Top
        lblCard(1).Left = txtCard(4).Left + txtCard(4).Width + 90
        txtCard(1).Top = lblCard(1).Top
        txtCard(1).Left = lblCard(1).Left + lblCard(1).Width
        lblCard(7).Left = lblCard(0).Left
        txtCard(7).Left = lblCard(7).Left + lblCard(7).Width
        txtCard(1).Move txtCard(1).Left, txtCard(1).Top, picCard(Index).Width - txtCard(1).Left - 45
    Else
        lblCard(8).Visible = False
        txtCard(8).Visible = False
        lblCard(2).Visible = True
        txtCard(2).Visible = True
        lblCard(5).Visible = True
        txtCard(3).Visible = True
        lblCard(3).Caption = "床号:"
        lblCard(3).Top = lblCard(7).Top
        lblCard(3).Left = lblCard(0).Left
        txtCard(4).Top = lblCard(3).Top
        txtCard(4).Left = lblCard(3).Left + lblCard(3).Width
        lblCard(1).Caption = "住院号:"
        lblCard(1).Top = lblCard(3).Top
        lblCard(1).Left = lblCard(6).Left
        txtCard(1).Top = lblCard(1).Top
        txtCard(1).Left = lblCard(1).Left + lblCard(1).Width
        lblCard(7).Left = lblCard(2).Left
        txtCard(7).Left = lblCard(7).Left + lblCard(7).Width
        txtCard(3).Move txtCard(3).Left, txtCard(3).Top, picCard(Index).Width - txtCard(3).Left - 45
    End If
...
Public Sub Paint_CanvasNew(Optional ByVal blnAdjust As Boolean = False)
    ...
    gstrSQL = " Select /*+ Rule*/ A.项目序号,A.排列序号,A.记录名,A.记录符,A.记录色,nvl(A.最大值,0) 最大值,nvl(A.最小值,0) 最小值," & _
        "nvl(A.单位值,0) 单位值,A.刻度间隔,A.警示线,C.项目单位 单位,Decode(记录法,3,A.最高行,nvl(A.最高行,2)-2) AS 最高行,B.部位,A.记录法,DECODE(A.项目序号,3,1,0) 序号" & _
    ...
    With rsTemp
        Do While Not .EOF
            ...
            If lng最高行 < 0 Then lng最高行 = 0
             ...
             '修改问题51442
            If Val(Nvl(!最小值, 0)) > 34 Then
                lngMaxRows = lng最高行 + (Val(Nvl(!最大值, 0)) - 35) / 0.1
            Else
                lngMaxRows = lng最高行 + (Val(Nvl(!最大值, 0)) - Val(Nvl(!最小值, 0))) / 0.1
            End If
    ...
    gbln呼吸曲线 = rsTemp.RecordCount > 0
    rsTemp.Filter = "记录法=1"
    rsTemp.Sort = "排列序号"
    intLables = rsTemp.RecordCount
    If gbln呼吸曲线 = True Then intLables = intLables - 1
    If rsTemp.RecordCount > 0 Then rsTemp.MoveFirst
    ...
    For i = 1 To mintNullRow
        If mintNullRow > 1 Then
            Call DrawLine(mlngMemDC, T_DrawClient.偏移量X + lng刻度宽度, lngCurY + (lngInitRowStep * 2 * i), lngMaxX, lngCurY + (lngInitRowStep * 2 * i), PS_SOLID, 1, RGB_FleetGRAY)
        Else
            Call DrawLine(mlngMemDC, T_DrawClient.偏移量X, lngCurY + (lngInitRowStep * 2 * i), lngMaxX, lngCurY + (lngInitRowStep * 2 * i), PS_SOLID, 1, RGB_BLACK)
        End If
    Next i
    ...
    For lngRow = 1 To T_BodyStyle.lng监测次数 * T_BodyStyle.lng天数
        ...
        Call DrawLine(mlngMemDC, lngCurX, lngCurY, lngCurX, lngMaxY, PS_SOLID, IIf(lngRow Mod T_BodyStyle.lng监测次数 = 0, 2, 1), IIf(lngRow Mod T_BodyStyle.lng监测次数 = 0 And lngRow < T_BodyStyle.lng监测次数 * T_BodyStyle.lng天数, RGB_RED, RGB_GRAY))
    Next
    ...
    '呼吸为曲线再画外线
    If gbln呼吸曲线 = True Then
        Call DrawLine(mlngMemDC, T_DrawClient.体温区域.Right + T_BodyStyle.lngBreashLabelStep, T_DrawClient.体温区域.Top, T_DrawClient.体温区域.Right + T_BodyStyle.lngBreashLabelStep, lngAllMaxY, PS_SOLID, 1, RGB_BLACK)
    End If
    Set mobjPart = New Collection
    
    '画刻度框的标尺（从固定不变的10行开始标识）
    rsTemp.Filter = "记录法=1"
    rsTemp.Sort = "序号,排列序号"  '排序后呼吸在最后
    With rsTemp
        Do While Not .EOF
            '显示刻度框项目的名称及符号,如体温×
            If !项目序号 = gint呼吸 Then
                lngCurX = T_DrawClient.体温区域.Right
                lngCurY = T_DrawClient.刻度区域.Top
                sinY单位 = lngCurY
            Else
                lngCurX = T_DrawClient.刻度区域.Left + ((.AbsolutePosition - 1) * T_DrawClient.刻度单位)
                If .AbsolutePosition = (.RecordCount - IIf(gbln呼吸曲线 = True, 1, 0)) Then
                ...
                Call GetTextRect(mobjDraw, lngCurX, lngCurY + mobjDraw.TextHeight(Nvl(!记录名)) / Screen.TwipsPerPixelY / 2, Trim(Nvl(!记录名)), lngLableStep)
                Call DrawText(mlngMemDC, Trim(Nvl(!记录名)), -1, T_LableRect, DT_CENTER)
                Call SelectObject(mlngMemDC, mlngOldFont)
                Call DeleteObject(mlngFont)
                ...
                '输出项目单位
                Call GetTextRect(mobjDraw, lngCurX, lngCurY + lngInitRowStep * 2 + mobjDraw.TextHeight(Nvl(!单位)) / Screen.TwipsPerPixelY / 2, Trim(Nvl(!单位)), lngLableStep)
                Call DrawText(mlngMemDC, Trim(Nvl(!单位)), -1, T_LableRect, DT_CENTER)
                Call SelectObject(mlngMemDC, mlngOldFont)
                Call DeleteObject(mlngFont)
                Call ReleaseFontIndirect(mobjDraw)
                If T_LableRect.Bottom > lngInitRowStep * 2 * mintNullRow + T_DrawClient.刻度区域.Top Then
                    sinY单位 = T_LableRect.Bottom
                Else
                    sinY单位 = lngInitRowStep * 2 * mintNullRow + T_DrawClient.刻度区域.Top
                End If
            ...
            mlngOldFont = SelectObject(mlngMemDC, mlngFont)
            '输出体温项目的标识符
            'Call DrawMarker(False, !项目序号, NVL(!部位, "空"), lngCurX + T_TwipsPerPixel.x / 2, lngCurY + 15, "空", True)
            ...
            Select Case !项目序号
                ...
                Case gint体温  '体温整数时输出刻度
                    ...
                    lblCommText.Caption = lblCommText.Caption & "、" & Nvl(!记录名) & "(口温" & arrTemp(0) & ",腋温" & arrTemp(1) & ",肛温" & arrTemp(2) & ",耳温" & arrTemp(3) & ")"
                ...
                Case gint脉搏, gint心率  '脉搏/心跳按10的倍数输出刻度
                    ...
                    If !项目序号 = gint脉搏 Then
                        lblCommText.Caption = lblCommText.Caption & "、" & Nvl(!记录名) & "(缺省记录符" & Nvl(!记录符, "+") & ",起搏器H)"
                    Else
                        lblCommText.Caption = lblCommText.Caption & "、" & Nvl(!记录名) & "(" & Nvl(!记录符, "Ο") & ")"
                    End If
                ...
                Case gint呼吸  '呼吸按5的倍数输出刻度
                    ...
                    dbl单位值 = 1
                    sinAlertness = Nvl(!警示线, 0)
                    lblCommText.Caption = lblCommText.Caption & "、" & Nvl(!记录名) & "(自主呼吸" & Nvl(!记录符, "*") & ",呼吸机R)"
                Case Else
                    intTens_digit = 1
                    dbl单位值 = Val(Nvl(!单位值, 1))
                    sin刻度间隔 = Nvl(!刻度间隔, Val(Nvl(!单位值, 0)) * 10)
                    If sin刻度间隔 > Val(Nvl(!最大值)) - Val(Nvl(!最小值)) Then
                        sin刻度间隔 = Val(Nvl(!最大值)) - Val(Nvl(!最小值))
                    End If
                    sinAlertness = Nvl(!警示线, 0)
                    lblCommText.Caption = lblCommText.Caption & "、" & Nvl(!记录名) & "(" & Nvl(!记录符, "*") & ")"
            End Select
            
            If !项目序号 = gint体温 Then
                mobjPart.Add Array("" & !项目序号, "口温", arrTemp(0), Nvl(!记录色, RGB_BLACK), "B"), "B" & !项目序号
                mobjPart.Add Array("" & !项目序号, "腋温", arrTemp(1), Nvl(!记录色, RGB_BLACK), "A"), "A" & !项目序号
                mobjPart.Add Array("" & !项目序号, "肛温", arrTemp(2), Nvl(!记录色, RGB_BLACK), "C"), "C" & !项目序号
            ElseIf !项目序号 = gint脉搏 And mint心率应用 = 2 Then
            ...
            '根据最高行定位到有效位置
            lngCurY = lngCurY + (T_DrawClient.行单位 * Nvl(!最高行, 2))
            blnFirst = False
            Do While True
                bln显示刻度 = False
                ...
                If bln显示刻度 Then
                    '控制最大值不与曲线单位重复
                    '呼吸最大值输出项目名称
                    If sin刻度 = Val(Nvl(!最大值, 0)) And !项目序号 = gint呼吸 Then
                        strTmp = Trim(Nvl(!记录名))
                        lngY = lngCurY - Len(strTmp) / 2 * (mobjDraw.TextHeight("刘") / T_TwipsPerPixel.Y)
                        If lngY < sinY单位 Then lngY = sinY单位
                        For i = 1 To Len(strTmp)
                            Call GetTextRect(mobjDraw, lngCurX, lngY, Mid(strTmp, i, 1), False, False)
                            Call DrawText(mlngMemDC, Mid(strTmp, i, 1), -1, T_LableRect, DT_CENTER)
                            lngY = lngY + mobjDraw.TextHeight("呼") / T_TwipsPerPixel.Y
                        Next i
                    Else
                        If sin刻度 = Val(Nvl(!最大值, 0)) And lngCurY < sinY单位 Then
                            Call GetTextRect(mobjDraw, lngCurX, sinY单位, Format(sin刻度, "#0"), IIf(!项目序号 = gint呼吸, 0, lngLableStep))
                        ElseIf lngCurY = T_DrawClient.刻度区域.Bottom Then
                            Call GetTextRect(mobjDraw, lngCurX, lngCurY - (mobjDraw.TextHeight("1") / (T_TwipsPerPixel.Y * 2)), Format(sin刻度, "#0"), IIf(!项目序号 = gint呼吸, 0, lngLableStep))
                        Else
                            Call GetTextRect(mobjDraw, lngCurX, lngCurY, Format(sin刻度, "#0"), IIf(!项目序号 = gint呼吸, 0, lngLableStep))
                        End If
        ...
        Do While Not .EOF
            If Val(Nvl(!状态)) <> 3 Then
                ...
                If Not ((!项目序号 = gint体温 Or !项目序号 = gint疼痛强度) And !标记 = 1) Then
                    ...
                    '复查合格
                    If !项目序号 = gint体温 And Val(Nvl(!复查)) = 1 Then
...
Public Sub Paint_Assistant()
    ...
    With mrsNote
        Do While Not .EOF
            If X = !X坐标 Then
                If InStr(1, "," & strTemp & ",", "," & Nvl(!内容) & ",") <> 0 Then
                    mrsNote.Delete
                Else
                    strTemp = strTemp & "," & Nvl(!内容)
                End If
            Else
                X = !X坐标
                strTemp = Nvl(!内容)
...
Private Sub SaveMemory(ByVal blnAutoAdjust As Boolean)
        ...
        Do While Not .EOF
            ...
            If mrsItems.RecordCount > 0 Then
                ...
                If lng项目序号 = gint脉搏 Then
                    ...
                    strValues = lng项目序号 & "|" & Nvl(!数值) & "|" & SinX & "|" & Format(!时间, "yyyy-MM-dd HH:mm:ss")
                    Call Record_Add(rs脉搏, strFileds, strValues)
                End If
                
                If (Not IsNull(!未记说明)) And Nvl(!数值) <> "不升" Then
                    mrsNote.Filter = "项目序号=" & Val(Nvl(!项目序号)) & " AND X坐标=" & SinX
                    ...
                    gstrValues = Format(!时间, "yyyy-MM-dd HH:mm:ss") & "|" & !项目序号 & "|99|" & _
                        !未记说明 & "|" & lngNoRecordColor & "|" & SinX & "|0|0|0|0|" & Val(Nvl(!显示))
                   
                    If blnAdd Then
                        '提取接近中间时间点的值做为本列值
                         Call Record_Add(mrsNote, gstrFields, gstrValues)
                    Else
                        If (Nvl(mrsNote!显示, 0) = 1 And Nvl(!显示, 0) = 1) Or (Nvl(mrsNote!显示, 0) <> 1 And Nvl(!显示, 0) <> 1) Then
                ...
                Else
                    ...
                    mrsPoint.Filter = "项目序号=" & Val(Nvl(!项目序号, 0)) & " AND X坐标=" & SinX & " And 标记=" & int标记
                    ...
                    gstrValues = Val(Nvl(!序号)) & "|" & !数值 & "|" & strPart & "|" & int标记 & "|" & _
                                 Format(!时间, "yyyy-MM-dd HH:mm:ss") & "|" & lng项目序号 & "|0|" & Val(Nvl(!复试合格, 0)) & "|" & IIf(Nvl(!数值, 0) = "不升", 1, 0) & "|空|0|" & _
                                 SinX & "|" & sinY & "||" & str符号 & "|" & Val(Nvl(!显示, 0))
                    If blnAdd Then '添加
                        Call Record_Add(mrsPoint, gstrFields, gstrValues)
                    Else
                        If (Nvl(mrsPoint!显示, 0) = 1 And Nvl(!显示, 0) = 1) Or (Nvl(mrsPoint!显示, 0) <> 1 And Nvl(!显示, 0) <> 1) Then
    ...
    If mint心率应用 = 2 Then
        ...
        With rs脉搏
            Do While Not .EOF
                ...
                SinX = Val(Nvl(!X坐标))
                sinY = Val(Nvl(!Y坐标))
                mrsPoint.Filter = "项目序号=" & Val(Nvl(!项目序号, 0)) & " AND X坐标=" & SinX
                blnAdd = IIf(mrsPoint.RecordCount = 0, True, False)
                
                strFileds = "序号|数值|部位|标记|时间|项目序号|状态|复查|断开|重叠项目|重叠|X坐标|Y坐标|备注|符号|显示"
                strValues = Val(Nvl(!序号)) & "|" & !数值 & "|" & Nvl(!部位) & "|" & Val(Nvl(!标记, 0)) & "|" & _
                             Format(!时间, "yyyy-MM-dd HH:mm:ss") & "|" & Val(Nvl(!项目序号)) & "|0|0|" & Val(Nvl(!断开)) & "|空|0|" & _
                             SinX & "|" & sinY & "||" & Nvl(!符号) & "|" & Val(Nvl(!显示, 0))
    ...
    For j = 0 To 1
        ...
        For i = 0 To UBound(arrTmpValue)
            ...
            If mrsPoint.RecordCount <> 0 Then
                ...
                Call Record_Update(mrsPoint, gstrFields, gstrValues, "序号|" & Nvl(mrsPoint!序号))
            End If
        Next i
    ...
    With mrsPoint
        Do While Not .EOF
            strTmpString0 = strTmpString0 & ";" & Format(!时间, "yyyy-MM-dd HH:mm:ss") & "|" & Val(Nvl(!项目序号)) & "|99|" & _
            ...
            If Trim(Nvl(!内容)) <> "" Then
                ...
                If bytShow > 0 Then
                    ...
                    ElseIf bytShow = 3 Then
                        str内容 = str内容 & IIf(blnAutoAdjust = False, gstrCaveSplit & Nvl(!科室), "")
                    ElseIf bytShow = 4 Then
                        str内容 = str内容 & IIf(blnAutoAdjust = False, gstrCaveSplit & Nvl(!科室) & gstrCaveSplit & ConvertTimeToChinese(Format(!时间, "HH:mm")), "")
                        ...
                        Select Case bytShow
                            ...
                            Case 2
                                If Not blnAutoAdjust Then
                                    str内容 = Nvl(!内容) & gstrCaveSplit & ConvertTimeToChinese(Format(!时间, "HH:mm"))
                                Else
                                    str内容 = Nvl(!内容)
                                End If
                        End Select
    ...
    If Left(strTmpString0, 1) = ";" Then
        ...
        For i = 0 To UBound(Split(strTmpString0, ";"))
            ...
            mrsNote.Sort = "项目序号"
            If mrsNote.RecordCount > 0 Then
                mrsNote!内容 = IIf(lng体温不升显示方式 = 0 Or lng体温不升显示方式 = 2, "↓ ", "不升") & IIf(bytTag = 1, ";", " ") & Nvl(mrsNote!内容)
                mrsNote.Update
            Else
                If lng体温不升显示方式 = 0 Or lng体温不升显示方式 = 2 Then
...
Private Sub PrepareGraph()
    ...
    If mrsItems.RecordCount > 0 Then mint心率应用 = Nvl(mrsItems("应用方式").Value, 2): mrsItems.Filter = ""
    ...
    With rsTemp
        Do While Not .EOF
            If !展现方式 = 1 Then
                ...
                If !项目序号 = 1 Then '体温
                    ArrCode = Split("口温,腋温,肛温,耳温", ",")
                    strChar = Nvl(!标记符号, "·,×,○,△")
                    strChar = strChar & String(3 - UBound(Split(strChar, ",")), ",")
                    arrChar = Split(strChar, ",")
                    For i = 0 To UBound(ArrCode)
                        gstrFields = "项目序号|部位|重叠项目|记录符|记录色"
                        gstrValues = !项目序号 & "|" & ArrCode(i) & "|" & Nvl(!重叠项目) & "|" & arrChar(i) & "|" & Nvl(!标记颜色, 0)
                ...
                Else
                    ...
                    '产生相应的内存记录数据
                    gstrFields = "项目序号|部位|重叠项目|记录符|记录色"
                    gstrValues = !项目序号 & "|" & strPart & "|" & Nvl(!重叠项目) & "|" & strChar & "|" & Nvl(!标记颜色, 0)
                    Call Record_Add(mrsGraph, gstrFields, gstrValues)
                End If
            End If
            .MoveNext
        ...
        With rsTemp
            ...
            If rsTemp.RecordCount > 0 Then
                ...
                If Not IsNull(rsOverlap!标记符号) Then
                    ...
                    '产生相应的内存记录数据
                    gstrFields = "项目序号|部位|重叠项目|记录符|记录色"
                    gstrValues = lngID & "|" & strPart & "|" & strOverlap & "|" & Nvl(rsOverlap!标记符号) & "|" & rsOverlap!标记颜色
                    Call Record_Add(mrsGraph, gstrFields, gstrValues)
                Else
                    '输出图形文件
...
Private Sub ProcessPoint(ByVal strBeginDate As String)
    ...
    With mrsPoint
        If .RecordCount <> 0 Then .MoveFirst
        Do While Not .EOF
            If Not ((Val(!项目序号) = gint体温 Or Val(!项目序号) = gint疼痛强度) And Val(Nvl(!标记)) = 1) Then
                If lngCurX <> 0 Then
                ...
                If strDate <> "" Then
                    If DateDiff("d", CDate(strDate), CDate(Format(Nvl(!时间), "YYYY-MM-DD HH:mm:ss"))) > 1 Then
                        strPrimary = strPrimary & "," & lngOrder
                    End If
                End If
                '记录当前点的相关信息,供下一个点时检查
                strDate = Format(Nvl(!时间), "YYYY-MM-DD HH:mm:ss")
                lngOrder = Val(Nvl(!序号))
            End If
            .MoveNext
        Loop
...
Public Function PrintState(ByVal intPrintRange As Integer, ByVal blnPrint As Boolean, Optional lngBeginY As Long, _
    ...
    With MT_BodyStyle
        ...
        .lngBreashLabelStep = T_BodyStyle.lngBreashLabelStep
    End With
    With MT_BodyItem
...
ReStoreCuve:
    ...
    With T_BodyStyle
        ...
        .lngBreashLabelStep = MT_BodyStyle.lngBreashLabelStep
    End With
    With T_BodyItem
...
'-----------------------------------
    ...
    If mblnShowbreatheTab = False Or mItemNO.呼吸 = 0 Then Exit Sub
...
'-------------------------------------------------------------------------------------------
    ...
    End If
    T_BodyStyle.lngBreashLabelStep = glngBreashLableWidth / sinTwipsPerPixelX
