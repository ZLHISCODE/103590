...
Public Const glngBreashLableWidth As Long = 600 '呼吸曲线刻度列宽(缇)
...
Public Function PrintOrPreviewBodyStateNew(objOut As Object, _
    ...
    If intBaby <> 0 Then
        ...
        If rsTemp.BOF = False Then
            ...
            strBabyDay = Format(Nvl(rsTemp("出生时间").Value), "YYYY-MM-DD")
        End If
    ...
    If rsTemp.RecordCount > 0 Then
        rsTemp.MoveFirst
        rsTemp.Filter = "项目序号=" & gint呼吸 & " And 记录法=1"
        ...
        Else
            rsTemp.Filter = 0
        End If
        If gbln呼吸曲线 = True Then intDrawLineCOL = intDrawLineCOL - 1
    ...
    sigSign = 1
    If T_DrawClient.体温区域.Right + IIf(gbln呼吸曲线 = True, T_BodyStyle.lngBreashLabelStep, 0) > objDraw.Width / T_TwipsPerPixel.X - lngRight Then
        sigSign = Round((T_DrawClient.体温区域.Right + IIf(gbln呼吸曲线 = True, T_BodyStyle.lngBreashLabelStep, 0) - (objDraw.Width / T_TwipsPerPixel.X - lngRight)) / (T_DrawClient.体温区域.Right - T_DrawClient.刻度区域.Right), 2)
    ...
    For lngPage = 1 To lngCountPage
        ...
        If rsTmp.BOF = False Then
            ...
            If zlCommFun.Nvl(rsTmp("病区").Value) <> strTmp1 And zlCommFun.Nvl(rsTmp("病区").Value) <> "" Then
                    strTmp1 = zlCommFun.Nvl(rsTmp("病区").Value)
        ...
        lngOldFont = SelectObject(lngDc, lngFont)
        '输出病人信息
        Call DrawPatiInfo(lngDc, objDraw, strPatiInfo, lngCurX, lngCurY, T_DrawClient.体温区域.Right + IIf(gbln呼吸曲线 = True, T_BodyStyle.lngBreashLabelStep, 0), lngCurY, intBaby & "'" & strBabyDay, sngScale)
        Call SelectObject(lngDc, lngOldFont)
        Call DeleteObject(lngFont)
        Call ReleaseFontIndirect(objDraw)
        ...
        '此处的计算请和DrawBodyPageFooterNew保持一致
         sinY = 0
         If (Val(zlDatabase.GetPara("体温单不打印曲线说明", glngSys, 1255, "0")) = 1) = False And gbln呼吸曲线 = False Then
            sinY = 14 * msngTwips
         Else
            sinY = 6 * msngTwips
         End If
...
'
        ...
        rsTemp.Sort = "排列序号"
        rsTemp.MoveFirst
        str体温说明 = DrawCanvasNew(lngDc, objDraw, rsTemp, rsDrawItems, bln不打印心率列, str心率信息, sngScale)
        Call SelectObject(lngDc, lngOldFont)
        Call DeleteObject(lngFont)
        Call ReleaseFontIndirect(objDraw)
...
'        '处理体温不升 体温为不升需要在35度下纵向输出体温不升二字
        ...
        If Not gbln呼吸曲线 Then strItems = strItems & ",'呼吸'"
...
Public Function DrawCanvasNew(ByVal lngDc As Long, ByVal objDraw As Object, ByVal rsTemp As ADODB.Recordset, rsDrawItems As ADODB.Recordset, _
    Optional ByVal bln不打印心率列 As Boolean = False, Optional ByVal str心率信息 As String = "", Optional sngScale As Single = 1) As String
'------------------------------------------------------------------------------------------------------
...
'       str心率信息=""心率禁用，不为空格式为:int心率应用 & "'" & 项目名称 & "'" & str心率符号 & "'" & lng心率颜色
'出参:返回各个曲线的具体信息包括( "项目序号|最大值|最小值|单位值|最大值坐标|最小值坐标|单位刻度|显示模式|颜色")
'返回说明信息(项目的符号)
'-------------------------------------------------------------------------------------------------------
    Dim str说明 As String
    Dim i As Integer, strTmp As String, ArrCode, int心率应用 As Integer
    ...
    For i = 1 To mintNullRow
        If mintNullRow > 1 Then
            Call DrawLine(lngDc, T_DrawClient.偏移量X + lng刻度宽度, lngCurY + (lngInitRowStep * 2 * i), lngMaxX, lngCurY + (lngInitRowStep * 2 * i), PS_SOLID, intFine, RGB_BLACK)
        Else
            Call DrawLine(lngDc, T_DrawClient.偏移量X, lngCurY + (lngInitRowStep * 2 * i), lngMaxX, lngCurY + (lngInitRowStep * 2 * i), PS_SOLID, intFine, RGB_BLACK)
        End If
    Next i
    ...
    For lngRow = 1 To T_BodyStyle.lng监测次数 * T_BodyStyle.lng天数
        ...
        Call DrawLine(lngDc, lngCurX, lngCurY, lngCurX, lngMaxY, PS_SOLID, IIf(lngRow Mod T_BodyStyle.lng监测次数 = 0, intBold, intFine), IIf(lngRow Mod T_BodyStyle.lng监测次数 = 0 And lngRow < T_BodyStyle.lng监测次数 * T_BodyStyle.lng天数, RGB_RED, RGB_BLACK))
    Next
    ...
    '                Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), Trim(zlCommFun.Nvl(!单位, 0)), zlCommFun.Nvl(!记录色, RGB_BLACK))
            ...
            If !项目序号 = gint体温 Then
                ...
                mobjPart.Add Array("" & !项目序号, "耳温", arrTemp(3), Nvl(!记录色, RGB_BLACK), "D"), "D" & !项目序号
            ElseIf !项目序号 = gint脉搏 And int心率应用 = 2 Then
            ...
            Do While True
                ...
                If bln显示刻度 And Not (bln不打印心率列 = True And !项目序号 = gint心率) Then
                    '控制最大值不与曲线单位重复
                    If !项目序号 = gint呼吸 And sin刻度 = Val(Nvl(!最大值, 0)) Then
                        strTmp = Trim(Nvl(!记录名))
                        lngY = lngCurY - Len(strTmp) / 2 * (objDraw.TextHeight("刘") / T_TwipsPerPixel.Y)
                        ...
                        For i = 1 To Len(strTmp)
                            ...
                            lngY = lngY + objDraw.TextHeight("呼") / T_TwipsPerPixel.Y
                        Next i
                    Else
                        If sin刻度 = Val(Nvl(!最大值, 0)) And lngCurY < sinY单位 Then
                            Call GetTextRect(objDraw, lngCurX, sinY单位, Format(sin刻度, "#0"), IIf(!项目序号 = gint呼吸, 0, lngLableStep), , , sngScale)
                        ElseIf lngCurY = T_DrawClient.刻度区域.Bottom Then
                            Call GetTextRect(objDraw, lngCurX, lngCurY - (objDraw.TextHeight("1") / 2 / T_TwipsPerPixel.Y), Format(sin刻度, "#0"), IIf(!项目序号 = gint呼吸, 0, lngLableStep), , , sngScale)
                        Else
                            Call GetTextRect(objDraw, lngCurX, lngCurY, Format(sin刻度, "#0"), IIf(!项目序号 = gint呼吸, 0, lngLableStep), , , sngScale)
                        End If
...
'                        Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), Val(Format(sin刻度, "#0.0")), Nvl(!记录色, RGB_BLACK))
    ...
    '呼吸为曲线再画外线
    If gbln呼吸曲线 = True Then
        Call DrawLine(lngDc, T_DrawClient.体温区域.Right + T_BodyStyle.lngBreashLabelStep, T_DrawClient.体温区域.Top, T_DrawClient.体温区域.Right + T_BodyStyle.lngBreashLabelStep, T_DrawClient.曲线总区域.Bottom, PS_SOLID, intBold, RGB_BLACK)
    End If
...
Private Sub DrawBodyPageFooterNew(ByVal lngDc As Long, objDraw As Object, X As Long, Y As Long, ByVal LeftX As Long, ByVal intPageNo As Integer, _
    ...
    If blnPrintCurveInfo = False And gbln呼吸曲线 = False Then
        '打印体温说明信息
        Call SetTextColor(lngDc, RGB_BLACK)
...
Private Function getSQLString(ByVal strText As String, ByVal blnMoved As Boolean, Optional ByVal strItems As String) As String
    ...
    Select Case strText
        ...
        Case "提取所有曲线项目"
            ...
            strSQL = " Select /*+ RULE */ A.项目序号,A.排列序号,A.记录名,C.项目值域,A.记录法,A.记录符,A.记录色,nvl(A.最大值,0) 最大值 ,nvl(A.最小值,0) 最小值,A.临界值," & _
                "nvl(A.单位值,0) 单位值,A.刻度间隔,A.警示线,C.项目单位 单位,Decode(记录法,3,A.最高行,nvl(A.最高行,2)-2) AS 最高行,B.部位,DECODE(A.项目序号,3,1,0) 序号" & _
...
'在下表格左侧区域输入曲线部位符号信息
    ...
    If gbln呼吸曲线 = False Or mobjPart Is Nothing Then Exit Sub
    '将体温移动到前面
    For j = 0 To 1
    ...
    '计算行间隔
    lngLeading = (lngHeight - objHeat.Count * (objDraw.TextHeight("刘") / T_TwipsPerPixel.Y)) \ objHeat.Count
    If lngLeading > Format(objDraw.TextHeight("刘") / T_TwipsPerPixel.Y, "#0") Then
        lngLeading = Format(objDraw.TextHeight("刘") / T_TwipsPerPixel.Y, "#0")
    End If
    ...
    For i = 1 To objHeat.Count
        ...
        LPoint.Y = LPoint.Y + Format(objDraw.TextHeight("刘") / T_TwipsPerPixel.Y, "#0") + lngLeading
    Next i
