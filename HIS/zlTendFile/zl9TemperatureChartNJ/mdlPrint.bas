...
Public Type type_BodyStyle
    ...
    lng表格空行 As Long
    lng下表格高度 As Long
    lngBreashLabelStep  As Long '呼吸曲线刻度列宽
    bln专科 As Boolean
End Type
...
Public Sub DrawPatiInfo(ByVal lngDc As Long, ByVal objDraw As Object, ByVal strPatiInfo As String, ByVal lngX As Long, ByVal lngY As Long, _
    ByVal lngLeft As Long, lngOutY As Long, Optional ByVal strBaby As String = "", Optional ByVal sngScale As Single = 1)
'-----------------------------------------------------------------------------------------------------------------------
'输出病人基本信息
...
'-----------------------------------------------------------------------------------------------------------------------
    ...
    If Val(Left(strBaby, 1)) <> 0 And InStr(1, strBaby, "'") <> 0 Then
        strPatiName = "姓名:'年龄:'性别:'出生日期:''母亲床号:'母亲住院号:" & IIf(bln是否输出诊断 = True, "'诊断:", "")
        VarPatiInfo(3) = Format(Split(strBaby, "'")(1), "YYYY-MM-DD")
        VarPatiInfo(4) = ""
    Else
        strPatiName = "姓名:'年龄:'性别:'科别:'病区:'床号:'住院号:" & IIf(bln是否输出诊断 = True, "'诊断:", "")
    End If
