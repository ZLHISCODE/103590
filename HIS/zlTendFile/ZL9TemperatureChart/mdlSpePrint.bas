...
ZeroLine:
        ...
        Call DrawBodyPageFooterNew(lngDC, objDraw, lngCurX, lngCurY, T_DrawClient.体温区域.Right, intPageNo, intBeginPage, lngButtom, sngScale)
        '页脚图形输出
...
'-----------------------------------------------------------------------------------------------------------------------
    ...
    Set objDraw.Font = GetDrawFont(objDraw, str日期, T_DrawClient.刻度区域.Right - T_DrawClient.刻度区域.Left, False)
    ...
    Set objDraw.Font = GetDrawFont(objDraw, str住院天数, T_DrawClient.刻度区域.Right - T_DrawClient.刻度区域.Left, False)
    ...
    Set objDraw.Font = GetDrawFont(objDraw, str手术后天数, T_DrawClient.刻度区域.Right - T_DrawClient.刻度区域.Left, False)
    ...
    Set objDraw.Font = GetDrawFont(objDraw, str时间, T_DrawClient.刻度区域.Right - T_DrawClient.刻度区域.Left, False)
...
'                Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), Trim(zlCommFun.Nvl(!单位, 0)), zlCommFun.Nvl(!记录色, RGB_BLACK))
            ...
            If !项目序号 = gint体温 Then
                ...
                If UBound(arrTemp) > 2 Then mobjPart.Add Array("" & !项目序号, Nvl(!记录名), "耳温", arrTemp(3), Nvl(!记录色, RGB_BLACK), "D"), "D" & !项目序号
                If UBound(arrTemp) > 3 Then mobjPart.Add Array("" & !项目序号, Nvl(!记录名), "额温", arrTemp(4), Nvl(!记录色, RGB_BLACK), "E"), "E" & !项目序号
    ...
    With rsPoint
        Do While Not .EOF
            ...
            If zlCommFun.Nvl(!重叠) = 0 And zlCommFun.Nvl(!重叠项目) = "空" Then '未重叠的项目
                ...
                Select Case zlCommFun.Nvl(!项目序号)
                    Case gint体温
                        strTmp = strTmp & String(4 - UBound(Split(strTmp, ",")), ",")
                        If str部位 = "口温" Then
                        ...
                        ElseIf str部位 = "耳温" Then
                            strChar = Split(strTmp, ",")(3)
                        Else
                            strChar = Split(strTmp, ",")(4)
                        End If
    ...
    Select Case strText
        ...
        Case "显示皮试结果"
            ...
            strSQL = _
                ...
                " From (Select To_Char(decode(a.标本部位,null,a.开始执行时间,to_date(a.标本部位,'yyyy-mm-dd hh24:mi:ss')), 'YYYY-MM-DD') 时间," & vbNewLine & _
                ...
                "             a.病人id = [1] And a.主页id = [2] And a.婴儿 = [3] And Decode(a.标本部位, Null, a.开始执行时间, To_Date(a.标本部位, 'yyyy-mm-dd hh24:mi:ss')) Between [4] And [5]" & vbNewLine & _
                "       Order By Decode(a.标本部位, Null, a.开始执行时间, To_Date(a.标本部位, 'yyyy-mm-dd hh24:mi:ss')), a.皮试结果)" & vbNewLine & _
                " Group By 时间"
            strSQLText = strSQL
        Case "提取科室床号"
