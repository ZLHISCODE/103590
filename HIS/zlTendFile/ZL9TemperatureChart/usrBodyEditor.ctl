...
Private Sub RaiseShowTipInfo(ByVal vfgObj As Object, ByVal intType As Byte, ByVal X As Single, ByVal Y As Single)
    ...
    ElseIf lngRow >= 0 And lngCol >= 0 And lngRow = vfgObj.Rows - IIf(intType = 1, 1, 0) And lngCol < vfgObj.Cols Then
        Call ShowUpTabInfo(X, Y)
    Else
        RaiseEvent ShowTipInfo(vfgObj, "", True)
    End If
End Sub

Private Sub ShowUpTabInfo(ByVal sgnX As Single, ByVal sgnY As Single)
     ':陈刘,光标移动到对应的时间，显示数据信息
    Dim sgnTmp As Single
    ...
    sgnX = picDraw.ScaleX(sgnX, vbTwips, vbPixels)
    sgnX = sgnX + mshUpTab.Left / T_TwipsPerPixel.X - 1
    If sgnX >= T_DrawClient.体温区域.Left And sgnX <= T_DrawClient.曲线总区域.Right Then
        ...
        RaiseEvent ShowTipInfo(mshUpTab, strInfo, True)
    Else
        RaiseEvent ShowTipInfo(mshUpTab, "", False)
    End If
End Sub
    ...
    With rsTemp
        Do While Not .EOF
            ...
            If !项目序号 = gint体温 Then
                ...
                If UBound(arrTemp) > 3 Then mobjPart.Add Array("" & !项目序号, Nvl(!记录名), "额温", arrTemp(3), Nvl(!记录色, RGB_BLACK), "E"), "E" & !项目序号
    ...
    With rsTemp
        Do While Not .EOF
            If !展现方式 = 1 Then
                ...
                If !项目序号 = 1 Then '体温
                    ArrCode = Split("口温,腋温,肛温,耳温,额温", ",")
                    strChar = zlCommFun.Nvl(!标记符号, "·,×,○,△,★")
                    strChar = strChar & String(4 - UBound(Split(strChar, ",")), ",")
                    arrChar = Split(strChar, ",")
                    For i = 0 To UBound(ArrCode)
                        gstrFields = "项目序号|部位|重叠项目|记录符|记录色"
                        gstrValues = !项目序号 & "|" & ArrCode(i) & "|" & zlCommFun.Nvl(!重叠项目) & "|" & IIf(arrChar(i) = "", Split("·,×,○,△,★", ",")(i), arrChar(i)) & "|" & zlCommFun.Nvl(!标记颜色, 0)
...
'            Printer.EndDoc
       ...
       ElseIf blnPrint = True Then Printer.Print ""
