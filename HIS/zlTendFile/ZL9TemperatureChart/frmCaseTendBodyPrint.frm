...
'    Set mobjParent = objParent
    ...
    If mlngEndIndex - mlngStartIndex >= 0 Then Call ShowPage(mintCurPage, msngScale)
...
Private Function InitMenuBar() As Boolean
    ...
    With cbrToolBar.Controls
        ...
        For intLoop = 0 To mlngEndIndex - mlngStartIndex
...
Private Sub ScalePicture(ByVal sngScale As Single)
    ...
    For intCOl = mlngStartIndex To mlngEndIndex
        ...
        Set obj = picPage(0).Image
        
        picPage(0).Width = picPage(0).Width * sngScale
        picPage(0).Height = picPage(0).Height * sngScale
        
        X = picPage(0).Width
        Y = picPage(0).Height
        
        picPage(0).Cls
        picPage(0).PaintPicture obj, 0, 0, X, Y
...
'    If PageNum = 0 Then Exit Function
    ...
    If PrintOrPreviewBodyStateNew(Me, mlngPatiId, mlngPageId, mlng文件ID, mlng婴儿, _
                    mlng科室ID, mlngBeginY, gPrinter.lngLeft, Me, PageNum <> 0, _
                    PageNum + 1, PageNum + 1, mintPageNo, , False) = True Then
    Else
        GoTo Errhand
    End If
    
    picShow.Width = picPage(0).Width * sngScale
    picShow.Height = picPage(0).Height * sngScale
    picShow.Cls
    picBack.Cls
    
    '采用半色调缩放效果最好！
    SetStretchBltMode picShow.hDC, HALFTONE
    StretchBlt picShow.hDC, 0, 0, picShow.Width, picShow.Height, picPage(0).hDC, 0, 0, picPage(0).Width, picPage(0).Height, SRCCOPY
...
Private Sub cbsThis_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_View_Navigatebeginning
        ...
        mintCurPage = mlngStartIndex
        mobjPage.ListIndex = mintCurPage + 1 - mlngStartIndex
        Call ShowPage(mintCurPage, msngScale)
    ...
    Case conMenu_View_Navigateright
        ...
        If mintCurPage + 1 <= mlngEndIndex Then
            mintCurPage = mintCurPage + 1
            mobjPage.ListIndex = mintCurPage + 1 - mlngStartIndex
            Call ShowPage(mintCurPage, msngScale)
        End If
    ...
    Case conMenu_View_Navigateend
        ...
        If mintCurPage <> mlngEndIndex Then
            mintCurPage = mlngEndIndex
            mobjPage.ListIndex = mintCurPage + 1 - mlngStartIndex
            Call ShowPage(mintCurPage, msngScale)
        End If
    Case 6
        ...
        If Val(Control.Text) > 0 Then
            ...
            End If
            mintCurPage = Val(Control.ListIndex - 1) + mlngStartIndex
        Else
            mintCurPage = Val(Control.ListIndex - 1) + mlngStartIndex
        End If
...
Private Sub cbsThis_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_View_Navigatebeginning
        ...
        Control.Enabled = (mlngEndIndex - mlngStartIndex) > 0 And mintCurPage - mlngStartIndex <> 0
    ...
    Case conMenu_View_Navigateright
        ...
        Control.Enabled = (mlngEndIndex - mlngStartIndex) > 0 And mintCurPage < mlngEndIndex
...
Private Sub picPage_MouseUp(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Button = 1 Then Set picPage(0).MouseIcon = scrVsc.MouseIcon
End Sub
...
Private Sub Form_Load()
    ...
    picPage.Item(0).AutoRedraw = True
    mintCurPage = mlngStartIndex
