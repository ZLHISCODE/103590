...
Begin VB.UserControl UserBodyMutilEditor 
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsfPrint 
      ...
      AccessibleRole  =   24
   End
   Begin VB.PictureBox picInputStyle 
      BorderStyle     =   0  'None
      Height          =   345
      Left            =   14265
      ScaleHeight     =   345
...
Private mcbrFilter As CommandBar
...
Private Sub ReSetFontSize()
    ...
    lblDate.Left = cboFormat.Left + cboFormat.Width + TextWidth("刘") / 2
    dtpDate.Left = lblDate.Left + lblDate.Width + TextWidth("刘") / 2
    lblUnit.Left = dtpDate.Left + dtpDate.Width + TextWidth("刘") / 2
    cboUnit.Left = lblUnit.Left + lblUnit.Width + TextWidth("刘") / 2
    lblFilter.Left = cboUnit.Left + cboUnit.Width + TextWidth("刘") / 2
    ...
    lblPati.Left = txtFilter.Left + txtFilter.Width + TextWidth("刘") / 2
    ...
    cmdAddUser.Left = cmdFilter.Left + cmdFilter.Width + TextWidth("刘") / 2
    ...
    picInputStyle.Width = optLevel(1).Left + optLevel(1).Width + TextWidth("刘") / 2
    ...
    With cbrToolBar.Controls
        Set cbrCustom = .Add(xtpControlCustom, conMenu_View_LocationItem * 10 + 1, "")
        cbrCustom.flags = xtpFlagAlignLeft
        cbrCustom.Handle = pic过滤条件.hWnd
        cbrCustom.ToolTipText = "条件"
        
        Set cbrCustom = .Add(xtpControlCustom, conMenu_View_LocationItem * 10 + 2, "")
        cbrCustom.flags = xtpFlagAlignLeft
        cbrCustom.Handle = picInputStyle.hWnd
        cbrCustom.ToolTipText = "录入方式"
    End With
    For Each objControls In mcbrFilter.Controls
        objControls.Delete
    Next
    mcbrFilter.Delete
    Set mcbrFilter = cbrToolBar
...
'                            If strContent <> "" Then strContent = IIf(strPart = "", "", strPart & ":") & strContent
                ...
                If strContent <> "" Then
                    ...
                    vsfHistory.TextMatrix(lngHistoryRow, lngHistoryCol) = IIf(strPart = "", "", strPart & ":") & strContent
...
Private Function InitMenuBar() As Boolean
    ...
    With cbrToolBar.Controls
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_Edit_Modify, "添加项目"): cbrControl.ToolTipText = "添加活动项目": cbrControl.BeginGroup = True
...
Private Sub AddActiveMenu(ByVal lngItemNO As Long)
    ...
    If lngItemNO = gint体温 Then '体温
        ...
        If UBound(varTmp) > 2 Then Set cbrControl = mcbrMenuBar部位.CommandBar.Controls.Add(xtpControlButton, conMenu_Edit_Compend * 10, "耳温" & varTmp(3) & " (&4)", -1, False): cbrControl.Parameter = "耳温": cbrControl.IconId = 1
        If UBound(varTmp) > 3 Then Set cbrControl = mcbrMenuBar部位.CommandBar.Controls.Add(xtpControlButton, conMenu_Edit_Compend * 10, "额温" & varTmp(4) & " (&5)", -1, False): cbrControl.Parameter = "额温": cbrControl.IconId = 1
...
Private Function InitEnv() As Boolean
    ...
    dtpDate.MaxDate = Format(DateAdd("d", mintPreDays, date), "YYYY-MM-DD")
    ...
    With cboFormat
        ...
        .Tag = ""
        Do While Not RS.EOF
            If lngWidth < UserControl.TextWidth(Nvl(RS!名称)) + 60 Then
                lngWidth = UserControl.TextWidth(Nvl(RS!名称)) + 60
            End If
            .AddItem Nvl(RS!名称)
        ...
        Else
             ...
             Exit Function
        End If
        Call zlControl.CboSetWidth(cboFormat.hWnd, lngWidth)
    End With
...
Private Sub LoadPatient(ByVal lngFormatID As Long)
    ...
    If chkScope(待入科).Value = 1 Then
        strSQL = _
            ...
            "Where B.病人类型=Z.名称(+) And A.在院=1 And a.病人id = b.病人id  And A.主页ID=B.主页ID And Nvl(b.主页id, 0) <> 0 And b.病人id = c.病人id And b.主页id = c.主页id And c.科室id = d.Id" & vbNewLine & _
    ...
    Select Case Control.Id
        ...
        Case conMenu_Edit_Append * 10, conMenu_Edit_Append * 10 + 1, conMenu_Edit_Append * 10 + 2, conMenu_Edit_Append * 10 + 30, conMenu_Edit_Append * 10 + 31, conMenu_Edit_Append * 10 + 4, conMenu_Edit_Append * 10 + 5, conMenu_Edit_Append * 10 + 6
            ...
            If mintType = 1 And picInput.Visible = True Then
                ...
                For Each cbrCheck In mcbrToolBar.Controls(6).CommandBar.Controls
...
'    If mrsSelItems.Records.Count = 0 Then Exit Sub
    ...
    objPrint.Title.Text = cboFormat.Text
...
'---------------------------------------
    ...
    If mrsCopy.RecordCount > 0 Then
        Do While Not mrsCopy.EOF
            ...
            If InStr(1, ",0,3,9", "," & mrsCopy!数据来源 & ",") <> 0 Then
...
Private Sub cmdAddUser_Click()
    ...
    If picPati.Left + picPati.Width > UserControl.Width Then
        picPati.Left = UserControl.Width - picPati.Width
    End If
    If picPati.Left < 0 Then picPati.Left = 0
...
'-----------------------------------------------------------------------------------------------------------------
    ...
    '82659,选择的日期如果是当天或者最大超期录入那一天
    strCurDate = Format(zlDatabase.Currentdate, "YYYY-MM-DD HH:mm:ss")
    strTime = Format(strCurDate, "HH:mm:ss")
    If Format(mstrDate, "YYYY-MM-DD") = Format(strCurDate, "YYYY-MM-DD") Or Format(mstrDate, "YYYY-MM-DD") = Format(dtpDate.MaxDate, "YYYY-MM-DD") Then
        strEnd = Format(Format(CDate(mstrDate), "YYYY-MM-DD") & " " & strTime, "YYYY-MM-DD HH:mm:ss")
        strBegin = Format(DateAdd("s", 1, DateAdd("d", "-3", CDate(strEnd))), "YYYY-MM-DD HH:mm:ss")
    Else
    ...
    Select Case mintPatiNo
        ...
        Case Else
            ...
            strPatient = strPatient & _
                  ...
                  " Where A.病人ID=B.病人ID And A.主页ID=B.主页ID"
    End Select
    strTime = Format(strEnd, "YYYY-MM-DD HH:mm:ss")
...
Private Sub txtInput_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyDown Then '显示未记说明信息
        ...
        If InStr(1, ",0,3,9,", "," & mint数据来源 & ",") = 0 Then Exit Sub
...
Private Sub UserControl_Terminate()
    ...
    If Not (mcbrMenuBar部位 Is Nothing) Then Set mcbrMenuBar部位 = Nothing
    If Not (mcbrToolBar Is Nothing) Then Set mcbrToolBar = Nothing
    If Not (mcbrFilter Is Nothing) Then Set mcbrFilter = Nothing
End Sub
...
Private Sub VsfData_AfterRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long)
    ...
    If mrsItems.RecordCount <> 0 Then
        ...
        ElseIf lngItemNO = gint脉搏 And mint心率应用 = 2 Then
            If mbln脉搏共用显示 Then
                strInfo = strInfo & Space(4) & "脉搏短绌表示法(心率/脉搏)100/130"
            Else
                strInfo = strInfo & Space(4) & "脉搏短绌表示法(脉搏/心率)100/130"
            End If
        ...
        Else
            ...
            strText = Trim(VsfData.TextMatrix(NewRow, NewCol))
            blnCheck = False
            For Each cbrControl In mcbrToolBar.Controls(6).CommandBar.Controls
                cbrControl.Checked = False
                If lngItemNO = gint大便 Then
                    Select Case cbrControl.Id
                        ...
                        Case conMenu_Edit_Append * 10 + 3
                            ...
                            For Each cbrChild In cbrControl.CommandBar.Controls
                                Select Case cbrChild.Id
                                    Case conMenu_Edit_Append * 10 + 30
                                        cbrChild.Checked = (UCase(strText) = "※")
                                    Case conMenu_Edit_Append * 10 + 31
                                        cbrChild.Checked = (UCase(strText) = "*")
                                End Select
                            Next
            ...
            If blnCheck = False Then
                 mcbrToolBar.Controls(6).CommandBar.Controls(2).Checked = True
            End If
        End If
    End If
...
Private Sub VsfData_EnterCell()
    ...
    If VsfData.Col = c时间 Then
        ...
        Do While Not mrsCopy.EOF
            ...
            If InStr(1, ",0,3,9,", "," & mint数据来源 & ",") = 0 Then
    ...
    If rsObj.RecordCount > 0 Then
        ...
        If InStr(1, ",0,3,9,", "," & Val(rsObj!数据来源) & ",") = 0 Then
...
'    Next i
    ...
    With mrsCell
        Do While Not .EOF
            If InStr(1, ",0,3,9,", "," & Val(Nvl(mrsCell!数据来源)) & ",") = 0 Then
                strKey = Nvl(mrsCell!Id, ",")
            ...
            Do While Not .EOF
                If lngItemNO <> 4 Then
                    ...
                    If InStr(1, "," & gint体温 & "," & gint脉搏 & "," & gint心率 & "," & gint疼痛强度 & ",", "," & Val(Nvl(!项目序号)) & ",") > 0 Then
                        ...
                        If blnAllow = True Then
                            If Val(Nvl(!记录标记)) = 0 And InStr(1, ",0,3,9,", "," & Val(Nvl(!数据来源)) & ",") = 0 Then
                            ...
                            Else '组装物理降温和脉搏短轴
                                If bln心率单独 = False Then
                                    ...
                                    If InStr(1, ",0,3,9,", "," & Val(Nvl(!数据来源)) & ",") = 0 Then
                                        intModify = 1
                                    End If
                        ...
                        End If
                    Else
                        If Val(Nvl(!项目序号)) = lngItemNO And InStr(1, ",0,3,9,", "," & Val(Nvl(!数据来源)) & ",") = 0 Then
                            strPart = Nvl(!体温部位)
                            lng来源ID = Val(Nvl(!来源ID))
                            int共用 = Val(Nvl(!共用))
                            int显示 = Val(Nvl(!显示))
...
'intType=1 为大便项目 否则为入液项目
    ...
    If rsObj.RecordCount > 0 Then
        If InStr(1, ",0,3,9,", "," & Val(rsObj!数据来源) & ",") = 0 Then Exit Function
    End If
