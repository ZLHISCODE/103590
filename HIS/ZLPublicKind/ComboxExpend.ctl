...
Event NodeCheckValied(ByVal Node As Node, blnCancel As Boolean)
...
Private mblnCancel As Boolean, mblnCheck As Boolean '主要是取消
...
Public Sub AddItem(ByVal strID As String, ByVal strCatpion As String, _
    ...
    Optional ByVal lngForeColor As Long = -1, Optional strTag As String)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:增加子项
    '编制:刘兴洪
...
Private Sub tvwList_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    Dim objNode As Node
    If Not mblnCancel Then Exit Sub
    Set objNode = tvwList.HitTest(X, Y)
    If objNode Is Nothing Then Exit Sub
    Call NodeCheck(objNode)
    
    'objNode.Checked = Not mblnCheck
...
Private Sub tvwList_NodeCheck(ByVal Node As MSComctlLib.Node)
    ...
    Dim blnFind As Boolean, blnAllSel As Boolean, blnCancel As Boolean
    ...
    mblnNotChange = True: mblnCancel = False
    RaiseEvent NodeCheckValied(Node, blnCancel)
    If blnCancel Then
        mblnCancel = True: mblnCheck = Node.Checked
        mblnNotChange = False
        Exit Sub
    End If
    ...
    If mblnAddItemAddNode Then
        ...
        Else
            ...
            Else
                blnAllSel = True
                For Each objNode In tvwList.Nodes
                    If objNode.Key <> Node.Key And objNode.Key <> "Root" And objNode.Checked = False Then
                        blnAllSel = False
                         Exit For
                    End If
                Next
                tvwList.Nodes("Root").Checked = blnAllSel
             End If
        End If
    End If
...
Private Sub NodeCheck(ByRef objNode As Node)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:勾选操作
    '编制:刘兴洪
    '日期:2017-11-13 15:39:56
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim objNodeTmp As Node, strCaption As String
    Dim blnFind As Boolean, blnAllSel As Boolean, blnCancel As Boolean
    ...
    '锁住控件，不允许更改,避免重复操作
    tvwList.Enabled = False
    
    RaiseEvent NodeCheckValied(objNode, blnCancel)
    If blnCancel Then
        objNode.Checked = Not objNode.Checked
        mblnNotChange = False
        tvwList.Enabled = True
        Exit Sub
    End If
    ...
    If mblnAddItemAddNode Then
        If objNode.Key = "Root" Then
            ...
            For Each objNodeTmp In tvwList.Nodes
                If objNodeTmp.Key <> objNode.Key Then
                    objNodeTmp.Checked = objNode.Checked
                End If
            Next
            If objNode.Checked = False Then
                For Each objNodeTmp In tvwList.Nodes
                    If objNodeTmp.Key <> objNode.Key Then
                        objNodeTmp.Checked = True
                        Exit For
                    End If
                Next
            End If
        Else
            ...
            Else
                blnAllSel = True
                For Each objNodeTmp In tvwList.Nodes
                    If objNodeTmp.Key <> objNode.Key And objNodeTmp.Key <> "Root" And objNodeTmp.Checked = False Then
                        blnAllSel = False
                         Exit For
                    End If
                Next
    ...
    RaiseEvent NodeCheck(objNode, strCaption)
    If mblnAddItemAddNode Then
        Call RefreshTextData
    Else
        txtInput.Text = strCaption
