...
Private Sub DebugTools(ByVal strInfo As String)
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Call WritLog("一卡通接口调试日志", "读卡接口返回(" & glngModul & ")", strInfo)
End Sub
...
Public Function zlXML_ExistNode(ByVal strXmlIn As String, ByVal strNodeName As String) As Boolean
    ...
    If zlXML_LoadXMLToDOMDocument(strXmlIn, False) = False Then Exit Function
    
    Set xmlElement = gobjXml.getElementsByTagName(strNodeName).Item(0)
    If xmlElement Is Nothing Then Exit Function
    
    Set xmlElement = Nothing
    zlXML_ExistNode = True
    Exit Function
ErrHandler:
