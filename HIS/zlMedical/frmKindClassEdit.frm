VERSION 5.00
Begin VB.Form frmKindClassEdit 
   ...
   ClientTop       =   3750
   ClientWidth     =   5805
   Icon            =   "frmKindClassEdit.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   End
End
Attribute VB_Name = "frmKindClassEdit"
...
Private Function AnalyzeCode(ByVal lngKey As Long, ByRef objTxtParent As Object, ByRef objTxt As Object) As Boolean
    ...
    strSQL = "SELECT B.编码 AS 上级编码,A.编码 FROM 体检类型 A,体检类型 B WHERE A.上级序号=B.序号(+) AND A.序号=[1]"
...
Private Function NewDefaultCode(ByVal lngUpKey As Long, ByRef objTxtParent As Object, ByRef objTxt As Object, ByRef objChk As Object) As Boolean
    ...
    '读取同级最大编码+1
    If lngUpKey = 0 Then
        strSQL = "SELECT MAX(B.编码) AS 最大编码 FROM 体检类型 B WHERE B.末级=0 AND B.上级序号 IS NULL "
        Set rs = zlDatabase.OpenSQLRecord(strSQL, Me.Caption)
    Else
        strSQL = "SELECT MAX(B.编码) AS 最大编码 FROM 体检类型 B WHERE B.末级=0 AND B.上级序号=[1]"
        Set rs = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lngUpKey)
    End If
...
Private Function InitData() As Boolean
    ...
    txtParentCode.MaxLength = GetMaxLength("体检类型", "编码")
...
Private Function SaveEdit(ByRef lngKey As Long) As Boolean
    ...
    If mlngKey = 0 Then
        ...
        lngKey = GetMaxNo
        strSQL(ReDimArray(strSQL)) = "ZL_体检类型_INSERT(" & lngKey & ",'" & Trim(txtParentCode.Text & txt(0).Text) & "','" & txt(1).Text & "',NULL,NULL," & Val(cmd(0).Tag) & ",0,0," & chk.Value & ")"
    Else
        '修改类型
        lngKey = mlngKey
        strSQL(ReDimArray(strSQL)) = "ZL_体检类型_UPDATE(" & lngKey & ",'" & Trim(txtParentCode.Text & txt(0).Text) & "','" & txt(1).Text & "',NULL,NULL," & Val(cmd(0).Tag) & ",0," & chk.Value & ")"
    End If
...
Private Sub cmd_Click(Index As Integer)
    ...
    If frmSelectDialog.ShowSelect(Me, 1, rs, "", "请从下面选择一个分类", objPoint.X * 15 - 30, objPoint.Y * 15 + txt(2).Height - 30, txt(2).Width, 3900, txt(2).Height, mlngKey, Me.Name & "\体检类型分类选择", , False) Then
...
Private Sub cmdOK_Click()
    ...
    If cmdOK.Tag <> "" Then
        ...
        Call mfrmMain.EditRefresh("体检类型分类", lngKey)
