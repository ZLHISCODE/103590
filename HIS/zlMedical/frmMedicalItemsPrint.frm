...
Private Function ReadData() As Boolean
    ...
    gstrSQL = "Select a.ID,a.编码,a.名称,A.计算单位 As 单位,DECODE(A.类别,'C','检验','检查') AS 类别,b.排列顺序 As 份数 " & _
                    "From 诊疗项目目录 a,体检项目排列 b where b.诊疗项目id=a.ID and 排列性质=2  "
    ...
    If rs.BOF = False Then
        Do While Not rs.EOF
            ...
            vsf.TextMatrix(vsf.Rows - 1, mCol.份数) = zlCommFun.NVL(rs("份数"))
...
Private Function SaveEdit() As Boolean
    ...
    strSQL(ReDimArray(strSQL)) = "ZL_体检项目排列_DELETE(2)"
    
    For lngLoop = 1 To vsf.Rows - 1
        If Val(vsf.RowData(lngLoop)) > 0 And Val(vsf.TextMatrix(lngLoop, mCol.份数)) > 1 Then
            
            gstrSQL = "ZL_体检项目排列_INSERT("
            gstrSQL = gstrSQL & Val(vsf.RowData(lngLoop)) & ","
            gstrSQL = gstrSQL & Val(vsf.TextMatrix(lngLoop, mCol.份数)) & ",2)"
