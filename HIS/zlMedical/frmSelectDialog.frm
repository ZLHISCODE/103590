...
Private Sub ReadListData()
    ...
    With lvw
        ...
        Do While Not mrsData.EOF
            ...
            If .Checkboxes And blnSelect Then
                objItem.Checked = (NVL(mrsData("选择").Value, 0) = 1)
            End If
...
Public Function ShowSelect(ByVal frmMain As Form, _
                            ...
                            Optional ByVal MuliSelect As Boolean = False) As Boolean
...
Private Function InitData() As Boolean
    ...
    If mbytWinStyle <> 3 Then
        If stb.Panels.Count >= 2 Then stb.Panels.Remove 2
    End If
    
    lblCaption.Caption = mstrTitle
    cmdOK.Visible = mblnMuliSelect
    cmd(1).Visible = cmdOK.Visible
    cmd(2).Visible = cmdOK.Visible
...
Private Sub cmd_Click(Index As Integer)
    ...
    Case 1
        For intLoop = 1 To lvw.ListItems.Count
            lvw.ListItems(intLoop).Checked = True
        Next
    Case 2
        For intLoop = 1 To lvw.ListItems.Count
            lvw.ListItems(intLoop).Checked = False
        Next
    End Select
End Sub
    ...
    Else
        If mblnLeftSelect = False Then
            If Not (lvw.SelectedItem Is Nothing) Then
                ...
                Else
                    mrsData.Filter = "ID=" & Val(Mid(lvw.SelectedItem.Key, 2))
                End If
...
Private Sub Form_Resize()
    ...
    With cmdOK
        .Left = chk.Left - .Width - 300
        .Top = stb.Top + 30 + (stb.Height - .Height) / 2
    End With
    
    cmd(2).Move cmdOK.Left - cmd(2).Width - 45, cmdOK.Top
    cmd(1).Move cmd(2).Left - cmd(1).Width - 45, cmdOK.Top
    cmd(1).Visible = cmdOK.Visible
    cmd(2).Visible = cmdOK.Visible
End Sub
