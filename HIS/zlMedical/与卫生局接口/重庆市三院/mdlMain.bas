...
Public Sub Main()
    ...
    gstrPrive = GetPrivFunc(100, 6000)
    gstrPrive = ";" & gstrPrive & ";"
...
Public Function RegRestoreByManager() As Boolean
    ...
    strComputerName = AnalyseComputer
    strSQL = " Select 类别,用户名,目录,键名,键值 From zlClientParas " & _
              " Where 工作站='" & strComputerName & "'" & _
              " And (类别 In ('公共全局','公共模块')" & _
              " Or (类别 In ('私有全局','私有模块') And 用户名='" & gstrDBUser & "'))" & _
              " And Nvl(修改标志,0)=1 "
    ...
    With rsReg
        ...
        Do While Not .EOF
            Call SaveSetting("ZLSOFT", !类别 & IIf(IsNull(!用户名), "", "\" & !用户名) & _
                IIf(IsNull(!目录), "", "\" & !目录), !键名, IIf(IsNull(!键值), "", !键值))
            .MoveNext
        Loop
    End With
    
    strSQL = " Update zlClientParas " & _
              " Set 修改标志=0 " & _
              " Where 工作站='" & strComputerName & "'" & _
              " And (类别 In ('公共全局','公共模块')" & _
              " Or (类别 In ('私有全局','私有模块') And 用户名='" & gstrDBUser & "'))" & _
