...
Private Sub InitCardsObject()
    ...
    If objDatabase Is Nothing Then Set objDatabase = objDatabase
...
Private Function IDKindClick(ByVal objCard As Card, Optional ByVal blnFastKeyUse As Boolean = False) As Boolean
    ...
    If gobjSquare.zlReadCard(mobjParent, glngModul, objCard.接口序号, m_OnlyReadCardNo, strExpand, strOutCardNO, strOutPatiInforXML, strPhotoFile) = False Then Exit Function
...
Public Function GetIDKindCard(ByVal strCardType As String, _
    Optional MachMode As Mach_Mode, Optional bln消费卡 As Boolean = False) As Card
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取指定卡
...
Public Sub zlInit(ByVal frmMain As Object, Optional ByVal lngSys As Long, Optional ByVal lngModul As Long, _
    ...
    Optional OnlyThreeCard As Boolean = False)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:初始化
    '入参:frmMain-调用的主窗口
    '     lngSys : 系统编号
...
Public Function SetBrushCardObject(ByVal blnComm As Boolean) As Boolean
    ...
    If gobjSquare.zlSetBrushCardObject(objCard.接口序号, IIf(blnComm, mobjTxtInput, Nothing), strExpend, objCard.消费卡) Then
...
Private Function GetPatient(ByVal strInput As String, objPati As PatiInfor, ByRef objCard As Card, _
    ...
    If objCard.接口序号 > 0 Then
        If gobjSquare.zlGetPatiID(objCard.接口序号, strInput, False, lng病人ID, strCardPassWord, strErrMsg, lngCardTypeID) Then
    ...
    If objCard.名称 Like "姓名*" Or objCard.模糊查找项 Then
        ...
        If blnBrushCard Then
            If gobjSquare.zlGetPatiID(lngCardTypeID, strInput, False, lng病人ID, strCardPassWord, strErrMsg, _
