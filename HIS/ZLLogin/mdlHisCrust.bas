...
'    Call SQLTest
    ...
    If gclsLogin.DeptName = "" Then
        strSQL = "Select c.名称" & vbNewLine & _
                "From 上机人员表 a, 部门人员 b, 部门表 c" & vbNewLine & _
                "Where a.人员id = b.人员id And b.部门id = c.Id And a.用户名 = [1] And 缺省 = 1"
        Set rsTmp = OpenSQLRecord(strSQL, "确定当前人员所属缺省部门", gclsLogin.DBUser)
        If rsTmp.RecordCount > 0 Then
            gclsLogin.DeptName = rsTmp!名称
        End If
    End If
    ...
    Else
        strSQL = "Zl_Zlclients_Set(1,'" & strRowID & "','" & strComputerName & "','" & strIpAddress & "','" & arrComputer(2) & "','" & arrComputer(3) & _
                    "','" & arrComputer(4) & "','" & arrComputer(5) & "','" & gclsLogin.DeptName & "',Null,Null,Null,Null," & int连接数 & ",'" & str院区编号 & "',0,Null,Null," & int启用视频源 & ")"
        '需要更新相关的站点信息
...
'返回：是否获取成功（当存在只存在2500系统，则从配置文件获取，若不存在100与2500系统，则返回FALSE
    ...
    If rsTmp.RecordCount <> 0 Then
        ...
        If rsTmp.RecordCount = 0 Then
            rsTmp.Filter = "编号=25"
            If rsTmp.RecordCount <> 0 Then
                If objFSO.FileExists(gstrSetupPath & "\Apply\接口配置.ini") Then
                    strConn = ReadIni("接口", "数据访问", gstrSetupPath & "\Apply\接口配置.ini")
...
'      blnBrwCall=是否导航台调用,导航台调用升级时检查预升级时点
    ...
    '非授权的程序不自动升级
    strTmpGet = gobjRegister.GetPassword(glnghInstance)
