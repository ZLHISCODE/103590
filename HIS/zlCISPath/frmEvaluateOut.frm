...
Private mlngState           As Long             '0-查看(导入评估),1-评估,2-修改(阶段评估)。导入评估后不提供修改，要改只能取消导入，重新导入。阶段评估的查看通过调整功能实现,暂不提供取消功能
...
Private mlngDiagnosisSorce  As Long             '诊断来源1-病历;3-首页整理
...
Public Function ShowMe(frmParent As Object, ByVal lngFun As Long, ByVal lngState As Long, t_pati As TYPE_Pati, t_pp As TYPE_PATH_Pati, _
    ...
    Optional ByVal lng诊断ID As Long, Optional ByVal bln补录 As Boolean = False) As Boolean
'参数:bln补录  -True 补录评估
    mlngFun = lngFun
    mlngState = lngState
    mPati = t_pati
...
'功能：获取路径评估条件
    ...
    Else
        strSql = " Select a.指标ID, a.关系式, a.条件值, a.条件组合, Nvl(a.项目ID,0) as 项目ID,Nvl(b.执行结果,'无结果') as 执行结果,B.项目内容 " & vbNewLine & _
                 " From 门诊路径评估条件 A, (Select A.项目ID, A.执行结果, B.项目内容 From 病人门诊路径执行 A,门诊路径项目 B" & vbNewLine & _
...
'功能：获取导入评估和阶段评估的指标定义
    ...
    strSql = " Select a.ID 评估ID, b.ID 指标ID,b.序号, b.评估指标, b.指标结果,b.指标类型" & vbNewLine & _
             " From 门诊路径评估 A, 门诊路径评估指标 B" & vbNewLine & _
...
'功能：初始化界面设置
    ...
    If mlngFun = 0 Then
        ...
        optResult(0).Caption = "符合(&0)"
        optResult(1).Caption = "不符合(&1)"
    ...
    Else
        ...
        Else
            If GetNextPhaseOut(mPP.当前阶段ID) = 0 Then                         '没有后续阶段时，不允许选择下一阶段提前
        ...
        Else
            '达到标准治疗时间天数即可变异后完成。
            '如果没有达到标准治疗时间，则提供一个选项：提前结束
            If IsLastDate(True, lngMin, lngMax, mPP.路径ID, mPP.版本号, mPP.当前阶段ID, mPP.当前天数) Then
            ...
            Else
                ...
                optResult(3).Visible = True
            End If
            If InStr(GetInsidePrivs(P门诊路径应用), ";结束路径;") = 0 Then
                optResult(2).Visible = False
                optResult(3).Visible = False
            Else
                '具备结束路径和提前完成才允许提前完成,否则禁止。
                If optResult(3).Caption = "提前完成(&3)" Then
                    optResult(3).Visible = (InStr(GetInsidePrivs(P门诊路径应用), ";提前完成;") > 0)
                End If
            End If
...
'功能：初始化变异原因列表
    ...
    strSql = " Select b.名称 As 分类, a.编码, a.名称, a.简码" & vbNewLine & _
             " From 门诊变异常见原因 A, 门诊变异常见原因 B" & vbNewLine & _
...
'功能：获取路径外项目的变异原因
    Dim strSql As String
    If mlngState = 1 Then
        strSql = "Select distinct 变异原因 From (Select 变异原因 From 病人门诊路径执行 " & _
                "Where 路径记录Id = [1] And 阶段ID = [2] And 日期 = [3] And 变异原因 Is Not Null Order by 登记时间)"
    ElseIf mlngState = 2 Then
        strSql = "Select 变异原因 From 病人门诊路径变异 Where 路径记录Id = [1] And 阶段ID = [2] And 日期 = [3] "
    End If
    On Error GoTo errH
    ...
    Else
        ...
        Else
            If optResult(0).Enabled Then
                optResult(0).Value = True  '选择进度延后或提前时，以及超过标准治疗时间，不能再选择“正常”
            End If
        End If
    End If
End Sub
...
Private Sub vsPersonnel_ValidateEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    With vsPersonnel
        ...
        End If
        strSql = "Select Distinct a.ID,a.编号 as 编码,a.姓名 From 人员表 a, 人员性质说明 b Where a.Id = b.人员id And b.人员性质 = '医生'" & strSql
        ...
        If rsTmp Is Nothing Then
            If Not blnCancel Then
                MsgBox "根据输入内容未找到匹配的医生。", vbInformation, gstrSysName
            End If
            Cancel = True
            Exit Sub
        End If
...
Private Sub cmdOK_Click()
    ...
    If mlngFun = 0 Then
        lngLen = Sys.FieldsLength("病人门诊路径", "导入说明")
    Else
        lngLen = Sys.FieldsLength("病人门诊路径评估", "评估说明")
    End If
    ...
    If mlngFun = 1 Then
        If optResult(0).Value Or optResult(1).Value Then
            blnOver = IsLastDate(False, lngMin, lngMax, mPP.路径ID, mPP.版本号, mPP.当前阶段ID, mPP.当前天数)
            If InStr(GetInsidePrivs(P门诊路径应用), ";结束路径;") = 0 Then
                blnOver = False
            End If
        End If
        ...
        If optResult(0).Value Or optResult(1).Value And optDate(0).Value Then
            ...
            If blnOver Then
                MsgBox "注意：目前已达到或超过标准治疗时间，评估执行后将自动完成病人路径。", vbInformation, gstrSysName
            End If
        ...
        '超过标准治疗时间变异继续,或变异退出，要审核
        If optResult(1).Value And mPP.当前天数 > lngMax Or optResult(2).Value Then
            If InStr(GetInsidePrivs(P门诊路径应用), ";变异审核;") = 0 Then
                str审核人 = zlDatabase.UserIdentify(Me, "变异退出或超期继续需要审核。", glngSys, P门诊路径应用, "变异审核")
    ...
    If blnOver Or optResult(2).Value Then
        ...
        If CheckPathOutLogOut Then
            blnOK = frmPathOutLogOut.ShowMe(Me, mPati.病人ID, mPati.挂号ID, 0, mcolSQL, mPP.路径ID, mPP.病人路径ID)
            If blnOK = False Then
                i = Val(zlDatabase.GetPara("必须填写出径登记表", glngSys, P门诊路径应用, "0"))
    ...
    Call SaveData(blnOver, str审核人, str评估人, strVariation)
...
Private Sub SaveData(ByVal blnOver As Boolean, ByVal str审核人 As String, ByVal str评估人 As String, ByVal strVariation As String)
'功能:保存数据
...
'     strVariation  =变异原因
    ...
    Else
        ...
        strSql = "Zl_病人门诊路径评估_Insert(" & mlngState & "," & mPP.病人路径ID & "," & mPP.当前阶段ID & _
            ",To_Date('" & mPP.当前日期 & "','YYYY-MM-DD')," & mPP.当前天数 & ",'" & _
            str评估人 & "'," & lng评估结果 & ",'" & str评估说明 & "','" & UserInfo.姓名 & "','" & str审核人 & "','" & strVariation & "'," & str时间进度
    ...
    strSql = ""
    For i = 1 To mcolSQL.count
        If InStr(UCase(mcolSQL("C" & i)), "Zl_病人门诊路径生成_INSERT") > 0 Then
            strSql = "do"
            Exit For
        End If
    Next
    ...
    If strSql <> "" Then
        For i = 1 To colSQL.count
            If InStr(UCase(colSQL("C" & i)), "Zl_病人门诊路径生成_INSERT") > 0 Then
                strSql = "do"
                Exit For
            End If
        Next
    ...
    If strSql <> "" Then
        Call ZLHIS_CIS_001(Nothing, mPati.病人ID, mPati.挂号ID, mPati.病区ID, mPati.科室ID)
    End If
    Exit Sub
errH:
...
Private Function IsLastState(Optional ByVal blnEnd As Boolean, Optional ByRef lngMin As Long, Optional ByRef lngMax As Long, _
                            Optional ByVal lng路径ID As Long, Optional ByVal lng版本号 As Long, Optional ByVal lng当前阶段ID As Long, _
                            Optional ByVal lng当前天数 As Long, Optional ByRef lngState As Long) As Boolean
'功能：判断是否退出路径

'功能：判断是否退出路径
'      blnEnd=false:判断当前阶段是否是最后阶段，且没有后续阶段
'      blnEnd= true:是否允许变异退出（在标准治疗时间范围内都可退出）

'返回：lngMin，lngMax 标准治疗时间
'      lngState :当blnBoth=true  返回0=未达到标准治疗时间，1=达到标准治疗时间，但为达到最后一天，2=标准治疗时间最后一天

'1，计算当前门诊路径的天数是否在标准治疗时间内了
'2，计算当前阶段是否已经是最后一个阶段了
'满足这两个条件: 结束路径
'满足1，不满足2：提示，不结束路径
'不满足1，满足2：提示，允许结束
'    Dim rsTmp As ADODB.Recordset, strSql As String
...
'
'    strSql = "Select 标准治疗时间 From 门诊路径版本 Where 路径id = [1] And 版本号 = [2]"
'    On Error GoTo errH
'    Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, lng路径ID, lng版本号)
'    If Not IsNull(rsTmp!标准治疗时间) Then
'        arrtmp = Split(rsTmp!标准治疗时间, "-")
...
'
'        If blnEnd Then
'            lng理论天数 = GetMustDayOut(mPP.病人路径ID, lng当前天数)
...
'
'        If Not blnEnd Then
'            lng实际天数 = GetMustDayOut(mPP.病人路径ID, lng当前天数, True)
'            If lng实际天数 >= lngMax Then
'                blnIsLastDate = GetNextPhaseOut(lng当前阶段ID) = 0
'                If blnIsLastDate Then
'                    lngState = 2
'                End If
'            End If
...
'        lngLen = Split(rsTmp!标准治疗时间, "-")(0)
'    Else
'        lngLen = Val(rsTmp!标准治疗时间)
'    End If
'    strInfo = "按标准治疗时间" & lngLen & "天估算，即将发生的费用(不含可选项目)："
'    DatIn = GetPatiInPathOut( mPP.病人路径ID)
'
...
'    On Error GoTo errH
'
'    strSql = " Select ID,名称 From 门诊路径阶段" & vbNewLine & _
...
'
'    Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, mPati.病人ID, mPati.挂号ID, mPP.路径ID, mPP.版本号, lng阶段ID, lng天数, DatIn)
