...
Begin VB.Form frmPathOutLogOut 
   Caption         =   "病人出径登记"
   ClientHeight    =   8970
   ClientLeft      =   120
   ClientTop       =   450
   ClientWidth     =   12015
   Icon            =   "frmPathOutLogOut.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   ScaleHeight     =   8970
   ScaleWidth      =   12015
   ...
   Begin VB.PictureBox picItem 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsItem 
         ...
         FormatString    =   $"frmPathOutLogOut.frx":6852
         ScrollTrack     =   -1  'True
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   2
   ...
   Begin VB.PictureBox picInfo 
      ...
      Begin VB.Label lblNote 
         ...
         Caption         =   "请填写以下要求登记的信息。浅绿色背景的单元格为必填项，日期请按YYYY-MM-DD格式录入。"
         Height          =   255
         Left            =   1200
         TabIndex        =   1
         Top             =   360
      ...
      Begin VB.Image imgInfo 
         Height          =   720
         Left            =   195
         Picture         =   "frmPathOutLogOut.frx":6990
         Top             =   45
         Width           =   720
      End
   End
   ...
   End
End
Attribute VB_Name = "frmPathOutLogOut"
...
Public Function ShowMe(frmMain As Object, ByVal lng病人ID As Long, ByVal lng挂号ID As Long, ByVal lngFun As Long, ByRef colSQL As Collection, _
...
'参数： lngFun=0-新增，1-查看，2-修改
    ...
    If mlngFun = 1 Then
        If CheckPatiPathOutLogOut(lng病人路径Id) = False Then
...
Private Function SaveData() As Boolean
    ...
    With vsItem
        For i = .FixedRows To .Rows - 1
            ...
            If strSql <> "" Then
                strSql = "Zl_病人门诊出径记录_Update(" & strSql & "," & mlng病人ID & "," & mlng挂号ID & "," & .TextMatrix(i, COL_行号) & _
...
'功能:输出到EXCEL
    ...
    strSql = "Select NVL(A.姓名,' ') 姓名 From 病人挂号记录 A Where A.ID=[1]"
    ...
    strSql = "Select 名称 From 门诊路径目录 Where ID=[1]"
...
'返回：页数
    ...
    strSql = "Select Count(Distinct NVL(页数,1)) as 页数 From 门诊路径报表结构 Where 报表id = 2 And 路径id Is Null Or 路径id = [1]"
...
Private Sub LoadData()
    ...
    If mlngFun = 1 Or mlngFun = 2 Then
        strSql = "Select a.行号, NVL(a.页数,1)  as 页数, a.项目序号, a.项目文本1, a.项目文本2, a.路径id, a.多选序号, b.数字值, b.字符值, b.日期值, b.备注" & vbNewLine & _
                ...
                "       From 门诊路径报表结构 A, 门诊路径报表序号 B" & vbNewLine & _
                "       Where a.报表id = b.报表id(+) And a.行号 = b.行号(+) And a.报表id = 2 And" & vbNewLine & _
                "             (Nvl(a.路径id, b.路径id) = [3] And (Exists (Select 1 From 门诊路径报表序号 Where 报表ID = 2  And 路径id = [3]) Or " & vbNewLine & _
                "              Not Exists (Select 1 From 门诊路径报表结构 Where 报表id = 2 And a.路径id Is Null)))" & vbNewLine & _
                "Union All" & vbNewLine & _
                "Select a.行号, NVL(a.页数,1)  as 页数, a.项目序号, a.项目文本1, a.项目文本2, a.路径id, a.多选序号" & vbNewLine & _
                "From 门诊路径报表结构 A" & vbNewLine & _
                "Where a.报表id = 2 And a.路径id Is Null And Not Exists (Select 1 From 门诊路径报表序号 Where 报表id = 2 And 路径id = [3])) A, 病人门诊出径记录 B" & vbNewLine & _
                "Where a.行号 = b.行号(+) And b.病人id(+) = [1] And b.挂号id(+) = [2] And B.路径记录ID(+)=[4]" & vbNewLine & _
                "Order By 项目序号, 多选序号"
 
        Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, mlng病人ID, mlng挂号ID, mlng路径ID, mlng病人路径ID)
    Else
        strSql = "Select a.行号, NVL(a.页数,1) as 页数, Nvl(b.序号, a.项目序号) As 项目序号, a.项目文本1, a.项目文本2, a.路径id, a.多选序号" & vbNewLine & _
                "From 门诊路径报表结构 A, 门诊路径报表序号 B" & vbNewLine & _
                "Where a.报表id = b.报表id(+) And a.行号 = b.行号(+) And a.报表id = 2 And" & vbNewLine & _
                "      (Nvl(a.路径id, b.路径id) = [1] And (Exists (Select 1 From 门诊路径报表序号 Where 报表id = 2 And 路径id = [1]) Or " & vbNewLine & _
                "       Not Exists (Select 1 From 门诊路径报表结构 Where 报表id = 2 And a.路径id Is Null)))" & vbNewLine & _
                "Union All" & vbNewLine & _
                "Select a.行号, NVL(a.页数,1)  as 页数, a.项目序号, a.项目文本1, a.项目文本2, a.路径id, a.多选序号" & vbNewLine & _
                "From 门诊路径报表结构 A" & vbNewLine & _
                "Where a.报表id = 2 And a.路径id Is Null And Not Exists (Select 1 From 门诊路径报表序号 Where 报表id = 2 And 路径id = [1])" & vbNewLine & _
                "Order By 项目序号, 多选序号"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, mlng路径ID)
    End If
    With vsItem
