...
Public Function ShowEdit(frmParent As Object, rsAdvice As ADODB.Recordset, vItem As TYPE_PATH_ITEM, vPreItem As TYPE_PATH_ITEM, ByVal blnAdjust As Boolean, _
    blnInherit As Boolean, Optional ByVal lng路径ID As Long, Optional ByVal strPrivs As String) As Boolean
'功能：设置当前选择项目的详细内容
...
'功能：编辑项目所对应的医嘱
    ...
    Dim lngPos As Long
    Dim bytType As Byte      '1-只修改了药品只需药剂科审核;2-修改了医嘱未修改药品只需要医务科审核;3-需要药剂科和医务科同审
    Dim lngRow As Long
    ...
    If mvItem.医嘱IDs <> "" Then
        ...
        Do While Not mrsAdvice.EOF
            ...
            If gbln双审核 Then
                rsScheme!类别 = mrsAdvice!类别
                rsScheme!操作类型 = mrsAdvice!操作类型
            End If
            rsScheme.Update
            mrsAdvice.MoveNext
        Loop
        ...
        If gbln双审核 And mvItem.原医嘱IDs <> "" Then
            strFilter = ""
            For i = 0 To UBound(Split(mvItem.原医嘱IDs, ","))
                strFilter = strFilter & " Or ID=" & Split(mvItem.原医嘱IDs, ",")(i)
            Next
            mrsAdvice.Filter = Mid(strFilter, 5)
            Set rsCopy = zlDatabase.CopyNewRec(mrsAdvice, , "ID 序号,相关ID 相关序号,诊疗项目ID,类别,操作类型", Array("标记", adInteger, 1, Empty))     '记录下修改前医嘱记录
        End If
    End If
    ...
    If Not rsScheme Is Nothing Then
        If mvItem.原医嘱IDs <> "" And gbln双审核 Then
            Set rsScheme = zlDatabase.CopyNewRec(rsScheme, , , Array("标记", adInteger, 1, Empty))
            '根据修改后内容判断是否需要走审核流程:判断依据
            '1:仅仅只是调整医嘱生成模式(缺省,备选)、频次、单量、规格，医嘱嘱托 不需要审核
            '2:新增或调整或删除了诊疗项目ID的需要审核；
            lng序号 = 0
            bytType = 0
            ...
            Do While Not rsScheme.EOF
                ...
                If lng序号 <> NVL(rsScheme!相关序号, rsScheme!序号) Then
                    lng序号 = NVL(rsScheme!相关序号, rsScheme!序号)
                    
                    rsScheme.Filter = "序号=" & lng序号 & " OR 相关序号=" & lng序号
                    lngRow = rsScheme.RecordCount
                    For i = 1 To lngRow
                        rsCopy.Filter = "标记 = 0 And 诊疗项目ID =" & Val(rsScheme!诊疗项目ID & "")
                        strFilter = ""
                        Do While Not rsCopy.EOF
                            If InStr(";" & strFilter & ";", ";" & NVL(rsCopy!相关序号, rsCopy!序号) & ";") = 0 Then
                                rsCopy!标记 = 1
                                strFilter = strFilter & ";" & NVL(rsCopy!相关序号, rsCopy!序号)     '一组医嘱存在相同诊疗项目ID只标记一次
                            End If
                            rsCopy.MoveNext
                        Loop
                        rsScheme.MoveNext
                    Next
                    '找到完全相同的记录
                    rsCopy.Filter = "": i = 0: lng医嘱ID = 0: lngCount = 0
                    Do While Not rsCopy.EOF
                        If NVL(rsCopy!相关序号, rsCopy!序号) <> lng医嘱ID Then
                            If i = lngRow And lngCount = i And lngCount <> 0 Then
FindedTag:
                                '找到相同记录
                                rsCopy.Filter = "序号=" & lng医嘱ID & " OR 相关序号=" & lng医嘱ID
                                Do While Not rsCopy.EOF
                                    rsCopy.Delete
                                    rsCopy.Update
                                    rsCopy.MoveNext
                            ...
                            End If
                            lngCount = 1: i = 0
                            lng医嘱ID = NVL(rsCopy!相关序号, rsCopy!序号)
                            If rsCopy!标记 = 1 Then i = i + 1
                        Else
                            lngCount = lngCount + 1
                            If rsCopy!标记 = 1 Then i = i + 1
                        End If
                ...
                rsScheme.MoveNext
            Loop
            '判断是否需要审核 blnUpdate=True 标识需要审核
            rsCopy.Filter = ""
            Do While Not rsCopy.EOF
                blnUpdate = True
                If bytType = 3 Then Exit Do
                If InStr(";5;6;7;", ";" & rsCopy!类别 & ";") > 0 Or _
                    (rsCopy!类别 & "" = "E" And InStr(";2;3;4;", ";" & rsCopy!操作类型 & ";") > 0) Then
                ...
                rsCopy.MoveNext
            Loop
            rsScheme.Filter = "标记=0"
            Do While Not rsScheme.EOF
                blnUpdate = True
                lngPos = rsScheme.AbsolutePosition
                If bytType = 3 Then Exit Do
                If InStr(";5;6;7;", ";" & rsScheme!类别 & ";") > 0 Or _
                    (rsScheme!类别 & "" = "E" And InStr(";2;3;4;", ";" & rsScheme!操作类型 & ";") > 0) Then
                    bytType = IIf(bytType = 2, 3, 1)
                Else
                    bytType = IIf(bytType = 1, 3, 2)
                End If
            ...
            rsScheme.Filter = ""
            If bytType = 1 Then
                blnUpdate = InStr(mstrPrivs, ";药剂科审核;") = 0
            ElseIf bytType = 2 Then
                blnUpdate = InStr(mstrPrivs, ";审核;") = 0
            ElseIf bytType = 3 Then
                blnUpdate = InStr(mstrPrivs, ";药剂科审核;") = 0 Or InStr(mstrPrivs, ";审核;") = 0
            End If
        End If
        'blnUpdate=True 将审核且未停用的路径医嘱变动记录保存到【路径医嘱变动】中，待审核后再更新表单路径医嘱。
        If gbln双审核 = False Then blnUpdate = InStr(mstrPrivs, "路径医嘱调整") = 0 And mvItem.原医嘱IDs <> ""
        ...
        Do While Not rsScheme.EOF
            ...
            If gbln双审核 Then
                mrsAdvice!审核状态 = bytType
                mrsAdvice!项目ID = mvItem.ID
                mvItem.审核状态 = bytType
            Else
...
Private Sub cmdOK_Click()
    ...
    If zlCommFun.ActualLen(txtItem.Text) > txtItem.MaxLength Then
        ...
        txtItem.SetFocus: Exit Sub
    End If
    '过程或函数的参数如果是varchar2类型,最大只能传入4000个字符
    If Len(mvItem.医嘱IDs) > 4000 Or Len(mvItem.待审核医嘱IDs) > 4000 Then
        MsgBox "当前项目所对应的医嘱内容超过最大值,请删除部分医嘱。", vbInformation, gstrSysName
...
Private Sub InitSchemeRecordset(rsScheme As ADODB.Recordset)
    ...
    rsScheme.Fields.Append "类别", adVarChar, 1, adFldIsNullable
    rsScheme.Fields.Append "操作类型", adVarChar, 20, adFldIsNullable
