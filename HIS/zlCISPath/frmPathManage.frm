...
Private mblnShowStop As Boolean   '是否显示已经停用的临床路径
...
'参数：lngModle=0 新增普通文件，=1 新增患者版路径表
    ...
    If Not Sys.GetLobSql(glngSys, 10, rptPath.SelectedRows(0).Record(COL_ID).Value & "," & strFileName, strFile, arrSQL()) Then
...
Private Sub cbsMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_File_ExportToXML * 10# + 3    '导入标准路径
        ...
        If frmSub.ShowMe(gfrmMain, 0, 1) Then
            '刷新
            Call RefreshData
        End If
...
'功能：主窗口菜单定义部份
    ...
    With objMenu.CommandBar.Controls
        ...
        objControl.BeginGroup = True
        Set objControl = .Add(xtpControlButton, conMenu_View_ShowStoped, "显示已停用的路径(&D)")
        objControl.BeginGroup = True
...
'参数：用于定位
    ...
    strSql = "Select Distinct a.Id, a.分类, a.编码, a.名称, a.病例分型, a.适用病情, a.适用性别, a.适用年龄, a.说明, a.通用, a.最新版本," & vbNewLine & _
             ...
             "Where a.Id = b.路径id(+) And a.Id = c.路径id(+)" & IIf(mblnShowStop, "", " And C.停用人 Is Null ")
...
'功能：批量导出为XML文件
    ...
    strPath = zlCommFun.OpenDir(Me.Hwnd, "临床路径导出目录", GetSetting("ZLSOFT", "公共模块\" & App.ProductName, "临床路径XML目录"))
