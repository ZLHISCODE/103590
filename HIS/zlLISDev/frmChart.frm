...
364                 'killFile = Replace(strFile, ".BMP", ".GIF") '原始图形不删除
                Else
366                 WriteLog "DrawImg", "Gif文件格式不正确", strFile
                    Exit Function
                End If
...
'378         Do Until i > 100
'380             i = i + 1
'382             If gobjFSO.FileExists(strFile) Then
'                    'WriteLog "DrawImg", "第" & i & "次删除文件" & strFile, ""
'                    'gobjFSO.CopyFile strFile, strFile & "bak"
'384                 Call gobjFSO.DeleteFile(strFile)
'                Else
'                    'If strFile <> "" Then WriteLog "DrawImg", "文件" & strFile & "已删除!", ""
'                    Exit Do
'                End If
'            Loop
        ...
        Exit Function
errH:
386     WriteLog "DrawImg", "第" & CStr(Erl()) & "行", Err.Description
...
Private Function ChartShowPic(ByVal strType As String, ByVal strImgName As String, ByVal strFileName As String, Optional ByVal intLayOut As Integer = oc2dImageFitted, Optional intSaveType As Integer = 0) As Boolean
    ...
    With Me.ChartThis
        ...
        If intSaveType = 0 Then
            ...
            gobjFSO.CopyFile strImgName, strImgFile, True
        Else
        ...
        With frmPic
            If DrawGam(.Picture1, strData) Then
                ...
                If InStr(strImgName, ".") > 0 Then
                    ...
                    PicShowChart = True
                    'If gobjFSO.FileExists(strImgName) Then gobjFSO.DeleteFile strImgName
                End If
            End If
        End With
