...
Private Sub LoadData(ByVal strKey As String)
    ...
    Do While Not mrsData.EOF
        Set objItem = lvw.ListItems.Add(, , mrsData(lvw.ColumnHeaders(1).Text).Value, 1, 1)
         
        For lngLoop = 2 To lvw.ColumnHeaders.Count
            objItem.SubItems(lngLoop - 1) = zlCommFun.Nvl(mrsData(lvw.ColumnHeaders(lngLoop).Text))
        Next
...
Private Sub ReadTreeData()
    ...
    Do While Not mrsData.EOF
        If zlCommFun.Nvl(mrsData("上级id").Value, 0) <> 0 Then
