...
Begin VB.UserControl usrLeavHospDiag 
   BackColor       =   &H80000009&
   ClientHeight    =   1530
   ClientLeft      =   0
   ClientTop       =   0
   ...
   End
End
Attribute VB_Name = "usrLeavHospDiag"
...
'统一调用，读出数据
    ...
    strSQL = _
        ...
        "        WHERE 类型 = 4 AND 名称 = '出院诊断记录卡')" & vbCrLf & _
        " AND A.id=" & mlng病历id
    Call zlDatabase.OpenRecordset(rsTmp, strSQL, "出院诊断记录卡")
    If rsTmp.RecordCount < 1 Then
        SetErr -1, "该病历不存在无法调用出院诊断记录卡！"
        Exit Sub
    End If
...
On Error GoTo ErrHandle
    ...
    strSQL = " Select * from 病人诊断记录 WHERE 诊断类型 in (3,13) AND  病历ID=" & mlng病历id & " ORDER BY ID"
    Call zlDatabase.OpenRecordset(rsTmp, strSQL, "出院诊断记录卡")
    If rsTmp.RecordCount > 0 Then
        ...
        If mWestDiag Then
            ...
            If rsTmp.RecordCount > 0 Then
                ...
                For i = 1 To rsTmp.RecordCount
                    msfW.RowHeight(i) = cmbLeav(0).Height + Screen.TwipsPerPixelY * 2
                    ...
                    msfW.TextMatrix(i, EnmDiag西医.x出院情况) = zlCommFun.Nvl(rsTmp!出院情况)
                    msfW.TextMatrix(i, EnmDiag西医.x疑诊) = IIf(zlCommFun.Nvl(rsTmp!是否疑诊, 0) = 0, "", "？")
                    msfW.TextMatrix(i, EnmDiag西医.x未治) = IIf(zlCommFun.Nvl(rsTmp!是否未治, 0) = 0, "", "√")
        ...
        Else
            If rsTmp.RecordCount > 0 Then
                ...
                For i = 1 To rsTmp.RecordCount
                    msfH.RowHeight(i) = cmbLeav(1).Height + Screen.TwipsPerPixelY * 2
                    msfH.TextMatrix(i, EnmDiag中医.z序号) = CStr(i) & "、"
                    msfH.TextMatrix(i, EnmDiag中医.z名称) = IIf(rsTmp!诊断类型 = 3, "西医", "中医")
                    msfH.TextMatrix(i, EnmDiag中医.z诊断) = zlCommFun.Nvl(rsTmp!诊断描述)
                    msfH.TextMatrix(i, EnmDiag中医.z出院情况) = zlCommFun.Nvl(rsTmp!出院情况)
                    ...
                    msfH.TextMatrix(i, EnmDiag中医.z未治) = IIf(zlCommFun.Nvl(rsTmp!是否未治, 0) = 0, "", "√")
    ...
    Else
        ...
        For i = 1 To UBound(astrDiags, 1) + 1
            ...
            msfW.TextMatrix(i, EnmDiag西医.x序号) = CStr(i) & "、"
            msfW.TextMatrix(i, EnmDiag西医.x名称) = "诊断："
            msfW.TextMatrix(i, EnmDiag西医.x诊断) = IIf(Split(astrDiags(i - 1), " ")(0) Like "#、*", Mid(Split(astrDiags(i - 1), " ")(0), 3), Split(astrDiags(i - 1), " ")(0))
            msfW.TextMatrix(i, EnmDiag西医.x出院情况) = "治愈"
            msfW.TextMatrix(i, EnmDiag西医.x未治) = IIf(Len(Split(astrDiags(i - 1), " ")(4)) > 0, "√", "")
            msfW.TextMatrix(i, EnmDiag西医.x疑诊) = Split(astrDiags(i - 1), " ")(2)
...
'                    Exit Function
    ...
    '诊断类型'诊断ID'疾病ID'证候ID'是否疑诊'出院情况'是否未治'诊断描述;
    strSQL = ""
    If mWestDiag Then
        For i = 1 To msfW.Rows - 1
            ...
            strSQL = strSQL & "3''" & msfW.RowData(i) & "''" & strIllnID & "''0''" & _
                    IIf(Trim(msfW.TextMatrix(i, EnmDiag西医.x疑诊)) = "？", 1, 0) & "''" & _
                    Trim(msfW.TextMatrix(i, EnmDiag西医.x出院情况)) & "''" & _
                    IIf(Trim(msfW.TextMatrix(i, EnmDiag西医.x未治)) = "√", 1, 0) & "''" & _
                    msfW.TextMatrix(i, EnmDiag西医.x诊断) & ";"
        Next
    Else
        For i = 1 To msfH.Rows - 1
            '得到诊断类型
            strTmp = IIf(msfH.TextMatrix(i, EnmDiag中医.z名称) = "中医", "13", "3")
            '得到疾病ID
            ...
            strSQL = strSQL & strTmp & "''" & msfH.RowData(i) & "''" & strIllnID & "''" & strZID & "''" & _
                    IIf(Trim(msfH.TextMatrix(i, EnmDiag中医.z疑诊)) = "？", 1, 0) & "''" & _
                    Trim(msfH.TextMatrix(i, EnmDiag中医.z出院情况)) & "''" & _
                    IIf(Trim(msfH.TextMatrix(i, EnmDiag中医.z未治)) = "√", 1, 0) & "''" & _
                    msfH.TextMatrix(i, EnmDiag中医.z诊断) & ";"
        Next
    End If
    
    ReturnStrSQL = "ZL_病人出院诊断记录单_INSERT(" & _
...
'对行进行初始化
If bln西医 Then
    msfW.RowHeight(lngRow) = 315
    ReSetRowCode msfW
    msfW.TextMatrix(lngRow, EnmDiag西医.x名称) = "诊断："
    msfW.TextMatrix(lngRow, EnmDiag西医.x诊断) = ""
    msfW.TextMatrix(lngRow, EnmDiag西医.x出院情况) = ""
    msfW.TextMatrix(lngRow, EnmDiag西医.x疑诊) = ""
    msfW.TextMatrix(lngRow, EnmDiag西医.x未治) = ""
    msfW.TextMatrix(lngRow, EnmDiag西医.x疾病ID) = ""
    msfW.RowData(lngRow) = 0
    If lngRow > 1 Then
        msfW.TextMatrix(lngRow, EnmDiag西医.x出院情况) = msfW.TextMatrix(lngRow - 1, EnmDiag西医.x出院情况)
    Else
        If cmbLeav(0).ListCount > 0 Then
            msfW.TextMatrix(lngRow, EnmDiag西医.x出院情况) = cmbLeav(0).List(0)
        Else
            msfW.TextMatrix(lngRow, EnmDiag西医.x出院情况) = ""
        End If
    End If
    msfW.TextMatrix(lngRow, EnmDiag西医.x疑诊) = ""
Else
    msfH.RowHeight(lngRow) = 315
    ReSetRowCode msfH
    msfH.TextMatrix(lngRow, EnmDiag中医.z名称) = "中医"
    msfH.TextMatrix(lngRow, EnmDiag中医.z诊断) = ""
    msfH.TextMatrix(lngRow, EnmDiag中医.z未治) = ""
    msfH.TextMatrix(lngRow, EnmDiag中医.z疑诊) = ""
    msfH.TextMatrix(lngRow, EnmDiag中医.z证ID) = ""
    msfH.TextMatrix(lngRow, EnmDiag中医.z疾病ID) = ""
    msfH.RowData(lngRow) = 0
    If lngRow > 1 Then
        msfH.TextMatrix(lngRow, EnmDiag中医.z出院情况) = msfH.TextMatrix(lngRow - 1, EnmDiag中医.z出院情况)
    Else
        If cmbLeav(1).ListCount > 0 Then
            msfH.TextMatrix(lngRow, EnmDiag中医.z出院情况) = cmbLeav(1).List(0)
        Else
            msfH.TextMatrix(lngRow, EnmDiag中医.z出院情况) = ""
        End If
    End If
End If
End Sub

Private Sub chkCuer_Click(Index As Integer)
If Index = 0 Then
    If chkCuer(Index).Value = 0 Then
        msfW.TextMatrix(msfW.Row, EnmDiag西医.x未治) = ""
    Else
        msfW.TextMatrix(msfW.Row, EnmDiag西医.x未治) = "√"
    End If
Else
    If chkCuer(Index).Value = 0 Then
        msfH.TextMatrix(msfH.Row, EnmDiag中医.z未治) = ""
    Else
        msfH.TextMatrix(msfH.Row, EnmDiag中医.z未治) = "√"
    End If
End If
...
Private Sub chkCuer_GotFocus(Index As Integer)
zlCommFun.OpenIme
End Sub

Private Sub chkCuer_KeyPress(Index As Integer, KeyAscii As Integer)
...
Private Sub cmbLeav_Click(Index As Integer)
If mblnMode = False Then
    If Index = 0 Then
        '西诊
        msfW.TextMatrix(msfW.Row, EnmDiag西医.x出院情况) = cmbLeav(Index).Text
    Else
        '中诊
        msfH.TextMatrix(msfH.Row, EnmDiag中医.z出院情况) = cmbLeav(Index).Text
    End If
End If
End Sub

Public Sub ReadInPatiDiag(lngInPatiDiagID As Long)
'读出入院时的诊断作为初始的内容
    If gcnOracle Is Nothing Then SetErr -1, "连接对象没有初始化": Exit Sub
    If gcnOracle.State <> adStateOpen Then SetErr -1, "连接对象没有连接": Exit Sub
    ...
    strSQL = " Select * from 病人诊断记录 WHERE 诊断类型 in (2,12) AND  病历ID=" & lngInPatiDiagID & " ORDER BY ID"
    Call zlDatabase.OpenRecordset(rsTmp, strSQL, "出院诊断记录卡")
    If rsTmp.RecordCount > 0 Then
        rsTmp.MoveFirst
        If mWestDiag Then
            ...
            If rsTmp.RecordCount > 0 Then
                ...
                For i = 1 To rsTmp.RecordCount
                    ...
                    rsTmp.MoveNext
                Next
                msfW.Row = 1: msfW.Col = EnmDiag西医.x诊断
                msfW_EnterCell
            End If
        Else
            If rsTmp.RecordCount > 0 Then
                ...
                For i = 1 To rsTmp.RecordCount
                    ...
                    rsTmp.MoveNext
                Next
                msfH.Row = 1: msfH.Col = EnmDiag中医.z诊断
                msfH_EnterCell
            End If
        End If
    End If
...
Dim strTmp As String
    mblnMode = True
    cmbLeav(0).Clear
    cmbLeav(0).AddItem "治愈"
    cmbLeav(0).AddItem "好转"
    cmbLeav(0).AddItem "未愈"
    cmbLeav(0).AddItem "其他"
    cmbLeav(0).ListIndex = 0
    cmbLeav(1).Clear
    cmbLeav(1).AddItem "治愈"
    cmbLeav(1).AddItem "好转"
    cmbLeav(1).AddItem "未愈"
    cmbLeav(1).AddItem "其他"
    cmbLeav(1).ListIndex = 0
    mblnMode = False
    ...
    SetMSFFormat msfW, "序号,600,flexAlignCenterCenter,0;诊断名,0,flexAlignCenterCenter,0;诊断,8000,flexAlignLeftCenter,0;出院情况,800,1,0;疑诊,800,4,0;未治,800,4,0;疾病ID,0,4,0"
    
    SetMSFFormat msfH, "序号,600,flexAlignCenterCenter,0;诊断名,800,flexAlignCenterCenter,0;诊断,8000,flexAlignLeftCenter,0;出院情况,800,1,0;疑诊,800,4,0;未治,800,4,0;证ID,0,4,0;疾病ID,0,4,0"
    
    '
    msfW.RowHeight(0) = 0
    msfW.Col = 0: msfW.Row = 1
    
    msfW.RowHeight(1) = 315
    ...
    msfW.RowHeight(1) = 315
    '设置表格其它初始
    msfW.ColAlignment(EnmDiag西医.x诊断) = AlignmentSettings.flexAlignLeftCenter
    msfH.ColAlignment(EnmDiag中医.z诊断) = AlignmentSettings.flexAlignLeftCenter
...
Select Case KeyAscii
    Case vbKeyReturn
        If chkCuer(Index).Enabled And chkCuer(Index).Visible Then
            chkCuer(Index).SetFocus
        End If
End Select
End Sub
...
Private Sub cmbLeav_GotFocus(Index As Integer)
zlCommFun.OpenIme
End Sub

Private Sub cmbLeav_KeyPress(Index As Integer, KeyAscii As Integer)
If KeyAscii = vbKeyReturn Then
    cmbLeav_Click Index
    On Error Resume Next
    If chkDiff(Index).Enabled And chkDiff(Index).Visible And UserControl.Enabled Then
...
Private Sub msfH_EnterCell()
    '调试用
'    Debug.Print "中医："; CStr(msfH.RowData(msfH.Row))
    ...
    SetSelColor msfH, msfH.Row
    '设置中医与医的名称
    chkWH.Left = msfH.Left + msfH.ColWidth(EnmDiag中医.z序号) + Screen.TwipsPerPixelY * 2
    chkWH.Top = msfH.Top + msfH.CellTop + Screen.TwipsPerPixelY * 0
    chkWH.Width = msfH.ColWidth(EnmDiag中医.z名称) - Screen.TwipsPerPixelX * 2
    ...
    '设置诊断内容的文本框
    txtDiag(1).Left = msfH.Left + msfH.ColWidth(EnmDiag中医.z序号) + msfH.ColWidth(EnmDiag中医.z名称) + Screen.TwipsPerPixelY * 2
    txtDiag(1).Top = msfH.Top + msfH.CellTop + Screen.TwipsPerPixelY * 0
    txtDiag(1).Width = msfH.ColWidth(EnmDiag中医.z诊断) - Screen.TwipsPerPixelX * 2
    ...
    '出院情况
    cmbLeav(1).Top = txtDiag(1).Top
    cmbLeav(1).Left = msfH.Left + msfH.ColWidth(EnmDiag中医.z序号) + msfH.ColWidth(EnmDiag中医.z名称) + msfH.ColWidth(EnmDiag中医.z诊断) + Screen.TwipsPerPixelY * 2
    cmbLeav(1).Width = msfH.ColWidth(EnmDiag中医.z出院情况)
    msfH.RowHeight(msfH.Row) = cmbLeav(1).Height + Screen.TwipsPerPixelY * 2
    If Trim(msfH.TextMatrix(msfH.Row, EnmDiag中医.z出院情况)) <> "" Then
        If cmbLeav(1).ListCount > 0 Then
            mblnMode = True
            cmbLeav(1).ListIndex = 0
            mblnMode = False
            For i = 0 To cmbLeav(1).ListCount - 1
                If cmbLeav(1).List(i) = msfH.TextMatrix(msfH.Row, EnmDiag中医.z出院情况) Then
                    mblnMode = True
                    cmbLeav(1).ListIndex = i
                    mblnMode = False
                    Exit For
                End If
            Next
    ...
    Else
        If cmbLeav(1).ListCount > 0 Then
            If msfH.Row > 1 Then
                msfH.TextMatrix(msfH.Row, EnmDiag中医.z出院情况) = msfH.TextMatrix(msfH.Row - 1, EnmDiag中医.z出院情况)
            Else
                msfH.TextMatrix(msfH.Row, EnmDiag中医.z出院情况) = cmbLeav(1).List(0)
            End If
            cmbLeav(1).ListIndex = 0
            For i = 0 To cmbLeav(1).ListCount - 1
                If cmbLeav(1).List(i) = msfH.TextMatrix(msfH.Row, EnmDiag中医.z出院情况) Then
    ...
    '设置疑诊复选择
    chkDiff(1).Left = msfH.Left + msfH.ColWidth(EnmDiag中医.z序号) + msfH.ColWidth(EnmDiag中医.z名称) + msfH.ColWidth(EnmDiag中医.z诊断) + msfH.ColWidth(EnmDiag中医.z出院情况) + Screen.TwipsPerPixelY * 2
    ...
    '是否未治
    chkCuer(1).Left = msfH.Left + msfH.ColWidth(EnmDiag中医.z序号) + msfH.ColWidth(EnmDiag中医.z名称) + msfH.ColWidth(EnmDiag中医.z诊断) + msfH.ColWidth(EnmDiag中医.z出院情况) + msfH.ColWidth(EnmDiag中医.z疑诊) + Screen.TwipsPerPixelY * 2
    chkCuer(1).Top = msfH.Top + msfH.CellTop + Screen.TwipsPerPixelY * 0
    chkCuer(1).Width = msfH.ColWidth(EnmDiag中医.z未治) - Screen.TwipsPerPixelX * 2
    chkCuer(1).Height = msfH.CellHeight - Screen.TwipsPerPixelY * 2
    If msfH.TextMatrix(msfH.Row, EnmDiag中医.z未治) = "√" Then
        chkCuer(1).Value = 1
    Else
        chkCuer(1).Value = 0
    End If
...
ElseIf Button = 2 And mDispMode = False Then
    ...
    ElseIf msfH.MouseRow = 1 And (msfH.TextMatrix(1, EnmDiag中医.z诊断) <> "" Or msfH.TextMatrix(1, EnmDiag中医.z证ID) <> "" Or msfH.RowData(1) <> 0) Then
        ...
        msfH_EnterCell
        If MsgBox("您要删除行号为 " & msfH.TextMatrix(msfH.Row, EnmDiag中医.z序号) & " 的诊断吗？", vbQuestion + vbYesNo, gstrSysName) = vbYes Then
            msfH.TextMatrix(1, EnmDiag中医.z名称) = "中医"
            msfH.TextMatrix(1, EnmDiag中医.z诊断) = ""
            msfH.TextMatrix(1, EnmDiag中医.z疑诊) = ""
            msfH.TextMatrix(1, EnmDiag中医.z未治) = ""
...
Private Sub msfW_EnterCell()
    ...
    '设置诊断内容的文本框
    txtDiag(0).Left = msfW.Left + msfW.ColWidth(EnmDiag西医.x序号) + msfW.ColWidth(EnmDiag西医.x名称) + Screen.TwipsPerPixelY * 3
    txtDiag(0).Top = msfW.Top + msfW.CellTop + Screen.TwipsPerPixelY * 0
    txtDiag(0).Width = msfW.ColWidth(EnmDiag西医.x诊断) - Screen.TwipsPerPixelX * 3
    ...
    '出院情况
    cmbLeav(0).Top = txtDiag(0).Top
    cmbLeav(0).Left = msfW.Left + msfW.ColWidth(EnmDiag西医.x序号) + msfW.ColWidth(EnmDiag西医.x名称) + msfW.ColWidth(EnmDiag西医.x诊断) + Screen.TwipsPerPixelY * 2
    cmbLeav(0).Width = msfW.ColWidth(EnmDiag西医.x出院情况)
    msfW.RowHeight(msfW.Row) = cmbLeav(0).Height + Screen.TwipsPerPixelY * 2
    If Trim(msfW.TextMatrix(msfW.Row, EnmDiag西医.x出院情况)) <> "" Then
        If cmbLeav(0).ListCount > 0 Then
            mblnMode = True
            cmbLeav(0).ListIndex = 0
            mblnMode = False
            For i = 0 To cmbLeav(0).ListCount - 1
                If cmbLeav(0).List(i) = msfW.TextMatrix(msfW.Row, EnmDiag西医.x出院情况) Then
                    mblnMode = True
                    cmbLeav(0).ListIndex = i
                    mblnMode = False
                    Exit For
                End If
            Next
    ...
    Else
        If cmbLeav(0).ListCount > 0 Then
            If msfW.Row > 1 Then
                msfW.TextMatrix(msfW.Row, EnmDiag西医.x出院情况) = msfW.TextMatrix(msfW.Row - 1, EnmDiag西医.x出院情况)
            Else
                msfW.TextMatrix(msfW.Row, EnmDiag西医.x出院情况) = cmbLeav(0).List(0)
            End If
            cmbLeav(0).ListIndex = 0
            For i = 0 To cmbLeav(0).ListCount - 1
                If cmbLeav(0).List(i) = msfW.TextMatrix(msfW.Row, EnmDiag西医.x出院情况) Then
    ...
    '设置疑诊复选择
    chkDiff(0).Left = msfW.Left + msfW.ColWidth(EnmDiag西医.x序号) + msfW.ColWidth(EnmDiag西医.x名称) + msfW.ColWidth(EnmDiag西医.x诊断) + msfW.ColWidth(EnmDiag西医.x出院情况) + Screen.TwipsPerPixelY * 2
    chkDiff(0).Top = msfW.Top + msfW.CellTop + Screen.TwipsPerPixelY * 0
    chkDiff(0).Width = msfW.ColWidth(EnmDiag西医.x疑诊) - Screen.TwipsPerPixelX * 2
    ...
    chkCuer(0).Left = msfW.Left + msfW.ColWidth(EnmDiag西医.x序号) + msfW.ColWidth(EnmDiag西医.x名称) + msfW.ColWidth(EnmDiag西医.x诊断) + msfW.ColWidth(EnmDiag西医.x出院情况) + msfW.ColWidth(EnmDiag西医.x疑诊) + Screen.TwipsPerPixelY * 2
    chkCuer(0).Top = msfW.Top + msfW.CellTop + Screen.TwipsPerPixelY * 0
    chkCuer(0).Width = msfW.ColWidth(EnmDiag西医.x未治) - Screen.TwipsPerPixelX * 2
    chkCuer(0).Height = msfW.CellHeight - Screen.TwipsPerPixelY * 2
    If msfW.TextMatrix(msfW.Row, EnmDiag西医.x未治) = "√" Then
        chkCuer(0).Value = 1
    Else
        chkCuer(0).Value = 0
    End If
...
ElseIf Button = 2 And mDispMode = False Then
    If msfW.MouseRow > 1 Then
        ...
        End If
        msfW_EnterCell
    ElseIf msfW.MouseRow = 1 And (msfW.TextMatrix(1, EnmDiag西医.x诊断) <> "" Or msfW.RowData(1) <> 0) Then
        msfW.Row = 1
        SetSelColor msfW, 1
        msfW_EnterCell
        If MsgBox("您要删除行号为 " & msfW.TextMatrix(1, EnmDiag西医.x序号) & " 的诊断吗？", vbQuestion + vbYesNo, gstrSysName) = vbYes Then
            msfW.TextMatrix(1, EnmDiag西医.x诊断) = ""
            msfW.TextMatrix(1, EnmDiag西医.x疑诊) = ""
            msfW.TextMatrix(1, EnmDiag西医.x未治) = ""
            msfW.TextMatrix(1, EnmDiag西医.x疾病ID) = ""
            msfW.RowData(1) = 0
            txtDiag(0).Text = ""
            chkDiff(0).Value = 0
            chkDiff_Click 0
        ...
        For i = 1 To msfW.Rows - 1
            If i = 1 Then
                If Trim(msfW.TextMatrix(i, EnmDiag西医.x诊断)) = "" Then
                    ...
                    Exit Property
                Else
                    strTmp = strTmp & msfW.TextMatrix(i, EnmDiag西医.x序号) & msfW.TextMatrix(i, EnmDiag西医.x诊断) & "  " & msfW.TextMatrix(i, EnmDiag西医.x疑诊) & "  " & IIf(msfW.TextMatrix(i, EnmDiag西医.x未治) = "√", "未治", "") & IIf(i = msfW.Rows - 1, "", vbCrLf)
                End If
            Else
                strTmp = strTmp & msfW.TextMatrix(i, EnmDiag西医.x序号) & msfW.TextMatrix(i, EnmDiag西医.x诊断) & "  " & msfW.TextMatrix(i, EnmDiag西医.x疑诊) & "  " & IIf(msfW.TextMatrix(i, EnmDiag西医.x未治) = "√", "未治", "") & IIf(i = msfW.Rows - 1, "", vbCrLf)
            End If
        ...
        For i = 1 To msfH.Rows - 1
            If i = 1 Then
                If Trim(msfH.TextMatrix(i, EnmDiag中医.z诊断)) = "" Then
                    ...
                    Exit Property
                Else
                    strTmp = strTmp & msfH.TextMatrix(i, EnmDiag中医.z序号) & msfH.TextMatrix(i, EnmDiag中医.z诊断) & "  " & msfH.TextMatrix(i, EnmDiag中医.z疑诊) & "  " & IIf(msfH.TextMatrix(i, EnmDiag中医.z未治) = "√", "未治", "") & IIf(i = msfH.Rows - 1, "", vbCrLf)
                End If
            Else
                strTmp = strTmp & msfH.TextMatrix(i, EnmDiag中医.z序号) & msfH.TextMatrix(i, EnmDiag中医.z诊断) & "  " & msfH.TextMatrix(i, EnmDiag中医.z疑诊) & "  " & IIf(msfH.TextMatrix(i, EnmDiag中医.z未治) = "√", "未治", "") & IIf(i = msfH.Rows - 1, "", vbCrLf)
            End If
