...
Begin VB.UserControl usrClinicDiag 
   BackColor       =   &H80000009&
   ClientHeight    =   1530
   ClientLeft      =   0
   ClientTop       =   0
   ...
   End
End
Attribute VB_Name = "usrClinicDiag"
...
'统一调用，读出数据
    ...
    strSQL = _
        ...
        "        WHERE 类型 = 4 AND 名称 = '门诊诊断记录卡')" & vbCrLf & _
        " AND A.id=" & mlng病历id
    Call ZlDataBase.OpenRecordset(rsTmp, strSQL, "门诊诊断记录卡")
    If rsTmp.RecordCount < 1 Then
        SetErr -1, "该病历不存在无法调用门诊诊断记录卡！"
        Exit Sub
    End If
...
On Error GoTo ErrHandle
    ...
    strSQL = " Select * from 病人诊断记录 WHERE 诊断类型 in (1,11) AND  病历ID=" & mlng病历id & " ORDER BY ID"
    Call ZlDataBase.OpenRecordset(rsTmp, strSQL, "门诊诊断记录卡")
        ...
        Else
            If rsTmp.RecordCount > 0 Then
                ...
                For i = 1 To rsTmp.RecordCount
                    ...
                    msfH.TextMatrix(i, EnmDiag中医.z名称) = IIf(rsTmp!诊断类型 = 1, "西医", "中医")
...
'                    Exit Function
    ...
    If mWestDiag Then
        For i = 1 To msfW.Rows - 1
            ...
            strSQL = strSQL & "1''" & msfW.RowData(i) & "''" & strIllnID & "''0''" & IIf(Trim(msfW.TextMatrix(i, EnmDiag西医.x疑诊)) = "？", 1, 0) & "''" & msfW.TextMatrix(i, EnmDiag西医.x诊断) & ";"
        Next
    Else
        For i = 1 To msfH.Rows - 1
            '得到诊断类型
            strTmp = IIf(msfH.TextMatrix(i, EnmDiag中医.z名称) = "中医", "11", "1")
            '得到疾病ID
...
If bln西医 Then
    ...
    msfW.TextMatrix(lngRow, EnmDiag西医.x参考) = ""
...
Else
    ...
    msfH.TextMatrix(lngRow, EnmDiag中医.z参考) = ""
...
Private Sub UserControl_Resize()
    ...
    If i < 600 Then
        msfW.ColWidth(EnmDiag西医.x诊断) = 600
    Else
        msfW.ColWidth(EnmDiag西医.x诊断) = i
    End If
    ...
    msfH.ColWidth(EnmDiag中医.z诊断) = IIf(i < 600, 600, i)
    txtDiag(1).Width = msfH.ColWidth(EnmDiag中医.z诊断) - Screen.TwipsPerPixelX * 3
    '设置所有编辑控件不可见
    chkDiff(0).Visible = False
    cmdCons(0).Visible = False
    txtDiag(0).Visible = False
...
ElseIf Button = 2 And mDispMode = False Then
    ...
    ElseIf msfH.MouseRow = 1 And (msfH.TextMatrix(1, EnmDiag中医.z诊断) <> "" Or msfH.TextMatrix(1, EnmDiag中医.z证ID) <> "" Or msfH.RowData(1) <> 0) Then
        ...
        SetSelColor msfH, msfH.Row
        msfH_EnterCell
        If MsgBox("您要删除行号为 " & msfH.TextMatrix(msfH.Row, EnmDiag中医.z名称) & " 的诊断吗？", vbQuestion + vbYesNo, gstrSysName) = vbYes Then
