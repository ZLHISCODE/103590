...
'统一调用，读出数据
    ...
    strSQL = _
        ...
        "        WHERE 类型 = 4 AND 名称 = '死亡诊断记录卡')" & vbCrLf & _
        " AND A.id=" & mlng病历id
    Call ZlDataBase.OpenRecordset(rsTmp, strSQL, "死亡诊断记录卡")
    If rsTmp.RecordCount < 1 Then
        SetErr -1, "该病历不存在无法调用死亡诊断记录卡！"
        Exit Sub
    End If
...
Dim strTmp As String
    ...
    SetMSFFormat msfW, "序号,600,flexAlignCenterCenter,0;诊断名,0,flexAlignCenterCenter,0;诊断,8000,flexAlignLeftCenter,0;疾病ID,0,4,0"
    
    SetMSFFormat msfH, "序号,600,flexAlignCenterCenter,0;诊断名,800,flexAlignCenterCenter,0;诊断,8000,flexAlignLeftCenter,0;证ID,0,4,0;疾病ID,0,4,0"
...
Private Sub msfW_EnterCell()
    ...
    mblnMode = True
    txtDiag(0).Text = msfW.TextMatrix(msfW.Row, EnmDiag西医.x序号)
    mblnMode = False
...
ElseIf Button = 2 And mDispMode = False Then
    If msfW.MouseRow > 1 Then
        ...
        End If
        msfW_EnterCell
    ElseIf msfW.MouseRow = 1 And (msfW.TextMatrix(1, EnmDiag西医.x诊断) <> "" Or msfW.TextMatrix(1, EnmDiag西医.x疾病ID) <> "" Or msfW.RowData(1) <> 0) Then
        msfW.Row = 1
        SetSelColor msfW, msfW.Row
        msfW_EnterCell
        If MsgBox("您要删除行号为 " & msfW.TextMatrix(msfW.Row, EnmDiag西医.x序号) & " 的诊断吗？", vbQuestion + vbYesNo, gstrSysName) = vbYes Then
        ...
        For i = 1 To msfW.Rows - 1
            If i = 1 Then
                If Trim(msfW.TextMatrix(i, EnmDiag西医.x诊断)) = "" Then
                    ...
                    Exit Property
                Else
                    strTmp = strTmp & msfW.TextMatrix(i, EnmDiag西医.x序号) & msfW.TextMatrix(i, EnmDiag西医.x诊断) & IIf(i = msfW.Rows - 1, "", vbCrLf)
                End If
            Else
                strTmp = strTmp & msfW.TextMatrix(i, EnmDiag西医.x序号) & msfW.TextMatrix(i, EnmDiag西医.x诊断) & IIf(i = msfW.Rows - 1, "", vbCrLf)
            End If
        ...
        For i = 1 To msfH.Rows - 1
            If i = 1 Then
                If Trim(msfH.TextMatrix(i, EnmDiag中医.z诊断)) = "" Then
                    ...
                    Exit Property
                Else
                    strTmp = strTmp & msfH.TextMatrix(i, EnmDiag中医.z序号) & msfH.TextMatrix(i, EnmDiag中医.z诊断) & IIf(i = msfH.Rows - 1, "", vbCrLf)
                End If
            Else
                strTmp = strTmp & msfH.TextMatrix(i, EnmDiag中医.z序号) & msfH.TextMatrix(i, EnmDiag中医.z诊断) & IIf(i = msfH.Rows - 1, "", vbCrLf)
            End If
