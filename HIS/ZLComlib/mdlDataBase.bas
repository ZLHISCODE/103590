...
'                3、公共方法
'---------------------------------------------------------------------------
Public Function CallProcedure(cnInput As ADODB.Connection, ByVal strProcName As String, ByVal strFormCaption As String, blnUseLog As Boolean, ParamArray arrProcParas() As Variant) As Variant
...
Public Function CallProcedureByArray(cnInput As ADODB.Connection, ByVal strProcName As String, ByVal strFormCaption As String, blnUseLog As Boolean, arrProcParas() As Variant, Optional ByVal blnAsSubCall As Boolean) As Variant
    Dim cmdData     As New ADODB.Command, blnOELDB      As Boolean
    Dim i           As Long
    Dim lngAdjust   As Long
    Dim pdCur       As ParameterDirectionEnum, dtCur    As DataTypeEnum, dtSize As Long, varValue       As Variant, lngMax As Long, lngParaUbound   As Long
...
Public Function OpenSQLRecord(cnInput As ADODB.Connection, ByVal strSql As String, ByVal strTitle As String, ParamArray arrInput() As Variant) As ADODB.Recordset
    Dim arrPars() As Variant
    arrPars = arrInput
    Set OpenSQLRecord = mdlDataBase.OpenSQLRecordByArray(cnInput, strSql, strTitle, arrPars)
End Function

Public Function OpenSQLRecordByArray(cnInput As ADODB.Connection, ByVal strSql As String, ByVal strTitle As String, arrInput() As Variant, Optional intLobOprate As Integer = 0) As ADODB.Recordset
...
'调用方式为：Set rsPati=OpenSQLRecord(strSQL, Me.Caption, CDate(Format(rsMove!转出日期,"yyyy-MM-dd")),dtp时间.Value, lng病人ID, "张%", "男", 20, 21)
    ...
    If Replace(strSQLTmp, " ", "") Like "*/[*]+DRIVING_SITE*[*]/*" Then
        If Not gobjComLib.Sys.CheckDatamoveRemote Then
            arrStr = Split(strSql, "/*")
...
'        End If
    Else
        ...
        Else
            If cnInput Is gcnOracle Then Set cnOLEDB = gcnOracleOLEDB
            If cnOLEDB Is Nothing Then
                If Not IsOLEDBConnection(cnInput) Then
                    Set cnOLEDB = gobjRegister.ReGetConnection(OraOLEDB, strError, cnInput)
                Else
                    Set cnOLEDB = cnInput
                End If
