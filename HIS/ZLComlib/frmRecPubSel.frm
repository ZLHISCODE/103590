...
Begin VB.Form frmRecPubSel 
   ...
   End
End
Attribute VB_Name = "frmRecPubSel"
...
'参数：
'     frmParent=显示的父窗体
'     rsBindings=数据来源,不同风格的选择器对字段有不同要求
'     bytStyle=选择器风格
'       为0时:列表风格:ID,…
...
'               格式为：HideCols=?隐藏列名称，多个用英文逗号分隔，格式:列名1,列名2,...
    ...
    ElseIf mbytStyle = 2 Then
        ...
        Else
            ...
            For i = 0 To UBound(Split(mstrKeys, ","))
                If IsType(mrsSel.fields("上级ID").type, adVarChar) Then
