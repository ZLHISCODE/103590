Attribute VB_Name = "mdlControlDef"
Option Explicit
...
Public Const conMenu_View_Order = 7405                       '*调整顺序(&F)
Public Const conMenu_View_Setting = 5010101                       '*参数设置

Public Const conMenu_File_PrintPageSet = 105
...
'        Call ShowHelp(App.ProductName, frmMain.hWnd, frmMain.Name, Int((glngSys) / 100))
    ...
    Case conMenu_Help_Web_Home 'Web上的中联
        ...
        Call gobjComLib.zlHomePage(frmMain.hWnd)
        
    Case conMenu_Help_Web_Forum         'Web上的论坛
    
        Call gobjComLib.zlWebForum(frmMain.hWnd)
        
    Case conMenu_Help_Web_Mail '发送反馈
        
        Call gobjComLib.zlMailTo(frmMain.hWnd)
            
    Case conMenu_Help_About '关于
        
        Call gobjComLib.ShowAbout(frmMain, App.Title, App.ProductName, App.Major & "." & App.Minor & "." & App.Revision)
...
Public Function GetLines(ByVal objDraw As Object, ByVal strText As String, ByVal lngCX As Long) As Integer
    ...
    Set rs = gobjDatabase.OpenSQLRecord(strSql, "", strText, lngChars)
    If rs.BOF = False Then
        GetLines = gobjCommFun.Nvl(rs("行数").Value)
    End If
...
Public Function GetLineText2(ByVal objDraw As Object, ByVal strText As String, ByVal intRow As Integer, ByVal lngCX As Long) As String
    ...
    Set rs = gobjDatabase.OpenSQLRecord(strSql, "", strText, lngChars, intRow)
    If rs.BOF = False Then
        GetLineText2 = gobjCommFun.Nvl(rs("行文本").Value)
    End If
...
Public Function CreateTmpFile(Optional ByVal strFile As String = "zlPublicLIS.tmp") As String
    '******************************************************************************************************************
    '功能：
    '参数：
    '返回：
