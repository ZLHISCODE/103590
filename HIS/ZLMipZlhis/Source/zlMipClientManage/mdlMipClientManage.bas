Attribute VB_Name = "mdlMipClientManage"
Option Explicit
...
'    gblnOwner = gclsBusiness.IsOwner
    gblnDBA = gclsBusiness.IsDBAUser
...
Public Function Lpad(ByVal strNum As String, ByVal intChars As Integer, ByVal strChar As String) As String
    
    Dim strTmp As String
    
    strTmp = String(intChars, strChar) & strNum
    Lpad = Right(strTmp, intChars)
    
End Function

Public Function LoadTree(ByRef objTvw As Object, ByVal rs As ADODB.Recordset, Optional ByVal strUpNodeKey As String) As Boolean
    '******************************************************************************************************************
    '功能：
    '参数：
    '返回：
    ...
    Do While Not rs.EOF
        ...
        If IsNull(rs("上级id").Value) Then
            If strUpNodeKey = "" Then
                Set objNode = objTvw.Nodes.Add(, , "K" & zlCommFun.NVL(rs("ID").Value, 0), strTmp, strIcon, strIconSel)
            Else
                Set objNode = objTvw.Nodes.Add(strUpNodeKey, tvwChild, "K" & zlCommFun.NVL(rs("ID").Value, 0), strTmp, strIcon, strIconSel)
            End If
        Else
...
Public Sub ExpandAllOutline(objVsf As Object)
...
Public Sub CollapseAllOutline(objVsf As Object)
    ...
    With objVsf
        ...
        For lngRow = 1 To .Rows - 1
'            If .IsSubtotal(lngRow) = True And .RowOutlineLevel(lngRow) > 0 Then
