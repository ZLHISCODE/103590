...
Begin VB.Form frmAppCreate 
   ...
   Begin MSComctlLib.ImageList imgList 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmAppCreate.frx":6852
            Key             =   "全选"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmAppCreate.frx":6DEC
            Key             =   "已完成"
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmAppCreate.frx":7386
            Key             =   "执行中"
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmAppCreate.frx":7920
            Key             =   "待执行"
         EndProperty
         BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmAppCreate.frx":7EBA
            Key             =   "全清"
         EndProperty
      EndProperty
   End
   ...
   Begin VB.PictureBox picPage 
      ...
      Begin VB.Label lbl 
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "请勾选需要安装的ZLHIS产品内部的消息数据"
         Height          =   180
         Index           =   5
         Left            =   975
         TabIndex        =   3
   ...
   Begin VB.PictureBox picPage 
      ...
      Begin VB.CommandButton cmd 
         ...
         Index           =   0
         Left            =   7500
         Picture         =   "frmAppCreate.frx":1055C
         Style           =   1  'Graphical
         TabIndex        =   47
         TabStop         =   0   'False
         Top             =   795
      ...
      Begin VB.Image img 
         ...
         Index           =   2
         Left            =   270
         Picture         =   "frmAppCreate.frx":16DAE
         Top             =   165
         Width           =   480
      End
      Begin VB.Label lbl 
   ...
   Begin VB.PictureBox picPage 
      ...
      Begin VB.Image img 
         ...
         Index           =   4
         Left            =   75
         Picture         =   "frmAppCreate.frx":18730
         Top             =   195
         Width           =   480
      End
   End
...
'        End With
'
'        .InsertItem 0, "病历接口 ", vsfUser.hWnd, 0
'        .InsertItem 1, "文本 ", vsfUser.hWnd, 0
'
...
Private Function OraDataOpen(ByVal strServerIP As String, ByVal strSID As String, ByVal strUserName As String, ByVal strUserPwd As String, ByVal strPort As String) As Boolean
    ...
    '创建ODBC连接
    Set mcnOracle = gobjRegister.GetConnection(strServer, strUserName, strUserName, False)
...
'            Call mobjScript.ExecuteSQL(mclsOracle.DatabaseConnection, mobjScript.SQL(intLoop))
    ...
    '安装过程
    If mobjScript.OpenScriptFile(strPath & "zlMipClientProgram.SQL") Then
        
        lbl(12).Caption = "正在执行过程脚本..."
        pgb.Value = 0
    ...
    With vsf
        For intRow = 1 To .Rows - 1
            If Abs(.TextMatrix(intRow, .ColIndex("选择"))) = 1 Then
                If (vsf.TextMatrix(intRow, vsf.ColIndex("标识")) <> "EMR") Or (vsf.TextMatrix(intRow, vsf.ColIndex("标识")) = "EMR" And mblnSpecialEMR = True) Then
                    If Dir(strPath & .TextMatrix(intRow, .ColIndex("标识")) & "\zlMipClientData" & ".SQL") = "" Then
                        MsgBox strPath & .TextMatrix(intRow, .ColIndex("标识")) & "\zlMipClientData" & ".SQL文件不存在!"
                    Else
...
0:
    With vsf
        ...
        '导入公共部分数据
        If Dir(strPath & "zlMipServerData.db") <> "" Then
            lbl(12).Caption = "正在向服务器导入公共部分消息..."
            vsfStep.Cell(flexcpPicture, intCount, vsfStep.ColIndex("图标")) = imgList.ListImages("执行中").Picture
            Call mclsMipClientManage.CommunicateProxyInstall(strPath & "zlMipServerData" & ".db", rsErr)
            vsfStep.Cell(flexcpPicture, intCount, vsfStep.ColIndex("图标")) = imgList.ListImages("已完成").Picture
            intCount = intCount + 1
        End If
    ...
    '保存注册表
    Call SaveSetting("ZLSOFT", "公共全局\消息平台客户端", "消息服务器连接", txt(6).Text & ";" & txt(7).Text & ";" & txt(9).Text)
    Call SaveSetting("ZLSOFT", "公共全局\消息平台客户端", "EMR连接", txt(1).Text & ";" & txt(2).Text & ";" & txt(5).Text & ";" & txt(3).Text)
    
    lbl(12).Caption = "客户端安装成功！"
...
'        End If
        ...
        With vsf
            For intRow = 1 To .Rows - 1
                If .TextMatrix(intRow, .ColIndex("id")) = 2 And .TextMatrix(intRow, .ColIndex("标识")) = "EMR" And Abs(.TextMatrix(intRow, .ColIndex("选择"))) = 1 Then
                    mblnSpecialEMR = True
                    Exit For
                End If
            Next
...
Private Sub InitVsfSetup()
    ...
    With vsfStep
        .Rows = 2
        intLoop = 1
        .TextMatrix(1, .ColIndex("step")) = "创建数据结构"
        .Cell(flexcpPicture, 1, .ColIndex("图标"), 1, .ColIndex("图标")) = imgList.ListImages("待执行").Picture
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If Not (mfrmErrorInfo Is Nothing) Then
        Unload mfrmErrorInfo
    End If
