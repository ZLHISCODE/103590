...
'## 日    期：
'## 描    述：  解压缩文件
'## 需    要：  zlUZip10.dll（对Unzip32.DLL v5.40进行了修正）
'#########################################################################
...
Private Declare Function Wiz_SingleEntryUnzip Lib "zlUZip10.dll" _
...
Public Declare Sub UzpVersion2 Lib "zlUZip10.dll" (uzpv As UZPVER)

' Object for callbacks:
Private m_cTabUnzip As cTabUnzip
...
Private Function UnzipPrintCallback( _
    ...
    If x > 1 And x < 1024 Then
        ...
        m_cTabUnzip.ProgressReport sFIle
    End If
...
Private Function UnzipPasswordCallBack( _
    ...
    ' Ask for password:
    m_cTabUnzip.PasswordRequest sPassword, bCancel
...
Private Function UnzipReplaceCallback(ByRef fname As CBChar) As Long
    ...
    m_cTabUnzip.OverwriteRequest sFIle, eResponse
...
Private Function UnZipServiceCallback(ByRef mname As CBChar, ByVal x As Long) As Long
    ...
    If x > 1 And x < 1024 Then
       ...
       m_cTabUnzip.Service sInfo, bCancel
       If bCancel Then
