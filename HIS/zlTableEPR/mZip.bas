...
'## 日    期：
'## 描    述：  压缩文件
'## 需    要：  zlZip11.dll（对Zip32.DLL v2.32进行了修正）
'##             - Without encryption, this DLL is renamed to zlzip10.dll
'##             - With encryption, this DLL is renamed to zlzip11.dll
'#########################################################################
...
Private Declare Function ZpInit Lib "zlZip11.dll" (ByRef tUserFn As ZIPUSERFUNCTIONS) As Long ' Set Zip Callbacks
Private Declare Function ZpSetOptions Lib "zlZip11.dll" (ByRef tOpts As ZPOPT) As Long ' Set Zip options
Private Declare Function ZpGetOptions Lib "zlZip11.dll" () As ZPOPT ' used to check encryption flag only
Private Declare Function ZpArchive Lib "zlZip11.dll" (ByVal argc As Long, ByVal funame As String, ByRef argv As ZIPnames) As Long ' Real zipping action

' Object for callbacks:
Private m_cTabZip As cTabZip
...
Public Function VBZip( _
    ...
    Set m_cTabZip = cZipObject
    
    If Not Len(Trim$(m_cTabZip.BasePath)) = 0 Then
       ChDir m_cTabZip.BasePath
    End If
...
Dim bCancel As Boolean
    ...
    If x > 1 And x < 1024 Then
       ...
       End If
       m_cTabZip.Service sInfo, bCancel
       If bCancel Then
            m_bCancel = True
...
Private Function ZipPrintCallback( _
    ...
    If x > 1 And x < 1024 Then
        ...
        m_cTabZip.ProgressReport sFIle
    End If
    ZipPrintCallback = 0
End Function
...
Private Function ZipCommentCallback( _
    ...
    ' Ask for a comment:
    m_cTabZip.CommentRequest sComment, bCancel
...
Private Function ZipPasswordCallback( _
    ...
    ' Ask for password:
    m_cTabZip.PasswordRequest sPassword, maxPasswordLength, m_bPasswordConfirm, bCancel
