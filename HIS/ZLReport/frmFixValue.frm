...
Option Explicit
Public IntSelType As Integer '选择模式：0-下拉框;1-单选框;2-复选框
Public strValues As String '入/出：可选固定值列表串(显示,值|...)
Public bytType As Byte    '入：参数数据类型
Public strName As String '入：参数名称

Private Sub cmdAdd_Click()
    Dim i As Integer, intLen As Integer
    Dim blnExist As Boolean, objItem As ListItem
    
    If InString(txtDisp.Text, "^~,|√") Then
        MsgBox "输入了非法字符！", vbInformation, App.Title
        txtDisp.SetFocus: Exit Sub
    End If
    If InString(txtBand.Text, "^~,|√") Then
        MsgBox "输入了非法字符！", vbInformation, App.Title
...
Private Sub cmdOK_Click()
    ...
    strValues = "" '不好的分隔符
    For i = 1 To lvwValue.ListItems.Count
        strValues = strValues & "|" & lvwValue.ListItems(i).SubItems(2) & lvwValue.ListItems(i).Text & "," & lvwValue.ListItems(i).SubItems(1)
    Next
    strValues = Mid(strValues, 2)

    If InStr(strValues, "√") = 0 Then
        MsgBox "必须设置一个缺省的固定值！", vbInformation, App.Title
        strValues = "": lvwValue.SetFocus: Exit Sub
    End If
...
Private Sub Form_Load()
    ...
    '不好的分隔符
    For i = 0 To UBound(Split(strValues, "|"))
        If Left(Split(Split(strValues, "|")(i), ",")(0), 1) = "√" Then
            Set objItem = lvwValue.ListItems.Add(, , Mid(Split(Split(strValues, "|")(i), ",")(0), 2), , 1)
            objItem.SubItems(2) = "√"
        Else
            Set objItem = lvwValue.ListItems.Add(, , Split(Split(strValues, "|")(i), ",")(0), , 1)
        End If
        objItem.SubItems(1) = Split(Split(strValues, "|")(i), ",")(1)
    Next
    
    fraValue.Caption = "参数：" & IIF(strName = "", "未定义", strName)
    Opt(IntSelType).Value = True
