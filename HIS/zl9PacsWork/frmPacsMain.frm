...
Public Sub ShowStation(ByVal lngModule As Long, owner As Object)
    ...
    mintState = 0
    mblnDBLink = CheckDatamoveRemote
...
On Error GoTo errHandle
    ...
    If Is_ExistReportWriting(curAdviceInf.lngAdviceID) Then
        MsgBoxD Me, "报告已经修改还未签名，不允许完成，请检查", vbInformation, Me.Caption
        Exit Sub
    End If
...
Private Function CheckDatamoveRemote() As Boolean
    Dim strSql As String
    Dim rsTemp As Recordset
    
    strSql = "Select 1 From zlBakSpaces Where 系统 = 100 And 当前 = 1 And Db连接 Is Not Null"
    Set rsTemp = zlDatabase.OpenSQLRecord(strSql, "查询是否dblink连接")
    
    CheckDatamoveRemote = rsTemp.RecordCount > 0
End Function
...
On Error GoTo errH
    ...
    '判断记录集是否找到对应数据
    If blnHaveRs Then
        Call ufgStudyList.AdoData.Find("医嘱ID=" & lngAdviceID, , , 1)
