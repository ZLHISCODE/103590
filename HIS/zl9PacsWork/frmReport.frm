...
'        OutputDebugString "ZLPACS>>AddSign:5 调用签名窗口."
        '调用签名窗口
        Set OneSign = frmEPRSign.ShowMe(Me, mlngPassType, mReportID, lngPatientID, lngPageID, mstrPrivs, lngMaxSignLevel, int开始版, GetSignInfo_NameWithReportImg("", False, int开始版), mlngAdviceID)
...
'            OutputDebugString "ZLPACS>>AddSign:6 多次签名判断."
            ...
            On Error Resume Next
            blTmp = False
            If mblnShowImage = True And gblUseImgSignValid Then
                blTmp = mfrmReportImage.dcmMark.Images.Count > 0
                If blTmp Then
...
'    OutputDebugString "ZLPACS>>subPrintReport:1 开始自定义报表格式打印..."
    ...
    If Not blnNoAsk Then
                '134295 单据格式打印选择框弹出时如若使用鼠标经过显示大图，在大图自动关闭后程序卡死，所以这里强制关闭大图。
        If mblnShowImage = True Then Call mfrmReportImage.CloseImageProcess
...
On Error GoTo errH
        ...
        If strSaveDeviceID <> "" Then
            ...
            If Inet1.hConnection = 0 Then
                If Inet1.FuncFtpConnect(strFtpIp, strFTPUser, strFTPPwd) = 0 Then
                    Call MsgBoxD(Me, "FTP不能正常连接，请检查网络设置。", vbInformation, Me.Caption)
                    Exit Sub
                End If
            End If
        End If
    ...
    lngResult = Inet1.FuncUploadFile(strFTPDirUrl & "/MarkImages/", strLocPath, strFileName)
