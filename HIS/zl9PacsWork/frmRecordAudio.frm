...
Private mstrRegPath As String           '参数存储路径（注册表）
...
On Error GoTo errHandle
    ...
    If cbxRecordDevice.ListCount > 0 Then cbxRecordDevice.ListIndex = GetSetting("ZLSOFT", mstrRegPath, "录音设备序号", 0)
    
    If cbxBufferCount.ListCount > 0 Then cbxBufferCount.ListIndex = GetSetting("ZLSOFT", mstrRegPath, "缓存数量类型", 3)
    
    txtBufferSize.Text = GetSetting("ZLSOFT", mstrRegPath, "缓存大小", 1024)
    
    lngCompress = GetSetting("ZLSOFT", mstrRegPath, "压缩比率", 64)
...
On Error Resume Next
    ...
    Call SaveSetting("ZLSOFT", mstrRegPath, "录音设备序号", Val(cbxRecordDevice.Text))
    Call SaveSetting("ZLSOFT", mstrRegPath, "缓存数量类型", Val(cbxBufferCount.Text))
    Call SaveSetting("ZLSOFT", mstrRegPath, "缓存大小", txtBufferSize.Text)
    Call SaveSetting("ZLSOFT", mstrRegPath, "压缩比率", Val(cbxCompressRate.Text))
End Sub
...
On Error GoTo errHandle
    '加载声音格式
    mciSound.FormatTag = GetSetting("ZLSOFT", mstrRegPath, "格式类型", 1)
    mciSound.SampleRate = GetSetting("ZLSOFT", mstrRegPath, "采样频率", 44100)
    mciSound.BitsPerSample = GetSetting("ZLSOFT", mstrRegPath, "采样位数类型", 1)
    mciSound.Channels = GetSetting("ZLSOFT", mstrRegPath, "通道类型", 1)
...
On Error Resume Next
    '保存声音格式
    Call SaveSetting("ZLSOFT", mstrRegPath, "采样频率", mciSound.SampleRate)
    Call SaveSetting("ZLSOFT", mstrRegPath, "采样位数类型", mciSound.BitsPerSample)
    Call SaveSetting("ZLSOFT", mstrRegPath, "通道类型", mciSound.Channels)
    Call SaveSetting("ZLSOFT", mstrRegPath, "格式类型", mciSound.FormatTag)
End Sub
...
On Error GoTo errHandle
    ...
    Exit Sub
errHandle:
    Call MsgBoxD(Me, err.Description)
End Sub
...
Private Sub Form_Load()
    ...
    '设置注册表参数保存路径
    mstrRegPath = "公共模块\" & App.ProductName & "\frmRecordAudio"
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
    If mciSound.RecordState <> adsStop Then
        ...
        lngState = MsgBoxD(Me, "录音尚未结束，是否停止录音并保存？", vbYesNoCancel, gstrSysName)
...
On Error GoTo errHandle
    ...
    If Dir(mstrRecordBufferFile) = "" Then
        '弹出提示框
        Call MsgBoxD(Me, "未找到需要另存的音频文件。", vbOKOnly Or vbInformation, gstrSysName)
