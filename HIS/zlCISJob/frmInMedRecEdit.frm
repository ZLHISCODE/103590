        ...
        Select Case UCase(Nvl(rsTmp!信息名))
            ...
            Case "昏迷时间"
                ...
                If Not strTmp Like "*,*,*|*,*,*" Then
                    strTmp = ",,|,,"
                End If
                strTmp = Replace(strTmp, "|", ","): varTmp = Split(strTmp, ",")
                txtInfo(txt入院前天).Text = varTmp(0)
                txtInfo(txt入院前小时).Text = varTmp(1)
                txtInfo(txt入院前分钟).Text = varTmp(2)
                txtInfo(txt入院后天).Text = varTmp(3)
                txtInfo(txt入院后小时).Text = varTmp(4)
                txtInfo(txt入院后分钟).Text = varTmp(5)
            Case "抢救病因"
