        ...
        For i = 1 To rsTmp.RecordCount
            Select Case rsTmp!项目名称
                ...
                Case "收缩压"
                    ...
                    If Not IsNull(rsTmp!项目单位) Then Call Cbo.SetIndex(cboBpUnit.hwnd, IIf("" & rsTmp!项目单位 = "mmHg", 0, 1))
                Case "舒张压"
                    txtInfo(I舒张压).Text = rsTmp!记录内容
                    txtInfo(I舒张压).Tag = rsTmp!记录内容
                    If Not IsNull(rsTmp!项目单位) Then Call Cbo.SetIndex(cboBpUnit.hwnd, IIf("" & rsTmp!项目单位 = "mmHg", 0, 1))
                Case "血压单位"
                    If Not IsNull(rsTmp!记录内容) Then Call Cbo.SetIndex(cboBpUnit.hwnd, IIf("" & rsTmp!记录内容 = "mmHg", 0, 1))
            End Select
            rsTmp.MoveNext
        Next
    End If
    ...
    If cboBpUnit.List(cboBpUnit.ListIndex) <> cboBpUnit.Tag Then
        ...
        If Not mblnNoCheck And mblnSaveNow Then
            ...
            If strTmp <> mstrPreState Then
                Call zlDatabase.ExecuteProcedure(strTmp, "生命体征")
                mstrPreState = strTmp
            End If
        End If
    End If
...
'功能：返回门诊生命体征填写的SQL
    ...
    InSideSaveSQL = GetRetrunSQL(mlng病人ID, mlng挂号ID)
End Function
