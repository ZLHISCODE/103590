...
Public Function ShowMe(ByVal objForm As Object, ByVal int场合 As Integer) As Boolean
'参数：mint场合=1门诊，2住院
    mint场合 = int场合
    Me.Show , objForm
End Function
...
'加载权限比操作员低的医生
    ...
    strSQL = "Select DISTINCT a.Id, A.性别" & IIf(Val(cboDept.ItemData(cboDept.ListIndex)) = -1, "", ",b.部门ID,e.名称 as 所属部门") & ",a.姓名,a.专业技术职务, Upper(zlSpellCode(a.姓名)) As 拼音简码, Upper(Zlwbcode(a.姓名)) As 五笔简码" & vbNewLine & _
            "From 人员表 A, 部门人员 B, 人员性质说明 D,部门表 E" & IIf(chkIsShowAll.Value, " ,(Select Distinct 开嘱医生 From 病人医嘱记录 F Where  f.审核状态 = 1 and f.医嘱状态=1 And F.开嘱时间 Between [4] And [5] And f.病人来源=[3] And f.诊疗类别 ='K') F", "") & vbNewLine & _
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, mlngLevel, Val(cboDept.ItemData(cboDept.ListIndex)), IIf(optOccasion(0).Value, 2, 1), CDate(dtpTimeY(0).Value), CDate(dtpTimeY(1).Value + 1 - 1 / 24 / 60 / 60))
...
'功能：修改已审核未通过的审核说明
    ...
    If Val(rptDoc.SelectedRows(0).Record(COL_人员ID).Value) = UserInfo.ID Then
        MsgBox "不能修改自己申请的输血医嘱", vbInformation, Me.Caption
        Exit Sub
    End If
    strDate = "To_Date('" & Format(zlDatabase.Currentdate, "yyyy-MM-dd HH:mm:ss") & "','YYYY-MM-DD HH24:MI:SS')"
        ...
        With vsAudit
            ...
            For i = 0 To UBound(varArr)
                If .TextMatrix(Val(varArr(i)), col_医嘱ID) <> "" And Val(varArr(i)) <> 0 Then
                    strSQL = "Zl_医嘱审核管理_Update(" & Val(.TextMatrix(Val(varArr(i)), col_医嘱ID)) & "," & strDate & ",'" & .TextMatrix(Val(varArr(i)), col_审核说明) & "',2)"
    ...
    With vsAudit
        ...
        For i = lngBegin To lngEnd
            ...
            If Val(.Cell(flexcpData, i, col_选择) & "") <> "0" Then
                int状态 = IIf(Val(.Cell(flexcpData, i, col_选择) & "") = 1 And gbln血库系统, 3, Val(.Cell(flexcpData, i, col_选择) & ""))
...
'    blnIsRecursive:是否递归，默认为要递归
    ...
    '取消审核
    If strIDs <> "" Then
        strSQL = "Zl_医嘱审核管理_Cancel('" & strIDs & "',2)"
        Call zlDatabase.ExecuteProcedure(strSQL, Me.Caption)
    End If
    gcnOracle.CommitTrans: blnTrans = False
...
Private Sub Form_Load()
    ...
    'vsFlexGrid
    '-----------------------------------------------------
    strHead = ",450,1; ;审核说明,2560,1;审核时间;病人姓名,1000,1;医嘱内容,3400,1;期效,500,1;单量;输血时间,1550,1;开始时间,1550,1;医嘱ID;相关ID ; 性别;年龄;体重;入院时间;床号; 组号; 病况;护理等级;病人ID; 主页ID; 组ID;诊疗类别 ;病人来源;签名id;医嘱状态;挂号单;审核状态"
...
Private Sub DelNotHavePrivs(ByVal strPatis As String, ByVal strDate As String, ByVal bln门诊 As Boolean)
'功能：根据当前传入的病人信息，删除操作员不能审核的项目
'参数：strPatis   '病人信息字符串：病人ID1:主页ID1,病人ID2:主页ID2···
'      strDate=需要排除的日期
    Dim lngBlood As Long   '拥有的审核的被血量,基本（800ML以下），科主任(800-1600ML)，医务科(1600ML以上)
    Dim strSQL As String, strTmp As String, rsTmp As Recordset
    Dim i As Long
    
    If gbln输血申请三级审核 Then
        ...
        strSQL = "Select * from (Select /*+Rule */ a.病人id, a.主页id, to_char(Nvl(Trunc(a.手术时间), Trunc(To_Date(a.标本部位, 'YYYY-MM-DD hh24:MI:ss'))),'YYYY-MM-DD') As 输血日期," & vbNewLine & _
                "       Sum(a.总给予量 * Nvl(b.计算系数, 0)) As 总量,A.挂号单" & vbNewLine & _
                "From 病人医嘱记录 A, 诊疗项目目录 B" & vbNewLine & _
                "Where a.诊疗项目id = b.Id And b.类别 = 'K' And a.医嘱状态 Not In (-1, 2, 4) And" & vbNewLine & _
                "      (a.病人id, " & IIf(bln门诊, "A.挂号单", "a.主页id") & ") In (Select TO_number(C1), C2 From Table(f_str2list2([3]))) And" & vbNewLine & _
                "      Nvl(Trunc(a.手术时间), Trunc(To_Date(a.标本部位, 'YYYY-MM-DD hh24:MI:ss'))) In" & vbNewLine & _
                "      (Select To_Date(Column_Value, 'yyyy-MM-dd') From Table(f_Str2list([1])))" & vbNewLine & _
                "Group By a.病人id, a.主页id,A.挂号单, Nvl(Trunc(a.手术时间), Trunc(To_Date(a.标本部位, 'YYYY-MM-DD hh24:MI:ss')))) Where 总量>=[2]"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, strDate, lngBlood, strPatis)
        If rsTmp.RecordCount > 0 Then
            With vsAudit
                For i = .Rows - 1 To 1 Step -1
                    If bln门诊 Then
                        rsTmp.Filter = "病人ID=" & Val(.TextMatrix(i, col_病人Id)) & " And 挂号单='" & .TextMatrix(i, col_挂号单) & "' And 输血日期='" & Format(.TextMatrix(i, COL_输血时间), "YYYY-MM-DD") & "'"
                    Else
                        rsTmp.Filter = "病人ID=" & Val(.TextMatrix(i, col_病人Id)) & " And 主页ID=" & Val(.TextMatrix(i, col_主页ID)) & " And 输血日期='" & Format(.TextMatrix(i, COL_输血时间), "YYYY-MM-DD") & "'"
                    End If
                    If rsTmp.RecordCount > 0 Then
                        .RemoveItem i
                    End If
...
'参数：是否加载已审核医嘱,为空为加载待审核医嘱
    ...
    strSQL = "Select a.Id, a.相关id, Nvl(a.相关id, a.Id) As 组id, a.诊疗类别, Null As 选择, Null As 输入, A.姓名,A.性别,A.年龄, p.当前床号 As 床号," & vbNewLine & _
            ...
            "       a.主页id, a.诊疗项目id, a.频率次数, a.频率间隔, a.间隔单位, b.计算单位 As 单量单位, e.体重,e.入院日期,e.入院病况,f.名称 as 护理等级,a.病人来源,A.挂号单" & vbNewLine & _
            ...
            IIf(blnIsAudited, ", (Select 医嘱id,操作时间,操作说明,操作类型,签名ID" & vbNewLine & _
                            ...
                            "       and C.操作类型 in(11,12,14,15) And C.操作人员 =[2])" & vbNewLine & _
                            "Where Top = 1)  C", "") & _
            ...
            " And A.开嘱时间 between [6] and [7]  And a.医嘱状态 = 1 And a.审核状态 = 1 ") & vbNewLine & _
        ...
        If rsTmp.RecordCount > 0 Then
            ...
            Do While Not rsTmp.EOF
                ...
                If blnIsAudited Then
                    ...
                    .Cell(flexcpData, i, col_选择) = int选择
                    .Cell(flexcpPicture, i, col_选择) = imgAdvice.ListImages(int选择).Picture
            ...
            '删除不能审核的内容
            Call DelNotHavePrivs(strPatis, strDate, optOccasion(1).Value)
            If .Rows = 1 Then .AddItem ""
            vsAudit.Cell(flexcpBackColor, 1, IIf(blnIsAudited, 1, 0), .Rows - 1, col_审核说明) = &HFAEADA
            If blnIsAudited Then
                For j = 1 To .Rows - 1
...
Private Sub vsAudit_BeforeEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    Else
        With vsAudit
            If Val(.Cell(flexcpData, Row, col_选择) & "") = "1" Or .TextMatrix(1, col_医嘱ID) & "" = "" Or _
        ...
        If tbcSub.Selected.Tag = "已审核" Then
            If .MouseCol = col_取消选择 And .MouseRow = .FixedRows - 1 Then
                ...
                For i = 1 To .Rows - 1
                    ...
                    If .ColData(col_取消选择) = "Check" Then
                        If Not (.TextMatrix(i, col_医嘱状态) & "" <> "1" And .TextMatrix(i, col_医嘱状态) & "" <> "") And _
                            Not (gbln血库系统 And InStr(",2,5,", "," & Val(.TextMatrix(i, col_审核状态)) & ",") > 0) Then
