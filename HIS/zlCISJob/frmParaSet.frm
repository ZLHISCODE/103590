...
Private Sub cmdOK_Click()
    ...
    Call zlDatabase.SetPara("检查对应报表", cboPacs.Tag, glngSys, mlngModul, True)
    Call zlDatabase.SetPara("检验对应报表", cboLis.Tag, glngSys, mlngModul, True)
    Call zlDatabase.SetPara("检验报告打印", optLIS(0).Tag, glngSys, mlngModul, True)
    Unload Me
End Sub
...
Private Sub Form_Load()
    ...
    strPara = zlDatabase.GetPara("检验对应报表", glngSys, mlngModul, "", cboLis, True) '报表名称,系统号,报表编号
    If strPara <> "" Then Call Cbo.Locate(cboLis, Split(strPara, ",")(0))
    strPara = zlDatabase.GetPara("检查对应报表", glngSys, mlngModul, "", cboPacs, True)
    If strPara <> "" Then Call Cbo.Locate(cboPacs, Split(strPara, ",")(0))
    If IsSysSetUp(2500) Then
        fraPara.Visible = True
        Me.Height = 5625
        strPara = zlDatabase.GetPara("检验报告打印", glngSys, mlngModul, "0", optLIS, True)
