...
'加载权限比操作员低的医生
    ...
    strSQL = "Select DISTINCT a.Id, A.性别" & IIf(Val(cboDept.ItemData(cboDept.ListIndex)) = -1, "", ",b.部门ID,e.名称 as 所属部门") & ",a.姓名,a.专业技术职务,Decode(c.级别,1,'非限制使用',2,'限制使用',3,'特殊使用','无使用权限') as 抗菌用药权限, Upper(zlSpellCode(a.姓名)) As 拼音简码, Upper(Zlwbcode(a.姓名)) As 五笔简码" & vbNewLine & _
            "From 人员表 A, 部门人员 B, 人员抗菌药物权限 C, 人员性质说明 D,部门表 E" & IIf(chkIsShowAll.Value, " ,(Select Distinct 开嘱医生 From 病人医嘱记录 F Where  f.审核状态 = 1 And F.开嘱时间 Between [4] And [5] and f.医嘱状态=1 And f.诊疗类别 In ('5','6')) F", "") & vbNewLine & _
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, mlngLevel, Val(cboDept.ItemData(cboDept.ListIndex)), IIf(optOccasion(0).Value, 1, 2), CDate(dtpTimeY(0).Value), CDate(dtpTimeY(1).Value + 1 - 1 / 24 / 60 / 60))
...
Private Sub Form_Load()
    ...
    'vsFlexGrid
    '-----------------------------------------------------
    strHead = ",450,1; ;审核说明,2560,1;审核时间;病人姓名,1000,1;医嘱内容,3500,1;期效,500,1;总量;单量;频率,1350,1;给药途径,1350,1;用药目的,1050,1;用药理由,2500,1;开始时间,2000,1;执行时间方案,1350,1;医嘱ID;相关ID ; 性别;年龄;体重;入院时间;床号; 组号; 病况;护理等级;病人ID; 主页ID;挂号单; 组ID;诊疗类别 ;病人来源;挂号单据;签名id;医嘱状态"
...
'参数：是否加载已审核医嘱,为空为加载待审核医嘱
    ...
    strSQL = "Select a.Id, a.相关id, Nvl(a.相关id, a.Id) As 组id, Nvl(x.序号, a.序号) As 组号, a.诊疗类别, Null As 选择, Null As 输入, A.姓名, p.当前床号 As 床号," & vbNewLine & _
            ...
            "               Round(a.总给予量 / Decode(a.病人来源, 2, d.住院包装, d.门诊包装), 5) || Decode(a.病人来源, 2, d.住院单位, d.门诊单位)) As 总量," & vbNewLine & _
            ...
            IIf(blnIsAudited, "", "  And A.开嘱时间 between [6] and [7] And Not Exists (Select 1 From 病人医嘱状态 I Where i.医嘱id = a.Id And i.操作类型 In (11,12) And i.操作人员 = [2]) ") & _
    ...
    With vsAudit
        ...
        Else
            '如果同组药物中有等级比当前操作员的权限更高时，则删除整组
            If Not blnIsAudited Then Call CheckIsExceed
