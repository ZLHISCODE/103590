...
'  blnLock：True加锁；False解锁
    ...
    strSQL = zlStr.FormatString("Zl_处方审查记录_Lock([1], '[2]', 1, [3])", _
                    IIf(blnLock, 1, 0), _
                    mstrPCName, _
...
'返回：True成功；False失败
    ...
    If bytFun = 1 Then
        '开启
        strSQL = zlStr.FormatString("ZL_处方审查参数_SAVE(2, '[1]', 1, 1, Null)", mstrPCName)
    ...
    Else
        ...
        '更新是否开启审方、最后操作时间
        strSQL = zlStr.FormatString("ZL_处方审查参数_SAVE(2, '[1]', 1, 0, Null)", mstrPCName)
...
'返回：收费项目ID
    ...
    If rsTemp.EOF = False Then
        GetExpenseID = NVL(rsTemp!药品id, 0)
    End If
    rsTemp.Close
