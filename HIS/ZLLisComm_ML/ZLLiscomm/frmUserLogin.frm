...
Private Sub CDM确认_Click()
    ...
    If Len(Trim(strPassword)) = 0 Then
        ...
        GoTo InputError
    End If
    blnTransPassword = Not (UCase(strUserName) = "SYS" Or UCase(strUserName) = "SYSTEM") Or mbln转换
    Set gcnOracle = gobjRegister.GetConnection(strServerName, strUserName, strPassword, blnTransPassword)
    ...
    If Not TXT密码.Enabled Then
        If Trim(FrmChangePass.TXT原密码.Text) <> Trim(FrmChangePass.TXT新密码.Text) Then
            ...
            '保存新密码
            If gobjRegister.UpdateUserPassword(gcnOracle, strUserName, Trim(FrmChangePass.TXT新密码.Text), blnTransPassword, strError) Then
