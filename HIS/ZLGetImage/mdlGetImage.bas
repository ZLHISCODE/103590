...
'    pstrLogPath = GetSetting("ZLSOFT", strRegPath, "日志路径", "")
    pstrLogPath = App.Path
    lngAppSoft = InStr(UCase(pstrLogPath), "APPSOFT")
    If lngAppSoft > 0 Then
        pstrLogPath = Mid(pstrLogPath, 1, lngAppSoft + 7 - 1) & "\Log\日志跟踪\Pacs_后台获取观片图像"
    End If
...
Public Sub WriteCommLog(logSubName As String, logTitle As String, logDesc As String, lngLogLevel As Long)
'------------------------------------------------
'功能：记录通讯日志
...
'------------------------------------------------
    ...
    If pblnLogEnable = True Then        '启动了记录日志，才记录当前的日志
        ...
        If plngLogLevel >= lngLogLevel Then
            ...
            intHour = intHour / 2
            intHour = intHour * 2
            strFileName = pstrLogPath & "\后台获取观片图像_" & Format(Date, "yyyymmdd") & "_" & intHour & ".log"
