...
100     Err = 0
102     DoEvents
103     If gcnOracle Is Nothing Then Set gcnOracle = New ADODB.Connection
104     With gcnOracle
...
146                 WriteLog "由于用户已经被禁用，无法登录。"
                Else
148                 WriteLog CStr(Erl()) & "," & strError
                End If
...
104         WriteLog "未能通HIS系统注册相关检查，请先检查ZLLIS能否正常运行！"
            Exit Function
        End If
106     strUnti = gobjComLib.zlRegInfo("单位名称", , -1)
...
144             WriteLog "试用日期错误！"
            End If
        Else
146         WriteLog "部件所在目录(" & App.Path & ")缺少授权文件（RegFile.ini）！"
        End If
        Exit Function
hErr:
...
158     lngPatientID = "" & rsTmp("病人ID")
        ...
        '提取申请单
160     gstrSql = "Select Distinct A.相关id As ID, D.姓名, D.性别, D.年龄, A.病人来源, D.门诊号, D.住院号, E.编码 As 申请科室编码, E.名称 As 申请科室名称, F.编号 As 医生编号,A.开嘱医生, A.开嘱时间,D.险类,C.样本条码,C.条码打印,D.当前床号 " & vbNewLine & _
                    "From 病人医嘱记录 A, 诊疗项目目录 B, 病人医嘱发送 C, 病人信息 D, 部门表 E,人员表 F" & vbNewLine & _
                    "Where A.诊疗项目id = B.ID And B.类别 = 'C' And A.ID = C.医嘱id And A.相关id Is Not Null And C.执行状态 = 0 And A.病人id = [1] And" & vbNewLine & _
                    "      A.病人id = D.病人id And A.开嘱科室id = E.ID And A.医嘱状态 = 8 And A.开嘱医生=F.姓名(+)"
...
174         GetApplication = Mid(strData, 2)
        ...
        Exit Function
errH:
176     WriteLog CStr(Erl()) & "行出现错误，" & Err.Number & " " & Err.Description
End Function
...
118         gstrSql = "Select Distinct A.相关id As ID, D.姓名, D.性别, D.年龄, A.病人来源, D.门诊号, D.住院号, E.编码 As 申请科室编码, E.名称 As 申请科室名称, F.编号 As 医生编号,A.开嘱医生, A.开嘱时间,D.险类,C.样本条码,C.条码打印,G.出院病床 as 床号 " & vbNewLine & _
                        ...
                        "      And G.出院日期 Is Null And E.编码  = [1] And A.开嘱时间 between [2] And [3]  Order By A.开嘱时间"
...
116         gstrSql = "Select A.诊疗项目id As ID, B.编码 As 诊疗项目编码, B.名称 As 诊疗项目名称, C.编码 As 执行科室编码, C.名称 As 执行科室名称, Sum(E.数量 * E.单价) As 实收金额," & vbNewLine & _
                "       Sum(E.单价) As 标准单价, D.计费状态, '0' As 是否采集, F.记录状态, A.紧急标志, A.标本部位" & vbNewLine & _
                ...
                "Group By A.诊疗项目id, B.编码, B.名称, C.编码, C.名称, D.计费状态, F.记录状态, A.紧急标志, A.标本部位" & vbNewLine & _
                "Union All" & vbNewLine & _
                "Select A.诊疗项目id As ID, B.编码 As 诊疗项目编码, B.名称 As 诊疗项目名称, C.编码 As 执行科室编码, C.名称 As 执行科室名称, Sum(E.数量 * E.单价) As 实收金额," & vbNewLine & _
                "       Sum(E.单价) As 标准单价, D.计费状态, '1' As 是否采集, F.记录状态, A.紧急标志, A.标本部位" & vbNewLine & _
                "From 住院费用记录 F, 病人医嘱计价 E, 病人医嘱发送 D, 病人医嘱记录 A, 诊疗项目目录 B, 部门表 C" & vbNewLine & _
                "Where D.医嘱id = F.医嘱序号(+) And D.No = F.No(+) And D.记录性质 = F.记录性质(+) And D.记录序号 = F.序号(+) And F.记录状态(+) <> 2 And" & vbNewLine & _
                "      A.Id = E.医嘱id And A.诊疗类别 = 'E' And A.Id = D.医嘱id And A.诊疗项目id = B.Id And A.执行科室id = C.Id And A.Id = [1]" & vbNewLine & _
                "Group By A.诊疗项目id, B.编码, B.名称, C.编码, C.名称, D.计费状态, F.记录状态, A.紧急标志, A.标本部位"
        Else
118         gstrSql = "Select A.诊疗项目id As ID, B.编码 As 诊疗项目编码, B.名称 As 诊疗项目名称, C.编码 As 执行科室编码, C.名称 As 执行科室名称, Sum(E.数量 * E.单价) As 实收金额," & vbNewLine & _
                "       Sum(E.单价) As 标准单价, D.计费状态, '0' As 是否采集, F.记录状态, A.紧急标志, A.标本部位" & vbNewLine & _
...
116         strErrInfo = "检验仪器未指定对应的检验小组！"
            Exit Function
        End If
118     dtSampleDate = CDate(Format(gobjComLib.zlDatabase.Currentdate, "yyyy-MM-dd HH:mm:ss"))
...
120     strSQL = "Select ID, 姓名, 性别, 年龄, NO, 项目id, 结果, 标志, 结果参考, 紧急, 采样时间, 采样人, Rownum As 排列序号, 诊疗项目id," & vbNewLine & _
                ...
                "             and (Y.仪器id + 0 = [1] Or (Y.仪器id Is Null And F.执行部门id = [3])) And nvl(F.执行状态,0) = 0  And A.相关ID = [2]" & vbNewLine & _
                "       Order By M.编码, G.排列序号)"

122     Set rsTmp = gobjComLib.zlDatabase.OpenSQLRecord(strSQL, "zlLISRegister", lngDeviceID, lngID, lngDeptID)
...
138     strSQL = "Select 核收人,核收时间 from 检验标本记录 where 核收时间 Between [3] and [4] and 仪器ID=[1] and 标本序号=[2] "
...
144         strErrInfo = strSampleNo & "号标本已存在！" & vbNewLine & "核收人：" & rsItem!核收人 & " 核收时间:" & Format(rsItem!核收时间, "yyyy-MM-dd HH:mm:ss")
            Exit Function
        End If
146     gstrSql = "Select B.病人id, B.主页id, B.序号, B.婴儿姓名, B.婴儿性别" & vbNewLine & _
                        "From 病人医嘱记录 A, 病人新生儿记录 B" & vbNewLine & _
                        "Where A.病人id = B.病人id And A.主页id = B.主页id And A.婴儿 = B.序号 And A.相关id = [1] And Rownum = 1"
148     Set rs = gobjComLib.zlDatabase.OpenSQLRecord(gstrSql, "zlLISRegister", lngID)
...
162         str年龄 = Trim("" & rsTmp("年龄"))
        ...
        '读出检验项目
164     gstrSql = "select distinct 医嘱内容 from 病人医嘱记录 a , 病人医嘱发送 b, 检验报告项目 c , 检验仪器项目 d " & vbNewLine & _
                  "  where a.id = b.医嘱ID and a.诊疗项目ID = c.诊疗项目ID and " & vbNewLine & _
                  "  c.报告项目ID = d.项目ID(+) and a.相关id=[1] "
166     Set rsItem = gobjComLib.zlDatabase.OpenSQLRecord(gstrSql, "读取检验内容", lngID)
...
178     gstrSql = "ZL_检验标本记录_标本核收(" & lngKey & "," & _
...
180     gobjComLib.zlDatabase.ExecuteProcedure gstrSql, "生成条码标本"
...
194         gstrSql = "Zl_检验普通结果_Write(" & lngKey & "," & _
                lngDeviceID & ",'" & strItemRecords & "',0,0)"
196         gobjComLib.zlDatabase.ExecuteProcedure gstrSql, "生成标本"
        End If
    
198     zlLISRegister = True
        Exit Function
errH:
200     strErrInfo = CStr(Erl()) & "," & Err.Number & " " & Err.Description
201     WriteLog "zlLISRegister " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
