...
'功能：打开项目选择器
    ...
    ElseIf mintType = 4 Then
        ...
        If mlng项目ID > 0 Then '指定了诊疗单据
            strSQLItem = "From 诊疗项目目录 A,病历单据应用 B,检验项目参考 C,检验报告项目 D " & _
...
'                " And (B.诊疗项目ID is Null Or B.应用场合=" & mint服务对象 & ")"
        ...
        strSQL = "Select Distinct 0 as 末级,ID,上级ID,编码,名称,' ' As 操作类型,0 As 病历文件ID,' ' As 标本部位" & _
            " From 诊疗分类目录 Where 类型=5" & _
            " Start With ID In (Select A.分类ID " & strSQLItem & ") Connect by Prior 上级ID=ID"
        strSQL = strSQL & " Union ALL" & _
            " Select Distinct 1 as 末级,A.ID,分类ID as 上级ID,A.编码,A.名称,A.操作类型,B.病历文件ID,A.标本部位 " & strSQLItem & " Order By 编码"
        ...
        If rsTmp Is Nothing Then
            ...
            Exit Sub
        End If
        If rsTmp("操作类型") = "微生物" And vsExt.Rows > 2 Then
...
'参数：mstrExtData=包含检查部位的信息,为空时表示新输入检查组合项目
    ...
    '按照设置的部位顺序号
    strSQL = "Select A.类别,A.编码,A.名称,A.标本部位,B.*" & _
...
'参数：mstrExtData=包含缺省的检验项目的信息,为空时表示新输入检验项目
    ...
    If strItems <> "" Then
        If mlng项目ID > 0 Then '指定了诊疗单据
            strSQL = "Select A.* From 诊疗项目目录 A,病历单据应用 B " & _
                ...
                " Order by A.编码"
        Else
            strSQL = "Select A.*,B.病历文件ID From 诊疗项目目录 A,病历单据应用 B " & _
        ...
        ElseIf mintType = 4 Then
            ...
            If mlng项目ID > 0 Then '指定了诊疗单据
                strSQL = "Select Distinct A.ID,A.编码,A.名称,A.操作类型 as 检验类型,A.标本部位" & _
                    " From 病历单据应用 B,检验项目参考 D,检验报告项目 E,(" & strSQL & ") A" & _
