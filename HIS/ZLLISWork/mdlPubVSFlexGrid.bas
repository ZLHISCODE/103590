...
Public Function vfgLoadFromRecord(ByRef objVfg As VSFlexGrid, _
    ...
    With objVfg
        ...
        Do Until rsTmp.EOF
            For i = 0 To rsTmp.Fields.Count - 1
                If InStr("" & rsTmp.Fields(i).Name, "应收金额") > 0 Or InStr("" & rsTmp.Fields(i).Name, "实收金额") > 0 Then
                    .TextMatrix(.Rows - 1, i) = CStr(Format("" & rsTmp.Fields(i).Value, "0.00"))
                ElseIf IsNumeric("" & rsTmp.Fields(i).Value) Then
                    .TextMatrix(.Rows - 1, i) = CStr(IIf(Left("" & rsTmp.Fields(i).Value, 1) = ".", "0" & "" & rsTmp.Fields(i).Value, "" & rsTmp.Fields(i).Value))
                Else
...
Public Function vfgFindInsertRow(ByRef objVfg As VSFlexGrid, strField As String, FindstrValue, Optional strErr As String) As Long
    '功能       查找标本号应该插入的位置
    '参数
    '           objVfg      VSF对象
    '           strField    字段
    ...
    With objVfg
        For lngLoop = 1 To .Rows - 1
            ...
            If Format(Trim(.TextMatrix(lngLoop, .ColIndex(strField))), "000000000000") > Format(Trim(FindstrValue), "000000000000") Then
'                .Row = lngLoop
                vfgFindInsertRow = lngLoop
                Exit Function
            End If
        Next
    End With
    Exit Function
errH:
    strErr = "出错函数(vfgFindInsertRow),出错信息:" & Err.Number & " " & Err.Description
End Function
