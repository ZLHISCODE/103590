...
Begin VB.Form frmClientsUpgrade 
   ...
   Begin VB.Label lblNote 
      ...
      Caption         =   "对站点部件升级进行设置和收集文件服务系统的最新部件信息."
      Height          =   345
      Left            =   885
      TabIndex        =   4
      Top             =   780
...
Private Sub chkAllSel_Click()
    ...
    With vsClients
        .Cell(flexcpChecked, 1, .ColIndex("升级"), .Rows - 1, .ColIndex("升级")) = IIf(Me.chkAllSel.value = 1, True, False)
    End With
    mblnChange = True
    Call SetCtlEnabled
...
Private Sub cmd保存_Click()
    If mblnChange Then
        ...
        '如果需要更新类型,在下面处理LoadClientsInfor
        If mblnTypeChange = False Then
             Call LoadClientsInfor
        End If
    End If
...
Private Sub Form_Resize()
    ...
    With chkAllSel
        .Left = cmdUpdateS.Left - .Width - 50
    End With
    
    With txtSearch
        .Width = chkAllSel.Left - .Left - 100
    End With
...
Private Sub LoadClientsInfor(Optional blnFilter As Boolean = False)
    '---------------------------------------------------------------------------------------------
    '功能：加载站点信息
    ...
    strSQL = "" & _
    "   Select 工作站, ip, cpu, 内存, 硬盘, 操作系统, 部门,zlspellcode(部门) as 部门简码, 用途,Decode(升级情况,0,'未升级',1,'成功',2,'失败') as 升级情况,升级情况 as 升级状态, 说明, " & _
    "           升级标志, 收集标志, 禁止使用, 连接数, 升级服务器,FTP服务器,预升时点,预升完成" & _
    "   From zlClients" & _
    "   Order by IP"
    
    If blnFilter = False Or mrsClients Is Nothing Then
    ...
    If strKey <> "" Or blnFilter = False Then
...
'            strKey = IIf(Val(strKey) = 0, "", strKey)
            ...
            If Nvl(mrsClients!升级状态, 0) = 2 Then
                '红色
                .Cell(flexcpForeColor, i, .ColIndex("升级情况"), i, .ColIndex("升级情况")) = vbRed
            ElseIf Nvl(mrsClients!升级状态, 0) = 1 Then
                '蓝色
                .Cell(flexcpForeColor, i, .ColIndex("升级情况"), i, .ColIndex("升级情况")) = vbBlue
            Else
                '黑色
                .Cell(flexcpForeColor, i, .ColIndex("升级情况"), i, .ColIndex("升级情况")) = 0
            End If
            
            '使用颜色标识预升是否完成!
            If Nvl(mrsClients!预升完成, 0) = 0 Then
            ...
            If Val(Nvl(mrsClients!升级标志)) = 1 Then
                .Cell(flexcpChecked, i, .ColIndex("升级")) = True
            Else
                .Cell(flexcpChecked, i, .ColIndex("升级")) = False
            End If
            i = i + 1
            mrsClients.MoveNext
        Loop
    ...
    With vsClients
        For i = 1 To .Rows - 1
            If Trim(.TextMatrix(i, .ColIndex("工作站"))) <> "" Then
                ...
                int升级 = Abs(Val(.TextMatrix(i, .ColIndex("升级"))))
