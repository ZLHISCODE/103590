...
Begin VB.Form frmClientUpgradeFileManage 
   BackColor       =   &H80000004&
   BorderStyle     =   0  'None
   ClientHeight    =   6615
   ClientLeft      =   0
   ...
   Begin VB.PictureBox picMain 
      ...
      Top             =   1650
      Width           =   9450
      Begin VB.PictureBox picFileList 
         BorderStyle     =   0  'None
         Height          =   1515
         Left            =   75
         ScaleHeight     =   1515
         ...
         Begin VSFlex8Ctl.VSFlexGrid vsfFileList 
            ...
            FormatString    =   $"frmClientUpgradeFileManage.frx":0000
            ScrollTrack     =   0   'False
            ScrollBars      =   3
            ScrollTips      =   0   'False
            MergeCells      =   0
         ...
         End
      End
      Begin VB.PictureBox picExpFileList 
         BorderStyle     =   0  'None
         Height          =   1515
         Left            =   75
         ScaleHeight     =   1515
         ...
         Begin VSFlex8Ctl.VSFlexGrid vsfExpFileList 
            ...
            ColWidthMax     =   0
            ExtendLastCol   =   0   'False
            FormatString    =   $"frmClientUpgradeFileManage.frx":0161
            ScrollTrack     =   0   'False
            ScrollBars      =   3
            ScrollTips      =   0   'False
            MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmClientUpgradeFileManage"
...
Private Sub cmdRepairFiles_Click()
    Dim frmRepair As New frmClientUpgradeFileRepair
...
Private Sub picExpFileList_Resize()
    vsfExpFileList.Move 0, 0, picExpFileList.Width, picExpFileList.Height
End Sub

Private Sub picFileList_Resize()
    vsfFileList.Move 0, 0, picFileList.Width, picFileList.Height
End Sub

Private Sub picMain_Resize()
    tbcPage.Move 0, 0, picMain.Width, picMain.Height
End Sub
...
Private Sub Form_Resize()
    ...
    picMain.Move 50, 600, Me.ScaleWidth - 120, Me.ScaleHeight - 950 - 300
    lblItem.Item(0).Move 100, picMain.Top + picMain.Height + 200
...
Private Sub InitTabControl()
    ...
    With tbcPage
        With .PaintManager
            ...
            .Color = &H8000000F
        End With
        Set objTabItem = .InsertItem(0, "在用文件", picFileList.hwnd, 0)
        Set objTabItem = .InsertItem(1, "弃用文件", picExpFileList.hwnd, 0)
...
Private Sub vsfFileList_SelChange()
    ...
    If m_strCurTypeName = "三方部件" And mblnAllowEdit Then
...
'        .AllowSelection = False
        ...
        '刷新修改、删除按钮状态
        vsfFileList_SelChange
        If .Visible = True And mblnAllowEdit Then .SetFocus
'         Call SetMenu
    End With
    Exit Sub
errH:
...
Private Sub StandardDel()
    ...
    Dim frmMsgbox As New frmMessageBox
    Dim strRemarks As String, strNote As String
    On Error GoTo errH
...
'                If MsgBox("你确认要删除选择的" & vsfFileList.SelectedRows & "个部件吗？", vbQuestion + vbOKCancel, gstrSysName) = vbCancel Then Exit Sub
            ...
            End If
            '验证身份并输入操作说明
            If Not CheckAuditStatus("0307", "文件升级管理-删除", strRemarks) Then Exit Sub
...
'            lngRow = vsfExpFileList.FindRow(CStr(vsfExpFileList.Cell(flexcpText, lngCurRow, 1)), , 2)
            For i = 0 To vsfExpFileList.SelectedRows
                If vsfExpFileList.SelectedRow(i) Then
                    ...
                    If lngCurRow <> -1 Then
                        ...
                        gcnOracle.Execute gstrSQL
                        strNote = strNote & "," & Trim(vsfExpFileList.Cell(flexcpText, lngCurRow, 1))
                    End If
                End If
            Next
        '    gcnOracle.CommitTrans
        ...
        Call RefreshData
        '插入重要操作日志
        Call SaveAuditLog(3, "文件升级管理-删除", "删除名称为“" & Mid(strNote, 2) & "”的第三方部件", strRemarks)
    Exit Sub
errH:
...
'        If MsgBox("你确认要弃用 " & vsfFileList.TextMatrix(lngRow, 2) & " 个部件吗？", vbQuestion + vbOKCancel, gstrSysName) = vbCancel Then Exit Sub
    ...
    For i = 0 To vsfFileList.SelectedRows
        If vsfFileList.SelectedRow(i) > 0 Then
            ...
            If vsfFileList.TextMatrix(lngCurRow, FC_文件类型) = "三方部件" Then
                ...
                gstrSQL = "Zl_Zlfilesexpired_Insert(" & _
                                                                                            SQLAdjust(vsfFileList.Cell(flexcpText, lngCurRow, 2)) & "," & _
                                                                                            SQLAdjust(vsfFileList.Cell(flexcpText, lngCurRow, 7)) & "," & _
                                                                                            SQLAdjust(vsfFileList.Cell(flexcpData, lngCurRow, 5)) & "," & _
                                                                                            SQLAdjust(vsfFileList.Cell(flexcpText, lngCurRow, 3)) & "," & _
                                                                                            SQLAdjust(vsfFileList.Cell(flexcpText, lngCurRow, 10)) & _
                                                                                            ")"
                Call ExecuteProcedure(gstrSQL, "StandardAbandon")
                gstrSQL = "delete zlFilesUpgrade where upper(文件名)= upper('" & strName & "')"
                gcnOracle.Execute gstrSQL
                strNote = strNote & "," & strName
            End If
    ...
    Next
    '插入重要操作日志
    Call SaveAuditLog(2, "文件升级管理-弃用", "弃用名称为“" & Mid(strNote, 2) & "”的第三方部件")
    Call RefreshData
    If lngRow <> -1 Then
        If lngRow >= 2 And vsfFileList.Rows > 2 Then
          vsfFileList.Select lngRow - 1, 2
...
Private Sub StandardUpLoad()
    Dim frmUpload As New frmClientUpgradeFileUpload
...
'strProgFunc:权限字符串
    ...
    If mblnAllowEdit = False Then
        ...
        cmdRepairList.Enabled = False
        cmdRepairFiles.Enabled = False
        cmdAdd.Enabled = False
        cmdEdit.Enabled = False
        cmdDel.Enabled = False
        cmdExpired.Enabled = False
        vsfFileList.Editable = flexEDNone
        vsfExpFileList.Editable = flexEDNone
    End If
End Sub
