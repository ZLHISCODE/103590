...
Begin VB.Form frmClientsUpgradeConfigure 
   Appearance      =   0  'Flat
   BackColor       =   &H80000004&
   BorderStyle     =   0  'None
   ClientHeight    =   6690
   ...
   Begin VB.CommandButton cmdAllBefUpgrade 
      Caption         =   "全部预升级(&E)"
      Enabled         =   0   'False
      Height          =   300
      Left            =   1830
      ...
      Width           =   1500
   End
   Begin VB.CommandButton cmdAllUpgrade 
      Caption         =   "全部升级(&W)"
      Height          =   300
      Left            =   120
      TabIndex        =   4
   ...
   Begin VB.PictureBox picMonthSet 
      ...
      Begin VB.CommandButton cmdMonthSet 
         ...
         Height          =   285
         Left            =   -30
         Picture         =   "frmClientsUpgradeConfigure.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   31
         TabStop         =   0   'False
         Top             =   -30
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsfMain 
      ...
      FormatString    =   $"frmClientsUpgradeConfigure.frx":00F6
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   Begin MSComctlLib.ImageList imgEdit 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmClientsUpgradeConfigure.frx":01CD
            Key             =   "Check"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmClientsUpgradeConfigure.frx":0767
            Key             =   ""
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmClientsUpgradeConfigure.frx":0D01
            Key             =   "签名"
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmClientsUpgradeConfigure.frx":1053
            Key             =   "Woman"
         EndProperty
         BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmClientsUpgradeConfigure.frx":78B5
            Key             =   "Man"
         EndProperty
         BeginProperty ListImage6 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmClientsUpgradeConfigure.frx":E117
            Key             =   "UnCheck"
         EndProperty
         BeginProperty ListImage7 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmClientsUpgradeConfigure.frx":E5DF
            Key             =   "AllCheck"
         EndProperty
      EndProperty
   End
   ...
   End
End
Attribute VB_Name = "frmClientsUpgradeConfigure"
...
Private Sub cmdAllBefUpgrade_Click()
    Dim i, j As Long
    Dim strSQL As String
    Dim strTemp As String
    Dim strUpdateVal As String
    
    strTemp = cmdAllBefUpgrade.Caption
    strUpdateVal = IIf(strTemp = "全部预升级(&E)", "1", "0")
    If MsgBox("是否要" & IIf(strUpdateVal = "1", "设置", "取消") & "全部客户端启用预升级？", vbQuestion + vbOKCancel, gstrSysName) = vbCancel Then Exit Sub

    On Error GoTo errH
    Call UpdateData(Col_预升级, strUpdateVal)
    cmdAllBefUpgrade.Caption = IIf(strTemp = "全部预升级(&E)", "全部不预升级(&E)", "全部预升级(&E)")
    RefreshData
    Exit Sub
errH:
...
Private Sub cmdAllUpgrade_Click()
    Dim i As Long
    Dim strSQL As String
    Dim strUpdateVal As String
    Dim strTemp As String

    strTemp = cmdAllUpgrade.Caption
    strUpdateVal = IIf(strTemp = "全部升级(&W)", "1", "0")
    If MsgBox("是否要" & IIf(strUpdateVal = "1", "设置", "取消") & "全部客户端启用升级？", vbQuestion + vbOKCancel, gstrSysName) = vbCancel Then Exit Sub

    On Error GoTo errH
    Call UpdateData(Col_升级, strUpdateVal)
    cmdAllUpgrade.Caption = IIf(strTemp = "全部升级(&W)", "全部不升级(&W)", "全部升级(&W)")
    RefreshData
    Exit Sub
errH:
    MsgBox err.Description, vbInformation, gstrSysName
...
Private Sub cmdFileSeverSet_Click()
    Dim frmSeverSet As New frmClientsSever
...
'    picBtn.Top = Me.ScaleHeight - picBtn.Top - 60
    ...
    picStatus.Move vsfMain.Left + vsfMain.Width - picStatus.Width
End Sub
...
'        strSQL = "select 工作站,IP,部门,升级服务器,升级标志,是否预升级,收集标志,预升时点,升级情况,预升完成,收集状态,修复状态,升级说明,预升级说明,收集说明,修复说明 from zlclients"
        strSQL = "select A.工作站,A.IP,A.部门,A.用途,A.升级文件服务器,B.位置, A.升级标志,A.是否预升级,A.收集标志,A.预升时点,A.升级情况,A.预升完成,A.收集状态,A.修复状态,A.升级说明,A.预升级说明,A.收集说明,A.修复说明,A.批次 from zlclients A,zlupgradeserver B where A.升级文件服务器 = B.编号(+)"
        Call OpenRecordset(rsTemp, strSQL, Me.Caption)
...
'            .Cell(flexcpAlignment, .FixedRows, Col_收集说明) = flexAlignCenterCenter
    ...
    '设置按键状态
    If blnUpgrade Then
        cmdAllUpgrade.Caption = "全部升级(&W)"
    Else
        cmdAllUpgrade.Caption = "全部不升级(&W)"
    End If
    If blnBefUpgrade Then
        cmdAllBefUpgrade.Caption = "全部预升级(&E)"
    Else
        cmdAllBefUpgrade.Caption = "全部不预升级(&E)"
    End If
    If blnCollect Then
...
Private Sub optUpgradeTime_Click(Index As Integer)
    Select Case Index
        Case 0
            ...
            cmdAllBefUpgrade.Enabled = False
            ...
            If optUpgradeTime(Index).Visible Then
                cmdAllBefUpgrade.Caption = "全部不预升级(&E)"
                cmdAllBefUpgrade_Click
                SaveUpgradeDate True
            End If
        Case 1
            dtpTime.Enabled = True
            cmdTimeSet.Enabled = True
            cmdAllBefUpgrade.Enabled = True
            ...
            If optUpgradeTime(Index).Visible Then SaveUpgradeDate
    End Select
End Sub
...
Private Sub vsfMain_CellButtonClick(ByVal Row As Long, ByVal Col As Long)
    ...
    frmFilesUpgradeLogView.mstrName = vsfMain.TextMatrix(Row, Col_客户端)
