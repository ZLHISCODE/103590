...
Public gstrErrDes As String '错误信息
...
Public Function CurrentDate() As Date
    ...
    Exit Function
errH:
    SaveSetting "LIS图片转出", "转出进度", "转出错误", "获取服务费时间发生错误。"
    WriteErrLog "获取服务费时间发生错误" & Err.Description
    CurrentDate = 0
    Err = 0
    Unload frmPcs
End Function
...
Public Sub WriteErrLog(strErrDes As String)
    '功能:在当前路径保存错误信息
    Dim strFile As String, txtStream As TextStream
    
    If Right(App.Path, 1) = "\" Then
        strFile = Mid(App.Path, 1, Len(App.Path) - 1)
    Else
        strFile = App.Path
    End If
    strFile = strFile & "\Lis2FtpErrLog.txt"
    
    If Not gobjFile.FileExists(strFile) Then
        Set txtStream = gobjFile.CreateTextFile(strFile)
    Else
        Set txtStream = gobjFile.OpenTextFile(strFile, ForAppending)
    End If
    
    txtStream.WriteLine strErrDes & "   " & date & " " & Time
    txtStream.Close
End Sub
...
Public Function OraDataOpen(ByVal strServerName As String, ByVal strUserName As String, ByVal strUserPwd As String) As Boolean
    ...
    '------------------------------------------------
    Dim strError As String, blnTransPassword As Boolean
    ...
    If Not objRegister35 Is Nothing Then '检测当前环境
       Set gcnOracle = objRegister35.GetConnection(strServerName, strUserName, strUserPwd, blnTransPassword, OraOLEDB, strError)
    Else
        '支持strServerName = "192.168.2.13:1521/dyyy"这种格式
        Set gcnOracle = gobjRegister.GetConnection(strServerName, strUserName, strUserPwd, blnTransPassword, OraOLEDB, strError)
    End If
