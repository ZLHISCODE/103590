...
Public Function DrawImg(ByVal strType As String, ByVal strData As String, ByVal strFileName As String, Optional intSaveType As Integer = 1) As Boolean
    ...
    If aImage(0) >= 100 And aImage(0) <= 227 Then
        ...
        If UCase$(strFile) Like "*.ZIP" Then
            ...
            If killFile <> "" Then
                If gobjFile.FileExists(killFile) Then
                    Kill killFile
                    killFile = "" '解压后的原始ZIP要删除
                End If
            End If
        End If
            ...
            ElseIf UCase(strFile) Like "*.GIF" Then
                If CheckGif(strFile) Then
                    ...
                    SavePicture frmObj.picTmp, strFile
                    killFile = Replace(UCase(strFile), ".BMP", ".GIF")
                Else
                    WriteErrLog "图片:" & strFileName & "数据有损坏，未能保存成功"
                    DrawImg = True
                    Unload frmObj
                    Exit Function
                End If
    ...
    Exit Function
errH:
    SaveSetting "LIS图片转出", "转出进度", "转出错误", "图片转换时发生错误。"
        ...
        With frmPic
            If DrawGam(.Picture1, strData) Then
                ...
                If InStr(strImgName, ".") > 0 Then
                    ...
                    If gobjFile.FileExists(strImgName) Then Kill strImgName
...
errHandle:
    SaveSetting "LIS图片转出", "转出进度", "转出错误", "图片转换时发生错误。"
    WriteErrLog Err.Description & "错误位置-DrawGam"
End Function
