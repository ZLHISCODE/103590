...
'功能：获取当前会话最近一次执行的SQLID,并将语句的CHILD_NUMBER赋值到传入变量中
    ...
    On Error GoTo errH
    strSql = "select prev_sql_id,PREV_CHILD_NUMBER from V$session where AUDSID=UserENV('SessionID')  And Sid= Userenv('Sid')"
...
Public Function ChangeSQL(ByVal intMod As Integer, ByVal strOldID As String, ByVal strSqlText As String, ByRef strChildNum As String, ByVal strInstID, Optional strOptVersion As String) As String
    ...
    If intMod = 1 Then
        ...
        strNewSQL = Mid(strNewSQL, 1, InStr(1, UCase(strNewSQL), "SELECT") + 6) & " /*+ RULE*/ " & Mid(strNewSQL, InStr(1, UCase(strNewSQL), "SELECT") + 7)
    '删除RULE提示
...
Public Function CreateSqlProfiles(strBdSqlID As String, strGdSqlID As String, strChildNum As String, Optional ByRef strErr As String) As Boolean
    ...
    strSql = "Declare" & vbNewLine & _
                ...
                "Begin" & vbNewLine & _
                "  With A As(Select Rownum As r_No, a.* From Table( Dbms_Xplan.Display_Cursor('" & strGdSqlID & "', Null, 'OUTLINE') ) A)," & vbNewLine & _
