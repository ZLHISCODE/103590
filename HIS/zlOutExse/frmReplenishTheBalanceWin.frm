...
Private mobjPayCards As Cards  '当前支持的支付卡类别
...
Public Function zlChargeWin(ByVal frmMain As Object, ByVal bytFunc As EM_Balance_Type, _
    ...
    Optional blnElsePersonErrBill As Boolean = False, Optional ByVal blnRegister As Boolean) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:程序入口:表示进入支付结算窗口
    '入参:frmMain-调用的主窗体
...
Private Function LoadData(ByVal str结算序号 As String) As Boolean
    ...
    strSQL = _
        "Select a.结算方式, Sum(a.冲预交) As 冲预交, b.性质" & vbNewLine & _
        "From 病人预交记录 A, 结算方式 B" & vbNewLine & _
        "Where a.结算方式 = b.名称(+) And a.结算序号 = [1]" & vbNewLine & _
    ...
    With rsTemp
        ...
        Do While Not .EOF
            If Nvl(rsTemp!结算方式) <> "" Then
                ...
                If Val(Nvl(rsTemp!性质)) = 3 Or Val(Nvl(rsTemp!性质)) = 4 Then '医保结算方式
                    dblYbMoney = RoundEx(dblYbMoney + Val(Nvl(rsTemp!冲预交)), 6)
                End If
            Else
                mdblDelMoney = -1 * RoundEx(mdblDelMoney + Val(Nvl(rsTemp!冲预交)), 6)
            End If
            .MoveNext
        Loop
    End With
    txtYBMoney.Text = Format(dblYbMoney, "0.00")
...
Private Sub InitFace()
    ...
    Call Load支付方式
    txt缴款.Text = Format(mdbl当前未退, "0.00")
    txt缴款.Locked = True
...
'    End If
    ...
    With cbo支付方式
        ...
        If .ListCount = 0 Then
            MsgBox "补结算没有可用的结算方式,请先到结算方式管理中设置。", vbExclamation, gstrSysName
            mblnUnLoad = True: Exit Sub
        End If
    End With
    mblnNotClick = False
...
Private Function Exist预交款() As Boolean
    ...
    mdbl可退预交 = 0
    txt预存款.Text = ""
    If RoundEx(mdblDelMoney, 6) = 0 Then Exit Function '退款金额等于零不能使用预存款
    ...
    '医保报销大于了费用总金额
    If RoundEx(mdbl可退预交, 6) < mdblDelMoney Then
        lbl预存款.Visible = True
        txt预存款.Visible = True
        ...
        mdbl当前未退 = mdblDelMoney - mdbl可退预交
        Exit Function
    End If
...
Private Sub ShowLedInfor()
    ...
    If objCard.结算性质 = 1 Then
        zl9LedVoice.DispCharge -1 * mdbl当前未退, 0, 0
    Else '部分支付现金时的处理
        Call zl9LedVoice.DisplayBank( _
            "合计:" & mdblDelMoney & "元,应收:" & -1 * mdbl当前未退 & "元")
    End If
...
Private Function SaveCharge() As Boolean
    ...
    Dim strSQLUpdate As String, dbl冲预交 As Double
    ...
    If mblnThreeSwapSingle = False Then
        ...
        If mstrReclaimInvoice <> "无可退票据" Then
            ...
            For i = 0 To UBound(varData)
                ...
                strSQL = strSQL & "To_Date('" & Format(mdtDate, "yyyy-mm-dd hh:mm:ss") & "','YYYY-MM-DD HH24:MI:SS'),"
                '  退费_In       Number := 0,
                strSQL = strSQL & "0,"
                '  票据张数_In   Number := 0,
                strSQL = strSQL & "0,"
...
Private Function ExecuteThreeSwapPayInterface(objCard As Card, ByVal str结算序号 As String, ByVal dblMoney As Double) As Boolean
    ...
    Dim lng原结帐ID As Long, str卡号 As String, strTemp As String
    ...
    Else
        If mblnThreeSwapSingle Then
            ...
            Do While Not rsBalance.EOF And RoundEx(dblTemp, 6) > 0
                ...
                strSwapExtendInfor = "3|" & "0": strTemp = strSwapExtendInfor
            ...
            Do While Not rsBalance.EOF And RoundEx(dblTemp, 6) > 0
                ...
                If objCard.是否退款验卡 Then
                    ...
                    If gobjSquare.objSquareCard.zlBrushCard(Me, mlngModule, Nothing, _
                        objCard.接口序号, False, mobjPati.姓名, mobjPati.性别, mobjPati.年龄, _
...
Private Sub txt预存款_LostFocus()
    ...
    mdbl当前未退 = mdblDelMoney - Val(txt预存款.Text)
    Call Show误差金额(-1 * mdbl当前未退)
End Sub
