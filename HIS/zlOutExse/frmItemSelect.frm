...
Public Function ShowSelect(frmParent As Object, ByVal strPrivs As String, _
    ...
    Optional ByVal str普通价格等级 As String, _
...
'      strIDs=读取的项目ID集,用于判断输入时是否别名不同的同一个收费项目
    ...
    If blnLoad Then
        ...
        '60623:李南春,2015/4/10,门诊收费处不能选择特殊使用级抗菌药物进行收费
            ...
            strSQL = _
                ...
                "       Decode(A.是否变价,1,'时价',LTrim(To_Char(Sum(C.现价)" & _
                        ...
                        IIf(InStr(1, mstrPrivs, "显示库存") > 0, _
                            ...
                            "Decode(Sign(X.库存),1,'有','无')") & " as 库存,-1 as 批次,A.保险费用等级" & _
    ...
    If blnLoad Then
        If glng发料部门 <> 0 Then
            strStock = _
                ...
                " And (Nvl(A.批次, 0) = 0 Or A.效期 Is Null Or A.效期 > Trunc(Sysdate))" & _
                " Group by A.药品ID Having Sum(Nvl(A.可用数量,0))<>0"
            If gbln不显示无库存卫材 Then str不显示无库存卫材 = " And (Nvl(x.库存, 0) > 0 And Nvl(d.跟踪在用, 0) = 1 Or Nvl(d.跟踪在用, 0) = 0) "
        End If

        strCheckBarCode = ""
        If gTy_System_Para.byt条码卫材识别控制 = 1 Then strCheckBarCode = " And Not Exists(select 1 From 药品库存 where  a.ID=药品ID and (内部条码 is Not null or 商品条码 is Not NULL)  And Rownum <2)"

        If mstr输入 <> "" Then
            strInput = " And (A.编码 Like [6] Or B.名称 Like [7] Or B.简码 Like [7]) And B.码类=[8]" & strCheckBarCode

           If zlCommFun.ActualLen(mstr输入) > 10 And zlCommFun.IsCharChinese(mstr输入) = False And mbln条码扫描 = False Then
                strStuffIds = GetStuffIDsFromInputBarCode(mstr输入)
                If strStuffIds <> "" Then
                    strStuffTable = "With C_材料批次 as (Select  /*+cardinality(j,10)*/ C1 as 材料ID,C2 as 批次 From table(f_num2list2([24])) J)  " & vbCrLf
            ...
            If mbln条码扫描 And Len(mstr输入) > 10 Then
                'strInput = " And (A.编码 Like [6] Or M.商品条码=[23] Or M.内部条码=[23] Or B.简码 Like [7]) And B.码类=[8]"
                strStuffIds = GetStuffIDsFromInputBarCode(mstr输入)
                If strStuffIds <> "" Then
                    strStuffTable = "With C_材料批次 as (Select  /*+cardinality(j,10)*/ C1 as 材料ID,C2 as 批次 From table(f_num2list2([24])) J)  " & vbCrLf
            ...
            ElseIf IsNumeric(mstr输入) Then                         '10,11.输入全是数字时只匹配编码
                ...
                End If
            ElseIf zlCommFun.IsCharAlpha(mstr输入) Then         '01,11.输入全是字母时只匹配简码
                If Mid(gstrMatchMode, 2, 1) = "1" Then strInput = " And B.简码 Like [7] And B.码类=[8]" & strCheckBarCode
            ElseIf zlCommFun.IsCharChinese(mstr输入) Then
    ...
    Set mrsItem = zlDatabase.OpenSQLRecord(strSQL, Me.Name, int类型, lng诊疗分类ID, lng收费分类ID, str类别, mstr类别, _
        ...
        lng药房ID, glng西药房, glng成药房, glng中药房, glng发料部门, mint险类, mint中药形态, mstr排除类别, gstr所属部门ID, _
        mstr药品价格等级, mstr卫材价格等级, mstr普通价格等级, UCase(mstr输入), strStuffIds)
