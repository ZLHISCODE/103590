...
'功能：重新计算指定药品行的价格和金额
    ...
    Dim dblMoney As Double, dbl加班加价率 As Double
    Dim str费别 As String, cur金额 As Currency
    Dim strInfo As String, strSQL As String, i As Long, strAdvance As String
    ...
    If Not rsTmp.EOF Then
        ...
        If InStr(",5,6,7,", objBillDetail.收费类别) > 0 Then
            ...
            If dblAllTime <> 0 Or Not objBillDetail.Detail.变价 Then
                Set rsPrice = zlDatabase.OpenSQLRecord("Select Zl_Fun_Getprice([1],[2],[3]) As Price From Dual", _
                            Me.Caption, objBillDetail.收费细目ID, objBillDetail.执行部门ID, dblAllTime)
                If rsPrice.EOF Then
                    '获取价格失败
                    MsgBox "第 " & Split(objBillDetail.Key, "_")(0) & " 行药品""" & objBillDetail.Detail.名称 & """获取价格失败！", vbInformation, gstrSysName
                Else
                    ...
                    If dbl剩余数量 <> 0 And objBillDetail.Detail.变价 Then
                        '数量未分解完毕
...
Private Function SelectChineDrug(ByVal strInput As String) As Boolean
    ...
    '特殊药品权限
    str特性 = ""
    If int中药形态 = 0 And lng药房ID <> 0 And Not zlStr.IsHavePrivs(mstrPrivs, "不检查库存") Then
        '只有散装才有库存
        strStock = _
    ...
    If int中药形态 = 0 Then
        ...
        "   And (D.站点='" & gstrNodeNo & "' Or D.站点 is Null)" & _
         IIf(lng药房ID <> 0 And Not zlStr.IsHavePrivs(mstrPrivs, "不检查库存"), " And nvl(X.库存,0)<>0", "")
    Else
    ...
    If strInput <> "" Then
            ...
            Set rsTemp = zlDatabase.ShowSQLSelect(Me, strSQL, 0, "中草药", False, "", "", False, False, True, vPoint.X, vPoint.Y, _
                        vsBill.CellHeight, blnCancel, True, True, strInput & "%", gstrLike & strInput & "%", gbytCode + 1, lng药房ID, mint险类, int中药形态, gint病人来源, mlng开单科室ID, "bytSize=1")
    Else
            If int中药形态 = 0 Then
                '散装才显示到规格级,保持原来不变
            ...
            Else
                ...
                "           And exists(Select 1 From 诊疗执行科室 A1 Where A1.诊疗项目ID=A.ID And A1.执行科室ID=[4]   And (A1.病人来源 is NULL Or A1.病人来源=[7]) and (A1.开单科室ID is null or A1.开单科室ID=[8])  ) " & vbNewLine
            End If
            Set rsTemp = zlDatabase.ShowSQLSelect(Me, strSQL, 2, "中草药", False, "", "", False, True, False, 0, 0, 0, blnCancel, False, True, "", "", "", lng药房ID, "", int中药形态, gint病人来源, mlng开单科室ID, "bytSize=1")
    End If
    With vsBill
...
Private Function 分解中药规格(ByVal lng药名ID As Long, ByVal dbl数量 As Double, _
    ...
    Else
        ...
        Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "规格分配", lng药名ID, int中药形态, dbl数量, Val(txt付数.Text), lng药房ID, 0)
        str规格数量 = "" & rsTemp!txt
    End If
