...
Public Sub ReadData(ByVal intType As Integer, Optional ByVal lngPatiID As Long)
    ...
    If intType = 1 Then
        ...
        strSQL = " Select A.No, Decode(Nvl(A.附加标志,0),1,'挂号','收费'), B.姓名, B.性别, B.年龄, -1*Sum(C.冲预交), A.操作员姓名, A.登记时间, A.结算序号" & _
                 " From 费用补充记录 A, 病人信息 B, 病人预交记录 C  " & _
                 " Where B.病人ID= [1] And Nvl(A.费用状态,0)=1 And A.病人ID=B.病人ID And A.记录状态 = 2 " & _
                 "      And A.操作员姓名=[2] And C.结帐ID In (Select Distinct 结帐ID From 病人预交记录 Where 结算序号=A.结算序号) " & _
...
Public Sub zlRptPrint(ByVal bytFunc As Integer)
    ...
    objPrint.Title.Text = "保险补充结算异常退费记录清单"
