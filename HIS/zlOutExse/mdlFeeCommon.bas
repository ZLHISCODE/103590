...
Public Function Get结算方式(str场合 As String, Optional str性质 As String, _
    Optional ByVal bln医疗付款方式 As Boolean) As ADODB.Recordset
    'bln医疗付款方式 - 是否获取医疗付款方式缺省的结算方式
    ...
    End If
    strSQL = _
        " Select B.编码,B.名称,Nvl(Nvl(A.缺省标志,B.缺省标志),0) as 缺省,Nvl(B.性质,1) as 性质,Nvl(B.应付款,0) as 应付款,a.付款方式" & _
        " From 结算方式应用 A,结算方式 B" & _
        " Where A.应用场合=[1] And B.名称=A.结算方式 " & _
                IIf(bln医疗付款方式, "And a.付款方式 Is Not Null", "And a.付款方式 Is Null") & _
        " And (B.性质<>7 Or B.性质=7 And Exists(Select 1 From 一卡通目录 C Where C.结算方式=B.名称 And C.启用=1))   " & strIF
    If InStr(1, str性质, ",9") > 0 Then
        strSQL = strSQL & " Union " & _
                 " Select 编码,名称,Nvl(缺省标志,0) As 缺省,Nvl(性质,1) as 性质,Nvl(应付款,0) as 应付款,'' As 付款方式 " & _
                 " From 结算方式 " & _
                 " Where 性质=9 " & _
                 " Order by 性质,编码"
    Else
...
Public Function GetPersonnel(str性质 As String, Optional blnBaseInfo As Boolean, _
    ...
    If str性质 <> "" Then
        ...
        Else
            strSQL = _
            " Select Distinct a.Id, a.编号, a.姓名, a.简码, a.身份证号, a.出生日期, a.性别, a.民族, a.工作日期," & _
            "        a.办公室电话, a.电子邮件, a.执业类别, a.执业范围, a.管理职务, a.专业技术职务, a.聘任技术职务, " & _
            "        a.学历, a.所学专业, a.留学时间, a.留学渠道, a.接受培训, a.科研课题, a.个人简介, a.建档时间, " & _
            "        a.撤档时间, a.撤档原因, a.别名, a.站点" & _
            " From 人员表 a,人员性质说明 b" & _
            " Where a.ID = b.人员ID" & strWhere & _
            "       And (a.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or a.撤档时间 Is Null)" & _
            "       And (A.站点='" & gstrNodeNo & "' Or A.站点 is Null)" & _
    ...
    Else
        ...
        Else
            strSQL = _
            zlGetFullFieldsTable("人员表", 0, "", False) & _
            " Where (撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or 撤档时间 Is Null)" & _
            "       And (A.站点='" & gstrNodeNo & "' Or A.站点 is Null)" & strWhere & _
            " Order by 简码"
        End If
...
Public Function zlAddThreeSwapSQLToCollection(ByVal bln预交款 As Boolean, _
    ...
    Optional lng原交易ID As Long) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:保存三方结算数据
    '入参: bln预交款-是否预交款
    '       lngID-如果是预交款,则是预交ID,否则结帐ID
