...
Public Function ShowEdit(ByVal frmParent As Object, ByVal intNowCol As Integer, ByVal intMinCol As Integer, ByVal intMaxCol As Long, ByVal lng护理等级 As Long, ByVal int心率应用 As Integer, Optional Marr呼吸 As Variant) As Boolean
...
'        .TextMatrix(GraphDataRow.断开标志, mintNowCol + .FixedCols) = chk(0).Value
        ...
        For intCount = 1 To vsf.Rows - 1
            '呼吸为表格是始终在最后
            If vsf.RowData(intCount) = 3 And mfrmParent.呼吸表格 = True Then
                ReDim Preserve aryValue(UBound(aryValue) + 1)
            ...
            If vsf.RowData(intCount) = 3 And mfrmParent.呼吸表格 = True Then
                mfrmParent.Getvsf.ColData(mintNowCol + mfrmParent.Getvsf.FixedCols) = aryValue(intCount)
                mfrmParent.Getvsf.TextMatrix(1, mintNowCol + mfrmParent.Getvsf.FixedCols) = Trim(vsf.TextMatrix(intCount, mCol.数据))
                
                str方式 = Trim(vsf.TextMatrix(intCount, mCol.部位))
                If UBound(mArrTmp) >= mintNowCol Then
                    arrCode(0) = Trim(vsf.TextMatrix(intCount, mCol.数据))
                    If Trim(vsf.TextMatrix(intCount, mCol.数据)) = "" Then str方式 = ""
                    arrCode(1) = str方式
                    arrCode(2) = vsf.TextMatrix(intCount, mCol.未记说明)
            ...
            Else
                ...
                Case 1, 2, 3            '.TextMatrix(GraphDataRow.部位标志, mintNowCol + .FixedCols)
                    ...
                    If Trim(vsf.TextMatrix(intCount, mCol.部位)) <> "" Then
                        str方式 = Trim(vsf.TextMatrix(intCount, mCol.部位))
                    Else
                        If Val(vsf.RowData(intCount)) = 1 And Val(Split(.TextMatrix(GraphDataRow.曲线数据, mintNowCol + .FixedCols), ";")(intCount)) > 0 Then str方式 = "腋温"
                    End If
...
Private Sub LoadNowData()
    ...
    Dim aryPara() As String
    Dim strValue As String, strNote As String
    Dim dtNow As Date
...
'        chk(0).Value = Val(.TextMatrix(GraphDataRow.断开标志, mintNowCol + .FixedCols))
    ...
    '呼吸为表格也需要显示
    If mfrmParent.呼吸表格 Then
        With mfrmParent.Getvsf
            strValue = .TextMatrix(1, mintNowCol + .FixedCols)
            strNote = ""

            If Val(vsf.RowData(vsf.Rows - 1)) <> 0 Then vsf.Rows = vsf.Rows + 1
            
            vsf.RowData(vsf.Rows - 1) = 3
            vsf.TextMatrix(vsf.Rows - 1, mCol.项目) = .TextMatrix(1, 1)
            
            aryPara = Split(.ColData(0), ";")
            'RS!最大值 & ";" & RS!最小值 & ";" & RS!单位值 & ";" & RS!最高行
            
            vsf.TextMatrix(vsf.Rows - 1, mCol.最小值) = IIf(UBound(aryPara) < 1, "50", aryPara(1))
            vsf.TextMatrix(vsf.Rows - 1, mCol.最大值) = IIf(UBound(aryPara) < 0, "10", aryPara(0))
            vsf.TextMatrix(vsf.Rows - 1, mCol.单位值) = IIf(UBound(aryPara) < 2, "", aryPara(2))
            vsf.TextMatrix(vsf.Rows - 1, mCol.最高行) = 1

             
            vsf.TextMatrix(vsf.Rows - 1, mCol.数据) = Format(strValue, "0")
            vsf.TextMatrix(vsf.Rows - 1, mCol.未记说明) = strNote
            strTmp = ""
            If UBound(mArrTmp) >= mintNowCol Then
                If InStr(1, mArrTmp(mintNowCol), "-") > 0 Then
                    strTmp = Split(mArrTmp(mintNowCol), "-")(1)
                End If
                If InStr(1, mArrTmp(mintNowCol), "-") > 1 Then
                    vsf.TextMatrix(vsf.Rows - 1, mCol.未记说明) = Split(mArrTmp(mintNowCol), "-")(2)
                End If
            End If
            vsf.TextMatrix(vsf.Rows - 1, mCol.部位) = strTmp
