...
Begin VB.Form frmTechnicGroup 
   ...
   Begin VB.CommandButton cmdSure 
      ...
      Width           =   1100
   End
   Begin zl9BaseItem.ucFlexGrid ufgRoomSelect 
      Height          =   2970
      Left            =   60
      TabIndex        =   4
      Top             =   1110
...
'数据有效性检查
    ...
    If Trim(txtGrounName.Text) = "" Then
        Call MsgBox("分组名称不能为空，请录入有效的分组名称。", vbOKOnly, "提示")
        txtGrounName.SetFocus
        Exit Function
    End If
    
    If Not ufgRoomSelect.IsCheckedRow Then
        Call MsgBox("请选择该分组下所对应的执行间。", vbOKOnly, "提示")
        ufgRoomSelect.SetFocus
        Exit Function
    End If
    
    If Trim(txtPrefix.Text) = "" Then
        lngMsgResult = MsgBox("尚未录入分组前缀,排号时，不同组之间可能产生相同排队号码，是否继续？", vbYesNo, "提示")
