...
Public Function GetDepartments(ByVal str性质 As String, _
    ...
    Optional ByVal blnCheck站点 As Boolean = True, Optional ByVal strDeptIDs As String) As ADODB.Recordset
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:读取指定性质的部门列表
    ...
    strSQL = _
        ...
        " Order by A.编码"
    Set GetDepartments = zlDatabase.OpenSQLRecord(strSQL, App.ProductName, str性质, str服务对象, UserInfo.ID, strDeptIDs)
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
'      blnDept:True表示获取操作员所属科室,以及所属病区下的所有科室,否则返回操作员所属病区,以及所在科室所属的病区
    ...
    Else
        strSQL = "Select 部门ID as 部门ID From 部门人员 Where 人员ID=[1]"
    End If
...
Public Function GetMoneyInfo(lng病人ID As Long, Optional dblModiMoney As Double, _
    Optional blnInsure As Boolean, Optional int类型 As Integer = -1, _
    Optional bln按类型统计 As Boolean = False, Optional bytModiMoneyType As Byte = 0, _
    Optional ByVal blnFamilyMoney As Boolean, Optional ByVal lng主页ID As Long) As ADODB.Recordset
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取指定病人的剩余额
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim rsTmp As ADODB.Recordset, bln医保 As Boolean
    Dim strSQL As String
