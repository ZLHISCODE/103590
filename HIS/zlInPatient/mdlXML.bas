...
Public Function zlXML_Init(Optional ByVal strNode As String = "DATA", _
    ...
    If Err <> 0 Then
        LogWrite "入院管理病人信息XML读取调试日志", "" & glngModul, "zlXML_Init", "初始化XML对象失败(InitXML)《 " & strNode & "》:"
...
Public Function zlXML_GetRows(ByVal strNodeName As String, ByRef lngOutRows As Long) As Boolean
    ...
    LogWrite "入院管理病人信息XML读取调试日志", "" & glngModul, "zlXML_GetRows", "获取XML的记录行数(GetOutXMLRows)《 " & strNodeName & "》:" & lngOutRows
    zlXML_GetRows = True
    Exit Function
ErrHand:
    LogWrite "入院管理病人信息XML读取调试日志", "" & glngModul, "zlXML_GetRows", "获取XML的记录行数(GetOutXMLRows)《 " & strNodeName & "》" & vbCrLf & "错误序号:" & vbCrLf & "   " & Err.Description
    If ErrCenter = 1 Then Resume
End Function
...
Public Function zlXML_GetNodeValue( _
    ...
    Else
        ...
        LogWrite "入院管理病人信息XML读取调试日志", "" & glngModul, "zlXML_GetNodeValue", strErrMsg
        If Not gobjXml Is Nothing Then
            LogWrite "入院管理病人信息XML读取调试日志", "" & glngModul, "zlXML_GetNodeValue", gobjXml.xml
        Else
            LogWrite "入院管理病人信息XML读取调试日志", "" & glngModul, "zlXML_GetNodeValue", "gobjXml.xml=nothing"
        End If
    End If
...
Public Function zlXML_GetChildNodeValue(strParentName As String, strChildName As String, Optional ByVal lngParentRow As Long = 0, Optional ByVal lngChildRow As Long = 0, Optional ByRef strOutPut As String, Optional ByRef strErrMsg As String) As Boolean
    ...
    Else
        ...
        LogWrite "入院管理病人信息XML读取调试日志", "" & glngModul, "zlXML_GetChildNodeValue", strErrMsg
        If Not gobjXml Is Nothing Then
            LogWrite "入院管理病人信息XML读取调试日志", "" & glngModul, "zlXML_GetChildNodeValue", gobjXml.xml
        Else
            LogWrite "入院管理病人信息XML读取调试日志", "" & glngModul, "zlXML_GetChildNodeValue", "gobjXml.xml=nothing"
        End If
    End If
...
Public Function zlXML_GetChildRows(ByVal strParentName, ByVal strChildName As String, ByRef lngOutRows As Long, Optional ByVal lngRow As Long) As Boolean
    ...
    LogWrite "入院管理病人信息XML读取调试日志", "" & glngModul, "zlXML_GetChildRows", "获取XML的记录行数(GetOutXMLRows)《 " & strChildName & "》:" & lngOutRows
    zlXML_GetChildRows = True
    Exit Function
ErrHand:
    LogWrite "入院管理病人信息XML读取调试日志", "" & glngModul, "zlXML_GetChildRows", "获取XML的记录行数(GetOutXMLRows)《 " & strChildName & "》" & vbCrLf & "错误序号:" & vbCrLf & "   " & Err.Description
    If ErrCenter = 1 Then Resume
End Function
