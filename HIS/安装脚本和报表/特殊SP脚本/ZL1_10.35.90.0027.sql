...
--130788:余伟节,2018-08-30,病人状态服务配置目录
Insert Into 三方服务配置目录(系统标识,服务名称) 
Select '知识库','病人状态编辑' From Dual Union All
Select '知识库','病人状态查询' From Dual Union All
Select '知识库','病人状态保存' From Dual;
-------------------------------------------------------------------------------
--权限修正部份
-------------------------------------------------------------------------------
...
Begin
  ...
  If 审核对象_In = 2 And n_结果 = 11 Then
    EXECUTE IMMEDIATE 'Call b_Message_Blood.Zlhis_Blood_010(:1)' USING 医嘱id_In;
  End If;	
Exception
  When Err_Item Then
...
End Zl_医嘱审核管理_Audit;
/
--130541:胡俊勇,2018-08-30,集成平台消息问题
Create Or Replace Package b_Message Is
  ...
  Procedure Zlhis_Drug_004
   (
    ...
    序号_In 药品收发记录.序号%Type,
    记录状态_In 药品收发记录.记录状态%Type
    );
  --部门发药
...
------------------------------------------------------------------------------------
--系统版本号
Update zlSystems Set 版本号='10.35.90.0027' Where 编号=&n_System;
Commit;