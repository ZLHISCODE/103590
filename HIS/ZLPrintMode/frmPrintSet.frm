...
'2016-12-29
'调整说明：
'1.非当前打印机不设置Printer对象，避免网络打印机加载缓慢现象；
'2.窗体初始化都不对Printer对象设置，只有“保存”操作才设置；
...
Private Sub Form_Load()
    ...
    For i = 0 To Printers.Count - 1
        ...
        If Printers(i).DeviceName = DefDeviceName Then
            '设置当前objPrinter对象的属性
            Call SetPrinterPaper(objPrinter)    '打印机集合加载对象
            '调整界面控件状态
            Call PrinterChanged(objPrinter)
        ...
        If Printers(i).DeviceName = strDevName Then 'IIf(cboPrinterName.Text = "", DefDeviceName, cboPrinterName.Text) Then
            ...
            If Not objPrinter Is Nothing Then
                ...
                For j = 1 To lngCount
                    ...
                    If intTemp >= 1 And intTemp <= 41 Then '只列出标准支持的纸张
                        If intTemp = intPaperSize Then
                            objPrinter.PaperSizes = objPrinter.PaperSizes & "," & intTemp & "*" '原有的
                        Else
                ...
                For j = 1 To lngCount
                    ...
                    If intTemp >= 1 And intTemp <= 11 Then '只列出标准支持的进纸大小
                        If intTemp = intPaperBin Then
                            objPrinter.PaperBins = objPrinter.PaperBins & "," & intTemp & "*"  '原有的
                        Else
