...
Begin VB.Form frmLabItemSons 
   ...
   Begin VB.PictureBox picEdit 
      ...
      Begin VB.CommandButton cmdFind 
         Height          =   300
         Left            =   3495
         Picture         =   "frmLabItemSons.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   3
         TabStop         =   0   'False
   ...
   End
End
Attribute VB_Name = "frmLabItemSons"
...
Public Function zlRefresh(lngItemID As Long) As Boolean
    ...
    gstrSql = "Select 组合项目, 操作类型 From 诊疗项目目录 Where ID = [1]"
    ...
    If mbln组合 Then
        gstrSql = "Select V.ID, R.排列序号 As 序号, V.编码, V.中文名, V.英文名" & vbNewLine & _
                "From 检验报告项目 R, 诊治所见项目 V" & vbNewLine & _
                "Where R.报告项目id = V.ID And R.诊疗项目id = [1] And 细菌id Is Null" & vbNewLine & _
                "Order By R.排列序号"
    Else
        gstrSql = "Select V.ID, R.排列序号 As 序号, V.编码, V.中文名, V.英文名" & vbNewLine & _
                "From 检验报告项目 R, 检验细菌 V" & vbNewLine & _
                "Where R.细菌id = V.ID And R.诊疗项目id = [1] And 报告项目id Is Null" & vbNewLine & _
                "Order By R.排列序号"
    End If
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSql, Me.Caption, lngItemID)
...
Public Function zlEditSave() As Long
    ...
    '数据保存
    gstrSql = "Zl_检验报告项目_Edit(" & mlngItemID & ",'" & strLists & "')"
...
Private Sub cmdFind_Click()
    ...
    If mbln组合 Then
        If Me.chkUpper.Value = 0 Then
            ...
            gstrSql = "Select L.诊治项目id As ID, I.编码, I.名称 As 中文名, L.缩写 As 英文名" & vbNewLine & _
                    "From 诊疗项目目录 I, 检验报告项目 R, 检验项目 L" & vbNewLine & _
                    "Where I.ID = R.诊疗项目id And R.报告项目id = L.诊治项目id And I.组合项目 <> 1 And I.操作类型 = '" & mstr类型 & "' And" & vbNewLine & _
    ...
    Else
        If Me.chkUpper.Value = 0 Then
            ...
            gstrSql = "Select V.ID, V.编码, V.中文名, V.英文名" & vbNewLine & _
                    "From 检验细菌 V" & vbNewLine & _
                    "Where V.编码 Like '" & strFind & "%' Or Upper(V.中文名) Like '" & gstrMatch & strFind & "%' Or Upper(V.英文名) Like '" & gstrMatch & strFind & "%'"
        Else
            strFind = Trim(Me.txtFind.Text)
            gstrSql = "Select V.ID, V.编码, V.中文名, V.英文名" & vbNewLine & _
                    "From 检验细菌 V" & vbNewLine & _
                    "Where V.编码 Like '" & strFind & "%' Or V.中文名 Like '" & gstrMatch & strFind & "%' Or V.英文名 Like '" & gstrMatch & strFind & "%'"
        End If
    End If
    Err = 0: On Error GoTo ErrHand
