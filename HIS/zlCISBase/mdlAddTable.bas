...
'theTable的父窗体要加载VisItem控件数组
    ...
    gstrSql = "Select * From 诊治所见项目 Where ID=[1]"
    Set rsTmp = zldatabase.OpenSQLRecord(gstrSql, "", VisItemID)
    ...
    With frmParent.VisItem(frmParent.VisItem.UBound)
        ...
        Select Case rsTmp("编码")
            ...
            Case "101009" '当前时间
                iItemLen = 19: sItemFormat = "YYYY-MM-DD HH:MM:SS"
                sDefaultValue = Format(Date & " " & Time, sItemFormat)
            Case Else
...
Public Sub ReadTable(theTable As TTF160Ctl.F1Book, ByVal ElementID As Long, Optional ByVal Seq As Integer = 1, Optional objProgBar As ProgressBar)
    ...
    With theTable
        '读取报表属性
        .Tag = ElementID
        gstrSql = "Select * From 病历所见单 Where 元素ID=[1] And (控件号=[2] Or 控件号 Is Null) And 控件类=3"
        Set rsTmp = zldatabase.OpenSQLRecord(gstrSql, "", ElementID, -1 * Seq)
        ...
        gstrSql = "Select * From 病历所见单 Where 元素ID=[1] And (控件号=[2] Or 控件号=0) And 控件类 is Null"
        Set rsTmp = zldatabase.OpenSQLRecord(gstrSql, "", ElementID, -1 * Seq)
...
Public Sub ReadTable_Patient(theTable As TTF160Ctl.F1Book, ByVal ElementID As Long, Optional ByVal Seq As Integer = 1, Optional objProgBar As ProgressBar)
    ...
    With theTable
        '读取报表属性
        gstrSql = "Select * From 病人病历所见单 Where 病历ID=[1] And 控件号=[2] And 控件类=3"
        Set rsTmp = zldatabase.OpenSQLRecord(gstrSql, "", ElementID, -1 * Seq)
        ...
        gstrSql = "Select * From 病人病历所见单 Where 病历ID=[1] And 控件号=[2] And 控件类 is Null"
        Set rsTmp = zldatabase.OpenSQLRecord(gstrSql, "", ElementID, -1 * Seq)
...
Public Sub ReadForm(theForm As Object, ContainerName As String, ByVal VisFormID As String, Optional FormWidth As Long, Optional FormHeight As Long, Optional objProgBar As ProgressBar)
    ...
    gstrSql = "Select a.*,b.表示法,b.类型,b.长度,b.小数,b.数值域,b.替换域,b.中文名,b.编码 As 所见项编码 From 病历所见单 a,诊治所见项目 b Where a.元素ID=[1] And a.控件号>=0 " + _
        "And a.所见项ID=b.ID(+)"
    Set rsTmp = zldatabase.OpenSQLRecord(gstrSql, "查询所见单项目", VisFormID)
