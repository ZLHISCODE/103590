...
Public Sub OutPutData(ByVal strItem As String, ByVal strMess As String)
'功能：调试程序使用，输出部分业务数据或指定文本
'参数：
'  strItem：业务环节/分类/过程/函数
'  strMess：输出内容
    ...
    '检查文件是否存在
    Set objTarget = objFile.OpenTextFile(App.Path & "\zlDrugPacker.log")
    If Err <> 0 Then
        '创建目标文件
        Set objFile = CreateObject("Scripting.FileSystemObject")
        Set objTarget = objFile.CreateTextFile(App.Path & "\zlDrugPacker.log", True)
        objTarget.Close
    End If
    ...
    Open App.Path & "\zlDrugPacker.log" For Append Shared As #1
    
    Print #1, Now & "  " & strItem & vbCrLf & strMess & vbCrLf
    Close #1
