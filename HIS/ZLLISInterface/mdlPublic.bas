...
2       Err = 0
3       DoEvents
4       On Error GoTo errH
5       Set objRegister = CreateObject("zlRegister.clsRegister")
6       If Not objRegister Is Nothing Then
7           Set gcnOracle = objRegister.GetConnection(strServerName, strUserName, strUserPwd, True, , strError, True)
8               If strError <> "" And strError <> "无须返回错误信息" Then
9                   MsgBox strError, vbInformation, "中联信息"
10                  OraDataOpen = False
11                  Set objRegister = Nothing
12                  Exit Function
13              End If
14      Else
errH:
15          Err.Clear
16          If gcnOracle Is Nothing Then Set gcnOracle = New ADODB.Connection
17          With gcnOracle
                
18              If .State = adStateOpen Then .Close
19              .Provider = "MSDataShape"
20              .Open "Driver={Microsoft ODBC for Oracle};Server=" & strServerName, strUserName, TranPasswd(strUserPwd)
21              If Err <> 0 Then
                    '保存错误信息
22                  strError = Err.Description
23                  If InStr(strError, "自动化错误") > 0 Then
24                      WriteLog "mdlPublic", "OraDataOpen", "连接串无法创建，请检查数据访问部件是否正常安装。"
25                  ElseIf InStr(strError, "ORA-12154") > 0 Then
26                      WriteLog "mdlPublic", "OraDataOpen", "无法分析服务器名，" & vbCrLf & "请检查在Oracle配置中是否存在该本地网络服务名（主机字符串）。"
27                  ElseIf InStr(strError, "ORA-12541") > 0 Then
28                      WriteLog "mdlPublic", "OraDataOpen", "无法连接，请检查服务器上的Oracle监听器服务是否启动。"
29                  ElseIf InStr(strError, "ORA-01033") > 0 Then
30                      WriteLog "mdlPublic", "OraDataOpen", "ORACLE正在初始化或在关闭，请稍候再试。"
31                  ElseIf InStr(strError, "ORA-01034") > 0 Then
32                      WriteLog "mdlPublic", "OraDataOpen", "ORACLE不可用，请检查服务或数据库实例是否启动。"
33                  ElseIf InStr(strError, "ORA-02391") > 0 Then
34                      WriteLog "mdlPublic", "OraDataOpen", "用户" & UCase(strUserName) & "已经登录，不允许重复登录(已达到系统所允许的最大登录数)。"
35                  ElseIf InStr(strError, "ORA-01017") > 0 Then
36                      WriteLog "mdlPublic", "OraDataOpen", "由于用户、口令或服务器指定错误，无法登录。"
37                  ElseIf InStr(strError, "ORA-28000") > 0 Then
38                      WriteLog "mdlPublic", "OraDataOpen", "由于用户已经被禁用，无法登录。"
39                  Else
40                      WriteLog "mdlPublic", "OraDataOpen", CStr(Erl()) & "," & strError
41                  End If
                
42                  OraDataOpen = False
43                  Exit Function
44              End If
45          End With
46      End If
...
47      Err = 0
48      On Error GoTo errHand
    
49      gstrDbUser = UCase(strUserName)
    
    
50      Call gobjComLib.InitCommon(gcnOracle)
51      Call gobjComLib.SetDbUser(gstrDbUser)
        
        '2012-05-23 读取本地参数设置
52      mblnOldVer = True
53      gstrSql = "Select 版本号 From zlSystems Where 编号=100"
54      Set rsTmp = gobjComLib.zlDataBase.OpenSQLRecord(gstrSql, "取版本号")
55      Do Until rsTmp.EOF
56          WriteLog "mdlPublic", "OraDataOpen", "版本：" & rsTmp!版本号
            
57          If Trim$("" & rsTmp!版本号) >= "10.30.10" Then mblnOldVer = False
58          WriteLog "mdlPublic", "OraDataOpen", "版本：" & rsTmp!版本号 & "," & IIf(mblnOldVer, "True", "False")
59          rsTmp.MoveNext
60      Loop
61      Call GetUserInfo
        
62      If mblnOldVer Then
63          If CheckRegInfo = True Then
64              gstrPrivs = gobjComLib.GetPrivFunc(100, 1215)
65              OraDataOpen = True
66          Else
67              gcnOracle.Close
68              Set gcnOracle = Nothing
69          End If
70      Else
71          mstrVirtualHis = gobjComLib.GetPrivFunc(100, 1215)    '读取虚拟模块权限
72          If Right(mstrVirtualHis, 1) <> ";" Then mstrVirtualHis = mstrVirtualHis & ";"
73          If Left(mstrVirtualHis, 1) <> ";" Then mstrVirtualHis = ";" & mstrVirtualHis

74          If InStr(mstrVirtualHis, ";基本;") <= 0 Then
75              WriteLog "mdlPublic", "OraDataOpen", "OraOpen,权限不足，请在管理工具中将“1215 第三方LIS接口”权限授予" & gstrDbUser
76              gcnOracle.Close
77              Set gcnOracle = Nothing
78          Else
79              OraDataOpen = True
80          End If

81          mstrVirtualPeis = gobjComLib.GetPrivFunc(2100, 2138)    '读取体检虚拟模块权限
82          If Right(mstrVirtualPeis, 1) <> ";" Then mstrVirtualPeis = mstrVirtualPeis & ";"
83          If Left(mstrVirtualPeis, 1) <> ";" Then mstrVirtualPeis = ";" & mstrVirtualPeis
    
84          gstrPrivs = gobjComLib.GetPrivFunc(100, 1215)
85          If Right(gstrPrivs, 1) <> ";" Then gstrPrivs = gstrPrivs & ";"
86          If Left(gstrPrivs, 1) <> ";" Then gstrPrivs = ";" & gstrPrivs
87      End If
         
88      Exit Function
    
errHand:
89      WriteLog "mdlPublic", "OraDataOpen", "OraOpen " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
90      OraDataOpen = False
91      Err = 0
End Function
...
106             WriteLog "mdlPublic", "CheckRegInfo", "未能通HIS系统注册相关检查，请先检查ZLLIS能否正常运行！"
                Exit Function
            End If
...
146                         WriteLog "mdlPublic", "CheckRegInfo", "授权码不正确！" & vbNewLine & _
                                   strUnti & vbNewLine & _
                                   strCode & vbNewLine & _
...
150                         If strPrivs <> "" Then mstrVirtualHis = strPrivs
                    ...
                    End If
                Else
152                 WriteLog "mdlPublic", "CheckRegInfo", IIf(CONS_DEBUG = 1, "调试部件-", "正式部件-") & "已超过试用期限！" & str日期
                End If
            Else
154             WriteLog "mdlPublic", "CheckRegInfo", "试用日期错误！"
            End If
        Else
156         WriteLog "mdlPublic", "CheckRegInfo", "部件所在目录(" & App.Path & ")缺少授权文件（RegFile.ini）！"
        End If
        Exit Function
hErr:
158     WriteLog "mdlPublic", "CheckRegInfo", "CheckReg " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
...
190         GetApplication = Mid(strData, 2)
        ...
        Exit Function
errH:
192     WriteLog "mdlPublic", "GetApplication", CStr(Erl()) & "行出现错误，" & Err.Number & " " & Err.Description
End Function
...
156     WriteLog "mdlPublic", "GetDeptPatiList", CStr(Erl()) & "行出现错误," & Err.Number & " " & Err.Description
End Function
...
138     WriteLog "mdlPublic", "InsertReport", "InsertReport " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
...
158     WriteLog "mdlPublic", "zlLisBlobSql", "zlLisBlobSql " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
'################################################################################################################
...
104     DeleteReport = True
        Exit Function
errH:
106     WriteLog "mdlPublic", "DeleteReport", "DeleteReport " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
...
130         GetClinicItem = Mid(strData, 2)
        ...
        Exit Function
errH:
132     WriteLog "mdlPublic", "GetClinicItem", "GetClinicItem " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
...
112         GetItemList = Mid(strData, 2)
        ...
        Exit Function
errH:
114     WriteLog "mdlPublic", "GetItemList", "GetItemList " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
...
104     SetRegister = True
        ...
        Exit Function
errH:
106     WriteLog "mdlPublic", "SetRegister", "SetRegister " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
...
138     GetAllItem = astrItem
        ...
        Exit Function
errH:
140     WriteLog "mdlPublic", "GetAllItem", "GetAllItem " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
...
176     WriteLog "mdlPublic", "UpdateTestResults", "UpdateTestResults " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
...
Public Function zlLISRegister(ByVal lngDeviceID As Long, ByVal lngID As Long, ByVal strSampleNo As String, ByRef strErrInfo As String) As Boolean
        ...
        On Error GoTo errH
        WriteLog "mdlPublic", "zlLISRegister", "授权版本" & mblnOldVer & ",权限" & mstrVirtualHis
...
208     WriteLog "mdlPublic", "zlLISRegister", "zlLISRegister " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
...
118     WriteLog "mdlPublic", "zlLisUnRegister", "zlLisUnRegister " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
...
206     WriteLog "mdlPublic", "ZLLisInsterReport", "ZLLisInsterReport " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
...
148     WriteLog "mdlPublic", "zlLisUnAudit", "zlLisUnAudit " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
...
120     WriteLog "mdlPublic", "GetAllDevice", "GetAllDevice " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
...
Public Function CriticalvalueNotice(ByVal lngID As Long, ByVal strNoticeTitle As String, ByVal strNotice As String) As Long
    ...
    If lngPatiID <= 0 Then
        WriteLog "mdlPublic", "CriticalvalueNotice", "CriticalvalueNotice :ID" & lngID & "无对应的医嘱记录！"
        Exit Function
    End If
    ...
    Else
        WriteLog "mdlPublic", "CriticalvalueNotice", "CriticalvalueNotice : 标题和内容不能为空！"
    End If
     
    Exit Function
errH:
     WriteLog "mdlPublic", "CriticalvalueNotice", "CriticalvalueNotice :" & CStr(Erl()) & "行," & Err.Description
End Function
...
200  WriteLog "mdlPublic", "Incomeverify", "Incomeverify :" & strErrInfo
End Function
...
132         rsTemp.MoveNext
        ...
        Exit Sub
errHand:
134     WriteLog "mdlPublic", "GetUserInfo", "GetUserInfo " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
136     Err = 0
End Sub
...
136     WriteLog "mdlPublic", "CheckChargeState", "CheckChargeState " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function
...
116     WriteLog "mdlPublic", "CheckExesState", "CheckExesState " & CStr(Erl()) & "," & Err.Number & " " & Err.Description
End Function


Private Sub WriteLog(ByVal strModule As String, ByVal strFunctin As String, ByVal strOutput As String)
    '------------------------------------------------------
    '--  功能:根据调试标志,写日志到当前目录
    '------------------------------------------------------
    
    '调用公共方法记录日志
    Call gobjComLib.LogWrite("LIS老版通讯程序错误调试日志", strModule, strFunctin, strOutput)
End Sub
