...
Private Sub Cmd确定_Click()
    ...
    If strSQL = "" Then
        ...
        If RecSystems.State = 1 Then RecSystems.Close
        strSQL = "   Select Count(*) Records From zlprogfuncs " & _
                 "   Where 系统 Is Null" & _
                 "   And 序号 in (Select 序号 From zlRoleGrant G,session_roles S Where G.角色=S.Role)"
        
        Set RecSystems = zlDatabase.OpenSQLRecord(strSQL, Me.Caption)
        strSQL = ""
        
        If Not RecSystems.EOF Then
            If RecSystems!Records <> 0 Then
                strSQL = "REPORT"
            End If
        End If
