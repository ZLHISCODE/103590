...
Private Function GetDepend() As Boolean
    ...
    With rsDepend
        ...
        gstrSQL = "Select ID,上级ID,编码,简码,名称,末级,地址||电话 as 电话地址,开户银行,帐号,税务登记号 From 药品供应商 Where 末级<>1 Or " & _
              " To_Char(撤档时间,'yyyy-MM-dd')='3000-01-01' Start with 上级ID is Null Connect by prior ID=上级ID"
        Call SQLTest(App.Title, "付款单", gstrSQL)
        .Open gstrSQL, gcnOracle
        Call SQLTest
        
        If .EOF Then
            MsgBox "供应商的信息不全，请在供应商管理中进行设置！", vbInformation, gstrSysName
            Exit Function
        End If
...
Public Sub ShowCard(FrmMain As Form, ByVal str单据号 As String, ByVal int编辑状态 As Integer, _
    ...
    mintUnit = GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\付款事务", "药品单位", "0")
...
Private Sub Cmd核销_Click()
    ...
    If mint编辑状态 = 4 Then    '查看
        '打印
        FrmBillPrint.ShowME Me, glngSys, IIf(glngSys \ 100 = 1, "zl1_bill_1320_1", "zl8_bill_1320_1"), mint记录状态, 0, 1320, "付款单", TxtNo.Text
        '退出
        Unload Me
        Exit Sub
    End If
    ...
    If mint编辑状态 = 3 Then        '审核
        If SaveVerify = True Then
            ...
            If GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\付款事务", "审核打印", "0") = "1" Then
                '打印
                If InStr(mstrPrivs, "付款通知单打印") <> 0 Then
    ...
    If BlnSuccess = True Then
        ...
        mblnSave = False
        mblnSuccess = True
        If GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\付款事务", "存盘打印", "0") = "1" Then
            '打印
            If InStr(mstrPrivs, "付款通知单打印") <> 0 Then
                ReportOpen gcnOracle, glngSys, IIf(glngSys \ 100 = 1, "zl1_bill_1320_1", "zl8_bill_1320_1"), Me, "单据编号=" & TxtNo.Text, "记录状态=" & mint记录状态, 2
            End If
...
Private Sub initGrid()
    ...
    With mshPurchaseList
        ...
        .TextMatrix(0, mconintcol药品信息) = "商品信息"
...
Private Sub initCard()
    ...
    Else
        ...
        gstrSQL = "SELECT a.序号, a.金额, a.结算方式, a.结算号码, a.摘要,a.付款序号,a.填制人,a.填制日期,a.审核人,a.审核日期, " _
                & " b.名称, b.id,b.地址 || b.电话 as 电话地址,开户银行,税务登记号 " _
               ...
               & " order by a.序号 "
        Call SQLTest(App.Title, Me.Caption, gstrSQL)
        rsPayment.Open gstrSQL, gcnOracle
        Call SQLTest
...
Private Sub tvwProvider_DblClick()
    ...
    With rsProvider
        gstrSQL = "Select 名称,地址||电话 as 电话地址,开户银行,帐号,税务登记号 " _
            & " From 药品供应商  " _
...
Private Sub RefreshImprest()
    ...
    End If
    Call SQLTest(App.Title, Me.Caption, gstrSQL)
    rsImprest.Open gstrSQL, gcnOracle
    Call SQLTest
...
Private Sub txt供药单位_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    With rec供应商
        gstrSQL = "Select ID,编码,名称,简码,地址||电话 as 电话地址,开户银行,帐号,税务登记号  From 药品供应商 Where (编码 like '" & UCase(Txt供药单位) & "%' Or 名称 like '" & UCase(Txt供药单位) & "%' Or 简码 like '" & UCase(Txt供药单位) & "%') And To_Char(撤档时间,'yyyy-MM-dd')='3000-01-01' And 末级=1"
