...
Private Const mconIntCol构原产地 As Integer = 4
Private Const mconIntCol构单位 As Integer = 5
Private Const mconIntCol构数量 As Integer = 6
Private Const mconIntCol构组成数量 As Integer = 7
Private Const mconIntCol构可用数量 As Integer = 8
...
Private Const mconIntCol构药品编码和名称 As Integer = 18
Private Const mconIntCol构药品编码 As Integer = 19
Private Const mconIntCol构药品名称 As Integer = 20
...
Private Sub cboStock_Click()
    ...
    Set rsStructure = zldatabase.OpenSQLRecord(gstrSQL, "判断库房性质", cboStock.ItemData(cboStock.ListIndex))
    Do While Not rsStructure.EOF
        str库房性质 = str库房性质 & "," & rsStructure!工作性质
        rsStructure.MoveNext
    Loop
    If str库房性质 Like "*中药*" Or str库房性质 Like "*制剂室*" Then mbln中药库房 = True
    mshStructure.ColWidth(mconIntCol构原产地) = IIf(mbln中药库房, 1000, 0)
...
Private Sub Form_Load()
    ...
    mshStructure.ColWidth(mconIntCol构原产地) = IIf(bln中药库房, 1000, 0)
...
Private Sub initGrid()
    ...
    With mshStructure
        ...
        .TextMatrix(0, mconIntCol构产地) = "生产商"
        .TextMatrix(0, mconIntCol构原产地) = "原产地"
...
Private Sub mshbill_CommandClick()
    ...
    gstrSQL = "" & _
        ...
        "   ,D.商品名,D.药品来源,D.基本药物,D.规格,D.产地 as 生产商," & IIf(mbln中药库房, "d.原产地,", "") & "D.药品ID," & _
             strUnitQuantity & _
        "    S.库存金额, D.最大效期,D.是否变价,D.加成率,D.药房分批核算,E.库房货位 " & _
        " FROM  " & _
        "    (SELECT DISTINCT J.名称 剂型,M.类别,M.编码,M.名称 通用名称,A.名称 商品名,D.药品来源,D.基本药物," & _
        "        M.规格,M.产地, d.原产地,D.药名ID, D.药品ID, M.计算单位,NVL (TO_CHAR (D.最大效期, '9999990'), 0) 最大效期,D.门诊单位," & _
            ...
            Select Case .Col
                ...
                Case mconIntCol药名
                    If strKey <> "" Then
                        ...
                        "      D.药品来源,D.基本药物,D.规格,D.产地 as 生产商," & IIf(mbln中药库房, "d.原产地,", "") & "D.药品ID," & _
                               strUnitQuantity & _
                        "      S.库存金额, D.最大效期,D.是否变价,D.加成率,D.药房分批核算,E.库房货位  " & _
                        " FROM  " & _
                        "     (SELECT DISTINCT J.名称 剂型,M.类别,M.编码,M.名称 通用名称,A.名称 商品名,d.药品来源,d.基本药物," & _
                        ...
                        ElseIf RecReturn.RecordCount = 1 Then
                            If SetColValue(.Row, RecReturn!药品ID, "[" & RecReturn!编码 & "]", RecReturn!通用名称, IIf(IsNull(RecReturn!商品名), "", RecReturn!商品名), _
                               ...
                               RecReturn!比例系数, RecReturn!是否变价, RecReturn!加成率, RecReturn!药房分批核算, RecReturn!原产地) = False Then
                               RecReturn.Close
                               Cancel = True
                               Exit Sub
                            End If
...
'                    .TextMatrix(intRow, mconIntCol构采购价) =Str.FormatEx((.TextMatrix(intRow, mconIntCol构售价金额) - .TextMatrix(intRow, mconintCol构差价)) / (IIf(dblConstruct = 0, 1, dblConstruct)), intCostDigit)
                ...
                If Not CheckUsableNum(cboStock.ItemData(cboStock.ListIndex), .TextMatrix(intRow, mconintcol构药品id), 0, dblConstruct, 1, txtNo.Caption, 3, mint库存检查, intNumberDigit) Then
                    GetStructureNum = False
                    Exit Function
                End If
            End If
...
Private Function SetColValue(ByVal intRow As Integer, ByVal int药品id As Long, _
    ...
    ByVal int是否变价 As Integer, ByVal dbl加成率 As Double, ByVal int药房分批核算 As Integer, ByVal str原产地 As String) As Boolean
...
Private Function SetStructure(ByVal int药品id As Long) As Boolean
    ...
    On Error GoTo errHandle
    If mint编辑状态 <> 4 Then
        gstrSQL = "SELECT DISTINCT B.药品ID,'[' || F.编码 || ']' As 编码,F.名称 As 通用名称,E.名称 AS 商品名称, F.规格, C.上次产地,C.原产地,F.计算单位 AS 单位,c.平均成本价," & _
                  " C.实际差价,C.实际金额, to_char(D.现价, " & mstrPriceFormat & ") 售价, " & _
                  " (A.分子 / A.分母) AS 组成,C.可用数量,B.加成率,F.是否变价,B.药房分批 AS 药房分批核算, Nvl(F.是否变价, 0) 定价,b.剂量系数 " & _
                  ...
                  " AND (F.站点 = [3] Or F.站点 is Null) And A.药品ID =[2] " & _
                  " order by 药品id"
        ...
        With mshStructure
            ...
            Do While Not rsStructure.EOF
                ...
                .TextMatrix(.Row, mconIntCol构原产地) = IIf(IsNull(rsStructure!原产地), "", rsStructure!原产地)
                .TextMatrix(.Row, mconIntCol构单位) = rsStructure!单位
                .TextMatrix(.Row, mconIntCol构售价) = zlStr.FormatEx(rsStructure!售价, mintPriceDigit, , True)
    ...
    Else            '查看
        gstrSQL = " SELECT DISTINCT A.药品ID,'[' || F.编码 || ']' As 编码,F.名称 As 通用名称,E.名称 AS 商品名称,F.规格," & _
                  "     A.产地,A.原产地,F.计算单位 AS 单位,A.实际数量,A.成本价,A.成本金额,A.零售价,A.零售金额,A.差价 " & _
                  " FROM " & _
                  "     (SELECT a.药品ID,a.产地,b.原产地,a.实际数量,a.成本价,a.成本金额,a.零售价,a.零售金额,a.差价 FROM 药品收发记录 a,药品库存 b " & _
                  "     WHERE a.药品id=b.药品id and b.库房id=[5] and a.NO=[1] AND a.单据=3 AND a.记录状态=[3] " & _
                  "     AND a.入出系数=-1 AND a.扣率=[4] AND a.费用ID =[2]) A," & _
                  "     药品规格 B,收费项目别名 E,收费项目目录 F " & _
                  " WHERE A.药品ID = B.药品ID AND B.药品ID=F.ID " & _
                  " AND B.药品ID = E.收费细目ID(+) AND E.性质(+)=3 " & _
                  " order by 药品id"
        Set rsStructure = zldatabase.OpenSQLRecord(gstrSQL, MStrCaption, txtNo.Tag, int药品id, mint记录状态, mshBill.Row, cboStock.ItemData(cboStock.ListIndex))
        ...
        With mshStructure
            ...
            Do While Not rsStructure.EOF
                ...
                .TextMatrix(.Row, mconIntCol构采购价) = zlStr.FormatEx(rsStructure!成本价, mintCostDigit, , True)
...
Private Sub mshDrug_KeyPress(KeyAscii As Integer)
    With mshDrug
        If KeyAscii = 13 Then
            If mbln中药库房 Then
                If Not SetColValue(mshBill.Row, .TextMatrix(.Row, 10), "[" & .TextMatrix(.Row, 2) & "]", .TextMatrix(.Row, 3), .TextMatrix(.Row, 4), _
                    .TextMatrix(.Row, 5), .TextMatrix(.Row, 6), .TextMatrix(.Row, 7), .TextMatrix(.Row, 11), .TextMatrix(.Row, 14), _
                    IIf(IsNull(.TextMatrix(.Row, 16)), "0", .TextMatrix(.Row, 16)), .TextMatrix(.Row, 13), Val(.TextMatrix(.Row, 17)), _
                    Val(.TextMatrix(.Row, 18)), Val(.TextMatrix(.Row, 19)), Val(.TextMatrix(.Row, 9))) Then
                    mshBill.SetFocus
            ...
            For intLop = 1 To .rows - 1
                If Trim(.TextMatrix(intLop, mconIntCol药名)) <> "" Then
                    ...
                    If gtype_UserSysParms.P275_零差价管理模式 = 2 Then
                        If IsPriceAdjustMod(Val(.TextMatrix(intLop, 0))) = True Then
                            If Val(.TextMatrix(intLop, mconIntCol采购价)) <> Val(.TextMatrix(intLop, mconIntCol售价)) Then
                                MsgBox "第" & intLop & "行协定药品已启用零差价管理，但入库单据售价和成本价不一致，不能进行业务，请检查！", vbInformation + vbOKOnly, gstrSysName
                                mshBill.SetFocus
                                .Row = intLop
                                .MsfObj.TopRow = intLop
                                Exit Function
        ...
        For n = 1 To recSort.RecordCount
            ...
            If .TextMatrix(intRow, 0) <> "" Then
                ...
                If Val(Split(.TextMatrix(intRow, mconIntCol原销期), "||")(2)) = 0 Then
                    '如果是定价药品，则售价取原始价格保存
                    dbl零售价 = Get零售价(Val(Split(.TextMatrix(intRow, mconIntCol原销期), "||")(2)) = 1, lng药品id, lng库房id, 0)
