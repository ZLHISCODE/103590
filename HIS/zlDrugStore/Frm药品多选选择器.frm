    ...
    Case 1
        With Msf药品规格
            ...
            If BlnStartUp = False Then
                ...
                End If
                .Row = 1
                Call RestoreFlexState(Msf药品规格, App.ProductName & Me.Name)
    ...
    Case 0
        With Msf批次
            ...
            If BlnStartUp = False Then
                ...
                Call RestoreFlexState(Msf批次, App.ProductName & Me.Name)
...
Private Sub Form_Resize()
    ...
    With Msf药品规格
        .Width = Me.ScaleWidth
        .Height = IIf(Msf批次.Visible = False, Me.ScaleHeight - .Top, Msf批次.Top - 45 - .Top)
        If .RowIsVisible(.Row) = False Then .TopRow = .TopRow + IIf(.Row > .TopRow, 1, -1)
    End With
    
    With Msf批次
        .Width = Me.ScaleWidth
        .Height = Me.ScaleHeight - Msf药品规格.Top - Msf药品规格.Height - 45
    End With
End Sub

Private Sub Form_Unload(Cancel As Integer)
    Call SaveWinState(Me, App.ProductName)
    Call SaveFlexState(Msf药品规格, App.ProductName & Me.Name)
    Call SaveFlexState(Msf批次, App.ProductName & Me.Name)
End Sub
    ...
    If In_编辑状态 = 2 Then
        ...
        With RecStock
            ...
            If bln空批次 Then
                ...
                gstrSQL = "Select 1 RID,名称,0 批次,'' As 入库日期,'新增批次药品' 批号,NULL 生产日期,sysdate " & strSql效期 & ",'' 产地,'' 成本价,''售价," & _
...
Private Function RefreshData() As Boolean
    ...
    With RecPhysic
        ...
        gstrSQL = gstrSQL & " (SELECT DISTINCT J.名称 剂型,C.编码 药名编码,C.名称 AS 通用名称,0 AS 药典ID,M.分类ID AS 用途分类ID,M.计算单位 AS 剂量单位,C.编码 AS 药品编码," & _
                ...
                " FROM (" & _
                "     Select A.* From 收费项目目录 A,收费项目别名 B" & _
                "     Where A.ID=B.收费细目ID AND A.类别 IN ('5','6','7') And (A.站点 = '" & gstrNodeNo & "' Or A.站点 is Null)" & strFindString & _
                " ) C,药品规格 D,收费项目别名 A,药品剂型 J,药品特性 T,诊疗项目目录 M," & _
...
'       如果目标库房是服务于住院病人，则住院用药可以进入；
        gstrSQL = gstrSQL & "" & _
            ...
            " and ([3] is null" & _
                ...
                " or exists(select 1 from 部门性质说明 where 工作性质='制剂室' and 部门id=[3])" & _
                " or decode(C.服务对象,1,1,3,1,0)=(select distinct '1' from 部门性质说明 where 工作性质 like '%药房' and 部门id=[3] and 服务对象 in(1,3))" & _
...
'        If lng供应商ID <> 0 Then gstrSQL = gstrSQL & " And (上次供应商ID Is NULL Or 上次供应商ID=" & lng供应商ID & ")"
    ...
    Set RecPhysic = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, StrFindStyle & strInput & "%", Lng源库房ID, Lng目库房ID, Lng使用部门ID, Format(zlDatabase.Currentdate(), "yyyy"))
