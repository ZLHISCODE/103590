...
Private Sub GetSlicesData()
    ...
    strSql = "select a.Id,c.病理号,c.病理医嘱ID, e.姓名, c.检查类型, a.材块ID,b.序号,b.取材位置, d.标本名称, d.标本类型, a.制片类型, a.制片方式,a.制片数,b.取材时间, a.当前状态,a.清单状态 " & _
                " from 病理制片信息 a, 病理取材信息 b, 病理检查信息 c, 病理标本信息 d, 病人医嘱记录 e " & _
                IIf(optRequisitionTime.value, ",病理申请信息 f ", "") & _
                " Where a.材块id = b.材块id And b.病理医嘱ID = c.病理医嘱ID and b.确认状态=1 And c.医嘱ID = e.ID And b.标本id = d.标本id " & _
...
'过滤查询出来的制片数据
    ...
    Select Case tabFilter.Selected.Tag
        Case "常规"
            strStudyTypeFilter = "检查类型=0"
        Case "快速石蜡"
            strStudyTypeFilter = "检查类型=5"
        Case "冰冻"
            strStudyTypeFilter = "检查类型=1"
        Case "细胞"
            strStudyTypeFilter = "检查类型=2"
        Case "会诊"
            strStudyTypeFilter = "检查类型=3"
        Case "尸检"
            strStudyTypeFilter = "检查类型=4"
    End Select
...
'载入指定的病理制片数据
    ...
    strSql = "select a.ID, a.Id,c.病理号,c.病理医嘱ID, e.姓名, c.检查类型, a.材块ID,b.序号,b.取材位置, d.标本名称, d.标本类型, a.制片类型, a.制片方式, a.制片数,a.当前状态,a.清单状态 " & _
                 " from 病理制片信息 a, 病理取材信息 b, 病理检查信息 c, 病理标本信息 d, 病人医嘱记录 e " & _
                " Where a.材块id = b.材块id And b.病理医嘱ID = c.病理医嘱ID And c.医嘱ID = e.ID And b.标本id = d.标本id " & _
                " and c.病理医嘱ID=[1] and a.当前状态 <> 2 " & _
                " order by 检查类型,病理号,材块ID,当前状态 "
    'If mblnMoved Then strSql = GetMovedDataSql(strSql)
...
Private Sub InitFilterPage()
    With tabFilter
        ...
        .InsertItem 0, "常  规", picTag.hWnd, 0
        .Item(tabFilter.ItemCount - 1).Tag = "常规"
'        .Item(tabFilter.ItemCount - 1).Visible = true
'        If Not .Item(tabFilter.ItemCount - 1).Visible Then lngHideCount = lngHideCount + 1
        
        
        .InsertItem 1, "快速石蜡", picTag.hWnd, 0
        .Item(tabFilter.ItemCount - 1).Tag = "快速石蜡"
        
        
        .InsertItem 2, "冰  冻", picTag.hWnd, 0
        .Item(tabFilter.ItemCount - 1).Tag = "冰冻"
        
        
        .InsertItem 3, "细  胞", picTag.hWnd, 0
        .Item(tabFilter.ItemCount - 1).Tag = "细胞"


        .InsertItem 4, "会  诊", picTag.hWnd, 0
        .Item(tabFilter.ItemCount - 1).Tag = "会诊"
        
         
        .InsertItem 5, "尸  检", picTag.hWnd, 0
        .Item(tabFilter.ItemCount - 1).Tag = "尸检"
