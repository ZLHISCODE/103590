VERSION 5.00
Object = "{9AC3F5EF-1E37-407F-BF04-027E86C0AD8D}#4.0#0"; "zlQueueOper.ocx"
...
'读取排队叫号参数
    ...
    mstrCurTechnicRoomName = NeedNo(strRoomName)
    mstrCurTechnicDevice = NeedName(strRoomName)
...
'Pacs排队叫号呼叫事件处理
    ...
    If lngCallWay = cwOrder Or lngCallWay = cwSpecify Or lngCallWay = cwWaitRoom Then
        ...
        strOldTechnicRoomName = Nvl(ucPacsQueue.QueueOper.GetQueueInf(lngQueueId, "诊室")!诊室)
...
On Error GoTo ErrHandle
    ...
    If objCfgWindow.ShowQueueConfig(ucPacsQueue, mlngModule, mstrPrivs, Me) Then
        '重新读取和应用配置
