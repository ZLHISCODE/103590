...
Public Sub UpdateImageVideoState(ByVal lngEventType As TVideoEventType, ByVal lngAdviceId As Long, _
    ...
    If mblnShowImage Then
        If lngEventType = TVideoEventType.vetAfterUpdateImg Or ((lngEventType = TVideoEventType.vetUpdateImg Or lngEventType = TVideoEventType.vetCaptureFirstImg Or lngEventType = TVideoEventType.vetDelAllImg) And lngAdviceId = mlngAdviceId) Then
...
On Error GoTo ErrHandle
    ...
    If mblnShowImage = True And (Not mfrmReportImage Is Nothing) Then
        mfrmReportImage.zlRefresh mlngAdviceId, mFileID, mReportID, mblnSingleWindow, mlngShowBigImg, mdblBigImgZoom, mintImageDblClick, mblnEditable, mblnMoved, mintMinImageCount, GetReportImageSelected, mlngModule, mlngDeptId, mlngStudyState
    End If
    '初始化专科报告窗口
        ...
        Case conMenu_File_Print, conMenu_File_Preview, conMenu_File_BatPrint       '打印报告,预览报告,批量打印
            ...
            Else    '可以打印
                ...
                If control.ID = conMenu_File_Print And mlngHintType = 2 Then 'mlngHintType = 2表示打印前提醒
                    ...
                    If IsNull(rsTemp!影像质量) And mstrImageLevel <> "" And mintImageLevel <> 0 And InStr(mstrPrivs, "影像质控") > 0 Then strResultInput = strResultInput & "影像质量|"
                    If IsNull(rsTemp!报告质量) And mstrReportLevel <> "" And mintReportLevel <> 0 And InStr(mstrPrivs, "报告质控") > 0 Then strResultInput = strResultInput & "报告质量|"
...
Private Sub SaveReport(blnRaiseEvent As Boolean, Optional blnIncVer As Boolean = True)
'------------------------------------------------
    ...
    If mblnShowImage = True Then
       If mfrmReportImage.ImageCount > 0 Then
            ...
            If mfrmReportImage.pImageModified = False And blnCreate = True Then
                mfrmReportImage.zlRefresh mlngAdviceId, mFileID, mReportID, mblnSingleWindow, mlngShowBigImg, mdblBigImgZoom, mintImageDblClick, mblnEditable, mblnMoved, mintMinImageCount, True, mlngModule, mlngDeptId, mlngStudyState
            End If
...
'            End If
        ...
        Case conMenu_PacsReport_ClearWritingState       '清除报告“处理中”的状态,可以清除本科室的报告标记
            control.Visible = InStr(mstrPrivs, "PACS报告删除") > 0
    ...
    If mblnUseActiveCapture Then
        If Not mobjWork_ActiveVideo Is Nothing Then
            If mblnSingleWindow Then
                Call mobjWork_ActiveVideo.zlUpdateStudyInf(mlngAdviceId, mlngSendNo, mlngStudyState, mblnMoved)
                Call mobjWork_ActiveVideo.zlRefreshData
            End If
            
            If mobjWork_ActiveVideo.HasVideo Then Exit Sub
...
'初始化ActivexExe视频采集模块对象
    If mobjWork_ActiveVideo Is Nothing Then
        ...
        Call mobjWork_ActiveVideo.zlInitModule(gcnOracle, glngSys, mlngModule, mstrPrivs, mlngDeptId, Me.hWnd, mobjOwner, True)
    End If
End Sub
...
Private Sub subChangeRptFormat()
'更改被选中的自定义报表打印格式
