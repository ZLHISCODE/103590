...
On Error GoTo ErrHandle
    ...
    If mblnSelectDefRoom Then
        '分配检查的默认执行间
        Call SeekIndexWithNo(cboRoom, GetDefaultRoom(mlngAdviceId, mlngCurDeptId))
    End If
...
'    End If
    ...
    If mintEditMode = 2 Or (mblnRegToCheck And mintEditMode = 0) Then
        ...
        arrSQL(UBound(arrSQL)) = "ZL_影像检查_BEGIN('" & mstrTechnicRoom & "'," & txt检查号.Text & "," & mlngAdviceId & "," & mlngSendNo & ",'" & mstrItemType & "','" & _
            ...
            ToVarchar(Txt体重, 16) & "',Null,Null,'" & NeedNo(cboDevice.Text) & "','" & NeedName(cbo技师一.Text) & "','" & NeedName(cbo技师二.Text) & "','" & txt附加主述.Text & "'," & To_Date(CDate(dtp(1).value)) & "," & mlngCurDeptId & ")"
    ...
    If mintEditMode = 3 Then
        ...
        arrSQL(UBound(arrSQL)) = "ZL_影像检查记录_UPDATE(" & mlngAdviceId & ", " & mlngSendNo & "," & txt检查号.Text & ",'" & _
            Trim(Me.PatiIdentify.Text) & "','" & Trim(Txt英文名.Text) & "','" & NeedName(cbo性别.Text) & "','" & _
            Val(txt年龄.Text) & IIf(cboAge.Visible, cboAge.Text, "") & "'," & To_Date(dtp出生日期.value) & ",'" & ToVarchar(Txt身高, 16) & "','" & _
            ToVarchar(Txt体重, 16) & "','" & NeedNo(cboDevice.Text) & "','" & NeedName(cbo技师一.Text) & "','" & NeedName(cbo技师二.Text) & "','" & txt附加主述.Text & "','" & NeedNo(cboRoom.Text) & "'," & To_Date(dtp(1).value) & ")"
    ...
    '如果修改了检查信息且使用了新版pacs，则需要对病人信息同步
    If mintEditMode = 3 And gblnUseXinWangView And mlngModul = G_LNG_PACSSTATION_MODULE Then
        Call XWStudyUpdate(mlngAdviceId, Me.PatiIdentify.Text, NeedName(cbo性别.Text), Val(txt年龄.Text))
    End If
    ...
    If mintEditMode = 2 Or (mblnRegToCheck And mintEditMode = 0) Then
        ...
        Call zlDatabase.OpenSQLRecord(gstrSQL, "回填当前检查号", CLng(123), mlngCurDeptId, mstrItemType, txt检查号.Text)
...
'    End If
    ...
    If mintEditMode >= 2 Or mblnRegToCheck Then '报到,或报到后修改　或　登记后直接检查 (登记时或登记后修改不判断)
        ...
        If mlngAutoIncNum <> 0 Then
            If txt检查号.Text > Val(mstrNextCheckNo) + 10 Then
                If MsgBoxD(Me, "检查号过大，比当前的最大号码大了" & (txt检查号.Text - Val(mstrNextCheckNo)) & "，请确认是否继续" & IIf(mintEditMode = 3, "修改", "报到") & "？", vbOKCancel + vbDefaultButton2, gstrSysName) = vbCancel Then
