...
On Error GoTo ErrorHand
    ...
    ''显示大图''''''''''''''''''''''''''''''''''''''''''''''
    intShowBigImg = Val(zlCL_GetPara("鼠标移动时显示大图", glngSys, glngModule, "0"))
    ...
    Else
        ...
        cboZoom.Text = Val(zlCL_GetPara("采集大图放大倍数", glngSys, glngModule, "1"))
        ...
        chkZoomControl.value = Val(zlCL_GetPara("大图显示范围限制", glngSys, glngModule, "0"))
...
'    Call zlCL_SetPara("采集热键", cbxHotKey.Text, glngSys, glngModule)
    ...
    If optDriver(3).value Then Call zlCL_SetPara("视频驱动类型", 3, glngSys, glngModule)
    ...
    If chkShowBigImg.value = 0 Then
        Call zlCL_SetPara("鼠标移动时显示大图", "0", glngSys, glngModule)
    Else
        If optBigImgAction(0).value Then
            Call zlCL_SetPara("鼠标移动时显示大图", "1", glngSys, glngModule)
        Else
            Call zlCL_SetPara("鼠标移动时显示大图", "2", glngSys, glngModule)
        End If
    End If
    Call zlCL_SetPara("采集大图放大倍数", IIf(Val(cboZoom.Text) = 0, 1, Val(cboZoom.Text)), glngSys, glngModule)
    Call zlCL_SetPara("大图显示范围限制", chkZoomControl.value, glngSys, glngModule)
    Call zlCL_SetPara("大图显示最大分辨率", Val(txtImgWidth.Text) & "*" & Val(txtImgHeight.Text), glngSys, glngModule)
    ''''''''''''''''''''''''''''''''''''''''''''''''''
