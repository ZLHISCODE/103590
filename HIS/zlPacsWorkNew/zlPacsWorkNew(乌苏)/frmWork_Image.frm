...
'打开满足条件的最近一次图像，如果没有满足条件的最近一次图像，显示检查列表给用户选择
    ...
    Else
        ...
        strSQL = "select A.医嘱ID as ID,首次时间 as 检查时间,医嘱内容, 影像类别 from 病人医嘱发送 A, 影像检查记录 B ,病人医嘱记录 C" & _
                 " where A.医嘱ID = B.医嘱ID and C.ID=A.医嘱ID and B.检查UID is not null and a.医嘱ID in (" & strOrderIDs & ")"
...
Private Sub Form_Load()
    ...
    '判断当前用户是否具有 观片站的基本权限
    If CheckPopedom(GetPrivFunc(glngSys, 1289), "基本") Then
        mblnObserve = True
    Else
        mblnObserve = False
    End If
...
Private Sub lvwSeq_ItemClick(ByVal Item As MSComctlLib.ListItem)
    ...
    If mintImageLocation <> 1 Then
        ...
        Call ShowImageList(Item)
    Else
        mlngCurImageCount = Item.SubItems(4)
    End If
End Sub
