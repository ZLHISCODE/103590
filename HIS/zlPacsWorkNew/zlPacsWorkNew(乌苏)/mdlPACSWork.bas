...
Public gblnIsStudyChage As Boolean      '切换化后值为true,具体见问题81704
...
'------------------------------------------------
    ...
    If intImageLocation = 1 Then
        ...
        Call XWShowImage(lngViewerType, strXWViewerFilter, IIf(strSerials <> "", glngSeriesSchemeNo, glngStudySchemeNo))
    ...
    If rsTmp.RecordCount > 0 Then
        ...
        If CBool(GetSetting("ZLSOFT", "公共模块", "UseActiveLoad", "True")) Then
            ...
            Call funPreDownLoadImagesEx(objParent, oneMessage)
        Else
...
'参数： lng医嘱ID --医嘱ID
'       lng来源 -- 病人来源
'返回值： 0--未收费；1--已收费；2--无收费记录，3-记账记录
...
'一条医嘱会有多部位的子医嘱
    ...
    gstrSQL = "Select B.id as 医嘱ID,B.相关ID, A.计费状态, A.记录性质,D.结算模式" & vbNewLine & _
    ...
    If Nvl(rsData!记录性质, 2) = 2 Then
        ...
        If Val(Nvl(rsData!结算模式)) = 1 Then
            lngTempCharged = 3         '门诊记账病人显示为“记”
        Else
            '住院登记的病人，如果没有计费，则归为无费用
            If Nvl(rsData!计费状态, -1) = 0 Then
    
                    rsData.Filter = "相关ID = " & Nvl(rsData!医嘱ID)
                    Do While rsData.EOF = False
                        If Nvl(rsData!计费状态, -1) = 1 Then
                            '如果是记账医嘱，凡是已计费和全部收费的，表示为已收费
                            lngTempCharged = 3      '已记账
                            
                        ElseIf Nvl(rsData!计费状态, -1) = 3 Then
                            lngTempCharged = 1      '已收费
                     
                        ElseIf Nvl(rsData!计费状态, -1) = 2 Then
...
Public Function ItemHaveCash(ByVal int病人来源 As Integer, ByVal bln单独执行 As Boolean, ByVal lng医嘱ID As Long, ByVal lng相关ID As Long, _
    ...
    Optional ByVal blnMove As Boolean, Optional ByVal dat发送时间 As Date, Optional ByRef str医嘱IDs As String, Optional ByRef strNOs As String) As Boolean
...
'      返回：str医嘱IDs=该医嘱及相关的医嘱ID,NOs=医嘱发送的单据号和补的附费中的单据号
    ...
    If int病人来源 = 2 And int记录性质 = 2 And int门诊记帐 = 0 Then
        ...
        strFilter = " A.记录性质"
        strFilter1 = " B.记录性质"
        strFilter2 = " A.执行状态 as 原执行状态"
        strFilter3 = " B.执行状态 as 原执行状态"
    Else
        strTab = "门诊费用记录"
        strFilter = " decode(A.记录性质,1,1,11,1,A.记录性质)"
        strFilter1 = " decode(B.记录性质,1,1,11,1,B.记录性质)"
        strFilter2 = " Nvl(A.费用状态,0) as 原执行状态"
        strFilter3 = " Nvl(B.费用状态,0) as 原执行状态"
    End If
    ...
    strSql = _
        " Select A.记录状态,Nvl(B.相关ID,B.ID) as 医嘱ID,B.诊疗类别," & strFilter2 & ",A.NO" & _
        " From " & strTab & " A,病人医嘱记录 B" & _
        " Where A.NO=[4] And A.记录状态 IN(0,1,3) And A.医嘱序号+0=B.ID And " & strFilter & "=[5] " & IIf(bln单独执行, " And B.ID=[2]", "") & _
        " Union ALL " & _
        " Select B.记录状态,Nvl(C.相关ID,C.ID) as 医嘱ID,C.诊疗类别," & strFilter3 & ",A.NO" & _
        " From 病人医嘱记录 C," & strTab & " B,病人医嘱附费 A" & _
        " Where A.NO=B.NO And A.记录性质=" & strFilter1 & " And A.医嘱ID=B.医嘱序号+0" & IIf(bln单独执行, " And A.医嘱ID=[2]", _
    ...
    If Not rsTmp.EOF Then
        If int方式 = 0 Then
            rsTmp.Filter = "医嘱ID=" & IIf(lng相关ID <> 0, lng相关ID, lng医嘱ID) & " And 诊疗类别='" & str类别 & "' And 原执行状态=1"
            If Not rsTmp.EOF Then
                ItemHaveCash = False
            Else
        ...
        ElseIf int方式 = 1 Then
            rsTmp.Filter = "医嘱ID=" & IIf(lng相关ID <> 0, lng相关ID, lng医嘱ID) & " And 诊疗类别='" & str类别 & "' And 记录状态<>0 And 原执行状态<>1"
...
'返回值：1--已登记；2--已报到；3--已检查；4--已报告；5--已审核；6--已完成
    ...
    If intFinishReport = rsTemp.RecordCount Then blnAllReportFinished = True
        
    If blnAllReportFinished Then
        getStudyStateRich = 5
    Else
        getStudyStateRich = 4
    End If
