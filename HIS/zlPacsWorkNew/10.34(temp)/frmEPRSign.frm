...
'################################################################################################################
Public Function ShowMe(ByRef fParent As Object, ByVal lngReportID As Long, ByVal lngPatientID As Long, ByVal lngPageID As Long, _
...
Private Function Validation() As Boolean
    ...
    If chkEsign.value = vbChecked Then
        ...
        If objESign.CheckCertificate(strDBUser) = False Then
            MsgBoxD Me, "数字证书和当前用户不一致，请使用正确的数字证书签名。", vbInformation + vbOKOnly, "书写签名"
            GoTo LL
        End If
        ...
        strSign = objESign.signature(strSource, strDBUser, lngCertID, str时间戳)
        If strSign = "" Then
            MsgBoxD Me, "验证失败！请重新输入验证信息！", vbInformation + vbOKOnly, "书写签名"
            GoTo LL
        End If
