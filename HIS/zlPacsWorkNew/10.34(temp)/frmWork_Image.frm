...
'创建工具栏
    ...
    Set cbrLogOut = objToolBar.FindControl(, conMenu_Manage_Logout)
    
    lngIndex = 4
    If Not cbrLogOut Is Nothing Then lngIndex = cbrLogOut.Index
    
    Set cbrControl = CreateModuleMenu(objToolBar.Controls, xtpControlButton, conMenu_Img_Look, "观片", "影像观片", 0, True, lngIndex + 1)
...
'更新菜单
    Select Case control.ID
        ...
        Case conMenu_Img_Query ',获取图像
            If InStr(mstrPrivs, "清除图像") <= 0 Or mintImageLocation = 1 Then
                control.Visible = False
            Else
                control.Visible = True
            End If
        ...
        Case conMenu_Manage_TechDoctorExecute   '技师执行
            ...
            If mlngStudyState >= 2 And mlngStudyState < 5 Then
                ...
                If mblnExamineDoctorVerify Then
                    control.Enabled = UserInfo.姓名 = mstrExamineDoctorName Or InStr(mstrPrivs, "取消技师执行") > 0
                End If
            Else
                control.Enabled = False
            End If
        ...
        If objMenuControl.ID = conMenu_Img_Group And objMenuControl.type = xtpControlPopup Then
            For Each objControl In objMenuControl.CommandBar.Controls
                If objControl.ID = conMenu_Img_Look Or _
                   ...
                   objControl.ID = conMenu_Manage_ChangeDevice Then
...
Public Sub zlUpdateOtherInf(objStudyList As Object)
    Set mobjStudyList = objStudyList
    
    '技师是否确认
    If objStudyList.CurText("是否技师确认") = 1 Then
...
On Error GoTo errHandle
    ...
    If mblnExamineDoctorVerify Then     '技师取消
        ...
        mblnExamineDoctorVerify = False
        mobjStudyList.CurText("是否技师确认") = 0
    Else
        If mobjStudyList.CurText("检查技师") <> "" Then
            If Not MsgBoxD(Me, "当前人员与指定的检查技师不相同," & vbCrLf & "确定要继续执行吗？", vbYesNo, "技师执行") = vbNo Then
                ...
                mobjStudyList.CurText("是否技师确认") = 1
            End If
        Else
            strSql = "Zl_影像技师执行('" & UserInfo.姓名 & "'," & mlngAdviceID & ")"
            Call zlDatabase.ExecuteProcedure(strSql, "技师执行")
...
'显示观片站
    ...
    Else
        ...
        If gblnUseXinWangPacs = True Then
        
            Set rsTemp = GetStudyImageData(mlngAdviceID)
...
Private Function GetStudyImageData(ByVal lngAdviceID As Long) As ADODB.Recordset
'获取检查图像数据
...
Private Sub Form_Load()
    ...
    gblnUseXinWangPacs = IIf(InStr(GetPrivFunc(glngSys, G_LNG_XWPACSVIEW_MODULE), "基本") > 0, True, False)
    
   '如果是RIS工作站，则连接新网数据库，读取参数
    If gblnUseXinWangPacs Then
        Call XWDBServerOpen
...
'-----------------------------------------------------------------------------------------
    ...
    strSql = "Select /*+RULE*/ A.序列UID,A.序列号,A.序列描述,A.采集时间,B.影像类别,B.检查号," & _
...
Dim cbrToolPopup As CommandBarPopup
    ...
    If InStr(mstrPrivs, "清除图像") <= 0 Then Exit Sub
