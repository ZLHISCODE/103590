...
Dim blnExist As Boolean, i As Integer
    ...
    If Me.lblName.Tag = "" And Not blnExist Then
        gstrSQL = "zl_医技执行房间_Insert(" & Val(mlngdept) & ",'" & Trim(Me.txtName.Text) & "','" & NeedNo(cboDevice.Text) & "')"
    Else
        gstrSQL = "zl_医技执行房间_Update(" & Val(mlngdept) & ",'" & Trim(Me.lblName.Tag) & "','" & Trim(Me.txtName.Text) & "','" & NeedNo(cboDevice.Text) & "')"
    End If
