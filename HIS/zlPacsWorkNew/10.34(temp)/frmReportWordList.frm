...
'       strText --- 要保存的词句示范内容
    ...
    Dim lngPointer As Long      '当前文本内容指针的位置
    Dim lngEnd As Long          '文本的总长度
    Dim lngSegment As Long      '文本中回车换行的位置
    ...
    lngPointer = 1
    lng序号 = 1
    Do While lngPointer < lngEnd
        '获取vbCrlf位置
        lngSegment = InStr(lngPointer, strText, vbCrLf, vbTextCompare)
        If lngSegment = 0 Then
            lngSegment = lngEnd
        End If
        
        '保存文本
        strLine = Mid(strText, lngPointer, IIf(lngSegment >= lngEnd, lngSegment - lngPointer + 1, lngSegment - lngPointer + 2))
        Call GetPlainTextSaveSQL(ArrySQL, strLine, lng序号)
        
        lngPointer = lngSegment + 2
    Loop
