VERSION 5.00
Object = "*\A..\..\..\ZLHISEDITION\U10.32.0\ZLIDKind\zlIDKind.vbp"
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.PictureBox Picture4 
         ...
         Begin VB.ComboBox cbo造影剂 
            ...
            EndProperty
            Height          =   360
            ItemData        =   "frmtestEnreg.frx":0000
            Left            =   5280
            List            =   "frmtestEnreg.frx":0002
      ...
      Begin VB.PictureBox Picture3 
         ...
         Begin VB.ComboBox cbo性别 
            ...
            Height          =   360
            IMEMode         =   3  'DISABLE
            ItemData        =   "frmtestEnreg.frx":0004
            Left            =   1470
            List            =   "frmtestEnreg.frx":0011
            Style           =   2  'Dropdown List
            TabIndex        =   58
            Top             =   2010
            Width           =   2280
         ...
         Begin VB.ComboBox cboAge 
            ...
            EndProperty
            Height          =   360
            ItemData        =   "frmtestEnreg.frx":0023
            Left            =   2865
            List            =   "frmtestEnreg.frx":0030
            Style           =   2  'Dropdown List
            TabIndex        =   52
            Top             =   1590
            Width           =   915
      ...
      Begin VB.PictureBox Picture2 
         ...
         Begin VB.ComboBox cbo开单科室 
            ...
            EndProperty
            Height          =   360
            ItemData        =   "frmtestEnreg.frx":0040
            Left            =   1440
            List            =   "frmtestEnreg.frx":0042
            Style           =   2  'Dropdown List
            TabIndex        =   26
            Top             =   45
            Width           =   2325
         ...
         Begin VB.ComboBox cbo执行科室 
            ...
            ForeColor       =   &H000000FF&
            Height          =   360
            ItemData        =   "frmtestEnreg.frx":0044
            Left            =   5100
            List            =   "frmtestEnreg.frx":0046
            Style           =   2  'Dropdown List
            TabIndex        =   16
            Top             =   1785
            Width           =   2295
...
'    End If
    ...
    If mintEditMode = 2 Or (mblnRegToCheck And mintEditMode = 0) Then
        ...
        '开始检查
        ReDim Preserve arrSQL(UBound(arrSQL) + 1)
        arrSQL(UBound(arrSQL)) = "ZL_影像检查_BEGIN('" & NeedNo(cboRoom.Text) & "'," & Val(txt检查号.Text) & "," & mlngAdviceID & "," & mlngSendNo & ",'" & mstrItemType & "','" & _
...
'返回：mstrExtData "部位名1;方法名1,方法名2|部位名2;方法名1,方法名2|...<vbTab>0-常规/1-床旁/2-术中"
    ...
    blnOk = frmAdviceEditEx.ShowMe(Me, Nothing, lngHwnd, t_Pati, 0, 0, 0, 1, int服务对象, , , , rsInput!诊疗项目ID, strExtData, strAppend)
...
'mintEditMode '0－登记、1－登记后修改、2－报到、3－报到后修改
    ...
    cbo性别.Enabled = blnEditableState: cboAge.Enabled = blnEditableState
    dtp出生日期.Enabled = blnEditableState: cbo费别.Enabled = blnEditableState
    ...
    Call sutSetTxtEnable(txt姓名, blnEditableState)
    Call sutSetTxtEnable(txt年龄, blnEditableState)
    Call sutSetTxtEnable(Txt身份证号, blnEditableState)
    ...
    Select Case mintEditMode
        Case 0          '0－登记
            ...
            Call sutSetTxtEnable(Txt英文名, mblnRegToCheck)
        ...
        Case 3          '3－报到后修改
            ...
            cboRoom.Enabled = True: cbo技师一.Enabled = True: cbo技师二.Enabled = True
...
'功能：读取病人信息，并显示该病人存在的医嘱时间
    ...
    If mInputType = 1 Then '病人ID
        gstrSQL = "Select distinct A.病人id,A.姓名,A.性别,A.年龄,to_char(A.出生日期,'yyyy-mm-dd') 出生日期,Decode(A.当前科室id,Null,3,2) As 来源ID,Nvl(A.住院次数,0) As 主页ID," & _
                  ...
                  " From 病人信息 A Where A.病人ID=[2] and '%'='%'" '为免避一行也弹出窗口所以用%,%在ShowSQLSelect在限制
    ...
    ElseIf mInputType = 4 Then '挂号单
        ...
        txt姓名.Text = strNO
        mstrRegNo = strNO
        gstrSQL = "Select Distinct A.病人id, A.姓名, A.性别, A.年龄, To_Char(A.出生日期, 'yyyy-mm-dd') 出生日期, Decode(Nvl(A.在院, 0), 0, 3, 2) As 来源id," & vbNewLine & _
...
Private Sub InitEdit(ByVal blnIsChangeDept As Boolean)
    ...
    '执行间
    If mlngModul = 1291 And mlngQueueWay = 1 Then
        '如果是影像采集工作站，且排队方式使用按科室排队时，则不需要录入执行间
        cboRoom.Clear
        
        cboRoom.BackColor = &H8000000F
...
'------------------------------------------------
    ...
    If Not rsTemp.EOF Then
        If Nvl(rsTemp!参数值) <> "" Then
            ...
            ElseIf InStr(rsTemp!参数值, "执行间") > 0 And Trim(cboRoom.Text) = "" And cboRoom.Enabled = True And mlngQueueWay <> 1 Then
    ...
    If mintEditMode = 2 Or mintEditMode = 3 Or (mblnRegToCheck And mintEditMode = 0) Then
        If cboRoom.Text = "" And mlngQueueWay <> 1 Then
