...
'------------------------------------------------
    ...
    If intImageLocation = 1 Then
        ...
        Call XWShowImage(lngViewerType, strXWViewerFilter, IIf(strSerials <> "", 2, 1))
...
'一条医嘱会有多部位的子医嘱
    ...
    gstrSQL = "Select A.医嘱id, B.记录状态,B.执行状态" & vbNewLine & _
    ...
    '执行状态：记录状态=1,3时：0:未执行;1:完全执行;2:部份执行；记录状态=2时：-x:第x次退费;记录状态=0时:-1表示暂停收费(在提取划价单时,默认不勾选);9-表示交费过程中，属于无效单据。
    ...
    '有费用记录，则应该是已缴费和未交费两种情况
    '对于已缴费的判断，有执行状态=9的记录，表示有未缴费记录；有记录状态=0记录，表示有未缴费记录。
    rsTemp.Filter = "执行状态=9"
    If rsTemp.BOF = False Then
        CheckChargeState = 0
        Exit Function
    End If
    
    rsTemp.Filter = "记录状态 =0 or 记录状态 =2 or 记录状态 =3 "
    If rsTemp.BOF = False Then
        CheckChargeState = 0
        Exit Function
    End If
