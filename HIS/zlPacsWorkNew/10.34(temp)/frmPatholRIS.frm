...
Begin VB.Form frmPatholRIS 
   ...
   Begin VB.Frame Frame2 
      ...
      Begin zlIDKind.IDKind IDKind 
         ...
         _ExtentX        =   1138
         _ExtentY        =   635
         IDKindStr       =   "姓|姓名|0;医|医保号|0;身|身份证号|0;IC|IC卡号|1;门|门诊号|0;住|住院号|0;门|门诊号|0;挂|挂号单|0;收|收费单据号|0"
...
Private Sub cmdOK_Click()
    ...
    If mintEditMode = 2 Or (mblnRegToCheck And mintEditMode = 0) Then
        ...
        arrSQL(UBound(arrSQL)) = "ZL_影像检查_BEGIN(Null,Null," & mlngAdviceID & "," & mlngSendNo & ",'DG','" & _
            ...
            ToVarchar(Txt体重, 5) & "',Null,Null,Null,Null,Null,'" & txt附加主述.Text & "',Null," & mlngCurDeptId & ")"
    ...
    If mintEditMode = 3 Then
        ...
        arrSQL(UBound(arrSQL)) = "ZL_影像检查记录_UPDATE(" & mlngAdviceID & ", " & mlngSendNo & ",Null,'" & _
            ...
            Val(txt年龄.Text) & IIf(cboAge.Visible, cboAge.Text, "") & "'," & To_Date(dtp出生日期.value) & ",'" & ToVarchar(Txt身高, 5) & "','" & _
            ToVarchar(Txt体重, 5) & "',Null,Null,Null,'" & txt附加主述.Text & "',Null," & To_Date(dtp(1).value) & ")"
...
'mInputType  0-就诊卡 1-病人ID 2-住院号 3-门诊号 4-挂号单 5-收费单据号 6-姓名 7-医保号 8-身份证号 9-IC卡号
    ...
    ElseIf mInputType = 1 Then '病人ID
        gstrSQL = "Select distinct A.病人id,A.姓名,A.性别,A.年龄,to_char(A.出生日期,'yyyy-mm-dd') 出生日期,Decode(A.当前科室id,Null,3,2) As 来源ID,Nvl(A.住院次数,0) As 主页ID," & _
                        ...
                        "A.费别,A.医疗付款方式,A.身份证号,A.民族,A.职业,A.婚姻状况,nvl(A.家庭电话,A.联系人电话) 电话," & _
                        "nvl(A.家庭地址邮编,A.单位邮编) 邮编,nvl(A.家庭地址,A.工作单位) 地址,A.合同单位ID, 0 as 新病人" & _
                  " From 病人信息 A,病人挂号记录 B Where A.病人ID=[2] And A.病人ID=B.病人ID(+) And A.门诊号=B.门诊号(+) and '%'='%'" '为免避一行也弹出窗口所以用%,%在ShowSQLSelect在限制
    ...
    ElseIf mInputType = 5 Then '收费单据号
        ...
        gstrSQL = "Select Distinct Nvl(A.病人id, 0) 病人id, Nvl(A.姓名, B.姓名) 姓名, Nvl(A.性别, B.性别) 性别, Nvl(A.年龄, B.年龄) 年龄," & vbNewLine & _
                    "                To_Char(A.出生日期, 'yyyy-mm-dd') 出生日期, Decode(Nvl(A.在院, 0), 0, 3, 2) As 来源id, Nvl(A.住院次数, 0) As 主页id," & vbNewLine & _
...
'    mstrItemType = Nvl(rsTemp!影像类别)
    ...
    If bln报到 And InStr(mstrPrivs, "未缴费报到") = 0 And mintSourceType <> 3 Then '24361 有权限不判断，自行登记不控制，急诊也进行判断
        If lblCash.Caption = "已收" Or lblCash.Caption = "无" Then
            CmdOK.Enabled = True
        Else
            CmdOK.Enabled = False
        End If
