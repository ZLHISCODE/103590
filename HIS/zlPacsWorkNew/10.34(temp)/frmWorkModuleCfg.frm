...
Public Sub LoadDefaultModuleConfig()
    ...
    Select Case mlngModule
        Case G_LNG_PACSSTATION_MODULE
            strModuleName = "影像图像模块;影像报告模块;病历记录模块;费用记录模块;医嘱记录模块;"
            
        Case G_LNG_VIDEOSTATION_MODULE
            strModuleName = "影像采集模块;影像报告模块;病历记录模块;费用记录模块;医嘱记录模块;"
            
        Case G_LNG_PATHOLSYS_NUM
            strModuleName = "影像采集模块;标本核收模块;病理取材模块;病理制片模块;病理特检模块;过程报告模块;病理诊断模块;病历记录模块;费用记录模块;医嘱记录模块;"
        Case Else
            Exit Sub
    End Select
