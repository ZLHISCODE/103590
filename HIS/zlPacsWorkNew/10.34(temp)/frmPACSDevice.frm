...
Private Sub cbrMain_Update(ByVal control As XtremeCommandBars.ICommandBarControl)
    Select Case control.ID
        Case conMenu_Edit_NewItem '新增 没权限和增删改、配置任一功能开始后其它按键都不可用
            If InStr(mstrPrivs, "增删改") <= 0 Or blnBeginchange Then control.Enabled = False: Exit Sub
        ...
        Case conMenu_File_Excel '导出EXCEL
            If InStr(mstrPrivs, "增删改") <= 0 Then control.Enabled = False
    End Select
End Sub
...
Dim DevType As String
    ...
    gstrSQL = "zl_影像设备目录_Update('" & txtDevNO & "','" & Trim(txtName) & "'," & cboType.ListIndex + 1 & _
        ...
        Trim(txtPassWord) & "','" & Trim(txtShareDir) & "','" & Trim(txtSDUser) & "','" & Trim(txtSDPassword) & "','" & Trim(txtDevLocalAE) & "','" & Trim(txtDevAE) & "','" & Trim(txtDirPath) & "', 1,'" & DevType & "')"
    zlDatabase.ExecuteProcedure gstrSQL, Me.Caption
    DevSave = True
    Exit Function
DBError:
...
Dim FtpNet As New clsFtp, strPath As String, strTmpPath As String           'FTP类
    ...
    If FtpNet.FuncFtpConnect(txtDevAdress, txtUser, txtPassWord) > 0 Then
        ...
        Else
            If FtpNet.FuncUploadFile(txtFtpPath, strTmpPath, "temp.txt") > 0 Then
