...
On Error GoTo errHandle
    ...
    For i = 1 To 20
        strCurPar = strPars(i)
        strValues(i) = GetParameterValue(strCurPar)
        
        If strCurPar <> "" Then
            If Trim(strValues(i)) = "" Then    '如果录入的数据为空，则不使用该条件
                strField = GetParameterField(strCurPar, strSql)
                If strField <> "" Then
'                    strSql = Replace(strSql, strCurPar, strField)
...
On Error GoTo errHandle
    ...
    If Not cbxObj(Index).Visible Then Exit Sub
    
    strCurControlName = cbxObj(Index).Tag
...
On Error GoTo errHandle
    ...
    If Not dtpObj(Index).Visible Then Exit Sub
    
    strCurControlName = dtpObj(Index).Tag
...
On Error GoTo errHandle
    ...
    If Not lstObj(Index).Visible Then Exit Sub
    
    strCurControlName = lstObj(Index).Tag
...
On Error GoTo errHandle
    ...
    If Not txtObj(Index).Visible Then Exit Sub
    
    strCurControlName = txtObj(Index).Tag
...
On Error GoTo errHandle
    ...
    strSql = "select 查询语句,方案说明 from 影像查询方案 where id=[1]"
    Set rsData = zlDatabase.OpenSQLRecord(strSql, "查询方案信息", mlngSchemeId)
