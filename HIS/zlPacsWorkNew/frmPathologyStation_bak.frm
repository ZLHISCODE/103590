...
Begin VB.Form frmPathologyStation 
   ...
   Begin XtremeDockingPane.DockingPane dkpMain 
      Bindings        =   "frmPathologyStation.frx":52AC
      Left            =   0
      Top             =   0
      _Version        =   589884
      _ExtentX        =   450
...
Private Const ConstrCol = "路径;400|紧急;300|来源;400|收费;300|阳性;300|质量;300|姓名;1200|检查号;1400|检查过程;800|性别;450|年龄;450" & _
                        ...
                        "|绿色通道;0|报告打印;800|报告人;800|复核人;800|检查技师;800|采图时间;1800|随访描述;2400|病理号;800|核收情况;1200" & _
...
'子窗体对像
'Private WithEvents mfrmSample As frmSample              '病理标本核收和取材窗口
Private mfrmSample As frmPatholMaterials              '病理标本核收和取材窗口
Private mfrmSpecimen As frmPatholSpecimen
Private mfrmSlices As frmPatholSlices
Private mfrmReportDelay As frmPatholReportDelay
Private mfrmProReport As frmPatholProcedureRep
Private mfrmRequest As frmPatholRequisition
Private mfrmConsultation As frmPatholConsultation
Private mfrmSpeExam As frmPatholSpecialExamined
...
Private Sub Menu_Dept_Select(ByVal control As XtremeCommandBars.ICommandBarControl)
    If mlngCur科室ID <> control.DescriptionText Then
        ...
        Call frmVideoCapture.InitDeptPara(mlngCur科室ID)
...
Private Sub Command1_Click()
    frmPatholAntibody.Show 1
End Sub

Private Sub Command2_Click()
    frmPatholMeal.Show 1, Me
End Sub

Private Sub Command3_Click()
On Error GoTo errHandle
    Call frmPatholDecalcification.ShowDecalinTaskWind(Me)
Exit Sub
errHandle:
    If ErrCenter() = 1 Then Resume
End Sub
...
Private Sub Form_Load()
    ...
    '初始子窗体
    Set mfrmSample = New frmPatholMaterials           '标本核收和取材
    Set mfrmSpecimen = New frmPatholSpecimen
    Set mfrmSlices = New frmPatholSlices
    Set mfrmReportDelay = New frmPatholReportDelay
    Set mfrmProReport = New frmPatholProcedureRep
    Set mfrmRequest = New frmPatholRequisition
    Set mfrmConsultation = New frmPatholConsultation
    Set mfrmSpeExam = New frmPatholSpecialExamined
    ...
    FormSetCaption mfrmSample, False, False
    FormSetCaption mfrmSpecimen, False, False
    FormSetCaption mfrmSlices, False, False
    FormSetCaption mfrmReportDelay, False, False
    FormSetCaption mfrmProReport, False, False
    FormSetCaption mfrmRequest, False, False
    FormSetCaption mfrmConsultation, False, False
    FormSetCaption mfrmSpeExam, False, False
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set mfrmSample = Nothing
    Set mfrmSpecimen = Nothing
    Set mfrmSlices = Nothing
    Set mfrmReportDelay = Nothing
    Set mfrmProReport = Nothing
    Set mfrmRequest = Nothing
    Set mfrmConsultation = Nothing
    Set mfrmSpeExam = Nothing
    Set mobjSysHook = Nothing
...
Dim i As Integer
    With TabWindow
        ...
        If GetInsidePrivs(p诊疗报告管理, True) <> "" Then
            If mblnPacsReport = True Then
                .InsertItem 1, "影像报告", mfrmPacsReport.hWnd, 10008 'conMenu_Edit_Compend
            Else
                .InsertItem 1, "影像报告", mobjReport.zlGetForm.hWnd, 10008 'conMenu_Edit_Compend
            End If
            .Item(TabWindow.ItemCount - 1).Tag = "报告填写"
        End If
        
        If GetInsidePrivs(p医嘱附费管理, True) <> "" Then
            .InsertItem 2, "费用记录", mobjExpense.zlGetForm.hWnd, 10007  'conMenu_Manage_Request
            .Item(TabWindow.ItemCount - 1).Tag = "申请费用"
        End If
        
        If GetInsidePrivs(p住院医嘱下达, True) <> "" Then
            .InsertItem 3, "医嘱记录", mobjInAdvice.zlGetForm.hWnd, 10010 ' conMenu_Edit_NewItem
            .Item(TabWindow.ItemCount - 1).Tag = "住院医嘱"
        End If
        
        If GetInsidePrivs(p门诊医嘱下达, True) <> "" Then
            .InsertItem 4, "医嘱记录", mobjOutAdvice.zlGetForm.hWnd, 10010 ' conMenu_Edit_NewItem
            .Item(TabWindow.ItemCount - 1).Tag = "门诊医嘱": .Item(TabWindow.ItemCount - 1).Visible = False
        End If
        
        If GetInsidePrivs(p住院病历管理, True) <> "" Then
            .InsertItem 5, "病历记录", mobjInEPRs.zlGetForm.hWnd, 10009 ' conMenu_Edit_Archive
            .Item(TabWindow.ItemCount - 1).Tag = "住院病历"
        End If
        
        If GetInsidePrivs(p门诊病历管理, True) <> "" Then
            .InsertItem 6, "病历记录", mobjOutEPRs.zlGetForm.hWnd, 10009 ' conMenu_Edit_Archive
            .Item(TabWindow.ItemCount - 1).Tag = "门诊病历": .Item(TabWindow.ItemCount - 1).Visible = False
        End If
        
        .InsertItem 7, "标本核收", mfrmSpecimen.hWnd, 10006 'conMenu_Cap_Dynamic
        .Item(TabWindow.ItemCount - 1).Tag = "标本核收"
        
        .InsertItem 8, "标本取材", mfrmSample.hWnd, 10006 'conMenu_Cap_Dynamic
        .Item(TabWindow.ItemCount - 1).Tag = "标本取材"
        
        .InsertItem 9, "病理制片", mfrmSlices.hWnd, 10006
        .Item(TabWindow.ItemCount - 1).Tag = "病理制片"
        
        
        .InsertItem 10, "报告延迟", mfrmReportDelay.hWnd, 10006
        .Item(TabWindow.ItemCount - 1).Tag = "报告延迟"
        

        .InsertItem 11, "过程报告", mfrmProReport.hWnd, 10006
        .Item(TabWindow.ItemCount - 1).Tag = "过程报告"
        
        
        .InsertItem 12, "检查申请", mfrmRequest.hWnd, 10006
        .Item(TabWindow.ItemCount - 1).Tag = "检查申请"
        
        .InsertItem 13, "病理会诊", mfrmConsultation.hWnd, 10006
        .Item(TabWindow.ItemCount - 1).Tag = "病理会诊"
        
        .InsertItem 14, "特殊检查", mfrmSpeExam.hWnd, 1006
        ...
        If mblnUseQueue = True Then
            ...
            .InsertItem 15, "排队叫号", mobjQueue.zlGetForm.hWnd, 10011 'conMenu_File_PrintSingleBill
            .Item(TabWindow.ItemCount - 1).Tag = "排队叫号"
        End If
        
        If mstrFirstTab <> "" Then
            Set frmVideoCapture.ParentContainerObj = picVideoContainer
            
            For i = 0 To .ItemCount - 1
                If InStr(.Item(i).Tag, mstrFirstTab) > 0 And .Item(i).Visible Then
                    .Item(i).Selected = True
                    
                    If InStr("报告填写", mstrFirstTab) > 0 Then
                        If mblnPacsReport = True Then Call mfrmPacsReport.ShowVideoWindow
                    ElseIf InStr("影像采集", mstrFirstTab) > 0 Then
...
Dim C收费 As Long
    ...
    With lst
        ...
        .ColWidth(C报告发放) = GetCW("报告发放"):   .ColWidth(C病理号) = GetCW("病理号"):       .ColWidth(C核收情况) = GetCW("核收情况")
        .ColWidth(C诊断分类) = GetCW("诊断分类"):   .ColWidth(C关联ID) = GetCW("关联ID"):       .ColWidth(C病人科室) = GetCW("病人科室")
        .ColWidth(C就诊卡号) = GetCW("就诊卡号"):   .ColWidth(C单据号) = GetCW("单据号"):       .ColWidth(C身份证号) = GetCW("身份证号")
        ...
        .Cell(flexcpData, 0, C报告发放) = "报告发放":       .Cell(flexcpData, 0, C病理号) = "病理号":           .Cell(flexcpData, 0, C核收情况) = "核收情况"
        ...
        .TextMatrix(0, C报告发放) = "报告发放":     .TextMatrix(0, C病理号) = "病理号":         .TextMatrix(0, C核收情况) = "核收情况"
...
Dim blnUseTime As Boolean       '是否使用时间条件
    ...
    With SQLCondition
        ...
        gstrSQL = "Select /*+ RULE */ Distinct" & vbNewLine & _
                    ...
                    "       Nvl(H.姓名,C.姓名) 姓名,I.病理检查类别 影像类别,H.检查号,Nvl(H.性别,C.性别) 性别,Nvl(H.年龄,C.年龄) 年龄,H.身高,H.体重,H.影像质量," & vbNewLine & _
                    "       Decode(B.病人来源,3,B.开嘱医生,A.发送人) 登记人,H.报到人,I.病理号,I.核收情况,H.报告发放,H.关联ID, " & vbNewLine & _
                    ...
                    " From 病人医嘱发送 A,病人医嘱记录 B,病人信息 C,病人临床路径 D,影像检查记录 H,影像检查项目 G,影像标本核收取材 I,部门表 F " & vbNewLine & _
                    " Where B.相关ID is NULL And A.医嘱ID=B.ID And A.医嘱ID=H.医嘱ID(+) And A.发送号=H.发送号(+) " & vbNewLine & _
                    "       And A.医嘱ID=I.医嘱ID(+) And B.诊疗项目ID=G.诊疗项目ID And B.病人ID=C.病人ID And B.病人科室id=F.ID " & vbNewLine & _
        ...
        Set GetFilterData = zlDatabase.OpenSQLRecord(gstrSQL, "提取病人列表", .门诊号, .住院号, .就诊卡, .姓名, .身份证, _
                                            ...
                                            .标本部位, .诊断医生, .审核医生, .影像质量, .检查技师, .影像类别, .随访, _
...
Private Sub FillList(lst As VSFlexGrid, ByVal rsTemp As ADODB.Recordset)
    ...
    With lst
        ...
        Do Until rsTemp.EOF
            ...
            .TextMatrix(.Row, GetCN("病理号")) = Nvl(rsTemp!病理号)
            .TextMatrix(.Row, GetCN("影像类别")) = Nvl(rsTemp!影像类别)
            .TextMatrix(.Row, GetCN("核收情况")) = Decode(Nvl(rsTemp!核收情况, 1), 1, "收", 2, "拒", "")
    ...
    Select Case TabWindow(TabWindow.Selected.Index).Tag
        ...
        Case "标本核收"
            If intState = 6 Or intState = 0 Or intState = 1 Then '查看模式
                mfrmSpecimen.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1
            Else
                mfrmSpecimen.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1
            End If
        Case "标本取材"
            If intState = 6 Or intState = 0 Or intState = 1 Then '查看模式
                mfrmSample.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1
            Else
                mfrmSample.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1
            End If
        ...
        Case "病理制片"
            If intState = 6 Or intState = 0 Or intState = 1 Then '查看模式
                mfrmSlices.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1
            Else
                mfrmSlices.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1
            End If
        Case "报告延迟"
            If intState = 6 Or intState = 0 Or intState = 1 Then '查看模式
                mfrmReportDelay.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1
            Else
                mfrmReportDelay.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1
            End If
        Case "过程报告"
            If intState = 6 Or intState = 0 Or intState = 1 Then '查看模式
                mfrmProReport.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1
            Else
                mfrmProReport.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1
            End If
        Case "检查申请"
            If intState = 6 Or intState = 0 Or intState = 1 Then '查看模式
                mfrmRequest.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1, mlngCur科室ID
            Else
                mfrmRequest.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1, mlngCur科室ID
            End If
        Case "病理会诊"
            If intState = 6 Or intState = 0 Or intState = 1 Then '查看模式
                mfrmConsultation.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1, mlngCur科室ID
            Else
                mfrmConsultation.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1, mlngCur科室ID
            End If
        Case "特殊检查"
            If intState = 6 Or intState = 0 Or intState = 1 Then '查看模式
                mfrmSpeExam.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1, mlngCur科室ID
            Else
                mfrmSpeExam.zlRefresh lngAdviceID, mstrPrivs, intMoved = 1, mlngCur科室ID
            End If
