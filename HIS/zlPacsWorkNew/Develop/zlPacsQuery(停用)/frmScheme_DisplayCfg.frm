...
Private Const M_STR_COLNAME = "列名称|列图标|隐藏列|不显示标题|隐藏数据显示|排序对照|启用定位|数据转换|列统计|行关联显示配置"
...
Private Sub Form_Resize()
    ...
    vsfDisSet.Move lblTag.Left, fraLine.Top + 200, Me.ScaleWidth - cmdClear.Width - 300, Me.ScaleHeight - vsfDisSet.Top - 300
...
Private Sub vsfDisSet_CellButtonClick(ByVal Row As Long, ByVal Col As Long)
    ...
    If Col = ColTitle.ct行关联显示配置 Then
        Set objRelatedSet = New frmSetRelated
        ...
        If Not IsObject(vsfDisSet.RowData(Row)) Then
            Set ObjScShowCfg = mobjSetRelated.ShowScRowRelation(Nothing, vsfDisSet.TextMatrix(Row, ColTitle.ct数据转换), strPerformCol, mblnState, Me)
        Else
            Set ObjScShowCfg = vsfDisSet.RowData(Row)
            Set ObjScShowCfg = mobjSetRelated.ShowScRowRelation(ObjScShowCfg, vsfDisSet.TextMatrix(Row, ColTitle.ct数据转换), strPerformCol, mblnState, Me)
    ...
    If Col = ColTitle.ct数据转换 Then
        ...
        strValue = objCustomQueryForm.ShowSqlFromWindow(strValue, "", 3, mblnState, Me)
...
Private Sub vsfDisSet_Click()
    ...
    If mblnState Then
        ...
        If lngCol = ColTitle.ct隐藏列 Or lngCol = ColTitle.ct不显示标题 Or lngCol = ColTitle.ct隐藏数据显示 Or lngCol = ColTitle.ct启用定位 Or lngCol = ColTitle.ct列统计 Then
            mblnIsEdit = True
...
Private Sub vsfDisSet_RowColChange()
    ...
    If mblnState Then
        If lngCol = ColTitle.ct列名称 Or lngCol = ColTitle.ct隐藏列 Or lngCol = ColTitle.ct不显示标题 Or lngCol = ColTitle.ct隐藏数据显示 Or lngCol = ColTitle.ct启用定位 Or lngCol = ColTitle.ct列统计 Then
...
Private Sub DisDataDefault(lngRow As Long)
    ...
    With vsfDisSet
        For i = 1 To 9
            .TextMatrix(lngRow, i) = ""
            If i = ColTitle.ct隐藏列 Or i = ColTitle.ct隐藏数据显示 Or i = ColTitle.ct不显示标题 Or i = ColTitle.ct启用定位 Or i = ColTitle.ct列统计 Then
