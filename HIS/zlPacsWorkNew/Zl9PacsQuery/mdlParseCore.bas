...
Public Function RestoreQuote(ByVal strSql As String, ByRef strQuotes() As String, ByRef blnIsParSymbolTransfer As Boolean) As String
'恢复双引号之间的内容
    Dim i As Long
    Dim lngCount As Long
    Dim strQuoteContext As String
    
    lngCount = UBound(strQuotes)
    blnIsParSymbolTransfer = False
    
    RestoreQuote = strSql
    For i = 1 To lngCount
        strQuoteContext = strQuotes(i)
        
        If InStr(strQuoteContext, "[") > 0 Then
            strQuoteContext = Replace(strQuoteContext, "[", "&lbkt;")
            blnIsParSymbolTransfer = True
        End If
        
        If InStr(strQuoteContext, "]") > 0 Then
            strQuoteContext = Replace(strQuoteContext, "]", "&rbkt;")
            blnIsParSymbolTransfer = True
        End If
        
        RestoreQuote = Replace(RestoreQuote, "<@" & i & "/>", strQuoteContext)
    Next i
End Function
