...
Public Function ExecuteCoreEx(ByVal strSql As String, _
    ByVal strTitle As String, _
    objSqlParse As clsSqlParse, _
    Optional ByVal blnIsStore = False, _
    Optional ByVal blnStart0 As Boolean = True) As ADODB.Recordset
    
    Dim cmdData As New ADODB.Command

    Set cmdData = GetSqlCmd(strSql, strTitle, objSqlParse.ParValues, blnStart0)
    If cmdData Is Nothing Then Exit Function
    
    '为真时表示对单引号中含有的“[]”符号进行了转义，因此在实际查询时，需要替换回来
    If objSqlParse.IsParSymbolTransfer Then
        strSql = Replace(Replace(strSql, "&lbkt;", "["), "&rbkt;", "]")
    End If
    
    cmdData.CommandText = strSql
    
    Set ExecuteCoreEx = ExcuteCmd(strSql, cmdData, blnIsStore)
End Function


Private Function GetSqlCmd(ByRef strSql As String, ByVal strTitle As String, arrInput() As Variant, _
    Optional ByVal blnStart0 As Boolean = True) As ADODB.Command
...
'功能：通过Command对象打开带参数SQL的记录集
    ...
    Set cmdData = GetSqlCmd(strSql, strTitle, arrInput, blnStart0)
    If cmdData Is Nothing Then Exit Function
     
    '非方案的主体查询方式，如该方法用于查询过滤条件取值等，因此固定对单引号中含有的“[]”符号进行了转义
    cmdData.CommandText = Replace(Replace(strSql, "&lbkt;", "["), "&rbkt;", "]")
    
    Set ExecuteCore = ExcuteCmd(strSql, cmdData, blnIsStore)
 
End Function

Private Function ExcuteCmd(ByVal strSql As String, cmdData As ADODB.Command, Optional ByVal blnIsStore As Boolean = False)
    Dim rsData As New ADODB.Recordset
    Dim strprovider As String
    Dim lngtmpIndex As Long
    
    Set ExcuteCmd = Nothing
    
    strprovider = UCase(cmdData.ActiveConnection.Provider)
    
    '避免产生多余的SQL
    If InStr(strprovider, "MSDATASHAPE") > 0 Then
        lngtmpIndex = InStr(strSql, "SELECT * FROM (")
    ...
    Else
        ...
        rsData.CursorType = adOpenStatic ' adOpenDynamic
        rsData.LockType = adLockOptimistic
        
        rsData.Open cmdData
        
        Set rsData.ActiveConnection = Nothing
        Set ExcuteCmd = rsData
    End If
    
'    Set ExcuteCmd = cmdData.Execute
'    Set ExcuteCmd.ActiveConnection = Nothing
End Function
...
Private Function RecordSetToStr(rsData As ADODB.Recordset) As String
...
'recordset转xml字符
    ...
    strDataSource = RecordSetToStr(rsData)
...
Public Function ConnectRecordset(rsData1 As Recordset, rsData2 As Recordset) As Recordset
    Dim i As Long
    Dim rsTmpMax As ADODB.Recordset
    Dim rsTmpMin As ADODB.Recordset
    
    If rsData1.RecordCount > rsData2.RecordCount Then
        Set rsTmpMax = rsData1
        Set rsTmpMin = rsData2
    Else
        Set rsTmpMax = rsData2
        Set rsTmpMin = rsData1
    End If
    
    If rsTmpMin.RecordCount <= 0 Then
        Set ConnectRecordset = rsTmpMax
        Exit Function
    End If
    
    Do While Not rsTmpMin.EOF
        rsTmpMax.AddNew
        For i = 0 To rsTmpMax.Fields.Count - 1
            rsTmpMax.Fields(i).Value = rsTmpMin.Fields(i).Value
        Next
        rsTmpMax.Update
...
Public Function ConnectRecordsetEx(rsTempA As Recordset, rsTempB As Recordset) As Recordset
    Dim i As Long
    If rsTempB.RecordCount <= 0 Then
        Set ConnectRecordsetEx = rsTempA
        Exit Function
    End If
