...
Dim blnExist As Boolean, i As Integer
    ...
    If Me.lblName.tag = "" And Not blnExist Then
        gstrSQL = "zl_医技执行房间_Insert(" & Val(mlngdept) & ",'" & Trim(Me.txtName.Text) & "','" & zlStr.NeedCode(cboDevice.Text) & "','" & txtNoPrefix.Text & "')"
    Else
        gstrSQL = "zl_医技执行房间_Update(" & Val(mlngdept) & ",'" & Trim(Me.lblName.tag) & "','" & Trim(Me.txtName.Text) & "','" & zlStr.NeedCode(cboDevice.Text) & "','" & txtNoPrefix.Text & "')"
    End If
