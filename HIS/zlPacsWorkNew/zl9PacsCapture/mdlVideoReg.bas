...
On Error GoTo err
    ...
    Set rsTemp = zlCL_GetDBObj.OpenSQLRecord(strSql, "提取注册信息", strIP地址)
    
    If Not rsTemp.EOF Then funVideoRegTime = Now
    Exit Function
err:
    MsgboxEx frmParent, err.Description, vbOKOnly, G_STR_HINT_TITLE
    err.Clear
End Function
...
'返回值：注册成功注册日期；注册失败返回空
    ...
    If intNum > 0 Then  '按数量限制
        ...
        zlCL_ExecuteProcedure strSql, "注册" & str类型
        '检查注册是否成功
        strSql = "Select 启动时间,IP地址 from 影像操作记录 where  类型=[1]"
        Set rsTemp = zlCL_GetDBObj.OpenSQLRecord(strSql, "提取启动时间", str类型)
    ...
    Call MsgboxEx(frmParent, "打开的" & str类型 & "超过您购买的总数量（" & intNum & "）。请向软件供应商联系。", vbOKOnly, G_STR_HINT_TITLE)
    FunLogIn = ""
...
On Error GoTo errHandle
    ...
    Set rsTemp = zlCL_GetDBObj.OpenSQLRecord(strSql, "提取启动时间", strIP地址, str类型)
...
'返回值：合法注册True；非法启动的False
    ...
    If intNum > 0 Then '按照数量控制
        ...
        Else
            ...
            Set rsTemp = zlCL_GetDBObj.OpenSQLRecord(strSql, "提取数据库时间")
...
'参数： strLicenseName --- 授权名称
    ...
    strLiceseCount = zlCL_GetRegInfo(strLicenseName)
...
'------------------------------------------------
    ...
    If gethostname(hostname, 256) = SOCKET_ERROR Then
        MsgboxEx frmParent, "Windows Sockets error " & Str(WSAGetLastError()), vbOKOnly, G_STR_HINT_TITLE
        Exit Function
    Else
    ...
    If hostent_addr = 0 Then
        MsgboxEx frmParent, "Winsock.dll is not responding.", vbOKOnly, G_STR_HINT_TITLE
        Exit Function
    End If
...
'------------------------------------------------
    ...
    If lobyte(WSAD.wversion) < WS_VERSION_MAJOR Or (lobyte(WSAD.wversion) = _
        ...
        MsgboxEx frmParent, sMsg, vbOKOnly, G_STR_HINT_TITLE
        Exit Sub
    End If
...
Dim lReturn As Long
    ...
    If lReturn <> 0 Then
        MsgboxEx frmParent, "Socket error " & Trim$(Str$(lReturn)) & " occurred in Cleanup ", vbOKOnly, G_STR_HINT_TITLE
        Exit Sub
    End If
End Sub
