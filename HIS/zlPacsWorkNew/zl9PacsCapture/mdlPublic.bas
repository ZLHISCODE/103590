...
'返回：参数值，字符串形式
    ...
    Else
        ...
        If blnNew Then
            ...
            Set mrsDeptParas = zlCL_GetDBObj.OpenSQLRecord(strSQL, "读取参数")
        End If
...
'------------------------------------------------
    ...
    If rsTemp.EOF = True Then
        MsgboxEx frmParent, "没有找到可以删除的图像!", vbInformation, G_STR_HINT_TITLE
        DeleteImages = False
        Exit Function
    End If
    ...
    If Not IsNull(rsTemp!设备号1) Then
        ...
        If lngResult = 0 Then
            If Not IsNull(rsTemp!设备号2) Then
                ...
                If lngResult = 0 Then
                    If MsgboxEx(frmParent, "连接FTP失败，是否继续删除图像？" & vbCrLf & "此时继续删除，则只能删除数据库内容，无法删除图像文件。" & vbCrLf & "‘是’则继续删除，‘否’则不删除。", vbQuestion + vbYesNo, G_STR_HINT_TITLE) = vbNo Then
                        DeleteImages = False
                        Exit Function
                    End If
                End If
...
'-----------------------------------------------
    ...
    Set rsTemp = zlCL_GetDBObj.OpenSQLRecord(strSQL, "判断存储设备是否存在", objFtp.strDeviceId)
     ...
     '没有存储设备时退出
    If rsTemp.EOF = True Then
        MsgboxEx frmParent, "没有找到存储设备,请重新选择存储设备!", vbInformation, G_STR_HINT_TITLE
