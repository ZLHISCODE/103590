...
Object = "{5C493D4E-FD57-4FF4-9BA4-C6C670BFF9A7}#69.0#0"; "zl9PacsControl.ocx"
...
'配置ftp存储设备
    ...
    mobjFtp.strDeviceId = ""
    If rsTmp.EOF Then
        MsgboxEx Me, "影像存储设备未定义或处于停用，请检查！", vbInformation, G_STR_HINT_TITLE
    ...
    If Val(strBakStorageNo) > 0 Then
        ...
        If rsTmp.EOF Then
            MsgboxEx Me, "未取得有效的备份设备信息，不能对采集图像进行备份操作，请检查备份设备配置是否正确。", vbInformation, G_STR_HINT_TITLE
...
'判断是否为正常的采集方式
    ...
    If rsData.RecordCount > 0 Then
        If Val(Nvl(rsData!图像数)) <> ucPreview.ImageTotal Then
            Call MsgboxEx(Me, "当前加载的图像数量与实际记录数不一致，请检查是否另有用户对其进行操作，如无操作请刷新后重试。", vbInformation + vbOKOnly, G_STR_HINT_TITLE)
            Exit Function
        End If
    End If
...
'************************************************************
    ...
    Else
        ...
        If Not mblnRealTime Then
            blnIsRealCap = IIf(MsgboxEx(Me, "确定要采集当前静态图像吗？选“是”则采集当前处理图像。", vbQuestion + vbYesNo + vbDefaultButton1, G_STR_HINT_TITLE) = vbYes, False, True)
        End If
...
BUGEX "DoStateChange(frmWork_Video) 3 PostMessage lngAdviceId:" & mlngAdviceId
        '发送广播消息
        BoradcastMsg mlngAdviceId
    End If
...
'视频采集菜单处理
    Select Case control.ID
        Case conMenu_Cap_Dynamic       '动态显示
            If IsTwainCaptureWay Then
              Call MsgboxEx(Me, "TWAIN采集模式下，不能进行动态视频的显示。", vbOKOnly, G_STR_HINT_TITLE)
            Else
              Call ConfigVideoShowState(True)
            End If
...
err:
BUGEX "OpenComm 11"
    Call MsgboxEx(Me, "端口打开错误", vbOKOnly, G_STR_HINT_TITLE)
BUGEX "OpenComm 12"
End Sub
...
BUGEX "subCaptureImg 3"
        '设置影像检查状态，如果采集第一张图，且原来的状态是已报到，则修改成已检查
        If ucPreview.ImgViewer.Images.Count = 1 Then
            
            If mlngStudyState < 3 Then
                strSQL = "Zl_影像检查_State(" & mlngAdviceId & "," & mlngSendNo & ",3,NULL,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & glngDepartId & ")"
                zlCL_ExecuteProcedure strSQL, "采集第一个图像"
            End If
        End If
        
BUGEX "subCaptureImg 4"
        If ucPreview.ImgViewer.Images.Count = 1 Then
            '采集第一副图像
            Call DoStateChange(vetCaptureFirstImg, mlngAdviceId, mlngSendNo, mcurStudyInf.strStudyUid)
        Else
            '更新图像
    ...
    Else
        ...
        Else
            ...
            If curPic Is Nothing Then
                Call MsgboxEx(Me, "视频图像采集失败，请检查视频参数设置是否正确(如视频设备，显示模式等)。", vbOKOnly, G_STR_HINT_TITLE)
        ...
        If strFileName <> "" Then
            ...
            Select Case strFileType
                Case "AVI"
                    ...
                    Else
                        MsgboxEx Me, "静态图像无法保存成AVI格式，请重新选择图像格式。", vbInformation, G_STR_HINT_TITLE
                    End If
                Case "DCM"
...
'------------------------------------------------
    ...
    Exit Function
errHandle:
    MsgboxEx Me, "GetDlgSelectFileInfo函数执行错误！", vbOKOnly + vbCritical, G_STR_HINT_TITLE
End Function
...
continue1:
    ...
    If Trim(strError) <> "" Then MsgboxEx Me, strError, vbInformation, G_STR_HINT_TITLE
    ...
    Exit Sub
CapErr:
    Call MsgboxEx(Me, err.Description, vbOKOnly, G_STR_HINT_TITLE)
    err.Clear
End Sub
...
'保存后台采集图像
    ...
    If Trim(mAfterCaptureInf.strAfterTag) = "" Then
        Call MsgboxEx(Me, "不能获取有效的后台采集标记，请检查后台采集的检查数量是否已满，后台采集检查数不能超过1000。", vbOKOnly, G_STR_HINT_TITLE)
        Exit Function
    End If
    ...
    If lngResult = 0 Then
        ...
        MsgboxEx Me, "FTP连接失败，后台采集图像无法保存，请检查网络设置。", vbInformation, G_STR_HINT_TITLE
        Exit Function
    End If
    ...
    For i = 1 To dcmAfter.Images.Count
        ...
        If ImgTmp.Tag.Tag <> VIDEOTAG And ImgTmp.Tag.Tag <> AUDIOTAG Then
            '保存dicom图像
            WriteToURL mstrBufferDir & strReceivedTime & "/" & mAfterCaptureInf.strAfterStudyUid & "/" & ImgTmp.InstanceUID, mobjFtp.strFtpDir & _
                strReceivedTime & "/" & mAfterCaptureInf.strAfterStudyUid & "/" & ImgTmp.InstanceUID
                
            '上传报告图
            WriteToURL mstrBufferDir & strReceivedTime & "/" & mAfterCaptureInf.strAfterStudyUid & "/" & ImgTmp.InstanceUID & ".jpg", mobjFtp.strFtpDir & _
                strReceivedTime & "/" & mAfterCaptureInf.strAfterStudyUid & "/" & ImgTmp.InstanceUID & ".jpg"
        Else
            '保存录像
            WriteToURL ImgTmp.Tag.videoFile, mobjFtp.strFtpDir & strReceivedTime & "/" & mAfterCaptureInf.strAfterStudyUid & "/" & ImgTmp.InstanceUID
...
BUGEX "subSaveImage 1"
    ...
    Set rsTmp = zlCL_GetDBObj.OpenSQLRecord(strSQL, App.ProductName, mlngAdviceId)
...
BUGEX "subSaveImage 3"
    '创建缓冲目录
    MkLocalDir mstrBufferDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/"
    lngSendNo = rsTmp!发送号
...
BUGEX "subSaveImage 5"
        ...
        If Len(strReportImages & " ;" & ImgTmp.InstanceUID & ".jpg") >= 4000 Then
            MsgboxEx Me, "图像数量超过上限，请先删除部分图像后，再继续采集图像。", vbInformation, G_STR_HINT_TITLE
...
BUGEX "subSaveImage 6"
        ...
        Select Case iEncode
            Case 1          'Run-Length Encoding行程压缩
                ImgTmp.WriteFile mstrBufferDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID, True, "1.2.840.10008.1.2.5"
            Case 2          '不处理，保持原图的压缩方式
                ImgTmp.WriteFile mstrBufferDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID, True
            Case Else       'Lossless JPEG encoding JPEG无损压缩
                ImgTmp.WriteFile mstrBufferDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID, True, "1.2.840.10008.1.2.4.70"
        End Select
BUGEX "subSaveImage 7"
        ImgTmp.FileExport mstrBufferDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID & ".jpg", "JPG", 80
    End If
BUGEX "subSaveImage 8"
    lngResult = mobjFtpConnection.FuncFtpConnect(mobjFtp.strFTPIP, mobjFtp.strFTPUser, mobjFtp.strFTPPwd)
    If lngResult = 0 Then
        'FTP操作失败，提示错误，并删除缩略图中的图像
        MsgboxEx Me, "FTP连接失败，图像无法保存，请检查网络设置。", vbInformation, G_STR_HINT_TITLE
        Call ucPreview.DeleteImage(ucPreview.CurImageCount)
...
BUGEX "subSaveImage 9"
    If Val(mobjBakFtp.strDeviceId) > 0 Then
        ...
        If lngResult = 0 Then
            mobjBakFtp.strDeviceId = ""
            MsgboxEx Me, "备份ftp设备连接失败，采集的图像将不能进行备份操作，如需备份请检查流程管理中的备份设备配置。", vbInformation, G_STR_HINT_TITLE
        End If
    End If
...
BUGEX "subSaveImage 11"
        '保存dicom图像
        WriteToURL mstrBufferDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID, mobjFtp.strFtpDir & _
            dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID
            
        WriteToURL mstrBufferDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID & ".jpg", mobjFtp.strFtpDir & _
            dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID & ".jpg"
            
BUGEX "subSaveImage 12"
        '备份当前采集的图像
        If mobjBakFtpConnection.hConnection <> 0 Then
            BakImgToURL mstrBufferDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID, mobjBakFtp.strFtpDir & _
                dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID
        End If
...
BUGEX "subSaveImage 13"
        '保存录像
        WriteToURL ImgTmp.Tag.videoFile, mobjFtp.strFtpDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID


        '备份录像
        If mobjBakFtpConnection.hConnection <> 0 Then
            BakImgToURL ImgTmp.Tag.videoFile, mobjBakFtp.strFtpDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID
        End If
BUGEX "subSaveImage 14"
        
        If ImgTmp.Tag.Tag = VIDEOTAG Then
            '将录像复制到对应的目录中，避免重新下载
            Call FileCopy(ImgTmp.Tag.videoFile, mstrBufferDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID & ".avi")
            Call Kill(ImgTmp.Tag.videoFile)
        
            ImgTmp.Tag.videoFile = mstrBufferDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID & ".avi"
        ElseIf ImgTmp.Tag.Tag = AUDIOTAG Then
            '将音频文件复制到对应的目录中，避免重新下载
            Call FileCopy(ImgTmp.Tag.videoFile, mstrBufferDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID & ".wav")
            Call Kill(ImgTmp.Tag.videoFile)
        
            ImgTmp.Tag.videoFile = mstrBufferDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID & ".wav"
        End If
    End If
...
BUGEX "subSaveImage 16"
    ...
    If blnFirstImage Then
        strSQL = "ZL_影像检查记录_SET(" & mlngAdviceId & "," & lngSendNo & ",'" & _
            mcurStudyInf.strStudyUid & "',null," & _
...
BUGEX "subSaveImage 18"
    ...
    If rsTmp.EOF Or ImgTmp.Tag.Tag = VIDEOTAG Or ImgTmp.Tag.Tag = AUDIOTAG Then
        If ImgTmp.Tag.Tag = VIDEOTAG Then
            strSQL = "ZL_影像序列_INSERT('" & mcurStudyInf.strStudyUid & "','" & ImgTmp.SeriesUID & "','视频录像',0)"
        ElseIf ImgTmp.Tag.Tag = AUDIOTAG Then
            strSQL = "ZL_影像序列_INSERT('" & mcurStudyInf.strStudyUid & "','" & ImgTmp.SeriesUID & "','音频数据',0)"
        Else
            strSQL = "ZL_影像序列_INSERT('" & mcurStudyInf.strStudyUid & "','" & ImgTmp.SeriesUID & "','" & ImgTmp.SeriesDescription & "',0)"
        End If
...
BUGEX "subSaveImage 22"
    ...
    If ImgTmp.Tag.Tag <> VIDEOTAG And ImgTmp.Tag.Tag <> AUDIOTAG Then
        '如果不是检查图像，则不保存报告图
        strSQL = "ZL_影像检查报告_ADD('" & mcurStudyInf.strStudyUid & "','" & ImgTmp.InstanceUID & ".jpg')"
        ReDim Preserve arrSQL(UBound(arrSQL) + 1)
        arrSQL(UBound(arrSQL)) = strSQL
    End If
...
BUGEX "subSaveImage 25"
    If ImgTmp.Tag.Tag <> VIDEOTAG And ImgTmp.Tag.Tag <> AUDIOTAG Then
        Call frmCaptureHint.ShowCaptureHint( _
            IIf(gobjCapturePar.IsWindowHint, mstrBufferDir & dtReceived & "/" & mcurStudyInf.strStudyUid & "/" & ImgTmp.InstanceUID, ""), _
            gobjCapturePar.IsSountHint, hpRB, Me)
    End If
