...
Object = "{5C493D4E-FD57-4FF4-9BA4-C6C670BFF9A7}#45.0#0"; "zl9PacsControl.ocx"
...
On Error GoTo errHandle
    If lvwStudy.ListItems.Count <= 0 Then
        Call MsgboxEx(Me, "没有可进行采集的检查数据。", vbOKOnly, G_STR_HINT_TITLE)
        Exit Sub
    End If
    
    If lvwStudy.SelectedItem Is Nothing Then
        Call MsgboxEx(Me, "请选择需要进行采集的检查数据。", vbOKOnly, gstrSysName)
        Exit Sub
    End If
...
Exit Sub
errHandle:
    MsgboxEx Me, err.Description, vbOK, G_STR_HINT_TITLE
End Sub
