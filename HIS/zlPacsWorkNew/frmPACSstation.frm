...
Begin VB.Form frmPACSStation 
   ...
   Begin VB.PictureBox picList 
      ...
      Begin VB.TextBox txtFilter 
         ...
         Left            =   870
         TabIndex        =   14
         ToolTipText     =   "*门诊号；+住院号；或手选查找方式，输入完成后直接回车开始查找"
         Top             =   45
         Width           =   1485
      End
...
Option Explicit
Private Enum mcol
    Col紧急 = 0: Col来源: Col阳性: Col质量: Col姓名: Col检查号: Col检查过程: Col性别: Col年龄: Col标识号: Col医嘱内容: Col部位方法: Col执行间: Col检查时间: Col开嘱时间: Col开嘱医生
    
    Col身高 = 16: Col体重: Col婴儿: Col登记人: Col报到人: Col完成人: Col打印胶片: Col报告操作: Col绿色通道: Col报告打印: Col报告人: Col复核人: Col检查技师: Col采图时间: Col随访描述
    
    Col影像类别 = 31: Col病人ID: Col主页ID: Col挂号单: Col病人科室ID: Col医嘱ID: Col发送号: Col检查UID: Col检查状态: ColNO: Col记录性质: Col转出: Col床号: Col当前病区ID '从31列开始不显示
End Enum
...
Private Enum FilterID
    ...
    ID_查找方式 = 4013: ID_查找值 = 4014: ID_开始查找 = 4015: ID_本次住院 = 4016
End Enum
...
Private mblncmd登记 As Boolean, mblncmd报到 As Boolean, mblncmd报告 As Boolean, mblncmd审核 As Boolean, mblncmd完成 As Boolean, mblncmd本次 As Boolean
...
Private mobjQueue As zl9QueueManage.clsQueueManage          '排队叫号
'窗口变量
...
Private Type Type_SQLCondition
    开始时间 As Date
    结束时间 As Date
    时间类型 As Integer                                 '时间查询方式 1=按检查时间、2=按发送时间
    单据号 As String
    门诊号 As Long
    住院号 As Long
    就诊卡 As String
...
Private Sub Menu_RichEPR(ByVal cbrID As Long)
    ...
    If mblnPacsReport = True Then
        Call mfrmPacsReport.zlRefresh(Val(vsList.TextMatrix(vsList.Row, Col医嘱ID)), Nvl(vsList.TextMatrix(vsList.Row, Col发送号), 0), mlngCur科室ID, mstrPrivs, mlngModul, Me, vsList.TextMatrix(vsList.Row, Col转出) = 1)
    Else
        Call mobjReport.zlRefresh(Val(vsList.TextMatrix(vsList.Row, Col医嘱ID)), mlngCur科室ID, True)
    End If
...
Dim strFilter As String, rsTmp As ADODB.Recordset, lngAdviceID As Long, lngSendNO As Long
    ...
    '显示序列选择窗口
    With vsList
        lngAdviceID = Nvl(.TextMatrix(.Row, Col医嘱ID), 0)
        lngSendNO = Nvl(.TextMatrix(.Row, Col发送号), 0)
    End With
    ...
    If frmSelectMuli.mblnOK = True Then
        ...
        If vsList.TextMatrix(vsList.Row, Col检查状态) = 3 Then
...
'只有进行中的报告可以操作该菜单,因为此时还没有签名
        ...
        On Error GoTo ErrHandle
        With vsList
            If .TextMatrix(.Row, Col报告人) <> "" Or .TextMatrix(.Row, Col报告操作) <> "" Then
            ...
            If mblnFinishCommit And InStr(mstrPrivs, "检查完成") > 0 Then '无报告完成后无需再次确认完成,但需要有检查完成的权限
                ...
                If zlDatabase.GetPara(81, glngSys) = 1 And Not bln病人在院(.TextMatrix(.Row, Col病人ID), .TextMatrix(.Row, Col主页ID)) And bln存在未审划价单(.TextMatrix(.Row, Col医嘱ID)) Then '执行后自动审核划价单有效，并且病人已出院，且有未审核的划价单
                    MsgBoxD Me, "该病人已出院，且有未审核的划价单不能完成！", vbExclamation, gstrSysName
                Else
                    gstrSQL = "ZL_影像检查_STATE(" & .TextMatrix(.Row, Col医嘱ID) & "," & .TextMatrix(.Row, Col发送号) & ",6,1)"
                End If
            Else
                gstrSQL = "ZL_影像检查_STATE(" & .TextMatrix(.Row, Col医嘱ID) & "," & .TextMatrix(.Row, Col发送号) & ",5,1)"
            End If
        End With
...
Private Sub Menu_Edit_无报告回退()
    ...
    With vsList
            ...
            Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "检查是否有图像", CLng(.TextMatrix(.Row, Col医嘱ID)))
            
            gstrSQL = "ZL_影像检查_STATE(" & .TextMatrix(.Row, Col医嘱ID) & "," & .TextMatrix(.Row, Col发送号) & "," & IIf(Nvl(rsTemp!检查UID) = "", 2, 3) & ")"
...
'可能由其它过程调用，此时传入有医嘱ID，但需要权限判断
    ...
    On Error GoTo ErrHandle
    If lng医嘱ID = 0 Then
        lng医嘱ID = vsList.TextMatrix(vsList.Row, Col医嘱ID)
    End If
    ...
    If zlDatabase.GetPara(81, glngSys) = 1 And Not bln病人在院(rsTemp!病人ID, Nvl(rsTemp!主页ID, 0)) And bln存在未审划价单(Nvl(lng医嘱ID)) Then '执行后自动审核划价单有效，并且病人已出院，且有未审核的划价单
...
Private Sub Menu_Manage_取消检查完成()
    On Error GoTo ErrHandle
    With vsList
            If .TextMatrix(.Row, Col转出) = 1 Then MsgBox "该病人的本次住院数据已经转出到后备数据库，不允许操作。", vbInformation, gstrSysName: Exit Sub
            gstrSQL = "ZL_影像检查_STATE(" & .TextMatrix(.Row, Col医嘱ID) & "," & .TextMatrix(.Row, Col发送号) & ",5)"
...
Private Sub Menu_Manage_标记阴阳(ByVal lngID As Long)
    ...
    End Select
    With vsList
        gstrSQL = "ZL_影像检查_结果(" & .TextMatrix(.Row, Col医嘱ID) & "," & iresult & ")"
        Call zlDatabase.ExecuteProcedure(gstrSQL, "结果阴阳性")
        
        If iresult = 1 Then
            Set .Cell(flexcpPicture, .Row, Col阳性) = imgList.ListImages("阳性").Picture
        End If
    End With
    Exit Sub
ErrHandle:
...
Private Sub Menu_Manage_绿色通道(ByVal lngID As Long)
    ...
    End Select
    With vsList
        gstrSQL = "Zl_绿色通道_Update(" & .TextMatrix(.Row, Col医嘱ID) & ",'" & intResult & "')"
        Call zlDatabase.ExecuteProcedure(gstrSQL, "绿色通道")
        .TextMatrix(.Row, Col绿色通道) = intResult
        If intResult = 1 Then
            Set .Cell(flexcpPicture, .Row, Col姓名) = imgList.ListImages("绿色通道").Picture
        End If
    End With
    Exit Sub
ErrHandle:
...
Private Sub Menu_Manage_影像质量(ByVal lngID As Long)
    ...
    End Select
    With vsList
        gstrSQL = "Zl_影像质量_Update(" & .TextMatrix(.Row, Col医嘱ID) & ",'" & strResult & "')"
        Call zlDatabase.ExecuteProcedure(gstrSQL, "影像质量")
        .TextMatrix(.Row, Col质量) = strResult
    End With
    Exit Sub
ErrHandle:
    If ErrCenter = 1 Then
...
Private Sub Menu_Manage_修改()
    ...
    With frmRISRequest
        .mlngModul = mlngModul
        .mlngSendNo = vsList.TextMatrix(vsList.Row, Col发送号)
        .mlngAdviceID = vsList.TextMatrix(vsList.Row, Col医嘱ID)
        .mintEditMode = IIf(vsList.TextMatrix(vsList.Row, Col检查状态) > 1, 3, 1) '0－登记、1－登记后修改、2－报到、3－报到后修改
...
Private Sub Menu_Manage_复制登记()
    With frmRISRequest
        ...
        .mblnOK = False
        .InitMvar
        .CopyCheck vsList.TextMatrix(vsList.Row, Col医嘱ID), vsList.TextMatrix(vsList.Row, Col发送号) '刷新病人
        .zlShowMe Me
        If .mblnOK Then '成功返回
            If mbln直接检查 Then
                Call StateCheck(2)
...
Private Sub Menu_Manage_取消登记()
    ...
    gstrSQL = "ZL_病人医嘱执行_拒绝执行(" & vsList.TextMatrix(vsList.Row, Col医嘱ID) & "," & vsList.TextMatrix(vsList.Row, Col发送号) & ")"
...
'功能：召回被取消的登记
    ...
    gstrSQL = "ZL_病人医嘱执行_取消拒绝(" & vsList.TextMatrix(vsList.Row, Col医嘱ID) & "," & vsList.TextMatrix(vsList.Row, Col发送号) & ")"
...
Dim rsTemp As ADODB.Recordset, lngAdviceID As Long
    ...
    On Error GoTo ErrHandle
    With vsList
        If .TextMatrix(.Row, Col检查状态) <= 1 Then Call Menu_Manage_取消登记: Exit Sub '工具栏调用
        '------------------------------------有签名的需要先回退签名后再撤消
        lngAdviceID = .TextMatrix(.Row, Col医嘱ID)
        gstrSQL = "Select Distinct B.完成时间 From 病人医嘱报告 A, 电子病历记录 B Where A.病历ID=B.Id And A.医嘱ID=[1]"
        Set rsTemp = OpenSQLRecord(gstrSQL, "提取是否签名", lngAdviceID)
        ...
        If .TextMatrix(.Row, Col检查UID) <> "" And InStr(mstrPrivs, "清除图像") <= 0 Then
        ...
        If mblnUseQueue = True Then
            Call mobjQueue.zlDelQueue(Split(mstrCur科室, "-")(1) & .TextMatrix(.Row, Col执行间), lngAdviceID)
        End If
        
        gstrSQL = "ZL_影像检查_CANCEL(" & lngAdviceID & "," & .TextMatrix(.Row, Col发送号) & ",1)"
        ExecuteProc gstrSQL, Me.Caption
        '删除影像文件和目录
        RemoveCheckImages lngAdviceID, .TextMatrix(.Row, Col发送号)
    End With
...
Private Sub Menu_Manage_观片()
    If TabWindow.Selected.Tag <> "影像图象" Then '起到刷新图像作用
        ...
        Else
            Call mfrmPACSImg.zlRefresh(vsList.TextMatrix(vsList.Row, Col医嘱ID), vsList.TextMatrix(vsList.Row, Col发送号), mstrPrivs, vsList.TextMatrix(vsList.Row, Col转出) = 1)
        End If
    End If
...
Dim rsTemp As ADODB.Recordset, lngAdviceID As Long, lngSendNO As Long
    ...
    With vsList
        lngAdviceID = .TextMatrix(.Row, Col医嘱ID)
        lngSendNO = .TextMatrix(.Row, Col发送号)
    End With
    
    If TabWindow.Selected.Tag <> "影像图象" Then '起到刷新图像作用
        Call mfrmPACSImg.zlRefresh(lngAdviceID, lngSendNO, mstrPrivs, vsList.TextMatrix(vsList.Row, Col转出) = 1)
    End If
    
    gstrSQL = "select 检查UID from 影像检查记录 where 医嘱ID =[1] and  发送号 = [2]"
    Set rsTemp = OpenSQLRecord(gstrSQL, "提取检查UID", lngAdviceID, lngSendNO)
...
Dim strImageDeviceNumber As String, rsTemp As ADODB.Recordset
    ...
    frmPACSGetDeviceImage.ShowMe Me, rsTemp("IP地址"), rsTemp("端口号"), rsTemp("设备名"), Nvl(rsTemp("本地AE")), Nvl(rsTemp("设备AE")), vsList.TextMatrix(vsList.Row, Col医嘱ID)
    Call RefreshList
    Exit Sub
ErrHandle:
    If ErrCenter = 1 Then
...
Dim rsTemp As ADODB.Recordset, lngAdviceID As Long, lngSendNO As Long
    ...
    With vsList
        ...
        If .TextMatrix(.Row, Col检查状态) < 3 Then
            '如果病人已经有图像，则修改成已检查
...
Private Sub cboTimes_Click()
    ...
    If lngAdviceID = vsList.TextMatrix(vsList.Row, Col医嘱ID) Then Call vsList_RowColChange: Exit Sub '当次与当前选中医嘱ID相同时不由本函数控制
...
'            Case zlWebForum(Me.Hwnd)
        ...
        Case conMenu_View_Filter * 100# To conMenu_View_Filter * 100# + UBound(Split(mstrCanUse科室, "|")) '更改当前科室
        ...
        Case conMenu_ReportPopup * 100# + 1 To conMenu_ReportPopup * 100# + 99
                ...
                With vsList
                    If .TextMatrix(.Row, Col医嘱ID) <> "" Then
                        Call ReportOpen(gcnOracle, Split(control.Parameter, ",")(0), Split(control.Parameter, ",")(1), Me, _
                            "NO=" & .TextMatrix(.Row, ColNO), "性质=" & .TextMatrix(.Row, Col记录性质), "医嘱id=" & .TextMatrix(.Row, Col医嘱ID), 1)
                    Else
            ...
            End If
        Case Else
            If vsList.TextMatrix(vsList.Row, Col医嘱ID) = "" Then Exit Sub
            
            Select Case TabWindow.Selected.Tag
                Case "报告填写"
                    '没报告不能打印和预览
                    If (vsList.TextMatrix(vsList.Row, Col报告人) = "" Or vsList.TextMatrix(vsList.Row, Col报告操作) = "") And (control.ID = conMenu_File_Preview Or control.ID = conMenu_File_Print) Then
                    ...
                    If control.ID = conMenu_Edit_Audit Or control.ID = conMenu_Edit_Modify Or control.ID = conMenu_PacsReport_Open Or control.ID = conMenu_Edit_Delete Then
                        If CheckConcurrentReport(vsList.TextMatrix(vsList.Row, Col医嘱ID)) = False Then Exit Sub
                    End If
                    ...
                    If mblnTechReptSame = True _
                        ...
                        And Nvl(vsList.TextMatrix(vsList.Row, Col检查技师)) <> "" _
                        And Nvl(vsList.TextMatrix(vsList.Row, Col检查技师)) <> UserInfo.姓名 Then
...
Private Sub Menu_View_Filter_click()
    ...
    With frmPACSFilter
        ...
        If .optFindType(0).Value = True Then '时间查找方式：1＝按检查时间、2＝按开嘱时间
            SQLCondition.时间类型 = 1
        Else
            SQLCondition.时间类型 = 2
        End If
        ...
        If .cboPart.ListIndex <> 0 Then '检查标本部位
            SQLCondition.标本部位 = .cboPart.Text
        Else
            SQLCondition.标本部位 = ""
        End If
        ...
        If .chk结果阳性.Value = 1 Then
            SQLCondition.结果阳性 = True
        Else
            SQLCondition.结果阳性 = False
        End If
...
Dim blnNoRecord As Boolean, intState As Integer, blnCancel As Boolean
    ...
    blnNoRecord = Val(vsList.TextMatrix(vsList.Row, Col医嘱ID)) = 0
    control.Style = xtpButtonIconAndCaption
    If Not blnNoRecord Then
        intState = Val(vsList.TextMatrix(vsList.Row, Col检查状态))
        blnCancel = vsList.TextMatrix(vsList.Row, Col检查过程) = "已拒绝"
    End If
    ...
    Select Case control.ID
        ...
        Case conMenu_Manage_Cancel   '取消关联(&B)
            ...
            ElseIf intState >= 2 And intState <= 5 Then
                control.Enabled = vsList.TextMatrix(vsList.Row, Col检查UID) <> ""
            Else
                control.Enabled = False
            End If
        ...
        Case conMenu_Manage_ClearUp   '无报告回退(&U)
            ...
            ElseIf intState = 5 Then
                control.Enabled = vsList.TextMatrix(vsList.Row, Col报告人) = ""
            Else
                control.Enabled = False
            End If
        ...
        Case conMenu_Img_3D     '三维重建
            ...
            If control.Visible = True Then
                ...
                If control.Parent.Type <> xtpControlPopup Then
                    control.Visible = vsList.TextMatrix(vsList.Row, Col检查UID) <> ""
        ...
        Case conMenu_Manage_ChangeDevice    '更改影像设备
            If blnNoRecord = True Then
                control.Enabled = False
            Else
                If UCase(vsList.TextMatrix(vsList.Row, Col影像类别)) = "CR" Or _
                    UCase(vsList.TextMatrix(vsList.Row, Col影像类别)) = "DR" Or _
                    UCase(vsList.TextMatrix(vsList.Row, Col影像类别)) = "DX" Or _
                    UCase(vsList.TextMatrix(vsList.Row, Col影像类别)) = "RF" Then
                    control.Enabled = True
                Else
                    control.Enabled = False
                End If
        ...
        Case Else
            ...
            If Not blnNoRecord Then
                '删除只能在已报告和进行中可用
                If control.ID = conMenu_Edit_Delete And Val(vsList.TextMatrix(vsList.Row, Col检查状态)) >= 4 Then
                    control.Enabled = False
                End If
                '当前查看的是历次记录则菜单均不可用
                If cboTimes.ListIndex <> -1 Then
                    If vsList.TextMatrix(vsList.Row, Col医嘱ID) <> cboTimes.ItemData(cboTimes.ListIndex) Then control.Enabled = False
                End If
                '已完成除查阅,以及医嘱中报告查看打印，观片菜单外均不可用
                If Val(vsList.TextMatrix(vsList.Row, Col检查状态)) = 6 Then
                    Select Case control.ID
...
'功能:初始化模块级变量,仅窗体加载时调用一次
    ...
    mintResultInput = GetDeptPara(mlngCur科室ID, "提示阴阳性", 1)    '              '提示阴阳性
    ...
    End If
    mlngRefreshInterval = GetDeptPara(mlngCur科室ID, "自动刷新间隔", 0) = 1 '      '自动刷新间隔,默认不自动刷新
...
Private Sub Menu_Manage_PACS报告()
    ...
    mfrmPacsReportDock.zlEditReport lngAdviceID, lngSendNO, mlngCur科室ID, Me, mstrPrivs, mlngModul, vsList.TextMatrix(vsList.Row, Col转出) = 1
...
Private Sub cmdInfo_Click()
    On Error GoTo ErrHandle
    frmDegreeCard.ShowMe Val(vsList.TextMatrix(vsList.Row, Col病人ID)), Val(vsList.TextMatrix(vsList.Row, Col主页ID))
    Exit Sub
ErrHandle:
    If ErrCenter = 1 Then
        Resume
...
Private Sub Form_Load()
    ...
    Set mobjQueue = New zl9QueueManage.clsQueueManage      '排队叫号
...
'初始化临时本地参数，以个人设置，注册表参数为主,窗体加载，本地设置等调用
    ...
    mstrCurFindtype = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\" & Me.Name, "定位方式", "检查号")
...
'Begin----------------------检查菜单--------------------------------------默认可见
    ...
    With cbrMenuBar.CommandBar.Controls
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_Manage_Review, "随访(&R)"):  cbrControl.BeginGroup = True: cbrControl.IconId = 232
...
'---------------------工具栏定义------------------------------------------
    ...
    With cbrToolBar.Controls
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_Manage_Review, "随访"):  cbrControl.BeginGroup = True: cbrControl.IconId = 232
...
Private Sub InitList()
    With vsList
        ...
        .ColWidth(Col紧急) = 300: .ColWidth(Col来源) = 400: .ColWidth(Col阳性) = 300: .ColWidth(Col质量) = 300: .ColWidth(Col姓名) = 1200
        .ColWidth(Col检查号) = 1400: .ColWidth(Col检查过程) = 800: .ColWidth(Col性别) = 450: .ColWidth(Col年龄) = 450: .ColWidth(Col标识号) = 1400: .ColWidth(Col医嘱内容) = 2400
        .ColWidth(Col部位方法) = 1400: .ColWidth(Col执行间) = 600: .ColWidth(Col检查时间) = 1700: .ColWidth(Col开嘱时间) = 1700: .ColWidth(Col开嘱医生) = 800
        .ColWidth(Col身高) = 450: .ColWidth(Col体重) = 450: .ColWidth(Col婴儿) = 450: .ColWidth(Col登记人) = 800: .ColWidth(Col报到人) = 800
        .ColWidth(Col完成人) = 800: .ColWidth(Col打印胶片) = 800: .ColWidth(Col报告操作) = 800: .ColWidth(Col绿色通道) = 0: .ColWidth(Col报告打印) = 800
        .ColWidth(Col报告人) = 800: .ColWidth(Col复核人) = 800: .ColWidth(Col检查技师) = 800: .ColWidth(Col采图时间) = 1700: .ColWidth(Col随访描述) = 2400
        
        .ColWidth(Col影像类别) = 0: .ColWidth(Col病人ID) = 0: .ColWidth(Col主页ID) = 0: .ColWidth(Col挂号单) = 0: .ColWidth(Col医嘱ID) = 1200: .ColWidth(Col发送号) = 0
        .ColWidth(Col病人科室ID) = 0: .ColWidth(Col检查UID) = 0: .ColWidth(Col检查状态) = 0: .ColWidth(ColNO) = 0: .ColWidth(Col记录性质) = 0: .ColWidth(Col转出) = 0
        .ColWidth(Col床号) = 0: .ColWidth(Col当前病区ID) = 0
        
        Set .Cell(flexcpPicture, 0, Col紧急) = imgList.ListImages("紧急").Picture
        Set .Cell(flexcpPicture, 0, Col来源) = imgList.ListImages("住院").Picture
        Set .Cell(flexcpPicture, 0, Col阳性) = imgList.ListImages("阳性").Picture
        
        .TextMatrix(0, Col质量) = "质": .TextMatrix(0, Col姓名) = "姓名": .TextMatrix(0, Col检查号) = "检查号": .TextMatrix(0, Col检查过程) = "检查过程"
        .TextMatrix(0, Col性别) = "性别": .TextMatrix(0, Col年龄) = "年龄": .TextMatrix(0, Col标识号) = "标识号": .TextMatrix(0, Col医嘱内容) = "医嘱内容": .TextMatrix(0, Col部位方法) = "部位方法"
        .TextMatrix(0, Col执行间) = "执行间": .TextMatrix(0, Col检查时间) = "检查时间": .TextMatrix(0, Col开嘱时间) = "开嘱时间": .TextMatrix(0, Col开嘱医生) = "开嘱医生"
        .TextMatrix(0, Col身高) = "身高": .TextMatrix(0, Col体重) = "体重": .TextMatrix(0, Col婴儿) = "婴儿": .TextMatrix(0, Col登记人) = "登记人"
        .TextMatrix(0, Col报到人) = "报到人": .TextMatrix(0, Col完成人) = "完成人": .TextMatrix(0, Col打印胶片) = "打印胶片": .TextMatrix(0, Col报告操作) = "报告操作"
        .TextMatrix(0, Col绿色通道) = "绿色通道": .TextMatrix(0, Col报告打印) = "报告打印": .TextMatrix(0, Col报告人) = "报告人": .TextMatrix(0, Col复核人) = "复核人"
        .TextMatrix(0, Col检查技师) = "检查技师": .TextMatrix(0, Col采图时间) = "采图时间": .TextMatrix(0, Col随访描述) = "随访描述"
        
        .TextMatrix(0, Col影像类别) = "影像类别": .TextMatrix(0, Col病人ID) = "病人ID": .TextMatrix(0, Col主页ID) = "主页ID": .TextMatrix(0, Col挂号单) = "挂号单"
        .TextMatrix(0, Col病人科室ID) = "病人科室ID": .TextMatrix(0, Col医嘱ID) = "医嘱ID": .TextMatrix(0, Col发送号) = "发送号": .TextMatrix(0, Col检查UID) = "检查UID"
        .TextMatrix(0, Col检查状态) = "检查状态": .TextMatrix(0, ColNO) = "NO": .TextMatrix(0, Col记录性质) = "记录性质": .TextMatrix(0, Col转出) = "转出"
        .TextMatrix(0, Col床号) = "床号": .TextMatrix(0, Col当前病区ID) = "当前病区ID"
...
Dim lngOrderID As Long
    ...
    On Error GoTo ErrHandle
    lngOrderID = vsList.TextMatrix(vsList.Row, Col医嘱ID)
...
Dim lngOrderID As Long
    ...
    If mblnShowImgAtReport And vsList.TextMatrix(vsList.Row, Col检查UID) <> "" Then
...
'功能：显示当前执行医嘱可以打印的诊疗单据在菜单上
    ...
    With vsList
        gstrSQL = "Select Distinct C.编号,C.名称,C.说明" & _
            ...
            " Order by C.编号"
        If .TextMatrix(.Row, Col转出) = 1 Then
            gstrSQL = Replace(gstrSQL, "病人医嘱记录", "H病人医嘱记录")
            gstrSQL = Replace(gstrSQL, "病人医嘱发送", "H病人医嘱发送")
        End If
        Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, CLng(.TextMatrix(.Row, Col医嘱ID)), CLng(Decode(.TextMatrix(.Row, Col来源), "门", 1, "住", 2, "外", 3, 4)))
    End With
...
'功能：打印诊疗单据
    ...
    If ReportPrintSet(gcnOracle, glngSys, objControl.Parameter, Me) Then
        Call ReportOpen(gcnOracle, glngSys, objControl.Parameter, Me, "NO=" & vsList.TextMatrix(vsList.Row, ColNO), "性质=" & vsList.TextMatrix(vsList.Row, Col记录性质), 1)
    End If
    Exit Sub
errH:
    If ErrCenter() = 1 Then Resume
...
Dim i As Integer, lngcur医嘱ID As Long
    With vsList
        lngcur医嘱ID = Val(.TextMatrix(.Row, Col医嘱ID)) '当前行医嘱ID
        ...
        '有记录时要重新定位回之前记录
        For i = 1 To .Rows - 1
            If lngcur医嘱ID = Val(.TextMatrix(i, Col医嘱ID)) Then
                Call .Select(i, Col姓名)
                Exit Sub
            End If
        Next
...
Dim strFilter As String
    ...
    On Error GoTo ErrHandle
    With SQLCondition
        If Not (.门诊号 = 0 And .住院号 = 0 And .就诊卡 = "" And .姓名 = "" And .身份证 = "" And .IC卡 = "") Then '界面查找条件不使用时间索引
            If .门诊号 <> 0 Then
                strFilter = " And C.门诊号=[1]"
            ElseIf .住院号 <> 0 Then
                strFilter = " And C.住院号=[2]"
            ElseIf .就诊卡 <> "" Then
                strFilter = " And C.就诊卡=[3]"
            ElseIf .姓名 <> "" Then
                strFilter = " And C.姓名=[4]"
            ElseIf .身份证 <> "" Then
                strFilter = " And C.身份证=[5]"
            ElseIf .IC卡 <> "" Then
        ...
        Else
            If .结束时间 <> CDate(0) Then
                strFilter = " And " & IIf(mblncmd登记, "A.发送时间", IIf(.时间类型 = 2, "A.发送时间", "A.首次时间")) & " Between [9] and [10] "
            Else '缺省查询条件
                strFilter = " And " & IIf(mblncmd登记, "A.发送时间", IIf(.时间类型 = 2, "A.发送时间", "A.首次时间")) & " Between [9] and Sysdate+1/(24*3600) "
            End If
            ...
            If .疾病诊断 <> "" Then '-------------------------------------------------------------------------需要改
                strFilter = strFilter & " And (B.病人ID,B.主页ID) IN(Select Distinct A.病人Id,A.主页ID  " & _
            ...
            If strSubFilter <> "" Then
                ...
                strFilter = strFilter & " And (B.病人ID,B.主页ID) IN(Select Distinct a.病人Id,a.主页ID From 电子病历记录 a, 电子病历内容 b,电子病历内容 c " _
        ...
        gstrSQL = "Select Distinct" & vbNewLine & _
                    "       A.医嘱ID,A.发送号,A.首次时间 检查时间,A.发送时间 开嘱时间,A.执行状态,nvl(A.执行过程,0) 检查过程,A.执行间,A.结果阳性 阳性," & vbNewLine & _
                    ...
                    "       Decode(B.病人来源,3,B.开嘱医生,A.发送人) 登记人,H.报到人," & vbNewLine & _
                    "       H.完成人,H.是否打印,H.报告操作,H.绿色通道,H.报告打印,H.报告人,H.复核人,H.检查技师,H.接收日期 采图时间,H.随访描述,H.检查UID,0 as 转出" & vbNewLine & _
                    " From 病人医嘱发送 A,病人医嘱记录 B,病人信息 C,影像检查记录 H,影像检查项目 G" & vbNewLine & _
                    " Where B.相关ID is NULL And A.医嘱ID=B.ID And A.医嘱ID=H.医嘱ID(+) And A.发送号=H.发送号(+) And B.诊疗项目ID=G.诊疗项目ID And B.病人ID=C.病人ID"
        gstrSQL = gstrSQL & vbNewLine & strFilter & " And A.执行部门ID+0=[25]"
        
        If mblncmd已缴 Xor mblncmd未缴 Then '互斥选择
            strFilter = "(Select Distinct No From 病人费用记录 D Where A.NO = D.NO And A.记录性质=D.记录性质 And D.记录状态 = 1)"
            gstrSQL = gstrSQL & vbNewLine & IIf(mblncmd已缴, " And Exists ", " And Not Exists") & strFilter
        End If
        
        If .检查号 <> 0 Then                        '当使用检查号查找时一定是报过到的，影像检查记录中有记录，此时取消左连接避免全表扫描
        ...
        End If
        gstrSQL = "Select * From (" & vbNewLine & gstrSQL & vbNewLine & ") Order by 检查过程,检查时间,开嘱时间"
    
        Set rsList = zlDatabase.OpenSQLRecord(gstrSQL, "提取病人列表", .门诊号, .住院号, .就诊卡, .姓名, .身份证, .IC卡, .单据号, .检查号, .开始时间, .结束时间, _
                                            .病人科室, .标本部位, .诊断医生, .审核医生, .影像质量, .检查技师, .影像类别, .随访, _
                                            .疾病诊断, .检查所见, .诊断意见, .建议, str来源, mstrRoom, mlngCur科室ID)
    End With
    
    strFilter = ""
    If mblncmd登记 Then strFilter = "检查过程=0 or 检查过程=1 or "
    If mblncmd报到 Then strFilter = IIf(strFilter <> "", strFilter & "检查过程=2 or 检查过程=3 or ", "检查过程=2 or 检查过程=3 or ")
    ...
    If mblncmd登记 And mblncmd报到 And mblncmd报告 And mblncmd审核 And mblncmd完成 Then
        strFilter = ""
    End If
    ...
    With vsList
        ...
        Do Until rsTemp.EOF
            ...
            If rsTemp!紧急标志 <> 0 Then
                Set .Cell(flexcpPicture, .Row, Col紧急) = imgList.ListImages("紧急").Picture
            End If
            If rsTemp!来源 = "住" Then
                Set .Cell(flexcpPicture, .Row, Col来源) = imgList.ListImages("住院").Picture
            Else
                .TextMatrix(.Row, Col来源) = rsTemp!来源
            End If
            ...
            If Nvl(rsTemp!检查UID) <> "" Then
                Set .Cell(flexcpPicture, .Row, Col检查号) = imgList.ListImages("影像").Picture
            End If
            
            .TextMatrix(.Row, Col质量) = Nvl(rsTemp!影像质量)
            .TextMatrix(.Row, Col姓名) = Nvl(rsTemp!姓名)
            .TextMatrix(.Row, Col检查号) = Nvl(rsTemp!检查号)
            .TextMatrix(.Row, Col检查过程) = IIf(rsTemp!执行状态 = 2, "已拒绝", Decode(Nvl(rsTemp!检查过程, 0), 0, "已登记", 1, "已登记", _
                                                                                        ...
                                                                                        5, "已审核", "已完成"))
            .TextMatrix(.Row, Col性别) = Nvl(rsTemp!性别)
            .TextMatrix(.Row, Col年龄) = Nvl(rsTemp!年龄)
            If InStr(Nvl(rsTemp!医嘱内容), ":") > 0 Then '新的模式保存在医嘱内容中信息是 名称,执行标记:部位(方法,方法),部位---
                .TextMatrix(.Row, Col医嘱内容) = Split(rsTemp!医嘱内容, ":")(0)
                .TextMatrix(.Row, Col部位方法) = Split(rsTemp!医嘱内容, ":")(1)
            Else
                .TextMatrix(.Row, Col医嘱内容) = Nvl(rsTemp!医嘱内容)
            End If
            .TextMatrix(.Row, Col执行间) = Nvl(rsTemp!执行间)
            .TextMatrix(.Row, Col检查时间) = Nvl(rsTemp!检查时间)
            .TextMatrix(.Row, Col开嘱时间) = Nvl(rsTemp!开嘱时间)
            .TextMatrix(.Row, Col开嘱医生) = Nvl(rsTemp!开嘱医生)
            .TextMatrix(.Row, Col身高) = Nvl(rsTemp!身高)
            .TextMatrix(.Row, Col体重) = Nvl(rsTemp!体重)
            .TextMatrix(.Row, Col婴儿) = Nvl(rsTemp!婴儿)
            .TextMatrix(.Row, Col登记人) = Nvl(rsTemp!登记人)
            .TextMatrix(.Row, Col报到人) = Nvl(rsTemp!报到人)
            .TextMatrix(.Row, Col完成人) = Nvl(rsTemp!完成人)
            .TextMatrix(.Row, Col打印胶片) = Nvl(rsTemp!是否打印)
            .TextMatrix(.Row, Col报告操作) = Nvl(rsTemp!报告操作)
            .TextMatrix(.Row, Col绿色通道) = Nvl(rsTemp!绿色通道)
            .TextMatrix(.Row, Col报告打印) = Nvl(rsTemp!报告打印)
            .TextMatrix(.Row, Col报告人) = Nvl(rsTemp!报告人)
            .TextMatrix(.Row, Col复核人) = Nvl(rsTemp!复核人)
            .TextMatrix(.Row, Col检查技师) = Nvl(rsTemp!检查技师)
            .TextMatrix(.Row, Col采图时间) = Nvl(rsTemp!采图时间)
            .TextMatrix(.Row, Col影像类别) = Nvl(rsTemp!影像类别)
            .TextMatrix(.Row, Col病人ID) = Nvl(rsTemp!病人ID)
            .TextMatrix(.Row, Col主页ID) = Nvl(rsTemp!主页ID)
            .TextMatrix(.Row, Col挂号单) = Nvl(rsTemp!挂号单)
            .TextMatrix(.Row, Col病人科室ID) = Nvl(rsTemp!病人科室ID)
            .TextMatrix(.Row, Col医嘱ID) = Nvl(rsTemp!医嘱ID)
            .TextMatrix(.Row, Col发送号) = Nvl(rsTemp!发送号)
            .TextMatrix(.Row, Col检查UID) = Nvl(rsTemp!检查UID)
            .TextMatrix(.Row, Col检查状态) = Nvl(rsTemp!检查过程)
            .TextMatrix(.Row, Col随访描述) = Nvl(rsTemp!随访描述)
            .TextMatrix(.Row, ColNO) = Nvl(rsTemp!NO)
            .TextMatrix(.Row, Col转出) = Nvl(rsTemp!转出)
            .TextMatrix(.Row, Col床号) = Nvl(rsTemp!当前床号)
            .TextMatrix(.Row, Col当前病区ID) = Nvl(rsTemp!当前病区ID)
            .TextMatrix(.Row, Col标识号) = Nvl(rsTemp!标识号)
            ...
            If Nvl(rsTemp!婴儿) <> 0 Then
                ...
                If Not rsBaby.EOF Then
                    .TextMatrix(.Row, Col姓名) = rsBaby!婴儿姓名
                    .TextMatrix(.Row, Col性别) = Nvl(rsBaby!婴儿性别)
                    .TextMatrix(.Row, Col年龄) = Nvl(rsBaby!出生时间)
                End If
            End If
            
            If .TextMatrix(.Row, Col检查过程) = "已拒绝" Then .Cell(flexcpBackColor, .Row, 0, .Row, .Cols - 1) = &HFFFF&
            If .TextMatrix(.Row, Col检查过程) = "已完成" Then .Cell(flexcpBackColor, .Row, 0, .Row, .Cols - 1) = &HFF00&
...
Private Sub TabWindow_SelectedChanged(ByVal Item As XtremeSuiteControls.ITabControlItem)
    ...
    ElseIf Val(vsList.TextMatrix(vsList.Row, Col医嘱ID)) = 0 Then
        RefreshTabWindow 0, True
    Else
        RefreshTabWindow 0, False, True
        If vsList.TextMatrix(vsList.Row, Col检查UID) = "" And mfrmPACSImg.lvwSeq.ListItems.Count > 0 Then '如果经过刷新后发现有图了，则刷新病人列表，目的是为了让观片等按键可用
            vsList.TextMatrix(vsList.Row, Col检查UID) = mfrmPACSImg.lvwSeq.Tag
        End If
    End If
...
Private Sub txtFilter_KeyPress(KeyAscii As Integer)
    ...
    Else
        If Trim(txtFilter.Text) <> "" Then
            If Mid(txtFilter.Text, 1, 1) = "*" Then mstrCurFindtype = "门诊号"
...
Dim strSQL As String
    ...
    frmChangeDevice.ShowMe UCase(vsList.TextMatrix(vsList.Row, Col影像类别)), Me
    strModality = frmChangeDevice.strDeviceType
    
     If strModality <> "" Then
         strSQL = "Zl_影像检查_影像类别(" & vsList.TextMatrix(vsList.Row, Col医嘱ID) & "," & vsList.TextMatrix(vsList.Row, Col发送号) & ",'" & strModality & "')"
...
Dim strReview As String
    ...
    If frmReview.ShowMe(vsList.TextMatrix(vsList.Row, Col医嘱ID), vsList.TextMatrix(vsList.Row, Col发送号), Me, strReview) = True Then
        vsList.TextMatrix(vsList.Row, Col随访描述) = strReview
    End If
...
Public Function zlInQueue(str队列名称 As String, lng业务ID As Long, lng科室ID As Long, _
        str患者姓名 As String, str诊室 As String, str医生姓名 As String, Optional str排队标记 As String = "", Optional lng排队号码 As Long) As Boolean
        
        On Error GoTo err
        
        If mblnUseQueue = True Then
            mobjQueue.zlInQueue str队列名称, lng业务ID, lng科室ID, str患者姓名, str诊室, str医生姓名, str排队标记, lng排队号码
        End If
        zlInQueue = True
        Exit Function
err:
...
'功能:显示病人卡片按钮
    If vsList.TextMatrix(NewRow, Col医嘱ID) = "" Then
        cmdInfo.Visible = False
    Else
        cmdInfo.Height = vsList.CellHeight - 30
        cmdInfo.Left = vsList.Cell(flexcpLeft, NewRow, Col姓名) + vsList.Cell(flexcpWidth, NewRow, Col姓名) - cmdInfo.Width - 15
...
'功能:显示病人卡片按钮
    If vsList.TextMatrix(vsList.Row, Col医嘱ID) = "" Then
        cmdInfo.Visible = False
    Else
        If NewLeftCol > Col姓名 Then
            cmdInfo.Visible = False
        Else
            cmdInfo.Height = vsList.CellHeight - 30
            cmdInfo.Left = vsList.Cell(flexcpLeft, vsList.Row, Col姓名) + vsList.Cell(flexcpWidth, vsList.Row, Col姓名) - cmdInfo.Width - 15
            cmdInfo.Top = vsList.CellTop + 15
            cmdInfo.Visible = True
        End If
    End If
...
Private Sub vsList_DblClick()
    If vsList.TextMatrix(vsList.Row, Col医嘱ID) <> "" Then
        Select Case vsList.TextMatrix(vsList.Row, Col检查状态)
            Case 1, 0
                Call Menu_Manage_报到
...
Private Sub vsList_RowColChange()
    ...
    If Val(vsList.TextMatrix(vsList.Row, Col医嘱ID)) = 0 Then '无记录时处理
...
Dim rsTemp As ADODB.Recordset
    ...
    With vsList
        lbl个人信息.Caption = "姓  名:" & Rpad(.TextMatrix(.Row, Col姓名), 12, " ") & "性  别:" & Rpad(.TextMatrix(.Row, Col性别), 13, " ") & _
                          "年  龄:" & Rpad(.TextMatrix(.Row, Col年龄), 10, " ") & "标识号:" & Rpad(.TextMatrix(.Row, Col标识号), 12, " ") & _
                          "床  号:" & Rpad(.TextMatrix(.Row, Col床号) & "", 10, " ")
                          
        If lngAdviceID = 0 Then '---------------------------非历次检查直接用列表中记录填充
            gstrSQL = "Select 名称 From 部门表 Where ID=[1]"
            Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "提取病人科室", CLng(.TextMatrix(.Row, Col病人科室ID)))
            lbl检查信息.Caption = "检查号:" & Rpad(.TextMatrix(.Row, Col检查号), 12, " ") & "病人科室:" & Rpad(rsTemp!名称, 11, " ") & _
                                  "开嘱医生:" & Rpad(.TextMatrix(.Row, Col开嘱医生), 8, " ") & "检查项目:" & .TextMatrix(.Row, Col医嘱内容)
            If .TextMatrix(.Row, Col部位方法) <> "" Then lbl检查信息.Caption = lbl检查信息.Caption & "(" & .TextMatrix(.Row, Col部位方法) & ")"
            lblCash.Caption = "收": lblCash.Visible = False
            lblCash.Visible = CheckChargeState(.TextMatrix(.Row, Col医嘱ID)) = 1
        Else
            Dim strSQLBak As String
            gstrSQL = "Select A.ID, A.病人科室id, A.开嘱医生,A.病人来源, A.医嘱内容, Nvl(A.婴儿, 0) 婴儿, A.病人id, A.主页id, A.挂号单, B.检查号, B.检查uid, C.名称, D.发送号,D.执行状态,0 as 转出" & vbNewLine & _
            ...
            If Not rsTemp.EOF Then
                ...
                fraInfo.Tag = rsTemp!病人ID & "|" & rsTemp!主页ID & "|" & rsTemp!ID & "|" & rsTemp!发送号 & "|" & rsTemp!病人科室ID & "|" & rsTemp!挂号单 & "|" & Nvl(rsTemp!病人来源, 3) & "|" & rsTemp!检查UID & "|" & rsTemp!转出 & "|" & rsTemp!执行状态
                ...
                If rsTemp!婴儿 <> 0 Then
                    ...
                    If Not rsTemp.EOF Then
                        lbl个人信息.Caption = "姓  名:" & Rpad(rsTemp!婴儿姓名, 12, " ") & "性  别:" & Rpad(rsTemp!婴儿性别, 13, " ") & _
                                            "年  龄:" & Rpad(rsTemp!出生时间, 10, " ") & "标识号:" & Rpad(.TextMatrix(.Row, Col标识号), 12, " ") & _
                                            "床  号:" & Rpad(.TextMatrix(.Row, Col床号) & "", 10, " ")
                    End If
                End If
            Else
...
Dim lngAdviceID As Long, strAppend As String, rsTemp As ADODB.Recordset, i As Integer
    ...
    With vsList
        If lngAdviceIDtmp = 0 Then
            lngAdviceID = Val(.TextMatrix(.Row, Col医嘱ID))
        Else
            lngAdviceID = lngAdviceIDtmp
        End If
        
        If lngAdviceIDtmp = 0 Then '-------------------------------------------列表选择调用
            If .TextMatrix(.Row, Col部位方法) <> "" Then
                For i = 0 To UBound(Split(.TextMatrix(.Row, Col部位方法), "),"))
                    If i = 0 Then
                        txtAppend = "检查部位:" & vbCrLf & Space(2) & "1:" & Split(.TextMatrix(.Row, Col部位方法), "),")(i) & ")"
                    Else
                        txtAppend = txtAppend & vbCrLf & Space(2) & i + 1 & ":" & Split(.TextMatrix(.Row, Col部位方法), "),")(i) & ")"
                    End If
                Next
                If Trim(txtAppend) <> "" Then txtAppend = Mid(txtAppend, 1, Len(txtAppend) - 1) '取掉最后的括号
            Else
                txtAppend = "检查部位:" & .TextMatrix(.Row, Col医嘱内容)
            End If
            gstrSQL = "Select 项目,内容 From 病人医嘱附件 Where 医嘱ID=[1] Order By 排列"
            If .TextMatrix(.Row, Col转出) = 1 Then gstrSQL = Replace(gstrSQL, "病人医嘱附件", "H病人医嘱附件")
        Else                    '-------------------------------------------历次记录选择调用
...
Dim rsTemp As ADODB.Recordset, strTemp As String
    ...
    With vsList
        ...
        gstrSQL = "Select A.ID 医嘱ID,A.开嘱时间  开嘱时间,A.医嘱内容 " & _
                   ...
                   "" & IIf(.TextMatrix(.Row, Col检查过程) = "已拒绝", "", " And B.执行状态<>2 ") & _
                   " AND A.ID=C.医嘱ID"
        ...
        Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "", CLng(.TextMatrix(.Row, Col病人ID)), mlngCur科室ID)
        ...
        Do Until rsTemp.EOF
           ...
           If rsTemp!医嘱ID = .TextMatrix(.Row, Col医嘱ID) Then cboTimes.ListIndex = cboTimes.NewIndex
           rsTemp.MoveNext
        Loop
        cboTimes.Tag = "完成"
    End With
...
Dim int来源 As Integer, i As Integer
    ...
    If lngAdviceID = 0 Then '-------------------------------------------列表选择调用
        int来源 = Val(vsList.TextMatrix(vsList.Row, Col来源))
        Dim blnShowReport As Boolean
        '判断 无图像不许写报告
        blnShowReport = True
        If mblnReportWithImage = True Then
            If vsList.TextMatrix(vsList.Row, Col检查UID) = "" Then blnShowReport = False
        End If
    Else                    '-------------------------------------------历次记录选择调用
        ...
        For i = 0 To TabWindow.ItemCount - 1
            Select Case TabWindow(i).Tag
                ...
                Case "影像图象"
                    TabWindow(i).Visible = True
                Case "报告填写"
                    TabWindow(i).Visible = IIf(lngAdviceID = 0, vsList.TextMatrix(vsList.Row, Col检查状态) > 1 And blnShowReport, True)
            End Select
        Next
    Else
        For i = 0 To TabWindow.ItemCount - 1
...
Dim lngUnit As Long, lngPatDept As Long, strRegNo As String, intMoved As Boolean, intState As Integer, i As Integer
    ...
    If lngAdviceIDtmp = 0 Then '-----------------------列表选择调用
        ...
        Else
            With vsList
                lngAdviceID = .TextMatrix(.Row, Col医嘱ID): lngSendNO = .TextMatrix(.Row, Col发送号)
                lngPatID = .TextMatrix(.Row, Col病人ID): lngPageID = Val(.TextMatrix(.Row, Col主页ID))
                lngPatDept = .TextMatrix(.Row, Col病人科室ID): strRegNo = .TextMatrix(.Row, Col挂号单)
                intMoved = .TextMatrix(.Row, Col转出): intState = .TextMatrix(.Row, Col检查状态)
                lngUnit = Val(.TextMatrix(.Row, Col当前病区ID))
            End With
        End If
    Else                       '----------------------历次记录选择调用
    ...
    For i = 0 To TabWindow.ItemCount - 1 '循环到了才刷新,可能没有授权该模块
        If TabWindow(i).Tag = "报告填写" Then
            If mblnPacsReport = True Then
                mfrmPacsReport.zlRefresh lngAdviceID, lngSendNO, mlngCur科室ID, mstrPrivs, mlngModul, Me, intMoved = 1
            Else
                mobjReport.zlRefresh lngAdviceID, mlngCur科室ID, Not mblnIsHistory, intMoved = 1
            End If
        End If
    Next
    ...
    Select Case TabWindow(TabWindow.Selected.Index).Tag
        ...
        Case "排队叫号"
            If Not mblnIsHistory Then
                mobjQueue.zlRefresh gcnOracle, mAstr队列名称, Split(mstrCur科室, "-")(1) & vsList.TextMatrix(vsList.Row, Col执行间), lngAdviceID
            End If
        Case "住院医嘱"
