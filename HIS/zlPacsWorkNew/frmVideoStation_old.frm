...
Private Const ConstrCol = "紧急;300|来源;400|阳性;300|质量;300|姓名;1200|检查号;1400|检查过程;800|性别;450|年龄;450" & _
                        "|标识号;1400|医嘱内容;2400|部位方法;1400|执行间;600|检查时间;1800|开嘱时间;1800|开嘱医生;800" & _
                        ...
                        "|检查状态;0|NO;0|记录性质;0|转出;0|床号;0|当前病区ID;0|报告发放;800"
...
'子窗体对像
Private WithEvents mfrmCapture As frmVideoCapture       '视频采集子窗体
Attribute mfrmCapture.VB_VarHelpID = -1
Private WithEvents mfrmDockVideo As frmVideoCapture                '浮动的采集窗口
...
Private mblnReportWithResult As Boolean                     '有阴阳性结果才能写报告
...
Private mblnChangeUser As Boolean                           '允许交换用户
...
Private Sub Menu_RichEPR(ByVal cbrID As Long)
    ...
    With vsList
        ...
        If mblnPacsReport = True Then
            Call mfrmPacsReport.zlRefresh(Val(.TextMatrix(.Row, GetCN("医嘱ID"))), Nvl(.TextMatrix(.Row, GetCN("发送号")), 0), mlngCur科室ID, mstrPrivs, mlngModul, Me, .TextMatrix(.Row, GetCN("转出")) = 1)
        Else
...
'只有进行中的报告可以操作该菜单,因为此时还没有签名
        ...
        With vsList
            ...
            If mblnFinishCommit And InStr(mstrPrivs, "检查完成") > 0 Then '无报告完成后无需再次确认完成,但需要有检查完成的权限
                ...
                If zlDatabase.GetPara(81, glngSys) = 1 And Not bln病人在院(.TextMatrix(.Row, GetCN("病人ID")), .TextMatrix(.Row, GetCN("主页ID"))) And bln存在未审划价单(.TextMatrix(.Row, GetCN("医嘱ID"))) Then '执行后自动审核划价单有效，并且病人已出院，且有未审核的划价单
...
Private Sub Menu_Manage_取消检查完成()
    On Error GoTo ErrHandle
    With vsList
            If .TextMatrix(.Row, GetCN("转出")) = 1 Then MsgBox "该病人的本次住院数据已经转出到后备数据库，不允许操作。", vbInformation, gstrSysName: Exit Sub
            gstrSQL = "ZL_影像检查_STATE(" & .TextMatrix(.Row, GetCN("医嘱ID")) & "," & .TextMatrix(.Row, GetCN("发送号")) & ",5)"
...
Private Sub Menu_Manage_复制登记()
    With frmRISRequest
        ...
        .mblnOK = False
        .InitMvar
        .CopyCheck vsList.TextMatrix(vsList.Row, GetCN("医嘱ID")), vsList.TextMatrix(vsList.Row, GetCN("发送号")) '刷新病人
        .zlShowMe Me
        If .mblnOK Then '成功返回
            If mbln直接检查 Then
...
Dim rsTemp As ADODB.Recordset, lngAdviceID As Long
    ...
    On Error GoTo ErrHandle
     With vsList
        ...
        gstrSQL = "ZL_影像检查_CANCEL(" & lngAdviceID & "," & .TextMatrix(.Row, GetCN("发送号")) & ",1)"
        zlDatabase.ExecuteProcedure gstrSQL, Me.Caption
        '删除影像文件和目录
...
Private Sub Menu_Dept_Select(ByVal control As XtremeCommandBars.ICommandBarControl)
    If mlngCur科室ID <> control.DescriptionText Then
        ...
        If mblnDockVideo = True Then '重新初始化采集窗口科室级参数
            mfrmDockVideo.InitDeptPara mlngCur科室ID
        Else
            mfrmCapture.InitDeptPara mlngCur科室ID
        End If
...
'--------------------------文件------------------
        ...
        Case conMenu_Manage_ChangeUser '交换用户
            Call ChangeUser
...
'--------------------------帮助-----------------
        ...
        Case Else
            ...
            Select Case TabWindow.Selected.Tag
                Case "报告填写"
                    If vsList.TextMatrix(vsList.Row, GetCN("报告人")) = "" And (control.ID = conMenu_File_Preview Or control.ID = conMenu_File_Print) Then
                    ...
                    If control.ID = conMenu_Edit_Audit Or control.ID = conMenu_Edit_Modify Or control.ID = conMenu_PacsReport_Open Or control.ID = conMenu_Edit_Delete Then
                        If CheckConcurrentReport(vsList.TextMatrix(vsList.Row, GetCN("医嘱ID"))) = False Then Exit Sub
                    End If
                    ...
                    If mblnTechReptSame = True _
                        ...
                        And vsList.TextMatrix(vsList.Row, GetCN("检查技师")) <> "" _
                        And vsList.TextMatrix(vsList.Row, GetCN("检查技师")) <> IIf(mblnCnOracleIsHIS = True, mstrUserNameHIS, mstrUserNameNew) Then
                    ...
                    End If
                Case "申请费用"
                    If mblnChangeUser = True And mintChangeUserState = 2 Then '交换了用户，则不允许操作
        ...
        Case Else
            ...
            If Not blnNoRecord Then
                ...
                If cboTimes.ListIndex <> -1 Then
                    If vsList.TextMatrix(vsList.Row, GetCN("医嘱ID")) <> cboTimes.ItemData(cboTimes.ListIndex) Then control.Enabled = False
                End If
...
Private Sub Menu_Manage_浮动采集(Optional blnUnload As Boolean = True)
    ...
    With vsList
        lngAdviceID = vsList.TextMatrix(vsList.Row, GetCN("医嘱ID"))
        intState = vsList.TextMatrix(vsList.Row, GetCN("检查状态"))
        strInfor = vsList.TextMatrix(vsList.Row, GetCN("姓名"))
        blnMoved = vsList.TextMatrix(vsList.Row, GetCN("转出")) = 1
    End With

    blnReadOnly = (intState = 6 Or intState = 0 Or intState = 1)
    blnReadOnly = IIf(blnReadOnly = True, True, InStr(mstrPrivs, "视频采集") <= 0)
...
Private Sub Menu_Manage_PACS报告()
    '程序分成两部分，一部分是原来已经通过报告窗口打开了视频源的
    '判断原有的报告窗体中是否已经打开了视频源，如果是，则关闭视频源后直接打开新报告窗口
    ...
    '另一部分是视频源在浮动窗口或者嵌入式窗口打开的。
    '判断当前新打开的报告窗体中是否需要启动视频源，如果需要，
    '如果有浮动窗口，先关闭浮动窗口，再关闭嵌入窗体的视频源，然后再打开新报告窗口
    If mblnPacsReportShowVideoCapture = True Then
        If mblnDockVideo = True Then
            '先关闭浮动采集窗口
            Menu_Manage_浮动采集 (True)
        End If
    End If
    Call OpenPacsReport
End Sub
...
Private Sub OpenPacsReport()
    ...
    mfrmPacsReportDock.zlEditReport vsList.TextMatrix(vsList.Row, GetCN("医嘱ID")), vsList.TextMatrix(vsList.Row, GetCN("发送号")), mlngCur科室ID, Me, mstrPrivs, mlngModul, vsList.TextMatrix(vsList.Row, GetCN("转出")) = 1
...
Private Sub Form_Load()
   ...
   '初始子窗体
    Set mfrmCapture = New frmVideoCapture  '视频采集
        mfrmCapture.mlngModul = mlngModul
        mfrmCapture.mlngCurDeptId = mlngCur科室ID
    ...
    Me.stbThis.Panels(3).Text = "报告医生：" & mstrUserNameHIS & "   检查医生：" & mstrUserNameNew
...
'Begin------------------------文件菜单--------------------------------------默认可见
    ...
    With cbrMenuBar.CommandBar.Controls
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_Manage_ChangeUser, "交换用户(&T)"): cbrControl.IconId = 3012
...
'Begin----------------------检查菜单--------------------------------------默认可见
    ...
    With cbrMenuBar.CommandBar.Controls
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_Manage_Review, "附加信息(&R)"):  cbrControl.BeginGroup = True: cbrControl.IconId = 232
...
'----------------------快键绑定------------------------------------------
    With Me.cbrMain.KeyBindings
        ...
        .Add FCONTROL, Asc("G"), conMenu_View_FindType    '定位方式---------Ctrl+F
...
'---------------------工具栏定义------------------------------------------
    ...
    With cbrToolBar.Controls
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_Manage_Review, "附加信息"):  cbrControl.BeginGroup = True: cbrControl.IconId = 232
        Set cbrControl = .Add(xtpControlButton, conMenu_Manage_ReportRelease, "发放"): cbrControl.ToolTipText = "报告发放": cbrControl.IconId = 3013
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_Manage_ChangeUser, "交换"): cbrControl.IconId = 3012: cbrControl.ToolTipText = "交换检查医生和报告医生"
...
Dim i As Integer
    With TabWindow
        ...
        '图像采集在同一时刻只能使用一种方式，要么使用TWAIN接口的方式，要么使用WDMvfw类型的采集方式
        
        .InsertItem 0, "影像采集", mfrmCapture.hwnd, conMenu_Cap_Dynamic
        .Item(TabWindow.ItemCount - 1).Tag = "影像采集"
        
        
        If GetInsidePrivs(p诊疗报告管理, True) <> "" Then
            If mblnPacsReport = True Then
                .InsertItem 2, "影像报告", mfrmPacsReport.hwnd, conMenu_Edit_Compend
            Else
...
Dim C标识号 As Long, C医嘱内容 As Long, C部位方法 As Long, C执行间 As Long, C检查时间 As Long, C开嘱时间 As Long, C开嘱医生 As Long
...
Dim C检查状态 As Long, CNO As Long, C记录性质 As Long, C转出 As Long, C床号 As Long, C当前病区ID As Long, C报告发放 As Long
    ...
    With vsList
        ...
        C执行间 = GetCN("执行间"):      C检查时间 = GetCN("检查时间"):  C开嘱时间 = GetCN("开嘱时间")
        ...
        .ColWidth(C执行间) = GetCW("执行间"):       .ColWidth(C检查时间) = GetCW("检查时间"):   .ColWidth(C开嘱时间) = GetCW("开嘱时间")
        ...
        .Cell(flexcpData, 0, C执行间) = "执行间":           .Cell(flexcpData, 0, C检查时间) = "检查时间":       .Cell(flexcpData, 0, C开嘱时间) = "开嘱时间"
        ...
        .TextMatrix(0, C执行间) = "执行间":         .TextMatrix(0, C检查时间) = "检查时间":     .TextMatrix(0, C开嘱时间) = "开嘱时间"
...
Private Sub mfrmDockVideo_StateChanged(strStudyUID As String, blnAddImage As Boolean, blnStateChanged As Boolean)
    ...
    If blnStateChanged Then
        If blnAddImage Then '采图
            vsList.TextMatrix(vsList.Row, GetCN("检查UID")) = Nvl(strStudyUID, "A123456789")
            Set vsList.Cell(flexcpPicture, vsList.Row, GetCN("检查号")) = Imglist.ListImages("影像").Picture '改变图标
        Else '最后一次册图
            vsList.TextMatrix(vsList.Row, GetCN("检查UID")) = ""
            Set vsList.Cell(flexcpPicture, vsList.Row, GetCN("检查号")) = Nothing '改变图标
        End If
    End If
    '更新报告窗口图像显示
    If mblnPacsReport = True Then
    ...
    If mblnWriteCapDoctor = True And blnStateChanged = True Then
        If mblnCnOracleIsHIS Then
            strSQL = "Zl_影像检查_检查技师( " & vsList.TextMatrix(vsList.Row, GetCN("医嘱ID")) & "," & vsList.TextMatrix(vsList.Row, GetCN("发送号")) & ",'" & IIf(blnAddImage = True, mstrUserNameNew, "") & "')"
        Else
            strSQL = "Zl_影像检查_检查技师( " & vsList.TextMatrix(vsList.Row, GetCN("医嘱ID")) & "," & vsList.TextMatrix(vsList.Row, GetCN("发送号")) & ",'" & IIf(blnAddImage = True, mstrUserNameHIS, "") & "')"
        End If
...
'        mfrmPacsReport.zlRefresh 0, 0, 0
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "提取是否签名", CLng(lngOrderID))
    If rsTemp.EOF Then Exit Sub
...
Private Sub mfrmCapture_StateChanged(strStudyUID As String, blnAddImage As Boolean, blnStateChanged As Boolean)
    ...
    If mblnWriteCapDoctor = True And blnStateChanged = True Then
        If mblnCnOracleIsHIS Then
            gstrSQL = "Zl_影像检查_检查技师( " & vsList.TextMatrix(vsList.Row, GetCN("医嘱ID")) & "," & vsList.TextMatrix(vsList.Row, GetCN("发送号")) & ",'" & IIf(blnAddImage = True, mstrUserNameNew, "") & "')"
        Else
            gstrSQL = "Zl_影像检查_检查技师( " & vsList.TextMatrix(vsList.Row, GetCN("医嘱ID")) & "," & vsList.TextMatrix(vsList.Row, GetCN("发送号")) & ",'" & IIf(blnAddImage = True, mstrUserNameHIS, "") & "')"
        End If
        zlDatabase.ExecuteProcedure gstrSQL, Me.Caption
    End If
End Sub
...
Dim strModalitys As String
    ...
    With SQLCondition
        ...
        gstrSQL = "Select Distinct" & vbNewLine & _
                    ...
                    "       Decode(B.病人来源,3,B.开嘱医生,A.发送人) 登记人,H.报到人, H.报告发放, " & vbNewLine & _
        ...
        If mblncmd已缴 Xor mblncmd未缴 Then '互斥选择
            strFilter = "("
            strFilter = strFilter & "Select Distinct NO From 住院费用记录 D Where A.NO = D.NO And A.记录性质 = D.记录性质 And D.记录状态 = 1" & vbNewLine & _
                                    "Union" & vbNewLine & _
        ...
        Set rsList = zlDatabase.OpenSQLRecord(gstrSQL, "提取病人列表", .门诊号, .住院号, .就诊卡, .姓名, .身份证, .IC卡, .单据号, .检查号, .开始时间, .结束时间, _
                                            ...
                                            .疾病诊断, .检查所见, .诊断意见, .建议, str来源, mstrRoom, mlngCur科室ID, strModalitys)
    End With
...
Dim rsBaby As ADODB.Recordset
    ...
    With vsList
        ...
        Do Until rsTemp.EOF
            ...
            .TextMatrix(.Row, GetCN("检查时间")) = Nvl(rsTemp!检查时间)
            .TextMatrix(.Row, GetCN("开嘱时间")) = Nvl(rsTemp!开嘱时间)
            ...
            .TextMatrix(.Row, GetCN("打印胶片")) = Nvl(rsTemp!是否打印)
            .TextMatrix(.Row, GetCN("报告操作")) = Nvl(rsTemp!报告操作)
            .TextMatrix(.Row, GetCN("绿色通道")) = Nvl(rsTemp!绿色通道)
            .TextMatrix(.Row, GetCN("报告打印")) = Nvl(rsTemp!报告打印)
            ...
            .TextMatrix(.Row, GetCN("病人ID")) = Nvl(rsTemp!病人ID)
            .TextMatrix(.Row, GetCN("主页ID")) = Nvl(rsTemp!主页ID)
            .TextMatrix(.Row, GetCN("挂号单")) = Nvl(rsTemp!挂号单)
            .TextMatrix(.Row, GetCN("病人科室ID")) = Nvl(rsTemp!病人科室ID)
            ...
            .TextMatrix(.Row, GetCN("当前病区ID")) = Nvl(rsTemp!当前病区ID)
            ...
            If Nvl(rsTemp!婴儿) <> 0 Then
                ...
                End If
            End If
            If .TextMatrix(.Row, GetCN("检查过程")) = "已拒绝" Then .Cell(flexcpBackColor, .Row, 0, .Row, .Cols - 1) = &HFFFF&
            If .TextMatrix(.Row, GetCN("检查过程")) = "已完成" Then .Cell(flexcpBackColor, .Row, 0, .Row, .Cols - 1) = &HFF00&
            rsTemp.MoveNext
        Loop
    End With
...
Private Sub ChangeUser()
    ...
    If mblnCnOracleIsHIS Then
        Me.stbThis.Panels(3).Text = "报告医生：" & mstrUserNameHIS & "   检查医生：" & mstrUserNameNew
    Else
        Me.stbThis.Panels(3).Text = "报告医生：" & mstrUserNameNew & "   检查医生：" & mstrUserNameHIS
    End If
End Sub
...
Dim rsTemp As ADODB.Recordset
    ...
    With vsList
        ...
        If lngAdviceID = 0 Then '---------------------------非历次检查直接用列表中记录填充
            ...
            lblCash.Visible = CheckChargeState(.TextMatrix(.Row, GetCN("医嘱ID"))) = 1
        Else
...
Dim rsTemp As ADODB.Recordset, strTemp As String
    ...
    With vsList
        ...
        Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "", CLng(.TextMatrix(.Row, GetCN("病人ID"))), mlngCur科室ID)
    ...
    If lngAdviceIDtmp = 0 Then '-----------------------列表选择调用
        ...
        Else
            With vsList
                ...
                intMoved = .TextMatrix(.Row, GetCN("转出")): intState = .TextMatrix(.Row, GetCN("检查状态"))
    ...
    Select Case TabWindow(TabWindow.Selected.Index).Tag
        ...
        Case "排队叫号"
            If Not mblnIsHistory Then
                mobjQueue.zlRefresh gcnOracle, mAstr队列名称, Split(mstrCur科室, "-")(1) & vsList.TextMatrix(vsList.Row, GetCN("执行间")), lngAdviceID
            End If
        Case "住院医嘱"
        ...
        Case "影像采集"
            ...
            '处理视频源的刷新，优先级别是：浮动采集窗口，PACS报告编辑器窗口，嵌入式PACS报告编辑器，图像采集窗口
            With vsList
                If mblnDockVideo = True Then    '如果打开了浮动采集窗口，则刷新这个窗口
                    If intState = 6 Or intState = 0 Or intState = 1 Then    '查看模式
                        mfrmDockVideo.zlBeginCapture lngAdviceID, False, True, True, intMoved = 1, .TextMatrix(.Row, GetCN("姓名"))
                    Else
                        mfrmDockVideo.zlBeginCapture lngAdviceID, False, InStr(mstrPrivs, "视频采集") <= 0, True, intMoved = 1, .TextMatrix(.Row, GetCN("姓名"))
                    End If
                ...
                ElseIf Not mfrmPacsReportDock Is Nothing Then
                    If mfrmPacsReportDock.pblnShowVideoCapture = True Then
                        '不用刷新
                    End If
                ElseIf mblnPacsReport = True And mfrmPacsReport.pblnShowVideoCapture = True And TabWindow(TabWindow.Selected.Index).Tag = "报告填写" Then
                    '不用刷新
                Else                            '检查状态= 6只能以查方式
                    If intState = 6 Or intState = 0 Or intState = 1 Then  '查看模式
                        mfrmCapture.zlBeginCapture lngAdviceID, True, True, Not mblnDockVideo, intMoved = 1
                    Else
                        mfrmCapture.zlBeginCapture lngAdviceID, True, InStr(mstrPrivs, "视频采集") <= 0, Not mblnDockVideo, intMoved = 1
                    End If
                End If
            End With
    End Select
