...
Begin VB.Form frmVideoStation 
   ...
   Begin MSComctlLib.ImageList img16 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmVideoStation.frx":4D12
            Key             =   "复选选中"
            Object.Tag             =   "90001"
         EndProperty
      EndProperty
   ...
   Begin XtremeDockingPane.DockingPane dkpMain 
      Bindings        =   "frmVideoStation.frx":52AC
      Left            =   0
      Top             =   0
      _Version        =   589884
      _ExtentX        =   450
...
'
'    Call CopyMemory(keyStruct, ByVal lParam, LenB(keyStruct))
'
'    Select Case keyStruct.VKCode
'        Case VK_F8
'            '执行快捷采集
'            MsgBox "Capture"
'    End Select
...
Dim rsTemp As ADODB.Recordset
    ...
    With frmRISRequest
        ...
        .mlngCurDeptId = mlngCur科室ID
        .InitMvar
        If .RefreshPatiInfor(True) Then   '刷新病人
            .mblnOK = False
            .zlShowMe Me
        End If
        If .mblnOK Then  '成功返回
...
'功能:初始化模块级变量,仅窗体加载时调用一次
    ...
    mblnChangeUser = GetDeptPara(mlngCur科室ID, "允许交换用户", 0) = "1"              '允许交换用户
...
'    cbrToolBar.EnableDocking xtpFlagStretched '+ xtpFlagHideWrap
    ...
    With cbrToolBar.Controls
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_Manage_Regist, "登记"): cbrControl.BeginGroup = True: cbrControl.IconId = 211: cbrControl.ToolTipText = "登记"
        Set cbrControl = .Add(xtpControlButton, conMenu_Manage_Receive, "报到"): cbrControl.IconId = 744: cbrControl.ToolTipText = "报到"
        Set cbrControl = .Add(xtpControlButton, conMenu_Manage_Logout, "取消"): cbrControl.IconId = 743: cbrControl.ToolTipText = "取消报到"
        
        Set cbrControl = .Add(xtpControlButton, conMenu_Manage_Review, "备注"): cbrControl.BeginGroup = True: cbrControl.IconId = 232: cbrControl.ToolTipText = "附加信息"
        Set cbrControl = .Add(xtpControlButton, conMenu_Manage_ReportRelease, "发放"): cbrControl.ToolTipText = "报告发放": cbrControl.IconId = 3013
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_View_Filter, "过滤"): cbrControl.BeginGroup = True: cbrControl.ToolTipText = "过滤"
        Set cbrControl = .Add(xtpControlButton, conMenu_View_Refresh, "刷新"): cbrControl.ToolTipText = "刷新"
        Set cbrControl = .Add(xtpControlButton, conMenu_File_Exit, "退出"): cbrControl.ToolTipText = "退出"
...
Dim i As Integer
    With TabWindow
        ...
        If mblnUseQueue = True Then
            ...
            .InsertItem 7, "排队叫号", mobjQueue.zlGetForm.hWnd, 10011 'conMenu_File_PrintSingleBill
...
Dim C收费 As Long
    ...
    With lst
        ...
        C报告发放 = GetCN("报告发放"):  C诊断分类 = GetCN("诊断分类"):  C关联ID = GetCN("关联ID")
        C病人科室 = GetCN("病人科室"):  C就诊卡号 = GetCN("就诊卡号"):  C单据号 = GetCN("单据号")
        C身份证号 = GetCN("身份证号"):  C收费 = GetCN("收费")
        ...
        .ColWidth(C报告发放) = GetCW("报告发放"):   .ColWidth(C诊断分类) = GetCW("诊断分类"):   .ColWidth(C关联ID) = GetCW("关联ID")
        .ColWidth(C病人科室) = GetCW("病人科室"):   .ColWidth(C就诊卡号) = GetCW("就诊卡号"):   .ColWidth(C单据号) = GetCW("单据号")
        .ColWidth(C身份证号) = GetCW("身份证号"):   .ColWidth(C收费) = GetCW("收费")
        ...
        .Cell(flexcpData, 0, C报告发放) = "报告发放":       .Cell(flexcpData, 0, C诊断分类) = "诊断分类":       .Cell(flexcpData, 0, C关联ID) = "关联ID"
        .Cell(flexcpData, 0, C病人科室) = "病人科室":       .Cell(flexcpData, 0, C就诊卡号) = "就诊卡号":       .Cell(flexcpData, 0, C单据号) = "单据号"
        .Cell(flexcpData, 0, C身份证号) = "身份证号":       .Cell(flexcpData, 0, C收费) = "收费"
        ...
        .TextMatrix(0, C报告发放) = "报告发放":     .TextMatrix(0, C诊断分类) = "诊断分类":     .TextMatrix(0, C关联ID) = "关联ID"
        .TextMatrix(0, C病人科室) = "病人科室":     .TextMatrix(0, C就诊卡号) = "就诊卡号":     .TextMatrix(0, C单据号) = "单据号"
        .TextMatrix(0, C身份证号) = "身份证号"
...
Private Sub mobjReport_AfterSaved(ByVal lngOrderID As Long)
    Call AfterReportSaved(lngOrderID, Me) '如果是电子病例编辑器，则传自身窗体
End Sub
...
Private Sub UpdateStudyListState(lngAdviceID As Long, strStudyUID As String, blnAddImage As Boolean, blnStateChanged As Boolean)
    ...
    If mblnWriteCapDoctor = True And blnStateChanged = True Then
        If mblnCnOracleIsHIS Then
            gstrSQL = "Zl_影像检查_检查技师( " & vsList.TextMatrix(intRowIndex, GetCN("医嘱ID")) & "," & vsList.TextMatrix(intRowIndex, GetCN("发送号")) & ",'" & IIf(blnAddImage = True, mstrUserNameNew, "") & "')"
        Else
            gstrSQL = "Zl_影像检查_检查技师( " & vsList.TextMatrix(intRowIndex, GetCN("医嘱ID")) & "," & vsList.TextMatrix(intRowIndex, GetCN("发送号")) & ",'" & IIf(blnAddImage = True, mstrUserNameHIS, "") & "')"
        End If
        zlDatabase.ExecuteProcedure gstrSQL, Me.Caption
    End If
End Sub
...
Dim blnUseTime As Boolean       '是否使用时间条件
    ...
    With SQLCondition
        ...
        '其他条件查询，使用时间索引
            ...
            If .结果阳性 <> -1 Then
                strFilter = strFilter & " And Nvl(A.结果阳性, 0)=[28]"
            End If
        ...
        If .报告内容 <> "" Then
            strFilter = strFilter & " And B.id IN ( Select t.医嘱id From 病人医嘱报告 t Where t.病历id In " & _
                                                                    ...
                                                                    " And B.对象类型=2 And instr(B.内容文本,[27])>0 And B.终止版 = 0)) "
        End If

        gstrSQL = "Select /*+ RULE */ Distinct" & vbNewLine & _
                    "       A.医嘱ID,B.相关ID,A.发送号,A.首次时间 报到时间,A.发送时间 申请时间,A.执行状态,nvl(A.执行过程,0) 检查过程,A.执行间,A.结果阳性 阳性," & vbNewLine & _
                    ...
                    "       Decode(B.病人来源,3,B.开嘱医生,A.发送人) 登记人,H.报到人, H.报告发放,H.关联ID,A.记录性质, " & vbNewLine & _
        ...
        Set GetFilterData = zlDatabase.OpenSQLRecord(gstrSQL, "提取病人列表", .门诊号, .住院号, .就诊卡, .姓名, .身份证, _
                                            .IC卡, .单据号, .检查号, .开始时间, .结束时间, .病人科室, .标本部位, _
...
'    End If
    ...
    frmOpenStudyList.vsStudyList.Row = -1
    ...
    With lst
        Do Until rsList.EOF
            ...
            If Nvl(rsList!相关ID) = "" And ((mblncmd已缴 = True And blnCharged = True) Or (mblncmd未缴 = True And blnCharged = False) _
                ...
                End If
                .TextMatrix(.Row, GetCN("执行间")) = Nvl(rsList!执行间)
