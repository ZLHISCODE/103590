...
create or replace package b_PacsInterface is
  ...
  --
	---------------------------------------------------------------------------------------------------------------
  Procedure GetFinishedRequestInfo
  (
    Cursor_Out	Out	t_Refcur,
...
create or replace package body b_PacsInterface is
    ...
    End If;
     ...
     strSQL := 'Select  ''主费用'' As 费用类型,decode(A.记录性质,1,''收费单据'',''记帐单据'') As 单据类型,
                 ...
                 Where A.NO= ''' || v_单据号 || ''' And A.记录状态 IN(0,1,3) And A.医嘱序号+0=B.ID And A.记录性质= ' || v_记录性质
                       || ' And  c.医嘱ID=b.Id  And A.收费细目ID=E.Id and A.执行状态 <>9
          Union ALL
          Select  ''附加费用'' As 费用类型,decode(B.记录性质,1,''收费单据'',''记帐单据'') As 单据类型,
                 ...
                 Where A.NO=B.NO And A.记录性质=B.记录性质 And A.医嘱ID=B.医嘱序号+0
                       ...
                       v_记录性质 || ' And d.医嘱ID =c.Id  And B.收费细目ID=E.Id and B.执行状态 <>9 ';
  ...
  begin
    Open Cursor_Out For
         select b.id as 医嘱ID,d.检查号,a.姓名,a.性别,a.年龄,b.开嘱医生 as 申请人, e.名称 as 申请科室, 
                b.医嘱内容, f.名称 as 执行部门, d.影像类别,d.接收日期,d.报告人, d.完成人
         from 病人信息 a, 病人医嘱记录 b, 病人医嘱发送 c, 影像检查记录 d,部门表 e, 部门表 f
         where a.病人id = b.病人id and b.id =c.医嘱id and b.id=d.医嘱id and b.开嘱科室id=e.id 
               and b.执行科室id=f.id and b.相关id is null and c.执行过程=6 and a.病人ID=病人ID_IN;  
  Exception
    When Others Then
      Zl_Errorcenter(Sqlcode, Sqlerrm);    
  end GetFinishedRequestInfo; 
  ...
  Begin
    ...
    strSql :='Select /*+ RULE */
        ...
        Decode(m.病人来源, 1, ''门诊'', 2, ''住院'', 3, ''外来'', 4, ''体验'') 病人来源,
  ...
  Begin
    ...
    Zl_影像检查_Cancel(医嘱id_In, v_发送号,单独执行_In);
  ...
  Begin
    ...
    --处理费用
    Select 发送号 Into v_发送号 From 病人医嘱发送 Where 医嘱ID = 医嘱id_IN;
    zl_影像费用执行(医嘱id_IN, v_发送号, 2,单独执行_In,v_人员编号,v_人员姓名);
  ...
  begin
    ...
    if v_格式ID = 0 then
       ...
       select nvl(max(编号),0) + 1 into v_编号 from 病历文件列表;
       ...
       values(7, v_编号,v_格式名,'256;1;16840;12907;240;240;1587;1417;10070188;16777215;1');
    end if;
    ...
    --插入电子病历记录 
    insert into 电子病历记录(ID,病人来源,病人ID,主页ID,科室ID,病历种类,文件ID,病历名称,完成时间,创建人,创建时间)
      select v_病历ID,r_AdviceInf.病人来源,r_AdviceInf.病人ID,r_AdviceInf.主页ID,r_AdviceInf.开嘱科室ID,
             7,v_格式ID,'心电检查报告单',sysdate,报告医生_IN,sysdate 
      from 病人医嘱记录 
      where ID=医嘱ID_IN; 
