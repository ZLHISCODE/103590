...
begin
    ...
    IF (N_ExecState =3 AND N_ExecProcess = 2) THEN 
        ...
        IF N_RowCount=1 THEN
            update 病人医嘱发送 set 执行状态 = 0 where 医嘱ID = 医嘱ID_IN;
        ELSE
...
begin
    --只有符合以下条件的申请可以被恢复
    --1.原来正在进行，但是被取消的检查(医嘱执行状态=0，执行过程=2)
    --2.有相关联的影像检查记录
    BEGIN
        select 执行状态,执行过程 into N_ExecState, N_ExecProcess 
            from 病人医嘱发送 where 医嘱ID = 医嘱ID_IN;
    EXCEPTION
        WHEN No_Data_Found THEN 
    		  	...
    		  	Raise Err_Custom;
    END;
    IF N_ExecState =0 AND N_ExecProcess = 2 THEN 
        select Count(*) into N_RowCount from 影像检查记录 
            where 医嘱ID = 医嘱ID_IN;
        IF N_RowCount=1 THEN
...
--触发器部份
-------------------------------------------------------------------------------
create or replace trigger TBI_ZLPACS_病人病历记录_INSERT
  after insert on 病人病历记录
  for each row
declare
...
begin
       --查找书写医生编号
       select count(*) into N_RowCount from 人员表 A where a.姓名=:NEW.书写人;
       if N_RowCount =1 then
          select 编号 into N_WriteDoctorNo from 人员表 A where a.姓名=:NEW.书写人;
       else
          N_WriteDoctorNo:='9999';
       end if;
       --查找审阅医生编号
       select count(*) into N_RowCount from 人员表 A where a.姓名=:NEW.审阅人;
       if N_RowCount =1 then
          select 编号 into N_CheckDoctorNo from 人员表 A where a.姓名=:NEW.审阅人;
       else
       ...
       --插入临时病人病历记录表
       insert into PACS_tmp病人病历记录(id,报告id,病人ID,科室ID,病历名称,书写人ID,书写人,
              书写日期,审阅人ID,审阅人,审阅日期,记录类型)
              values(N_ID,:NEW.id,:NEW.病人ID,:NEW.科室ID,:NEW.病历名称,N_WriteDoctorNo,
              :NEW.书写人,:NEW.书写日期,N_CheckDoctorNo,:NEW.审阅人,:NEW.审阅日期,1);
             
exception
       when others then 
            null;  
end TBI_ZLPACS_病人病历记录_INSERT;
/

----------------------------

create or replace trigger TBI_ZLPACS_病人病历记录_UPDATE
  after update on 病人病历记录  
  for each row
declare
      -- local variables here
      N_RowCount Number;
...
begin
      ...
      insert into PACS_tmp病人病历记录(id,报告id,病人ID,科室ID,病历名称,书写人ID,书写人,
             书写日期,审阅人ID,审阅人,审阅日期,记录类型)
             values(N_ID,:NEW.id,:NEW.病人ID,:NEW.科室ID,:NEW.病历名称,N_WriteDoctorNo,
             :NEW.书写人,:NEW.书写日期,N_CheckDoctorNo,:NEW.审阅人,:NEW.审阅日期,2);
exception
       when others then 
            null; 
end TBI_ZLPACS_病人病历记录_UPDATE;
/
