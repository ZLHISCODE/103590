create or replace package b_PacsInterface is
  ...
  --
	---------------------------------------------------------------------------------------------------------------
  Procedure GetUnPrintedStudyInfo
  (
    Cursor_Out	Out	t_Refcur,
		开始日期_In	In	Varchar2,
		结束日期_In	In	Varchar2
...
create or replace package body b_PacsInterface is
  ...
  begin
    open cursor_out for
         select /*+ RULE*/ c.姓名,c.性别,c.检查号, a.开嘱医生 as 申请医生, d.名称 as 申请科室, b.报到时间, a.医嘱内容, c.影像类别, b.执行状态, b.执行过程 as 检查状态, c.报告人, c.完成人,c.报告打印,To_Number(a.id) as 医嘱ID,To_Number(a.病人id) as 病人ID 
         from 病人医嘱记录 a, 病人医嘱发送 b, 影像检查记录 c, 部门表 d 
         where a.id = b.医嘱id and a.id = c.医嘱id and a.开嘱科室id = d.id and c.报告打印 is null and nvl(b.报到时间, sysdate) between 开始日期_In and 结束日期_In order by 检查号;
  Exception
    When Others Then
      Zl_Errorcenter(Sqlcode, Sqlerrm);        
  end GetUnPrintedStudyInfo;   
