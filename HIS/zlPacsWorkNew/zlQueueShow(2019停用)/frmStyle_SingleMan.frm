...
On Error GoTo errHandle
    ...
    Call LoadCallingData(mlngLastCallQueueId)
    'Call SetStyleFont
...
Private Sub LoadCallingData(Optional ByVal lngQueueId As Long)
'加载处于呼叫中的数据
    Dim strSql As String
    Dim rsRecord As ADODB.Recordset
...
On Error GoTo ErrorHand:
    ...
    If rsRecord.RecordCount <= 0 Then rsRecord.Filter = "排队状态=9"    '待呼叫
    If rsRecord.RecordCount <= 0 Then rsRecord.Filter = "排队状态=1"    '呼叫中
    If rsRecord.RecordCount <= 0 Then rsRecord.Filter = "排队状态=7"    '已呼叫
    If rsRecord.RecordCount <= 0 Then rsRecord.Filter = "排队状态=8"    '接诊中
    If rsRecord.RecordCount <= 0 Then Exit Sub
    
    rsRecord.Sort = "呼叫时间 desc"
    lblPatientInfo.Caption = Format(Nvl(rsRecord!排队号码), "000") & "号   " & GetSecName(Nvl(rsRecord!患者姓名)) & IIf(Len(Trim(Nvl(rsRecord!患者姓名))) <= 3, "   ", "")
