...
Private mlngLastCallQueueId As Long
...
On Error GoTo errHandle
    mlngInterval = 0
    tmrRefreshInterval.Enabled = False
    
    If lngQueueId <> 0 Then mlngLastCallQueueId = lngQueueId
    
    Call LoadListData(mlngLastCallQueueId)
    Call SetStyleFont
    
    '数据刷新后将计时器清0
    mlngInterval = 0
    If Not gblnUseSound Then tmrRefreshInterval.Enabled = True
Exit Sub
errHandle:
    If Not gblnUseSound Then tmrRefreshInterval.Enabled = True
End Sub
...
'加载排队呼叫列表数据
    ...
    Dim lngWaitCount As Long
    '该过程如需兼容临床排队状态显示，则需要根据对应状态进行处理
...
On Error GoTo ErrorHand:
    ...
    Set rsRecord = gobjComLib.zlDatabase.OpenSQLRecord(strSql, "获取排序方式", glngBusinessType) 'glngBusinessType
    ...
    '如果启用了本地语音呼叫，喇叭图标处则根据队列ID只显示最后一次呼叫的信息
    '如果没有启用语音呼叫，则检索排队状态为1的呼叫数据进行显示
    If gblnUseSound Then
        If lngQueueId <> 0 Then
            rsRecord.Filter = "id=" & lngQueueId
        Else
            rsRecord.Filter = "id=-1"
        End If
    Else
        rsRecord.Filter = "排队状态=1"  '新的排队处理1表示呼叫中，临床中1目前表示呼叫
        '显示最近呼叫的数据
        rsRecord.Sort = "呼叫时间 desc"  '如果是门诊系统呼叫，使用新版显示方式，在没有使用新版排队控件时，这里需要调整为根据排队语音呼叫表中的id排序
    End If
  
    '加载当前呼叫信息到界面
    If rsRecord.RecordCount > 0 Then
        lblPatientName.Caption = Format(Nvl(rsRecord!排队号码), "000") & "号 " & GetSecName(Nvl(rsRecord!患者姓名))
    ...
    '数据查询时，已经默认根据“Zl_排队叫号队列_获取排序方式”过程中的排序方式进行了数据检索
    rsRecord.Sort = ""
    vsfCallingList.Cell(flexcpText, 0, 0, vsfCallingList.Rows - 1, vsfCallingList.Cols - 1) = ""
    
    '根据列表中对应的队列信息，加载该队列下对应的数据
    'strQueryQueueNames中已经预先将队列信息进行了读取
    For i = 1 To vsfQueueList.Rows - 1
        ...
        '排队状态=6 已预呼叫(已预呼和待预呼数据需要优先加载处理)
        If strTemp = "" Then
            rsRecord.Filter = "队列名称='" & Split(mLcdCommonParameter.strQueryQueueNames, ",")(i - 1) & "' And 排队状态=6"
        Else
            rsRecord.Filter = "队列名称='" & Split(mLcdCommonParameter.strQueryQueueNames, ",")(i - 1) & "' And 诊室='" & strTemp & "' And 排队状态=6"
        End If
        
'        rsRecord.Sort = ""
        
        For j = 0 To rsRecord.RecordCount - 1
            strQueuePatients = strQueuePatients & ", " & Format(Nvl(rsRecord!排队号码), "000") & "-" & GetSecName(Nvl(rsRecord!患者姓名)) & IIf(Nvl(rsRecord!备注) <> "", "(" & Nvl(rsRecord!备注) & ")", "")
            If j = mtpPageObj.tplngQueueListShowNum - 1 Then Exit For '每个队列显示准备就诊数据
            rsRecord.MoveNext
        Next
        
        lngWaitCount = lngWaitCount + rsRecord.RecordCount
        
        
        '排队状态=5 待预呼(已预呼和待预呼数据需要优先加载处理)
        If strTemp = "" Then
            rsRecord.Filter = "队列名称='" & Split(mLcdCommonParameter.strQueryQueueNames, ",")(i - 1) & "' And 排队状态=5"
        Else
            rsRecord.Filter = "队列名称='" & Split(mLcdCommonParameter.strQueryQueueNames, ",")(i - 1) & "' And 诊室='" & strTemp & "' And 排队状态=5"
        End If
...
'        rsRecord.Sort = ""
        ...
        If strQueuePatients <> "" Then
            vsfQueueList.TextMatrix(i, 1) = Mid(strQueuePatients, 2)
        Else
            vsfQueueList.TextMatrix(i, 1) = ""
        End If
        
        vsfQueueList.TextMatrix(i, 2) = "共 " & lngWaitCount & "人"
        
        '加载呼叫列表数据,并显示准备中
        If strTemp = "" Then
            rsRecord.Filter = "队列名称='" & Split(mLcdCommonParameter.strQueryQueueNames, ",")(i - 1) & "' And 排队状态=9" '9表示待呼叫，临床无对应
            If rsRecord.RecordCount <= 0 Then rsRecord.Filter = "队列名称='" & Split(mLcdCommonParameter.strQueryQueueNames, ",")(i - 1) & "' And 排队状态=1"
...
'            If rsClone.RecordCount <= 0 Then rsClone.Filter = "队列名称='" & Split(mLcdCommonParameter.strQueryQueueNames, ",")(i - 1) & "' And 排队状态=8"
        Else
            rsRecord.Filter = "队列名称='" & Split(mLcdCommonParameter.strQueryQueueNames, ",")(i - 1) & "' And 诊室='" & strTemp & "' And 排队状态=9" '9表示待呼叫，临床无对应
            If rsRecord.RecordCount <= 0 Then rsRecord.Filter = "队列名称='" & Split(mLcdCommonParameter.strQueryQueueNames, ",")(i - 1) & "' And 诊室='" & strTemp & "' And 排队状态=1"
...
'            If rsClone.RecordCount <= 0 Then rsClone.Filter = "队列名称='" & Split(mLcdCommonParameter.strQueryQueueNames, ",")(i - 1) & "' And 诊室='" & strTemp & "' And 排队状态=8"
        ...
        If rsRecord.RecordCount > 0 Then
            ...
            If vsfCallingList.Cols = 1 Then
                If glngBusinessType = TBusinessType.btClinical Then
                    vsfCallingList.TextMatrix(k - 1, 0) = "●请 " & Format(Nvl(rsRecord!排队号码), "000") & "号 " & GetSecName(Nvl(rsRecord!患者姓名)) & " " & IIf(Nvl(rsRecord!诊室) = "", Nvl(rsRecord!名称), Nvl(rsRecord!诊室)) & " 准备 "
                Else
                    vsfCallingList.TextMatrix(k - 1, 0) = "●请 " & Format(Nvl(rsRecord!排队号码), "000") & "号 " & GetSecName(Nvl(rsRecord!患者姓名)) & " " & Nvl(rsRecord!诊室) & " 准备 "
                End If
            Else
                If glngBusinessType = TBusinessType.btClinical Then
                    vsfCallingList.TextMatrix(lngRow, lngCol) = "●请 " & Format(Nvl(rsRecord!排队号码), "000") & "号 " & GetSecName(Nvl(rsRecord!患者姓名)) & " " & IIf(Nvl(rsRecord!诊室) = "", Nvl(rsRecord!名称), Nvl(rsRecord!诊室)) & " 准备 "
                Else
                    vsfCallingList.TextMatrix(lngRow, lngCol) = "●请 " & Format(Nvl(rsRecord!排队号码), "000") & "号 " & GetSecName(Nvl(rsRecord!患者姓名)) & " " & Nvl(rsRecord!诊室) & " 准备 "
                End If
    ...
    If mLcdCommonParameter.blnScrollDisplay Then
        For i = 1 To vsfQueueList.Rows - 1
            ...
            If strTemp = "" Then
                '7表示已呼叫，在医生站中可能不存在7的状态，则读取1的数据，1可表示已呼叫
            ...
            If rsRecord.RecordCount > 0 Then
                ...
                For j = 0 To rsRecord.RecordCount - 1
                    If glngBusinessType = TBusinessType.btClinical Then
                        lblCallContext.Caption = lblCallContext.Caption & " ●" & Format(Nvl(rsRecord!排队号码), "000") & "号 " & GetSecName(Nvl(rsRecord!患者姓名)) & " " & IIf(Nvl(rsRecord!诊室) = "", Nvl(rsRecord!名称), Nvl(rsRecord!诊室)) & " 就诊"
                    Else
                        lblCallContext.Caption = lblCallContext.Caption & " ●" & Format(Nvl(rsRecord!排队号码), "000") & "号 " & GetSecName(Nvl(rsRecord!患者姓名)) & " " & Nvl(rsRecord!诊室) & " 就诊"
                    End If
...
On Error GoTo ErrorHand
    ...
    If mlngInterval < glngLoopInterval Then Exit Sub
    '将累计的时间清0
    mlngInterval = 0
