...
'end sub
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Private Const M_LNG_准备就诊显示行 = 0  '排队列表中，准备就诊数据显示行数
...
On Error GoTo errHandle
    ...
    Call LoadCallingData(blnExist就诊, mlngLastCallQueueId)
...
'设置界面各控件字体属性
    ...
    Dim strFontPropertys5 As String
    ...
    Dim strFontProperty5() As String
...
On Error GoTo ErrorHand
   ...
   '备注内容
    ...
    strFontPropertys3 = Trim(ReadValue("字体设置", "准备就诊状态行字体", "字体:宋体|字号:20|粗体:FALSE|前景色:260872"))
    strFontPropertys4 = Trim(ReadValue("字体设置", "候诊状态行字体", "字体:宋体|字号:20|粗体:FALSE|前景色:16777215"))
    strFontPropertys5 = Trim(ReadValue("字体设置", "呼叫状态行字体", "字体:宋体|字号:20|粗体:FALSE|前景色:65535|背景色:11570257"))
    ...
    strFontProperty5 = Split(strFontPropertys5, "|")
    
    SetVSFListFont vsfCallingData, 0, strFontProperty1
    
    For i = IIf(mtpPageObj.tpblnShowListHeader, 1, 0) To vsfCallingData.Rows - 1
        If InStr(vsfCallingData.TextMatrix(i, 3), "准备") > 0 Then
            SetVSFListFont vsfCallingData, i, strFontProperty3
        ElseIf InStr(vsfCallingData.TextMatrix(i, 3), "就诊") > 0 Or InStr(vsfCallingData.TextMatrix(i, 3), "接诊") > 0 Then
        ...
        If InStr(vsfCallingData.TextMatrix(i, 0), "呼叫") > 0 Then
            'vsfCallingData.Cell(flexcpBackColor, i, 0, i, vsfCallingData.Cols - 1) = &HB08C51  ' &HAB884E '&HBEA070'&H9F7E48    '
            SetVSFListFont vsfCallingData, i, strFontProperty5
        End If
    Next
...
Private Sub LoadCallingData(ByRef blnExist就诊 As Boolean, Optional ByVal lngQueueId As Long)
'加载排队列表数据
...
'    Set rsClone = rsRecord.Clone
    ...
    If mLcdCommonParameter.blnScrollDisplay Then        '获取处于“呼叫中”和“已呼叫”的数据
        rsRecord.Filter = "排队状态=7"  '7表示已呼叫
'        rsClone.Filter = "排队状态=1"  ’1主要是用于兼容临床，但临床如果使用新的排队显示呼叫，则呼叫完成后，状态也会变更为7
        ...
        If rsRecord.RecordCount > 0 Then
            ...
            For i = 0 To IIf(rsRecord.RecordCount > 3, 3, rsRecord.RecordCount - 1)
                If glngBusinessType = TBusinessType.btClinical Then
                    lblCallContext.Caption = lblCallContext.Caption & " ●" & Format(Nvl(rsRecord!排队号码), "000") & "号 " & GetSecName(Nvl(rsRecord!患者姓名)) & " 到 " & IIf(Nvl(rsRecord!诊室) = "", Nvl(rsRecord!名称), Nvl(rsRecord!诊室)) & " 就诊"
                Else
                    lblCallContext.Caption = lblCallContext.Caption & " ●" & Format(Nvl(rsRecord!排队号码), "000") & "号 " & GetSecName(Nvl(rsRecord!患者姓名)) & " 到 " & Nvl(rsRecord!诊室) & " 就诊"
                End If
                rsRecord.MoveNext
            Next
        End If
    ...
    With vsfCallingData
        ...
        If rsRecord.RecordCount <= 0 Then rsRecord.Filter = "排队状态=1"    '1表示呼叫中，临床1表示呼叫
        
        If rsRecord.RecordCount > 0 Then
            .TextMatrix(lngRow, 0) = "呼叫"
            .TextMatrix(lngRow, 1) = Format(rsRecord!排队号码, "000") & "号"
            .TextMatrix(lngRow, 2) = GetSecName(Nvl(rsRecord!患者姓名))
        ...
        If rsRecord.RecordCount > 0 Then
            ...
            If mblnShowCallTarget Then
                If glngBusinessType = TBusinessType.btClinical Then
                    .TextMatrix(lngRow, 3) = "准备(" & IIf(Nvl(rsRecord!诊室) = "", Nvl(rsRecord!名称), Nvl(rsRecord!诊室)) & ")"
                Else
                    .TextMatrix(lngRow, 3) = "准备(" & Nvl(rsRecord!诊室) & ")"
                End If
            Else
                .TextMatrix(lngRow, 3) = "准备"
            End If
...
'        End If
        ...
        If rsRecord.RecordCount <= 0 Then
            rsRecord.Filter = "排队状态=8"
            
            If rsRecord.RecordCount > 0 Then
                rsRecord.Sort = "呼叫时间 desc"
    
                .TextMatrix(lngRow, 0) = "当前"
                .TextMatrix(lngRow, 1) = Format(rsRecord!排队号码, "000") & "号"
                .TextMatrix(lngRow, 2) = GetSecName(Nvl(rsRecord!患者姓名))
                
                If mblnShowCallTarget Then
                    If glngBusinessType = TBusinessType.btClinical Then
                        .TextMatrix(lngRow, 3) = "接诊(" & IIf(Nvl(rsRecord!诊室) = "", Nvl(rsRecord!名称), Nvl(rsRecord!诊室)) & ")"
                    Else
                        .TextMatrix(lngRow, 3) = "接诊(" & Nvl(rsRecord!诊室) & ")"
                    End If
                Else
                    .TextMatrix(lngRow, 3) = "接诊"
                End If
        ...
        rsRecord.Filter = "排队状态=5 or 排队状态=6"
...
'                    .TextMatrix(lngRow, 3) = "准备就诊" & strWaitCallWord
'                Else
'                    .TextMatrix(lngRow, 3) = "候诊" & strWaitCallWord
'                End If
'            Else
