...
Type
  ...
  public
    ...
    class function ConnectFilters(Graph: IGraphBuilder;
      Src: IBaseFilter; Dst: IBaseFilter; IsSmartTee: Boolean; smtPinType: integer): HResult;

    //配置分辨率和色深
    class function ConfigCaptureScale(Filter: IBaseFilter;
      Width, Height, Bits, frame: Integer): HResult;
    ...
    try
      ...
      if smtPinType = 0 then begin
        for i := 0 to pins.Count - 1 do begin
          ...
          ShowMessage(Pchar('Smart:' + IntToStr(i) + ' Total' + IntToStr( pins.Count )));
          if found then pinOut := pins[i];

          found := True;
        end;
      end else begin
        pinOut := pins[pins.Count - 1];
        found := True;
      end;
    finally
      FreeAndNil(pins);
...
class function TDS9Ex.ConfigCaptureScale(Filter: IBaseFilter; Width, Height, Bits, frame: Integer): HResult;
var
  pin: IPin;
  ...
  try
    ...
    try
      ...
      if frame > 0 then begin
        //设置采集速率
        pvih.AvgTimePerFrame :=round( 10000000 / frame);
        pvih.dwBitRate :=round( pvih.bmiHeader.biSize * dword(frame)) * 8;
      end;

      //pmt.subtype := MEDIASUBTYPE_RGB24; //如果没有指定，则自动匹配颜色空间

      //pmt.lSampleSize := pvih.bmiHeader.biSize;
