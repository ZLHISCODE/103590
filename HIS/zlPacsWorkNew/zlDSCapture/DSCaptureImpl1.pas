...
HISTORY
2018-07：处理部分OkC20采集卡或9158虚拟视频升级到4.6后，只能预览不能采集的问题；
...
uses
  ...
  ExtCtrls, jpeg, ComCtrls, DSUtil, CapParameterCfg, CaptureDebug;
...
type
  ...
  private
    ...
    //2018-07-13将SmartTee调整为InfTee，如下
  ...
  protected
    ...
    procedure CloseTiming; safecall;
    procedure StartTiming(IsSysTime: WordBool); safecall;
    function Get_CurTiming: Integer; safecall;
  public
    { Public declarations }
...
uses
  ComObj, ComServ, GraphicProcess, Types, DirectShow9Ex, DateUtils,
  FullScreenWindow, Clipbrd, DSCapParameterConfigObj, Math;
...
const
  ...
  FILTER_NAME_NULLRENDER: WideString = 'NULLRENDER>CBC32CB1F1AB444168165E11E1816D54';
...
begin
  ...
  CaptureSource>>Signal Pin ---> Smart Tee                             /Capture Pin --->连接图像采集相关Filter
                                           \Preview Pin  --->Smart Tee1
                                                   ...
                                                   2018-07-13 调整Filter连接
                                           /Capture Pin  --->连接录像相关Filter
  CaptureSource>>Signal Pin ---> Smart Tee                          |output- Color Convert --> SampleGrabber Pin --->连接图像采集相关Filter  --> Null Render
                                           \Preview Pin  --->INFTEE |output- Render Viewer --->连接视频预览相关Filter
                                                                    |output- ...
  ...
  try
    ...
    //PX1000E Begin
    //2018-07-13将SmartTee调整为InfTee 因为InfTee性能要高一些，没有时间戳处理，但没有针对天朗采集进行测试
    hr := TDS9Ex.AddFilterToGraphBuilder(_FilterGraphic as IGraphBuilder, CLSID_INFTEE{CLSID_SmartTee}, _InfTee, FILTER_NAME_INFINITE_PIN);
    if Failed(hr) then begin
      errMsg := '创建InfTeeFilter分流接口,并加入到FilterGraphic中时失败。[错误代码:' + IntToStr(hr) + ']';
      Exit;
    end;

    TDebug.OutputDebug('CAP>>>Preview Step 12');
    hr := TDS9Ex.AddFilterToGraphBuilder(_FilterGraphic as IGraphBuilder, CLSID_NullRenderer {CLSID_SmartTee}, _nullRender, FILTER_NAME_NULLRENDER);
    if Failed(hr) then begin
      errMsg := '创建NullRendererFilter接口,并加入到FilterGraphic中时失败。[错误代码:' + IntToStr(hr) + ']';
      Exit;
    end;
    ...
    //连接CapSourceFilter和(SmartTee或InfTee), 如果使用RenderStream方法，将会再添加一个smartTee接口
    //该方法将连接采集pin
    //hr := TDS9Ex.ConnectFilters(_FilterGraphic as IGraphBuilder, _CapSourceFilter, _SmartTee, False);
    hr := TDS9Ex.ConnectFilters(_FilterGraphic as IGraphBuilder, _SmartTee, _InfTee, True, 1);
    if Failed(hr) then begin
      errMsg := '创建SmartTeeFilter与InfTeeFilter之间的连接时失败。[错误代码:' + IntToStr(hr) + ']';
      Exit;
    end;
    ...
    if isCaptureVideo then begin
      ...
      TDebug.OutputDebug('CAP>>>Preview Step 15.1');

      //创建视频编码器接口
      _EncoderFilter := nil;
      if _CaptureParameter.EncoderName <> '' then begin

        TDebug.OutputDebug('CAP>>>Preview Step 15.2');
        ...
        TDebug.OutputDebug('CAP>>>Preview Step 15.3');
      ...
      TDebug.OutputDebug('CAP>>>Preview Step 15.4');
      //创建MULTIPLEXER多路复用接口
      _AviMultiplexer := nil;
      ...
      TDebug.OutputDebug('CAP>>>Preview Step 15.5');
      //创建文件写入接口
      _AviWriter := nil;
      ...
      TDebug.OutputDebug('CAP>>>Preview Step 15.6');
      //查询视频文件设置接口
      fs := nil;
      ...
      TDebug.OutputDebug('CAP>>>Preview Step 15.7');
      //设置视频文件路径
      ...
      TDebug.OutputDebug('CAP>>>Preview Step 15.8');

      if _EncoderFilter <> nil then begin
        TDebug.OutputDebug('CAP>>>Preview Step 15.9');
        ...
        TDebug.OutputDebug('CAP>>>Preview Step 15.10');
        ...
        TDebug.OutputDebug('CAP>>>Preview Step 15.11');

        hr := TDS9Ex.ConnectFilters(_FilterGraphic as IGraphBuilder, _SmartTee, _AviMultiplexer, false, 0);
        if Failed(hr) then begin
          errMsg := '创建EncoderFilter与AviMultiplexerFilter之间的连时失败，请检查编码器设置。[错误代码:' + IntToStr(hr) + ']';
          Exit;
        end;
      ...
      TDebug.OutputDebug('CAP>>>Preview Step 15.12');
      ...
      TDebug.OutputDebug('CAP>>>Preview Step 15.13');
    ...
    if _CaptureParameter.SnatchWay = swVMR then begin
      TDebug.OutputDebug('CAP>>>Preview Step 16.1.1');
      ...
      TDebug.OutputDebug('CAP>>>Preview Step 16.1.2');
    end else begin
      TDebug.OutputDebug('CAP>>>Preview Step 16.2.1');
      _VideoWindow.Mode := vmNormal;
      _VideoWindow.VMROptions.Mode := vmrWindowed;

      _VideoWindow.FilterGraph := _FilterGraphic;
      TDebug.OutputDebug('CAP>>>Preview Step 16.2.2');
    end;
    ...
    hr := TDS9Ex.ConnectFilters(_FilterGraphic as IGraphBuilder, _InfTee, _ColorSpace, false, 0);
    if Failed(hr) then begin
    ...
    hr := TDS9Ex.ConnectFilters(_FilterGraphic as IGraphBuilder, _ImgCaptureFilter as IBaseFilter, _nullRender, false, 0);
    if Failed(hr) then begin
      errMsg := '创建ImgCaptureFilter与nullRender之间的连接时失败。[错误代码:' + IntToStr(hr) + ']';
      Exit;
    end;
    ...
    //2018-07-19尝试使用RenderStream进行智能连接
    //hr := _GraphManager.RenderStream(nil, nil, _infTee, nil, _VideoWindow as IBaseFilter);
    hr := TDS9Ex.ConnectFilters(_FilterGraphic as IGraphBuilder, _InfTee, _VideoWindow as IBaseFilter, false, 0);
    if Failed(hr) then begin
      errMsg := '创建InfTeeFilter与VideoWindow之间的连接时失败。[错误代码:' + IntToStr(hr) + ']';
      Exit;
    end; //}
    ...
    TDebug.OutputDebug('CAP>>>Preview Step 24');
    imgLogo.Visible := False;

    //调整窗口位置
    AdjustWindowSize();

    TDebug.OutputDebug('CAP>>>Preview Step 25');
    try
...
begin
  ...
  TDS9Ex.ConfigCaptureScale(curBaseFilter, curVideoSize.Width, curVideoSize.Height, captureParameter.ColorDepth, _otherPar.Frame);
end;
...
begin
  try
    if _FilterGraphic.Active then begin
      with _FilterGraphic as IMediaSeeking do begin
...
procedure TDSCapture.timerSysTimer(Sender: TObject);
  ...
  begin
    ...
    hdcDraw :=  GetDC(paneTime.Handle);

    try
      if _TimeCalc = -1 then begin
        DrawText(hdcDraw, pAnsichar(FormatDateTime(' HH:MM:SS', Now)), 9, nr, 0);
      end else begin
        curTime := IncMilliSecond(StrToDateTime('00:00:00'), GetTickCount - _TimeCalc );
        DrawText(hdcDraw, pAnsichar(FormatDateTime(' HH:MM:SS', curTime)), 9, nr, 0);
      end;

    finally
      ReleaseDC(paneTime.Handle , hdcDraw );
    end;
  end;

begin
  try
    if paneTime.Visible then begin
      paneTime.BringToFront;
      DrawTime();
    end;
...
begin
  ...
  paneTime.Left := _VideoWindow.Left + 5;
  paneTime.top := _VideoWindow.top + 5;

  if self.Left < 0 then  paneTime.Left := paneTime.Left + ABS(self.Left);
  if self.Top < 0 then  paneTime.top := paneTime.top + ABS(self.Top);
...
var
  ...
  procedure DrawTime();
  const CONTROL_BORDER_SPACE: Integer = 2;
  var
    nR: TRect;
    hdcDraw: HDC;

    stateBarHeight: Longint;
    zoomRate: Double;
    curVideoSizeInf: TVideoSize;
    curTime: TDateTime;
  begin
    ...
    //取得缩放比率
    if (curVideoSizeInf.Height) / curVideoSizeInf.Width > (Self.Height - stateBarHeight) / (Self.Width) then begin
      zoomRate := curVideoSizeInf.Height / (Self.Height - stateBarHeight) ;
    end else begin
      zoomRate := curVideoSizeInf.Width / Self.Width;
    end;

    if _VideoWindow.Left > 0 then begin
      nR.Left :=5;
    end else begin
      nR.Left :=5+ round((paneTime.Left-5) * zoomRate);
    end;

    if _VideoWindow.Top > 0 then begin
      nR.Top :=5;
    end else begin
      nR.Top :=5+ round((paneTime.Top-5) * zoomRate);
    end;



    curBitMap.Canvas.Font.Style := [fsBold];


    if curVideoSizeInf.Width <= 320 then begin
      curBitMap.Canvas.Font.Size := 9;
      nR.Right := nR.Left + 65;
      nR.Bottom := nR.Top + 15;
    end else if curVideoSizeInf.Width < 640 then begin
      curBitMap.Canvas.Font.Size := 11;
      nR.Right := nR.Left + 80;
      nR.Bottom := nR.Top + 25;
    end else if curVideoSizeInf.Width < 1280 then begin
      curBitMap.Canvas.Font.Size := 14;
      nR.Right := nR.Left + 90;
      nR.Bottom := nR.Top + 30;
    end else if curVideoSizeInf.Width < 1920 then begin
      curBitMap.Canvas.Font.Size := 16;
      nR.Right := nR.Left + 95;
      nR.Bottom := nR.Top + 35;
    end else begin
      curBitMap.Canvas.Font.Size := 18;
      nR.Right := nR.Left + 110;
      nR.Bottom := nR.Top + 45;
    end;            //}


    hdcDraw :=curBitMap.Canvas.Handle; //  GetDC(paneTime.Handle);
    try
      if _TimeCalc = -1 then begin
        DrawText(hdcDraw, pAnsichar(FormatDateTime(' HH:MM:SS ', Now)), 10, nr, 0);
      end else begin
        curTime := IncMilliSecond(strtodatetime('00:00:00'), GetTickCount - _TimeCalc );
        DrawText(hdcDraw, pAnsichar(FormatDateTime(' HH:MM:SS ', curTime)),  10, nr, 0);
      end;
                  
    finally
      ReleaseDC(curBitMap.Handle , hdcDraw );
    end;
  end;
  ...
  var
    ...
    filestream:TMemoryStream;  //DELPHI的文件流类型变量
  ...
  try
    ...
    TDebug.OutputDebug('CAP>>>GetBuffer-Step 1:Query Interface.');

    _FilterGraphic.QueryInterface(IID_IMediaControl,MediaControl);
    SampleGrabber := _ImgCaptureFilter.SampleGrabber;

    TDebug.OutputDebug('CAP>>>GetBuffer-Step 2:Get Media Type.');
    ...
    TDebug.OutputDebug('CAP>>>GetBuffer-Step 3:Get Buffer Size.');
    ...
    try
      ...
      if  (ptype.pbFormat<>nil)
        and (IsEqualGUID(ptype.formattype,FORMAT_VideoInfo)) then //判断Sampler Grabber中的输入Pin上的类型是否满足我们的要求
          begin

            TDebug.OutputDebug('CAP>>>GetBuffer-Step 3.1:Config Bmp Format.');
            //=设置bmp Head部分=//
            with bmphead do
            begin
              bftype:=$4d42;
           ...
           if catchcolor <> fillcolor then
            ...
            TDebug.OutputDebug('CAP>>>GetBuffer-Step 3.2:Read Buffer To Memory Stream.');

            filestream:=TMemoryStream.Create; //创建TMemoryStream视频流
            ...
            TDebug.OutputDebug('CAP>>>GetBuffer-Step 3.3:Return Memory Stream.');
    ...
    TDebug.OutputDebug('CAP>>>GetBuffer-Step 4:End Process.');
...
begin
  ...
  try
      ...
      TDebug.OutputDebug('CAP>>>Capimage-Step 1:Use SampleGrabber Capture Image.');

      if {(_CaptureParameter.SnatchWay = swDEVICE) and} _ImgCaptureFilter.GetBitmap(curBitMap) then begin
        if (curBitMap.Width > 0) then begin

          TDebug.OutputDebug('CAP>>>Capimage-Step 1.1:Set Pixel Format.');
          
          case _CaptureParameter.colorDepth of
            4: curBitMap.PixelFormat := pf4bit;
            8: curBitMap.PixelFormat := pf8bit;
            12: curBitMap.PixelFormat := pf24bit;//pfDevice;
            16: curBitMap.PixelFormat := pf24bit;//pf16bit;
            24: curBitMap.PixelFormat := pf24bit;
            32: curBitMap.PixelFormat := pf32bit;
          end;
        
          //if curBitMap.Width <= 0 then ShowMessage('SampleGrabber采集失败！');
          //为避免发音时，占用过程时间，因此使用postmessage方法触发声音提示
          if _CaptureParameter.IsSoundHint then PostMessage(Self.Handle, WM_BEEPMSG, 0, 0);

          if paneTime.Visible then DrawTime();

          Result := curBitMap;

          TDebug.OutputDebug('CAP>>>Capimage-Step 1.2:Return Bmp.');
          
          Exit;
        end;
      end;

      TDebug.OutputDebug('CAP>>>Capimage-Step 2:Use VideoWindow Capture Image.');
      //当使用SampleGrabber不能采集到图像时，则直接使用VideoWindow的VMRGetBitmap方法采集图像
      bmpStream := TMemoryStream.Create;
      try
        if  _VideoWindow.VMRGetBitmap(bmpStream)  then begin

          if bmpStream.Size > 0 then Begin

            TDebug.OutputDebug('CAP>>>Capimage-Step 2.1:Load Bmp Stream.');
            curBitMap.LoadFromStream(bmpStream);

            TDebug.OutputDebug('CAP>>>Capimage-Step 2.2:Set Pixel Format.');
            case _CaptureParameter.colorDepth of
              4: curBitMap.PixelFormat := pf4bit;
              8: curBitMap.PixelFormat := pf8bit;
              12: curBitMap.PixelFormat := pf24bit;//pfDevice;
            ...
            TDebug.OutputDebug('CAP>>>Capimage-Step 2.3:Return Bmp.');
      ...
      TDebug.OutputDebug('CAP>>>Capimage-Step 3:Use Buffer Capture Image.');
      //当使用SampleGrabber和VideoWindow都不能获取图像时，则直接读取缓存数据
      bmpStream := GetBuffer();
      try
        if Assigned(bmpStream) then begin

          TDebug.OutputDebug('CAP>>>Capimage-Step 3.1:Load Bmp Stream.');
          
          bmpStream.Position :=0;
          curBitMap.LoadFromStream(bmpStream);

          TDebug.OutputDebug('CAP>>>Capimage-Step 3.2:Set Pixel Format.');
          ...
          TDebug.OutputDebug('CAP>>>Capimage-Step 3.3:Return Bmp.');
    ...
    on e: Exception do begin
      ...
      TDebug.DebugMsg('TDSCapture', 'CaptureImageToBmpObj', e.Message);
    end;
  end;
end;
...
begin
  try
    Result := '';
    TfrmCapParameterCfg.ReadCaptureParameterFromFile(_CaptureParameterCfgFileName, _CaptureParameter, _otherPar);
...
procedure TDSCapture.CloseTiming;
begin
  paneTime.Visible := false;
  _TimeCalc := -1;
end;

procedure TDSCapture.StartTiming(IsSysTime: WordBool);
begin
  _TimeCalc := GetTickCount;
  if IsSysTime then _TimeCalc := -1;

  paneTime.BringToFront;
  paneTime.Visible := True;
end;

function TDSCapture.Get_CurTiming: Integer;
begin
  Result := GetTickCount - _TimeCalc;
end;
