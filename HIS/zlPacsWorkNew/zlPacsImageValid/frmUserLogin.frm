...
Private Sub CMD确认_Click()
    ...
    Set gcnOracle = OraDataOpen(strServerName, strUserName, strPassword)
    ...
    If gcnOracle Is Nothing Then
        ...
        Exit Sub
    Else
        SaveSetting "ZLSOFT", "公共模块\" & App.EXEName, "User", Trim(txt用户.Text)
        SaveSetting "ZLSOFT", "公共模块\" & App.EXEName, "Adress", Trim(cmb数据库.Text)
    End If
...
Private Sub Form_Load()
    ...
    txt用户.Text = GetSetting("ZLSOFT", "公共模块\" & App.EXEName, "USER")
    strAdress = GetSetting("ZLSOFT", "公共模块\" & App.EXEName, "Adress")
    
    For i = 0 To cmb数据库.ListCount
        If cmb数据库.List(i) = strAdress Then
            blnOk = True
            Exit For
        End If
    Next
    
    If blnOk Then cmb数据库.Text = strAdress
    mbln转换 = True
    mblnFirst = False
    mintTimes = 1
