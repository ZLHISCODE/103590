...
Private Sub cmdOK_Click()
    ...
    lng记录ID = Val(mshPlan.TextMatrix(mshPlan.Row, GetPlanCol("记录ID")))
    ...
    strSQL = "Select 开始时间,终止时间" & vbNewLine & _
            ...
            "       From 临床出诊记录 A, 时间段 B, 临床出诊号源 C" & vbNewLine & _
            "       Where C.号码 = [1] And A.号源ID = C.ID And A." & strDay & " = B.时间段 And ([2] Between A.开始时间 And A.终止时间 Or A.开始时间 IS Null))" & vbNewLine & _
    ...
    Next
    If strSNS <> "" Then
        strSQL = "Select 序号 From 临床出诊序号控制 Where Trunc(开始时间) = [1] And Instr([2], ','''||序号||'''') > 0 And Not (状态=3 And 操作员姓名=[3])"
    ...
    strSQL = "zl_病人预约挂号_Defer('" & str号别 & "',To_date('" & Format(dtpBooking.Value, "yyyy-MM-dd 00:00:00") & "','YYYY-MM-DD HH24:MI:SS'),To_date('" & _
            Format(dtpDefer.Value, "yyyy-MM-dd 00:00:00") & "','YYYY-MM-DD HH24:MI:SS'),'" & UserInfo.姓名 & "'," & lng记录ID & ")"
...
Private Sub ShowPlan(datBooking As Date)
    ...
    strSQL = "Select Distinct E.号类,E.号码 号别, C.名称 科室, B.医生姓名 医生, Nvl(B.限约数,0) 限约," & vbNewLine & _
            " To_Char(B.开始时间,'YYYY-MM-DD') 开始时间,To_Char(B.终止时间,'YYYY-MM-DD') 终止时间,Decode(Nvl(B.是否序号控制,0),1,'√',' ') as 序号控制,B.ID As 记录ID" & vbNewLine & _
            "From 门诊费用记录 A, 病人挂号记录 D, 临床出诊记录 B, 临床出诊号源 E, 部门表 C " & vbNewLine & _
            ...
            "      And A.NO = D.NO And D.出诊记录ID = B.ID And B.号源ID=E.ID And E.科室id = C.ID "
...
Private Sub SetPlanGrid()
    ...
    strHead = "号类,1,600|号别,1,600|科室,1,1050|医生,4,800|限约,4,500|开始时间,4,1000|终止时间,4,1000|序号控制,4,850|记录ID,1,0"
