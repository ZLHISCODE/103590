...
Private Sub Show挂号安排信息(enum查询类型 As mEnum_查询类型)
    ...
    "            '7', '周六', Null)  " & _
    " And A.项目ID=C.ID(+) " & IIf(enum查询类型 = 可换号排班, " And A.停用日期 Is Null ", "") & _
    ...
    "                      E.失效时间 Group By E.安排ID Having Count(1) > 1 " & _
    "              ) "
    ...
    "               '7', '周六', Null) " & _
    "               And " & str时间 & " Between A.生效时间 + 0 And A.失效时间" & _
    ...
    If enum查询类型 = 可换号排班 Then
        ...
        "   And [1] Between Nvl(B.生效时间,to_date('1900-01-01','yyyy-MM-dd')) And B.失效时间 And B.审核时间 is Not Null" & _
