...
'功能：读取当日安排内容
    ...
    strSQL = _
        ...
        " Where P.ID=A.安排ID(+) And P.停用日期 Is Null And P.科室ID=B.ID And P.项目ID=C.ID AND P.科室id <>[1] and P.项目ID=[2]" & vbCrLf & _
        " And P.Id = f.安排id(+) And Not Exists (Select 1 From 挂号安排停用状态 Where Sysdate Between 开始停止时间 And 结束停止时间 And 安排ID=P.ID) And" & _
