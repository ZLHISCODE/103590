        ...
        private void Load药品库存(int intOne = 0, int intRow = 0, Boolean blnLoad = false)
        {
            try
            {
                ...
                if (strIds != "")
                {
                    ...
                    if (dtTmp != null)
                    {
                        if (dtTmp.Rows[0]["药品ids"].ToString() != "")
                        {
                            string strIn = JsonConvert.SerializeObject(new
                            {
                                drug_ids = dtTmp.Rows[0]["药品ids"].ToString(),
                                phermacy_ids = str药房ID,
                                return_price = 0,
                                return_dept = 0
                            });
                            strIn = @"{""input"":" + strIn + "}";
                            //获取药品库存
                            strOut = DataCenter.GetDrugService(m_baseData, "zl_DrugSvr_GetStockBatch", strIn);

                            if (strOut != "")
                            {
                                JObject jObject = JObject.Parse(strOut);
                                JArray jlist = JArray.Parse(jObject["output"]["item_list"].ToString());
                                for (int i = 0; i < jlist.Count; ++i)  //遍历JArray  
                                {
                                    JObject Jitem = JObject.Parse(jlist[i].ToString());
                                    str药品库存字符串 = str药品库存字符串 + "," + Jitem["drug_id"].ToString() + ":" + str药房ID + ":" + Jitem["stock"].ToString();
                                }
                                if (str药品库存字符串 != "")
                                {
                                    str药品库存字符串 = str药品库存字符串.Substring(1).ToString();
                                    dtTmp = new DataTable();
                                    message = JsonConvert.SerializeObject(new
                                    {
                                        获取药品ID = "0",
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < dtInfo.Rows.Count; i++)
                        {
                            ...
                            Dgvr.Cells["脚注"].Value = dtInfo.Rows[i]["脚注"].ToString();
