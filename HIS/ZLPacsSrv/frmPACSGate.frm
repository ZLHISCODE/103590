...
Private Sub Form_Load()
    ...
    With gcnAccess
        .ConnectionString = "DBQ=" & App.Path & "\ZlPacsLog.mdb;DefaultDir=" & App.Path & ";Driver={Microsoft Access Driver (*.mdb)}"
        .Open
...
Private Function ReadPara() As Boolean
    ...
    '获取“自动清空临时表”的设置
    gstrClearTable = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\接收服务", "自动清空临时表", "")
    gintClearInteval = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\接收服务", "自动清空临时表间隔天数", "0")
    gstrClearExeDay = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\接收服务", "自动清空临时表日期", Date)
    ...
    strSQL = "Select 设备名,影像类别,PACSAE名称,PACS端口,设备IP地址,设备AE名称,设备端口,服务功能 From 影像dicom服务对 a ,影像设备目录 b " & _
             " Where a.设备号=b.设备号 And (a.PACS角色='SCP' or a.PACS角色='SCU' ) And instr([1],PACSIP地址)>0"
    ...
    '从数据库获取参数设置
    strSQL = "Select a.设备IP地址,a.PACSAE名称,b.参数名称,b.参数值 From 影像DICOM服务对 a,影像DICOM服务参数 b " & _
             "Where a.服务ID = b.服务ID And a.PACS角色='SCP' And instr([1],a.PACSIP地址)>0"
    ...
    '从数据库获取FTP存储设备
    strSQL = "Select 设备号,IP地址,FTP目录,FTP用户名,FTP密码 From 影像设备目录 Where 类型 =[1]"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "读取FTP存储设备", CLng(1))
    If rsTmp.RecordCount > 0 Then
...
Private Function funGetServiceIndex(strServiceAE As String) As Integer
    Dim i As Integer
...
Private Sub DViewer_QueryRequest(ByVal connection As DicomObjects.DicomConnection)
    ...
    If root = "WORKLIST" Then    '处理Worklist的请求
        ...
        '获取服务的基本参数设置
        funGetAEMWLParas connection.CalledAET, connection.RemoteIP, intFilterModality, intDayInterval, blnUseForceResult
        
        subLogDataset rq, "QueryRequest", "WORKLIST接收数据集"
        
        sql = "select e.影像类别,a.医嘱ID,c.医嘱内容,a.发送号,a.英文名,b.执行间,b.发送人,a.出生日期,a.性别,b.首次时间,b.执行过程,a.检查UID,a.检查号,Decode(C.病人来源,2,D.住院号,D.门诊号) As 标识号,a.年龄,a.姓名,a.检查设备 " & _
              "from 影像检查记录 a, 病人医嘱发送 b,病人医嘱记录 C,病人信息 D, 影像设备目录 E where a.医嘱ID = b.医嘱ID+0 and a.发送号 = b.发送号+0 And A.检查设备 =E.设备号 " & _
              "And B.医嘱ID+0=C.ID And C.病人ID+0=D.病人ID " & _
              "And B.执行状态=3 And B.执行过程=2 And A.检查UID IS Null AND C.相关ID IS NULL  "
...
'        AddLinkedDateTimeCondition sql, rq1.Attributes(&H40, 2), rq1.Attributes(&H40, 3), "b.首次时间"
        ...
        '传输DICOM连接信息
        '添加强制信息
        If blnUseForceResult Then subReturnDataSet connection, 2
        While Not result.EOF
            subReturnDataSet connection, 1, result
            result.MoveNext
        Wend
        connection.SendStatus 0
        Exit Sub
        ...
        If connection.operation = "C-FIND" Then
            ...
            If Level = "PATIENT" Then   '处理病人级别的查询,支持病人姓名，病人ID作为查询条件
                ...
                If intPatientIDMatch = 0 Then       '检查号
                    sql = "Select a.医嘱ID,a.检查号 as PatientID,a.姓名,a.性别,a.英文名,a.出生日期 From 影像检查记录 a Where 1 = 1 "
                ElseIf intPatientIDMatch = 1 Then   '住院号/门诊号
                    sql = "Select a.医嘱ID,Decode(b.病人来源, 2, c.住院号, c.门诊号) as PatientID,a.姓名,a.性别,a.英文名,a.出生日期 " _
                          & " From 影像检查记录 a,病人医嘱记录 b,病人信息 c Where a.医嘱ID=b.Id And b.病人ID=c.病人ID "
                Else                                '或者医嘱ID
                    sql = "Select a.医嘱ID,a.医嘱ID as PatientID,a.姓名,a.性别,a.英文名,a.出生日期 From 影像检查记录 a Where 1 = 1 "
                End If
            ...
            If Level = "STUDY" Then
                ...
                If intPatientIDMatch = 0 Then       '检查号
                    sql = "Select a.医嘱ID,a.检查号 as PatientID,a.影像类别,a.检查uid,a.性别,a.英文名,a.出生日期,a.接收日期 " _
                        & " From 影像检查记录 a Where 1 = 1 "
                ElseIf intPatientIDMatch = 1 Then   '住院号，门诊号
                    sql = "Select a.医嘱ID,Decode(b.病人来源, 2, c.住院号, c.门诊号) as PatientID,a.影像类别,a.检查uid,a.性别,a.英文名,a.出生日期,a.接收日期 " _
                          & " From 影像检查记录 a,病人医嘱记录 b,病人信息 c Where a.医嘱ID=b.Id And b.病人ID=c.病人ID "
                Else                                '医嘱ID
                    sql = "Select a.医嘱ID,a.医嘱ID as PatientID,a.影像类别,a.检查uid,a.性别,a.英文名,a.出生日期,a.接收日期 From 影像检查记录 a Where 1 = 1 "
                End If
                ...
                AddDateCondition sql, rq.Attributes(&H8, &H20), "a.接收日期"
                ...
                Do Until result.EOF
                    ...
                    AddResultItem ds, rq, &H8, &H20, result!接收日期
                    If root = "STUDY" Then
            ...
            If Level = "SERIES" Then
                sql = "select b.序列uid,b.序列描述,b.序列号,a.影像类别 from 影像检查记录 a ,影像检查序列 b " _
            ...
            If Level = "IMAGE" Then
                sql = "  select t.图像uid,t.序列uid,t.图像号 from 影像检查图象 t where 1=1"
        ...
        ElseIf connection.operation = "C-GET" Or connection.operation = "C-MOVE" Then
            ...
            If connection.operation = "C-MOVE" Then
                ...
                sql = "Select decode(PACS角色,'SCP',PACSIP地址,设备IP地址) As IP地址,decode(PACS角色,'SCP',PACS端口,设备端口) As 端口号 " & _
                      "From 影像DICOM服务对 Where 服务功能='图像接收' And (PACS角色='SCP' And PACSAE名称 =[1]) Or (PACS角色='SCU' And 设备AE名称=[1])"
                WriteCommLog "QueryRequest", "C-MOVE查找图像移动目的主机", Replace(sql, "'", "‘")
                WriteCommLog "QueryRequest", "C-MOVE查找图像移动目的主机--参数", "[1] = " & RemoteAET
                
                Set result = zlDatabase.OpenSQLRecord(sql, "查找C-MOVE的目的地", RemoteAET)
                If result.EOF Then
            ...
            If Level = "PATIENT" Then Set ResultImages = GetAllImageFiles(Level, rq.PatientID)
...
'-----------------------------------------------
    ...
    If Level = "PATIENT" Then
        strSQL = "select e.序列uid from 影像检查记录 c , 影像检查序列 e , " _
    ...
    ElseIf Level = "STUDY" Then
        strSQL = "select b.序列uid from 影像检查记录 a, 影像检查序列 b where a.检查uid = b.检查uid " _
                    & "and a.检查uid = [1]"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, SearchValue)
    ElseIf Level = "SERIES" Then
        strSQL = "select t.序列uid from 影像检查序列 t where t.序列uid = [1]"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, SearchValue)
    ElseIf Level = "IMAGE" Then
        strSQL = "select t.序列uid from 影像检查序列 t ,影像检查图象 q where t.序列uid = q.序列uid " _
                    & "and q.图像uid = [1]"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, SearchValue)
    End If
    ...
    For i = 1 To UBound(aSeriesUIDs)
        ...
        strSQL = "Select A.图像号,D.FTP用户名 as User1 ,D.FTP密码 as Psw1 , D.IP地址 as IP1 , " & _
...
Private Sub subReturnDataSet(connection As DicomConnection, intType As Integer, Optional rsOracle As Recordset)
    ...
    '初始化非序列的属性
'    For Each D1 In rq.Attributes
'        dsResult.Attributes.Add D1.group, D1.element, D1.value
'    Next
    Set dsResult = rq
    ...
    Else
        ...
        dsResult.Attributes.Add &H8, &H1110, NullSequence
    End If
    ...
    Else
        ...
        dsResult.Attributes.Add &H8, &H1120, NullSequence
    End If
    ...
    Else
        ...
        dsResult.Attributes.Add &H32, &H1064, NullSequence
    End If
    ...
    If rq.Attributes(&H40, &H100).Exists And Not IsNull(rq.Attributes(&H40, &H100).value) Then
        ...
        Else
            ...
            dsSeqItem(5).Attributes.Add &H40, &H8, NullSequence
        End If
    
    Else
        Set rqs(5) = NullSequence
        Set rq1(5) = NullSeqItem
        dsResult.Attributes.Add &H40, &H100, NullSequence
    End If
    ...
    strSQL = "Select a.Id, a.组号, a.元素号 From 影像MWL结果集 a ,影像DICOM服务对 b " & _
             " Where a.值类型 = 'SQ' and a.服务ID =b.服务ID and  a.选中 = 1 and b.PACSAE名称=[1] and b.设备IP地址 =[2]  Order by id "
    Set dbResult = zlDatabase.OpenSQLRecord(strSQL, "读取MWL数据集序列", CStr(connection.CalledAET), CStr(connection.RemoteIP))
    Dim lngMin As Long
    Dim lngMax As Long
    ...
    strSQL = "Select a.组号, a.元素号, a.上级id, a.数据值, a.是否递增,a.值类型, a.元素类型, a.强制结果值 From 影像MWL结果集 a , " & _
             " 影像DICOM服务对 b Where a.服务ID =b.服务ID and  a.选中 = 1 and b.PACSAE名称=[1] and b.设备IP地址 =[2]"
    Set dbResult = zlDatabase.OpenSQLRecord(strSQL, "读取MWL数据集", CStr(connection.CalledAET), CStr(connection.RemoteIP))
    While Not dbResult.EOF
        If dbResult!值类型 <> "SQ" Then  '数据类型不是SQ的，才解码返回值
            ...
            If intType = 1 Then         '返回正常查询结果
                ...
                Do While InStr(strValue, "[") <> 0
                    ...
                    Else
                        strFieldValue = funGetFieldValue(strField, rsOracle)
                    End If
        ...
        If dbResult!值类型 <> "SQ" Then
            ...
            Else    '有上级ID，说明是嵌套在序列中的数据
                '知道上级ID，需要查找到使用那个数据集
                AddResultItem dsSeqItem(UpID(dbResult!上级ID)), rq1(UpID(dbResult!上级ID)), Int("&H" & dbResult!组号), Int("&H" & dbResult!元素号), strValue
            End If
        End If
        dbResult.MoveNext
    Wend
...
Private Function funGetFieldValue(strField As String, rsDataSet As Recordset) As String
    Select Case strField
        ...
        Case "年龄"
            funGetFieldValue = Format(Val(Nvl(rsDataSet!年龄, "0")), "#" & _
                IIf(Right(Nvl(rsDataSet!年龄), 1) = "天", "D", IIf(Right(Nvl(rsDataSet!年龄), 1) = "月", "M", "Y")))
        Case "出生日期"
...
Private Sub Timer1_Timer()
    ...
    '判断当前是否有图像，如果没有图像，而且90秒之内没有Association，则清空Association数组
    If DateDiff("S", mdtLastAssociation, Time) > 90 And DViewer.Images.Count = 0 Then
        ReDim AEconnections(0) As AEconnection
    End If
    
    '自动清空临时表
    On Error Resume Next
    If gintClearInteval <> 0 And Date - CDate(gstrClearExeDay) >= gintClearInteval Then
        subClearTempTable
        gstrClearExeDay = Date
        SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\接收服务", "自动清空临时表日期", gstrClearExeDay
    End If
    On Error GoTo 0
End Sub
...
Private Function funSaveFilmImages(ByVal ruid As String, connection As DicomConnection, ByVal iService As Integer)
    ...
    For i = 1 To intImageCount
        ...
        If DImageAtt.Exists Then
            ...
            '把图像放到DViewer中，等待保存
            subWriteDicomPara DImage, iService, Str(i)
...
Private Sub subWriteDicomPara(img As DicomImage, ByVal iService As Integer, strImageNum As String)
'------------------------------------------------
