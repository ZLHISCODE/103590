...
Public Function GetSearch(ByVal FrmMain As Form, _
        ...
        ByRef str发票号开始 As String, ByRef str发票号结束 As String, _
        ByRef lng药品分类 As Long, _
        ByRef str剂型 As String, _
        ByRef date发票审核日期开始 As Date, ByRef date发票审核日期结束 As Date, _
        ByRef intNo标记 As Integer, ByRef intYes标记 As Integer, _
        ByRef intNo发票 As Integer, ByRef intYes发票 As Integer, _
        Optional ByRef intTmp As Integer = 0, _
        Optional ByRef int未审核单据 As Integer, Optional ByRef int已审核单据 As Integer, Optional ByRef int包含冲销 As Integer, _
        Optional ByRef int未财务审核 As Integer, Optional ByRef int已财务审核 As Integer, Optional ByRef int未审核退库 As Integer, Optional ByRef int已审核退库 As Integer) As String
    ...
    '临时查询初始化
    '---------------------
    SQLCondition.date填制时间开始 = date填制时间开始
    SQLCondition.date填制时间结束 = date填制时间结束
    SQLCondition.date审核时间开始 = date审核时间开始
    SQLCondition.date审核时间结束 = date审核时间结束
    
    TemporaryInquiries.int未审核单据 = int未审核单据
    TemporaryInquiries.int已审核单据 = int已审核单据
    TemporaryInquiries.int包含冲销 = int包含冲销
    TemporaryInquiries.int未财务审核 = int未财务审核
    TemporaryInquiries.int已财务审核 = int已财务审核
    
    SQLCondition.int已标记 = intYes标记
    SQLCondition.int未标记 = intNo标记
    SQLCondition.int无发票 = intNo发票
    SQLCondition.int有发票 = intYes发票
    
    TemporaryInquiries.int未审核退库 = int未审核退库
    TemporaryInquiries.int已审核退库 = int已审核退库
    '---------------------
    ...
    '临时查询保留
    '---------------------
    int未审核单据 = TemporaryInquiries.int未审核单据
    int已审核单据 = TemporaryInquiries.int已审核单据
    int包含冲销 = TemporaryInquiries.int包含冲销
    int未财务审核 = TemporaryInquiries.int未财务审核
    int已财务审核 = TemporaryInquiries.int已财务审核
    int未审核退库 = TemporaryInquiries.int未审核退库
    int已审核退库 = TemporaryInquiries.int已审核退库
    '---------------------
...
Private Sub Form_Load()
    ...
    '恢复上一次的设置
    '--------------------------------
    Me.dtp结束时间(0) = SQLCondition.date填制时间结束
    Me.dtp结束时间(1) = SQLCondition.date审核时间结束
    Me.dtp开始时间(0) = SQLCondition.date填制时间开始
    Me.dtp开始时间(1) = SQLCondition.date审核时间开始
    
    chk填制.Value = TemporaryInquiries.int未审核单据
    chk审核.Value = TemporaryInquiries.int已审核单据
