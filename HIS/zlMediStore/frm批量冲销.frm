            ...
            If mlng模块号 <> 模块号.其他出库 And mlng模块号 <> 模块号.药品领用 Then
                ...
                If mint库存检查入库库房 <> 0 And .TextMatrix(intRow, 1) <> "" Then
                    ...
                    If CheckStrickUsable(mInt单据号, Me.cbo库房.ItemData(Me.cbo库房.ListIndex), Val(.TextMatrix(intRow, 1)), .TextMatrix(intRow, mintcol药名), _
                        IIf(mlng模块号 = 模块号.其他入库, 0, (.TextMatrix(intRow, mintcol批次))), Val(冲销数量_IN), mint库存检查入库库房, Trim(.TextMatrix(intRow, mIntColNO)), Val(.TextMatrix(intRow, mintcol序号))) = False Then 'IIf(mlng模块号 = 模块号.药品移库, Val(.TextMatrix(intRow, mintcol序号)) + 1, Val(.TextMatrix(intRow, mintcol序号)))
                        Exit Function
                    End If
        ...
        For i = 1 To mrecSort.RecordCount
            ...
            If .TextMatrix(intRow, 1) <> "" And Val(.TextMatrix(intRow, mintcol冲销数量)) <> 0 Then
                ...
                Else
                    ...
                    If mlng模块号 = 模块号.外购入库 Then int全部冲销 = 0 '外购对应单据药品全冲
                End If
