...
'    mlngMode = lngMode
    ...
    mbln空批次 = False
    '盘点单要记录
    If UCase(FrmMain.Name) = UCase("frmNewCheckCard") Or UCase(FrmMain.Name) = UCase("frmNewCheckCourseCard") Then
        mbln空批次 = True
...
Private Sub cmdRefresh_Click()
    ...
    ElseIf mfrmMain.Caption Like "药品盘点记录单*" Or mfrmMain.Caption Like "药品盘点表*" Then
        Call SetSelectorRS(mbyt编辑模式, "药品盘点管理", mlng来源库房, mlng目标库房, , mlng供应商, , IIf(mbyt包含停用药品 = 1, True, False), False, 0, True, False, True, mstr盘点时间)
...
Private Sub tvw类别_NodeClick(ByVal Node As MSComctlLib.Node)
    ...
    If tvw类别.Tag <> Node.Key Then
        ...
        '根据条件过滤数据
        With fra形态
            If Node.Tag = "3" Or Node.Tag = "Root7" Then
                .Visible = True
            Else
                .Visible = False
            End If
        ...
        Else
            ...
            If UCase(mfrmMain.Name) = UCase("frmTransferCard") Or UCase(mfrmMain.Name) = UCase("frmRequestDrugCard") Or UCase(mfrmMain.Name) = UCase("frmDrawCard") Then
                ...
                Else
                    ...
                    vsf批次.Visible = False: vsf批次.TabStop = False
                End If
            ElseIf UCase(mfrmMain.Name) = UCase("frmNewcheckCoursecard") Or UCase(mfrmMain.Name) = UCase("frmNewCheckCard") Then
                picSplit04_S.Visible = True
                vsf批次.Visible = True: vsf批次.TabStop = True
                '增加空批次记录
                If mbln空批次 Then
...
Private Sub vsf规格_DblClick()
    ...
    Else
        If glngModul = 1304 Or glngModul = 1305 Or glngModul = 1306 Or glngModul = 1307 Or glngModul = 1343 Then
            If glngModul = 1307 And vsf规格.TextMatrix(vsf规格.Row, vsf规格.ColIndex("时价")) = "是" Then
                 If CheckNoStock(mlng来源库房, Val(vsf规格.TextMatrix(vsf规格.Row, vsf规格.ColIndex("药品ID")))) = True Then
                    '时价不分批并且无库存作为盘点入库，不在选择器中判断零差价
                    dbl忽略零差价检查 = True
                 End If
            End If
            
            If dbl忽略零差价检查 = False Then
                '零差价管理：选定药品不满足零差价要求，则不能进行出库业务
                If gtype_UserSysParms.P275_零差价管理模式 = 2 And IsPriceAdjustMod(Val(vsf规格.TextMatrix(vsf规格.Row, vsf规格.ColIndex("药品ID")))) = True Then
                    If CheckPriceAdjust(Val(vsf规格.TextMatrix(vsf规格.Row, vsf规格.ColIndex("药品ID"))), mlng来源库房, -1) = False Then
                        MsgBox "该药品启用零差价管理模式，但成本价和售价不一致，不能开展业务。请先调整价格！", vbInformation, gstrSysName
        ...
        If UCase(mfrmMain.Name) <> UCase("frmOtherOutputCard") And UCase(mfrmMain.Name) <> UCase("frmTransferCard") And UCase(mfrmMain.Name) <> UCase("frmPurchaseCard") And UCase(mfrmMain.Name) <> UCase("frmDrugQualityCard") Then
...
'        End Select
        ...
        If vsf批次.Visible Then
            ...
            Else
                ...
                .TextMatrix(lngRow, .ColIndex("成本价")) = vsf批次.TextMatrix(vsf批次.Row, vsf批次.ColIndex("成本价"))
            End If
        Else
            '提取不分批药品的批号与效期信息
            gstrSQL = "Select 上次批号,效期,上次供应商id,上次生产日期 AS 生产日期,批准文号,上次产地,平均成本价 From 药品库存 " & _
            ...
            If rsTemp.RecordCount > 0 Then
                ...
                If Not IsNull(rsTemp!平均成本价) Then
                    Select Case mintUnit
                        Case mconint门诊单位
                            dblPrice = rsTemp!平均成本价 * IIf(Val(.TextMatrix(lngRow, .ColIndex("门诊包装"))) = 0, 1, Val(.TextMatrix(lngRow, .ColIndex("门诊包装"))))
                        Case mconint住院单位
                            dblPrice = rsTemp!平均成本价 * IIf(Val(.TextMatrix(lngRow, .ColIndex("住院包装"))) = 0, 1, Val(.TextMatrix(lngRow, .ColIndex("住院包装"))))
                        Case mconint药库单位
                            dblPrice = rsTemp!平均成本价 * IIf(Val(.TextMatrix(lngRow, .ColIndex("药库包装"))) = 0, 1, Val(.TextMatrix(lngRow, .ColIndex("药库包装"))))
                    End Select
                    .TextMatrix(lngRow, .ColIndex("成本价")) = dblPrice
                End If
            End If
...
Private Sub HiddenColumns()
    ...
    With vsf规格
        If .rows > 1 Then
            ...
            Set rsDetail = zlDataBase.OpenSQLRecord(gstrSQL, "判断是库房性质", .TextMatrix(1, .ColIndex("药品id")))
