    ...
    If Val(Txt药品.Tag) <> 0 Then
        If mstrResult = "" Then
            mstrResult = " a.调价号 In (Select 调价汇总号 From 收费价目 Where 收费细目id = " & Txt药品.Tag & GetPriceClassString("") & _
                            " union all " & _
                         " Select  调价汇总号 From 成本价调价信息 Where 药品id = " & Txt药品.Tag & " " & _
                         " union all " & _
                         " Select  调价汇总号 From 药品价格记录 Where 药品id = " & Txt药品.Tag & ")"
        Else
...
Private Sub Cmd药品_Click()
    ...
    Call SetSelectorRS(1, "", 0, , , , , True, , , , True)
    
    Set RecReturn = frmSelector.ShowME(Me, 0, 1, , , , , , , , , , , 2, False)
...
'    Set RecReturn = Frm药品多选选择器.ShowME(Me, 1, , mfrmMain.cboStock.ItemData(mfrmMain.cboStock.ListIndex), mfrmMain.cboStock.ItemData(mfrmMain.cboStock.ListIndex), strkey, sngLeft, sngTop)
    Set RecReturn = frmSelector.ShowME(Me, 1, 1, strkey, sngLeft, sngTop, , , , , , , , 2, False)
