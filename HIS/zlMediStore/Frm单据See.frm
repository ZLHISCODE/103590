        ...
        Select Case byt单据
            Case 8, 9, 27            '处方
                ...
                Else
                    ...
                    Me.Lbl标题 = "药品留存单"
                End If
                .Cols = IIf(byt单据 = 8 Or byt单据 = 9, 11, 10)
                ...
                .TextMatrix(0, 8) = "成本金额"
                .TextMatrix(0, 9) = "差价"
                If byt单据 = 8 Or byt单据 = 9 Then .TextMatrix(0, 10) = "发药人"
                ...
                .ColWidth(8) = 800
                .ColWidth(9) = 800
                If byt单据 = 8 Or byt单据 = 9 Then .ColWidth(10) = 1000
...
Private Sub RefreshBodyData()
    ...
    Dim intRow As Long
    Dim int发药人数 As Integer
    Dim str发药人 As String
...
'                " ORDER BY A.序号"
            gstrSQL = "SELECT DISTINCT '【'||F.编码||'】'||NVL(E.名称,F.名称) AS 药品信息,F.规格,DECODE(NVL(A.产地,''),'',DECODE(NVL(F.产地,''),'','',F.产地),A.产地) 产地,B.门诊单位 AS 单位," & _
                " B.门诊包装,A.实际数量/DECODE(B.门诊包装,0,1,B.门诊包装) AS 数量,A.零售价*B.门诊包装 AS 单价," & _
                " A.零售金额,A.付数,A.成本金额,A.差价,A.药品ID,A.库房ID,A.入出系数,A.序号,A.ID,B.最大效期,B.指导零售价,B.加成率,a.审核人" & _
                " FROM 药品收发记录 A,药品规格 B,收费项目别名 E,收费项目目录 F " & _
                " WHERE A.药品ID=B.药品ID AND B.药品ID=F.ID " & _
                " AND B.药品ID=E.收费细目ID(+) AND E.性质(+)=3 AND E.码类(+)=1" & _
...
'                " ORDER BY A.序号"
        ...
        Case 10        '记帐表处方
            gstrSQL = "SELECT DISTINCT P.名称 科室,G.姓名,G.性别,G.年龄,G.病人ID,G.主页ID,D.住院号,G.床号," & _
                ...
                " A.零售金额,A.付数,A.成本金额,A.差价,A.药品ID,A.库房ID,A.入出系数,A.序号,A.ID,B.最大效期,B.指导零售价,B.加成率" & _
        ...
        Select Case byt单据
            Case 8, 9, 27         '处方
                    For intRow = 1 To .RecordCount
                        ...
                        If byt单据 = 8 Or byt单据 = 9 Then
                            msf.TextMatrix(intRow, 10) = IIf(IsNull(!审核人), "", !审核人)
                            If InStr(1, ";" & str发药人 & ";", ";" & msf.TextMatrix(intRow, 10) & ";") = 0 Then
                                str发药人 = IIf(str发药人 = "", "", str发药人 & ";") & msf.TextMatrix(intRow, 10)
                                int发药人数 = int发药人数 + 1
                            End If
                        End If
                    ...
                    Next
                    If int发药人数 < 2 Then
                        If byt单据 = 8 Or byt单据 = 9 Then msf.ColWidth(10) = 0
                    Else
                        Txt发药人.Text = Txt发药人.Text & "(等" & int发药人数 & "人)"
                    End If
            Case 10         '摆药
                    For intRow = 1 To .RecordCount
...
Private Function SetEV()
    ...
    End Select
    If Not zlStr.IsHavePrivs(mstrPrivs, "医生查询") Then
        Lbl科室.Visible = False
        Me.Cbo科室.Visible = False
