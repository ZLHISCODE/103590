...
Private Sub GetSelect(ByVal strInput As String)
    ...
    vRect = zlControl.GetControlRect(txt明细药品.hWnd)
...
'        "       Group By A.库房id, A.药品id, B.名称, Decode(B.系数, 1, '2-入库', '3-出库')"
        gstrSQL = gstrSQL & _
          "       Union All " & _
          "       Select a.库房id, Decode(a.入出系数, 1, '2-入库', '3-出库') As 业务大类, b.名称 As 业务分类, a.药品id, sum(a.数量) 数量, sum(a.金额) 金额, sum(a.差价) 差价" & vbNewLine & _
          "       From 药品结存汇总 A, 药品入出类别 B" & vbNewLine & _
          "       Where a.结存id = [1] and a.入出类别id =b.id " & vbNewLine & _
          "       Group By a.库房id, a.药品id, b.名称, Decode(a.入出系数, 1, '2-入库', '3-出库')"
    End If
