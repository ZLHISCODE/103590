...
Private Sub cmdPrint_Click()
    If optAll Then
        Call RealPrint(1, frmTendFileReader.GetPages)
    ElseIf optCurrent Then
        Call RealPrint(gintPage, gintPage)
    Else
        If CInt(txtBegin.Text) < 1 Or CInt(txtBegin.Text) > frmTendFileReader.GetPages Then
            ...
            Exit Sub
        End If
        If CInt(txtEnd.Text) < 1 Or CInt(txtEnd.Text) > frmTendFileReader.GetPages Then
            MsgBox "输出的页码超出范围了。", vbCritical, gstrSysName
            txtEnd.SelStart = 0
            txtEnd.SelLength = 5
            txtEnd.SetFocus
...
Private Sub Form_Load()
    ...
    txtBegin.Text = 1
    txtEnd.Text = CStr(frmTendFileReader.GetPages)
End Sub
