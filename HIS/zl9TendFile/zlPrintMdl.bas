...
'    On Error GoTo errHand
    ...
    blnShowNull = frmTendFileReader.blnShowNullCollet
    If bytCollType = 2 Or bytCollType = 4 Then
        If InStr(1, "|" & frmTendFileReader.GetCollectCols(ROW) & ";", "|" & COL - (glngHideCols + objGrid.FixedCols - 1) & ";") = 0 Then
            bytCollType = 0   '不处理
        Else
            If Not IIf(blnShowNull, True, Val(objGrid.TextMatrix(ROW, COL)) <> 0) Then
                bytCollType = 0     '不处理
            End If
        End If
    End If
...
Public Function GetDrawWidth(ByRef Dev As Object) As Long
    Dim lngDc As Long, lngPPI As Long
    If TypeName(Dev) = "Printer" Then
                lngDc = GetDC(0)
        lngPPI = GetDeviceCaps(lngDc, LOGPIXELSY)
        GetDrawWidth = GetDeviceCaps(Printer.hDC, LOGPIXELSY) / IIf(lngPPI = 0, 1, lngPPI)
        ReleaseDC 0, lngDc
    Else
        GetDrawWidth = 1
    End If
...
Public Sub DrawSingePic(ByVal StrSingeName As String, ByVal X As Long, ByVal Y As Long, ByVal Wide, ByVal High, Optional Alignment As Byte = 0)
    ...
    '1---先处理第一个人
    '90637问题将人员表.签名图片，有long Raw调整为了BLOB,读取方法需要改变
    '有zlDatabase.ReadPicture 调整为sys.ReadLob读取
    gstrSQL = "select ID from 人员表 Where 姓名=[1]"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "签名图片", CStr(arrText(0)))
    If rsTemp.RecordCount > 0 Then
        strPicPath = Sys.ReadLob(glngSys, 15, CStr(rsTemp!ID))
...
ErrOut1:
    ...
    If rsTemp.RecordCount > 0 Then
        strPicPath2 = Sys.ReadLob(glngSys, 15, CStr(rsTemp!ID))
    ...
    If UBound(arrText) > 1 Then
        ...
        If rsTemp.RecordCount > 0 Then
            strPicPath3 = Sys.ReadLob(glngSys, 15, CStr(rsTemp!ID))
            If strPicPath3 = "" Then strPicPath3 = "[LPF]"
        Else
            strPicPath3 = "[LPF]"
        End If
    Else
