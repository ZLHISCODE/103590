...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If Not mcbsThis Is Nothing Then Set mcbsThis = Nothing
    If Not cbrControl Is Nothing Then Set cbrControl = Nothing
    If Not cbrMenuBar Is Nothing Then Set cbrMenuBar = Nothing
    If Not cbrToolBar Is Nothing Then Set cbrToolBar = Nothing
End Sub
...
'记录界面选定信息，因为工作站在切换页卡时是释放了对象，换回来时重新初始化刷新的。
    GetFormOperation = mfrmDockInTend_TendList.OperationValue
End Function

Public Sub RestoreFormOperation(ByVal strValue As String)
'恢复界面选定信息，工作站在刷新之前调用
    mfrmDockInTend_TendList.OperationValue = strValue
End Sub
