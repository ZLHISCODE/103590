...
Begin VB.UserControl usrTendFileMutilEditor 
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsfprint 
      ...
      ColWidthMax     =   0
      ExtendLastCol   =   0   'False
      FormatString    =   $"usrTendFileMutilEditor.ctx":0C51
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Private mstrID As String                    '记录ID字符串,94221
            ...
            If Val(VsfData.TextMatrix(lngRow, mlngSignLevel)) > 0 Then
                ...
                If clsSign Is Nothing Then
                    ...
                    If Not clsSign Is Nothing Then
                        If Not clsSign.CheckCertificate(gstrDBUser) Then Exit Sub
                    Else
...
Private Sub txtLst_Change()
    ...
    If PicLst.Height + PicLst.Top > picMain.Height Then
        If picMain.Height - PicLst.Height < 0 Then
            PicLst.Top = 10
            PicLst.Height = picMain.Height - 10
        Else
            PicLst.Top = picMain.Height - PicLst.Height
        End If
    End If
...
Private Function InitMenuBar() As Boolean
    ...
    cbsThis.ActiveMenuBar.EnableDocking (xtpFlagAlignTop)
        ...
        With cbrToolBar.Controls
            '100648,
            Set objMenu = .Add(xtpControlPopup, conMenu_FilePopup, "记录单打印(&F)", -1, False)
...
Private Function CheckValid(strReturn As String, ByVal strOrders As String, strInfo As String) As Boolean
    ...
    Dim i As Integer, j As Integer, blnNumber As Boolean, blnCanves As Boolean
    ...
    Case 1, 2
        ...
        If mintType = 1 Then
            ...
            With PicLst
                ...
                If .Height < CellRect.Bottom Then .Height = CellRect.Bottom
                If .Height + .Top > picMain.Height Then
                    If picMain.Height - .Height < 0 Then
                        .Top = 10
                        .Height = picMain.Height - 10
                    Else
                        .Top = picMain.Height - .Height
                    End If
                End If
        ...
        Else
            ...
            With lstSelect(mintType - 1)
                ...
                If .Height + .Top > picMain.Height Then
                    If picMain.Height - .Height < 0 Then
                        .Top = 10
                        .Height = IIf(picMain.Height - 10 < PicLst.TextHeight("刘") + PicLst.TextHeight("刘") \ 3, PicLst.TextHeight("刘") + PicLst.TextHeight("刘") \ 3, picMain.Height - 10)
                    Else
                        .Top = picMain.Height - .Height
                    End If
                End If
...
Private Sub FillPage()
    ...
    '在院病人清单
    strPatient = "" & _
        " SELECT 1 AS 性质,B.病人ID, B.主页ID, NVL(B.姓名,A.姓名) 姓名, NVL(B.性别,A.性别) 性别, B.住院号, lpad(Nvl(B.出院病床, ' '),10,' ')  AS 床号,0 AS 婴儿" & _
    ...
    If chk出院.Value = 1 Then
        ...
        strPatient = strPatient & _
            " UNION " & _
            " SELECT 3 AS 性质,B.病人ID, B.主页ID, NVL(B.姓名,A.姓名) 姓名, NVL(B.性别,A.性别) 性别, B.住院号,lpad(Nvl(B.出院病床, ' '),10,' ') AS 床号,0 AS 婴儿" & _
    ...
    If chk出科.Value = 1 Then
        ...
        strPatient = strPatient & _
            " UNION " & _
            " Select 2 AS 性质,B.病人ID, B.主页ID, NVL(B.姓名,A.姓名) 姓名, NVL(B.性别,A.性别) 性别, B.住院号, lpad(Nvl(c.床号, ' '),10,' ')  AS 床号,0 AS 婴儿" & _
    ...
    strPatient = strPatient & _
              " UNION " & _
              " Select B.性质,B.病人ID,B.主页ID,NVL(A.婴儿姓名,B.姓名||'之子'||A.序号) AS 姓名,B.性别,B.住院号,lpad(Nvl(B.床号, ' '),10,' ')  AS 床号,A.序号 AS 婴儿" & _
