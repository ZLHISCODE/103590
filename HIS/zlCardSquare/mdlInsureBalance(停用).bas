...
Public Function zlExecuteInsureSwap(ByVal bytMode As Byte, ByVal lng病人ID As Long, _
    ...
    '先保存预结算结果
    Call SaveInsureBalance(lng病人ID, lng结帐ID, strAll预结算)
        ...
        For p = 1 To objBalanceBills.Count
            ...
            Else
                ...
                If Not gclsInsure.ClinicSwap(lng结帐ID, cur个帐支付, cur医保基金, cur全自付, cur先自付, _
                    ...
                    If blnCurrentCommit Then Call CorrectInsureErrBalance(lng病人ID, lng结帐ID) '医保结算校对
                    Exit Function
                End If
    ...
    If zlInsureCheck(strAll预结算, strAdvance) Then
        Call SaveInsureBalance(lng病人ID, lng结帐ID, strAdvance)
    End If
...
ErrHandler:
    ...
    If bytMode = 2 And strSavedNos <> "" Then
        ...
        ElseIf blnCurrentCommit Then
            Call CorrectInsureErrBalance(lng病人ID, lng结帐ID) '医保结算校对
        End If
    ElseIf Err <> 0 Then
        If ErrCenter() = 1 Then
            Resume
...
Public Sub SaveInsureBalance(ByVal lng病人ID As Long, ByVal lng结帐ID As Long, _
...
Public Function CorrectInsureErrBalance(ByVal lng病人ID As Long, _
    ...
    If zlInsureCheck(strBalanceSaved, strBalance) Then
        Call SaveInsureBalance(lng病人ID, lng结帐ID, strBalance, blnDel)
    End If
...
Public Function GetChargeBalance(ByVal lng结帐ID As Long) As ADODB.Recordset
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "获取结算数据", lng结帐ID)
    Exit Function
ErrHandler:
    If ErrCenter() = 1 Then
        Resume
