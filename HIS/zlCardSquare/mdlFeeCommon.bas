...
Public Function GetShareInvoiceGroupID(ByVal bytKind As Byte) As ADODB.Recordset
    ...
    ElseIf bytKind = 6 Then
        '消费卡
        strSQL = "" & _
        "   Select A.ID,nvl(To_Char(M.编号),' ') as 使用类别编码,M.编号 As 使用类别ID,M.名称 As 使用类别,A.领用人,A.登记时间," & _
        "          A.开始卡号 As 开始号码,A.终止卡号 As 终止号码,A.剩余数量 " & _
        "   From 消费卡领用记录 A,人员表 B,消费卡类别目录 M" & vbNewLine & _
        "   Where A.使用方式=2 And A.剩余数量>0 And A.领用人=B.姓名" & _
        "         And A.接口编号 = M.编号(+) " & _
        "         And (B.站点='" & gstrNodeNo & "' Or B.站点 is Null)" & vbNewLine & _
        "   Order by 使用类别编码,剩余数量 Desc"
    Else
...
Public Function zlGetSpecialItemFee(strClass As String, Optional ByVal strPriceGrade As String, Optional ByVal lng收费细目ID As Long) As Recordset
    ...
    Else
        strSQL = _
            ...
            strWherePriceGrade
    End If
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "获取特定项目的费用集", strClass, strPriceGrade, lng收费细目ID)
...
Public Function zlGetSaveCardFeeSQL(ByVal lngCardTypeID As Long, bytStyle As Byte, strNO As String, lng病人ID As Long, lng主页ID As Long, _
        ...
        Optional lng刷卡类别ID As Long, Optional bln消费卡 As Boolean, Optional str刷卡卡号 As String, Optional lng结帐ID As Long, Optional str摘要 As String, _
        Optional ByVal cur支付金额 As Double) As String
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:产生一条医疗卡费用记录SQL语句
    ...
    '0-不明确,1-病人科室,2-病人病区,3-操作员科室,4-指定科室,5-院外执行(预留,程序暂未用),6-开单人科室
     lngUnitID = zlGetCardFeeExcuteDeptID(Val(nvl(rsMoney!收费细目ID)), Val(nvl(rsMoney!科室标志)), lng病人科室ID)
    'Zl_医疗卡记录_Insert
...
Public Function zlAddUpdateSwapSQL(ByVal bln预交 As Boolean, ByVal strIDs As String, _
    ...
    ByVal str卡号 As String, ByVal str交易流水号 As String, ByVal str交易说明 As String, _
    ByRef cllPro As Collection, Optional ByVal int校对标志 As Integer = 0, _
    Optional ByVal int发送标志 As Integer = 0, Optional ByVal bln消费卡管理 As Boolean) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:更新三方交易流水号和流水说明
    '入参: bln预交款-是否预交款
...
Public Function zlAddThreeSwapSQLToCollection(ByVal bln预交款 As Boolean, _
    ...
    ByVal str卡号 As String, ByVal strExpend As String, ByRef cllPro As Collection, _
...
Public Function zlLoadDefaultAddr(ByVal objCtrl As PatiAddress) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:读取区域表中的缺省地址
    '入参:objCtrl-结构化地址控件
    '返回:
