...
Private Function LoadData() As Boolean
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim rsTemp As ADODB.Recordset, objItem As Object
    On Error GoTo errHandle
    
    If gobjOneCardComLib.zlGetOneCardTypes(rsTemp) = False Then Exit Function
    With lvwSel
        .ListItems.Clear
        Do While Not rsTemp.EOF
            If (mstrCardTypeIDs = "" Or InStr("," & mstrCardTypeIDs & ",", "," & rsTemp!id & ",") > 0) And Val(nvl(rsTemp!是否启用)) = 1 Then
