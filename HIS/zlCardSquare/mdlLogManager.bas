...
Public Const G_STR_PROJECT = "zl9CardSquare"
...
Public Sub WritLogCall(strFuncName As String, strCallName As String, lcsCurentLogCallState As gLogCallState, ParamArray arrPars() As Variant)
    ...
    If gobjLog Is Nothing Then
        If mblnCreateLog Then Exit Sub    '只创建一次.
...
End Sub
Public Sub WritLog(strFuncName As String, strLogInfor As String, ParamArray arrPars() As Variant)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:日志跟踪
    '入参:
    '返回:成功返回true,否则返回False
