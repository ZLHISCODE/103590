...
Private Function GetBoundData() As Boolean
    ...
    If UBound(marrInput) >= 0 Then
        Set mrsBindings = zlDatabase.OpenSQLRecordByArray(mstrSql, mstrWinTittle & "选择", marrInput)
    Else
        Set mrsBindings = zlDatabase.OpenSQLRecord(mstrSql, mstrWinTittle & "选择", mstrSql)
    End If
    '没有数据则返回
...
Public Function ShowSelect(ByVal frmMain As Form, ByVal cnOracle As ADODB.Connection, ByVal lngSys As Long, _
    ...
    ByRef blnUserCancel As Boolean, _
...
Private Sub InitCtrlLocal()
   ...
   '通过Api计算出控件的相关坐标信息
    ...
    Case Else
        vRect = zlControl.GetControlRect(mobjControl.hWnd)
        sngX = vRect.Left - 15
        sngY = vRect.Top
