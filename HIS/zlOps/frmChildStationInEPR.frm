...
Object = "{099B2A6C-9CCE-43CF-AEF0-C526C98F4B7F}#4.0#0"; "zlRichEditor.ocx"
...
Public Sub zlDefCommandBars(ByVal cbsMain As CommandBars)
    ...
    Set objControl = NewCommandBar(objMenu, xtpControlButton, conMenu_Edit_Audit, "审核(&U)"): objControl.BeginGroup = True
    Set objControl = NewCommandBar(objMenu, xtpControlButton, conMenu_Edit_Archive, "归档(&I)")
    Set objControl = NewCommandBar(objMenu, xtpControlButton, conMenu_Edit_UnArchive, "撤档(&C)")
    ...
    Set objControl = NewToolBar(objBar, xtpControlPopup, conMenu_Edit_NewItem, "新增", True, , , , objControl.Index + 1)
    Set objControl = NewToolBar(objBar, xtpControlButton, conMenu_Edit_Modify, "修改", , , , , objControl.Index + 1)
        
    Set objControl = NewToolBar(objBar, xtpControlButton, conMenu_Edit_Audit, "审核", , , , , objControl.Index + 1)
    Set objControl = NewToolBar(objBar, xtpControlButton, conMenu_Edit_Archive, "归档", , , , , objControl.Index + 1)
    
    '命令的快键绑定:公共部份主界面已处理
    '------------------------------------------------------------------------------------------------------------------
    With cbsMain.KeyBindings
        .Add FCONTROL, vbKeyU, conMenu_Edit_Audit              '保存
    End With
    ...
    Case conMenu_Edit_Modify
        ...
        With vsf(0)
            ...
            If rs("名称").Value = "麻醉记录" And zlCommFun.NVL(rs("保留").Value, 0) = -1 Then
                ...
                '特别的处理方式
'                Call mobjDocNarcosis.ShowCaseNarcosis(mlng病人id, mlng主页id, Val(.RowData(.Row)), mlng科室ID, 2, mfrmMain, True)
            ...
            For Each frmAudit In Forms
                If frmAudit.Name = "frmMain" Then
                    If frmAudit.Document.EPRPatiRecInfo.ID = Val(.RowData(.Row)) _
                        And frmAudit.Document.EPRPatiRecInfo.病人来源 = cprPF_住院 And frmAudit.Document.EPRPatiRecInfo.病人ID = mlng病人id _
                        And frmAudit.Document.EPRPatiRecInfo.主页ID = mlng主页id And frmAudit.ChildMode = False Then
                        frmAudit.Show
                        bFindedAudit = True
                    End If
                End If
...
'                Set mobjDoc = New cEPRDocument
                mobjDoc.InitEPRDoc cprEM_修改, cprET_单病历审核, Val(.RowData(.Row)), cprPF_住院, mlng病人id, CStr(mlng主页id)
                mobjDoc.ShowEPREditor mfrmMain
            End If
        End With
    '------------------------------------------------------------------------------------------------------------------
    Case conMenu_Edit_Archive
        With vsf(0)
            If Trim(.TextMatrix(.Row, .ColIndex("归档人"))) = "" Then
                If Trim(.TextMatrix(.Row, .ColIndex("病人状态"))) = "在院" Then
                
                    strInfo = "真的将该份“" & .TextMatrix(.Row, .ColIndex("病历名称")) & "”归档吗？"
                    If MsgBox(strInfo, vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Sub
                    
                    gstrSQL = "Zl_电子病历记录_Archive(" & Val(.RowData(.Row)) & ",0)"
                    
                Else
                    
                    strInfo = "病人已经" & Trim(.TextMatrix(.Row, .ColIndex("病人状态"))) & "，真的将该份“" & .TextMatrix(.Row, .ColIndex("病历名称")) & "”归档吗？"
                    If MsgBox(strInfo, vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Sub
    ...
    Case conMenu_Edit_UnArchive
        With vsf(0)
            If Trim(.TextMatrix(.Row, .ColIndex("归档人"))) <> "" Then
                ...
                strInfo = "需要撤销“" & .TextMatrix(.Row, .ColIndex("病历名称")) & "”的归档？"
                If MsgBox(strInfo, vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Sub
                
                gstrSQL = "Zl_电子病历记录_Archive(" & Val(.RowData(.Row)) & ",1)"
...
Public Sub zlUpdateCommandBars(ByVal Control As CommandBarControl)
    ...
    With vsf(0)
        ...
        Case conMenu_Edit_Modify
            ...
            If Control.Enabled Then Control.Enabled = (Val(.TextMatrix(.Row, .ColIndex("处理状态"))) <= 0)  '已经进入后续处理的病历不能处理

            If Control.Enabled Then
                If Trim(.TextMatrix(.Row, .ColIndex("完成时间"))) = "" Then
                    
                    Control.Enabled = (Trim(.TextMatrix(.Row, .ColIndex("创建人"))) = Trim(UserInfo.姓名))
                    
                ElseIf Trim(.TextMatrix(.Row, .ColIndex("归档人"))) = "" And Val(.TextMatrix(.Row, .ColIndex("当前版本"))) <= 1 And InStr(1, ",1,2,4,", Val(.TextMatrix(.Row, .ColIndex("签名级别")))) > 0 Then
                    Control.Enabled = (InStr(1, .TextMatrix(.Row, .ColIndex("保存人")), Trim(UserInfo.姓名)) > 0)
                Else
        ...
        Case conMenu_Edit_Delete
            ...
            If Control.Enabled Then Control.Enabled = (Val(.TextMatrix(.Row, .ColIndex("处理状态"))) <= 0)          '已经进入后续处理的病历不能处理
            If Control.Enabled Then Control.Enabled = (Trim(.TextMatrix(.Row, .ColIndex("完成时间"))) = "")         '未完成病历可以删
            If Control.Enabled Then Control.Enabled = (Trim(.TextMatrix(.Row, .ColIndex("创建人"))) = Trim(UserInfo.姓名))
                    
                    
        '--------------------------------------------------------------------------------------------------------------
        Case conMenu_Edit_Audit
            
            Control.Visible = InStr(mstrPrivs, "病历审阅") > 0
            Control.Enabled = (mblnAllowModify And mlngKey > 0 And Val(.RowData(.Row)) > 0 And Val(.TextMatrix(.Row, .ColIndex("本次"))) = 1 And Control.Visible)
            
            If Control.Enabled Then Control.Enabled = (Val(.TextMatrix(.Row, .ColIndex("处理状态"))) <= 0)                                                      '已经进入后续处理的病历不能处理
            If Control.Enabled Then Control.Enabled = (Val(.TextMatrix(.Row, .ColIndex("病历种类"))) <> 2 Or Val(.TextMatrix(.Row, .ColIndex("保留"))) >= 0)    '特殊的住院病历即“麻醉记录”不提供审阅
            If Control.Enabled Then Control.Enabled = (Trim(.TextMatrix(.Row, .ColIndex("完成时间"))) <> "")                                                    '完成病历才可以审
            If Control.Enabled Then Control.Enabled = (Trim(.TextMatrix(.Row, .ColIndex("归档人"))) = "")                                                       '未归档病历可以审
            
        '--------------------------------------------------------------------------------------------------------------
        Case conMenu_Edit_Archive, conMenu_Edit_UnArchive
            
            Control.Visible = InStr(mstrPrivs, "病历归档") > 0
            
            Control.Enabled = (mblnAllowModify And mlngKey > 0 And Val(.RowData(.Row)) > 0 And Val(.TextMatrix(.Row, .ColIndex("本次"))) = 1 And Control.Visible)
            
            If Control.Enabled Then Control.Enabled = (Val(.TextMatrix(.Row, .ColIndex("处理状态"))) <= 0)          '已经进入后续处理的病历不能处理
            If Control.Enabled Then Control.Enabled = (Val(.TextMatrix(.Row, .ColIndex("签名级别"))) <> 0)          '当前版本已经签名完成才可以归档
            
            If Control.ID = conMenu_Edit_Archive Then
                If Control.Enabled Then Control.Enabled = (.TextMatrix(.Row, .ColIndex("归档人")) = "")
            Else
                If Control.Enabled Then Control.Enabled = (.TextMatrix(.Row, .ColIndex("归档人")) <> "")
            End If
...
Public Sub zlPopupCommandBars(ByVal CommandBar As CommandBar)
    ...
    Case conMenu_Edit_NewItem
        With CommandBar.Controls
            ...
            strSQL = "Select a.ID,a.名称,a.保留 From 病历文件列表 a,病历时限要求 b Where a.Id=b.文件id And ((b.事件='手术' And 种类=2) or 种类=4)"
...
Private Function ExecuteCommand(strCommand As String, ParamArray varParam() As Variant) As Boolean
    ...
    Case "初始数据"
        ...
        With mclsVsf
            ...
            Call .AppendColumn("处理状态", 0, flexAlignLeftCenter, flexDTString, "", , True, , , True)
            Call .AppendColumn("归档人", 0, flexAlignLeftCenter, flexDTString, "", , True, , , True)
            Call .AppendColumn("保留", 0, flexAlignLeftCenter, flexDTString, "", , True, , , True)
            Call .AppendColumn("病历种类", 0, flexAlignLeftCenter, flexDTString, "", , True, , , True)
            Call .AppendColumn("完成时间", 0, flexAlignLeftCenter, flexDTString, "", , True, , , True)
            Call .AppendColumn("签名级别", 0, flexAlignLeftCenter, flexDTString, "", , True, , , True)
            Call .AppendColumn("病人状态", 0, flexAlignLeftCenter, flexDTString, "", , True, , , True)
    ...
    Case "读取数据"
        ...
        gstrSQL = "Select Decode(Sign(r.处理状态),1,'转交',Decode(r.归档人,Null,Decode(Sign(Nvl(r.最后版本,0)-1),1,'修订','书写'),'归档')) As 图标," & _
                "        r.Id, r.病历名称, r.创建人 As 创建人,Decode(s.记录id,Null,0,[3],1,0) As 本次," & _
                "        r.创建时间, r.保存人,r.病历种类," & _
                "        r.完成时间, r.最后版本 As 当前版本,r.签名级别," & _
                ...
                "        Where d.种类 In (2, 4, 6) And d.种类 = f.种类 And d.页面 = f.编号) f" & _
                " Where r.文件id = f.文件id(+) And r.病人来源 = 2 And r.病历种类 In (2, 4, 6) And r.科室id = d.Id And r.病人id = [1] And r.主页id = [2] And s.文件id(+)=r.ID" & _
...
Private Sub vsf_BeforeUserResize(Index As Integer, ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    Cancel = (Col = vsf(Index).ColIndex("图标"))
End Sub
