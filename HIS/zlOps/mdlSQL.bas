    ...
    Case SQL.材料项目选择
        ...
        strSQL = strSQL & _
                  "Select A.材料ID AS ID, " & _
                     ...
                     "B.规格,B.产地, " & _
                     "B.计算单位 as 单位, " & _
                     "B.是否变价,D.原价 as 最低现价,D.现价 as 最高现价,DECODE(B.是否变价,1,TRIM(TO_CHAR(D.缺省价格,'999999990.99')),TRIM(TO_CHAR(D.现价,'999999990.99'))) as 单价 " & _
                "FROM 材料特性 A,收费项目目录 B,诊疗项目目录 C,收费价目 d  " & _
               "Where A.材料id+0=B.ID AND (B.撤档时间 = To_Date('30000101', 'YYYYMMDD') Or B.撤档时间 is NULL) " & _
                    "AND C.ID=A.诊疗id+0 And d.收费细目ID=b.id And d.执行日期<=SYSDATE AND (d.终止日期>=SYSDATE OR d.终止日期 IS NULL)"
    '------------------------------------------------------------------------------------------------------------------
    Case SQL.材料项目过滤
            
        strSQL = "SELECT A.是否变价,B.原价 as 最低现价,B.现价 as 最高现价,C.名称 AS 类别,A.编码,A.名称,A.规格,A.产地,A.计算单位,DECODE(A.是否变价,1,TRIM(TO_CHAR(B.缺省价格,'999999990.99')),TRIM(TO_CHAR(B.现价,'999999990.99'))) as 单价,A.ID  " & _
    ...
    Case SQL.收费执行科室
        ...
        Case "5", "6", "7", "4"
            strSQL = "Select a.ID,a.编码,a.名称" & vbNewLine & _
                        "From  部门表 a,收费执行科室 b " & vbNewLine & _
                        "Where b.收费细目ID=[2] And a.ID=b.执行科室id And (a.撤档时间 IS NULL OR a.撤档时间 =TO_DATE('3000-01-01','YYYY-MM-DD'))" & vbNewLine & _
