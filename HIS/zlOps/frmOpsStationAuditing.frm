...
Private Function ExecuteCommand(strCommand As String, ParamArray varParam() As Variant) As Boolean
    ...
    Case "读取数据"
        ...
        gstrSQL = "Select Trim(Substrb(Nvl(a.内容, ' '), 1, 200)) As 内容 From 病人医嘱附件 a Where a.医嘱id=[1] And a.项目 = '术前诊断'"
        Set rs = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, mlngKey)
        If rs.BOF = False Then
            
            With vsfDiagnose

                .TextMatrix(1, .ColIndex("诊断描述")) = zlCommFun.NVL(rs("内容").Value)
...
Private Function SaveData(ByRef lngOrderKey As Long) As Boolean
    ...
    With vsfDiagnose
        For lngLoop = 1 To .Rows - 1
            ...
            If .TextMatrix(lngLoop, .ColIndex("诊断描述")) <> "" Then
                intNo = intNo + 1
                strSQL = "ZL_病人诊断记录_INSERT(" & lng病人id & "," & IIf(lng主页id > 0, lng主页id, ZVal(lng挂号id)) & ",1,NULL,8," & ZVal(.TextMatrix(lngLoop, .ColIndex("疾病id"))) & "," & ZVal(.TextMatrix(lngLoop, .ColIndex("诊断id"))) & ",NULL,'" & .TextMatrix(lngLoop, .ColIndex("诊断描述")) & "',NULL,NULL,NULL,SYSDATE," & lngOrderKey & "," & intNo & ")"
                Call SQLRecordAdd(rsSQL, strSQL)
                
                strSQL = "ZL_病人诊断记录_INSERT(" & lng病人id & "," & IIf(lng主页id > 0, lng主页id, ZVal(lng挂号id)) & ",1,NULL,9," & ZVal(.TextMatrix(lngLoop, .ColIndex("疾病id"))) & "," & ZVal(.TextMatrix(lngLoop, .ColIndex("诊断id"))) & ",NULL,'" & .TextMatrix(lngLoop, .ColIndex("诊断描述")) & "',NULL,NULL,NULL,SYSDATE," & lngOrderKey & "," & intNo & ")"
                Call SQLRecordAdd(rsSQL, strSQL)
