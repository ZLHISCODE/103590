...
Private mblnNotAutoCharge As Boolean
...
Public Function GetRecordID() As Long
    
    With vsf(mintIndex)
        GetRecordID = Val(.RowData(.Row))
    End With
End Function

Public Property Get NotAutoCharge() As Boolean
    NotAutoCharge = mblnNotAutoCharge
End Property
...
Private Property Let AutoRefresh(vData As Boolean)
    ...
    If vData = True Then
        mlngCountTmr = 0
        tmr.Tag = Val(GetPara("自动刷新间隔", mlng模块号, , 0))
...
Private Sub zlDefCommandBars(ByVal strMenuKind As String)
    ...
    Case "概要", "药品", "材料", "治疗"
        ...
        Set objControl = NewCommandBar(objMenu, xtpControlButton, conMenu_Edit_Compend, "生成方案(&R)", True)
    End Select
...
Private Sub SubWinDefCommandBar(ByVal objItem As TabControlItem)
    ...
    Me.Caption = "手术室工作站 - " & objItem.Caption
    ...
    mstrFindKey = Trim(GetPara("定位依据", mlng模块号, , "姓名"))
    ...
    Set objControl = NewCommandBar(mobjFindKey, xtpControlButton, conMenu_View_LocationItem, "&2.床号", , , "床号")
    Set objControl = NewCommandBar(mobjFindKey, xtpControlButton, conMenu_View_LocationItem, "&3.住院号", , , "住院号")
    Set objControl = NewCommandBar(mobjFindKey, xtpControlButton, conMenu_View_LocationItem, "&4.门诊号", , , "门诊号")
    Set objControl = NewCommandBar(mobjFindKey, xtpControlButton, conMenu_View_LocationItem, "&5.身份证", , , "身份证")
    Set objControl = NewCommandBar(mobjFindKey, xtpControlButton, conMenu_View_LocationItem, "&6.IC卡号", , , "IC卡号")
    Set objControl = NewCommandBar(mobjFindKey, xtpControlButton, conMenu_View_LocationItem, "&7.手术名称", , , "手术名称")
    ...
    Set objControl = cbsMain.ActiveMenuBar.Controls.Add(xtpControlButton, conMenu_View_ReadIC, "读卡")
    ...
    With cbsMain.KeyBindings
        ...
        .Add FCONTROL, vbKeyF12, conMenu_View_ReadIC
...
Private Sub InitTabControl()
    ...
    With tbcPage
        ...
        mcolSubForm.Add mfrmChildStationOutLine, "_概要"
        mcolSubForm.Add mfrmChildStationDrug, "_药品"
        mcolSubForm.Add mfrmChildStationMaterial, "_材料"
        mcolSubForm.Add mfrmChildStationCure, "_治疗"
        mcolSubForm.Add mclsExpenses.zlGetForm, "_附费"
        mcolSubForm.Add mclsOutAdvices.zlGetForm, "_门诊医嘱"
        mcolSubForm.Add mclsInAdvices.zlGetForm, "_住院医嘱"
        mcolSubForm.Add mfrmChildStationInEPR, "_住院病历"
        mcolSubForm.Add mfrmChildStationOutEPR, "_门诊病历"
...
Private Function ExecuteCommand(strCommand As String, ParamArray varParam() As Variant) As Boolean
    ...
    Case "初始控件"
        For intCount = 0 To 2
            ...
            With mclsVsf(intCount)
                ...
                Call .AppendColumn("身份证", 0, flexAlignLeftCenter, flexDTString, "", "身份证号", True, , , True)
                Call .AppendColumn("IC卡", 0, flexAlignLeftCenter, flexDTString, "", , True, , , True)
    ...
    Case "初始数据"
        ...
        strStart = GetDateTime(GetPara("待手术时间范围", mlng模块号, , "今  天"), 1)
        strEnd = GetDateTime(GetPara("待手术时间范围", mlng模块号, , "今  天"), 2)
        ...
        strStart = GetDateTime(GetPara("已完手术时间范围", mlng模块号, , "今  天"), 1)
        strEnd = GetDateTime(GetPara("已完手术时间范围", mlng模块号, , "今  天"), 2)
        If strStart = "" Then strStart = GetDateTime("今  天", 1)
        If strEnd = "" Then strEnd = GetDateTime("今  天", 2)
        strEnd = Format(strEnd, "yyyy-MM-dd") & " 23:59:59"
    ...
    Case "本地参数设置"
        ...
        ExecuteCommand = frmOpsStationPara.ShowPara(Me, mstrPrivs)
        GoTo errEnd
    ...
    Case "读取申请信息"
        ...
        gstrSQL = "SELECT Distinct B.当前床号,b.门诊号,b.住院号,A.ID,B.姓名,C.发送号,A.医嘱内容,A.开嘱医生,A.开嘱时间,A.开始执行时间,Decode(A.病人来源,1,'门诊',2,'住院',3,'外来') AS 来源,Decode(A.病人来源,1,B.就诊诊室,2,E.名称) AS 病人科室,G.名称 AS 申请科室 " & _
        ...
        With vsf(mintIndex)
            ...
            If rs.BOF = False Then
                ...
                lblValue(0).Caption = Format(zlCommFun.NVL(rs("开始执行时间")), "YYYY-MM-DD HH:MM")
    ...
    Case "装载等待手术"
        ...
        If Split(mstrCondition, ";")(0) <> "" Then
            strTmp = " AND a.开始执行时间 BETWEEN [2] AND [3] "
        End If
        ...
        gstrSQL = "Select   /*+rule*/ Decode(e.ID,Null,-100,e.ID) As ID,Decode(e.手术状态,1,'审核',2,'安排',3,'手术',4,'完成','申请') As 图标,a.Id As 医嘱id," & vbNewLine & _
    ...
    Case "读取手术概要"
        ...
        With vsf(mintIndex)
            Call mfrmChildStationOutLine.RefreshData(Val(.RowData(.Row)), (Val(.TextMatrix(.Row, .ColIndex("手术状态"))) > 0 And Val(.TextMatrix(.Row, .ColIndex("手术状态"))) <= 3 And IsPrivs(mstrPrivs, "概要登记")), mlng手术部门id)
        End With
    ...
    Case "读取病人医嘱"
        ...
        With vsf(mintIndex)
            ...
            Case "门诊"
                If GetInsidePrivs(p门诊医嘱下达, True) <> "" Then
                    If Val(.TextMatrix(.Row, .ColIndex("病人id"))) > 0 Then
                        Call mclsOutAdvices.zlRefresh(Val(.TextMatrix(.Row, .ColIndex("病人id"))), .TextMatrix(.Row, .ColIndex("挂号单")), Val(.TextMatrix(.Row, .ColIndex("手术状态"))) <= 3 And Val(.TextMatrix(.Row, .ColIndex("手术状态"))) > 0, False, Val(.TextMatrix(.Row, .ColIndex("医嘱id"))))
                    Else
            ...
            Case "住院"
                ...
                If GetInsidePrivs(p住院医嘱下达, True) <> "" Then
                    If Val(.TextMatrix(.Row, .ColIndex("病人id"))) > 0 Then
                        ...
                        Select Case Val(.TextMatrix(.Row, .ColIndex("手术状态")))
                        Case Is > 3, 0
                            int类型 = 4
                        End Select
                                              
                        Call mclsInAdvices.zlRefresh(Val(.TextMatrix(.Row, .ColIndex("病人id"))), Val(.TextMatrix(.Row, .ColIndex("主页id"))), Val(.TextMatrix(.Row, .ColIndex("当前病区id"))), Val(.TextMatrix(.Row, .ColIndex("当前科室id"))), int类型, False, Val(.TextMatrix(.Row, .ColIndex("医嘱id"))), Val(.TextMatrix(.Row, .ColIndex("执行状态"))))
                    Else
            ...
            Case Else
                ...
                If GetInsidePrivs(p门诊医嘱下达, True) <> "" Then Call mclsOutAdvices.zlRefresh(0, "", False)
                If GetInsidePrivs(p住院医嘱下达, True) <> "" Then Call mclsInAdvices.zlRefresh(0, 0, 0, 0, CDate(0), 0)
            ...
            Case "门诊"
                ...
                If rs.BOF = False Then
                    Call mfrmChildStationOutEPR.RefreshData(Val(.RowData(.Row)), Val(.TextMatrix(.Row, .ColIndex("病人id"))), _
                                                            ...
                                                            mlng手术部门id, _
                                                            Val(.TextMatrix(.Row, .ColIndex("医嘱id"))), _
                                                            (Val(.TextMatrix(.Row, .ColIndex("手术状态"))) >= 1))
                Else
    ...
    Case "安排手术申请"
        ...
        With vsf(mintIndex)
            If Val(.TextMatrix(.Row, .ColIndex("医嘱id"))) = 0 Or mintIndex <> 0 Then GoTo errEnd
            If Not (Val(.TextMatrix(.Row, .ColIndex("手术状态"))) = 1 Or Val(.TextMatrix(.Row, .ColIndex("手术状态"))) = 2) Then GoTo errEnd
...
'        mblnNew = False
    ...
    Case "读注册表"
        ...
        mblnNotAutoCharge = (Val(GetPara("发送时不自生成费用", Me.模块号, , "0")) = 1)
    ...
    Case "写注册表"
        ...
        If Val(zlDatabase.GetPara("使用个性化风格")) = 1 Then
            '使用个性化设置
            Call SetPara("定位依据", mstrFindKey, mlng模块号)
        End If
...
Private Sub cbsMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Edit_Compend
        ...
        Call frmOpsStationScheme.ShowEdit(Me)
...
'                mblnNew = False
    ...
    Case conMenu_View_ReadIC '读IC卡
        ...
        If Not mobjICCard Is Nothing Then
            txtLocation.Text = mobjICCard.Read_Card(Me)
            If txtLocation.Text <> "" Then
                Call txtLocation_KeyPress(13)
            End If
        End If
    ...
    Case Else
        ...
        If Between(Control.ID, conMenu_ReportPopup * 100# + 1, conMenu_ReportPopup * 100# + 99) And Control.Parameter <> "" Then
            '执行发布到当前模块的报表
            With vsf(mintIndex)
                Call ReportOpen(gcnOracle, Val(Split(Control.Parameter, ",")(0)), Split(Control.Parameter, ",")(1), Me, "医嘱id=" & Val(.TextMatrix(.Row, .ColIndex("医嘱id"))), "病人id=" & Val(.TextMatrix(.Row, .ColIndex("病人id"))), "主页id=" & Val(.TextMatrix(.Row, .ColIndex("主页id"))))
            End With
        Else
...
'
        ...
        Case conMenu_Edit_Compend
            ...
            Control.Visible = IsPrivs(mstrPrivs, "生成方案")
            Control.Enabled = (DataChanged = False And Val(.TextMatrix(.Row, .ColIndex("手术状态"))) >= 2 And Control.Visible)
        ...
        '--------------------------------------------------------------------------------------------------------------
        Case conMenu_View_ReadIC

            Control.Visible = (mstrFindKey = "IC卡号")
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    '------------------------------------------------------------------------------------------------------------------
    For intLoop = 1 To mcolSubForm.Count
        Unload mcolSubForm(intLoop)
    Next
...
Private Sub mfrmChildStationCure_AfterMakeCharge()
    '刷新当前手术的费用
    Call ExecuteCommand("读取手术费用")
End Sub
...
Private Sub mfrmChildStationDrug_AfterMakeCharge()
    '刷新当前手术的费用
    Call ExecuteCommand("读取手术费用")
End Sub
...
Private Sub mfrmChildStationMaterial_AfterMakeCharge()
    '刷新当前手术的费用
    Call ExecuteCommand("读取手术费用")
End Sub
...
Private Sub mfrmChildStationOutLine_AfterMakeCharge()
    Call ExecuteCommand("读取手术费用")
End Sub
...
Private Sub mobjIDCard_ShowIDCardInfo(ByVal strID As String, ByVal strName As String, ByVal strSex As String, ByVal strNation As String, ByVal datBirthDay As Date, ByVal strAddress As String)
    If txtLocation.Text = "" And Not txtLocation.Locked And Me.ActiveControl Is txtLocation Then
        txtLocation.Text = strID
        Call txtLocation_KeyPress(13)
    End If
End Sub
