...
Begin VB.Form frmChildStationOutEPR 
   BorderStyle     =   0  'None
   Caption         =   "Form1"
   ClientHeight    =   6585
   ClientLeft      =   0
   ...
   End
End
Attribute VB_Name = "frmChildStationOutEPR"
...
Public Sub zlDefCommandBars(ByVal cbsMain As CommandBars)
    ...
    Set objControl = NewCommandBar(objMenu, xtpControlButton, conMenu_Edit_Archive, "归档(&I)"): objControl.BeginGroup = True
...
'                End If
        ...
        ElseIf Val(Split(Control.Parameter, ";")(0)) > 0 Then
            ...
            Call mobjDoc.InitEPRDoc(cprEM_新增, cprET_单病历编辑, Val(Split(Control.Parameter, ";")(0)), cprPF_门诊, mlng病人id, mlng主页id, 0, mlng科室ID, mlng医嘱id)
...
'                Call mobjDocNarcosis.ShowCaseNarcosis(mlng病人id, mlng主页id, Val(.RowData(.Row)), mlng科室ID, 2, mfrmMain, True)
            ...
            Else
                ...
                If Val(.RowData(.Row)) > 0 Then
                    Call mobjDoc.InitEPRDoc(cprEM_修改, cprET_单病历编辑, Val(.RowData(.Row)), cprPF_门诊, mlng病人id, mlng主页id, 0, mlng科室ID, mlng医嘱id)
                    mobjDoc.ShowEPREditor mfrmMain
                End If
            End If
        End With
...
Public Sub zlPopupCommandBars(ByVal CommandBar As CommandBar)
    ...
    Case conMenu_Edit_NewItem
        With CommandBar.Controls
            ...
            Set rs = zlDatabase.OpenSQLRecord(strSQL, mfrmMain.Caption, 1)
...
Private Function ExecuteCommand(strCommand As String, ParamArray varParam() As Variant) As Boolean
    ...
    Case "读取数据"
        ...
        gstrSQL = "Select Decode(Sign(r.处理状态),1,'转交',Decode(r.归档人,Null,Decode(Sign(Nvl(r.最后版本,0)-1),1,'修订','书写'),'归档')) As 图标," & _
                ...
                " Where r.文件id = f.文件id(+) And r.病人来源 = 1 And r.病历种类 In (1, 5, 6) And r.科室id = d.Id And r.病人id = [1] And r.主页id = [2] And s.文件id(+)=r.ID" & _
