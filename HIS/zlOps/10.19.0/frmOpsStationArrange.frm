...
Private Function SaveData() As Boolean
    ...
    gstrSQL = "zl_病人手术记录_Arrange(" & mlngKey & "," & _
                                        ...
                                        "TO_DATE('" & Format(DateAdd("h", Val(txt(0).Text), dtp.Value), "YYYY-MM-DD HH:MM:SS") & "','YYYY-MM-DD HH24:MI:SS'),'" & _
                                        txt(1).Text & "'," & _
                                        mlngDeptKey & ",'" & _
                                        strTmp & "',2,'" & cbo.Text & "'," & chk(0).Value & "," & chk(1).Value & "," & chk(2).Value & "," & chk(3).Value & ")"
...
Private Function ExecuteCommand(strCommand As String, ParamArray varParam() As Variant) As Boolean
    ...
    '------------------------------------------------------------------------------------------------------------------
    Case "读取数据"
        gstrSQL = "SELECT 紧急程度,接台手术,无菌手术,污染手术,感染手术,手术开始时间,Ceil((手术结束时间-手术开始时间)*24) As 小时,手术间 FROM 病人手术记录 Where ID=[1]"
        Set rs = zlDatabase.OpenSQLRecord(gstrSQL, mfrmMain.Caption, mlngKey)
        If rs.BOF = False Then
            
            zlControl.CboLocate cbo, zlCommFun.NVL(rs("紧急程度").Value)
            chk(0).Value = zlCommFun.NVL(rs("接台手术").Value, 0)
            chk(1).Value = zlCommFun.NVL(rs("无菌手术").Value, 0)
