...
Public Function CreateOrderCharge(ByVal lngKey As Long, Optional ByVal strPrivs As String) As Boolean
    ...
    gstrSQL = "SELECT A.医嘱id,B.ID AS 收费细目ID," & _
                  ...
                  "B.计算单位," & _
                  "B.类别," & _
                  "C.现价 AS 单价,Decode(A.附加手术,1,Nvl(C.附术收费率,100)/100,1) As 附术率," & _
                  "D.收据费目," & _
                  "C.收入项目ID," & _
                  "A.执行科室id," & _
                  "DECODE(A.主页id,NULL,F.门诊号,0,F.门诊号,F.住院号) AS 标识号," & _
    ...
    gstrSQL = gstrSQL & _
        "SELECT Decode(GG.相关id,Null,0,Decode(GG.诊疗类别,'F',1,0)) As 附加手术,GG.诊疗项目id,AA.医嘱id,bb.No,bb.记录性质,HH.可否分零,Decode(HH.剂量系数,0,1,Null,1,HH.剂量系数) As 剂量系数,Decode(GG.病人来源,2,HH.住院包装,HH.门诊包装) As 包装,GG.病人科室id,3 AS 序号,AA.收费细目id,AA.数量,Decode(AA.执行科室id,Null,GG.执行科室id,AA.执行科室id) As 执行科室id,GG.病人id,GG.主页id ,0 AS 单价 " & _
