...
Public Function ValidData() As Boolean
    ...
    With vsf(0)
        For lngLoop = 1 To .Rows - 1
            ...
            If Val(.RowData(lngLoop)) > 0 Then
                If IsNumeric(.TextMatrix(lngLoop, .ColIndex("总量"))) = False And Trim(.TextMatrix(lngLoop, .ColIndex("总量"))) <> "" Then
    ...
    With vsf(0)
        For lngLoop = 1 To .Rows - 1
            If Val(.RowData(lngLoop)) > 0 And Val(.TextMatrix(lngLoop, .ColIndex("总量"))) >= 0 Then
                strSQL = "ZL_方案用药参考_INSERT(" & lngKey & ",'" & zlCommFun.GetNeedName(.TextMatrix(lngLoop, .ColIndex("用药类型"))) & "'," & Val(.RowData(lngLoop)) & "," & Val(.TextMatrix(lngLoop, .ColIndex("总量"))) & ",'')"
                Call SQLRecordAdd(rsSQL, strSQL)
            End If
        Next
    End With
...
Private Function ExecuteCommand(strCommand As String, ParamArray varParam() As Variant) As Boolean
    ...
    Case "初始数据"
        ...
        With mclsVsf
            ...
            If mblnAllowModify Then
                ...
                gstrSQL = "Select 编码,编码||'-'||名称 As 名称 From 手术用药类型 Order By 编码"
                Set rs = zlDatabase.OpenSQLRecord(gstrSQL, mfrmMain.Caption)
                If rs.BOF = False Then
                    .DropColData(.ColIndex("用药类型")) = vsf(0).BuildComboList(rs, "名称", "编码")
                End If
    ...
    Case "读取数据"
        ...
        gstrSQL = "SELECT D.ID,D.规格,B.计算单位 As 剂量单位,A.类型 As 用药类型,D.名称 AS 药品名称,A.总量 " & _
