...
Private Function ExecuteCommand(strCommand As String, ParamArray varParam() As Variant) As Boolean
    ...
    Case "收费执行科室"
        ...
        With vsf(0)
            ...
            Set rs = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, 0, Val(.RowData(.Row)), mlng病人科室id, UserInfo.部门ID)
    ...
    Case "生成治疗收费单"
        ...
        If MsgBox("确定要将手术治疗生成收费单吗（新单）？", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) = vbNo Then Exit Function
    ...
    Case "生成治疗记帐单"
        ...
        If MsgBox("确定要将手术治疗生成记帐单吗（新单）？", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) = vbNo Then Exit Function
    ...
    Case "生成治疗零费单"
        ...
        If MsgBox("确定要将手术治疗生成零费单吗（新单）？", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) = vbNo Then Exit Function
...
Private Sub cbsMain_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    With vsf(0)
        ...
        Case conMenu_Edit_Preferences
            ...
            Control.Enabled = blnAllowModify And mbytMode = 1
...
Private Sub mclsVsf_BeforeNewRow(ByVal Row As Long, Col As Long, Cancel As Boolean)
    With vsf(0)
        Cancel = (Val(.RowData(Row)) <= 0 Or Val(.TextMatrix(Row, .ColIndex("数量"))) = 0)
    End With
End Sub
