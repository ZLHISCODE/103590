...
Begin VB.Form frmChildSchemeMaterial 
   BorderStyle     =   0  'None
   Caption         =   "Form1"
   ClientHeight    =   5880
   ClientLeft      =   0
   ...
   Begin VB.PictureBox picPane 
      ...
      Begin VB.CommandButton cmd 
         ...
         Index           =   1
         Left            =   5730
         Picture         =   "frmChildSchemeMaterial.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   1
         TabStop         =   0   'False
   ...
   End
End
Attribute VB_Name = "frmChildSchemeMaterial"
...
Public Function ValidData() As Boolean
    ...
    With vsf(0)
        For lngLoop = 1 To .Rows - 1
            ...
            If Val(.RowData(lngLoop)) > 0 Then
                If IsNumeric(.TextMatrix(lngLoop, .ColIndex("数量"))) = False Then
                    ShowSimpleMsg "第 " & lngLoop & " 行数据输入不完整，材料的数量必须为数值型！"
                ...
                If Val(.TextMatrix(lngLoop, .ColIndex("数量"))) <= 0 Then
                    ShowSimpleMsg "第 " & lngLoop & " 行数据输入不完整，必须输入材料的数量[0-999999999]！"
                    .ShowCell .Row, .ColIndex("数量")
                    LocationGrid vsf(0)
                    Exit Function
                End If
...
Public Function SaveData(ByRef rsSQL As ADODB.Recordset, ByRef lngKey As Long) As Boolean
    ...
    strSQL = "ZL_方案材料参考_DELETE(" & lngKey & ")"
    Call SQLRecordAdd(rsSQL, strSQL)
    
    With vsf(0)
        For lngLoop = 1 To .Rows - 1
            If Val(.RowData(lngLoop)) > 0 And Val(.TextMatrix(lngLoop, .ColIndex("数量"))) > 0 Then
                strSQL = "ZL_方案材料参考_INSERT(" & lngKey & "," & Val(.RowData(lngLoop)) & "," & Val(.TextMatrix(lngLoop, .ColIndex("数量"))) & ")"
                Call SQLRecordAdd(rsSQL, strSQL)
            End If
        Next
    End With
...
Private Function ExecuteCommand(strCommand As String, ParamArray varParam() As Variant) As Boolean
    ...
    Case "初始数据"
        ...
        With mclsVsf
            ...
            End If
            Call .AppendColumn("材料名称", 2700, flexAlignLeftCenter, flexDTString, "", "名称", True)
            Call .AppendColumn("规格", 900, flexAlignLeftCenter, flexDTString, "", , True)
            Call .AppendColumn("计算单位", 750, flexAlignLeftCenter, flexDTString, "", , True)
            Call .AppendColumn("数量", 900, flexAlignLeftCenter, flexDTDecimal, "", "数次", True)
            Call .AppendColumn("", 15, flexAlignLeftCenter, flexDTString, "", , True)
            
            If mblnAllowModify Then
                Call .InitializeEdit(True, True, True)
                Call .InitializeEditColumn(.ColIndex("材料名称"), True, vbVsfEditCommand)
    ...
    Case "读取数据"
        ...
        gstrSQL = "SELECT B.计算单位,A.材料id As ID,B.名称,A.数次,B.规格 " & _
...
Private Sub cmd_Click(Index As Integer)
    ...
    Case 1
        ...
        If ShowPubSelect(Me, cmd(1), 3, "编码,1200,0,0;名称,3000,0,0;规格,900,0,0;计算单位,900,0,0", Me.Name & "\材料项目多选", "请从下表中选择一个或多个材料项目", rsData, rs, 8790, 4800, True) = 1 Then
            With vsf(0)
                For lngLoop = 1 To rs.RecordCount
                    If mclsVsf.CheckHave(zlCommFun.NVL(rs("ID").Value)) = False Then
                        ...
                        .TextMatrix(.Rows - 1, mclsVsf.ColIndex("材料名称")) = zlCommFun.NVL(rs("名称").Value)
                        .TextMatrix(.Rows - 1, mclsVsf.ColIndex("规格")) = zlCommFun.NVL(rs("规格").Value)
                        .TextMatrix(.Rows - 1, mclsVsf.ColIndex("计算单位")) = zlCommFun.NVL(rs("计算单位").Value)
    ...
    With vsf(0)
        If Col = .ColIndex("材料名称") Then
            ...
            If bytRet = 1 Then
                ...
                .TextMatrix(Row, mclsVsf.ColIndex("材料名称")) = zlCommFun.NVL(rs("名称").Value)
        ...
        If KeyCode = vbKeyReturn Then
            If Col = .ColIndex("材料名称") Then
                ...
                If ShowPubSelect(Me, vsf(0), 2, "编码,1200,0,0;名称,3000,0,0;规格,900,0,0;计算单位,900,0,0", Me.Name & "\材料项目过滤", "请从下表中选择一个材料项目", rsData, rs, 8790, 4500, , Val(.RowData(Row))) = 1 Then
                    ...
                    .EditText = zlCommFun.NVL(rs("名称").Value)
                    .TextMatrix(Row, .ColIndex("材料名称")) = zlCommFun.NVL(rs("名称").Value)
                    .TextMatrix(Row, .ColIndex("计算单位")) = zlCommFun.NVL(rs("计算单位").Value)
