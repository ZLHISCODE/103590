...
Begin VB.Form frmChildSchemeDrug 
   BorderStyle     =   0  'None
   Caption         =   "Form1"
   ClientHeight    =   6255
   ClientLeft      =   0
   ...
   Begin VB.PictureBox picPane 
      ...
      Begin VB.CommandButton cmd 
         ...
         Index           =   1
         Left            =   5730
         Picture         =   "frmChildSchemeDrug.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   1
         TabStop         =   0   'False
   ...
   End
End
Attribute VB_Name = "frmChildSchemeDrug"
...
Public Function ValidData() As Boolean
    ...
    With vsf(0)
        For lngLoop = 1 To .Rows - 1
            ...
            If Val(.RowData(lngLoop)) > 0 Then
                If IsNumeric(.TextMatrix(lngLoop, .ColIndex("总量"))) = False Then
                    ShowSimpleMsg "第 " & lngLoop & " 行数据输入不完整，药品的总量必须为数值型！"
                    .ShowCell .Row, .ColIndex("总量")
                    LocationGrid vsf(0)
                    Exit Function
                End If
                
                If Val(.TextMatrix(lngLoop, .ColIndex("总量"))) <= 0 Then
                    ShowSimpleMsg "第 " & lngLoop & " 行数据输入不完整，必须输入药品的总量[0-999999999]！"
                    .ShowCell .Row, .ColIndex("总量")
                    LocationGrid vsf(0)
                    Exit Function
                End If
                
                If Val(.TextMatrix(lngLoop, .ColIndex("总量"))) > 999999999 Then
...
Public Function SaveData(ByRef rsSQL As ADODB.Recordset, ByRef lngKey As Long) As Boolean
    ...
    strSQL = "ZL_方案用药参考_DELETE(" & lngKey & ")"
    Call SQLRecordAdd(rsSQL, strSQL)
    
    With vsf(0)
        For lngLoop = 1 To .Rows - 1
            If Val(.RowData(lngLoop)) > 0 And Val(.TextMatrix(lngLoop, .ColIndex("总量"))) > 0 Then
                strSQL = "ZL_方案用药参考_INSERT(" & lngKey & "," & Val(Left(.TextMatrix(lngLoop, .ColIndex("用药类型")), 1)) & "," & Val(.RowData(lngLoop)) & "," & Val(.TextMatrix(lngLoop, .ColIndex("总量"))) & ",'')"
                Call SQLRecordAdd(rsSQL, strSQL)
            End If
        Next
    End With
...
Private Function ExecuteCommand(strCommand As String, ParamArray varParam() As Variant) As Boolean
    ...
    Case "初始数据"
        ...
        With mclsVsf
            ...
            End If
            Call .AppendColumn("用药类型", 1200, flexAlignLeftCenter, flexDTString, "", , True)
            ...
            Call .AppendColumn("总量", 750, flexAlignLeftCenter, flexDTDecimal, "", , True)
            Call .AppendColumn("剂量单位", 810, flexAlignLeftCenter, flexDTString, "", , True)
            
            Call .AppendColumn("", 15, flexAlignLeftCenter, flexDTString, "", , True)
            
            If mblnAllowModify Then
                Call .InitializeEdit(True, True, True)
                Call .InitializeEditColumn(.ColIndex("用药类型"), True, vbVsfEditCombox, "1-术前用药|2-麻 醉 剂|3-其它用药")
                Call .InitializeEditColumn(.ColIndex("药品名称"), True, vbVsfEditCommand)
                Call .InitializeEditColumn(.ColIndex("给药途径"), True, vbVsfEditCombox)
                Call .InitializeEditColumn(.ColIndex("总量"), True, vbVsfEditText)
    ...
    Case "读取数据"
        ...
        gstrSQL = "SELECT D.ID,D.规格,B.计算单位 As 剂量单位,DECODE(A.类型,1,'1-术前用药',2,'2-麻 醉 剂',3,'3-其它用药') As 用药类型,D.名称 AS 药品名称,A.总量 " & _
...
Private Sub cmd_Click(Index As Integer)
    ...
    Case 1
        ...
        If ShowPubSelect(Me, cmd(1), 3, "编码,1200,0,;名称,2700,0,;规格,900,0,;单位,900,0,", Me.Name & "\药品项目多选", "请从下表中选择一个或多个药品项目", rsData, rs, 8790, 4800, True) = 1 Then
            With vsf(0)
                For lngLoop = 1 To rs.RecordCount
                    If mclsVsf.CheckHave(zlCommFun.NVL(rs("药品ID").Value)) = False Then

                        If Val(.RowData(.Rows - 1)) > 0 Then .Rows = .Rows + 1

                        .TextMatrix(.Rows - 1, mclsVsf.ColIndex("药品名称")) = zlCommFun.NVL(rs("名称").Value)
                        .TextMatrix(.Rows - 1, mclsVsf.ColIndex("规格")) = zlCommFun.NVL(rs("规格").Value)
                        .TextMatrix(.Rows - 1, mclsVsf.ColIndex("剂量单位")) = zlCommFun.NVL(rs("单位").Value)
                        
                        If .TextMatrix(.Row, .ColIndex("用药类型")) = "" Or Left(.TextMatrix(.Row, .ColIndex("用药类型")), 1) = "2" Then
                            .TextMatrix(.Rows - 1, mclsVsf.ColIndex("用药类型")) = "1-术前用药"
                        Else
                            .TextMatrix(.Rows - 1, mclsVsf.ColIndex("用药类型")) = .TextMatrix(.Row, .ColIndex("用药类型"))
                        End If
                        
                        If Val(.TextMatrix(.Row, .ColIndex("总量"))) = 0 Then
                            .TextMatrix(.Rows - 1, mclsVsf.ColIndex("总量")) = "1"
                        Else
                            .TextMatrix(.Rows - 1, mclsVsf.ColIndex("总量")) = .TextMatrix(.Row, .ColIndex("总量"))
                        End If
                        
                        .RowData(.Rows - 1) = zlCommFun.NVL(rs("药品ID").Value, 0)
...
Private Sub mclsVsf_BeforeNewRow(ByVal Row As Long, Col As Long, Cancel As Boolean)
    Cancel = (Val(vsf(0).RowData(Row)) <= 0 Or Val(vsf(0).TextMatrix(Row, vsf(0).ColIndex("总量"))) = 0)
End Sub
...
Private Sub vsf_CellButtonClick(Index As Integer, ByVal Row As Long, ByVal Col As Long)
    ...
    With vsf(0)
        If Col = .ColIndex("药品名称") Then
            ...
            If Left(.TextMatrix(Row, .ColIndex("用药类型")), 1) = 2 Then
            ...
            If bytRet = 1 Then
                ...
                .TextMatrix(Row, mclsVsf.ColIndex("剂量单位")) = zlCommFun.NVL(rs("单位").Value)
        ...
        If KeyCode = vbKeyReturn Then
            If Col = .ColIndex("药品名称") Then
                ...
                If ShowPubSelect(Me, vsf(0), 2, "编码,1200,0,;名称,2700,0,;规格,900,0,;单位,900,0,", Me.Name & "\药品项目过滤", "请从下表中选择一个药品项目", rsData, rs, 8790, 4500, , Val(.RowData(Row))) = 1 Then
                    ...
                    .TextMatrix(Row, .ColIndex("剂量单位")) = zlCommFun.NVL(rs("单位").Value)
