...
Private Function ExecuteCommand(strCommand As String, ParamArray varParam() As Variant) As Boolean
    ...
    Case "读取数据"
        ...
        gstrSQL = "Select a.诊疗项目id As ID,Decode(a.相关id,Null,1,0) As 缺省,Decode(a.诊疗项目id,Null,a.医嘱内容,b.名称) As 手术名称,b.操作类型 From 病人医嘱记录 a,诊疗项目目录 b Where a.诊疗项目id=b.ID(+) And [1] In (a.ID,a.相关id) And Nvl(a.诊疗类别,'F')<>'G' Order By Decode(a.相关id,Null,1,0) Desc"
...
Private Function SaveData(ByRef lngOrderKey As Long) As Boolean
    ...
    gstrSQL = "SELECT 病人id,主页id From 病人医嘱记录 Where ID=[1]"
    Set rs = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, lngOrderKey)
        ...
        For lngLoop = 1 To .Rows - 1
            If Val(.RowData(lngLoop)) > 0 Then
                If Abs(Val(.TextMatrix(lngLoop, .ColIndex("缺省")))) = 1 Then
                    strKeys = Val(.RowData(lngLoop)) & ",1" & ";" & strKeys
                Else
                    If strKeys = "" Then
                        strKeys = Val(.RowData(lngLoop)) & ",0"
                    Else
                        strKeys = strKeys & ";" & Val(.RowData(lngLoop)) & ",0"
                    End If
                End If
            End If
        Next
    ...
    If Val(cmd(0).Tag) > 0 Then strKeys = strKeys & ";" & Val(cmd(0).Tag) & ",0"
    
    lngID = zlDatabase.GetNextId("病人医嘱记录")
    
    strSQL = "zl_病人手术记录_UpdateAdvice(" & lngID & "," & lngOrderKey & ",'" & strKeys & "'," & UserInfo.部门ID & ",'" & UserInfo.姓名 & "',Sysdate,'" & UserInfo.姓名 & "')"
    Call SQLRecordAdd(rsSQL, strSQL)
    
    lng记录ID = zlDatabase.GetNextId("病人手术记录")
    strSQL = "zl_病人手术记录_Aduit(" & lng记录ID & "," & lngID & ",'" & txt(0).Text & "','" & cbo(1).Text & "','" & cbo(0).Text & "')"
    Call SQLRecordAdd(rsSQL, strSQL)
    ...
    With vsfDiagnose
        For lngLoop = 1 To .Rows - 1
            If Val(.RowData(lngLoop)) > 0 And (Val(.TextMatrix(lngLoop, .ColIndex("疾病id"))) > 0 Or Val(.TextMatrix(lngLoop, .ColIndex("诊断id"))) > 0) Then
                strSQL = "ZL_病人诊断记录_INSERT(" & lng病人id & "," & IIf(lng主页id > 0, lng主页id, ZVal(lng挂号id)) & ",1,NULL,8," & Val(.TextMatrix(lngLoop, .ColIndex("诊断id"))) & "," & Val(.TextMatrix(lngLoop, .ColIndex("疾病id"))) & ",NULL,'" & .TextMatrix(lngLoop, .ColIndex("诊断描述")) & "',NULL,NULL,NULL,SYSDATE," & lngOrderKey & ")"
                Call SQLRecordAdd(rsSQL, strSQL)
            End If
        Next
    End With
