...
Begin VB.Form frmChildStationMaterial 
   BorderStyle     =   0  'None
   Caption         =   "Form1"
   ClientHeight    =   3195
   ClientLeft      =   0
   ...
   End
End
Attribute VB_Name = "frmChildStationMaterial"
...
Private mbytMode As Byte                                            '1-准备;2-登记
        ...
        For lngLoop = 1 To .Rows - 1
            If lngLoop <> .Rows - 1 Then
                If .RowData(lngLoop) = 0 Then
                    ShowSimpleMsg "第 " & lngLoop & " 行数据输入不完整，必须输入有效的材料项目！"
            ...
            If .RowData(lngLoop) > 0 Then
                If IsNumeric(.TextMatrix(lngLoop, .ColIndex("准备数量"))) = False And mbytMode = 1 Then
                    ShowSimpleMsg "第 " & lngLoop & " 行数据输入不完整，材料的准备数量必须为数值型！"
                ...
                If Val(.TextMatrix(lngLoop, .ColIndex("准备数量"))) > 99999999 And mbytMode = 1 Then
                    ShowSimpleMsg "第 " & lngLoop & " 行数据太大，必须输入[0-99999999]内的数值！"
                    Call LocationGrid(vsf(0), lngLoop, .ColIndex("准备数量"))
                    Exit Function
                End If
                
                If IsNumeric(.TextMatrix(lngLoop, .ColIndex("实用数量"))) = False And Val(.TextMatrix(lngLoop, .ColIndex("准备数量"))) = 0 And mbytMode = 2 Then
                    ShowSimpleMsg "第 " & lngLoop & " 行数据输入不完整，材料的实用数量必须为数值型！"
                    Call LocationGrid(vsf(0), lngLoop, .ColIndex("实用数量"))
                    Exit Function
                End If
                
                If Val(.TextMatrix(lngLoop, .ColIndex("实用数量"))) <= 0 And Val(.TextMatrix(lngLoop, .ColIndex("准备数量"))) = 0 And mbytMode = 2 Then
                    ShowSimpleMsg "第 " & lngLoop & " 行数据输入不完整，必须输入材料的实用数量[0-99999999]！"
                    Call LocationGrid(vsf(0), lngLoop, .ColIndex("实用数量"))
                    Exit Function
                End If
                ...
                If StrIsValid(.TextMatrix(lngLoop, .ColIndex("附加说明")), 250) = False And mbytMode = 2 Then
                    Call LocationGrid(vsf(0), lngLoop, .ColIndex("附加说明"))
                    Exit Function
                End If
...
Public Function SaveData(ByRef rsSQL As ADODB.Recordset) As Boolean
    ...
    '
    '------------------------------------------------------------------------------------------------------------------
    strSQL = "ZL_病人手术材料_DELETE(" & mlngKey & ")"
    ...
    With vsf(0)
        For lngLoop = 1 To .Rows - 1
            If .RowData(lngLoop) > 0 Then
                strSQL = "ZL_病人手术材料_INSERT(" & mlngKey & "," & lngLoop & "," & Val(.RowData(lngLoop)) & "," & Val(.TextMatrix(lngLoop, .ColIndex("准备数量"))) & "," & Val(.TextMatrix(lngLoop, .ColIndex("实用数量"))) & "," & Val(Left(.TextMatrix(lngLoop, .ColIndex("清点结果")), 1)) & "," & Val(.TextMatrix(lngLoop, .ColIndex("执行科室id"))) & ",'" & .TextMatrix(lngLoop, .ColIndex("附加说明")) & "')"
                Call SQLRecordAdd(rsSQL, strSQL)
            End If
        Next
    End With
...
Private Function ExecuteCommand(strCommand As String, ParamArray varParam() As Variant) As Boolean
    ...
    Case "初始控件"
        ...
        With mclsVsf
            ...
            Call .AppendColumn("准备数量", 900, flexAlignLeftCenter, flexDTString, "0.00", , True)
            Call .AppendColumn("实用数量", 900, flexAlignLeftCenter, flexDTString, "0.00", , True)
            Call .AppendColumn("单位", 600, flexAlignLeftCenter, flexDTString, "", , True)
            Call .AppendColumn("清点结果", 900, flexAlignLeftCenter, flexDTString, "", , True)
            ...
            Call .AppendColumn("附加说明", 1200, flexAlignLeftCenter, flexDTString, "", , True)
        ...
        With mclsVsf
            ...
            If blnAllowModify Then
                ...
                If mbytMode = 1 Then
                    ...
                    Call .InitializeEditColumn(.ColIndex("清点结果"), False, vbVsfEditText)
                    Call .InitializeEditColumn(.ColIndex("附加说明"), False, vbVsfEditText)
                Else
                    Call .InitializeEditColumn(.ColIndex("准备数量"), False, vbVsfEditText)
                    Call .InitializeEditColumn(.ColIndex("实用数量"), True, vbVsfEditText)
                    Call .InitializeEditColumn(.ColIndex("清点结果"), True, vbVsfEditCombox, "1-完好|2-遗漏")
                    Call .InitializeEditColumn(.ColIndex("附加说明"), True, vbVsfEditText)
                End If
            Else
    ...
    Case "读取数据"
        ...
        gstrSQL = "SELECT B.计算单位 As 单位," & _
                    ...
                    "A.准备数量," & _
                    "A.实用数量," & _
                    "DECODE(A.清点结果,1,'1-完好',2,'2-遗漏',' ') AS 清点结果," & _
                    "A.附加说明,D.材料NO,D.材料性质 " & _
                    "FROM 病人手术材料 A,收费项目目录 B,部门表 C,病人手术记录 D " & _
                    "WHERE A.材料id=B.ID And D.ID=a.记录id " & _
                        "AND C.ID=A.执行科室id " & _
                        "AND A.记录id=[1] ORDER BY A.序号"
                            
        Set rs = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, mlngKey)
        If rs.BOF = False Then
            If IsNull(rs("材料NO").Value) = False Then
                Select Case rs("材料性质").Value
                Case 1
                    mobjStateInfo.Caption = "已生成收费单，单据号：" & rs("材料NO").Value
                Case 2
                    mobjStateInfo.Caption = "已生成记帐单，单据号：" & rs("材料NO").Value
                End Select
            Else
                mobjStateInfo.Caption = ""
            End If
    ...
    Case "收费执行科室"
        ...
        With vsf(0)
            ...
            gstrSQL = GetPublicSQL(SQL.收费执行科室, "4")
            Set rs = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, GetDefaultDept("4", mint病人来源), Val(.RowData(.Row)), mlng病人科室id, UserInfo.部门ID)
    ...
    Case "材料参考方案"
        ...
        With vsf(0)
            gstrSQL = GetPublicSQL(SQL.手术材料选择)

            Set rsData = zlDatabase.OpenSQLRecord(gstrSQL, mfrmMain.Caption, mlngKey)
            
            If ShowPubSelect(Me, Nothing, 3, "编码,900,0,;名称,2400,0,;规格,900,0,;数量,1200,2,;单位,810,0,", mfrmMain.Name & "\手术材料选择", "请从下面左边列表中选择手术材料参考", rsData, rs, 8790, 4500, False, , , True) = 1 Then
                
                gstrSQL = GetPublicSQL(SQL.方案材料参考)
    ...
    Case "生成材料收费单"
        ...
        If MsgBox("确定要将手术材料生成收费单吗（新单）？", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) = vbNo Then Exit Function
        
        strTmp = MakeChargeBill(mlng医嘱id, 1, "材料", False, mstrPrivs)
        If strTmp <> "" Then
            ShowSimpleMsg "材料收费单已经生成，单据号：" & strTmp
    ...
    Case "生成材料记帐单"
        ...
        If MsgBox("确定要将手术材料生成记帐单吗（新单）？", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) = vbNo Then Exit Function
        
        strTmp = MakeChargeBill(mlng医嘱id, 2, "材料", False, mstrPrivs)
        If strTmp <> "" Then
            ShowSimpleMsg "材料记帐单已经生成，单据号：" & strTmp
    ...
    Case "生成材料零费单"
        ...
        If MsgBox("确定要将手术材料生成零费单吗（新单）？", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) = vbNo Then Exit Function
        
        strTmp = MakeChargeBill(mlng医嘱id, 2, "材料", True, mstrPrivs)
        If strTmp <> "" Then
            ShowSimpleMsg "材料零费单已经生成，单据号：" & strTmp
            mobjStateInfo.Caption = "已生成记帐单，单据号：" & strTmp
            cbsMain.RecalcLayout
        End If
...
Private Function CheckStorage(ByVal intRow As Integer) As Boolean
    ...
    With vsf(0)
        ...
        gstrSQL = GetPublicSQL(SQL.出库检查方式, "4")
        Set rs = zlDatabase.OpenSQLRecord(gstrSQL, mfrmMain.Caption, "4")
...
Private Sub cbsMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Edit_MakeCharge * 2# + 1
        ...
        Call ExecuteCommand("生成材料收费单")

    '--------------------------------------------------------------------------------------------------------------
    Case conMenu_Edit_MakeCharge * 2# + 2

        Call ExecuteCommand("生成材料记帐单")
        
    '--------------------------------------------------------------------------------------------------------------
    Case conMenu_Edit_MakeCharge * 2# + 3
                
        Call ExecuteCommand("生成材料零费单")
...
Private Sub cbsMain_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Edit_Preferences
        ...
        Control.Enabled = blnAllowModify
    ...
    Case conMenu_Edit_Default
        ...
        Control.Enabled = blnAllowModify And mbytMode = 2
...
Private Sub mclsVsf_BeforeNewRow(ByVal Row As Long, Col As Long, Cancel As Boolean)
    With vsf(0)
        Cancel = (Val(.RowData(Row)) <= 0 Or Val(.TextMatrix(Row, .ColIndex("准备数量"))) + Val(.TextMatrix(Row, .ColIndex("实用数量"))) = 0)
    End With
End Sub
...
Private Sub vsf_CellButtonClick(Index As Integer, ByVal Row As Long, ByVal Col As Long)
    ...
    With vsf(0)
        If Col = .ColIndex("名称") Then
            ...
            gstrSQL = GetPublicSQL(SQL.材料项目选择)
            Set rsData = zlDatabase.OpenSQLRecord(gstrSQL, mfrmMain.Caption)
            bytRet = ShowPubSelect(Me, vsf(0), 3, "编码,1200,0,0;名称,3000,0,0;规格,900,0,0;单位,900,0,0", Me.Name & "\材料项目选择", "请从下表中选择一个材料项目", rsData, rs, 8790, 4500, , Val(.RowData(Row)))
    ...
    With vsf(0)
        If KeyCode = vbKeyReturn Then
            If Col = .ColIndex("名称") Then
                ...
                gstrSQL = GetPublicSQL(SQL.材料项目过滤, bytMode)
                ...
                If ShowPubSelect(Me, vsf(0), 2, "编码,1200,0,0;名称,3000,0,0;规格,900,0,0;计算单位,900,0,0", Me.Name & "\材料项目过滤", "请从下表中选择一个材料项目", rsData, rs, 8790, 4500, , Val(.RowData(Row))) = 1 Then
                    ...
                    .TextMatrix(Row, .ColIndex("单位")) = zlCommFun.NVL(rs("计算单位").Value)
