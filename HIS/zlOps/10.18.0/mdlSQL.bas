...
Public Function GetPublicSQL(ByVal intMenu As SQL, Optional ByVal strParam As String) As String
    ...
    Select Case intMenu
        ...
        Case SQL.手术申请记录
            ...
            Else
                If Split(strParam, ";")(0) <> "" Then strTmp = " AND a.开嘱时间 BETWEEN [3] AND [4] "
            End If
    ...
    Case SQL.材料项目选择
        ...
        strSQL = strSQL & _
                  "Select A.材料ID AS ID, " & _
                     ...
                     "B.计算单位 as 单位, " & _
                     "B.是否变价 " & _
                "FROM 材料特性 A,收费项目目录 B,诊疗项目目录 C  " & _
               "Where A.材料id=B.ID AND (B.撤档时间 = To_Date('30000101', 'YYYYMMDD') Or B.撤档时间 is NULL) " & _
                    "AND C.ID=A.诊疗id "
    '------------------------------------------------------------------------------------------------------------------
    Case SQL.材料项目过滤
            
        strSQL = "SELECT A.是否变价,B.原价 as 最低现价,B.现价 as 最高现价,C.名称 AS 类别,A.编码,A.名称,A.规格,A.计算单位,DECODE(A.是否变价,1,TRIM(TO_CHAR(B.原价,'999999990.99'))||'～'||TRIM(TO_CHAR(B.现价,'999999990.99')),TRIM(TO_CHAR(B.现价,'999999990.99'))) as 单价,A.ID  " & _
...
'        End Select
    ...
    Case SQL.方案用药参考
        ...
        strSQL = "SELECT D.ID,d.类别,D.规格,B.计算单位 As 单位,DECODE(A.类型,1,'1-术前用药',2,'2-麻 醉 剂',3,'3-其它用药') As 用药类型,D.名称 AS 药品名称,A.总量,A.总量 As 准备数量 " & _
    ...
    Case SQL.手术用药选择
        ...
        strSQL = strSQL & _
                 "SELECT DECODE(A.类型, 1, '1-术前用药', 2, '2-麻 醉 剂', 3, '3-其它用药') AS 类型, " & _
                 "      D.ID, " & _
                 "      d.编码, " & _
                 "      d.名称, " & _
                 "      d.计算单位 AS 单位, " & _
