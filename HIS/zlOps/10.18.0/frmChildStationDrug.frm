    ...
    With vsf(0)
        For lngLoop = 1 To .Rows - 1
            If .RowData(lngLoop) > 0 Then
                strSQL = "ZL_病人手术用药_INSERT(" & mlngKey & "," & Val(Left(.TextMatrix(lngLoop, .ColIndex("用药类型")), 1)) & "," & lngLoop & "," & Val(.RowData(lngLoop)) & ",'" & .TextMatrix(lngLoop, .ColIndex("药品名称")) & "'," & Val(.TextMatrix(lngLoop, .ColIndex("准备数量"))) & "," & Val(.TextMatrix(lngLoop, .ColIndex("实用数量"))) & "," & Val(.TextMatrix(lngLoop, .ColIndex("执行科室id"))) & ")"
                Call SQLRecordAdd(rsSQL, strSQL)
            End If
        Next
    End With
...
Private Function ExecuteCommand(strCommand As String, ParamArray varParam() As Variant) As Boolean
    ...
    Case "读取数据"
        ...
        gstrSQL = "SELECT a.序号," & _
                        "f.计算单位 As 单位," & _
                        "DECODE(a.类型,1,'1-术前用药',2,'2-麻 醉 剂',3,'3-其它用药') AS 用药类型," & _
                        "a.药品ID As ID," & _
                        "a.药品名称," & _
                        "a.准备总量 AS 准备数量," & _
                        "a.使用总量 AS 实用数量," & _
        ...
        If rs.BOF = False Then
            If IsNull(rs("用药NO").Value) = False Then
                Select Case rs("用药性质").Value
                Case 1
                    mobjStateInfo.Caption = "已生成收费单，单据号：" & rs("用药NO").Value
                Case 2
                    mobjStateInfo.Caption = "已生成记帐单，单据号：" & rs("用药NO").Value
                End Select
            Else
                mobjStateInfo.Caption = ""
            End If
    ...
    Case "生成药品收费单"
        ...
        If MsgBox("确定要将手术用药生成收费单吗（新单）？", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) = vbNo Then Exit Function
    ...
    Case "生成药品记帐单"
        ...
        If MsgBox("确定要将手术用药生成记帐单吗（新单）？", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) = vbNo Then Exit Function
    ...
    Case "生成药品零费单"
        ...
        If MsgBox("确定要将手术用药生成零费单吗（新单）？", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) = vbNo Then Exit Function
