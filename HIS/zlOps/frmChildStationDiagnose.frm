...
'            If Val(.RowData(lngRow)) > 0 And (Val(.TextMatrix(lngRow, .ColIndex("疾病id"))) > 0 Or Val(.TextMatrix(lngRow, .ColIndex("诊断id"))) > 0) Then
            If .TextMatrix(lngRow, .ColIndex("术前诊断")) <> "" Then
                intNo = intNo + 1
                strSQL = "ZL_病人诊断记录_INSERT(" & lng病人id & "," & IIf(lng主页id > 0, lng主页id, ZVal(lng挂号id)) & ",1,NULL,8," & ZVal(.TextMatrix(lngRow, .ColIndex("疾病id"))) & "," & ZVal(.TextMatrix(lngRow, .ColIndex("诊断id"))) & ",NULL,'" & .TextMatrix(lngRow, .ColIndex("术前诊断")) & "',NULL,NULL,NULL,SYSDATE," & lngOrderKey & "," & intNo & ")"
                Call SQLRecordAdd(rsSQL, strSQL)
            End If
        Next
    End With
    ...
    With vsf(1)
        For lngRow = 1 To .Rows - 1
            'If Val(.RowData(lngRow)) > 0 And (Val(.TextMatrix(lngRow, .ColIndex("疾病id"))) > 0 Or Val(.TextMatrix(lngRow, .ColIndex("诊断id"))) > 0) Then
            If .TextMatrix(lngRow, .ColIndex("术后诊断")) <> "" Then
                intNo = intNo + 1
                strSQL = "ZL_病人诊断记录_INSERT(" & lng病人id & "," & IIf(lng主页id > 0, lng主页id, ZVal(lng挂号id)) & ",1,NULL,9," & ZVal(.TextMatrix(lngRow, .ColIndex("疾病id"))) & "," & ZVal(.TextMatrix(lngRow, .ColIndex("诊断id"))) & ",NULL,'" & .TextMatrix(lngRow, .ColIndex("术后诊断")) & "',NULL,NULL,NULL,SYSDATE," & lngOrderKey & "," & intNo & ")"
                Call SQLRecordAdd(rsSQL, strSQL)
            End If
        Next
    End With
