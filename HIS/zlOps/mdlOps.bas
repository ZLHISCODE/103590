...
Public Function InitSysPara() As Boolean
    ...
    '费用金额小数位数
    '------------------------------------------------------------------------------------------------------------------
    strTmp = Val(zlDatabase.GetPara(9, ParamInfo.系统号, , 2))
    ...
    '票号严格控制
    '------------------------------------------------------------------------------------------------------------------
    strTmp = zlDatabase.GetPara(24, ParamInfo.系统号)
    ...
    '------------------------------------------------------------------------------------------------------------------
    '就诊卡字母前缀
    ParamInfo.就诊卡字母前缀 = zlDatabase.GetPara(27, ParamInfo.系统号)
...
Public Function CalcTimePrice(ByVal lng药品id As Long, lng药房ID As Long, ByVal sng数量 As Single) As Single
    '******************************************************************************************************************
    '功能：计算实价药品的实际出库价
    '******************************************************************************************************************
    Dim rsTmp As New ADODB.Recordset
    Dim sng住院包装 As Single
    Dim sng出库数量 As Single
    Dim cur指导零售价 As Single
    Dim cur出库金额 As Double
    ...
    gstrSQL = "Select Nvl(批次,0) as 批次,Nvl(可用数量,0) as 库存," & _
        ...
        " And (Nvl(批次,0)=0 Or 效期 is NULL Or 效期>Trunc(Sysdate)) And Nvl(可用数量, 0)>0 " & _
        " Order by Nvl(批次,0)"
    Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, "mdlOps", lng药品id, lng药房ID)
    If Not rsTmp.EOF Then
        While sng数量 > 0 And Not rsTmp.EOF
            If rsTmp!库存 > sng数量 Then
                cur出库金额 = cur出库金额 + Format(sng数量 * Format(rsTmp!时价, "0.0000000"), "0.0000000")
                sng数量 = 0
            Else
                cur出库金额 = cur出库金额 + Format(rsTmp!库存 * Format(rsTmp!时价, "0.0000000"), "0.0000000")
        ...
        If sng数量 <= 0 Then
            If sng出库数量 <> 0 Then
                CalcTimePrice = Format(cur出库金额 / sng出库数量, "0.00000")
            Else
...
Public Function MakeChargeBill(ByVal lngKey As Long, ByVal int记录性质 As Integer, ByVal strMenuItem As String, Optional ByVal blnZeroBill As Boolean = False, Optional ByVal strPrivs As String) As String
    ...
    strSQL = "Select ID From 病人手术记录 Where 医嘱id=[1]"
    Set rs = zlDatabase.OpenSQLRecord(strSQL, "mdlOps", lngKey)
    If rs.BOF = False Then
        lng手术id = Val(rs("ID").Value)
    End If
    ...
    gstrSQL = "SELECT B.ID AS 收费细目ID," & _
                  ...
                  "B.计算单位," & _
                  "B.类别," & _
                  "Decode(b.是否变价,1,c.缺省价格,c.现价) AS 单价," & _
                  "D.收据费目," & _
                  "C.收入项目ID," & _
                  "A.执行科室id," & _
    ...
    Case "手术"
        ...
        gstrSQL = gstrSQL & _
             "SELECT 0 As 可否分零,1 As 剂量系数,1 As 包装,II.病人科室id,2 AS 序号,aa.收费项目id AS 收费细目id,aa.收费数量 AS 数量,dd.手术室id As 执行科室id,DD.病人id,DD.主页id ,0 AS 单价 " & _
             "FROM (Select 诊疗项目id,记录id From 病人手术情况 Where 性质=2 And 记录id=[2] Union All Select 麻醉方式id As 诊疗项目id,ID As 记录id From 病人手术记录 Where ID=[2]) CC,病人手术记录 DD,病人医嘱记录 II,诊疗收费关系 AA " & _
             "Where CC.记录id = DD.ID And II.ID = DD.医嘱id And DD.医嘱id =[1] And aa.诊疗项目id=cc.诊疗项目id"
    ...
    gstrSQL = gstrSQL & _
            ...
            "Where C.收费细目id = B.ID " & _
               ...
               "AND A.收费细目id = B.ID " & _
               "AND F.病人id=A.病人id And Nvl(A.数量,0)>0 " & _
            "ORDER BY B.ID"
    
    Set rsCharge = zlDatabase.OpenSQLRecord(gstrSQL, "mdlOps", lngKey, lng手术id)
    ...
    Select Case strMenuItem
    Case "用药"
        strSQL = "Zl_病人手术单据_Delete(" & lng手术id & ",1,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "')"
    Case "材料"
        strSQL = "Zl_病人手术单据_Delete(" & lng手术id & ",2,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "')"
    Case "治疗"
        strSQL = "Zl_病人手术单据_Delete(" & lng手术id & ",3,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "')"
    Case "手术"
        strSQL = "Zl_病人手术单据_Delete(" & lng手术id & ",4,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "')"
    End Select
    ...
    With rsCharge
        ...
        For lngLoop = 1 To .RecordCount
            ...
            Case "4", "5", "6", "7"
                ...
                Case "5", "6", "7"
                    ...
                    If zlCommFun.NVL(rsCharge("可否分零").Value, 0) = 0 Then
                        dbl数量 = IntEx(dbl数量 / zlCommFun.NVL(rsCharge("包装").Value, 1)) * zlCommFun.NVL(rsCharge("包装").Value, 1)
                    End If
            ...
            str费别 = rsPati("费别").Value
            strSQL = "Select Zl_Actualmoney([1],[2],[3],[4],[5],[6]) As 结果 From Dual"
            Select Case rsCharge("类别").Value
            Case "5", "6", "7"
                Set rs = zlDatabase.OpenSQLRecord(strSQL, "mdlOps", str费别, Val(!收费细目id), Val(!收入项目ID), cur应收, dbl数量, lng执行部门ID)
            Case Else
                Set rs = zlDatabase.OpenSQLRecord(strSQL, "mdlOps", str费别, Val(!收费细目id), Val(!收入项目ID), cur应收, 0, 0)
            End Select
            If rs.BOF = False Then
                strTmp = Trim(zlCommFun.NVL(rs("结果").Value))
                If strTmp <> "" Then
                    If InStr(strTmp, ":") > 0 And blnZeroBill = False Then
                        cur实收 = Format(Val(Mid(strTmp, InStr(strTmp, ":") + 1)), str费用小数位)
...
'                        If 欠费情况(zlCommFun.NVL(rsPati!姓名), lng病人id, lng主页id, curMoneyTotal, bln医保, lng报警级别, bln强制记帐, str已强制报警姓名) = "是" Then
        ...
        Select Case strMenuItem
        Case "治疗"
            strSQL = "zl_病人手术单据_Update(" & lng手术id & ",3,'" & strNo & "'," & int记录性质 & ")"
        Case "用药"
            strSQL = "zl_病人手术单据_Update(" & lng手术id & ",1,'" & strNo & "'," & int记录性质 & ")"
        Case "材料"
            strSQL = "zl_病人手术单据_Update(" & lng手术id & ",2,'" & strNo & "'," & int记录性质 & ")"
        Case "手术"
            strSQL = "zl_病人手术单据_Update(" & lng手术id & ",4,'" & strNo & "'," & int记录性质 & ")"
        End Select
...
Public Function Get开嘱科室ID(ByVal lng医生ID As Long, ByVal lng病人科室ID As Long, _
    Optional ByVal int范围 As Integer = 2, Optional ByVal lng执行科室ID As Long) As Long
'功能：由医生确定开嘱科室
'参数：int范围=1-门诊,2-住院(缺省)
'说明：在医生所属科室范围内,优先顺序如下：
'      1、病人科室
'      2、服务于门诊/住院病人的某些特殊医嘱的执行科室
'      3、服务于门诊/住院病人的科室且为默认科室
'      4、服务于门诊/住院病人的科室
'      5、默认科室

    Dim rsTmp As New ADODB.Recordset
    Dim strSQL As String, i As Integer
    Dim arr科室ID(1 To 5) As Long
    ...
    strSQL = "Select Distinct C.编码,A.部门ID,Nvl(A.缺省,0) as 缺省,Nvl(B.服务对象,0) as 服务对象" & _
        ...
        " Order by C.编码"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlCISKernel", lng医生ID)
    
    For i = 1 To rsTmp.RecordCount
        If rsTmp!部门ID = lng病人科室ID Then
            arr科室ID(1) = rsTmp!部门ID
        ElseIf InStr("," & int范围 & ",3,", rsTmp!服务对象) > 0 And lng执行科室ID <> 0 And rsTmp!部门ID = lng执行科室ID Then
        ...
        ElseIf InStr("," & int范围 & ",3,", rsTmp!服务对象) > 0 Then
            If arr科室ID(4) = 0 Then arr科室ID(4) = rsTmp!部门ID
...
Public Function CreateOrderCharge(ByVal lngKey As Long, Optional ByVal strPrivs As String) As Boolean
    ...
    gstrSQL = "SELECT A.医嘱id,B.ID AS 收费细目ID," & _
                  ...
                  "B.计算单位," & _
                  "B.类别," & _
                  "Decode(b.是否变价,1,c.缺省价格,c.现价) AS 单价,Decode(A.附加手术,1,Nvl(C.附术收费率,100)/100,1) As 附术率," & _
                  "D.收据费目," & _
                  "C.收入项目ID," & _
                  "A.执行科室id," & _
                  "DECODE(A.主页id,NULL,F.门诊号,0,F.门诊号,F.住院号) AS 标识号," & _
    ...
    gstrSQL = gstrSQL & _
        "SELECT Decode(GG.相关id,Null,0,Decode(GG.诊疗类别,'F',1,0)) As 附加手术,GG.诊疗项目id,AA.医嘱id,bb.No,bb.发送号,bb.记录性质,HH.可否分零,Decode(HH.剂量系数,0,1,Null,1,HH.剂量系数) As 剂量系数,Decode(GG.病人来源,2,HH.住院包装,HH.门诊包装) As 包装,GG.病人科室id,3 AS 序号,AA.收费细目id,AA.数量,Decode(AA.执行科室id,Null,GG.执行科室id,AA.执行科室id) As 执行科室id,GG.病人id,GG.主页id ,0 AS 单价 " & _
    ...
    With rsCharge
        ...
        For lngLoop = 1 To .RecordCount
            ...
            If rsNo.RecordCount = 0 Then
                ...
                rsNo("序号").Value = 1
            Else
                rsNo("序号").Value = Val(rsNo("序号").Value) + 1
            End If
            ...
            lng执行部门ID = zlCommFun.NVL(!执行科室id, 0)
            
            Select Case rsCharge("类别").Value
            Case "5", "6", "7"
                lng执行部门ID = GetDefaultDept(rsCharge("类别").Value, int病人来源)
            ...
            '计算应收和实收金额
            '----------------------------------------------------------------------------------------------------------
            cur应收 = Format(dbl数量 * cur单价 * Val(rsCharge("附术率").Value), str费用小数位)
            cur实收 = cur应收
...
'                        If 欠费情况(zlCommFun.NVL(rsPati!姓名), lng病人id, lng主页id, curMoneyTotal, bln医保, lng报警级别, bln强制记帐, str已强制报警姓名) = "是" Then
            ...
            If int来源 = 1 Then
                If int记录性质 = 1 Then
                    ...
                    '--------------------------------------------------------------------------------------------------
                    strSQL = _
                        "zl_门诊划价记录_Insert('" & strNo & "'," & Val(rsNo("序号").Value) & "," & lng病人id & ",NULL," & _
                        ZVal(zlCommFun.NVL(rsPati!门诊号, 0)) & ",'" & zlCommFun.NVL(rsPati!付款码) & "','" & zlCommFun.NVL(rsPati!姓名) & "'," & _
                        "'" & zlCommFun.NVL(rsPati!性别) & "','" & zlCommFun.NVL(rsPati!年龄) & "','" & str费别 & "',NULL," & _
                        lng病人病区ID & "," & lng病人科室ID & "," & UserInfo.部门ID & ",'" & UserInfo.姓名 & "'," & _
                        "NULL," & lng项目ID & ",'" & !类别 & "','" & !计算单位 & "',NULL,1," & dbl数量 & "," & _
                        "0," & ZVal(lng执行部门ID) & "," & IIf(int父号 = lngLoop, "NULL", int父号) & "," & _
                        !收入项目ID & ",'" & zlCommFun.NVL(!收据费目) & "'," & cur单价 & "," & cur应收 & "," & cur实收 & "," & _
                        strDate & "," & strDate & ",NULL,'" & UserInfo.姓名 & "'," & ZVal(lng类别ID) & ",NULL," & _
                        Val(rsCharge("医嘱id").Value) & ",'" & zlCommFun.NVL(rsAdvice!执行频次) & "',NULL,NULL," & zlCommFun.NVL(rsAdvice!医嘱期效, 0) & "," & _
                        zlCommFun.NVL(rsAdvice!计价特性, 0) & ",1)"
                    Call SQLRecordAdd(rsSQL, strSQL)
                Else
                    '生成门诊记帐单据
                    '--------------------------------------------------------------------------------------------------
                    strSQL = _
                        "zl_门诊记帐记录_Insert('" & strNo & "'," & Val(rsNo("序号").Value) & "," & lng病人id & "," & _
                        ZVal(zlCommFun.NVL(rsPati!门诊号, 0)) & ",'" & zlCommFun.NVL(rsPati!姓名) & "','" & zlCommFun.NVL(rsPati!性别) & "'," & _
                        "'" & zlCommFun.NVL(rsPati!年龄) & "','" & str费别 & "',NULL," & ZVal(rsAdvice!婴儿) & "," & _
                        lng病人病区ID & "," & lng病人科室ID & "," & UserInfo.部门ID & "," & _
                        "'" & UserInfo.姓名 & "',NULL," & lng项目ID & ",'" & !类别 & "'," & _
                        "'" & !计算单位 & "',1," & dbl数量 & ",0," & ZVal(lng执行部门ID) & "," & _
                        IIf(int父号 = lngLoop, "NULL", int父号) & "," & !收入项目ID & ",'" & zlCommFun.NVL(!收据费目) & "'," & cur单价 & "," & _
                        cur应收 & "," & cur实收 & "," & strDate & "," & strDate & ",NULL,NULL,'" & UserInfo.编号 & "'," & _
                        "'" & UserInfo.姓名 & "'," & ZVal(lng类别ID) & ",NULL,NULL," & Val(rsCharge("医嘱id").Value) & "," & _
                        "'" & zlCommFun.NVL(rsAdvice!执行频次) & "',NULL,NULL," & zlCommFun.NVL(rsAdvice!医嘱期效, 0) & "," & _
                        zlCommFun.NVL(rsAdvice!计价特性, 0) & ")"
                    Call SQLRecordAdd(rsSQL, strSQL)
                End If
            Else
                ...
                '------------------------------------------------------------------------------------------------------
                strSQL = _
                    "zl_住院记帐记录_Insert('" & strNo & "'," & Val(rsNo("序号").Value) & "," & lng病人id & "," & ZVal(lng主页id) & "," & _
                    ZVal(zlCommFun.NVL(rsPati!住院号, 0)) & ",'" & zlCommFun.NVL(rsPati!姓名) & "','" & zlCommFun.NVL(rsPati!性别) & "'," & _
                    "'" & zlCommFun.NVL(rsPati!年龄) & "','" & Trim(zlCommFun.NVL(rsPati!床号)) & "','" & str费别 & "'," & _
                    lng病人病区ID & "," & lng病人科室ID & ",NULL," & ZVal(rsAdvice!婴儿) & "," & _
                    UserInfo.部门ID & ",'" & UserInfo.姓名 & "',NULL," & lng项目ID & ",'" & !类别 & "'," & _
                    "'" & !计算单位 & "'," & int保险项目否 & "," & ZVal(lng保险大类ID) & ",'" & str保险编码 & "'," & _
                    "1," & dbl数量 & ",0," & ZVal(lng执行部门ID) & "," & _
...
Public Function GetPara(ByVal varPara As Variant, Optional ByVal lngModual As Long, Optional ByVal blnPrivate As Boolean, Optional ByVal strDefault As String, Optional ByVal blnNotCache As Boolean) As String
    ...
    GetPara = zlDatabase.GetPara(varPara, ParamInfo.系统号, lngModual, strDefault)
...
Public Function SetPara(ByVal varPara As Variant, ByVal strValue As String, Optional ByVal lngModual As Long, Optional ByVal blnPrivate As Boolean) As Boolean
    ...
    SetPara = zlDatabase.SetPara(varPara, strValue, ParamInfo.系统号, lngModual)
...
Public Function AutoSendMaterial(ByVal strNo As String) As Boolean
    ...
    If Val(zlDatabase.GetPara(63, ParamInfo.系统号, , 0)) <> 1 Then
        AutoSendMaterial = True
        Exit Function
    End If
    
    '------------------------------------------------------------------------------------------------------------------
    strSQL = "SELECT DISTINCT A.执行部门id FROM 病人费用记录 A,材料特性 B WHERE A.收费细目id=B.材料id AND NVL(B.跟踪在用,0)=1 AND A.收费类别='4' and A.NO=[1]"
    Set rs = zlDatabase.OpenSQLRecord(strSQL, "mdlOps", strNo)
    If rs.BOF = False Then
        Do While Not rs.EOF
            If zlCommFun.NVL(rs("执行部门id").Value, 0) > 0 Then
                
                strSQL = "zl_材料收发记录_处方发料(" & rs("执行部门id").Value & ",25,'" & strNo & "','" & UserInfo.姓名 & "','" & UserInfo.姓名 & "','" & UserInfo.姓名 & "',1,Sysdate)"
                Call SQLRecordAdd(rsSQL, strSQL)
