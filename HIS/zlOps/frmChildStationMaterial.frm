...
Private Function InitCommandBar() As Boolean
    ...
    Set objPopup = NewToolBar(objBar, xtpControlPopup, conMenu_Edit_MakeCharge, "生成费用", True, , xtpButtonIconAndCaption)
    
    Set objControl = NewToolBar(objBar, xtpControlButton, conMenu_Edit_Preferences, "选择方案", True, , xtpButtonIconAndCaption)
...
Private Function ExecuteCommand(strCommand As String, ParamArray varParam() As Variant) As Boolean
    ...
    Case "读取数据"
        ...
        gstrSQL = "SELECT B.计算单位 As 单位," & _
                    ...
                    "A.附加说明 " & _
                    "FROM 病人手术材料 A,收费项目目录 B,部门表 C " & _
                    "WHERE A.材料id=B.ID  " & _
                        "AND C.ID=A.执行科室id " & _
    ...
    Case "生成材料收费单"
        ...
        If MsgBox("如果已生成，则会自动删除或作废，确定要将手术材料生成收费单吗（新单）？", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) = vbNo Then Exit Function
    ...
    Case "生成材料记帐单"
        ...
        If MsgBox("如果已生成，则会自动删除或作废，确定要将手术材料生成记帐单吗（新单）？", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) = vbNo Then Exit Function
    ...
    Case "生成材料零费单"
        ...
        If MsgBox("如果已生成，则会自动删除或作废，确定要将手术材料生成零费单吗（新单）？", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) = vbNo Then Exit Function
...
Private Sub vsf_CellButtonClick(Index As Integer, ByVal Row As Long, ByVal Col As Long)
    ...
    With vsf(0)
        If Col = .ColIndex("名称") Then
            ...
            bytRet = ShowPubSelect(Me, vsf(0), 3, "编码,1200,0,0;名称,1800,0,0;规格,900,0,0;产地,1200,0,0;单位,450,0,0;单价,990,0,0", Me.Name & "\材料项目选择", "请从下表中选择一个材料项目", rsData, rs, 8790, 4500, , Val(.RowData(Row)))
    ...
    With vsf(0)
        If KeyCode = vbKeyReturn Then
            If Col = .ColIndex("名称") Then
                ...
                If ShowPubSelect(Me, vsf(0), 2, "编码,1200,0,0;名称,1800,0,0;规格,900,0,0;产地,900,0,0;计算单位,900,0,0;单价,990,0,0", Me.Name & "\材料项目过滤", "请从下表中选择一个材料项目", rsData, rs, 8790, 4500, , Val(.RowData(Row))) = 1 Then
