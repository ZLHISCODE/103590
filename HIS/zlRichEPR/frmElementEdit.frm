...
'            vfg单选.Cell(flexcpChecked, 1, 0, vfg单选.Rows - 1, 0) = flexUnchecked
    ...
    Me.Left = X
    Me.Top = Y
    Me.Width = GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & "frmElementEdit", "MainWidth", 2500)
    If Me.Element.要素表示 <> 1 And (Me.Element.替换域 = 0 Or EditType = cprET_单病历编辑 Or EditType = cprET_单病历审核) Then
        Me.Height = GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & "frmElementEdit", "MainHeight", 3690)
    End If
    Call Form_Resize
...
Private Sub Form_Deactivate()
    On Error Resume Next
    RaiseEvent pCancel
    SaveSetting "ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & "frmElementEdit", "MainWidth", Me.Width
    If Me.Element.要素表示 <> 1 And (Me.Element.替换域 = 0 Or EditType = cprET_单病历编辑 Or EditType = cprET_单病历审核) Then SaveSetting "ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & "frmElementEdit", "MainHeight", Me.Height
