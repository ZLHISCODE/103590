...
Public Function DrawPatiInfo(objDraw As Object, x As Long, y As Long, strInfo As String, ByVal lngPaperWidth As Long) As Boolean
    ...
    '打印在右侧
    x = lngPaperWidth - (objDraw.TextWidth("公医号:" & Split(strInfo, "'")(8)) + W_9pt / 3)
    Call DrawText(objDraw, x, y, "公医号:", 0)
    Call DrawText(objDraw, x + objDraw.TextWidth("公医号:"), y, Split(strInfo, "'")(8), 16711680)
    x = x + objDraw.TextWidth("公医号:" & Split(strInfo, "'")(8)) + W_9pt / 3
...
Private Function DrawBodyInfo(objDraw As Object, x As Long, y As Long, ByVal colFirstWidth As Long, _
    ...
    Call DrawCell(objDraw, "手术（产）后日数", x, y + (H_9pt + H_9pt \ 2) * 2, colFirstWidth, H_9pt + H_9pt \ 2, 0, 0, , , , objDraw.Font, "1111", 1, 1, False)
...
'        End If
        '按月日格式显示
        strtmp = Mid(strtmp, 6, 2) & "月" & Mid(strtmp, 9, 2) & "日"
        ...
        If intDay <= UBound(Split(strPatiDay, "'")) + 1 Then
            Call DrawCell(objDraw, GetSplitStr(strPatiDay, intDay), lngLeft + (HOUR_STEP_Twips * 6) * intDay, y + H_9pt * 1 + H_9pt \ 2, (HOUR_STEP_Twips * 6), H_9pt + H_9pt \ 2, 0, 0, , &HFF0000, , objDraw.Font, "1111", 1, 1, False)
        Else
            Call DrawCell(objDraw, "", lngLeft + (HOUR_STEP_Twips * 6) * intDay, y + H_9pt * 1 + H_9pt \ 2, (HOUR_STEP_Twips * 6), H_9pt + H_9pt \ 2, 0, 0, , , , objDraw.Font, "1111", 1, 1, False)
        End If
...
Private Sub DrawBodyTopScale(objDraw As Object, x As Long, y As Long, Optional sngScale As Long = 1)
    ...
    For i = 1 To 7
        ...
        '原来是12小时制,是注释的代码复制两次,跑两次即实现4,8,12,4,8,12
        '改为24小时制,所以注释下面的代码,并把上面的12改为24解决
        intCount = 0
        For j = lngHourBegin To 24 - (4 - lngHourBegin) Step 4
            intCount = intCount + 1
            If intCount < 3 Then
...
'                End If
                ...
                '输出单位
                Call DrawText(objDraw, x + (lngPercW - objDraw.TextWidth(Split(aryItem(i), "'")(8))) / 2, y + (intGuageRow) * (H_9pt + H_9pt \ 2) + H_9pt * 3, Split(aryItem(i), "'")(8), lngColor)
...
'                            lng次数 = 12, "Ⅻ")
            ...
            If mblnStopFlag Then
                ...
                End If
            Else
                lngArryOPSDay(i) = IIf(lngArryOPSDay(i) <> "" And lngArryOPSDay(i) <> "-1" And strtmp <> "", strtmp & "/" & lngArryOPSDay(i), lngArryOPSDay(i))
            End If
...
'            objOut.SetPages
    ...
    If mlngFirstWidth < W_9pt * 6 Then mlngFirstWidth = W_9pt * 8
...
'                    objOut.txtPage.Text = "当前页" & Space(17) & "共 " & objOut.picPage.UBound + 1 & " 页"
        ...
        '打印医院名称,打印体温单标题
        strTmpString0 = IIf(GetUnitName = "-", "", GetUnitName) & "三测单"
        ...
        mstrSQL = "Select 医保号 From 病人信息 Where 病人ID=[1]"
        Set rsTmp = zlDatabase.OpenSQLRecord(mstrSQL, "体温单", lng病人id)
        strPatiInfo = strPatiInfo & "'" & zlCommFun.NVL(rsTmp!医保号)
