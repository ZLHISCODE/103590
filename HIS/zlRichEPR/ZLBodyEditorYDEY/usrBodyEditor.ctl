...
Private Const ROWHEIGHT = 18                        '最小单元格的高度*5,原39
Private Const MAXROWS = 108                         '原47
...
'            Call UserControl_Resize
    ...
    Case "添加项目"
        ...
        Dim rsItem As New ADODB.Recordset       '活动项目
        Dim rsPosition As New ADODB.Recordset   '活动项目对应的部位
        Dim rsTemp As New ADODB.Recordset
        Dim strNotItem As String
        Dim strPosition As String, strPosition_Sel As String
        ...
        If rsData.BOF = False Then
            If ShowTxtSelDialog(mfrmParent, Nothing, "名称,1500,0,1;单位,900,0,0;最小值,900,0,0;最大值,900,0,0", mfrmParent.Name & "\护理项目选择", "请从下面选择一个护理项目。", rsData, rsItem, 6000, 3000, , , 2, False) Then
                If rsItem.BOF = False Then
                    '提取部位
                    strPosition_Sel = GetPosition(rsItem!名称, rsItem!项目号)
                    gstrSQL = " Select rownum AS ID,部位 AS 部位名称,1 AS 末级 From 体温部位 Where 项目序号=[1] " & strPosition_Sel
                    Set rsPosition = zlDatabase.OpenSQLRecord(gstrSQL, "提取该活动项目对应的部件供操作员选择", CLng(rsItem!项目号))
                    If rsPosition.RecordCount <> 0 Then
                        If ShowTxtSelDialog(mfrmParent, Nothing, "部位名称,2000,0,1", mfrmParent.Name & "\活动项目部位选择", "请选择" & rsItem!名称 & "对应的部位。", rsPosition, rsTemp, 6000, 3000, , , 2, False) Then
                            If rsTemp.EOF = False Then
                                strPosition = rsTemp!部位名称
                            End If
                        Else
                            Exit Function
                        End If
                    End If
                    If AppendGridItem(rsItem, True, strPosition) Then
                        Call picPane_Resize
                    End If
                End If
            End If
...
'                            mshScale.TextMatrix(手术标志, intLoop) = ""
'                        Next
                        ...
                        Case "手术"
                            If intLoop >= mshScale.FixedCols And intLoop < mshScale.Cols Then
                                mshScale.TextMatrix(3, intLoop) = "手术;" & mstrOpsDays(intLoop)
                            End If
                        Case "分娩"
                            If intLoop >= mshScale.FixedCols And intLoop < mshScale.Cols Then
                                mshScale.TextMatrix(3, intLoop) = "分娩;" & mstrOpsDays(intLoop)
                            End If
                        End Select
                        mshUpTab.Tag = "填写手术日"
                    End If
...
'    Dim intPageNo As Integer
    ...
    If intPage = 256 Then
        If IsWindowsNT Then
'            If Val(GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\LocalSet\" & objReport.编号, "PaperForm", 0)) = 1 Then
'                If Not SetNTPrinterPaper_Form(frmMain.hWnd, objFmt.W / Twip_mm, objFmt.H / Twip_mm, IIf(intOrient = 0, 1, intOrient), intCopies) Then
'                    InitPrinter = False
'                End If
'            Else
                If Not SetNTPrinterPaper(Me.hWnd, lngWidth / Twip_mm, lngHeight / Twip_mm, IIf(intOrient = 0, 1, intOrient), 1) Then
                    PrintState = False
                    Exit Function
                End If
'            End If
...
Private Function Check是否包含(strSource As String) As Boolean
    '检查strSource中的每一个字符是否在strTarge中
    Dim i As Long
    Dim strTarge As String
    Check是否包含 = False
'
'    strTarge = "1234567890ZXCVBNMASDFGHJKLQWERTYUIOP[]{},.<>?/'"":;|\=+_)(*&^%$#@!`~"
...
Private Function ClearLineSelect() As Boolean
    ...
    For intLoop = 0 To picLable.UBound
        picLable(intLoop).BackColor = RGB(255, 255, 255)
    Next
End Function
        ...
        If IsNull(rs.Fields(0).Value) = False Then
            ...
            Do While Not rs.EOF
                ...
                If rs.AbsolutePosition > picLine.Count Then
                    ...
                    Load picLable(rs.AbsolutePosition - 1)
                    picLable(rs.AbsolutePosition - 1).Visible = True
                    picLable(rs.AbsolutePosition - 1).ZOrder
                End If
...
'                End If
                ...
                For intRow = 1 To mshScale.Rows - 1
                    ...
                    If intRow >= rs!最高行 And rs!最大值 - (intRow - rs!最高行) * rs!单位值 >= rs!最小值 Then
                        ...
                        '刚好为整数时求出下一刻度值
                        'Debug.Print "刚好为整数时求出下一刻度值:" & Int(rs("最大值").Value - (intRow - rs!最高行) * rs("单位值").Value), rs!最大值 - (intRow - rs!最高行) * rs!单位值
                        If Int(rs("最大值").Value - (intRow - rs!最高行) * rs("单位值").Value) = rs!最大值 - (intRow - rs!最高行) * rs!单位值 Or _
                            True Then
...
'                                mshScale.TextMatrix(intRow, rs.AbsolutePosition - 1) = rs!最大值 - (intRow - rs!最高行) * rs!单位值
   ...
   '------------------------------------------------------------------------------------------------------------------
    ...
    With mshUpTab
        ...
        .TextMatrix(0, 0) = "日       期"
        .TextMatrix(1, 0) = "住 院 日 数"
        .TextMatrix(2, 0) = "手 术 日 数"
...
Private Function AppendGridItem(ByVal rsTmp As ADODB.Recordset, Optional ByVal blnAppend As Boolean, Optional ByVal str部位 As String) As Boolean
    '******************************************************************************************************************
    '功能：填写表格项目的标题等
    '参数：rsTmp：要添加的表格项目
    '返回：
    ...
    Case Else
        ...
        Else
            If IIf(str部位 = "", NVL(rsTmp!体温部位), str部位) = "" Then
                mshDownTab.TextMatrix(intRow, 0) = zlCommFun.NVL(rsTmp!名称, "") & IIf(Not IsNull(rsTmp!单位), "(" & rsTmp!单位 & ")", "")
            Else
                mshDownTab.TextMatrix(intRow, 0) = IIf(str部位 = "", NVL(rsTmp!体温部位), str部位) & zlCommFun.NVL(rsTmp!名称, "")
            End If
        End If
        ...
        mItemStru(intRow).部位 = IIf(str部位 = "", NVL(rsTmp!体温部位), str部位)
...
'                    "AND a.发生时间<[3] And c.终止版本 Is Null "
    ...
    '找开始日期-14天前的手术次数(只取当天最大时间的手术)
    mstrSQL = "SELECT  Nvl(Count(distinct trunc(a.发生时间)), 0) As 次数 " & _
                "FROM 病人护理记录 a,病人护理内容 c " & _
                "Where a.ID = c.记录ID " & _
                    "AND a.病人来源=2 " & _
                    "AND Nvl(a.婴儿,0)=[4] " & _
    ...
    For intCol = 1 To mshUpTab.Cols - 1
        ...
        If DateDiff("d", CDate(strFrom), CDate(Split(picScale.Tag, ";")(1))) + 1 >= intCol Then
            ...
            If mshUpTab.ColData(intCol) = 1 Or mshUpTab.ColData(intCol) = 2 Then
                lng次数 = lng次数 + 1
                
                If mblnStopFlag Then
                    
                    If lng次数 = 1 Then
                        mshUpTab.TextMatrix(2, intCol) = "手术"
                    Else
                        mshUpTab.TextMatrix(2, intCol) = "手术(" & lng次数 & ")"
                    End If
                Else
...
'    lblTime.Caption = "日期:" & Format(strFrom, "MM-DD") & "～" & Format(strTo, "MM-DD")
    ...
    For intCol = 1 To UBound(mstrOpsDays)
         mstrOpsSvr(intCol) = ""
         mstrOpsDays(intCol) = ""
    Next
    ...
    With mshScale
        For intCol = .FixedCols To .Cols - 1
            ...
            .TextMatrix(GraphDataRow.手术标志, intCol) = ""                         '手术,一个时刻内只能发生一次手术,一天最多6次手术
...
'    mstrSQL = "" & _
'        " SELECT A.发生时间 AS 时间,C.项目名称" & _
'        " From" & _
'        "     (SELECT 日期,MAX(时间) AS 时间" & _
'        "     FROM (" & _
'        "         SELECT TRUNC(A.发生时间) AS 日期,A.发生时间 AS 时间" & _
'        "         FROM 病人护理记录 A, 病人护理内容 C" & _
'        "         Where a.Id = C.记录ID And a.病人来源 = 2 And NVL(a.婴儿, 0) = [5]" & _
'        "         AND A.病人ID+0 = [1] AND A.主页ID = [2] AND C.记录类型 = 4 AND C.终止版本 IS NULL" & _
'        "         AND A.发生时间 BETWEEN [3] And [4])" & _
'        "     GROUP BY 日期) B,病人护理记录 A,病人护理内容 C" & _
'        " Where a.Id = C.记录ID And a.病人来源 = 2 And NVL(a.婴儿, 0) = [5]" & _
'        "     AND A.病人ID+0 = [1] AND A.主页ID = [2] AND C.记录类型 = 4 AND C.终止版本 IS NULL" & _
'        "     AND A.发生时间=B.时间"
    ...
    If rsTmp.BOF = False Then
        Do While Not rsTmp.EOF
            ...
            For intCol = 1 To mshUpTab.Cols - 1
                If DateDiff("d", CDate(strFrom), CDate(Split(picScale.Tag, ";")(1))) + 1 >= intCol Then
                    '一天,只写一次手术标记
                    If Format(strDay, "yyyy-MM-dd") <> Format(rsTmp.Fields(0).Value, "yyyy-MM-dd") Then
...
'                                End If
                    ...
                    Case "分娩"
                        If intCount > 0 And intCount < mshScale.Cols Then
                            mshScale.TextMatrix(3, intCount) = rsTmp("项目名称").Value & ";" & Format(rsTmp("时间").Value, "yyyy-MM-dd HH:mm:ss")
            ...
            strDay = Format(rsTmp.Fields(0).Value, "yyyy-MM-dd")
            rsTmp.MoveNext
        Loop
    End If
...
'    End If
    ...
    '读取数据(活动项目的名称为体温部位+名称)
    '------------------------------------------------------------------------------------------------------------------
    If strItemName <> "" Then
        strItemName = Mid(strItemName, 2) & ",'呼吸','收缩压','舒张压'"
                        
        mstrSQL = "SELECT a.发生时间 As 时间,C.记录内容 As 结果,E.保留项目,E.项目性质,DECODE(E.项目性质,2,NVL(C.体温部位, ''),'')||D.记录名 AS 记录名,D.项目序号,D.记录频次 " & _
                    "FROM 病人护理记录 A,病人护理内容 C,体温记录项目 D,护理记录项目 E " & _
                    "Where A.ID = c.记录ID " & _
                        "AND A.病人来源=2 " & _
                        "AND Nvl(a.婴儿,0)=[6] " & _
                        "AND A.病人id=[1] " & _
                        "AND A.主页id=[2] " & _
                        "AND INSTR([5],','''||DECODE(E.项目性质,2,NVL(C.体温部位, ''),'')||D.记录名||''',')>0 " & _
                        "AND D.项目序号=C.项目序号 " & _
                        "AND c.记录类型=1 " & _
                        "AND E.项目序号=D.项目序号 " & _
                        "AND E.护理等级>=[7]  " & _
        ...
        With rsTmp
            ...
            If rsTmp.RecordCount > 0 Then
                ...
                For i = 0 To rsTmp.RecordCount - 1
                    ...
                    Case Else
                        '活动项目名称相同的直接定位行,考虑到单病人/多病人编辑程序在录入时可不录入部位,因此使用Like来定位行
                        For intRow = 1 To mshDownTab.Rows - 1
                            If Val(mshDownTab.RowData(intRow)) = !项目序号 And (!项目性质 = 2 And !记录名 Like Split(mshDownTab.TextMatrix(intRow, 1), "(")(0) & "*" Or !项目性质 = 1) Then
                                Exit For
                            End If
                        Next
...
Private Function DrawScale() As Boolean
    ...
    If picScale.Tag <> "" Then
        Call CalcMinMaxCol(picScale.Tag, intMinCol, intMaxCol)
        lblCur.Top = picScale.Height - lblCur.Height + 20
        lblCur.Left = intMinCol * HOUR_STEP_Twips + 30
    End If
    DrawText picScaleNote, 0, 100, " 时       间", &H80000012
...
Public Function DrawPaper() As Boolean
    ...
    For intRow = 1 To mshScale.Rows - 1
        ...
        If Not (intRow = 85 Or intRow = 98) Then
            If (intRow - 1) Mod 10 = 0 Then
                ...
                End If
            Else
                DrawLine picGraph, X0, Y0 + ROWHEIGHT * 5, X1, Y0 + ROWHEIGHT * 5, &H8000000A, 0, 1
            End If
        Else
            DrawLine picGraph, X0, Y0 + ROWHEIGHT * 5, X1, Y0 + ROWHEIGHT * 5, &H8000000A, 0, 1
        End If
                            ...
                            If i = 0 Then
                                ...
                                Case mItemSerial.呼吸
                                    ...
                                    Case Is >= GetMaxValue(intCol)
                                        Y1 = ConvertToY(intCol, GetMaxValue(intCol))
                                        Call DrawLine(picGraph, X1, Y1, X1, Y1 - 200, lngColor, , , True)
                                    Case Is < 10    '呼吸小于10次强制输出实际次数
                                        Call DrawText(picGraph, X1 - 40, ConvertToY(intCol, 12), ConvertToValue(intCol, Y1))
                                    End Select
                                End Select
...
'                End If
        ...
        Dim lngYMax As Long
        'lngYMax = ConvertToY(mItemSerial.体温, 34.2)
        lngYMax = ConvertToY(mItemSerial.体温, 30)
...
'                blnAdd = False
'                str手术 = Split(.TextMatrix(手术标志, intCol + .FixedCols), ";")(0)
'                If Left(.TextMatrix(手术标志, intCol + .FixedCols), 2) = "手术" Then
'                    blnAdd = (mBodyFlag.手术 > 0)
'                    If mBodyFlag.手术 = 2 Then str手术 = str手术 & "于" & ConvertTimeToChinese(Format(Split(.TextMatrix(手术标志, intCol + .FixedCols), ";")(1), "HH:mm"))
'                Else
'                    blnAdd = (mBodyFlag.分娩 > 0)
'                    If mBodyFlag.分娩 = 2 Then str手术 = str手术 & "于" & ConvertTimeToChinese(Format(Split(.TextMatrix(手术标志, intCol + .FixedCols), ";")(1), "HH:mm"))
'                End If
'                If blnAdd Then
'                    rsTmp.AddNew
...
'            End If
            ...
            Y1 = ConvertToY(mItemSerial.体温, 33.5)                             'ConvertToY(mItemSerial.呼吸, 15)
            'Y1 = 7020
            strComment = ""
...
'                            If Asc(strChar) < 0 Then
'                                If intCharNumber Mod 2 = 1 Then Y1 = Y1 + 97.5
'                            End If
...
Private Sub OutputNote(ByVal objDraw As Object, ByVal dblHeight As Double, ByRef rsNote As ADODB.Recordset)
    ...
    Dim intCount As Integer
    Dim strChar As String
    sinMaxY = ConvertToY(mItemSerial.体温, 42)
    ...
    With rsNote
        ...
        '先在入出转手术等中循环
        Do While Not .EOF
'            If Trim(NVL(!结果)) Like "下标*" Then Stop
            If Trim(NVL(!结果)) <> "" Then
...
'                    If Asc(strChar) < 0 Then
'                        If intCharNumber Mod 2 = 1 Then sinY1 = sinY1 + 97.5
'                    End If
    ...
    With mshUpTab
        ...
        If .Tag = "填写手术日" Or .Tag = "删除手术日" Then
            For intCol = .FixedCols To .Cols - 1
                ...
                For intLoop = (intCol - 1) * 6 + mshScale.FixedCols To intCol * 6 + mshScale.FixedCols - 1
                    '手术数据按组号保存,最多6组
                    If mshScale.TextMatrix(手术标志, intLoop) <> "" Then
                        strTmp = ""
                        strStart = Split(mshScale.TextMatrix(手术标志, intLoop), ";")(1)
                        strEnd = strStart
                        
                        If Left(mshScale.TextMatrix(手术标志, intLoop), 4) = "手术分娩" Then
                            strTmp = "手术分娩"
                        ElseIf Left(mshScale.TextMatrix(手术标志, intLoop), 2) = "手术" Then
                            strTmp = "手术"
                        ElseIf Left(mshScale.TextMatrix(手术标志, intLoop), 2) = "分娩" Then
                            strTmp = "分娩"
                        End If
                    ...
                    For intLoop = intCol To intCol + 1
                        ...
                        If (intMode = 2 Or intMode = 3) And mvarStrValue <> "" Then
                            ...
                            Else
                                ...
                                mstrSQL = mstrSQL & "0,"
                                mstrSQL = mstrSQL & "'" & mvarStrValue & "'"
                                mstrSQL = mstrSQL & ",'" & mItemStru(intCount + 1).部位 & "',1,1,0," & IIf(IsNumeric(mvarStrValue), 0, 1) & ")"
...
Private Sub mshDownTab_KeyPress(KeyAscii As Integer)
    ...
    Case Else
        ...
        Case 0 '数值型
            ...
            Case mItemNo.大便
                If Check是否包含(UCase(Chr(KeyAscii))) = False Then KeyAscii = 0
            Case mItemNo.出液
                If Check是否包含(UCase(Chr(KeyAscii))) = False Then KeyAscii = 0
            Case Else
                If Check是否包含(UCase(Chr(KeyAscii))) = False Then KeyAscii = 0
...
Private Sub mshUpTab_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Case vbKeyReturn ' 13     '设置手术日
        With mshUpTab
            ...
            strCaption = mshScale.TextMatrix(手术标志, intCol)
...
'                For intLoop = intStart To intEnd
'                    If Left(mshScale.TextMatrix(手术标志, intLoop), 2) = "手术" Then
'                        mshScale.TextMatrix(手术标志, intLoop) = ""
'                    End If
'                    If Left(mshScale.TextMatrix(手术标志, intLoop), 2) = "分娩" Then
'                        mshScale.TextMatrix(手术标志, intLoop) = ""
'                    End If
'                  If Left(mshScale.TextMatrix(手术标志, intLoop), 2) = "手术分娩" Then
'                        mshScale.TextMatrix(手术标志, intLoop) = ""
'                    End If
'                Next
                ...
                Case "分娩"
                    mshScale.TextMatrix(3, intCol) = strCaption & ";" & Format(strTime, "yyyy-MM-dd HH:mm:ss")
                Case Else
                    mshScale.TextMatrix(3, intCol) = strCaption & ";" & Format(strTime, "yyyy-MM-dd HH:mm:ss")
                End Select
                
                mshScale.Cell(flexcpData, 手术标志, intCol, 手术标志, intCol) = Format(strTime, "HH:mm:ss")
...
'            If .ColData(.Col) <> 1 And .ColData(.Col) <> 2 Then Exit Sub
            If mshScale.TextMatrix(手术标志, lblCur.Left \ HOUR_STEP_Twips + mshScale.FixedCols) = "" Then Exit Sub
            
            If MsgBox("是否清除病人" & Format(Split(mshScale.TextMatrix(手术标志, lblCur.Left \ HOUR_STEP_Twips + mshScale.FixedCols), ";")(1), "yyyy-MM-dd HH:mm") & "的手术登记？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Sub
            mshUpTab.Tag = "删除手术日"
            Select Case .ColData(.Col)
            Case 0      '非手术日
            Case 1      '原来就是手术日，设置为删除手术日
            ...
            mshScale.TextMatrix(手术标志, lblCur.Left \ HOUR_STEP_Twips + mshScale.FixedCols) = ""
            mstrOpsDays(.Col) = ""
...
Private Sub picGraph_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If Button = 1 Then
        ...
        If Left(aryValue(picGraph.Tag + 1), 1) = "," Then aryValue(picGraph.Tag + 1) = ""
        If Val(picGraph.Tag) = mItemSerial.体温 Or Val(picGraph.Tag) = mItemSerial.脉搏 Then
            If InStr(1, aryValue(picGraph.Tag + 1), ",") > 0 Then
                If IsNumeric(Split(aryValue(picGraph.Tag + 1), ",")(1)) Then
                    aryValue(picGraph.Tag + 1) = Y & "," & Split(aryValue(picGraph.Tag + 1), ",")(1)
                Else
...
Private Sub picLable_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Val(mrsParam("编辑")) = 0 Then Exit Sub
    If Not mvarEdit Then Exit Sub
    
    If Button = 1 Then
        If picGraph.Tag = CStr(Index) Then
            Call ClearLineSelect
            mlngLine = 0
            RaiseEvent PromptInfo("")
        Else
            ...
            RaiseEvent PromptInfo("")
            mlngLine = Index + 1
            picLable(Index).BackColor = RGB(0, 255, 255)
            
            picGraph.MousePointer = 2

            linHCur.BorderColor = picLable(Index).ForeColor
            linVCur.BorderColor = linHCur.BorderColor
            linHCur.X1 = 0: linHCur.X2 = 0: linHCur.Y1 = 0: linHCur.Y2 = 0
            linHCur.Visible = True
            linVCur.X1 = 0: linVCur.X2 = 0: linVCur.Y1 = 0: linVCur.Y2 = 0
            linVCur.Visible = True
        End If
        RaiseEvent SelectScale(Index)
    ElseIf mItemSerial.体温 = Index Or mItemSerial.呼吸 = Index Or mItemSerial.脉搏 = Index Then
        RaiseEvent RButton(Button, Shift, X, Y)
    End If
    Call DrawPicLable
End Sub
...
Private Sub picPane_Resize()
    ...
    With mshScale
        ...
        .Top = mshUpTab.Top + mshUpTab.Height - mshScale.ROWHEIGHT(0) * 4 / 6 + 80
        .Height = .Rows * .ROWHEIGHT(.Rows - 1) - mshScale.ROWHEIGHT(0) * 4 / 6 + 600
        .Refresh
    End With
    ...
    With picScaleNote
        .Left = mshUpTab.Left
        .Width = mshUpTab.ColWidth(0)
        .Top = mshUpTab.Top + mshUpTab.Height - 15
        .Height = mshScale.ROWHEIGHT(0) * 4 / 6  ' mshScale.ROWHEIGHT(0) + mshScale.ROWHEIGHT(1) / 2
    End With
    ...
    For intCol = 0 To picLine.UBound
        picLine(intCol).Move mshScale.Left + mshScale.ColWidth(0) * (intCol + 1), picScale.Top + picScale.Height, 15, picBack.Height - 15
        picLable(intCol).Move mshScale.Left + mshScale.ColWidth(0) * intCol, picScale.Top + picScale.Height, mshScale.ColWidth(0) - 7, picBack.Height - 15
    Next
...
Private Sub picScale_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Case 39     '右移动
        If lblCur.Left + HOUR_STEP_Twips <= intMaxCol * HOUR_STEP_Twips + 30 Then lblCur.Left = lblCur.Left + HOUR_STEP_Twips
    Case 13     'Enter输入数据
...
Private Sub picScale_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    aryValue = Split(picScale.Tag, ";")
    If X < (Int((CDate(aryValue(0)) - Int(CDate(aryValue(0)))) * 24) \ 4 - 1) * HOUR_STEP_Twips + HOUR_STEP_Twips Then Exit Sub
    If X > (Int((CDate(aryValue(1)) - Int(CDate(aryValue(0)))) * 24) \ 4 + 1) * HOUR_STEP_Twips - HOUR_STEP_Twips Then Exit Sub
    lblCur.Left = Int(X / HOUR_STEP_Twips) * HOUR_STEP_Twips + 30
    Exit Sub
ErrHead:
    If ErrCenter() = 1 Then
        Resume
    ...
    With mshDownTab
        ...
        Case mItemNo.大便
            If Index = 0 Then
                ...
                If strTmp <> "" Then
                    If Check是否包含(strTmp) = False Then
                        txtInput(Index).Text = ""
                    Else
...
Private Sub DrawPicLable()
    Dim intCol As Integer, intStep As Integer
    Dim intCur As Integer, intMax As Integer
    Dim sinHeight As Single, strName As String
    Dim arrData, lngColor As Long
    Dim lngY As Long, lngMaxY As Long, lngCurY As Long
    On Error Resume Next
    '根据表格内容画标尺(呼吸与脉搏需要填充各大整数值)
    
    If picLable.UBound = 0 Then Exit Sub
    sinHeight = mshScale.ROWHEIGHT(1)
    For intCol = 0 To picLable.UBound
        lngColor = mshScale.Cell(flexcpForeColor, MAXROWS - 1, intCol)
        '非体温项,如果能整除10则标记;体温项整数即标记
        arrData = Split(picLine(intCol).Tag, ";")  '最大值;最小值
        intMax = arrData(0)
        intStep = IIf(intCol = mItemSerial.体温, 1, IIf(intCol = mItemSerial.呼吸, 10, 20))
        strName = mshScale.TextMatrix(0, intCol)
        lngMaxY = ConvertToY(mItemSerial.体温, 42)
        DrawText picLable(intCol), 20, lngMaxY - sinHeight, strName, lngColor
        For intCur = intMax To arrData(1) Step -1 * intStep
            lngY = ConvertToY(intCol, intCur)
            If Len(CStr(intCur)) = 2 Then
                If intCol = mItemSerial.体温 And intCur = 42 Then
                Else
                    DrawText picLable(intCol), IIf(intCol = mItemSerial.体温, 20, 80), lngY - sinHeight, IIf(intCol = mItemSerial.体温, Format(intCur, "#0.0"), intCur), lngColor
                End If
            Else
                DrawText picLable(intCol), 50, lngY - sinHeight, intCur, lngColor
            End If
        Next
    Next
    
    '输出文字说明
'    DrawText picLable(mItemSerial.体温), 20, ConvertToY(mItemSerial.体温, 42) - sinHeight, "体温", &H80000012
'    DrawText picLable(mItemSerial.呼吸), 20, ConvertToY(mItemSerial.体温, 42) - sinHeight, "呼吸", &H80000012
'    DrawText picLable(mItemSerial.脉搏), 20, ConvertToY(mItemSerial.体温, 42) - sinHeight, "脉搏", &H80000012
End Sub

Public Function IsHaveOps() As Boolean
    Dim intCol As Integer
    Dim intLoop As Integer
...
'    Next
    ...
    IsHaveOps = (mshScale.TextMatrix(手术标志, lblCur.Left \ HOUR_STEP_Twips + mshScale.FixedCols) <> "")
End Function

Private Function GetPosition(ByVal str项目名称 As String, ByVal lng项目序号 As Long) As String
    Dim strPosition As String
    Dim lngPOS As Long, lngMax As Long
    
    lngMax = UBound(mItemStru)
    For lngPOS = 0 To lngMax
        If mItemStru(lngPOS).项目名称 = str项目名称 Then
            strPosition = strPosition & IIf(strPosition = "", "", ",") & "'" & mItemStru(lngPOS).部位 & "'"
        End If
    Next
    If strPosition <> "" Then GetPosition = " And 部位 Not In (" & strPosition & ")"
End Function
