...
Private Const MAXROWS = 101                         '原46
Private Const ROWHEIGHT = 17.7                      '原35
...
Public Function DrawPatiInfo(objDraw As Object, X As Long, Y As Long, strInfo As String, ByVal lngPaperWidth As Long) As Boolean
    '功能:打印顶行病人信息
    '参数:strInfo=姓名、住院号、科室、病区、床号、日期、年龄
    ...
    Call DrawText(objDraw, X, Y, Space(3) & "病房:", 0)
    Call DrawText(objDraw, X + objDraw.TextWidth(Space(3) & "病房:"), Y, Split(strInfo, "'")(4), 16711680)
    X = X + objDraw.TextWidth(Space(3) & "病房:" & Split(strInfo, "'")(4)) + W_9pt / 3
    
    Call DrawText(objDraw, X, Y, Space(3) & "入院日期:", 0)
    Call DrawText(objDraw, X + objDraw.TextWidth(Space(3) & "入院日期:"), Y, Split(strInfo, "'")(3), 16711680)
    X = X + objDraw.TextWidth(Space(3) & "入院日期:" & Split(strInfo, "'")(3)) + W_9pt / 3

    Call DrawText(objDraw, X, Y, Space(3) & "住院号:", 0)
    Call DrawText(objDraw, X + objDraw.TextWidth(Space(3) & "住院号:"), Y, Split(strInfo, "'")(1), 16711680)
    X = X + objDraw.TextWidth(Space(3) & "住院号:" & Split(strInfo, "'")(1)) + W_9pt / 3
    
    If zlDatabase.GetPara("体温单显示诊断", glngSys, 1255, 1) = 0 Then
        strTmp = ""
    Else
        strTmp = Split(strInfo, "'")(7)
    End If
    ...
    Call DrawText(objDraw, X, Y, "患者姓名:", 0)
    Call DrawText(objDraw, X + objDraw.TextWidth("患者姓名:"), Y, Split(strInfo, "'")(0), 16711680)
    X = X + objDraw.TextWidth("患者姓名:" & Split(strInfo, "'")(0)) + W_9pt / 3
    
    Call DrawText(objDraw, X, Y, Space(3) & "年龄:", 0)
    Call DrawText(objDraw, X + objDraw.TextWidth(Space(3) & "年龄:"), Y, Split(strInfo, "'")(6), 16711680)
    X = X + objDraw.TextWidth(Space(3) & "年龄:" & Split(strInfo, "'")(6)) + W_9pt / 3
    
    Call DrawText(objDraw, X, Y, Space(3) & "性别:", 0)
    Call DrawText(objDraw, X + objDraw.TextWidth(Space(3) & "性别:"), Y, Split(strInfo, "'")(5), 16711680)
    X = X + objDraw.TextWidth(Space(3) & "性别:" & Split(strInfo, "'")(5)) + W_9pt / 3

    Call DrawText(objDraw, X, Y, Space(3) & "诊断:", 0)
    Call DrawText(objDraw, X + objDraw.TextWidth(Space(3) & "诊断:"), Y, strTmp, 16711680)
    X = X + objDraw.TextWidth(Space(3) & "诊断:" & strTmp) + W_9pt / 3
...
Private Sub DrawBodyRecordItem(objDraw As Object, X As Long, Y As Long, ByVal colFirstWidth As Long, strValue() As String, ByVal rsArrRecordItemInfo As ADODB.Recordset, Optional sngScale As Single = 1, Optional ByVal intGridRows As Integer = 0)
    ...
    If InStr(1, strValue(0), ";") > 0 Then
        ...
        For intRow = LBound(strValue) To UBound(strValue)
            ...
            If intGridRows = 0 Or intGridRows > intFactRow Then
                ...
                If Split(strValue(intRow), ";")(0) = "呼吸" Then
                    ...
                    For intCol = 0 To 42
                        If intCol = 0 Then
                            ...
                            Call DrawCell(objDraw, Split(strValue(intRow), ";")(intCol) & Split(strValue(intRow), ";")(intCol + 2), lngTmpX, Y + intRow * (H_9pt + H_9pt \ 2), colFirstWidth, 2 * (H_9pt + H_9pt \ 2), , , , , , objDraw.Font, , 1, 1)
                            Call DrawLine(objDraw, lngTmpX, Y + intRow * (H_9pt + H_9pt \ 2), lngTmpX, Y + intRow * (H_9pt + H_9pt \ 2) + 2 * (H_9pt + H_9pt \ 2), , , 2)
                            Call DrawLine(objDraw, lngTmpX, Y + intRow * (H_9pt + H_9pt \ 2), lngTmpX + colFirstWidth, Y + intRow * (H_9pt + H_9pt \ 2), , , 2)
                            Call DrawLine(objDraw, lngTmpX + colFirstWidth, Y + intRow * (H_9pt + H_9pt \ 2), lngTmpX + colFirstWidth, Y + intRow * (H_9pt + H_9pt \ 2) + 2 * (H_9pt + H_9pt \ 2), , , 2)
                        ...
                        Else
                            ...
                            If int呼吸表格输出格式 = 0 Then
                                If intCol Mod 2 = 0 Then
                                    Call DrawCell(objDraw, strTmp, lngTmpX + colFirstWidth + HOUR_STEP_Twips * (intCol - 1), Y + intRow * (H_9pt + H_9pt \ 2), HOUR_STEP_Twips, (H_9pt + H_9pt \ 2), , , , COLOR.红色, , objDraw.Font, "1000")
                                Else
                                    Call DrawCell(objDraw, strTmp, lngTmpX + colFirstWidth + HOUR_STEP_Twips * (intCol - 1), Y + intRow * (H_9pt + H_9pt \ 2) + (H_9pt + H_9pt \ 2), HOUR_STEP_Twips, (H_9pt + H_9pt \ 2), , , , COLOR.红色, , objDraw.Font, "0000")
                                End If
                            Else
                                If int呼吸位置 = 2 Then
                                    Call DrawCell(objDraw, strTmp, lngTmpX + colFirstWidth + HOUR_STEP_Twips * (intCol - 1), Y + intRow * (H_9pt + H_9pt \ 2), HOUR_STEP_Twips, (H_9pt + H_9pt \ 2), , , , COLOR.红色, , objDraw.Font, "1000")
                    ...
                    For intCol = 0 To 42
                        If (intCol) Mod 6 = 0 Then
                            Call DrawLine(objDraw, lngTmpX + colFirstWidth + HOUR_STEP_Twips * intCol, Y + intRow * (H_9pt + H_9pt \ 2), lngTmpX + colFirstWidth + HOUR_STEP_Twips * intCol, Y + intRow * (H_9pt + H_9pt \ 2) + 2 * (H_9pt + H_9pt \ 2), , , 2)
                        Else
                            Call DrawLine(objDraw, lngTmpX + colFirstWidth + HOUR_STEP_Twips * intCol, Y + intRow * (H_9pt + H_9pt \ 2) - 15, lngTmpX + colFirstWidth + HOUR_STEP_Twips * intCol, Y + intRow * (H_9pt + H_9pt \ 2) + 2 * (H_9pt + H_9pt \ 2) + 15)
                            Call DrawLine(objDraw, lngTmpX + colFirstWidth + HOUR_STEP_Twips * (intCol + 1), Y + intRow * (H_9pt + H_9pt \ 2) - 15, lngTmpX + colFirstWidth + HOUR_STEP_Twips * (intCol + 1), Y + intRow * (H_9pt + H_9pt \ 2) + 2 * (H_9pt + H_9pt \ 2) + 15)
                                ...
                                If intCol Mod 2 = 1 Then
                                    ...
                                    If (Split(strValue(intRow), ";")(intCol + 2) <> "" And Split(strValue(intRow), ";")(intCol + 3) <> "") Or Val(Split(strValue(intRow), ";")(1)) = 2 Then
                                        ...
                                        Else
                                            ...
                                            Call DrawCell(objDraw, strTmp, lngTmpX + colFirstWidth + HOUR_STEP_Twips * 3 * (intCol - 1), Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2), HOUR_STEP_Twips * 3, (H_9pt + H_9pt \ 2), , , , lngColor, , objDraw.Font)
                                        End If
                                    ...
                                    Else
                                        ...
                                        Else
                                            ...
                                            Call DrawCell(objDraw, strTmp, lngTmpX + colFirstWidth + HOUR_STEP_Twips * 6 * Fix(intCol / 2), Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2), HOUR_STEP_Twips * 6, (H_9pt + H_9pt \ 2), , , , lngColor, , objDraw.Font)
                                        End If
    ...
    '            End If
        ...
        '补空行
        If intGridRows > 0 And intGridRows > intFactRow Then
...
Private Sub DrawBodyPageFooter(objDraw As Object, X As Long, Y As Long, ByVal intPageNo As Integer, ByVal intBeginPage As Integer)
    ...
    Call DrawCell(objDraw, strNOPage, X - W_9pt * 2, Y, W_9pt * 20, H_9pt + H_9pt \ 2, , , , , , objDraw.Font, "0000", 0, 0)
...
Private Function DrawBodyInfo(objDraw As Object, X As Long, Y As Long, ByVal colFirstWidth As Long, _
    ...
    Call DrawCell(objDraw, "住院日数", X, Y + (H_9pt + H_9pt \ 2) * 1, colFirstWidth, H_9pt + H_9pt \ 2, 0, 0, , , , objDraw.Font, "1111", 1, 1, False)
    Call DrawCell(objDraw, "手术日数", X, Y + (H_9pt + H_9pt \ 2) * 2, colFirstWidth, H_9pt + H_9pt \ 2, 0, 0, , , , objDraw.Font, "1111", 1, 1, False)
    Call DrawCell(objDraw, "时    间", X, Y + (H_9pt + H_9pt \ 2) * 3, colFirstWidth, H_9pt + H_9pt \ 2, 0, 0, , , , objDraw.Font, "1111", 1, 1, False)
    
    Call DrawLine(objDraw, X, Y, X + colFirstWidth, Y, , , 2)
    Call DrawLine(objDraw, X, Y, X, Y + (H_9pt + H_9pt \ 2) * 3 + H_9pt + H_9pt \ 2, , , 2)
    Call DrawLine(objDraw, X + colFirstWidth, Y, X + colFirstWidth, Y + (H_9pt + H_9pt \ 2) * 3 + H_9pt + H_9pt \ 2, , , 2)
    ...
    For intDay = 0 To 6
        ...
        End If
'        strTmp = Format(strTmp, "yyyyMMdd")
'        If intDay = 0 Or Right(strTmp, 6) = "0101" Then
'            '一年的第一天
'            strTmp = Val(Mid(strTmp, 7, 2)) & "/" & Val(Mid(strTmp, 5, 2))
'        ElseIf strTmp = Format(strBeginDate, "yyyyMMdd") Then
'            '入院第一天，写上年份
'            strTmp = Val(Mid(strTmp, 7, 2)) & "/" & Val(Mid(strTmp, 5, 2))
'        ElseIf Right(strTmp, 2) = "01" Then
'            strTmp = Val(Mid(strTmp, 7, 2)) & "/" & Val(Mid(strTmp, 5, 2))
'        Else
'            strTmp = Val(Mid(strTmp, 7, 2))
'        End If
...
Private Sub DrawBodyTopScale(objDraw As Object, X As Long, Y As Long, Optional sngScale As Long = 1)
    ...
    Dim H_9pt As Long
    Dim lngTmpX As Long, lngTmpY As Long, lngCurX As Long, lngCurY As Long
    ...
    lngTmpX = X: lngTmpY = Y
    lngCurX = lngTmpX + mlngFirstWidth
    lngCurY = Y
...
'                objDraw.FontSize = 8
                Call DrawCell(objDraw, CStr(j), lngCurX, lngCurY, HOUR_STEP_Twips, (H_9pt + H_9pt \ 2), , , , lngColor, , objDraw.Font, , 1, 1, True)
                lngCurX = lngCurX + HOUR_STEP_Twips
            Else
'                objDraw.FontSize = 9
                Call DrawCell(objDraw, CStr(j), lngCurX, lngCurY, HOUR_STEP_Twips, (H_9pt + H_9pt \ 2), , , , lngColor, , objDraw.Font, , 1, 1, True)
                lngCurX = lngCurX + HOUR_STEP_Twips
        ...
        For j = lngHourBegin To 12 - (4 - lngHourBegin) Step 4
            ...
            If j = 12 - lngHourBegin Then
'                objDraw.FontSize = 8
                Call DrawCell(objDraw, CStr(12 + j), lngCurX, lngCurY, HOUR_STEP_Twips, (H_9pt + H_9pt \ 2), , , , lngColor, , objDraw.Font, , 1, 1, True)
                lngCurX = lngCurX + HOUR_STEP_Twips
            Else
'                objDraw.FontSize = 9
                Call DrawCell(objDraw, CStr(12 + j), lngCurX, lngCurY, HOUR_STEP_Twips, (H_9pt + H_9pt \ 2), , , , lngColor, , objDraw.Font, , 1, 1, True)
    ...
    X = lngTmpX + mlngFirstWidth
    For i = 1 To 7
        X = X + HOUR_STEP_Twips * 6
        Call DrawLine(objDraw, X, lngCurY, X, lngCurY + (H_9pt + H_9pt \ 2), , 1, 2)
    Next
...
'    H_9pt = objDraw.TextHeight("字")
    ...
    lngYMax = (lngTmpY + 3 * H_9pt / 2) + 110 * 3 * H_9pt / 2 - H_9pt / 2
    Call PointInit(rsPoint)
    ...
    For lngCol = 0 To 41
        ...
        lngX = lngTmpX + lngCol * HOUR_STEP_Twips + HOUR_STEP_Twips / 2
        lngY = (lngTmpY + 3 * H_9pt / 2)
        lngY1 = (lngTmpY + 3 * H_9pt / 2) + 70 * 3 * H_9pt / 2                          '得到35度的坐标
        lngY2 = (lngTmpY + 3 * H_9pt / 2) + 100 * 3 * H_9pt / 2                          '得到32度的坐标
        dblHeight = (lngTmpY + 3 * H_9pt / 2) + 20 * 3 * H_9pt / 2 - H_9pt / 2          '得到40度的坐标,好像是按格子数来计算
        ...
        If strComment <> "" Then
            ...
            '下标(单独处理了下,后面跟上类型,1表示未记,6表示下标,因四川地区未记打在最下面,下标仍打在35度处)
            intCharNumber = 0
            If aryTmp(1) <> "" Then
                intCharNumber = Val(Split(aryTmp(1), ",")(1))
                strComment = Split(aryTmp(1), ",")(0)
                For i = 1 To Len(strComment)
                    If lngY1 < lngYMax Then
                        strTmp = Mid(strComment, i, 1)
                        Call DrawRotateText(objDraw, lngX - objDraw.TextWidth(strTmp) / 2 + 15, IIf(intCharNumber <> 1, lngY1, lngY2) + 15, strTmp, -2147483635)
                        
                        If intCharNumber <> 1 Then
                            lngY1 = lngY1 + ROWHEIGHT * 10
                        Else
                            lngY2 = lngY2 + ROWHEIGHT * 10
                        End If
                    End If
                Next
            End If
    ...
    lngY2 = (lngTmpY + 3 * H_9pt / 2) + 102 * 3 * H_9pt / 2                          '得到32度的坐标
    If Trim(strIndex) = "" Then
...
'                    lngArrXY(lngCol, 0) = 2
                    ...
                    ElseIf lng项目序号 = 3 Then     '呼吸
                        Call PointAdd(rsPoint, lngX, lngY, lng项目序号, strTag, lngColor, lngCol, CStr(aryTmp(5)), IIf(strTag = "", "BREATH", ""))
                        '如果呼吸值小于10,则在最下面打印实际的呼吸次数
                        If Val(aryTmp(0)) < 10 Then
                            Call DrawText(objDraw, X1 - 30, lngY2, CStr(aryTmp(0)))
                        End If
                    ElseIf lng项目序号 = 2 Then     '脉搏
                        Call PointAdd(rsPoint, lngX, lngY, lng项目序号, strTag, lngColor, lngCol, CStr(aryTmp(5)), IIf(strTag = "", "PACEMAKER", ""))
                    Else
    ...
    For intCount = 0 To 41
        If pt脉搏(intCount).X > 0 Then
            If pt心率(intCount).X = 0 Then
                If intEnd >= 0 And blnTrue = True Then
                    intEnd = intCount
            ...
            Else
                intEnd = intCount
                If intStart = 0 And blnTrue = False Then intStart = intCount
                blnTrue = True
            End If
        Else
            If intEnd >= 0 And intStart >= 0 And blnTrue = True Then
                Call DrawFillPoly(objDraw, intStart, intEnd, pt脉搏, pt心率)
            End If
            intStart = 0
            intEnd = 0
...
'    H_9pt = objDraw.TextHeight("字")
    ...
    '------------------------------------------------------------------------------------------------------------------
    '画横向坐标图线
    intGuageRow = intGuageRow + 2
    For intRow = 0 To intGuageRow - 1
        objDraw.DrawStyle = 2
        Y0 = lngTmpY + intRow * (H_9pt + H_9pt \ 2)
        
        If Not (intRow = 84 Or intRow = 97) Then
            If (intRow + 1) Mod 10 = 1 Then
                If Int((intRow + 1) / 10) = 5 Then
                    Call DrawLine(objDraw, lngTmpX + 10, Y0 + ROWHEIGHT * 5, lngTmpX + HOUR_STEP_Twips * 6 * 7, Y0 + ROWHEIGHT * 5, RGB(200, 0, 0), , 2)
                Else
                    Call DrawLine(objDraw, lngTmpX + 10, Y0 + ROWHEIGHT * 5, lngTmpX + HOUR_STEP_Twips * 6 * 7, Y0 + ROWHEIGHT * 5, COLOR.深灰色, , 2)
                End If
            Else
                Call DrawLine(objDraw, lngTmpX + 10, Y0 + ROWHEIGHT * 5, lngTmpX + HOUR_STEP_Twips * 6 * 7, Y0 + ROWHEIGHT * 5, COLOR.深灰色, , 1)
            End If
        Else
            Call DrawLine(objDraw, lngTmpX + 10, Y0 + ROWHEIGHT * 5, lngTmpX + HOUR_STEP_Twips * 6 * 7, Y0 + ROWHEIGHT * 5, COLOR.深灰色)
        End If
    Next
    ...
    For intCol = 0 To 6
        ...
        Call DrawLine(objDraw, X0 + HOUR_STEP_Twips * 6, lngTmpY, X0 + HOUR_STEP_Twips * 6, Y, COLOR.深灰色, , 2)
    Next
...
Private Function DrawBodyScale(objDraw As Object, X As Long, Y As Long, intGuageRow As Integer, arrstrItem() As String, Optional sngScale As Long = 1)
    ...
    int呼吸 = -1: int脉搏 = -1: int体温 = -1
    ReDim aryItem(intCountItem)
    For i = 0 To intCountItem '纵
        aryItem(i) = arrstrItem(i)
        If GetSplitStr(aryItem(i), 2) = "呼吸" And int呼吸 <= 0 Then
            int呼吸 = i
        ElseIf GetSplitStr(aryItem(i), 2) = "脉搏" And int脉搏 <= 0 Then
            int脉搏 = i
        Else
            int体温 = i
        End If
...
'            ElseIf j >= intItemTop And lngTmpVal >= CLng(GetSplitStr(aryItem(i), 4)) Then
            '以前先画的刻度再画的上表格的时间,有两空行,而现在先画的上表格的时间,所以直接从2开始
            If j >= intItemTop And lngTmpVal >= CLng(GetSplitStr(aryItem(i), 4)) Then
                '画递减间隔
                If (lngTmpVal - Fix(lngTmpVal)) = 0 Then
                    ...
                    Case "脉搏", "心率"
                        ...
                        End If
                    Case "呼吸"
                        If lngTmpVal Mod 5 = 0 Then
                            Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth(CStr(lngTmpVal))) / 2, Y + objDraw.TextHeight(CStr(lngTmpVal)) / 2 - 30, CStr(lngTmpVal), lngColor)
                        End If
                    Case Else
                        If lngTmpVal <> 42 Then
                            Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth(CStr(CStr(lngTmpVal)))) / 2 - 100, Y + objDraw.TextHeight(CStr(CStr(lngTmpVal))) / 2 - 30, Format(lngTmpVal, "#0.0"), lngColor)
                        Else
                            If int体温 >= 0 Then Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth(CStr(CStr(lngTmpVal)))) / 2 - 100, Y + objDraw.TextHeight(CStr(CStr(lngTmpVal))) / 2 - 30, "体温", CLng(GetSplitStr(aryItem(i), 0)))
                            If int脉搏 >= 0 Then Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth("脉")) / 2 - (int体温 - int脉搏) * lngPercW - 100, Y + objDraw.TextHeight(CStr(CStr(lngTmpVal))) / 2 - 30, "脉搏", CLng(GetSplitStr(aryItem(int脉搏), 0)))
                            If int呼吸 >= 0 Then Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth("脉")) / 2 - (int体温 - int呼吸) * lngPercW - 100, Y + objDraw.TextHeight(CStr(CStr(lngTmpVal))) / 2 - 30, "呼吸", CLng(GetSplitStr(aryItem(int呼吸), 0)))
                        End If
                    End Select
                End If
                Y = Y + H_9pt + H_9pt \ 2
...
'            objOut.SetPages
    ...
    If rsTemp.BOF = False Then
        ...
        varPatiInfo(3) = Format(zlCommFun.NVL(rsTemp("入院时间").Value), "yyyy年MM月DD日 HH时mm分")
    ...
    If rsTemp.BOF = False Then
        varPatiInfo(3) = Format(zlCommFun.NVL(rsTemp("开始时间").Value), "yyyy年MM月DD日 HH时mm分")
    End If
    ...
    Case Else
        ...
        If rsTemp.BOF = False Then
            ...
            If IsNull(rsTemp("出生时间").Value) = False Then varPatiInfo(3) = Format(zlCommFun.NVL(rsTemp("出生时间").Value), "yyyy年MM月DD日 HH时mm分")
        End If
    ...
    mstrSQL = "Select 时间,项目名称,rownum as 次数 From (SELECT distinct TRUNC(a.发生时间) As 时间,MAX(c.项目名称) AS 项目名称 " & _
                "FROM 病人护理记录 A,病人护理内容 C " & _
                "Where A.ID=C.记录ID " & _
                    "AND A.病人id=[1] And Nvl(a.婴儿,0)=[4] " & _
                    "AND A.主页id=[2] " & _
                    "AND c.记录类型=4 " & _
                    "AND A.发生时间<[3] And c.终止版本 Is Null group By TRUNC(a.发生时间)) order by 时间"
    If mblnMoved Then
    ...
    If rsTmp.RecordCount > 0 Then
        ...
        For i = 0 To rsTmp.RecordCount - 1
            ...
            If IsNull(rsTmp!时间) = False Then
                If Format(strDay, "yyyy-MM-dd") <> Format(rsTmp!时间, "yyyy-MM-dd") Then
                    intDay = intDay + 1
                    Call CalcOPS(lngArr手术后天数, TmplngDayCount, DateDiff("d", CDate(strBeginDate), rsTmp!时间), intDay)
                    strDay = Format(rsTmp!时间, "yyyy-MM-dd")
                End If
            End If
            rsTmp.MoveNext
        Next
...
'    Set rsArrRecordItemInfo = zlDatabase.OpenSQLRecord(mstrSQL, "mdlPrint", 2, 0, lngSectID, IIf(intBaby = 0, 1, 2))
    Set rsArrRecordItemInfo = GetGridDataItem_Print(0, lngSectID, IIf(intBaby = 0, 1, 2), lng病人id, lng主页id, _
                              CDate(Format(strBeginDate, "YYYY-MM-DD")), CDate(Format(strEndDate, "YYYY-MM-DD") & " 23:59:59"), intBaby)
    If rsArrRecordItemInfo.RecordCount > 0 Then
        ...
        For i = 0 To UBound(strArrRecordItemInfo)
            ...
            strArrRecordItemInfo(i) = IIf(rsArrRecordItemInfo!项目性质 = 2, NVL(rsArrRecordItemInfo!体温部位), "") & rsArrRecordItemInfo!名称 & "'" & IIf(IsNull(rsArrRecordItemInfo!单位), "", rsArrRecordItemInfo!单位) & "'" & zlCommFun.NVL(rsArrRecordItemInfo("记录频次").Value, 2)
    ...
    mlngFirstWidth = (UBound(strArrItemInfo) + 1) * W_9pt * 2.5
...
'                    objOut.txtPage.Text = "当前页" & Space(17) & "共 " & objOut.picPage.UBound + 1 & " 页"
        ...
        strSQL = " Select  c.名称 As 科室,b.名称 As 病区,rpad(nvl(d.房间号,' '),10,' ')||'床号:'||a.床号 AS 床号,a.开始原因 " & _
                "From 病人变动记录 a,部门表 b,部门表 c,床位状况记录 d " & _
                "Where a.病人id=[1] And a.主页id=[2] And a.科室id Is Not Null And a.病区id=b.id and a.科室id=c.id " & _
                "And A.病人ID=D.病人ID And d.科室ID=A.科室id And D.床号=A.床号 " & _
                "And a.开始时间-4/24<=[3] And Nvl(a.终止时间,Sysdate)>=[4] " & _
                "Order By a.开始时间"
...
'                strTmpString1 = strTmpString1 & "'" & Format(Int(CDate(strTmpDay)) + i - Int(CDate(strBeginDate)) + 1)
        ...
        '提取手术为入出转标识
        '------------------------------------------------------------------------------------------------------------------
        mstrSQL = "SELECT trunc(A.发生时间) As 时间,max(c.项目名称) AS 项目名称 " & _
                    "FROM 病人护理记录 A,病人护理内容 C " & _
                    "Where A.ID=C.记录ID " & _
                        "AND A.病人id=[1] " & _
                        "AND A.主页id=[2]  And Nvl(a.婴儿,0)=[5] " & _
                        "AND c.记录类型=4 " & _
                        "AND A.发生时间 Between [3] And [4] And c.终止版本 Is Null Group By Trunc(A.发生时间)"
        If mblnMoved Then
            mstrSQL = Replace(mstrSQL, "病人护理记录", "H病人护理记录")
            mstrSQL = Replace(mstrSQL, "病人护理内容", "H病人护理内容")
        End If
...
'                            strArrItemDataInOut(lngCol) = IIf(Trim(strArrItemDataInOut(lngCol)) = "", "", Trim(strArrItemDataInOut(lngCol)) & ";") & strTmp
        ...
        For l = 0 To UBound(strArrItemInfo)
            ...
            mstrSQL = "SELECT c.记录类型,A.发生时间 As 时间,Decode(c.记录类型,1,c.未记说明,c.记录内容) As 说明,Decode(c.记录类型,1,1,c.记录标记) As 记录标记 " & _
                        "FROM 病人护理记录 A,病人护理内容 C " & _
                        "Where A.ID=C.记录ID " & _
                            "AND A.病人id=[1] " & _
                            "AND A.主页id=[2]  And Nvl(a.婴儿,0)=[5] " & _
                    ...
                    For j = 0 To UBound(strArrNewTmpComment)
                        ...
                        If Format(Split(strArrNewTmpComment(j), "'")(0), "yyyy-MM-dd HH:mm:ss") = strTmp Then
                            ...
                            Case 1
                                ...
                                Else
                                    If aryTmp(1) = "" Then
                                        aryTmp(1) = Split(strArrNewTmpComment(j), "'")(1) & ",1"    '1表示未记说明,当未记说明也打印在表格下面时,因四川省要求未记打在32度,而下标是打印在35处,所以此处区分下
                                    Else
                                        If InStr(1, aryTmp(1) & " ", Split(strArrNewTmpComment(j), "'")(1) & " ") = 0 Then
                                            aryTmp(1) = aryTmp(1) & " " & Split(strArrNewTmpComment(j), "'")(1) & ",1"
                                        End If
                                    End If
                                End If
                            Case 6
                                ...
                                If aryTmp(1) = "" Then
                                    aryTmp(1) = Split(strArrNewTmpComment(j), "'")(1) & ",6"
                                Else
                                    aryTmp(1) = aryTmp(1) & " " & Split(strArrNewTmpComment(j), "'")(1) & ",6"  '6表示下标
                                End If
        ...
        If intDrawGridRows > 0 Then
            ...
            For i = LBound(strArrRecordItemInfo) To UBound(strArrRecordItemInfo)
                ...
                mstrSQL = "SELECT A.发生时间 As 当时产生的时间,C.记录内容 As 说明,E.保留项目,DECODE(E.项目性质,2,NVL(C.体温部位,''),'')||D.记录名 AS 记录名,D.项目序号,D.记录频次 " & _
                            "FROM 病人护理记录 A,病人护理内容 C,体温记录项目 D,护理记录项目 E " & _
                            "Where A.ID=C.记录id " & _
                                "AND A.病人id=[1] " & _
                                "AND A.主页id=[2]  And Nvl(a.婴儿,0)=[7] " & _
                                ...
                                "AND E.护理等级>=0  " & _
                                "AND A.发生时间 BETWEEN [3] And [4] And c.终止版本 Is Null " & _
                                "AND D.记录法=2 AND DECODE(E.项目性质,2,NVL(C.体温部位,''),'')||D.记录名 In ([5],[6]) " & _
...
'        Call DrawBodyTips(objDraw, lngNewTmpX, Y, mlngFirstWidth, strStateTips, sngScale)
        ...
        '10、最后画出页码，然后进入下一页
        Call DrawBodyPageFooter(objDraw, Printer.Width / 2 - 300, Y, intPageNo, intEndPage)
...
Public Function GetGridItem(ByVal byt护理等级 As Byte, ByVal lng科室id As Long, ByVal byt婴儿 As Byte, Optional ByVal byt项目性质 As Byte = 1, Optional ByVal strNotItem As String, Optional ByVal blnBodyItem As Boolean = True) As ADODB.Recordset
    ...
    If blnBodyItem Then
        mstrSQL = " Select A.排列序号,A.记录频次,A.记录法,c.项目名称,'' AS 体温部位,c.项目序号,c.项目类型,c.项目表示,c.项目值域,c.项目单位,A.记录名 as 名称,A.项目序号 as 项目号,A.项目序号 As ID,Nvl(B.ID,0) as 项目ID,c.项目性质,C.保留项目,1 As 末级," & _
    ...
    Else
        ...
        mstrSQL = " Select c.项目名称,c.项目序号,c.项目类型,c.项目表示,c.项目值域,c.项目单位,c.项目名称 as 名称,'' AS 体温部位,c.项目序号 as 项目号,c.项目序号 As ID,Nvl(B.ID,0) as 项目ID,c.项目性质,C.保留项目,1 As 末级," & _
...
Public Function GetGridDataItem(ByVal byt护理等级 As Byte, _
    ...
    mstrSQL = " Select A.排列序号,A.记录频次,A.记录法,A.记录名 as 名称,D.体温部位,A.项目序号 as 项目号,A.项目序号 As ID,Nvl(B.ID,0) as 项目ID,c.项目性质,C.保留项目,1 As 末级," & _
                " C.项目单位 As 单位,记录符,最小值,最大值,记录色,1 as 记录否,单位值,最高行,Nvl(C.项目类型,1) as 存储类型,c.项目长度,c.项目小数,c.分组名 " & _
                " From 体温记录项目 A,诊治所见项目 B,护理记录项目 C,[ZYB]" & _
                " Where C.项目ID=B.ID(+) And A.项目序号=C.项目序号 And A.记录法=2 And c.项目性质=2 And Nvl(C.应用方式,0)=1 And C.护理等级>=[1] And Nvl(C.适用病人,0) In (0,[3]) " & _
                " And (c.适用科室=1 Or (c.适用科室=2 And Exists (Select 1 From 护理适用科室 D Where D.项目序号=c.项目序号 And D.科室id=[2])))"
                
    strSQL = "(Select distinct e.项目序号,C.体温部位 " & _
                "FROM 病人护理记录 A,病人护理内容 C,体温记录项目 D,护理记录项目 E " & _
                "Where A.ID = c.记录ID " & _
                    "AND A.病人来源=2 " & _
                    "AND Nvl(a.婴儿,0)=[8] " & _
    ...
    mstrSQL = mstrSQL & " And c.项目序号 = D.项目序号"
    mstrSQL = mstrSQL & " Order by A.排列序号"
    mstrSQL = Replace(mstrSQL, "[ZYB]", strSQL)
...
'                If intCharNumber Mod 2 = 1 Then Y1 = Y1 + ROWHEIGHT * 5
'            End If
'
'            If Asc(strChar) < 0 Then
'                intCharNumber = 0
            ...
            If !类型 = 2 Then       '上标说明,未记说明
                ...
                If rsTarget.RecordCount <> 0 Then
                    ...
                    Do While Not rsTarget.EOF
                        ...
                        rsTarget.MoveNext
                    Loop
                    sinY1 = (lngTmpY + 3 * H_9pt / 2) + 20 * 3 * H_9pt / 2 - H_9pt / 2      '40度的坐标
...
Public Function GetGridDataItem_Print(ByVal byt护理等级 As Byte, _
                                ByVal lng科室id As Long, _
                                ByVal byt适用病人 As Byte, _
                                ByVal lng病人id As Long, _
                                ByVal lng主页id As Long, _
    ...
    mstrSQL = " Select A.排列序号,A.记录频次,A.记录法,DECODE(A.项目序号,4,'血压',A.记录名) as 名称,D.体温部位,A.项目序号 as 项目号,A.项目序号 As ID,Nvl(B.ID,0) as 项目ID,c.项目性质,C.保留项目,1 As 末级," & _
                " C.项目单位 As 单位,记录符,最小值,最大值,记录色,1 as 记录否,单位值,最高行,Nvl(C.项目类型,1) as 存储类型,c.项目长度,c.项目小数,c.分组名 " & _
                " From 体温记录项目 A,诊治所见项目 B,护理记录项目 C,[ZYB]" & _
                " Where C.项目ID=B.ID(+) And A.项目序号=C.项目序号 And A.记录法=2 And Nvl(C.应用方式,0)=1 And C.护理等级>=[1] And Nvl(C.适用病人,0) In (0,[3]) " & _
                " And (c.适用科室=1 Or (c.适用科室=2 And Exists (Select 1 From 护理适用科室 D Where D.项目序号=c.项目序号 And D.科室id=[2])))"
                
    strSQL = "(Select distinct e.项目序号,C.体温部位 " & _
             " From   " & _
                "   (Select C.项目序号,C.体温部位 " & _
                "    FROM 病人护理记录 A,病人护理内容 C " & _
                "    Where A.ID = c.记录ID AND A.病人来源=2 AND Nvl(a.婴儿,0)=[8] " & _
                    "AND a.病人id=[4] AND a.主页id=[5] AND c.记录类型=1 " & _
                    "AND a.发生时间 BETWEEN [6] And [7] And c.终止版本 Is Null ) C,体温记录项目 D,护理记录项目 E" & _
              " Where d.项目序号 = C.项目序号(+) AND E.项目序号 = d.项目序号 And d.记录法=2 AND E.护理等级>=[1]) D"
    ...
    mstrSQL = mstrSQL & " And c.项目序号 = D.项目序号 And C.项目序号<>5"
    mstrSQL = mstrSQL & " Order by DECODE(A.项目序号,3,0,1),C.项目性质,A.排列序号"
    mstrSQL = Replace(mstrSQL, "[ZYB]", strSQL)
    mstrSQL = " Select RowNum-1 AS 序号,A.* From (" & mstrSQL & ") A"
    
    Set GetGridDataItem_Print = zlDatabase.OpenSQLRecord(mstrSQL, "mdlPrint", byt护理等级, lng科室id, byt适用病人, lng病人id, lng主页id, dt开始时间, dt结束时间, byt婴儿)
End Function
...
Public Function SetNTPrinterPaper_Form(ByVal lngHwnd As Long, ByVal lngWidth As Long, ByVal lngHeight As Long, _
    ByVal intOrient As Integer, ByVal intCopys As Integer) As Boolean
...
'      (取DevMode也反映不出来，可能要用GetJob才能获取最近的打印文档属性)
    ...
    If OpenPrinter(strPrtName, lngHandle, 0&) Then
        ...
        If GetFormName(lngHandle, vFormSize, ZL_FORM_NAME) <> 0 Then
            If DeleteForm(lngHandle, ZL_FORM_NAME & Chr(0)) <> 0 Then
                '删除成功才重新加入
                AddNewForm lngHandle, vFormSize, ZL_FORM_NAME
            Else
                '未删除成功,直接利用当前Form
                SetTheForm lngHandle, vFormSize, ZL_FORM_NAME
            End If
        Else
            '没有则直接加入要用的Form
            AddNewForm lngHandle, vFormSize, ZL_FORM_NAME
        ...
        'Change the appropriate member in the DevMode.
        'In this case, you want to change the form name.
        vDevMode.dmFormName = ZL_FORM_NAME & Chr(0)  'Must be NULL terminated!
        vDevMode.dmOrientation = intOrient
        vDevMode.dmCopies = intCopys
        'Set the dmFields bit flag to indicate what you are changing.
