...
Private edtType As EditTypeEnum
...
Dim lngCount As Long
    ...
    gstrSQL = "Select 0 As Id, -null As 对象标记, 1 As 版本, '新增编辑' As 操作, l.创建人 As 人员," & _
        "        To_Char(l.创建时间, 'yyyy-mm-dd hh24:mi:ss') As 时间,0 顺序" & _
        " From 电子病历记录 l" & _
        " Where L.ID = [1]" & _
        " Union All" & _
        ...
        "        Decode(nvl(instr(c.内容文本,';'),0),0,C.内容文本,Substr(c.内容文本,1, Instr(c.内容文本, ';') - 1)) As 人员," & _
        "        Rtrim(Substr(c.对象属性, Instr(c.对象属性, ';', 1, 4) + 1)) As 时间,c.对象标记 as 顺序" & _
        ...
        "        To_Char(l.保存时间, 'yyyy-mm-dd hh24:mi:ss') As 时间,999 as 顺序" & _
        " From 电子病历记录 l," & _
        "      (Select Max(c.开始版) As 开始版, Max(Id + 1) As Id From 电子病历内容 c Where c.文件id = [1] And c.对象类型 = 8) c" & _
    ...
    mSignLevel = GetUserSignLevel(glngUserId)
    If mSignLevel <= cprSL_空白 Then Me.cmdUntread.Enabled = False
    
    Me.Show vbModal, objOwner
