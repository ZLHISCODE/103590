    ...
    For i = 0 To intCountItem '纵
        ...
        For j = 2 To intGuageRow '横
            ...
            If j = 2 Then
                ...
                '--根据最高行决定刻度输入
                lngY = Y + H_9pt \ 2 + (intItemTop - 2) * (H_9pt + H_9pt \ 2)
                ...
                Call DrawText(objDraw, X + IIf(GetSplitStr(aryItem(i), 2) = "体温", 50, 0), lngY, strItem, lngColor)
                lngY = lngY + objDraw.TextHeight("1")
                Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth(CStr(lngTmpVal))) / 2, lngY, lngTmpVal, lngColor)
                
                Y = Y + H_9pt + IIf(intItemTop > 2, H_9pt + H_9pt / 2, 0)
            ...
            ElseIf j >= intItemTop And lngTmpVal >= CLng(GetSplitStr(aryItem(i), 4)) Then
                ...
                If (lngTmpVal - Fix(lngTmpVal)) = 0 Then
                    ...
                    Case "脉搏", "心率"
                        If lngTmpVal Mod 20 = 0 Then
                            If CStr(lngTmpVal) <> "200" Then
...
'            objOut.SetPages
    ...
    mstrSQL = " Select RowNum-1 As 序号,A.* From (Select Decode(A.项目序号,4,'血压',A.记录名) as 项目名,C.项目单位 As 单位,A.项目序号,A.记录频次,C.项目性质 " & _
                ...
                " And (c.适用科室=1 Or (c.适用科室=2 And Exists (Select 1 From 护理适用科室 D Where D.项目序号=c.项目序号 And D.科室id=[3]))) " & _
                " Order by DECODE(A.项目序号,3,0,1),A.排列序号) A"
    ...
    If rsTemp.RecordCount > 0 Then
        rsTemp.MoveFirst
        intDrawLineRows = MAXROWS + 2  '目前西安周至脉搏范围为40-200 目前总行数为 83行
    Else
        CloseRs rsTemp
        GoTo ErrPrint
    End If
