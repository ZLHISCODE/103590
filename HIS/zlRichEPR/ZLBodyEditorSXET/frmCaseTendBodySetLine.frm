...
'        .TextMatrix(GraphDataRow.断开标志, mintNowCol + .FixedCols) = chk(0).Value
        ...
        For intCount = 1 To vsf.Rows - 1
            ...
            If Trim(vsf.TextMatrix(intCount, mCol.数据)) <> "" Then
                ...
                dbValue = ((Val(vsf.TextMatrix(intCount, mCol.最大值)) - Val(vsf.TextMatrix(intCount, mCol.数据))) / Val(vsf.TextMatrix(intCount, mCol.单位值)) + Val(vsf.TextMatrix(intCount, mCol.最高行)) - 2) * .ROWHEIGHT(1)
                aryValue(intCount) = dbValue
                
                If Trim(vsf.TextMatrix(intCount, mCol.标记)) <> "" Then
                    dbValue = ((Val(vsf.TextMatrix(intCount, mCol.最大值)) - Val(vsf.TextMatrix(intCount, mCol.标记))) / Val(vsf.TextMatrix(intCount, mCol.单位值)) + Val(vsf.TextMatrix(intCount, mCol.最高行)) - 2) * .ROWHEIGHT(1)
        ...
        For intCount = 0 To .FixedCols - 1
            ...
            If Trim(aryValue(intCount + 1)) <> "" Then
                For intCol = 0 To UBound(Split(aryValue(intCount + 1), ","))
                    ...
                    dblValue = aryPara(0) - (Val(Split(aryValue(intCount + 1), ",")(intCol)) / .ROWHEIGHT(1) - aryPara(3) + 2) * aryPara(2)
