   ...
   Begin VB.PictureBox picPane 
      ...
      Begin VB.PictureBox pic 
         ...
         Begin VB.PictureBox picLine 
            ...
            Width           =   15
         End
         Begin zl9BodyEditorSXET.VsfGrid vsf 
            Height          =   270
            Left            =   450
            TabIndex        =   5
            Top             =   4440
...
Private Const ROWHEIGHT = 21                       '最小单元格的高度*5,原39
Private Const MAXROWS = 100                          '原47
...
Public Function ConvertToValue(ByVal intNo As Integer, ByVal Y As Long) As Double
    ...
    ConvertToValue = aryValue(0) - (Y / mshScale.ROWHEIGHT(1) - aryValue(3) + 2) * aryValue(2)
...
Public Function ConvertToY(ByVal intCol As Integer, ByVal dbValue As Double) As Long
    ...
    ConvertToY = ((Val(aryValue(0)) - dbValue) / Val(aryValue(2)) + Val(aryValue(3)) - 2) * mshScale.ROWHEIGHT(1)
...
'        Next
        ...
        For intCur = arrData(1) To intMax Step intStep
            If intCur = intMax Then
                If intCol = mItemSerial.体温 Then
                    DrawText picLable(intCol), 80, sinHeight * (MAXROWS - 9), "体温"
                    DrawText picLable(intCol), 80, sinHeight * (MAXROWS - 7), "(℃)"
                Else
                    DrawText picLable(intCol), 20, sinHeight * (MAXROWS - 9), "脉搏"
                    DrawText picLable(intCol), 20, sinHeight * (MAXROWS - 7), "(次/分)"
                End If
                Y0 = ConvertToY(intCol, intCur)
                DrawText picLable(intCol), 80, IIf(Y0 = 0, Y0, Y0 - sinHeight), intCur
            Else
                DrawText picLable(intCol), 80, ConvertToY(intCol, intCur) - sinHeight, intCur
            End If
        Next
    Next
End Sub
