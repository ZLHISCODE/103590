                                ...
                                If intCol Mod 2 = 1 Then
                                    ...
                                    If (Split(strValue(intRow), ";")(intCol + 2) <> "" And Split(strValue(intRow), ";")(intCol + 3) <> "") Or Val(Split(strValue(intRow), ";")(1)) = 2 Then
                                        ...
                                        Else
                                            ...
                                            Call DrawCell(objDraw, strTmp, lngTmpX + colFirstWidth + HOUR_STEP_Twips * 3 * (intCol - 1), Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2), HOUR_STEP_Twips * 3, (H_9pt + H_9pt \ 2), , , , lngColor, , objDraw.Font, , 1, 1)
                                        End If
                                    ...
                                    Else
                                        ...
                                        Else
                                            ...
                                            Call DrawCell(objDraw, strTmp, lngTmpX + colFirstWidth + HOUR_STEP_Twips * 6 * Fix(intCol / 2), Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2), HOUR_STEP_Twips * 6, (H_9pt + H_9pt \ 2), , , , lngColor, , objDraw.Font, , 1, 1)
                                        End If
...
Private Sub DrawBodyTopScale(objDraw As Object, X As Long, Y As Long, Optional sngScale As Long = 1)
    ...
    Dim lngHourBegin As Long, lngHourBegin1 As Long
...
'                objDraw.FontSize = 9
        ...
        '下午
        intCount = 0
        For j = lngHourBegin1 To 24 - (4 - lngHourBegin) Step 4
            intCount = intCount + 1
            If intCount = 3 Then
                lngColor = RGB(200, 0, 0)
            Else
...
'                    lngArrXY(lngCol, 0) = 2
                    ...
                    Case Else
                        If lng项目序号 = 3 Then
                            aryTmp = Split(aryData(0), ";")
                            strTag = IIf(aryTmp(5) = "呼吸机", "", mstrBreath)
                            '45987,刘鹏飞,2012-09-10,湖南需求,如果为呼吸机设置断开
                            lngArrXY(lngCol, 5) = IIf(aryTmp(5) = "呼吸机", 1, 0)
                        End If
                                        ...
                                        If blnStop = False Then
                                            If lng项目序号 = 3 Then
                                                If strTag = "" Then
                                                    blnStop = True
                                                Else
                                                    aryTmp = Split(Split(strArrValue(intItemDrawCount), "'")(lngUpIndex + 1), ",")
                                                    If Val(aryTmp(0)) <> -2 Then
                                                        If Split(aryTmp(0), ";")(5) = "呼吸机" Then blnStop = True
                                                    End If
                                                End If
                                            End If
                                        End If
...
Private Function DrawBodyScale(objDraw As Object, X As Long, Y As Long, intGuageRow As Integer, arrstrItem() As String, Optional sngScale As Long = 1)
    ...
    For i = 0 To intCountItem + 1
        Call DrawCell(objDraw, 1, X, Y, 0, (intGuageRow) * (H_9pt + H_9pt \ 2) + H_9pt * 4)

        If i = 0 Or i = intCountItem + 1 Then
            Call DrawLine(objDraw, X, Y, X, Y + (intGuageRow) * (H_9pt + H_9pt \ 2) + H_9pt * 4, , , 2)
        End If
    ...
    For i = Index To Index + mintOpDays
        ...
        Case 0
            ...
            If mblnStopFlag Then
                ...
                End If
            Else
                lngArryOPSDay(i) = IIf(lngArryOPSDay(i) <> "" And lngArryOPSDay(i) <> "-1", lngArryOPSDay(i) & "/" & strTmp, strTmp)
            End If
...
'                strTmpString1 = strTmpString1 & "'" & Format(Int(CDate(strTmpDay)) + i - Int(CDate(strBeginDate)) + 1)
        ...
        For l = 0 To UBound(strArrItemInfo)
            ...
            '1.呼吸显示为曲线；2、呼吸统一用黑色实行点（●）表示，不在体温单上显示呼吸数字
            '3. 使用呼吸机的患者，呼吸以黑R表示，在相应时间内呼吸30次横线下顶格用黑笔划R，相邻的R之间以及R和自主呼吸之间不连线
            mstrSQL = "SELECT A.发生时间 As 时间,Decode(D.项目序号,3,Decode(C.体温部位,'呼吸机','29',C.记录内容) ,c.记录内容) As 数值,c.记录标记,c.体温部位,c.复试合格 " & _
        ...
        If intDrawGridRows > 0 Then
            ...
            For i = LBound(strArrRecordItemInfo) To UBound(strArrRecordItemInfo)
                ...
                mstrSQL = "SELECT A.发生时间 As 当时产生的时间,C.记录内容 As 说明,E.保留项目,D.记录名,D.项目序号,DECODE(nvl(D.记录频次,0),1,1,2) 记录频次 " & _
                            "FROM 病人护理记录 A,病人护理内容 C,体温记录项目 D,护理记录项目 E " & _
                            "Where A.ID=C.记录id " & _
                                "AND A.病人id=[1] " & _
                                "AND A.主页id=[2]  And Nvl(a.婴儿,0)=[7] " & _
