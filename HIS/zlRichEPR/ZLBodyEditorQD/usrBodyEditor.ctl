   ...
   Begin VB.PictureBox picPane 
      ...
      Begin VB.PictureBox pic 
         ...
         Begin VB.PictureBox picLine 
            ...
            Width           =   15
         End
         Begin zl9BodyEditorQD.VsfGrid vsf 
            Height          =   270
            Left            =   450
            TabIndex        =   5
            Top             =   4440
...
Private Function InitData() As Boolean
    ...
    '检查是否有曲线体温项目
    '------------------------------------------------------------------------------------------------------------------
    mstrSQL = " Select C.项目序号 From 体温记录项目 A,诊治所见项目 B,护理记录项目 C " & _
                "Where C.项目序号=A.项目序号 " & _
                        "AND C.项目ID=B.ID(+) " & _
                        "AND C.护理等级>=[1] " & _
                        "And A.记录法=1 And C.项目序号<>-1 "    '只提取体温\脉搏
                
    Set rs = zlDatabase.OpenSQLRecord(mstrSQL, mstrMsgTitle, Val(mrsParam("护理等级")))
    If rs.RecordCount <> 2 Then
        ShowSimpleMsg "按政策要求只允许设置体温、脉搏为体温曲线项目！"
    ...
    If rs.BOF = False Then
        If IsNull(rs.Fields(0).Value) = False Then
            ...
            Do While Not rs.EOF
                ...
                mshScale.ColWidth(rs.AbsolutePosition - 1) = IIf(Val(rs("项目号").Value) = 1, 1200, 400)
                mshScale.TextMatrix(0, rs.AbsolutePosition - 1) = rs!项目名 & IIf(rs!项目号 = 1, vbCrLf & " F      C  ", "")
                ...
                For intRow = 1 To mshScale.Rows - 1
                    ...
                    If intRow >= rs!最高行 And rs!最大值 - (intRow - rs!最高行) * rs!单位值 >= rs!最小值 - IIf(rs!项目名 = "体温", 1, 0) Then
                        ...
                        If Int(rs("最大值").Value - (intRow - rs!最高行) * rs("单位值").Value) = rs!最大值 - (intRow - rs!最高行) * rs!单位值 Then
                            ...
                            Case "脉搏", "心率", "呼吸"
                                ...
                                End If
                            Case "体温"
                                strValue = rs!最大值 - (intRow - rs!最高行) * rs!单位值
                                Call GetValue(strValue)
                                mshScale.TextMatrix(intRow, rs.AbsolutePosition - 1) = strValue
                            'Case "呼吸"
                            '    mshScale.TextMatrix(intRow, rs.AbsolutePosition - 1) = rs!最大值 - (intRow - rs!最高行) * rs!单位值
                            End Select
...
'    lblTime.Caption = "日期:" & Format(strFrom, "MM-DD") & "～" & Format(strTo, "MM-DD")
    ...
    '读取病人指定时间段内最后科室、床号等信息
    txtCard(2).Text = ""
    txtCard(4).Text = ""
    mstrSQL = " Select  c.名称 As 科室,b.名称 As 病区,a.床号,a.开始原因 " & _
                "From 病人变动记录 a,部门表 b,部门表 c " & _
                "Where a.病人id=[1] And a.主页id=[2] And a.科室id Is Not Null And a.病区id=b.id and a.科室id=c.id And a.开始时间-4/24<=[3] And Nvl(a.终止时间,Sysdate)>=[4] Order By a.开始时间 desc"

    Set rsTmp = zlDatabase.OpenSQLRecord(mstrSQL, mstrMsgTitle, Val(mrsParam("病人id")), Val(mrsParam("主页id")), CDate(strTo), CDate(strFrom))
    If rsTmp.BOF = False Then
        txtCard(2).Text = zlCommFun.NVL(rsTmp("科室").Value)
        txtCard(4).Text = zlCommFun.NVL(rsTmp("床号").Value)
...
'        If Left(txtCard(4).Text, 2) = "->" Then txtCard(4).Text = Mid(txtCard(4).Text, 3)
    ...
    For intCol = 0 To mshScale.FixedCols - 1
        ...
        aryItemName(intCol) = IIf(intCol = mItemSerial.脉搏, "脉搏", "体温")
        ...
        Do While Not .EOF
            ...
            If (intCol - mshScale.FixedCols) < 42 Then
                ...
                If blnAllow Then
                    ...
                    '填写未记说明(低于35度或不升,都以不升来显示)
                    If zlCommFun.NVL(!数值) = "不升" Or (Val(zlCommFun.NVL(!数值)) <= 35 And IsNull(!未记说明)) Then
...
Private Function DrawScale() As Boolean
    ...
    For intCol = 1 To mshUpTab.Cols - 1
        ...
        Y0 = 0
        DrawLine picScale, X0 - HOUR_STEP_Twips * 3, Y0, X0 - HOUR_STEP_Twips * 3, Y1, &H8000000A
        DrawLine picScale, X0, Y0, X0, Y1, vbRed, , 2
        ...
        For i = 6 To 1 Step -1
            ...
            If picScale.Tag <> "" Then
                DrawText picScale, X0 - HOUR_STEP_Twips * i + 135 - picScale.TextWidth(strTmp) / 2, 400, strTmp, IIf(intCol * 6 - i >= intMinCol And intCol * 6 - i <= intMaxCol, lngColor, &H8000000A)
            End If
        Next
    Next
End Function
...
Public Function DrawPaper() As Boolean
    ...
    For intCol = 1 To mshUpTab.Cols - 1
        ...
        DrawLine picGraph, X0 - HOUR_STEP_Twips * 0, Y0, X0 - HOUR_STEP_Twips * 0, Y1, vbRed, , 2
    Next
                            ...
                            If i = 0 Then
                                ...
                                Case mItemSerial.体温
                                    ...
                                    Case Is <= GetMinValue(intCol)
                                        ...
                                        Y1 = ConvertToY(intCol, GetMinValue(intCol))
                                        '输出黑色的不升二字
                                        Call DrawText(picGraph, X1 - 90, Y1, "不")
                                        Call DrawText(picGraph, X1 - 90, Y1 + 180, "升")
                                    Case Is >= GetMaxValue(intCol)
                                        '体温42度以上
                                        strChar = "·"
                                        Y1 = ConvertToY(intCol, GetMaxValue(intCol))
                                    ...
                                    If Val(.TextMatrix(10, intCount + .FixedCols)) = 1 Then
                                        '复试合格
                                        Call DrawText(picGraph, X1 - 50, Y1 - 250, "√", vbRed)
                                    End If
                                ...
                                If (DateDiff("d", CDate(strDate0), CDate(strDate1)) <= 1) Then
                                    If blnStop = False Then
                                        ...
                                        If intCol = mItemSerial.体温 And (Split(.TextMatrix(GraphDataRow.未记说明, intCount + .FixedCols), ";")(intCol + 1) = "不升") Then
                                            'nothing to do
                                        Else
                                            DrawLine picGraph, X0, Y0, X1, Y1, 0
                                        End If
                                    End If
                                End If
                ...
                If Trim(strComment) <> "" Then
                    ...
                    For intCount = 1 To Len(strComment)
                        If Y1 <= lngYMax Then
                            ...
                            Call DrawRotateText(picGraph, X1 - picGraph.TextWidth(strChar) / 2, Y1 + 15, strChar, vbRed)
...
Private Sub mshScale_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    With mshScale
        intCol = IIf((X - .Left) >= .ColWidth(0), 1, 0)
        If Button = 1 Then
...
Private Sub picPane_Resize()
    ...
    For intCol = 0 To picLine.UBound
        If intCol = 0 Then
            picLine(intCol).Move mshScale.Left + mshScale.ColWidth(0), mshScale.Top, 15, mshScale.Height - 15
        Else
            picLine(intCol).Move mshScale.Left + mshScale.ColWidth(0) + mshScale.ColWidth(1), mshScale.Top, 15, mshScale.Height - 15
        End If
    Next
