...
'    Erase mvarDataType
    ...
    '------------------------------------------------------------------------------------------------------------------
    '求出最高行
    mstrSQL = " Select Max((A.最大值-A.最小值)/Decode(A.单位值,0,1,A.单位值)+Decode(Sign(Nvl(A.最高行, 2) - 2), 0, 2, 1, 2 + Decode(Mod(A.最高行, 2), 0, 0, 1) + (A.最高行 - 2) / 2)) From 体温记录项目 A Where A.记录法=1 "
    ...
    If rs.BOF = False Then
        If IsNull(rs.Fields(0).Value) = False Then
            ...
            mstrSQL = " Select A.记录法,A.记录名 as 项目名,A.项目序号 as 项目号,Nvl(B.ID,0) as 项目ID," & _
                        " C.项目单位 As 单位,记录符,最小值,最大值,记录色,1 as 记录否,单位值,Decode(Sign(Nvl(A.最高行, 2) - 2), 0, 2, 1, 2 + Decode(Mod(A.最高行, 2), 0, 0, 1) + (A.最高行 - 2) / 2) 最高行,Nvl(B.类型,1) as 存储类型 " & _
...
Private Function SaveData() As Boolean
    ...
    With mshUpTab
        ...
        If .Tag = "填写手术日" Or .Tag = "删除手术日" Then
            For intCOl = .FixedCols To .Cols - 1
                ...
                If strStart <> "" Then
                    If Format(strStart, "YYYY-MM-DD HH:mm:ss") < Format(mstrEnterDate, "YYYY-MM-DD HH:mm:ss") Then
                        strStart = Format(mstrEnterDate, "YYYY-MM-DD HH:mm") & ":00"
                        strEnd = Format(mstrEnterDate, "YYYY-MM-DD HH:mm:ss")
                    End If
                    strTmp = ""
    ...
    With mshScale
        ...
        For intCOl = .FixedCols To .Cols - 1
            ...
            '如果发生时间小于体温单起始时间，则以起始时间为准
            If Format(strMakeTime, "YYYY-MM-DD HH:mm:ss") < Format(mstrEnterDate, "YYYY-MM-DD HH:mm:ss") Then strMakeTime = Format(mstrEnterDate, "YYYY-MM-DD HH:mm") & ":00"
        ...
        For intCount = 0 To .FixedCols - 1
            ...
            For intCOl = .FixedCols To .Cols - 1
                ...
                Else
                    strEnd = strTime
                    strMakeTime = strTime
                    If Format(strEnd, "YYYY-MM-DD HH:mm:ss") < Format(mstrEnterDate, "YYYY-MM-DD HH:mm:ss") Then strEnd = mstrEnterDate
                    blnHistoryData = True
                End If
...
'                End If
    ...
    With vsf
        For intCOl = 2 To .Cols - 1
            ...
            If Format(strTime, "YYYY-MM-DD HH:mm:ss") < Format(mstr最小时间, "YYYY-MM-DD HH:mm:ss") Then strTime = mstr最小时间
                ...
                For intCount = 0 To .Rows - 2
                    ...
                    For intLoop = intCOl To intCOl + (int频次 - 1)
                       ...
                       If Not IsDate(arrItem(intLoop - .FixedCols)) Then
                        ...
                        If Format(strStart, "YYYY-MM-DD HH:mm:ss") < Format(mstr最小时间, "YYYY-MM-DD HH:mm:ss") Then strStart = mstr最小时间
...
Private Sub mshUpTab_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Case vbKeyReturn ' 13     '设置手术日
        With mshUpTab
            ...
            If Format(strTime, "YYYY-MM-DD HH:mm:ss") < Format(CStr(mrsParam("开始时间")), "YYYY-MM-DD HH:mm:ss") Then strTime = CStr(mrsParam("开始时间"))
            If frmInputDate.ShowMe(strTime, CStr(mrsParam("开始时间")), Split(picScale.Tag, ";")(1), strCaption) Then
