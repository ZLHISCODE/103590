...
Private Function DrawBodyInfo(objDraw As Object, X As Long, Y As Long, ByVal colFirstWidth As Long, _
    ...
    Call DrawCell(objDraw, "术/娩后日数", X, Y + (H_9pt + H_9pt \ 2) * 2, colFirstWidth, H_9pt + H_9pt \ 2, 0, 0, , , , objDraw.Font, "1111", 1, 1, False)
...
'            objOut.SetPages
    ...
    '得到所有曲线项目
    mstrSQL = " Select c.项目序号,A.记录名 as 项目名,A.记录符,To_Char(A.记录色)||''''||To_Char(Decode(Sign(Nvl(A.最高行, 2) - 2), 0, 2, 1, 2 + Decode(Mod(A.最高行, 2), 0, 0, 1) + (A.最高行 - 2) / 2))||''''||A.记录名||''''||To_Char(A.最大值)||''''||To_Char(A.最小值)||''''||To_Char(A.单位值)||''''||A.记录符||''''||To_Char(A.项目序号)||''''||c.项目单位 As 列表 " & _
    ...
    '4、求得首列宽，求左边标尺总共有多少行
    '求得体温表项目的总行数
    strSQL = "Select Max((A.最大值-A.最小值)/Decode(A.单位值,0,1,A.单位值)+Decode(Sign(Nvl(A.最高行, 2) - 2), 0, 2, 1, 2 + Decode(Mod(A.最高行, 2), 0, 0, 1) + (A.最高行 - 2) / 2)) as 总高度 From 体温记录项目 A Where A.记录法=[1]"
