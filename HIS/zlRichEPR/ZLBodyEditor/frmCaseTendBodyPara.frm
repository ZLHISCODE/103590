...
Public Function ShowPara(ByVal frmMain As Object, ByVal strPrivs As String) As Boolean
    ...
    strTmp = Val(zlDatabase.GetPara("未记说明显示位置", glngSys, 1255, "0", Array(chk(2)), InStr(mstrPrivs, "护理选项设置") > 0))
    chk(2).Value = IIf(Val(strTmp) = 1, 1, 0)
    ...
    '新版体温单呼吸输出方式包含四种:从上到下(无数据继承/有数据继承)、从下到上(无数据继承/有数据继承)
    strTmp = Val(zlDatabase.GetPara("呼吸表格输出", glngSys, 1255, "0", Array(chk(4)), InStr(mstrPrivs, "护理选项设置") > 0))
    If Val(strTmp) = 0 Or Val(strTmp) = 2 Then
        chk(4).Value = 1
    Else
        chk(4).Value = 0
    End If
