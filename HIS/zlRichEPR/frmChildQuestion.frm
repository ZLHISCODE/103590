...
Begin VB.Form frmChildQuestion 
   ...
   ClientTop       =   450
   ClientWidth     =   8070
   Icon            =   "frmChildQuestion.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   7695
   ScaleWidth      =   8070
   ...
   Begin MSComctlLib.StatusBar stbThis 
      ...
      BeginProperty Panels {8E3867A5-8586-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty Panel1 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            ...
            Object.Width           =   2355
            MinWidth        =   882
            Picture         =   "frmChildQuestion.frx":6852
            Text            =   "中联软件"
            TextSave        =   "中联软件"
            Key             =   "ZLFLAG"
         EndProperty
   ...
   Begin XtremeDockingPane.DockingPane dkpMain 
      Bindings        =   "frmChildQuestion.frx":70E4
      Left            =   810
      Top             =   0
      _Version        =   589884
      _ExtentX        =   450
...
Private mstr数据转出 As String
Private mlng出院科室ID As Long
...
Public Function SetParamter(ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal strObject As String, ByVal strParam As String, Optional ByVal lng提交Id As Long = 0) As Boolean
    ...
    Set rsTemp = GetAuditInfo(lng病人ID, lng主页ID)
    If rsTemp.RecordCount = 1 Then
         mstr数据转出 = NVL(rsTemp!数据转出)
         mlng出院科室ID = NVL(rsTemp!出院科室ID)
    End If
    
    
    '读取病案结构
    Set RsType = GetCISStruct(lng病人ID, lng主页ID, Val(mlng出院科室ID), Val(mstr数据转出) = 1)
    Set rsEmr = GetEmrCISStruct(lng病人ID, lng主页ID)
    '显示病案结构
    Call mfrmChildQuestionEdit.Init病案类型(RsType, rsEmr)
...
Private Function ExecuteCommand(ByVal strCmd As String, ParamArray varParam() As Variant) As Boolean
    ...
    Case "刷新指定反馈"
        ...
        Set rs = GetQuestion(mrsCondition, "", 1, mlngTmp)
    ...
    Case "增加反馈记录"
        ...
        '检查是否已经启动了方案
        If GetExamineStartUse = False Then
            Call MsgBox("必须在电子病案审查项目中启动一个审查方案,才能添加病案反馈!", vbQuestion + vbDefaultButton2, "电子病案审查评分")
            GoTo EndHand
        End If
    ...
    Case "复制反馈记录"
        ...
        With vsf(mintIndex)
            '检查并获取相关的反馈记录
            Set rs = GetRelevanceID(Val(.RowData(.Row)))
            If Not rs.EOF Then
                If NVL(rs!相关ID) = "" Then
                    mlngReferKey = -1
                Else
    ...
    Case "删除反馈记录"
        ...
        With vsf(0)
            ...
            If MsgBox("您是否真的要删除当前反馈问题吗？", vbQuestion + vbYesNo + vbDefaultButton2, "电子病案审查评分") = vbYes Then
    ...
    Case "完成反馈记录"
        ...
        With vsf(mintIndex)
            ...
            If MsgBox("您是否真的要结束当前反馈问题吗？", vbQuestion + vbYesNo + vbDefaultButton2, "电子病案审查评分") = vbYes Then
    ...
    Case "回退完成反馈"
        ...
        With vsf(mintIndex)
            ...
            If MsgBox("您是否真的要回退当前已结束的反馈问题吗？", vbQuestion + vbYesNo + vbDefaultButton2, "电子病案审查评分") = vbYes Then
    ...
    Case "读取未改反馈"
        ...
        If mblnCurrentPatient Then
            Set rs = GetQuestion(mrsCondition, mstrDepts, 2, , , , mlng病人ID, mlng主页ID, mlngCurNum, mstr抽查开始时间, mstr抽查结束时间, mstr反馈人)
        Else
            Set rs = GetQuestion(mrsCondition, mstrDepts, 2, , , , , , mlngCurNum, mstr抽查开始时间, mstr抽查结束时间, mstr反馈人)
        End If
    ...
    Case "读取未审反馈"
        ...
        If mblnCurrentPatient Then
            Set rs = GetQuestion(mrsCondition, mstrDepts, 3, , , , mlng病人ID, mlng主页ID, mlngCurNum, mstr抽查开始时间, mstr抽查结束时间, mstr反馈人)
        Else
            Set rs = GetQuestion(mrsCondition, mstrDepts, 3, , , , , , mlngCurNum, mstr抽查开始时间, mstr抽查结束时间, mstr反馈人)
        End If
        If rs.BOF = False Then
    ...
    Case "读取完成反馈"
        ...
        If mblnCurrentPatient Then
            Set rs = GetQuestion(mrsCondition, mstrDepts, 4, , mstrStart, mstrEnd, mlng病人ID, mlng主页ID, mlngCurNum, mstr抽查开始时间, mstr抽查结束时间, mstr反馈人)
        Else
            Set rs = GetQuestion(mrsCondition, mstrDepts, 4, , mstrStart, mstrEnd, , , mlngCurNum, mstr抽查开始时间, mstr抽查结束时间, mstr反馈人)
        End If
    ...
    Case "刷新次数"
        ...
        If mstr日期选择 = "自定义" Or mstr日期选择 = "所  有" Then
...
'        call ExecuteCommand("刷新数据")
    ...
    Case "读注册表"
        ...
        mblnCurrentPatient = True '(Val(zlDatabase.GetPara("当前病人", glngSys, mlngMoudal, "0")) = 1)
        
        On Error GoTo errHand

        If Val(zlDatabase.GetPara("使用个性化风格")) = 1 Then
            '使用个性化设置
            mclsVsf(0).LoadStateFromString Trim(GetRegister(私有模块, Me.Name, "表格参数_0_20120926", ""))
            mclsVsf(1).LoadStateFromString Trim(GetRegister(私有模块, Me.Name, "表格参数_1_20120926", ""))
            mclsVsf(2).LoadStateFromString Trim(GetRegister(私有模块, Me.Name, "表格参数_2_20120926", ""))
        End If
    ...
    Case "写注册表"
        ...
        Call SetRegister(私有模块, Me.Name, "表格参数_0_20120926", mclsVsf(0).SaveStateToString)
        Call SetRegister(私有模块, Me.Name, "表格参数_1_20120926", mclsVsf(1).SaveStateToString)
        Call SetRegister(私有模块, Me.Name, "表格参数_2_20120926", mclsVsf(2).SaveStateToString)
...
'            Control.Enabled = (Control.Visible And DataChanged = False And AllowModify And mlng病人ID > 0)
        ...
        Case conMenu_View_Filter
            Control.Visible = False '(IsPrivs(mstrPrivs, "院级反馈") And IsPrivs(mstrPrivs, "科级反馈")) Or (IsPrivs(mstrPrivs, "院级反馈") And IsPrivs(mstrPrivs, "科级反馈") = False)
...
Private Sub dkpMain_AttachPane(ByVal Item As XtremeDockingPane.IPane)
    ...
    Case 2
        If ObjPtr(mfrmChildQuestionEdit) = 0 Then
        ...
        mfrmChildQuestionEdit.RefActiveFrom
    End Select
End Sub
...
Private Sub mfrmChildQuestionEdit_AfterParaments(ByVal strObject As String, ByVal strParam As String)
    ...
    Case "住院病历", "护理病历", "知情文件", "疾病证明"
        If strParam <> "" Then
            ...
            If IsNumeric(varParam(0)) Then
                mstr文件ID = Val(varParam(0))
            Else
                mstr文件ID = strParam
            End If
            mlng医嘱id = 0
...
Private Sub mfrmChildQuestionEdit_RefStatus()
    '重新刷新
     Dim objPane As Pane
     Set objPane = dkpMain.Panes(2)
     Call dkpMain.DestroyPane(objPane)
