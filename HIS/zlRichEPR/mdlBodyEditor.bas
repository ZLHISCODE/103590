Attribute VB_Name = "mdlBodyEditor"
Option Explicit
'----------------------------------------------------------------------------------------------
'说明:
...
Public mblnMoved_HL As Boolean                      '当前病人本次住院的护理数据是否已经转移
Public glngHours As Long                            '补录有效时限
...
Public Function CreateBodyEditor() As Boolean
    ...
    If gobjBodyEditor Is Nothing Then
        gstrSQL = " Select 部件 From 体温部件 Where Nvl(启用,0)=1"
        Call zlDatabase.OpenRecordset(rsTemp, gstrSQL, "提取体温部件")
        If Err <> 0 Then
            strDLL = "zl9BodyEditor"
        Else
        ...
        If Err <> 0 Then
            ...
            strDLL = "zl9BodyEditor.clsBodyEditor"
            Set gobjBodyEditor = CreateObject(strDLL)
        End If
...
Public Function GetFormName(ByVal PrinterHandle As Long, FormSize As Size, FormName As String) As Integer
...
Public Function AddNewForm(lngPrtHandle As Long, vFormSize As Size, strFormName As String) As String
...
'        dtFrom = dtFrom - 14
    ...
    Case 2              '入出转标志(入院,出院,转科,换床)
        ...
        strSQL = _
                ...
                "  Union All" & vbNewLine & _
                "    Select '' As 科室,Decode(出院方式,'死亡',Nvl(b.开始执行时间,a.出院日期),a.出院日期) As 时间, Decode(出院方式, '正常', '出院', 出院方式) As 内容,8 As 行号 " & vbNewLine & _
                "    From 病案主页 A,(Select x.病人id,x.主页id,Max(x.开始执行时间) As 开始执行时间 From 病人医嘱记录 x Where x.病人id=[1] And x.主页id=[2] And x.诊疗项目id+0=[6] And x.医嘱状态=3 Group By x.病人id,x.主页id) B " & vbNewLine & _
        ...
        Set GetDataFromHis = zlDatabase.OpenSQLRecord(strSQL, "体温单", lng病人ID, lng主页ID, lng婴儿, dtFrom, dtTo, lng诊疗项目id)
    '------------------------------------------------------------------------------------------------------------------
...
'            objOut.SetPages
    ...
    strSQL = " Select  NVL(A.姓名,b.姓名) 姓名,A.住院号,A.入院日期 入院时间,NVL(A.性别,b.性别) 性别,NVL(A.年龄,b.年龄) 年龄" & _
        " From 病人信息 B,病案主页 A Where B.病人ID=A.病人Id And A.病人id=[1] And A.主页id=[2] "
    ...
    Case Else
        ...
        gstrSQL = "Select Decode(a.婴儿姓名,Null,NVL(c.姓名,b.姓名) ||'之子'||Trim(To_Char(a.序号,'9')),a.婴儿姓名) As 婴儿姓名,a.婴儿性别,a.出生时间" & _
            " From 病人信息 b,病案主页 C,病人新生儿记录 a Where b.病人id=c.病人id And c.病人id=a.病人id And c.主页id=a.主页id And c.病人id=[1] And c.主页id=[2] And a.序号=[3]"
