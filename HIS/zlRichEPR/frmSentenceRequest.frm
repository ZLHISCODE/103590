...
Begin VB.Form frmSentenceRequest 
   ...
   ClientTop       =   3750
   ClientWidth     =   6615
   Icon            =   "frmSentenceRequest.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.Image imgNote 
      Height          =   240
      Left            =   150
      Picture         =   "frmSentenceRequest.frx":000C
      Top             =   60
      Width           =   240
   End
   Begin VB.Label lblNote 
      AutoSize        =   -1  'True
      Caption         =   "根据词句示范的适用范围，可以基于以下项目设置词句的特定应用条件。"
      Height          =   180
      Left            =   525
      TabIndex        =   8
      Top             =   90
   ...
   End
End
Attribute VB_Name = "frmSentenceRequest"
...
'以下为外部公共程序
'-----------------------------------------------------
Public Function ShowMe(ByVal frmParent As Form, ByVal lngWordId As Long) As Boolean
    '功能：显示本编辑窗体
    '参数： frmParent-父窗体
    '       lngWordId-词句示范ID
    ...
    gstrSQL = "Select 分类id From 病历词句示范 Where ID = [1]"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, lngWordId)
    If rsTemp.RecordCount <= 0 Then MsgBox "当前词句不存在(可能被其他用户删除)！", vbInformation, gstrSysName: Exit Function
    Me.cmdApply.Tag = rsTemp!分类id
...
Private Function RefList(Optional strItem As String) As Boolean
    ...
    Err = 0: On Error GoTo ErrHand
    gstrSQL = "Select 名称 As 条件项, 简码 As 条件值 From Table(Cast(f_Sentence_条件项([1]) As " & gstrDbOwner & ".t_Dic_Rowset))"
...
Private Sub cmdApply_Click()
    ...
    If MsgBox("真的将该词句条件应用于当前分类的所有词句吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
        Me.vfgItems.SetFocus: Exit Sub
    End If
    gstrSQL = "Zl_病历词句条件_Apply(" & mlngWordId & ")"
...
Dim strItem As String, strTerm As String
    ...
    If Index = 0 Then
        ...
        gstrSQL = "Zl_病历词句条件_Edit(" & mlngWordId & ",'" & strItem & "','" & strTerm & "')"
        Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
        mblnOK = True
    Else
...
Private Sub vfgItems_AfterRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long)
    ...
    Err = 0: On Error GoTo ErrHand
    gstrSQL = "Select 名称 As 可选值 From Table(Cast(f_Sentence_可选值([1], [2]) As " & gstrDbOwner & ".t_Dic_Rowset))"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, mlngWordId, strItem)
