...
Begin VB.UserControl usrBodyEditor 
   ...
   Begin VB.PictureBox picPane 
      ...
      Begin VB.PictureBox pic 
         ...
         Top             =   225
         Width           =   10920
         Begin zl9BodyEditorSXHZ.VsfGrid vsf 
            Height          =   375
            Left            =   600
            TabIndex        =   41
            Top             =   4440
...
Private mstrEnterDate As String                      '体温单开始日期，缺省为入院日期，可指定为入科或转科日期
...
Public Function zlMenuClick(ByVal strMenuItem As String, Optional ByVal strParam As String) As Boolean
    ...
    Case "初始数据"
        ...
        If Not FaceInit Then Exit Function
        Call SetBodyMode
...
'             If InitBody(Val(mrsParam("病人id").Value), Val(mrsParam("主页id").Value), Val(mrsParam("病区id").Value), Val(mrsParam("婴儿").Value)) = False Then Exit Function
            ...
            Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "提取病案主页从表中的体温单开始日期", Val(mrsParam("病人id").Value), Val(mrsParam("主页id").Value))
            If rsTemp.RecordCount <> 0 Then
                mstrEnterDate = Format(rsTemp!信息值, "yyyy-MM-dd HH:mm:ss")
            End If
...
Private Function InitBody(ByVal lng病人ID As Long, ByVal lng主页id As Long, ByVal lng病区id As Long, ByVal int婴儿 As Integer) As Boolean
    ...
    '------------------------------------------------------------------------------------------------------------------
    strSQL = "Select Decode(c.出生时间,Null," & IIf(strEnterDate = "", "b.入院时间", "to_date('" & strEnterDate & "','yyyy-MM-dd hh24:mi:ss')") & ",c.出生时间) As 入院时间, 出院时间, 1 + Round((b.出院时间 - Decode(c.出生时间,Null," & IIf(strEnterDate = "", "b.入院时间", "to_date('" & strEnterDate & "','yyyy-MM-dd hh24:mi:ss')") & ",c.出生时间)) / 7) As 页数" & vbNewLine & _
    ...
    If rs.BOF = False Then
        If IsNull(rs.Fields(0).Value) = False Then
            ...
            Do While Not rs.EOF
                ...
                '设置表格内项目
                mshScale.ColWidth(rs.AbsolutePosition - 1) = 600
                picLable(rs.AbsolutePosition - 1).Width = mshScale.ColWidth(rs.AbsolutePosition - 1)
                If rs!项目名 = "呼吸" Or rs!项目名 = "体温" Then
                    mshScale.ColWidth(rs.AbsolutePosition - 1) = CLng(mshScale.ColWidth(rs.AbsolutePosition - 1) / 2)
                    picLable(rs.AbsolutePosition - 1).Width = mshScale.ColWidth(rs.AbsolutePosition - 1)
                End If
...
'                End If
    ...
    If mItemSerial.呼吸 < 0 Then
        MsgBox "呼吸必须设置为曲线项目！", vbInformation, gstrSysName
        Exit Function
    End If
    ...
    With vsf
        .Cols = 0
        .NewColumn "", 0, 1
        .NewColumn "项目", (mshScale.FixedCols - 1) * mshScale.ColWidth(0) + 15, 1
        For intCol = 1 To 42
   ...
   '------------------------------------------------------------------------------------------------------------------
    ...
    '初始上面表格
    With mshUpTab
        .ColWidth(0) = (mshScale.FixedCols - 1) * mshScale.ColWidth(0)
        .TextMatrix(0, 0) = "日       期"
...
Private Function AppendGridItem(ByVal rsTmp As ADODB.Recordset, Optional ByVal blnAppend As Boolean) As Boolean
    ...
    Case Else
        ...
        mshDownTab.ROWHEIGHT(mshDownTab.Rows - 1) = 255
...
'                End If
    ...
    '现有单元格中循环,对于手术后第一天进行标记
    For intCol = 1 To mshUpTab.Cols - 1
        
        If Right(mshUpTab.TextMatrix(2, intCol), 2) = "/1" Then
            If UBound(Split(mshUpTab.TextMatrix(2, intCol), "/")) >= 1 Then
                mshUpTab.TextMatrix(2, intCol) = mshUpTab.TextMatrix(2, intCol) & "(" & UBound(Split(mshUpTab.TextMatrix(2, intCol), "/")) + 1 & ")"
            End If
        End If
    Next
...
'    lblTime.Caption = "日期:" & Format(strFrom, "MM-DD") & "～" & Format(strTo, "MM-DD")
    ...
    '------------------------------------------------------------------------------------------------------------------
    With mshUpTab
        .Cell(flexcpForeColor, 1, 1, 1, .Cols - 1) = RGB(255, 0, 0)
        intSvrCol = .Col
        ...
        For intCol = 1 To .Cols - 1
            ...
            ElseIf Right(strTmp, 5) = "01-01" Then
                '一年的第一天
                .TextMatrix(0, intCol) = strTmp
...
Private Function DrawScale() As Boolean
    ...
    End If
    DrawText picScaleNote, 0, 100, " 每 日 时 间", &H80000012
...
Public Function DrawPaper() As Boolean
    ...
    For intRow = 1 To mshScale.Rows - 1
        ...
        If Not (intRow = 70 Or intRow = 88) Then    '警示线/辅助线的输出
            If (intRow - 1) Mod 5 = 0 Then
                If (intRow - 1) / 10 = 5 Then
                ...
                End If
            Else
                DrawLine picGraph, X0, Y0 + ROWHEIGHT * 5, X1, Y0 + ROWHEIGHT * 5, &H8000000A, 0 ' IIf((intRow - 1) Mod 10 = 5, 1, 2)
            End If
        Else
            DrawLine picGraph, X0, Y0 + ROWHEIGHT * 5, X1, Y0 + ROWHEIGHT * 5, &H8080FF, 0, 1
        End If
                            ...
                            If i = 0 Then
                                ...
                                Case mItemSerial.体温
                                    ...
                                    Case Is <= GetMinValue(intCol)
                                        ...
                                        strChar = "·"
                                        Y1 = ConvertToY(intCol, GetMinValue(intCol))
                                        Call DrawLine(picGraph, X1, Y1, X1, Y1 + 200, lngColor, , 1, True)
                                    ...
                                    If Val(.TextMatrix(10, intCount + .FixedCols)) = 1 Then
                                        '复试合格
                                        Call DrawText(picGraph, X1 - 50, Y1 - 250, "v", vbRed)
                                    End If
                                ...
                                If (DateDiff("d", CDate(strDate0), CDate(strDate1)) <= 1) Then
                                    If blnStop = False Then
                                        ...
                                        If intCol = mItemSerial.体温 And Split(.TextMatrix(GraphDataRow.未记说明, intCount + .FixedCols), ";")(intCol + 1) = "不升" Then
                                            'nothing to do
                                        Else
                                            DrawLine picGraph, X0, Y0, X1, Y1, .CellForeColor, , 1
                                        End If
                                    End If
                                    blnStop = False
                                End If
...
'            Y1 = ConvertToY(mItemSerial.体温, 42)
            ...
            Y1 = ConvertToY(mItemSerial.呼吸, 15)
            'Y1 = 7020
            strComment = ""
...
'                            End If
                            ...
                            Call DrawRotateText(picGraph, X1 - picGraph.TextWidth(strChar) / 2, Y1 + 15, strChar, RGB(255, 0, 0))
    ...
    With mshScale
        ...
        For intCol = .FixedCols To .Cols - 1
            ...
            Else
                ...
                If Mid(Format(strMakeTime, "yyyy-MM-dd HH:mm"), 12, 5) = "00:00" Then
                    strMakeTime = Format(DateAdd("h", 1, CDate(strTime)), "yyyy-MM-dd HH:mm:ss")
                End If
            End If
...
Private Sub picPane_Resize()
    ...
    End With
    dblLeft = mshScale.Left
    For intCol = 0 To picLine.UBound
        picLine(intCol).Move dblLeft, picScale.Top + picScale.Height, 15, picBack.Height - 15
        If intCol = 2 Then picLine(2).Visible = False
        picLable(intCol).Move dblLeft, picScale.Top + picScale.Height, picLable(intCol).Width, picBack.Height - 30
        dblLeft = dblLeft + picLable(intCol).Width
    Next
...
Private Sub DrawPicLable()
    ...
    For intCol = 0 To picLable.UBound       '体温与呼吸打印在同一刻度内,体温在右,呼吸在左下
        '非体温项,如果能整除10则标记;体温项整数即标记
        arrData = Split(picLine(intCol).Tag, ";")  '最大值;最小值
        intMax = arrData(0)
        intStep = IIf(intCol = mItemSerial.体温, 1, IIf(intCol = mItemSerial.呼吸, 5, 10))
        
        For intCur = arrData(1) To intMax Step intStep
            If Len(CStr(intCur)) = 2 Then
                DrawText picLable(intCol), 80, ConvertToY(intCol, intCur) - sinHeight, intCur, &H80000012
            Else
                DrawText picLable(intCol), 50, ConvertToY(intCol, intCur) - sinHeight, intCur, &H80000012
            End If
        Next
    Next
    
    '输出文字说明
    Call DrawRotateText(picLable(mItemSerial.呼吸), 80, ConvertToY(mItemSerial.体温, 41.7), "体", &H80000012)
    Call DrawRotateText(picLable(mItemSerial.呼吸), 80, ConvertToY(mItemSerial.体温, 41.5), "温", &H80000012)
    Call DrawRotateText(picLable(mItemSerial.呼吸), 120, ConvertToY(mItemSerial.体温, 39), "(", &H80000012)
    Call DrawRotateText(picLable(mItemSerial.呼吸), 40, ConvertToY(mItemSerial.体温, 38.8), "摄", &H80000012)
    Call DrawRotateText(picLable(mItemSerial.呼吸), 40, ConvertToY(mItemSerial.体温, 38.6), "氏", &H80000012)
    Call DrawRotateText(picLable(mItemSerial.呼吸), 40, ConvertToY(mItemSerial.体温, 38.4), "表", &H80000012)
    Call DrawRotateText(picLable(mItemSerial.呼吸), 120, ConvertToY(mItemSerial.体温, 38.2), ")", &H80000012)
    Call DrawRotateText(picLable(mItemSerial.脉搏), 80, ConvertToY(mItemSerial.脉搏, 30), "脉", &H80000012)
    Call DrawRotateText(picLable(mItemSerial.脉搏), 80, ConvertToY(mItemSerial.脉搏, 25), "搏", &H80000012)
    Call DrawRotateText(picLable(mItemSerial.体温), 80, ConvertToY(mItemSerial.体温, 32.5), "呼", &H80000012)
    Call DrawRotateText(picLable(mItemSerial.体温), 80, ConvertToY(mItemSerial.体温, 32.3), "吸", &H80000012)
End Sub
