...
Private Sub cmdOK_Click()
    ...
    gcnOracle.Execute "ZL_体温单开始日期_UPDATE(" & mlng病人ID & "," & mlng主页ID & ",'" & Mid(Me.cbo开始日期.Text, 2, 19) & "')", , adCmdStoredProc
...
Private Sub Form_Load()
    ...
    With rsTemp
        Me.cbo开始日期.Clear
        Do While Not .EOF
            Me.cbo开始日期.AddItem !内容
            .MoveNext
        Loop
        Me.cbo开始日期.ListIndex = 0
    End With
    ...
    '定位
    If str开始日期 <> "" Then
        intEnd = Me.cbo开始日期.ListCount
        For intStart = 1 To intEnd
            If InStr(1, Me.cbo开始日期.List(intStart - 1), str开始日期) <> 0 Then
                Me.cbo开始日期.ListIndex = intStart - 1
                Exit For
            End If
        Next
    End If
    
    '只要存在体温数据则不允许修改体温单开始日期
    gstrSQL = " Select 1 From 病人护理记录 A,病人护理内容 B Where B.记录ID=A.ID And B.项目序号 IN (1,2,3) and A.病人ID=[1] And A.主页ID=[2] And Rownum<2"
    Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption, mlng病人ID, mlng主页ID)
