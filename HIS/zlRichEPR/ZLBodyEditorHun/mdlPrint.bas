...
Private Const MAXROWS = 90                          '原46
Private Const ROWHEIGHT = 14.7                      '原35
                        ...
                        Else
                            ...
                            If int呼吸表格输出格式 = 0 Then
                                If intCOl Mod 2 = 0 Then
                                    Call DrawCell(objDraw, strTmp, lngTmpX + colFirstWidth + HOUR_STEP_Twips * (intCOl - 1), Y + intFactRow * (H_9pt + H_9pt \ 2), HOUR_STEP_Twips, (H_9pt + H_9pt \ 2), , , , , , objDraw.Font, "1000")
                                Else
                                    Call DrawCell(objDraw, strTmp, lngTmpX + colFirstWidth + HOUR_STEP_Twips * (intCOl - 1), Y + intFactRow * (H_9pt + H_9pt \ 2) + (H_9pt + H_9pt \ 2), HOUR_STEP_Twips, (H_9pt + H_9pt \ 2), , , , , , objDraw.Font, "0000")
                                End If
                            Else
                                If int呼吸位置 = 2 Then
                                    Call DrawCell(objDraw, strTmp, lngTmpX + colFirstWidth + HOUR_STEP_Twips * (intCOl - 1), Y + intFactRow * (H_9pt + H_9pt \ 2), HOUR_STEP_Twips, (H_9pt + H_9pt \ 2), , , , , , objDraw.Font, "1000")
...
Private Function DrawBodyInfo(objDraw As Object, X As Long, Y As Long, ByVal colFirstWidth As Long, _
    ...
    Call DrawCell(objDraw, "术后天数", X, Y + (H_9pt + H_9pt \ 2) * 2, colFirstWidth, H_9pt + H_9pt \ 2, 0, 0, , , , objDraw.Font, "1111", 1, 1, False)
    ...
    For intDay = 0 To 6
        ...
        If Right(strTmp, 5) = "01-01" Then
            '一年的第一天
            strTmp = Format(strTmp, "yyyy-MM-D")
        ElseIf strTmp = Format(strBeginDate, "yyyy-MM-dd") Then
            '入院第一天，写上年份
            strTmp = Format(strTmp, "yyyy-MM-D")
        ElseIf intDay = 0 Then
            strTmp = Format(strTmp, "MM-D") ' Right(strTmp, 5)
        ElseIf Right(strTmp, 2) = "01" Then
            strTmp = Format(strTmp, "MM-D") 'Right(strTmp, 5)
        Else
            strTmp = Val(Right(strTmp, 2))
        End If

        Call DrawCell(objDraw, strTmp, lngLeft + (HOUR_STEP_Twips * 6) * intDay, Y, (HOUR_STEP_Twips * 6), H_9pt + H_9pt \ 2, 0, 0, , , , objDraw.Font, "1111", 1, 1, False)
...
'                objDraw.FontSize = 9
        ...
        For j = lngHourBegin To 12 - (4 - lngHourBegin) Step 4
            ...
            If j = 12 - lngHourBegin Then
'                objDraw.FontSize = 8
                Call DrawCell(objDraw, CStr(j + 12), lngCurX, lngCurY, HOUR_STEP_Twips, (H_9pt + H_9pt \ 2), , , , lngColor, , objDraw.Font, , 1, 1, True)
                lngCurX = lngCurX + HOUR_STEP_Twips
            Else
'                objDraw.FontSize = 9
                Call DrawCell(objDraw, CStr(j + 12), lngCurX, lngCurY, HOUR_STEP_Twips, (H_9pt + H_9pt \ 2), , , , lngColor, , objDraw.Font, , 1, 1, True)
...
'    H_9pt = objDraw.TextHeight("字")
    ...
    lngYMax = (lngTmpY + 3 * H_9pt / 2) + 90 * 3 * H_9pt / 2 - H_9pt / 2
    Call PointInit(rsPoint)
    ...
    For lngCol = 0 To 41
        ...
        lngX = lngTmpX + lngCol * HOUR_STEP_Twips + HOUR_STEP_Twips / 2
        'lngY = (lngTmpY + 3 * H_9pt / 2) + 4 * H_9pt / 2
        'lngY1 = (lngTmpY + 3 * H_9pt / 2) + 70 * 3 * H_9pt / 2                          '得到35度的坐标
        'lngY2 = (lngTmpY + 3 * H_9pt / 2) + 70 * 3 * H_9pt / 2                          '得到35度的坐标
        'dblHeight = (lngTmpY + 3 * H_9pt / 2) + 18 * 3 * H_9pt / 2 - H_9pt / 2          '得到40度的坐标,好像是按格子数来计算
        lngY = lngTmpY + 3 * H_9pt / 2
        lngY1 = lngTmpY + 79 * 3 * H_9pt / 2                          '得到35度的坐标
        lngY2 = lngTmpY + 79 * 3 * H_9pt / 2                           '得到35度的坐标
        dblHeight = lngTmpY + 29 * 3 * H_9pt / 2
                ...
                For i = 1 To Len(strComment)
                    If lngY1 < lngYMax Then
                        ...
                        If intCharNumber <> 1 Then
                            lngY1 = lngY1 + objDraw.TextHeight("1") ' ROWHEIGHT * 10
                        Else
                            lngY2 = lngY2 + objDraw.TextHeight("1") ' ROWHEIGHT * 10
                        End If
                    End If
                Next
            End If
        ...
        For intItemDrawCount = 0 To intItemCount - 1
            ...
            For lngCol = 0 To 7 * 6 - 1 '纵
                ...
                Else
                    ...
                    For i = 0 To UBound(aryData)
                        'lngValue = (lngTopRow - 1) * lngStep + lngMax
                        lngValue = lngTopRow * lngStep + lngMax
...
Private Function DrawFillPoly(objDraw As Object, ByVal intStart As Integer, ByVal intEnd As Integer, pt脉搏() As POINTAPI, pt心率() As POINTAPI)
    ...
    For intCOl = intStart To intEnd
        If pt心率(intCOl).X > 0 Then
            Call DrawLine(objDraw, pt脉搏(intCOl).X, pt脉搏(intCOl).Y, pt心率(intCOl).X, pt心率(intCOl).Y, 255)
        End If
    Next intCOl
...
'    H_9pt = objDraw.TextHeight("字")
    ...
    '------------------------------------------------------------------------------------------------------------------
    '画横向坐标图线
    intGuageRow = intGuageRow + 1
'    For intRow = 0 To intGuageRow - 1
'        objDraw.DrawStyle = 2
'        Y0 = lngTmpY + intRow * (H_9pt + H_9pt \ 2)
'
'        If Not (intRow = 83 Or intRow = 96) Then
'            If (intRow) Mod 10 = 1 Then
...
'                    Call DrawLine(objDraw, lngTmpX + 10, Y0 + ROWHEIGHT * 5, lngTmpX + HOUR_STEP_Twips * 6 * 7, Y0 + ROWHEIGHT * 5, COLOR.深灰色, 0)
'                End If
'            End If
'        Else
'            Call DrawLine(objDraw, lngTmpX + 10, Y0 + ROWHEIGHT * 5, lngTmpX + HOUR_STEP_Twips * 6 * 7, Y0 + ROWHEIGHT * 5, RGB(200, 0, 0))
'        End If
'    Next
'修改为画10行
    For intRow = 0 To intGuageRow - 1
    ...
    For intCOl = 0 To 6
        ...
        Call DrawLine(objDraw, X0 + HOUR_STEP_Twips * 6, lngTmpY, X0 + HOUR_STEP_Twips * 6, Y, RGB(200, 0, 0), , 2)
    Next
    ...
    For i = 0 To intCountItem '纵
        ...
        For j = 0 To intGuageRow '横
            ...
            If j = 0 Then
                ...
                '--根据最高行决定刻度输入
                lngY = Y + objDraw.TextHeight("1") / 2
                For k = 1 To Len(strItem)
                    Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth(CStr(Mid(strItem, k, 1)))) / 2, lngY, Mid(strItem, k, 1), lngColor)
                    lngY = lngY + objDraw.TextHeight("1")
                Next k
                lngY = lngY + objDraw.TextHeight("1")
                strItem = GetSplitStr(aryItem(i), 8)
                'Call DrawLine(objDraw, X, lngY + H_9pt \ 2 - 20, X + lngPercW, lngY + H_9pt \ 2 - 20, 0, 0, 2)
                Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth(CStr(strItem))) / 2, lngY, strItem, lngColor)
                'lngY = lngY + objDraw.TextHeight("1")
                'Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth(CStr(lngTmpVal))) / 2, lngY, lngTmpVal, lngColor)
                Y = Y + H_9pt + H_9pt \ 2
                ...
                If (lngTmpVal - Fix(lngTmpVal)) = 0 Then
                    ...
                    Case "脉搏", "心率"
                        If lngTmpVal Mod 10 = 0 Then
                            If CLng(lngTmpVal) <> CLng(GetSplitStr(aryItem(i), 3)) Then
                                Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth(CStr(lngTmpVal))) / 2, Y - objDraw.TextHeight(CStr(lngTmpVal)) / 2, CStr(lngTmpVal), lngColor)
                            Else
                                If intItemTop = 0 Then
                                    Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth(CStr(lngTmpVal))) / 2, Y, CStr(lngTmpVal), lngColor)
                                Else
                                    Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth(CStr(lngTmpVal))) / 2, Y - objDraw.TextHeight(CStr(lngTmpVal)) / 2, CStr(lngTmpVal), lngColor)
                                End If
                            End If
                    ...
                    Case Else
                        If CLng(lngTmpVal) <> CLng(GetSplitStr(aryItem(i), 3)) Then
                            Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth(CStr(CStr(lngTmpVal)))) / 2, Y - objDraw.TextHeight(CStr(lngTmpVal)) / 2, CStr(lngTmpVal), lngColor)
                        Else
                            '体温不输出最大值
'                            If intItemTop = 0 Then
'                                Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth(CStr(CStr(lngTmpVal)))) / 2, Y, CStr(lngTmpVal), lngColor)
'                            Else
'                                Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth(CStr(CStr(lngTmpVal)))) / 2, Y - objDraw.TextHeight(CStr(lngTmpVal)) / 2, CStr(lngTmpVal), lngColor)
'                            End If
                        End If
    ...
    For i = 0 To intCountItem + 1
        Call DrawCell(objDraw, 1, X, Y, 0, (intGuageRow + 1) * (H_9pt + H_9pt \ 2))

        If i = 0 Or i = intCountItem + 1 Then
            Call DrawLine(objDraw, X, Y, X, Y + (intGuageRow + 1) * (H_9pt + H_9pt \ 2), , , 2)
        End If
...
'            objOut.SetPages
    ...
    '------------------------------------------------------------------------------------------------------------------
    Dim str入院时间 As String, bln入科显示入院 As Boolean
    strSQL = _
    ...
    If CDate(strBeginDate) > CDate(str入院时间) Then bln入科显示入院 = True
    If CDate(strBeginDate) = CDate(str入院时间) And mBodyFlag.入院 = 0 Then bln入科显示入院 = True
    ...
    If rsTemp.RecordCount > 0 Then
        rsTemp.MoveFirst
        lngMAXROWS = Val(rsTemp.Fields(0).Value) + 3
        If lngMAXROWS < MAXROWS Then lngMAXROWS = MAXROWS
        intDrawLineRows = lngMAXROWS
    Else
        CloseRs rsTemp
        GoTo ErrPrint
    End If
...
'                    objOut.txtPage.Text = "当前页" & Space(17) & "共 " & objOut.picPage.UBound + 1 & " 页"
        ...
        strDate = DateAdd("D", -Abs(DateDiff("d", CDate(strBeginDate), CDate(str入院时间))), CDate(strTmpDay))
...
'                strTmpString1 = strTmpString1 & "'" & Format(Int(CDate(strTmpDay)) + i - Int(CDate(strBeginDate)) + 1)
        ...
        lngNewTmpY = Y + (intDrawLineRows + 1) * (H_9pt + H_9pt \ 2) + 25
                    ...
                    Do While Not rsTemp.EOF
                        ...
                        If CStr(Split(strArrRecordItemInfo(i), "'")(0)) = "呼吸" Then
                            ...
                            If intCOl >= LBound(varNewTmpString) And intCOl <= UBound(varNewTmpString) Then
                                If Trim(zlCommFun.NVL(rsTemp!部位)) = "呼吸机" Then
                                    varNewTmpString(intCOl) = "A"
                                Else
...
Private Sub OutputNote(ByVal objDraw As Object, ByVal dblHeight As Double, ByRef rsNote As ADODB.Recordset, ByVal lngTmpX As Long, ByVal lngTmpY As Long)
    ...
    With rsNote
        ...
        sinY1 = (lngTmpY + 3 * H_9pt / 2) + 4 * H_9pt / 2       '42度
        .Filter = "打印列='" & intMax & "'"
        .Sort = "列号,时间"
            ...
            If !类型 = 2 Then       '上标说明,未记说明
                ...
                If rsTarget.RecordCount <> 0 Then
                    ...
                    Do While Not rsTarget.EOF
                        ...
                        rsTarget.MoveNext
                    Loop
                    'sinY1 = (lngTmpY + 3 * H_9pt / 2) + 18 * 3 * H_9pt / 2 - H_9pt / 2      '40度的坐标
                    sinY1 = lngTmpY + 29 * 3 * H_9pt / 2      '40度的坐标
                    If sinY1 < sinMaxY1 Or bln上标 Then sinY1 = sinMaxY1
                    sinHeight = !高度
                    intCOl = !列号
                Else
                    'sinY1 = (lngTmpY + 3 * H_9pt / 2) + 4 * H_9pt / 2       '42度
                    sinY1 = lngTmpY + 3 * H_9pt / 2
                    intCOl = !列号
                    sinHeight = !高度
                End If
                rsTarget.Filter = 0
...
'                    If Asc(strChar) < 0 Then
'                        intCharNumber = 0
                        sinY1 = sinY1 + objDraw.TextHeight("1") ' ROWHEIGHT * 10
'                    Else
