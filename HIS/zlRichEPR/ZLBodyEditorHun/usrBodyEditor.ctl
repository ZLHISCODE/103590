   ...
   Begin VB.PictureBox picPane 
      ...
      Begin VB.PictureBox pic 
         ...
         Begin VB.PictureBox picLine 
            ...
            Width           =   15
         End
         Begin zl9BodyEditorHUN.VsfGrid vsf 
            Height          =   270
            Left            =   450
            TabIndex        =   5
            Top             =   4440
...
Private Const ROWHEIGHT = 15                     '最小单元格的高度*5,原39
Private Const MAXROWS = 90                         '原47
...
Public Function ConvertToValue(ByVal intNo As Integer, ByVal Y As Long) As Double
    ...
    ConvertToValue = aryValue(0) - (Y / mshScale.ROWHEIGHT(1) - aryValue(3)) * aryValue(2)
...
Public Function ConvertToY(ByVal intCOl As Integer, ByVal dbValue As Double) As Long
    ...
    ConvertToY = ((Val(aryValue(0)) - dbValue) / Val(aryValue(2)) + Val(aryValue(3))) * mshScale.ROWHEIGHT(1)
...
'            Call UserControl_Resize
    ...
    '------------------------------------------------------------------------------------------------------------------
    Case "设定开始日期"
        If Not frm开始日期.ShowME(Val(mrsParam("病人ID").Value), Val(mrsParam("主页id").Value)) Then Exit Function
        zlMenuClick = True
...
Private Function InitBody(ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal lng病区id As Long, ByVal int婴儿 As Integer) As Boolean
    ...
    Dim strBeginDate As String '病人实际入院天数
    Dim intCOl As Integer
...
'    Erase mvarDataType
    ...
    '------------------------------------------------------------------------------------------------------------------
    '求出最高行
    mstrSQL = " Select Max((A.最大值-A.最小值)/Decode(A.单位值,0,1,A.单位值)+A.最高行) 最大值 From 体温记录项目 A Where A.记录法=1 "
    Set rs = zlDatabase.OpenSQLRecord(mstrSQL, mstrMsgTitle)
    If rs.BOF = False Then
        If IsNull(rs.Fields(0).Value) = False Then
            mshDownTab.Tag = "1"
            mvarEdit = True
            lngMAXROWS = Val(rs.Fields(0).Value) + 10
            If lngMAXROWS < MAXROWS Then lngMAXROWS = MAXROWS
            mshScale.Rows = lngMAXROWS 'MAXROWS
            ...
            mstrSQL = " Select A.记录法,A.记录名 as 项目名,A.项目序号 as 项目号,Nvl(B.ID,0) as 项目ID," & _
                        " C.项目单位 As 单位,记录符,最小值,最大值,记录色,1 as 记录否,单位值,Decode(最高行,0,2,1,2,最高行) 最高行,Nvl(B.类型,1) as 存储类型 " & _
...
'    lblTime.Caption = "日期:" & Format(strFrom, "MM-DD") & "～" & Format(strTo, "MM-DD")
    ...
    With mshUpTab
        ...
        For intCOl = 1 To .Cols - 1
            .Cell(flexcpForeColor, 0, intCOl) = 0
            .ColData(intCOl) = 0
            ...
            If Right(strTmp, 5) = "01-01" Then
                '一年的第一天
                .TextMatrix(0, intCOl) = Format(strTmp, "yyyy-MM-D")
            ElseIf strTmp = Format(mstrEnterDate, "yyyy-MM-DD") Then
                '入院第一天，写上年份
                .TextMatrix(0, intCOl) = Format(strTmp, "YYYY-MM-D")
            ElseIf intCOl = 1 Then
                .TextMatrix(0, intCOl) = Format(strTmp, "MM-D")  'Right(strTmp, 5)
            ElseIf Right(strTmp, 2) = "01" Then
                .TextMatrix(0, intCOl) = Format(strTmp, "MM-D") ' Right(strTmp, 5)
            Else
                .TextMatrix(0, intCOl) = Val(Right(strTmp, 2))
            End If
        ...
        Do While Not .EOF
            ...
            If (intCOl - mshScale.FixedCols) < 42 Then
                ...
                If blnAllow Then
                    ...
                    lng单位个数 = IIf(lng单位个数 + lng最高行 > (lngMAXROWS - 1), (lngMAXROWS - 1) - lng最高行, lng单位个数 + lng最高行)
                    '坐标值=((最大值-当前值)/单位值+最高行-1)*行高度
...
Private Function ReadGridData(ByVal byt护理等级 As Byte, _
    ...
    Dim intCount As Integer, intColNum As Integer
    Dim int频次 As Integer
                ...
                For i = 0 To rsTmp.RecordCount - 1
                    ...
                    Case mItemNo.呼吸
                        ...
                        If intCOl < vsf.Cols Then
                            ...
                            vsf.Cell(flexcpForeColor, 1, intCOl) = 0
                        End If
...
Public Function DrawPaper() As Boolean
    ...
    For intCOl = 1 To mshUpTab.Cols - 1
        ...
        DrawLine picGraph, X0 - HOUR_STEP_Twips * 0, Y0, X0 - HOUR_STEP_Twips * 0, Y1, &H8080FF, , 2
    Next
    
    '画横向坐标图线
'    For intRow = 2 To mshScale.Rows - 1
'
'        X0 = 0
'        Y0 = (intRow - 1) * ROWHEIGHT * 5
'        X1 = X0 + 15000
'
'        If Not (intRow = 86 Or intRow = 99) Then
'            If (intRow - 2) Mod 10 = 0 Then
'                If Int((intRow - 2) / 10) = 5 Then
...
'            End If
'        Else
'            DrawLine picGraph, X0, Y0 + ROWHEIGHT * 5, X1, Y0 + ROWHEIGHT * 5, &H8080FF, 0, 1
'        End If
'    Next
    '原有是两行显示一行 每行都显示
    For intRow = 1 To mshScale.Rows - 1
...
'                End If
        ...
        Dim lngYMax As Long
        lngYMax = ConvertToY(mItemSerial.体温, 34)
        ...
        For intCOl = 0 To .Cols - .FixedCols - 1
            ...
            Y1 = ConvertToY(mItemSerial.体温, 35) ' + 90 'ConvertToY(mItemSerial.呼吸, 15)
            'Y1 = 7020
            strComment = ""
...
'                            End If
            ...
            Y1 = ConvertToY(mItemSerial.体温, 35) '+ 90
            strComment = IIf(Trim(strComment) = "", "", " ") & .TextMatrix(GraphDataRow.下标说明, intCOl + .FixedCols)
            If Trim(strComment) <> "" Then
                intCharNumber = 0
                For intCount = 1 To Len(strComment)
...
'            If Trim(NVL(!结果)) Like "下标*" Then Stop
        ...
        sinY1 = 75
        'sinY1 = ConvertToY(mItemSerial.体温, 42)
        .Filter = "打印列='" & intMax & "'"
        .Sort = "列号,时间"
            ...
            If (!类型 = 未记说明 Or !类型 = 上标说明) Then
                ...
                If rsTarget.RecordCount <> 0 Then
                    ...
                    Do While Not rsTarget.EOF
                        ...
                        rsTarget.MoveNext
                    Loop
                    sinY1 = ConvertToY(mItemSerial.体温, 40) + ROWHEIGHT * 5
            ...
            For intCOl = .FixedCols To .Cols - 1 Step 2
                ...
                For intCount = 0 To .Rows - 2
                    ...
                    For intLoop = intCOl To intCOl + (int频次 - 1)
                        ...
                        If int频次 = 2 Then
                            If intLoop = intCOl + 1 Then  '表示一天记录一次
...
Private Sub picGraph_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If Y < (aryValue(3)) * mshScale.ROWHEIGHT(1) Then
        Y = (aryValue(3)) * mshScale.ROWHEIGHT(1)
        
        If picGraph.MousePointer <> 12 Then picGraph.MousePointer = 12
        
    End If
    If Y > (aryValue(3) + (aryValue(0) - aryValue(1)) / aryValue(2)) * mshScale.ROWHEIGHT(1) Then
        Y = (aryValue(3) + (aryValue(0) - aryValue(1)) / aryValue(2)) * mshScale.ROWHEIGHT(1)
        If picGraph.MousePointer <> 12 Then picGraph.MousePointer = 12
    End If
...
Private Sub DrawPicLable()
    ...
    Dim arrData
    Dim Y0 As Integer
    Dim DW As Single, DH As Single, Sw As Single
    On Error Resume Next
    ...
    For intCOl = 0 To picLable.UBound
        ...
        intMax = arrData(0)
        intStep = IIf(intCOl = mItemSerial.体温, 1, 10)
       ' Call DrawLine(picLable(intCol), 0, ConvertToY(intCol, intMax) + sinHeight, picLable(0).Width, ConvertToY(intCol, intMax) + sinHeight, , 0, 2)
        'Call DrawLine(picLable(intCol), 0, ConvertToY(intCol, intMax), picLable(0).Width, ConvertToY(intCol, intMax), , 0, 2)
        
'        For intCur = arrData(1) To intMax Step intStep
'            If intCur = intMax Then
'                If intCol = mItemSerial.体温 Then
'                    DrawText picLable(intCol), 80, ConvertToY(intCol, intCur) - sinHeight, "体温"
'                    DrawText picLable(intCol), 80, ConvertToY(intCol, intCur) + sinHeight, "(℃)"
'                Else
'                    DrawText picLable(intCol), 20, ConvertToY(intCol, intCur) - sinHeight, "脉搏"
'                    DrawText picLable(intCol), 20, ConvertToY(intCol, intCur) + sinHeight, "(次/分)"
'                End If
'                DrawText picLable(intCol), 80, ConvertToY(intCol, intCur) + sinHeight * 3, intCur
'            Else
'                DrawText picLable(intCol), 80, ConvertToY(intCol, intCur), intCur
'            End If
'        Next
        Sw = picLable(intCOl).Width
        DW = picLable(intCOl).TextWidth("字")
        DH = picLable(intCOl).TextHeight("字")
        For intCur = arrData(1) To intMax Step intStep
            If intCur = intMax Then
                If intCOl = mItemSerial.体温 Then
                    DrawText picLable(intCOl), (Sw - DW) / 2, sinHeight, "体"
                    DrawText picLable(intCOl), (Sw - DW) / 2, sinHeight * 2 + DH, "温"
                    DrawText picLable(intCOl), (Sw - DW) / 2, sinHeight * 2 + DH * 3, "℃"
                    Exit For '体温不输出最大值
                Else
                    DrawText picLable(intCOl), (Sw - DW) / 2, sinHeight, "脉"
                    DrawText picLable(intCOl), (Sw - DW) / 2, sinHeight * 2 + DH, "搏"
                    DW = picLable(intCOl).TextWidth("次/分")
                    DrawText picLable(intCOl), (Sw - DW) / 2, sinHeight * 2 + DH * 3, "次/分"
                    DW = picLable(intCOl).TextWidth("字")
                End If
    
                Y0 = ConvertToY(intCOl, intCur)
                DrawText picLable(intCOl), (Sw - DW) / 2, IIf(Y0 = 0, Y0, Y0 - sinHeight), intCur
            Else
                DrawText picLable(intCOl), (Sw - DW) / 2, ConvertToY(intCOl, intCur) - sinHeight, intCur
            End If
        Next
    Next
End Sub
