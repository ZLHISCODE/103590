...
Public Function zlRefresh(ByVal frmMain As Object, ByVal lngKey As Long, ByVal blnDataMove As Boolean, Optional ByVal bytKind As Byte = 0) As Boolean
    ...
    Set rs = zlDatabase.OpenSQLRecord(gstrSQL, "提取信息", lngKey)
...
Public Function zlPrintDocument(ByVal eDocType As EPRDocTypeEnum, Optional ByVal bytMode As Byte = 2, Optional ByVal lngKey As Long, Optional ByVal strPrintDeviceName As String, Optional ByVal blnDataMove As Boolean) As Boolean
    '1-预览,2-打印
    ...
    Else
        ...
        Set rs = zlDatabase.OpenSQLRecord(gstrSQL, "提取信息", lngEPRKey)
        If Not rs.EOF Then
            ...
            ElseIf NVL(rs!编辑方式, 0) = 1 Then
                If mObjTabEprView Is Nothing Then
                    Set mObjTabEprView = New zlTableEPR.cTableEPR
                    Call mObjTabEprView.InitTableEPR(gcnOracle, glngSys, gstrDbOwner)
                    Call mObjTabEprView.InitOpenEPR(Me, cprEM_修改, cprET_单病历审核, 0, False, 0, 2)
                End If
            ...
            ElseIf NVL(rs!编辑方式, 0) = 2 Then
                ...
                End If
                If Not mobjInfection Is Nothing Then
                    Call mobjInfection.PrintDoc(Me, rs!病人ID, rs!主页ID, lngEPRKey, strPrinterName)
                End If
            End If
        End If
    End If
...
Dim rs As New ADODB.Recordset, lngKey As Long, rsTmp As New ADODB.Recordset
Dim strTemp As String, strZipFile As String
    
    gstrSQL = "Select 病人ID,主页ID,病历种类,编辑方式 From 电子病历记录 Where ID=[1]"
    If mblnDataMove Then gstrSQL = Replace(gstrSQL, "电子病历记录", "H电子病历记录")
    Set rs = zlDatabase.OpenSQLRecord(gstrSQL, "提取信息", lngEPRid)
    
    dkpMain.FindPane(conPane_RichEpr).Close
    dkpMain.FindPane(conPane_TablEpr).Close
    dkpMain.FindPane(conPane_Infection).Close
    If NVL(rs!编辑方式, 0) = 1 Then
        dkpMain.ShowPane conPane_TablEpr
        Call mObjTabEprView.InitOpenEPR(Me, cprEM_修改, cprET_单病历审核, lngEPRid, False, 0, , , , , , , , mblnDataMove)
        Call mObjTabEprView.zlRefreshDockfrm
    ElseIf NVL(rs!编辑方式, 0) = 2 Then
        dkpMain.ShowPane conPane_Infection
        mobjInfection.zlRefresh CLng(rs!病人ID), CLng(rs!主页ID), lngEPRid, mblnDataMove
    ElseIf NVL(rs!编辑方式, 0) = 0 Then
        dkpMain.ShowPane conPane_RichEpr
        If SetRichDocsPos(lngEPRid) Then Exit Function '看是否已经加载(共享病历定位)
        edtThis.Freeze
        ...
        Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, "提取信息", lngEPRid)
        If Not rsTmp.EOF Then '有格式文件 病历编辑器保存的数据
            ...
            End If
        Else '无格式文件,可能是pacs报告编辑器保存,可能是外部接口接入数据
            Dim Doc As New cEPRDocument, strPath As String, strFile As String
            
            strPath = IIf(Environ$("tmp") <> vbNullString, Environ$("tmp"), Environ$("temp")) '保存临时文件
            strFile = strPath & "\" & App.hInstance & CLng(Timer) & ".TMP"
            Doc.InitEPRDoc cprEM_修改, cprET_单病历审核, lngEPRid, rs!病历种类, CLng(rs!病人ID), CLng(rs!主页ID), , , , mblnDataMove
            Doc.OpenEPRDoc Doc.frmEditor.Editor1, mblnDataMove       '打开该文件
            Doc.frmEditor.SaveDocToFile strFile, False     '存储非清洁临时文件
            
            edtThis.OpenDoc strFile
            Set edtThis.Picture = Doc.frmEditor.Editor1.Picture '设置页眉页脚
            edtThis.Head = Doc.frmEditor.Editor1.Head
            edtThis.Foot = Doc.frmEditor.Editor1.Foot
            Doc.EPRFileInfo.SetFormat edtThis, Doc.EPRFileInfo.格式 '设置页面格式
            edtThis.ResetWYSIWYG    '刷新所见即所得（WYSIWYG）显示
            edtThis.SelectAll
            edtThis.AuditMode = True
            edtThis.AcceptAuditText
            If gobjFSO.FileExists(strFile) Then gobjFSO.DeleteFile strFile, True  '删除临时文件
            Set Doc = Nothing
        End If
...
Dim strIDs As String, varPar() As String, StrKey As String
    ...
    Else
        ...
        gfrmPublic.edtBuff.RefreshTargetDC
        gfrmPublic.edtBuff.ReadOnly = True
        gfrmPublic.edtBuff.ForceEdit = False
        gfrmPublic.edtBuff.InProcessing = False
        gfrmPublic.edtBuff.Tag = ""
        Unload gfrmPublic
    End If
    Exit Sub
errHand:
...
Private Function ShowAdition(ByVal lngRecordId As Long) As Boolean
    ...
    End If
    Set rs = zlDatabase.OpenSQLRecord(gstrSQL, "提取信息", lngRecordId)
    With rs
        Do While Not .EOF
...
Dim Pane1 As Pane, pane2 As Pane, pane3 As Pane, Pane4 As Pane
    ...
    Set pane3 = dkpMain.CreatePane(conPane_Infection, 1200, 200, DockTopOf, Nothing)
    pane3.Options = PaneNoCaption Or PaneNoCloseable Or PaneNoFloatable Or PaneNoHideable
    pane3.Close
    
    Set Pane4 = dkpMain.CreatePane(conPane_Annex, 1200, 15, DockBottomOf, Nothing)
    Pane4.MinTrackSize.Height = 0: Pane4.MaxTrackSize.Height = 360 / Screen.TwipsPerPixelY
...
Dim lKSS As Long, lKSE As Long, lKES As Long, lKEE As Long, lKey As Long, bBeteenKeys As Boolean, sKeyType As String, bNeeded As Boolean, blnForce As Boolean
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "提取信息", lngRecordId)
    Do Until rsTemp.EOF
...
'自定义报表打印PACS报告
        ...
        Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "提取信息", lngKey)
        ...
        Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "提取信息", strReportCode)
