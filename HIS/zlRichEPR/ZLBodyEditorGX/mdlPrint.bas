...
'                objDraw.FontSize = 9
        ...
        For j = lngHourBegin To 12 - (4 - lngHourBegin) Step 4
            ...
            If j = 12 - lngHourBegin Then
'                objDraw.FontSize = 8
                Call DrawCell(objDraw, CStr(j + 12), X, Y, HOUR_STEP_Twips, (H_9pt * 2 + H_9pt \ 2), , , , lngColor, , objDraw.Font, , 1, 1, True)
                X = X + HOUR_STEP_Twips
            Else
'                objDraw.FontSize = 9
                Call DrawCell(objDraw, CStr(j + 12), X, Y, HOUR_STEP_Twips, (H_9pt * 2 + H_9pt \ 2), , , , lngColor, , objDraw.Font, , 1, 1, True)
...
'                    lngArrXY(lngCol, 0) = 2
                    ...
                    If lng项目序号 = 1 Then         '体温
                        Call PointAdd(rsPoint, lngX, lngY, lng项目序号, strTag, lngColor, lngCol, CStr(aryTmp(5)), "", lng项目序号)
                    ElseIf lng项目序号 = 3 Then     '呼吸
                        Call PointAdd(rsPoint, lngX, lngY, lng项目序号, strTag, lngColor, lngCol, CStr(aryTmp(5)), IIf(strTag = "", "BREATH", ""), lng项目序号)
                    ElseIf lng项目序号 = 2 Then     '脉搏
                        Call PointAdd(rsPoint, lngX, lngY, lng项目序号, strTag, lngColor, lngCol, CStr(aryTmp(5)), IIf(strTag = "", "PACEMAKER", ""), lng项目序号)
                    Else
                        Call PointAdd(rsPoint, lngX, lngY, lng项目序号, strTag, lngColor, lngCol, "", "", lng项目序号)
                    End If
...
'                            End If
                        ...
                        If lng项目序号 = 1 Then
                            Call PointAdd(rsPoint, lngX, Y2, lng项目序号, strTag, lngColor, lngCol, CStr(aryTmp(5)), "", lng项目序号)
                        Else
                            Call PointAdd(rsPoint, lngX, Y2 - H_9pt \ 3, lng项目序号, strTag, lngColor, lngCol, "", "", lng项目序号)
                        End If
        ...
        '--------------------------------------------------------------------------------------------------------------
        'If blnPrint Then Call DrawPoly(objDraw, mpt脉搏, mpt心率)
        If blnPrint Then Call DrawPoly(objDraw, mpt脉搏, mpt心率, mbyt脉搏)
...
Public Function PointAdd(ByRef rsPoint As ADODB.Recordset, ByVal X1 As Single, ByVal Y1 As Single, ByVal lngLine As Long, ByVal strChar As String, ByVal lngColor As Long, ByVal lngColumn As Long, ByVal str体温部位 As String, Optional ByVal str图形 As String, Optional ByVal lng序号 As Long = 0) As Boolean
    '******************************************************************************************************************
    '功能：
    '参数：
    '返回：
...
Public Function DrawPoly(objDraw As Object, pt脉搏() As POINTAPI, pt心率() As POINTAPI, byt脉搏() As Byte) As Boolean
    ...
    For intCount = 0 To 41
        ...
        Else
            ...
            intDay = intCount - (IIf(intEnd = 0, intStart, intEnd) + ((intCount + 1) Mod 6))
...
Private Function GetConverPoint(ByVal rsPoint As ADODB.Recordset, ByRef rsConverPoint As ADODB.Recordset, ByVal int体温 As Integer) As Boolean
    ...
    rsPoint.MoveFirst
    Do While Not rsPoint.EOF
        If rsPoint("X").Value = X0 And rsPoint("Y").Value = Y0 And int序号 <> rsPoint("序号").Value Then
...
Private Function DrawFillPoly(objDraw As Object, ByVal intStart As Integer, ByVal intEnd As Integer, pt脉搏() As POINTAPI, pt心率() As POINTAPI)
    ...
    '心率对应的脉搏直接连线
    For intCOl = intStart To intEnd
        If pt脉搏(intCOl).X > 0 And pt心率(intCOl).X > 0 And intCOl <> intStart And intCOl <> intEnd Then
            If pt脉搏(intCOl).X = pt心率(intCOl).X Then
...
Public Function PrintOrPreviewBodyState(objOut As Object, _
    ...
    Dim blnStop As Boolean '确定是否有拒测、外出的未记说明
    ...
    Dim lngX1 As Long, lngY1 As Long  '保存拒测和外出未记说明的坐标
                ...
                If rsTemp.RecordCount > 0 Then
                    For j = 0 To UBound(strArrNewTmpComment)
                        ...
                        If Format(Split(strArrNewTmpComment(j), "'")(0), "yyyy-MM-dd HH:mm:ss") = strTmp Then
                            ...
                            Case 1
                                If Split(strArrNewTmpComment(j), "'")(1) <> "外出" And Split(strArrNewTmpComment(j), "'")(1) <> "拒测" Then
                                    If byt未记显示位置 = 0 Then
                                        If aryTmp(0) = "" Then
                            ...
                            If blnStop = True Then
                                lngX1 = X + i * HOUR_STEP_Twips + HOUR_STEP_Twips / 2
                                lngY1 = (Y + 3 * H_9pt / 2) + 25 * 3 * H_9pt / 2            '得到37度的坐标
                                Call DrawRotateText(objDraw, lngX1 - objDraw.TextWidth("△") / 2, lngY1 - objDraw.TextHeight("△"), "△", -2147483635)
                            End If
                        End If
                    Next
                End If
                               ...
                               Select Case rsTemp!项目序号
                                Case 7
                                    ...
                                    If Right(zlCommFun.NVL(rsTemp!说明), 2) = "/C" Then
                                        ...
                                        varNewTmpString(intColTmp) = sgl饮入量(intColTmp) & "/C"
