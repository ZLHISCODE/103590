...
errHand:
    ...
    End If
End Sub
Private Sub ResizeDownTab(ByVal dt开始时间 As Date)
'--------------------------------------------------
            ...
            For intCount = 0 To .Cols - .FixedCols - 1
                ...
                If Trim(aryValue(intCOl + 1)) <> "" And Val(aryValue(intCOl + 1)) > 0 Then
                    ...
                    For i = 0 To UBound(aryDots)
                        ...
                        If intCOl = mItemSerial.体温 Then
                            Call PointAdd(rsPoint, X1, Y1, mshScale.ColData(intCOl), strChar, lngColor, intCount, mshScale.TextMatrix(4, intCount + .FixedCols), "", intCOl)
                        ElseIf intCOl = mItemSerial.呼吸 Then
                            Call PointAdd(rsPoint, X1, Y1, mshScale.ColData(intCOl), strChar, lngColor, intCount, mshScale.TextMatrix(4, intCount + .FixedCols), IIf(strChar = "", "BREATH", ""), intCOl)
                        ElseIf intCOl = mItemSerial.脉搏 Then
                            Call PointAdd(rsPoint, X1, Y1, mshScale.ColData(intCOl), strChar, lngColor, intCount, mshScale.TextMatrix(4, intCount + .FixedCols), IIf(strChar = "", "PACEMAKER", ""), intCOl)
                        Else
                            Call PointAdd(rsPoint, X1, Y1, mshScale.ColData(intCOl), strChar, lngColor, intCount, "", "", intCOl)
                        End If
...
'            Y1 = ConvertToY(mItemSerial.体温, 42)
            ...
            If byt未记显示位置 = 0 Then
                ...
                varNote = Split(.TextMatrix(GraphDataRow.未记说明, intCOl + .FixedCols), ";")
                For intCount = 0 To UBound(varNote)
                    If varNote(intCount) <> "不升" And varNote(intCount) <> "拒测" And varNote(intCount) <> "外出" Then
                        If InStr(";" & strTmp & ";", ";" & varNote(intCount) & ";") = 0 Then
                            strTmp = strTmp & ";" & varNote(intCount)
                        End If
                    End If
...
'            Y1 = ConvertToY(mItemSerial.体温, 35)
            ...
            '患者拒绝测体温或擅自离院时在体温单37℃线对应时间上用蓝色“△”表示
            Y1 = ConvertToY(mItemSerial.体温, 37)
            strComment = ""
            strTmp = ""
            varNote = Split(.TextMatrix(GraphDataRow.未记说明, intCOl + .FixedCols), ";")
            For intCount = 0 To UBound(varNote)
                If varNote(intCount) <> "不升" Then
                    If varNote(intCount) = "拒测" Or varNote(intCount) = "外出" Then
                        strTmp = "△"
                    End If
                End If
            Next
            ...
            If strTmp <> "" Then
                Call DrawRotateText(picGraph, X1 - picGraph.TextWidth(strTmp) / 2, Y1 - picGraph.TextHeight(strTmp), strTmp, -2147483635)
            End If
        Next
...
Private Function isSaved() As String
    ...
    With mshDownTab
        For intCOl = .FixedCols To .Cols - 1
            aryValue = Split(.TextMatrix(0, intCOl), ";")
            For intCount = 0 To UBound(aryValue)
                strItem = .TextMatrix(intCount + 1, 0)
                If aryValue(intCount) = "2" Or aryValue(intCount) = "3" Or aryValue(intCount) = "4" Then
                    isSaved = "改变了“" & strItem & "”数据尚未保存"
                    Exit Function
                End If
...
Private Function ShowInput() As Boolean
    Dim strTmp As String
    Dim intMerCol As Integer, int频次 As Integer
    
    With mshDownTab
        int频次 = Val(mItemStru(.Row).记录频次)
        If int频次 > 2 Or int频次 < 1 Then int频次 = 2
        If int频次 = 2 Then
            intMerCol = 1
        Else
        ...
        Else
            ...
            lblInput.Tag = .TextMatrix(.Row, .Col)
...
Private Sub txtInput_Validate(Index As Integer, Cancel As Boolean)
    ...
    With mshDownTab
        ...
        If mItemStru(intRow).数据类型 = 0 Then
            If Val(mshDownTab.RowData(mshDownTab.Row)) = mItemNo.血压 Then
                strInput = Trim(txtInput(0).Text) & "/" & Trim(txtInput(1).Text)
