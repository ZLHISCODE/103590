...
Public Function OraDBOpen(ByVal strServer As String, ByVal strUserName As String, ByVal strPassword As String, ByVal bytProvider As enuProvider, ByRef strError As String) As ADODB.Connection
...
'返回： 数据库打开成功，返回true；失败，返回false
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.mdlDataBase.OraDBOpen", strUserName, Sm4EncryptEcb(strPassword, G_UA_KEY), bytProvider, strError)
    ...
    Else
        ...
        End If
    End If
    Call Logger.PopMethod("ZLHelperMain.mdlDataBase.OraDBOpen", OraDBOpen, strError)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.mdlDataBase.OraDBOpen") = 1 Then
        Resume
    End If
    
    Call Logger.PopMethod("ZLHelperMain.mdlDataBase.OraDBOpen")
End Function
...
Public Function GetServer(ByVal strServer As String) As String
    ...
    If InStr(strServer, "/") > 0 Then
        ...
        Else
            ...
            strPort = "1521"
        End If
        GetServer = "(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=" & strIp & ")(PORT=" & strPort & "))(CONNECT_DATA=(SERVICE_NAME=" & strSID & ")))"
    Else
        GetServer = strServer
    End If
End Function
...
Public Function OpenSQLRecord(ByVal cnOracle As ADODB.Connection, ByVal strSQL As String, ByVal strTitle As String, ParamArray arrInput() As Variant) As ADODB.Recordset
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.mdlDataBase.OpenSQLRecord", cnOracle, strSQL, strTitle, arrInput())
    arrPars = arrInput
    Set OpenSQLRecord = OpenSQLRecordByArray(cnOracle, strSQL, strTitle, arrPars)
    Call Logger.PopMethod("ZLHelperMain.mdlDataBase.OpenSQLRecord", OpenSQLRecord)
    Exit Function
ErrH:
    Call Logger.PopMethod("ZLHelperMain.mdlDataBase.OpenSQLRecord", "[HasError]", Err.Number, Err.Description)
...
'  3.因为该过程是自动处理,不是一定使用绑定变量,对带"'"的字符参数,仍要使用"''"形式。
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.mdlDataBase.ExecuteProcedure", strSQL, strFormCaption)
    If Right(Trim(strSQL), 1) = ")" Then
        ...
        If blnStr Or intBra <> 0 Then
            Call Logger.PopMethod("ZLHelperMain.mdlDataBase.OpenSQLRecordByArray")
...
'        Call gobjComLib.SQLTest
    Else
        GoTo NoneVarLine
    End If
    Call Logger.PopMethod("ZLHelperMain.mdlDataBase.ExecuteProcedure")
    Exit Sub
NoneVarLine:
    On Error GoTo ErrH
'    Call gobjComLib.SQLTest(App.ProductName, strFormCaption, strSQL)
...
Public Function Currentdate(ByVal cnOracle As ADODB.Connection) As Date
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.mdlDataBase.Currentdate", cnOracle)
    Err = 0
    On Error GoTo ErrH
    With rsTemp
        .CursorLocation = adUseClient
    ...
    rsTemp.Close
    Call Logger.PopMethod("ZLHelperMain.mdlDataBase.Currentdate")
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.mdlDataBase.Currentdate") = 1 Then
        Resume
    End If
    Currentdate = 0
    Err = 0
...
'调用方式为：Set rsPati=OpenSQLRecord(strSQL, Me.Caption, CDate(Format(rsMove!转出日期,"yyyy-MM-dd")),dtp时间.Value, lng病人ID, "张%", "男", 20, 21)
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.mdlDataBase.OpenSQLRecordByArray", cnOracle, strSQL, strTitle, arrInput())
...
'    Call gobjComLib.SQLTest
    Call Logger.PopMethod("ZLHelperMain.mdlDataBase.OpenSQLRecordByArray", OpenSQLRecordByArray)
    Exit Function
ErrH:
    Call Logger.PopMethod("ZLHelperMain.mdlDataBase.OpenSQLRecordByArray", OpenSQLRecordByArray)
    Err.Raise Err.Number, Err.Source, Err.Description, Err.HelpFile, Err.HelpContext
