...
Public Function RunAsCurrentUser(ByVal strApplicationName As String, ByVal strCommandLine As String, Optional ByVal strCurrentDirectory As String) As Long
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.mdlRunas.RunAsCurrentUser")
...
'        Logger.Info  "OpenThreadToken失败", "错误", GetLastDllErr(Err.LastDllError)
'        If OpenProcessToken(GetCurrentProcess(), TOKEN_ALL_ACCESS, hProcessToken) = 0 Then
'            Logger.Info  "OpenProcessToken失败", "错误", GetLastDllErr(Err.LastDllError)
'        End If
'    End If
    ...
    If WTSQueryUserToken(glngWinSessionID, hProcessToken) <> 0 Then
        If DuplicateTokenEx(hProcessToken, TOKEN_ALL_ACCESS, ByVal 0, ByVal SecurityImpersonation, ByVal TokenPrimary, hNewToken) <> 0 Then
            If SetTokenInformation(hNewToken, TokenSessionId, glngWinSessionID, LenB(glngWinSessionID)) <> 0 Then
                Logger.Info "是否管理员用户", IsAdministrator(), "hToken=", hNewToken
'
'                    If SetTokenInformation(hSeToken, TokenSessionId, glngWinSessionID, LenB(glngWinSessionID)) <> 0 Then
                        If ImpersonateLoggedOnUser(hNewToken) <> 0 Then
                            ...
                            If lngRet <> 0 Then
                                ...
                                Logger.Info "CreateProcessAsUser成功"
                            Else
                                Logger.Info "CreateProcessAsUser失败", "错误", GetLastDllErr(Err.LastDllError)
                            End If
                        Else
                            Logger.Info "ImpersonateLoggedOnUser失败", "错误", GetLastDllErr(Err.LastDllError)
                        End If
                        If RevertToSelf() = 0 Then
                            Logger.Info "RevertToSelf失败", "错误", GetLastDllErr(Err.LastDllError)
                        End If
'                    Else
'                        Logger.Info  "SetTokenInformation2失败", "错误", GetLastDllErr(Err.LastDllError)
'                    End If
'                Else
'                    Logger.Info  "WTSQueryUserToken失败", "错误", GetLastDllErr(Err.LastDllError)
'                End If
            Else
                Logger.Info "SetTokenInformation1失败", "错误", GetLastDllErr(Err.LastDllError)
            End If
            CloseHandle hNewToken
        Else
            Logger.Info "DuplicateTokenEx失败", "错误", GetLastDllErr(Err.LastDllError)
        End If
        CloseHandle hToken
'    End If
    End If
    Call Logger.PopMethod("ZLHelperMain.mdlRunas.RunAsCurrentUser", RunAsCurrentUser)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.mdlRunas.RunAsCurrentUser") = 1 Then
        Resume
    End If
End Function
...
Public Function RunAsUser(ByVal strApplicationName As String, ByVal strCommandLine As String, Optional ByVal strCurrentDirectory As String, Optional ByVal strUserName As String, Optional ByVal strPassword As String, Optional ByVal strDomainName As String) As Boolean
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.clsRunas.RunAsUser", strUserName, Sm4EncryptEcb(strPassword), strDomainName, strApplicationName, Sm4EncryptEcb(strCommandLine), strCurrentDirectory)
...
'        End If
    ...
    End If
    Call Logger.PopMethod("ZLHelperMain.clsRunas.RunAsUser", RunAsUser)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.clsRunas.RunAsUser") = 1 Then
        Resume
    End If
End Function
...
Public Function GetUserSID(ByVal strAccountName As String) As String
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.mdlRunas.GetUserSID")
    ...
    If lngError = ERROR_INSUFFICIENT_BUFFER Then
        Logger.DebugEx "缓冲区不足，分配缓冲区", "返回", lngRet
        ...
        lngError = Err.LastDllError
        If lngRet = 0 Then
            Logger.DebugEx "LookupAccountName2失败", "错误", GetLastDllErr(Err.LastDllError), "返回", lngRet
        End If
        If ConvertSidToStringSid(bytSid(0), lpStrSid) Then
            ...
            Else
                strRet = ""
                Logger.Warn "lstrcpy失败", "错误", GetLastDllErr(Err.LastDllError)
            End If
            Call LocalFree(lpStrSid)
        Else
            Logger.Warn "ConvertSidToStringSid失败", "错误", GetLastDllErr(Err.LastDllError)
        End If
    ElseIf lngError = 0 Then
        Logger.Warn "用户不存在", "返回", lngRet
    Else
        Logger.Warn "LookupAccountName失败", "错误", GetLastDllErr(Err.LastDllError), "返回", lngRet
    End If
    GetUserSID = strRet
    Call Logger.PopMethod("ZLHelperMain.mdlRunas.GetUserSID", GetUserSID)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.mdlRunas.GetUserSID") = 1 Then
        Resume
    End If
End Function
'--------------------------------------------------------------------------------------------------
...
Public Function GetCurrentSessionID(Optional ByVal lngCurProcID As Long) As Long
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.clsRunas.GetCurrentSessionID", lngCurProcID)
    If lngCurProcID = 0 Then
        lngCurProcID = GetCurrentProcessId()
    End If
    If ProcessIdToSessionId(lngCurProcID, lngSessionID) = 0 Then
        Logger.Warn "ProcessIdToSessionId失败", "错误", GetLastDllErr(Err.LastDllError)
    End If
    GetCurrentSessionID = lngSessionID
    Call Logger.PopMethod("ZLHelperMain.clsRunas.GetCurrentSessionID", GetCurrentSessionID)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.clsRunas.GetCurrentSessionID") = 1 Then
        Resume
    End If
End Function
'--------------------------------------------------------------------------------------------------
...
Public Function IsProcessRunAsAdmin() As Boolean
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.mdlRunas.IsProcessRunAsAdmin")
    ...
    If lngRet <> 0 Then
        If CheckTokenMembership(0, psidAdmin, lngRet) = 0 Then
            Logger.Warn "CheckTokenMembership失败", "错误", GetLastDllErr(Err.LastDllError)
        End If
        Call FreeSid(psidAdmin)
    Else
        Logger.Warn "AllocateAndInitializeSid失败", "错误", GetLastDllErr(Err.LastDllError)
    End If
    IsProcessRunAsAdmin = lngRet <> 0
    Call Logger.PopMethod("ZLHelperMain.mdlRunas.IsProcessRunAsAdmin", IsProcessRunAsAdmin)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.mdlRunas.IsProcessRunAsAdmin") = 1 Then
        Resume
    End If
End Function
'--------------------------------------------------------------------------------------------------
...
'    Dim tpTokens            As TOKEN_GROUPS
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.clsRunas.IsAdministrator")
    ...
    If WTSQueryUserToken(GetCurrentSessionID(), hProcessToken) <> 0 Then
        If hProcessToken Then
            If GetTokenInformation(hProcessToken, ByVal TokenGroups, 0, 0, BufferSize) = 0 Then ' Determine required buffer size
                Logger.DebugEx "GetTokenInformation失败", "错误", GetLastDllErr(Err.LastDllError), "需要缓冲区大小", BufferSize
            End If
            If BufferSize Then
                ...
                If lResult = 0 Then
                    Logger.Warn "GetTokenInformation失败", "错误", GetLastDllErr(Err.LastDllError)
                Else
        ...
        End If
    Else
        Logger.Warn "WTSQueryUserTokend失败", "错误", GetLastDllErr(Err.LastDllError)
    End If
    Call Logger.PopMethod("ZLHelperMain.clsRunas.IsAdministrator", IsAdministrator)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.clsRunas.IsAdministrator") = 1 Then
        Resume
    End If
End Function
...
'    Dim tpTokens            As TOKEN_GROUPS
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.clsRunas.IsProcesssAdministrator")
    IsProcesssAdministrator = False
    tpSidAuth.value(5) = security_nt_authority
    
    If OpenThreadToken(GetCurrentThread(), TOKEN_QUERY, True, hProcessToken) = 0 Then
        Logger.DebugEx "OpenThreadToken失败", "错误", GetLastDllErr(Err.LastDllError)
        If lngProcessID = 0 Then
            If OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, hProcessToken) = 0 Then
                Logger.Warn "OpenProcessToken失败", "错误", GetLastDllErr(Err.LastDllError)
            End If
        End If
    End If
    If hProcessToken Then
        ...
        Call CloseHandle(hProcessToken)
    End If
    Call Logger.PopMethod("ZLHelperMain.clsRunas.IsProcesssAdministrator", IsProcesssAdministrator)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.clsRunas.IsProcesssAdministrator") = 1 Then
        Resume
    End If
End Function
...
Public Function GetProcessUserName(Optional lngType As Long = NameSamCompatible) As String
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.clsRunas.GetProcessUserName", lngType)
    lngLen = UNLEN + 1
    strTemp = String(UNLEN + 1, Chr$(0))
    If GetUserName(strTemp, lngLen) = 0 Then
        Logger.Warn "GetUserName失败", "错误", GetLastDllErr(Err.LastDllError), "长度", lngLen
    End If
...
'    GetUserNameEx NameSamCompatible, strTemp, lngLen
    ...
    Call Logger.PopMethod("ZLHelperMain.clsRunas.GetProcessUserName", GetProcessUserName)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.clsRunas.GetProcessUserName") = 1 Then
        Resume
    End If
End Function
...
Public Function CheckUserPassword(ByVal strUserName As String, ByVal strPassword As String, ByVal strDomainName As String) As Boolean
    Dim hToken      As Long
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.clsRunas.CheckUserPassword", strUserName, Sm4EncryptEcb(strPassword), strDomainName)
    ...
    If CheckUserPassword Then
        CloseHandle hToken
    Else
        Logger.Warn "LogonUser失败", "错误", GetLastDllErr(Err.LastDllError)
    End If
    Call Logger.PopMethod("ZLHelperMain.clsRunas.CheckUserPassword", CheckUserPassword)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.clsRunas.CheckUserPassword") = 1 Then
        Resume
    End If
End Function
...
Public Function GetPrivilegeList(Optional ByVal hProc As Long) As String
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.mdlRunas.GetPrivilegeList")
    If hProc = 0 Then
    ...
    Call Logger.PopMethod("ZLHelperMain.mdlRunas.GetPrivilegeList", GetPrivilegeList)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.mdlRunas.GetPrivilegeList") = 1 Then
        Resume
    End If
End Function
...
Public Function EnablePrivilegeTest() As Boolean
    ...
    If Not EnablePrivilege(, SE_INCREASE_QUOTA_NAME) Then
        Logger.DebugEx SE_INCREASE_QUOTA_NAME & "失败"
    Else
        Logger.DebugEx SE_INCREASE_QUOTA_NAME & "成功"
    End If
    '1: SeSecurityPrivilege -0
    If Not EnablePrivilege(, SE_SECURITY_NAME) Then
        Logger.DebugEx SE_SECURITY_NAME & "失败"
    Else
        Logger.DebugEx SE_SECURITY_NAME & "成功"
    End If
    '2: SeTakeOwnershipPrivilege -0
    If Not EnablePrivilege(, SE_TAKE_OWNERSHIP_NAME) Then
        Logger.DebugEx SE_TAKE_OWNERSHIP_NAME & "失败"
    Else
        Logger.DebugEx SE_TAKE_OWNERSHIP_NAME & "成功"
    End If
    '3: SeLoadDriverPrivilege -0
    If Not EnablePrivilege(, SE_LOAD_DRIVER_NAME) Then
        Logger.DebugEx SE_LOAD_DRIVER_NAME & "失败"
    Else
        Logger.DebugEx SE_LOAD_DRIVER_NAME & "成功"
    End If
    '4: SeSystemProfilePrivilege -0
    If Not EnablePrivilege(, SE_SYSTEM_PROFILE_NAME) Then
        Logger.DebugEx SE_SYSTEM_PROFILE_NAME & "失败"
    Else
        Logger.DebugEx SE_SYSTEM_PROFILE_NAME & "成功"
    End If
    '5: SeSystemtimePrivilege -0
    If Not EnablePrivilege(, SE_SYSTEMTIME_NAME) Then
        Logger.DebugEx SE_SYSTEMTIME_NAME & "失败"
    Else
        Logger.DebugEx SE_SYSTEMTIME_NAME & "成功"
    End If
    '6: SeProfileSingleProcessPrivilege -0
    If Not EnablePrivilege(, SE_PROF_SINGLE_PROCESS_NAME) Then
        Logger.DebugEx SE_PROF_SINGLE_PROCESS_NAME & "失败"
    Else
        Logger.DebugEx SE_PROF_SINGLE_PROCESS_NAME & "成功"
    End If
    '7: SeIncreaseBasePriorityPrivilege -0
    If Not EnablePrivilege(, SE_INC_BASE_PRIORITY_NAME) Then
        Logger.DebugEx SE_INC_BASE_PRIORITY_NAME & "失败"
    Else
        Logger.DebugEx SE_INC_BASE_PRIORITY_NAME & "成功"
    End If
    '8: SeCreatePagefilePrivilege -0
    If Not EnablePrivilege(, SE_CREATE_PAGEFILE_NAME) Then
        Logger.DebugEx SE_CREATE_PAGEFILE_NAME & "失败"
    Else
        Logger.DebugEx SE_CREATE_PAGEFILE_NAME & "成功"
    End If
    '9: SeBackupPrivilege -0
    If Not EnablePrivilege(, SE_BACKUP_NAME) Then
        Logger.DebugEx SE_BACKUP_NAME & "失败"
    Else
        Logger.DebugEx SE_BACKUP_NAME & "成功"
    End If
    
    '10: SeRestorePrivilege -0
    If Not EnablePrivilege(, SE_RESTORE_NAME) Then
        Logger.DebugEx SE_RESTORE_NAME & "失败"
    Else
        Logger.DebugEx SE_RESTORE_NAME & "成功"
    End If
    'AAA -11: SeShutdownPrivilege -0
    '12: SeDebugPrivilege -0
    If Not EnablePrivilege(, SE_DEBUG_NAME) Then
        Logger.DebugEx SE_DEBUG_NAME & "失败"
    Else
        Logger.DebugEx SE_DEBUG_NAME & "成功"
    End If
    '13: SeSystemEnvironmentPrivilege -0
    If Not EnablePrivilege(, SE_SYSTEM_ENVIRONMENT_NAME) Then
        Logger.DebugEx SE_SYSTEM_ENVIRONMENT_NAME & "失败"
    Else
        Logger.DebugEx SE_SYSTEM_ENVIRONMENT_NAME & "成功"
    End If
    'AAA -14: SeChangeNotifyPrivilege -3
    '15: SeRemoteShutdownPrivilege -0
    If Not EnablePrivilege(, SE_REMOTE_SHUTDOWN_NAME) Then
        Logger.DebugEx SE_REMOTE_SHUTDOWN_NAME & "失败"
    Else
        Logger.DebugEx SE_REMOTE_SHUTDOWN_NAME & "成功"
    End If
    'AAA -16: SeUndockPrivilege -0
    '17: SeManageVolumePrivilege -0
    If Not EnablePrivilege(, SE_MANAGE_VOLUME_NAME) Then
        Logger.DebugEx SE_MANAGE_VOLUME_NAME & "失败"
    Else
        Logger.DebugEx SE_MANAGE_VOLUME_NAME & "成功"
    End If
    '18: SeImpersonatePrivilege -3
    If Not EnablePrivilege(, SE_IMPERSONATE_NAME) Then
        Logger.DebugEx SE_IMPERSONATE_NAME & "失败"
    Else
        Logger.DebugEx SE_IMPERSONATE_NAME & "成功"
    End If
    '19: SeCreateGlobalPrivilege -3
    If Not EnablePrivilege(, SE_CREATE_GLOBAL_NAME) Then
        Logger.DebugEx SE_CREATE_GLOBAL_NAME & "失败"
    Else
        Logger.DebugEx SE_CREATE_GLOBAL_NAME & "成功"
    End If
    ...
    If Not EnablePrivilege(, SE_CREATE_SYMBOLIC_LINK_NAME) Then
        Logger.DebugEx SE_CREATE_SYMBOLIC_LINK_NAME & "失败"
    Else
        Logger.DebugEx SE_CREATE_SYMBOLIC_LINK_NAME & "成功"
    End If
End Function
...
Public Function EnablePrivilege(Optional ByVal hProc As Long, Optional ByVal strPrivilegeName As String) As Boolean
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.mdlRunas.EnablePrivilege", hProc, strPrivilegeName)
    If hProc = 0 Then
        hProc = GetCurrentProcess()
    End If
    ...
    Else
        If hToken <> 0 Then
            ...
            If lngRet = 0 Then
                Logger.Warn "LookupPrivilegeValue失败", "错误", GetLastDllErr(Err.LastDllError)
            Else
                ...
                If lngRet = 0 Then
                    Logger.Warn "AdjustTokenPrivileges失败", "错误", GetLastDllErr(Err.LastDllError)
                End If
        ...
        End If
    End If
    Call Logger.PopMethod("ZLHelperMain.mdlRunas.EnablePrivilege", EnablePrivilege)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.mdlRunas.EnablePrivilege") = 1 Then
        Resume
    End If
End Function
...
Public Function EnablePrivilegeToken(ByVal hToken As Long, Optional ByVal strPrivilegeName As String) As Boolean
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.mdlRunas.EnablePrivilegeToken", hToken, strPrivilegeName)
    tkp.PrivilegeCount = 1
    tkp.Privileges(0).PLUID = tmpLuid
    tkp.Privileges(0).Attributes = SE_PRIVILEGE_ENABLED
    lngRet = AdjustTokenPrivileges(hToken, 0, tkp, Len(tkp), tkpNewButIgnored, lBufferNeeded)
    ...
    Call Logger.PopMethod("ZLHelperMain.mdlRunas.EnablePrivilegeToken", EnablePrivilegeToken)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.mdlRunas.EnablePrivilegeToken") = 1 Then
        Resume
    End If
End Function
'--------------------------------------------------------------------------------------------------
...
Private Function RunAsAdmin(ByVal strApplicationName As String, Optional ByVal strCommandLine As String, Optional ByVal strDirectory As String) As Long
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.clsRunas.RunAsAdmin", strApplicationName, Sm4EncryptEcb(strCommandLine), strDirectory)
...
'pid1=GetProcessId(a.hProcess);
    ...
    If lngRet <= 32 Then
        Logger.Warn "ShellExecute失败", "错误", GetLastDllErr(Err.LastDllError), "返回", lngRet
    End If
    RunAsAdmin = lngRet > 32
    Call Logger.PopMethod("ZLHelperMain.clsRunas.RunAsAdmin", RunAsAdmin)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.clsRunas.RunAsAdmin") = 1 Then
        Resume
    End If
End Function
'--------------------------------------------------------------------------------------------------
...
Private Function IsWindows2000OrGreater() As Boolean
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.clsRunas.IsWindows2000OrGreater")
    osInfo.dwOSVersionInfoSize = Len(osInfo)
    osInfo.szCSDVersion = Space$(128)
    If GetVersionExA(osInfo) = 0 Then
        Logger.Info "GetVersionExA失败", "错误", GetLastDllErr(Err.LastDllError)
    End If
    IsWindows2000OrGreater = osInfo.dwMajorVersion >= 5
    Call Logger.PopMethod("ZLHelperMain.clsRunas.IsWindows2000OrGreater", IsWindows2000OrGreater)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.clsRunas.IsWindows2000OrGreater") = 1 Then
        Resume
    End If
End Function
...
Private Function IsWindowsVistaOrGreater() As Boolean
    Dim osInfo          As OSVERSIONINFOEX
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.clsRunas.IsWindowsVistaOrGreater")
    osInfo.dwOSVersionInfoSize = Len(osInfo)
    osInfo.szCSDVersion = Space$(128)
    If GetVersionExA(osInfo) = 0 Then
        Logger.Info "GetVersionExA失败", "错误", GetLastDllErr(Err.LastDllError)
    ...
    Call Logger.PopMethod("ZLHelperMain.clsRunas.IsWindowsVistaOrGreater", IsWindowsVistaOrGreater)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.clsRunas.IsWindowsVistaOrGreater") = 1 Then
        Resume
    End If
End Function
'--------------------------------------------------------------------------------------------------
...
Private Function RunAsUserW2K(ByVal strUserName As String, ByVal strPassword As String, ByVal strDomainName As String, ByVal strApplicationName As String, Optional ByVal strCommandLine As String, Optional ByVal strCurrentDirectory As String) As Long
    ...
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.clsRunas.RunAsUserW2K", strUserName, Sm4EncryptEcb(strPassword), strDomainName, strApplicationName, Sm4EncryptEcb(strCommandLine), strCurrentDirectory)
    ...
    If lngRet = 0 Then
        Logger.Info "CreateProcessWithLogonW失败", "错误", GetLastDllErr(Err.LastDllError)
    Else
        RunAsUserW2K = piInfo.dwProcessId
        CloseHandle piInfo.hThread
        CloseHandle piInfo.hProcess
    End If
    Call Logger.PopMethod("ZLHelperMain.clsRunas.RunAsUserW2K", RunAsUserW2K)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.clsRunas.RunAsUserW2K") = 1 Then
        Resume
    End If
End Function
...
Private Function RunAsUserNT4(ByVal strUserName As String, ByVal strPassword As String, ByVal strDomainName As String, ByVal strApplicationName As String, Optional ByVal strCommandLine As String, Optional ByVal strCurrentDirectory As String) As Long
    ...
    Dim lngRet      As Long
    On Error GoTo ErrH
    Call Logger.PushMethod("ZLHelperMain.clsRunas.RunAsUserNT4", strUserName, strPassword, strDomainName, strApplicationName, strCommandLine, strCurrentDirectory)
    If strCurrentDirectory = "" Then strCurrentDirectory = CurDir$()
    ...
    If lngRet <> 0 Then
        ...
        CloseHandle piInfo.hProcess
    Else
        Logger.Info "LogonUser失败", "错误", GetLastDllErr(Err.LastDllError)
    End If
    Call Logger.PopMethod("ZLHelperMain.clsRunas.RunAsUserNT4", RunAsUserNT4)
    Exit Function
ErrH:
    If Logger.ErrCenter("ZLHelperMain.clsRunas.RunAsUserNT4") = 1 Then
        Resume
    End If
End Function
