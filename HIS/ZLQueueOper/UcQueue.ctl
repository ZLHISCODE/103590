...
'获取排队队列的数据显示状态，排队，暂停，弃呼，完成
    ...
    If objReportControl.SelectedRows.Count > 0 Then
        If objReportControl.SelectedRows(0).GroupRow = False Then
            strState = objReportControl.SelectedRows(0).Record(GetColIndex("排队状态", objReportControl)).value
...
'    If DoCallPreBefore(lngQueueId, TCallWay.cwBroadcast, strCallContext) = False Then Exit Sub
    ...
    Call SetQueueRowState(rptQueueList, lngRowIndex, qss待预呼)
...
'恢复排队显示状态
    ...
    If lngStateColIndex >= 0 Then
        objList.Rows(lngRow).Record(lngStateColIndex).value = Decode(lngState, _
                                                                    ...
                                                                    TQueueState.qsBeforeWaitCall, "待预呼", _
                                                                    TQueueState.qsAfterWaitCall, "已预呼", _
                                                                    "")
    End If
    ...
    If lngStateColIndex >= 0 Then
        Select Case lngState
            ...
            Case TQueueState.qsCalled, TQueueState.qsAfterWaitCall
...
On Error GoTo errH
    ...
    Dim strState As String '排队状态
    Dim intListIndex As Integer 'qftWaitQueue = 0 qftCalledQueue = 1
    ...
    '首先判断呼叫的数据是否在排队列表
    If GetRowIndex(qftWaitQueue, "ID", lngQueueId) >= 0 Then
        intListIndex = qftWaitQueue
    ElseIf GetRowIndex(qftCalledQueue, "ID", lngQueueId) >= 0 Then
        intListIndex = qftCalledQueue
    End If
    ...
    Else
        ...
        If lngRowIndex >= 0 Then
            strState = GetListValue(qftCalledQueue, lngRowIndex, "排队状态")
            If strState <> "接诊中" Then
    ...
    If lngQueueId > 0 And intListIndex <> 3 Then
        '设置呼叫行的当前颜色
        lngRowIndex = GetRowIndex(intListIndex, "ID", lngQueueId)
                
        If lngRowIndex >= 0 Then
            strState = GetListValue(intListIndex, lngRowIndex, "排队状态")
            If strState = "呼叫中" And intListIndex = qftCalledQueue Then
                Call SetQueueRowState(rptCallList, lngRowIndex, qsCalled)
                Call rptCallList.Populate
            ElseIf strState = "待预呼" And intListIndex = qftWaitQueue Then
                Call SetQueueRowState(rptQueueList, lngRowIndex, qsAfterWaitCall)
                Call rptQueueList.Populate
            End If
        End If
    End If
