...
Begin VB.Form frmPrintAsk 
   ...
   Begin VB.Frame fraPageScope 
      ...
      Begin VB.CheckBox chkDuplex 
         ...
         Left            =   1560
         TabIndex        =   19
         ToolTipText     =   "全部打印或全部打印－偶数页时左右页边距会自动反向，以方便装订。"
         Top             =   330
         Width           =   1425
      End
...
Public lngPageStart As Long, lngPageEnd As Long, lngCopies As Long
...
Private Sub cmdOK_Click()
    ...
    If txtCopies.Text > 1 And chkCopyOrder.Value = Unchecked Then '只有逐份打印或只打一份才支持双面打印
        chkDuplex.Value = Unchecked: chkDuplex.Enabled = False
    End If
    
    If Me.optPageScope(2).Value = True Then
        '打印份数检查
        If Val(txtCopies.Text) <= 0 Then
            MsgBox "打印份数不能小于1，请检查！", vbInformation, Me.Caption
            Exit Sub
        End If
        '页码范围
...
Private Sub optPageScope_Click(Index As Integer)
    If Me.optPageScope(0).Value Then
        chkDuplex.Enabled = True
    Else
        chkDuplex.Value = Unchecked
