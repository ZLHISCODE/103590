...
INSERT INTO 电子票据类别 (编号,名称,简码,是否启用,部件,包名称) values(1,'博思电子票据平台','BSDZBJPT',0,'zlEInvoice.clsEinvoice_BS','b_Einvoice_Request_Bs');

insert into 三方接口配置 ( 接口名,参数号,参数名,参数值,说明) values ('博思电子票据平台',1,upper('URL_Type'),'http',NULL);
insert into 三方接口配置 ( 接口名,参数号,参数名,参数值,说明) values ('博思电子票据平台',2,upper('URL_Address'),'','<ip>:<port>/<service>/api/medical/接口服务标识');
...
insert into 三方接口配置 ( 接口名,参数号,参数名,参数值,说明) values ('博思电子票据平台',6,'数据传输方式','application/json','提交和返回数据可以为JSON格式（Content-Type: application/json）');
...
insert into 三方接口配置 ( 接口名,参数号,参数名,参数值,说明) values ('博思电子票据平台',13,'录入冲红原因','0','是否由操作员录入票据冲红原因');
insert into 三方接口配置 ( 接口名,参数号,参数名,参数值,说明) values ('博思电子票据平台',14,'误差费对照编码','','博思收费项目明细允许录入负数时指定一个项目处理误差费');
insert into 三方接口配置 ( 接口名,参数号,参数名,参数值,说明) values ('博思电子票据平台',15,'误差费对照名称','','博思收费项目明细允许录入负数时指定一个项目处理误差费');
insert into 三方接口配置 ( 接口名,参数号,参数名,参数值,说明) values ('博思电子票据平台',16,'零费用开具电子票据','','是否允许费用金额为0时开具电子票据');
insert into 三方接口配置 ( 接口名,参数号,参数名,参数值,说明) values ('博思电子票据平台',17,'收费纸质票据代码','','收费换开纸质票据时传入的纸质票据代码');
insert into 三方接口配置 ( 接口名,参数号,参数名,参数值,说明) values ('博思电子票据平台',18,'挂号纸质票据代码','','挂号换开纸质票据时传入的纸质票据代码');
insert into 三方接口配置 ( 接口名,参数号,参数名,参数值,说明) values ('博思电子票据平台',19,'结账纸质票据代码','','结账换开纸质票据时传入的纸质票据代码');
insert into 三方接口配置 ( 接口名,参数号,参数名,参数值,说明) values ('博思电子票据平台',20,'预交纸质票据代码','','预交换开纸质票据时传入的纸质票据代码');

--insert into 三方接口配置 ( 接口名,参数号,参数名,参数值,说明) values ('博思电子票据平台',12,'误差项目','','因小数与HIS不匹配时，产生误差费的收费项目');
...
CREATE TABLE 收据费目对照(
  ...
  名称 varchar2(20),
  编码 varchar2(20),
  费用场合 Number(1) Default 0 --0-不区分;1-门诊;2-住院
)
TABLESPACE zl9Expense;
Alter Table 收据费目对照 Add Constraint 收据费目对照_PK Primary Key(Id) Using Index Tablespace zl9Indexhis;
Alter Table 收据费目对照 Add Constraint 收据费目对照_UQ_收据费目  Unique(收据费目,费用场合)  Using Index Tablespace zl9Indexhis;
Alter Table 收据费目对照 Add Constraint 收据费目对照_FK_收据费目 Foreign Key (收据费目) References 收据费目(名称) on delete cascade;
CREATE SEQUENCE 收据费目对照_ID START WITH 1
...
Alter Table 支付类别对照 Add Constraint 支付类别对照_FK_保险大类ID Foreign Key (保险大类ID) References 保险支付大类(ID) on delete cascade;
...
Create Or Replace Package b_Einvoice_Request_Bs Is
  ------------------------------------------------------------------
...
End b_Einvoice_Request_Bs;
/

CREATE OR REPLACE Package Body b_Einvoice_Request_Bs Is
  ------------------------------------------------------------------
  --电子票据业务处理(主要是针对博思电子票据的处理)
  --一.公共函数说明
  --  0.get_charset-获取当前数据库的字符集
  ...
  Begin
    ...
    --base64编码
    c_Data := Zltools.Zlbase64.Encode(Reqdata_In, n_Charset);
  c_Data := Replace(Replace(c_Data, Chr(10), ''), Chr(13), '');
  ...
  Begin
    ...
    c_Temp := To_Clob('appid=' || Appid_In || '&' || 'data=') || c_Data ||
              To_Clob('&' || 'noise=' || v_Guid || '&' || 'key=' || Key_In || '&' || 'version=' || Version_In);
    --MD5加密
    v_Sign_Check := Upper(b_Einvoice_Request_Bs.Md5_Clob(c_Temp));
    ...
    n_Charset := b_Einvoice_Request_Bs.Get_Charset;
  ...
  Begin
    If Mv_Url Is Null Then
      ...
      For c_配置 In (Select 参数号, 参数名, 参数值 From 三方接口配置 Where 接口名 = '博思电子票据平台') Loop
  ...
  Begin
    ...
    Utl_Http.Set_Header(o_Http_Req, 'Content-Length', Length(c_Data));
  ...
  (
    ...
    v_医疗卡类型编号   Varchar2(50);
    v_身份证作卡类型编号   Varchar2(50);
    v_病人无卡的卡类别编号   Varchar2(50);
    v_病人无卡的卡号   Varchar2(50);
    v_误差费对照编码   Varchar2(100);
    v_误差费对照名称   Varchar2(100);
    n_零费用开票       number(1);
    n_Count            Number(3);
    v_疾病编码         Varchar2(20);    
    
    v_JsonKey_就诊科室     Varchar2(20);
    v_JsonFormat_就诊日期  Varchar2(20);
  Begin
    ...
    Select Max(Decode(参数名,'支持版本', Nvl(参数值, 'V2.0.3'),'')),
           Max(Decode(参数名,'缺省卡类别ID', 参数值,'')),
           Max(Decode(参数名,'医疗卡类型编号', 参数值,'')),
           Max(Decode(参数名,'身份证作卡类型编号', 参数值,'')),
           Max(Decode(参数名,'病人无卡的卡类别编号', 参数值,'')),
           Max(Decode(参数名,'病人无卡的卡号', 参数值,'')),
           Max(Decode(参数名,'误差费对照编码', 参数值,'')),
           Max(Decode(参数名,'误差费对照名称', 参数值,'')),
           Max(Decode(参数名,'零费用开具电子票据', 参数值,''))
    Into v_版本号, n_缺省卡类别id, v_医疗卡类型编号,
         v_身份证作卡类型编号, v_病人无卡的卡类别编号, v_病人无卡的卡号,
         v_误差费对照编码, v_误差费对照名称, n_零费用开票
    From 三方接口配置
    Where 接口名 = '博思电子票据平台';
    ...
    --版本差异
    If v_版本号 = 'V3.2.0' Then
      v_JsonKey_就诊科室 := 'patientCategory';
      v_JsonFormat_就诊日期 := 'yyyyMMdd';
    Else
      v_JsonKey_就诊科室 := 'category';
      v_JsonFormat_就诊日期 := 'yyyy-MM-dd';
    End If;
    ...
    c_明细         := Null;
    v_明细         := Null;
    For c_收费细目 In (Select Min(a.费用ID) As 费用ID, a.No, a.序号, a.收费细目id, a.计算单位, a.价格, Sum(a.数量) As 数量,
                             Sum(a.应收金额) As 应收金额, Sum(a.实收金额) As 实收金额, Sum(a.结帐金额) As 结帐金额, Sum(a.自费金额) As 自费金额,
                             Max(a.医保项目名称) As 医保项目编码, Max(a.医保项目名称) As 医保项目名称, Max(a.医保报销比例) As 医保报销比例, Max(a.备注) As 备注,
                             ...
                             Max(a.付款方式编码) As 付款方式编码, Max(a.收据费目) As 收据费目, Max(a.收据费目编码) As 收据费目编码, Max(a.医嘱序号) As 医嘱序号,
                             Max(a.挂号id) As 挂号id, Max(a.类别编码) As 类别编码, Max(a.类别名称) As 类别名称, Max(a.项目编码) As 项目编码, Max(a.项目名称) As 项目名称,
                             Max(a.规格) As 规格, Max(a.药品剂型) As 药品剂型
                      From (Select Min(a.Id) As 费用id, a.No, a.记录状态, a.结帐id, Nvl(a.价格父号, a.序号) As 序号, a.收费细目id, Max(a.计算单位) As 计算单位,
                                  ...
                                  Max(s.大类编码) As 医保项目编码, Max(s.大类名称) As 医保项目名称, Max(t.统筹比额) As 医保报销比例, Max(a.摘要) As 备注,
                                  Max(a.费用类型) As 费用类型, Max(a.操作员编号) As 操作员编号, Max(a.操作员姓名) As 操作员姓名, Max(a.姓名) As 姓名,
                                  Max(a.性别) As 性别, Max(a.年龄) As 年龄, Max(a.病人id) As 病人id, Max(a.登记时间) As 登记时间,
                                  Max(a.付款方式) As 付款方式编码, Max(Nvl(c.名称, c1.名称)) As 收据费目, Max(Nvl(c.编码, c1.编码)) As 收据费目编码, Max(a.医嘱序号) As 医嘱序号,
                                  Max(a.挂号id) As 挂号id, Max(d.编码) As 类别编码, Max(d.类别) As 类别名称, Max(b.编码) As 项目编码, Max(b.名称) As 项目名称,
                                  Max(b.规格) As 规格, Max(q.药品剂型) As 药品剂型
                           From 门诊费用记录 A, 收费项目目录 B, 收据费目对照 C, 收据费目 C1, 收费类别 D, 药品规格 M, 药品特性 Q, 诊疗项目目录 J, 保险支付大类 T, 支付类别对照 S
                           Where a.No In (Select Distinct NO From 门诊费用记录 Where 结帐id = n_结算id) And Mod(a.记录性质, 10) = 1 And
                                 a.收费类别 = d.编码(+) And a.收费细目id = b.Id And a.收据费目 = c1.名称(+) And a.收据费目 = c.收据费目(+) and Decode(c.费用场合(+), 0, 1, c.费用场合(+)) = 1 And a.收费细目id = m.药品id(+) And
                           ...
                           Order By NO, 序号) a
                      Group By a.No, a.序号, a.收费细目id, a.计算单位, a.价格
                      Having Sum(a.结帐金额) <> 0 Or n_零费用开票 = 1) Loop
      ...
      v_Temp := v_Temp || ',' || '"name":"' || b_Einvoice_Request_Bs.Zljsonstr(c_收费细目.项目名称) || '"';
      --form  剂型  String  50  否
      ...
      Else
        v_明细 := v_明细 || ',' || v_Temp;
      End If;
      n_票据总金额 := Nvl(n_票据总金额, 0) + Nvl(c_收费细目.实收金额, 0);
    End Loop;
    ...
    Into l_结帐id
    From 门诊费用记录
    Where (no, 记录性质) In (Select No, 记录性质 From 门诊费用记录 Where 结帐ID = n_结算ID);

    Select Sum(a.冲预交)
    Into n_误差总额
    From 病人预交记录 a, 结算方式 b, Table(l_结帐id) c
    Where a.结帐id = c.Column_Value And a.结算方式 = b.名称 And b.性质 = 9;

    If v_误差费对照编码 Is Not Null Then
      n_票据总金额 := n_票据总金额 - Nvl(n_误差总额, 0);
    End If;
    Totalmoney_Out := Round(n_票据总金额, 2);
    -------------------------------------------------------------------------------------------
    --分类明细
    v_分类明细 := Null;
    c_分类明细 := Null;
    ...
    For c_分类统计 In (Select Rownum As 序号, 收据费目编码, 收据费目名称, 数量, 计算单位, Round(单价, 2) As 单价, Round(结帐金额, 2) As 结帐金额,
                   ...
                   From (Select /*+cardinality(b,10)*/
                           Nvl(c.编码, c1.编码) As 收据费目编码, Nvl(c.名称, c1.名称) As 收据费目名称, 1 As 数量, '' As 计算单位, Sum(a.结帐金额) As 单价, a.收据费目,
                           Sum(a.结帐金额) As 结帐金额, Sum(a.结帐金额) - Sum(a.统筹金额) As 自费金额, '' As 备注
                          From 门诊费用记录 A, Table(l_结帐id) B, 收据费目对照 C, 收据费目 C1
                          Where a.结帐id = b.Column_Value And a.收据费目 = c1.名称(+) And a.收据费目 = c.收据费目(+) And Decode(c.费用场合(+), 0, 1, c.费用场合(+)) = 1
                          Group By c.编码, c1.编码, c.名称, c1.名称, a.收据费目)) Loop
      ...
      --remark  备注  String  200  否
      v_Temp := v_Temp || ',' || '"remark":"' || b_Einvoice_Request_Bs.Zljsonstr(c_分类统计.备注) || '"}';
    ...
    IF Totalmoney_Out <> n_票据总金额 And v_误差费对照编码 Is Not Null THEN 
      n_误差总额 := Totalmoney_Out - n_票据总金额;
      --sortNo  序号  Integer  不限  是  默认从1开始，每个收费项目序号值递增1，本次不允许重复
      v_Temp := '{' || '"sortNo":' || (n_Count + 1);
      --chargeCode  收费项目代码  String  50  是  填写业务系统内部编码值，由医疗平台配置对照
      v_Temp := v_Temp || ',' || '"chargeCode":"' || b_Einvoice_Request_Bs.Zljsonstr(v_误差费对照编码) || '"';
      --chargeName  收费项目名称  String  100  是  填写业务系统内部项目名称
      v_Temp := v_Temp || ',' || '"chargeName":"' || b_Einvoice_Request_Bs.Zljsonstr(v_误差费对照名称) || '"';
      --std  收费标准  Number  14,2  是
      v_Temp := v_Temp || ',' || '"std":' || b_Einvoice_Request_Bs.Zljsonstr(n_误差总额, 1);
      --number  数量  Number  14,2  是
      v_Temp := v_Temp || ',' || '"number":' || 1;
      --amt  金额  Number  14,2  是
      v_Temp := v_Temp || ',' || '"amt":' || b_Einvoice_Request_Bs.Zljsonstr(n_误差总额, 1);
      --selfAmt  自费金额  Number  14,2  是  如无金额，填写0
      v_Temp := v_Temp || ',' || '"selfAmt":' || b_Einvoice_Request_Bs.Zljsonstr(n_误差总额, 1);
      --remark  备注  String  200  否
      v_Temp := v_Temp || ',' || '"remark":""}';
    ...
    v_票据信息 := v_票据信息 || ',"busDateTime":"' ||
              b_Einvoice_Request_Bs.Zljsonstr(To_Char(d_业务发生时间, 'yyyymmddHH24miss') || '000') || '"'; --业务发生时间
    ...
    v_票据信息 := v_票据信息 || ',"checker":"' || b_Einvoice_Request_Bs.Zljsonstr(v_操作员姓名) || '"'; --票据审核人
    v_票据信息 := v_票据信息 || ',"totalAmt":' || b_Einvoice_Request_Bs.Zljsonstr(Totalmoney_Out, 1); --开票总金额
    If Nvl(n_误差总额, 0) = 0 Or v_误差费对照编码 Is Not Null Then
      v_票据信息 := v_票据信息 || ',"remark":"' || '' || '"'; --备注
    Else
      v_票据信息 := v_票据信息 || ',"remark":"' || '存在' || To_char(n_误差总额,'FM999999990.999999') || '误差金额不参与结算' || '"'; --备注
    End If;
    -------------------------------------------------------------------------------------------
    ...
    For c_通知 In (Select Max(a.病人id) As 病人id, Max(a.姓名) As 姓名, Max(a.手机号) As 手机号, Max(a.Email) As Email, Max(1) As 缴款类型,
                        Max(a.身份证号) As 身份证号, Max(m.卡号) As 卡号, Max(a.门诊号) As 门诊号
                 From 病人信息 A,
                      (
                        Select 病人id, 卡号
                        From (Select b.病人id, b.卡号
                                From 病人医疗卡信息 B
                                Where b.卡类别id = n_缺省卡类别id And b.病人id = n_病人id)
                        Where Rownum < 2) M
                 Where a.病人id = m.病人id(+) And a.病人id = n_病人id) Loop
      ...
      If c_通知.卡号 Is Not Null Then
        v_通知 := v_通知 || ',"cardType":"' || b_Einvoice_Request_Bs.Zljsonstr(v_医疗卡类型编号) || '"'; --卡类型
        v_通知 := v_通知 || ',"cardNo":"' || b_Einvoice_Request_Bs.Zljsonstr(c_通知.卡号) || '"'; --卡号
      Elsif c_通知.身份证号 Is Not Null And v_身份证作卡类型编号 IS NOT null Then
        v_通知 := v_通知 || ',"cardType":"' || b_Einvoice_Request_Bs.Zljsonstr(v_身份证作卡类型编号) || '"'; --卡类型
        v_通知 := v_通知 || ',"cardNo":"' || b_Einvoice_Request_Bs.Zljsonstr(c_通知.身份证号) || '"'; --卡号
      Else
        --没有一张卡，固定一种卡类别
        v_通知 := v_通知 || ',"cardType":"' || b_Einvoice_Request_Bs.Zljsonstr(v_病人无卡的卡类别编号) || '"'; --卡类型
        v_通知 := v_通知 || ',"cardNo":"' || b_Einvoice_Request_Bs.Zljsonstr(v_病人无卡的卡号) || '"'; --卡号
      End If;
    ...
    v_就诊编号 := Null;
    If Nvl(n_医嘱序号, 0) <> 0 Then
      Select Max(To_Char(a.发生时间, v_JsonFormat_就诊日期)), Max(b.编码), Max(b.名称), Max(a.No), Max(d.编码) As 疾病编码
      Into v_就诊日期, v_就诊科室编码, v_就诊科室名称, v_就诊编号, v_疾病编码
      From 病人挂号记录 A, 部门表 B, 病人诊断记录 C, 疾病编码目录 D
      Where a.执行部门id = b.Id And a.病人ID = c.病人ID(+) And a.ID = c.主页ID(+) And 
            c.诊断次序(+) = 1 and Mod(c.诊断类型(+), 10) = 1 And c.疾病ID = d.id(+) And
            a.No = (Select Max(挂号单) From 病人医嘱记录 Where ID = n_医嘱序号 Or 相关id = n_医嘱序号);
    Elsif Nvl(n_挂号id, 0) <> 0 Then
      Select Max(To_Char(a.发生时间, v_JsonFormat_就诊日期)), Max(b.编码), Max(b.名称), Max(a.No), Max(d.编码) As 疾病编码
      Into v_就诊日期, v_就诊科室编码, v_就诊科室名称, v_就诊编号, v_疾病编码
      From 病人挂号记录 A, 部门表 B, 病人诊断记录 C, 疾病编码目录 D
      Where a.执行部门id = b.Id And a.Id = n_挂号id And
            a.病人ID = c.病人ID(+) And a.ID = c.主页ID(+) And c.诊断次序(+) = 1 and 
            Mod(c.诊断类型(+), 10) = 1 And c.疾病ID = d.id(+);
    End If;
    If v_就诊编号 Is Null Then
      --取最近一次挂号ID
      Select Max(a.Id), Max(To_Char(a.发生时间, v_JsonFormat_就诊日期)), Max(b.编码), Max(b.名称), Max(a.No), Max(d.编码) As 疾病编码
      Into n_挂号id, v_就诊日期, v_就诊科室编码, v_就诊科室名称, v_就诊编号, v_疾病编码
      From 病人挂号记录 A, 部门表 B, 病人诊断记录 C, 疾病编码目录 D
      Where a.执行部门id = b.Id And
            a.病人ID = c.病人ID(+) And a.ID = c.主页ID(+) And c.诊断次序(+) = 1 and Mod(c.诊断类型(+), 10) = 1 And c.疾病ID = d.id(+) And
            a.Id = (Select ID
    ...
    v_就诊信息 := v_就诊信息 || ',"medicalCareType":"' || b_Einvoice_Request_Bs.Zljsonstr(v_医疗付款方式名称) || '"';
    ...
    --          格式:yyyy-MM-dd
    v_就诊信息 := v_就诊信息 || ',"consultationDate":"' || b_Einvoice_Request_Bs.Zljsonstr(v_就诊日期) || '"';
    --category  就诊科室  String  200  否
    v_就诊信息 := v_就诊信息 || ',"' || v_JsonKey_就诊科室 || '":"' || b_Einvoice_Request_Bs.Zljsonstr(v_就诊科室名称) || '"';
    --patientCategoryCode  就诊科室编码  String  60  否
    v_就诊信息 := v_就诊信息 || ',"patientCategoryCode":"' || b_Einvoice_Request_Bs.Zljsonstr(v_就诊科室编码) || '"';
    --patientNo  患者就诊编号  String  20  是  患者每次就诊一次就生成的一个新的编号。（患者登记号）
    v_就诊信息 := v_就诊信息 || ',"patientNo":"' || b_Einvoice_Request_Bs.Zljsonstr(v_就诊编号) || '"';
    --patientId  患者唯一ID  String  50  否  患者在业务系统中的唯一标识ID，类似身份证号码。
    v_就诊信息 := v_就诊信息 || ',"patientId":"' || b_Einvoice_Request_Bs.Zljsonstr(n_病人id) || '"';
    --sex  性别  String  2  是
    v_就诊信息 := v_就诊信息 || ',"sex":"' || b_Einvoice_Request_Bs.Zljsonstr(v_患者性别) || '"';
    --age  年龄  String  10  是
    v_就诊信息 := v_就诊信息 || ',"age":"' || b_Einvoice_Request_Bs.Zljsonstr(v_患者年龄) || '"';
    --caseNumber  病历号  String  50  否
    v_就诊信息 := v_就诊信息 || ',"caseNumber":"' || b_Einvoice_Request_Bs.Zljsonstr(n_门诊号) || '"';
    --ICD  疾病编码  String  20  否
    v_就诊信息 := v_就诊信息 || ',"ICD":"' || b_Einvoice_Request_Bs.Zljsonstr(v_疾病编码) || '"';
    --specialDiseasesName  特殊病种名称  String  200  否
    v_就诊信息 := v_就诊信息 || ',"specialDiseasesName":"' || b_Einvoice_Request_Bs.Zljsonstr(v_病种名称) || '"';
    -------------------------------------------------------------------------------------------
    --结算信息
    --误差费累计到个人现金支付
    v_费用 := Null;
    For c_结算 In (Select 现金预交, 支票预交, 转账预交, 个人帐户支付, 医保统筹基金支付, 其它医保支付, 个人现金支付, Decode(Sign(现金支付), -1, 现金支付, 0) As 现金退款,
                        Decode(Sign(支票支付), -1, 支票支付, 0) As 支票退款, Decode(Sign(转帐支付), -1, 转帐支付, 0) As 转帐退款,
                        Decode(Sign(现金支付), -1, 0, 现金支付) As 现金支付, Decode(Sign(支票支付), -1, 0, 支票支付) As 支票支付,
                        Decode(Sign(转帐支付), -1, 0, 转帐支付) As 转帐支付,
                 ...
                 From (Select /*+cardinality(b,10)*/
                         ...
                         Sum(Decode(Mod(a.记录性质, 10), 1, 0, Decode(c.开票结算方式, Null, Decode(a.结算方式, '现金', 1, 0), 0)) * a.冲预交) As 现金支付,
                         Sum(Decode(Mod(a.记录性质, 10), 1, 0, Decode(c.开票结算方式, Null, Decode(a.结算方式, '支票', 1, 0), 0)) * a.冲预交) As 支票支付,
                         Sum(Decode(Mod(a.记录性质, 10), 1, 0, Decode(c.开票结算方式, Null, Decode(a.结算方式, '现金', 0, '支票', 0, 1), 0)) * Decode(D.性质, 9, 0, 1) * a.冲预交) As 转帐支付,
                         Sum(冲预交) As 结算总额
                        From 病人预交记录 A, Table(l_结帐id) B, 开票结算对照 C, 结算方式 d
     ...
     Loop
      ...
      If v_版本号 = 'V3.2.0' Then
        --cashPay  预交款金额  Number  14,2  否
        v_费用 := v_费用 || ',"cashPay":' || b_Einvoice_Request_Bs.Zljsonstr(Nvl(c_结算.现金预交, 0) + Nvl(c_结算.支票预交, 0) + Nvl(c_结算.转账预交, 0), 1);
        --cashRecharge  补交金额  Number  14,2  否
        v_费用 := v_费用 || ',"cashRecharge":' || b_Einvoice_Request_Bs.Zljsonstr(Nvl(c_结算.现金支付, 0) + Nvl(c_结算.支票支付, 0) + Nvl(c_结算.转帐支付, 0), 1);
        --cashRefund  退还金额  Number  14,2  否
        v_费用 := v_费用 || ',"cashRefund":' || b_Einvoice_Request_Bs.Zljsonstr(Nvl(c_结算.现金退款, 0) + Nvl(c_结算.支票退款, 0) + Nvl(c_结算.转帐退款, 0), 1);
      Else
    ...
    For c_渠道 In (Select /*+cardinality(b,10)*/
     ...
     Loop
      If Nvl(c_渠道.渠道编码, '-') <> '-' Then
        --payChannelCode  交费渠道编码  String  10  是
        If v_缴费渠道 Is Null Then
          v_缴费渠道 := Nvl(v_缴费渠道, '') || '{"payChannelCode":"' || b_Einvoice_Request_Bs.Zljsonstr(Nvl(c_渠道.渠道编码, '-')) || '"';
        Else
          v_缴费渠道 := v_缴费渠道 || ',{"payChannelCode":"' || b_Einvoice_Request_Bs.Zljsonstr(Nvl(c_渠道.渠道编码, '-')) || '"';
        End If;
  ...
  Begin
    ...
    For c_收费细目 In (Select Min(a.Id) As 费用id, a.No, a.记录状态, a.结帐id, Nvl(a.价格父号, a.序号) As 序号, a.收费细目id, Max(a.计算单位) As 计算单位,
                          ...
                          Max(Nvl(c.名称, c1.名称)) As 收据费目, Max(Nvl(c.编码, c1.编码)) As 收据费目编码, Max(a.医嘱序号) As 医嘱序号, Max(0) As 挂号id,
                          Max(d.编码) As 类别编码, Max(d.类别) As 类别名称, Max(b.编码) As 项目编码, Max(b.名称) As 项目名称, Max(b.规格) As 规格,
                          Max(q.药品剂型) As 药品剂型
                   From 住院费用记录 A, 收费项目目录 B, 收据费目对照 C, 收据费目 C1, 收费类别 D, 药品规格 M, 药品特性 Q, 诊疗项目目录 J, 保险支付大类 T, 支付类别对照 S
                   Where a.No In (Select Distinct NO From 住院费用记录 Where 结帐id = n_结算id) And a.记录性质 = 5 And a.记录状态 = 1 And
                         a.收费类别 = d.编码(+) And a.收费细目id = b.Id And a.收据费目 = c1.名称(+) And a.收据费目 = c.收据费目(+) And Decode(c.费用场合(+), 0, 1, c.费用场合(+)) = 1 And a.收费细目id = m.药品id(+) And
                         m.药名id = q.药名id(+) And q.药名id = j.Id(+) And a.保险大类id = t.Id(+) And t.性质(+) = 1 And
                         a.保险大类id = s.保险大类id(+)
                   Group By a.No, a.记录状态, a.结帐id, Nvl(a.价格父号, a.序号), a.收费细目id, c.编码, c.名称, j.编码, j.名称
                   Having Sum(a.结帐金额) <> 0 Or n_零费用开票 = 1
                   Order By NO, 序号) Loop
      If v_患者姓名 Is Null Then
        v_患者姓名 := c_收费细目.姓名;
        v_患者性别 := c_收费细目.性别;
    ...
    Into l_结帐id
    From 住院费用记录
    Where (no, 记录性质) In (Select No, 记录性质 From 住院费用记录 Where 结帐ID = n_结算ID);
    ...
    For c_分类统计 In (Select Rownum As 序号, 收据费目编码, 收据费目名称, 数量, 计算单位, Round(单价, 2) As 单价, Round(结帐金额, 2) As 结帐金额,
                   ...
                   From (Select /*+cardinality(b,10)*/
                           Nvl(c.编码, c1.编码) As 收据费目编码, Nvl(c.名称, c1.名称) As 收据费目名称, 1 As 数量, '' As 计算单位, Sum(a.结帐金额) As 单价, a.收据费目,
                           Sum(a.结帐金额) As 结帐金额, Sum(a.结帐金额) - Sum(a.统筹金额) As 自费金额, '' As 备注
                          From 住院费用记录 A, Table(l_结帐id) B, 收据费目对照 C, 收据费目 C1
                          Where a.结帐id = b.Column_Value And a.收据费目 = c1.名称(+) And a.收据费目 = c.收据费目(+) And Decode(c.费用场合(+), 0, 1, c.费用场合(+)) = 1
                          Group By c.编码, c1.编码, c.名称, c1.名称, a.收据费目)) Loop
      --sortNo  序号  Integer  不限  是  默认从1开始，每个收费项目序号值递增1，本次不允许重复
      v_Temp := '{' || '"sortNo":' || b_Einvoice_Request_Bs.Zljsonstr(c_分类统计.序号, 1);
    ...
    v_就诊编号 := Null;
    If Nvl(n_医嘱序号, 0) <> 0 Then
      Select Max(To_Char(a.发生时间, v_JsonFormat_就诊日期)), Max(b.编码), Max(b.名称), Max(a.No)
    ...
    If v_就诊编号 Is Null Then
      --取最近一次挂号ID
      Select Max(a.Id), Max(To_Char(a.发生时间, v_JsonFormat_就诊日期)), Max(b.编码), Max(b.名称), Max(a.No)
    ...
    v_就诊信息 := v_就诊信息 || ',"' || v_JsonKey_就诊科室 || '":"' || b_Einvoice_Request_Bs.Zljsonstr(v_就诊科室名称) || '"'; --就诊科室
  ...
  Begin
    ...
    If v_版本号 = 'V3.2.0' Then
      v_JsonKey_就诊科室 := 'patientCategory';
      v_JsonKey_就诊科室编码 := 'patientCategoryCode';
      v_JsonFormat_就诊日期 := 'yyyyMMdd';
    Else
      v_JsonKey_就诊科室 := 'category';
      v_JsonKey_就诊科室编码 := 'patientCategory';
      v_JsonFormat_就诊日期 := 'yyyy-MM-dd';
    End If;
    ...
    For c_收费细目 In (Select Min(a.Id) As 费用id, a.No, a.记录状态, a.结帐id, Nvl(a.价格父号, a.序号) As 序号, a.收费细目id, Max(a.计算单位) As 计算单位,
                          ...
                          Max(b.规格) As 规格, Max(q.药品剂型) As 药品剂型
                   From 门诊费用记录 A, 病人挂号记录 B1, 收费项目目录 B, 收据费目对照 C, 收据费目 C1, 收费类别 D, 药品规格 M, 药品特性 Q, 诊疗项目目录 J, 保险支付大类 T, 支付类别对照 S
                   Where a.No = B1.No And a.No In (Select Distinct NO From 门诊费用记录 Where 结帐id = n_结算id) And a.记录性质 = 4 And a.记录状态 = 1 And
                         a.收费类别 = d.编码(+) And a.收费细目id = b.Id And a.收据费目 = c1.名称(+) And a.收据费目 = c.收据费目(+) And Decode(c.费用场合(+), 0, 1, c.费用场合(+)) = 1 And a.收费细目id = m.药品id(+) And
                         m.药名id = q.药名id(+) And q.药名id = j.Id(+) And a.保险大类id = t.Id(+) And t.性质(+) = 1 And
                         a.保险大类id = s.保险大类id(+)
                   Group By a.No, a.记录状态, a.结帐id, Nvl(a.价格父号, a.序号), a.收费细目id, c.编码, c.名称, j.编码, j.名称
    ...
    v_就诊信息 := v_就诊信息 || ',"consultationDate":"' || b_Einvoice_Request_Bs.Zljsonstr(v_就诊日期) || '"'; --患者就医时间
    v_就诊信息 := v_就诊信息 || ',"' || v_JsonKey_就诊科室 || '":"' || b_Einvoice_Request_Bs.Zljsonstr(v_就诊科室名称) || '"'; --就诊科室
    v_就诊信息 := v_就诊信息 || ',"' || v_JsonKey_就诊科室编码 || '":"' || b_Einvoice_Request_Bs.Zljsonstr(v_就诊科室编码) || '"'; --就诊科室编码
    --patientNo  患者就诊编号  String  20  是  患者每次就诊一次就生成的一个新的编号。（患者登记号）
    v_就诊信息 := v_就诊信息 || ',"patientNo":"' || b_Einvoice_Request_Bs.Zljsonstr(v_就诊编号) || '"';
    v_就诊信息 := v_就诊信息 || ',"patientId":"' || b_Einvoice_Request_Bs.Zljsonstr(n_病人id) || '"'; --患者在业务系统中的唯一标识ID，类似身份证号码。
    v_就诊信息 := v_就诊信息 || ',"sex":"' || b_Einvoice_Request_Bs.Zljsonstr(v_患者性别) || '"'; --性别
  ...
  (
    ...
    Cursor c_Balance_Record Is
      Select a.No, a.收费时间, a.结帐类型, a.操作员编号, a.操作员姓名, a.病人id, a.主页id,
             ...
             Nvl(b.病案号, b.住院号) As 病历号, Nvl(b.医疗付款方式, c.医疗付款方式) As 医疗付款方式, Nvl(b.出院日期, Sysdate) - b.入院日期 As 住院天数, F.编码 As 疾病编码
      From 病人结帐记录 A, 病案主页 B, 病人信息 C, 合约单位 Q, 人员表 J, 部门表 M, 部门表 P, 部门表 T, 病人诊断记录 E, 疾病编码目录 F
      Where a.Id = n_结算id And a.病人id = b.病人id(+) And a.主页id = b.主页id(+) And a.病人id = c.病人id(+) And a.原因 = q.名称(+) And
            a.病人ID = e.病人ID(+) And a.主页id = e.主页ID(+) And e.诊断次序(+) = 1 And Mod(e.诊断类型(+), 10) = 1 And e.疾病ID = f.id(+) And
  ...
  Begin
    ...
    Where a.结帐id = n_结算id And a.记录状态 = 1 And a.结算方式 = b.名称 And b.性质 = 9;
    ...
    v_住院次数 := Null;
    For c_收费细目 In (Select Min(a.费用ID) As 费用ID, a.No, a.序号, a.收费细目id, a.计算单位, Avg(a.价格) As 价格, Avg(a.数量) As 数量,
                             Sum(a.应收金额) As 应收金额, Sum(a.实收金额) As 实收金额, Sum(a.结帐金额) As 结帐金额, Sum(a.自费金额) As 自费金额,
                             Max(a.医保项目名称) As 医保项目编码, Max(a.医保项目名称) As 医保项目名称, Max(a.医保报销比例) As 医保报销比例, Max(a.备注) As 备注,
                             Max(a.费用类型) As 费用类型, Max(a.操作员编号) As 操作员编号, Max(a.操作员姓名) As 操作员姓名,
                             Max(a.病人id) As 病人id, Max(a.登记时间) As 登记时间,Max(a.收据费目) As 收据费目, Max(a.收据费目编码) As 收据费目编码, Max(a.主页id) As 主页id,
                             Max(a.类别编码) As 类别编码, Max(a.类别名称) As 类别名称, Max(a.项目编码) As 项目编码, Max(a.项目名称) As 项目名称,
                             Max(a.规格) As 规格, Max(a.药品剂型) As 药品剂型
                       From (Select Min(a.Id) As 费用id, a.No, a.记录状态, a.结帐id, Nvl(a.价格父号, a.序号) As 序号, a.收费细目id, Max(a.计算单位) As 计算单位,
                                    Sum(a.标准单价) As 价格, Avg(Nvl(a.付数, 1) * Nvl(a.数次, 0)) As 数量, Sum(a.应收金额) As 应收金额,
                                    Sum(a.实收金额) As 实收金额, Sum(a.结帐金额) As 结帐金额, Sum(a.实收金额) - Sum(a.统筹金额) As 自费金额,
                                    ...
                                    Max(a.登记时间) As 登记时间, Max(Nvl(c.名称, c1.名称)) As 收据费目, Max(Nvl(c.编码, c1.编码)) As 收据费目编码, Max(a.主页id) As 主页id,
                                    Max(d.编码) As 类别编码, Max(d.类别) As 类别名称, Max(b.编码) As 项目编码, Max(b.名称) As 项目名称, Max(b.规格) As 规格,
                                    Max(q.药品剂型) As 药品剂型
                             From 住院费用记录 A, 收费项目目录 B, 收据费目对照 C, 收据费目 C1, 收费类别 D, 药品规格 M, 药品特性 Q, 诊疗项目目录 J, 保险支付大类 T, 支付类别对照 S
                             Where a.No In (Select Distinct NO From 住院费用记录 Where 结帐id = n_结算id) And a.记帐费用 = 1 And a.收费类别 = d.编码(+) And a.收费细目id = b.Id And 
                                   a.收据费目 = c1.名称(+) And a.收据费目 = c.收据费目(+) And Decode(c.费用场合(+), 0, 2, c.费用场合(+)) = 2 and
                                   a.收费细目id = m.药品id(+) And m.药名id = q.药名id(+) And q.药名id = j.Id(+) And a.保险大类id = t.Id(+) And
                                   t.性质(+) = 1 And a.保险大类id = s.保险大类id(+)
                             Group By a.No, a.记录状态, a.结帐id, Nvl(a.价格父号, a.序号), a.收费细目id, c.编码, c.名称, j.编码, j.名称
                             Order By NO, 序号) a
                       Group By a.No, a.序号, a.收费细目id, a.计算单位
                       Having Sum(a.结帐金额) <> 0 Or n_零费用开票 = 1) Loop
    ...
    For c_分类统计 In (Select Rownum As 序号, 收据费目编码, 收据费目名称, 数量, 计算单位, Round(单价, 2) As 单价, Round(结帐金额, 2) As 结帐金额,
                   ...
                   From (Select /*+cardinality(b,10)*/
                           Nvl(c.编码, c1.编码) As 收据费目编码, Nvl(c.名称, c1.名称) As 收据费目名称, 1 As 数量, '' As 计算单位, Sum(a.结帐金额) As 单价, a.收据费目,
                           Sum(a.结帐金额) As 结帐金额, Sum(a.结帐金额) - Sum(a.统筹金额) As 自费金额, '' As 备注
                          From 住院费用记录 A, 收据费目对照 C, 收据费目 C1
                          Where a.结帐id = n_结算id And a.收据费目 = c1.名称(+) And a.收据费目 = c.收据费目(+) And Decode(c.费用场合(+), 0, 2, c.费用场合(+)) = 2
                          Group By c.编码, c1.编码, c.名称, c1.名称, a.收据费目)) Loop
      --sortNo  序号  Integer  不限  是  默认从1开始，每个收费项目序号值递增1，本次不允许重复
      v_Temp := '{' || '"sortNo":' || b_Einvoice_Request_Bs.Zljsonstr(c_分类统计.序号, 1);
      --chargeCode  收费项目代码  String  50  是  填写业务系统内部编码值，由医疗平台配置对照
    ...
    v_票据信息 := v_票据信息 || ',"busDateTime":"' ||
              b_Einvoice_Request_Bs.Zljsonstr(To_Char(r_Balance_Record.收费时间, 'yyyymmddHH24miss') || '000') || '"';
    v_票据信息 := v_票据信息 || ',"placeCode":"' || b_Einvoice_Request_Bs.Zljsonstr(v_开票点) || '"'; --开票点编码:直接填写业务系统内部编码值，由医疗平台配置对照
    ...
    Else
      ...
      For c_通知 In (Select 病人id, 卡号
                   From (Select b.病人id, b.卡号
                          From 病人医疗卡信息 B
                          Where b.卡类别id = n_缺省卡类别id And b.病人id = r_Balance_Record.病人id)
                   Where Rownum < 2) Loop

        If c_通知.卡号 Is Not Null Then

          v_Temp := ',"cardType":"' || b_Einvoice_Request_Bs.Zljsonstr(v_医疗卡类型编号) || '"'; --卡类型
      ...
      If v_Temp Is Not Null Then
        v_通知 := v_通知 || v_Temp;
      Else
        If r_Balance_Record.身份证号 Is Not Null And v_身份证作卡类型编号 IS NOT null Then
          v_通知 := v_通知 || ',"cardType":"' || b_Einvoice_Request_Bs.Zljsonstr(v_身份证作卡类型编号) || '"'; --卡类型
    ...
    v_就诊信息 := v_就诊信息 || ',"consultationDate":"' ||
              b_Einvoice_Request_Bs.Zljsonstr(To_Char(r_Balance_Record.入院日期, v_JsonFormat_就诊日期)) || '"';
    --patientId  患者唯一ID  String  50  否  患者在业务系统中的唯一标识ID，类似身份证号码。
    ...
    --age  年龄  String  10  是
    v_就诊信息 := v_就诊信息 || ',"age":"' || b_Einvoice_Request_Bs.Zljsonstr(r_Balance_Record.年龄) || '"';
    ...
    --ICD  疾病编码  String  20  否
    v_就诊信息 := v_就诊信息 || ',"ICD":"' || b_Einvoice_Request_Bs.Zljsonstr(r_Balance_Record.疾病编码) || '"';
    If Instr(v_住院次数, ',') > 0 Then
      ...
      For c_主页 In (Select Min(入院日期) As 入院日期, Max(出院日期) As 出院日期, Sum(Nvl(出院日期, Sysdate) - 入院日期) As 住院天数
        ...
        v_就诊信息 := v_就诊信息 || ',"inHospitalDate":"' || b_Einvoice_Request_Bs.Zljsonstr(To_Char(c_主页.入院日期, v_JsonFormat_就诊日期)) || '"';
        --outHospitalDate  出院日期  String  10  否  格式:yyyy-MM-dd
        v_就诊信息 := v_就诊信息 || ',"outHospitalDate":"' || b_Einvoice_Request_Bs.Zljsonstr(To_Char(c_主页.出院日期, v_JsonFormat_就诊日期)) || '"';
        --hospitalDays  住院天数  Number  6,2  否
        v_就诊信息 := v_就诊信息 || ',"hospitalDays":' || b_Einvoice_Request_Bs.Zljsonstr(Round(Nvl(c_主页.住院天数, 0), 2), 1);
        Exit;
    ...
    Else
      ...
      v_就诊信息 := v_就诊信息 || ',"outHospitalDate":"' ||
                b_Einvoice_Request_Bs.Zljsonstr(To_Char(r_Balance_Record.出院日期, v_JsonFormat_就诊日期)) || '"';
      --hospitalDays  住院天数  Number  6,2  否
      v_就诊信息 := v_就诊信息 || ',"hospitalDays":' || b_Einvoice_Request_Bs.Zljsonstr(Round(Nvl(r_Balance_Record.住院天数, 0), 2), 1);
    End If;
    ...
    For c_冲预交 In (
                  ...
                  From (Select NO, Sum(冲预交) As 冲预交
                          From 病人预交记录
                          Where 结帐id = n_结算id And Mod(记录性质, 10) = 1 Group by No) A, 病人预交记录 B, 电子票据使用记录 Q
                  Where a.No = b.No And b.记录性质 = 1 And b.Id = q.结算id And q.票种 = 2 And q.退款id Is Null
                  Group by q.凭证代码, q.凭证号码, a.No) Loop
      ...
      v_Temp := v_Temp || ',"voucherAmt":' || b_Einvoice_Request_Bs.Zljsonstr(c_冲预交.冲预交, 1) || '}';
    ...
    For c_结算 In (Select 现金预交, 支票预交, 转账预交, 个人帐户支付, 医保统筹基金支付, 其它医保支付, 个人现金支付, Decode(Sign(现金支付), -1, 现金支付, 0) As 现金退款,
                 ...
                 From (Select /*+cardinality(b,10)*/
                         ...
                         Sum(Decode(Mod(a.记录性质, 10), 1, 0, Decode(c.开票结算方式, Null, Decode(a.结算方式, '现金', 0, '支票', 0, 1), 0)) * Decode(D.性质, 9, 0, 1) * a.冲预交) As 转帐支付,
                         Sum(冲预交) As 结算总额
                        From 病人预交记录 A, 开票结算对照 C, 结算方式 d
                        Where a.结帐id = n_结算id And a.结算方式 = c.结算方式(+) And A.结算方式 = d.名称(+))) Loop
      --accountPay  个人账户支付  Number  14,2  是  按政策规定用个人账户支付参保人的医疗费用（含基本医疗保险目录范围内和目录范围外的费用）；
      --          如无金额，填写0
      v_费用 := ',"accountPay":' || b_Einvoice_Request_Bs.Zljsonstr(Nvl(c_结算.个人帐户支付, 0), 1);
      --fundPay  医保统筹基金支付  Number  14,2  是  患者本次就医所发生的医疗费用中按规定由基本医疗保险统筹基金支付的金额；
  ...
  Begin
    ...
    For c_收费细目 In (Select Min(a.费用ID) As 费用ID, a.No, a.序号, a.收费细目id, a.计算单位, Avg(a.价格) As 价格, Avg(a.数量) As 数量,
                       ...
                       From (Select Min(a.Id) As 费用id, a.No, a.记录状态, a.结帐id, Nvl(a.价格父号, a.序号) As 序号, a.收费细目id, Max(a.计算单位) As 计算单位,
                                    ...
                                    Max(b.规格) As 规格, Max(q.药品剂型) As 药品剂型
                             From 门诊费用记录 A, 收费项目目录 B, 收据费目对照 C, 收据费目 C1, 收费类别 D, 药品规格 M, 药品特性 Q, 诊疗项目目录 J, 保险支付大类 T, 支付类别对照 S
                             Where a.no In (Select No From 门诊费用记录 Where 结帐ID = n_结算id) And a.记帐费用 = 1 And a.收费类别 = d.编码(+) And a.收费细目id = b.Id And 
                                   a.收据费目 = c1.名称(+) And a.收据费目 = c.收据费目(+) And Decode(c.费用场合(+), 0, 1, c.费用场合(+)) = 1 and
                                   a.收费细目id = m.药品id(+) And m.药名id = q.药名id(+) And q.药名id = j.Id(+) And a.保险大类id = t.Id(+) And
                                   t.性质(+) = 1 And a.保险大类id = s.保险大类id(+)
                             Group By a.No, a.记录状态, a.结帐id, Nvl(a.价格父号, a.序号), a.收费细目id, c.编码, c.名称, j.编码, j.名称
                             Order By a.NO, 序号) a
    ...
    For c_分类统计 In (Select Rownum As 序号, 收据费目编码, 收据费目名称, 数量, 计算单位, Round(单价, 2) As 单价, Round(结帐金额, 2) As 结帐金额,
                   ...
                   From (Select /*+cardinality(b,10)*/
                           Nvl(c.编码, c1.编码) As 收据费目编码, Nvl(c.名称, c1.名称) As 收据费目名称, 1 As 数量, '' As 计算单位, Sum(a.结帐金额) As 单价, a.收据费目,
                           Sum(a.结帐金额) As 结帐金额, Sum(a.结帐金额) - Sum(a.统筹金额) As 自费金额, '' As 备注
                          From 门诊费用记录 A, 收据费目对照 C, 收据费目 C1
                          Where a.结帐id = n_结算id And a.收据费目 = c1.名称(+) And a.收据费目 = c.收据费目(+) And Decode(c.费用场合(+), 0, 1, c.费用场合(+)) = 1
                          Group By c.编码, c1.编码, c.名称, c1.名称, a.收据费目)) Loop
      --sortNo  序号  Integer  不限  是  默认从1开始，每个收费项目序号值递增1，本次不允许重复
      v_Temp := '{' || '"sortNo":' || b_Einvoice_Request_Bs.Zljsonstr(c_分类统计.序号, 1);
      --chargeCode  收费项目代码  String  50  是  填写业务系统内部编码值，由医疗平台配置对照
    ...
    If Nvl(r_Balance_Record.病人id, 0) <> 0 Then
      ...
      For c_通知 In (Select Max(卡号) As 卡号
                   From (

                          Select 病人id, 卡号
                          From (Select b.病人id, b.卡号
                                  From 病人医疗卡信息 B
                                  Where b.卡类别id = n_缺省卡类别id And b.病人id = Nvl(r_Balance_Record.病人id, 0))
                          Where Rownum < 2)) Loop
        ...
        Exit;
      End Loop;
      If r_Balance_Record.身份证号 Is Not Null And v_Temp Is Null And v_身份证作卡类型编号 IS NOT null Then
        v_Temp := ',"cardType":"' || b_Einvoice_Request_Bs.Zljsonstr(v_身份证作卡类型编号) || '"'; --卡类型
        v_Temp := v_Temp || ',"cardNo":"' || b_Einvoice_Request_Bs.Zljsonstr(r_Balance_Record.身份证号) || '"'; --卡号
      End If;
    End If;
    If v_Temp Is Null Then
      --没有一张卡，固定一种卡类别
      v_Temp := ',"cardType":"' || b_Einvoice_Request_Bs.Zljsonstr(v_病人无卡的卡类别编号) || '"'; --卡类型
      v_Temp := v_Temp || ',"cardNo":"' || b_Einvoice_Request_Bs.Zljsonstr(v_病人无卡的卡号) || '"'; --卡号
    End If;
    v_通知 := v_通知 || v_Temp;
    ...
    For c_结算 In (Select 现金预交, 支票预交, 转账预交, 个人帐户支付, 医保统筹基金支付, 其它医保支付, 个人现金支付, Decode(Sign(现金支付), -1, 现金支付, 0) As 现金退款,
                 ...
                 From (Select /*+cardinality(b,10)*/
                         ...
                         Sum(冲预交) As 结算总额
                        From 病人预交记录 A, 开票结算对照 C, 结算方式 d
                        Where a.结帐id = n_结算id And a.结算方式 = c.结算方式(+) And A.结算方式 = d.名称(+)))
  ...
  Begin
    ...
    Begin
      ...
      From (Select b.病人id, b.卡号
             From 病人医疗卡信息 B
             Where b.卡类别id = n_缺省卡类别id And b.病人id = r_Deposit_Rec.病人id)
      Where Rownum < 2;
    Exception
      When Others Then
        v_卡号 := Null;
    ...
    If v_卡号 Is Null Then
      If r_Deposit_Rec.身份证号 Is Not Null And v_医疗卡类型编号 Is Not Null Then
        v_卡类别 := v_医疗卡类型编号;
    ...
    If Nvl(n_冲销id, 0) <> 0 Then
      ...
      For c_票据 In (Select 代码, 号码, 凭证代码, 凭证号码
        ...
        v_预交 := v_预交 || ',"billBatchCode":"' || b_Einvoice_Request_Bs.Zljsonstr(c_票据.代码) || '"';
        --billNo  电子票据号码  String  20  是
        v_预交 := v_预交 || ',"billNo":"' || b_Einvoice_Request_Bs.Zljsonstr(c_票据.号码) || '"';
        --reason  冲红原因  String  200  是
        v_预交 := v_预交 || ',"reason":"' || b_Einvoice_Request_Bs.Zljsonstr('退款') || '"';
        --operator  经办人  String  60  是
        v_预交 := v_预交 || ',"operator":"' || b_Einvoice_Request_Bs.Zljsonstr(v_操作员姓名) || '"';
        ...
        --voucherBatchCode  预交金凭证代码  String  50  是
        v_预交 := v_预交 || ',"voucherBatchCode":"' || b_Einvoice_Request_Bs.Zljsonstr(c_票据.凭证代码) || '"';
        --voucherNo  预交金凭证号码  String  20  是
        v_预交 := v_预交 || ',"voucherNo":"' || b_Einvoice_Request_Bs.Zljsonstr(c_票据.凭证号码) || '"';
        --amt  预缴金退款金额  Number  14,2  是
        v_预交 := v_预交 || ',"amt":' || b_Einvoice_Request_Bs.Zljsonstr(-1 * n_票据总金额, 1);
        ...
        v_预交 := v_预交 || ',"remark":"' || b_Einvoice_Request_Bs.Zljsonstr(r_Deposit_Rec.摘要) || '"}';
        Exit;
      End Loop;
      If v_预交 Is Null Then
    ...
    v_预交 := v_预交 || ',"payer":"' || b_Einvoice_Request_Bs.Zljsonstr(r_Deposit_Rec.姓名) || '"';
    --busDateTime  业务发生时间  String  17  是  格式：yyyyMMddHHmmssSSS
    v_预交 := v_预交 || ',"busDateTime":"' ||
            b_Einvoice_Request_Bs.Zljsonstr(To_Char(r_Deposit_Rec.收款时间, 'yyyymmddhh24miss') || '000') || '"';
    --placeCode  开票点编码  String  50  是  直接填写业务系统内部编码值，由医疗平台配置对照
    v_预交 := v_预交 || ',"placeCode":"' || b_Einvoice_Request_Bs.Zljsonstr(v_开票点) || '"';
    ...
    v_预交 := v_预交 || ',"author":"' || b_Einvoice_Request_Bs.Zljsonstr(v_操作员姓名) || '"';
    ...
    v_预交 := v_预交 || ',"inHospitalDate":"' || b_Einvoice_Request_Bs.Zljsonstr(To_Char(r_Deposit_Rec.入院日期, 'yyyy-mm-dd')) || '"';
    --hospitalNo  患者住院号  String  20  是  从入院到出院结束后，整个流程的唯一号
  ...
  Begin
    ...
    If 业务场景_In = 1 Or 业务场景_In = 4 Then
      --收费及挂号
        ...
        Select a.病人id, Nvl(a.姓名, b.姓名) As 姓名, Nvl(a.性别, b.性别) As 性别, Nvl(a.年龄, b.年龄) As 年龄,
        b.门诊号, b.住院号, Sum(a.实收金额)
        Into n_病人id, v_姓名, v_性别, v_年龄, n_门诊号, n_住院号, n_票据金额
        From 门诊费用记录 A, 病人信息 B
        Where a.结帐id = 结算id_In And a.病人id = b.病人id(+)
        Group By a.病人id, a.姓名, b.姓名, a.年龄, b.年龄, a.性别, b.性别,
              b.门诊号, b.住院号;
    ...
    If 业务场景_In = 2 Then
      --预交
      Begin
        Select a.病人id, Nvl(c.姓名, b.姓名) As 姓名, Nvl(c.性别, b.性别) As 性别, Nvl(c.年龄, b.年龄) As 年龄, b.门诊号,
               Nvl(c.住院号, b.住院号) As 住院号, Nvl(预交类别, 2), a.金额
        Into n_病人id, v_姓名, v_性别, v_年龄, n_门诊号, n_住院号, n_是否门诊, n_票据金额
    ...
    If 业务场景_In = 3 Then
      --结帐
      Begin
        Select a.病人id, Decode(Nvl(a.病人id, 0), 0, a.原因, Nvl(c.姓名, b.姓名)) As 姓名, Nvl(c.性别, b.性别) As 性别,
               Nvl(c.年龄, b.年龄) As 年龄, b.门诊号, Nvl(c.住院号, b.住院号) As 住院号, Nvl(结帐类型, 2) As n_是否门诊,
               a.结帐金额
        Into n_病人id, v_姓名, v_性别, v_年龄, n_门诊号, n_住院号, n_是否门诊, n_票据金额
    ...
    If n_Code = 0 Then
      Delete From 电子票据使用记录 Where id = n_电子票据id;
      错误信息_Out := v_Err_Msg;
      Return 0;
    End If;
  ...
  (
    ...
    Cursor c_Einvoice Is
      ...
      From 电子票据使用记录 A
      Where a.结算Id = 结算id_In And a.记录状态 = 1 And 票种 = 业务场景_In;
  ...
  Begin
    ...
    If j_Json.Get_String('isPrtPaper') = '1' Then
      错误信息_Out := '已经打印纸质票据，不允许作废操作!';
      Return 0;
    End If;
    If b_Einvoice_Request_Bs.Get_Version <> 'V3.1.0' Then
      --无下帐接口
      Return 1;
    End If;

    --4.1.16  查询电子票据入账状态接口
    v_Service_Name := 'getEBillAccountStatus';
    v_Version      := '1.0';
...
(
  ...
  --功能：更新/插入电子票据修正记录
  --入参：
  --  HIS票据状态_In\平台票据状态_In 1-正常，2-作废，3-冲红
...
Create Or Replace Procedure Zl_收据费目对照_Update
(
  操作类型_In In Number,
  Id_In       In 收据费目对照.Id%Type,
  收据费目_In In 收据费目对照.收据费目%Type := Null,
  名称_In     In 收据费目对照.名称%Type := Null,
  编码_In     In 收据费目对照.编码%Type := Null,
  费用场合_In In 收据费目对照.费用场合%Type := Null
) Is
  --说明:
  --     操作类型_In:1-新增;2-修改;3-删除
Begin
  If Nvl(操作类型_In, 0) = 0 Then
    ...
    Insert Into 收据费目对照
      (ID, 收据费目, 名称, 编码, 费用场合)
    Values
      (Id_In, 收据费目_In, 名称_In, 编码_In, 费用场合_In);
  Elsif Nvl(操作类型_In, 0) = 1 Then
    --修改
    Update 收据费目对照
    Set 收据费目 = 收据费目_In, 名称 = 名称_In, 编码 = 编码_In, 费用场合 = 费用场合_In
    Where ID = Id_In;
  Else
    --删除
    Delete From 收据费目对照 Where ID = Id_In;
  End If;

Exception
  When Others Then
    zl_ErrorCenter(SQLCode, SQLErrM);
End Zl_收据费目对照_Update;
/

Create Or Replace Procedure Zl_收费渠道对照_Update
(
  操作类型_In   In Number,
  ...
  --说明:
  --     操作类型_In:0-新增;1-修改;2-删除;3:调整结算方式
Begin
  If Nvl(操作类型_In, 0) = 0 Then
    --新增
    Insert Into 收费渠道对照 (结算方式, 卡类别id, 渠道编码) Values (结算方式_In, 卡类别id_In, 渠道编码_In);
  Elsif Nvl(操作类型_In, 0) = 1 Then
    --修改
    Update 收费渠道对照
    Set 渠道编码 = 渠道编码_In
    Where 结算方式 = 结算方式_In And Nvl(卡类别id, 0) = Nvl(卡类别id_In, 0);
  Elsif Nvl(操作类型_In, 0) = 2 Then
    --删除
    Delete From 收费渠道对照
    Where 结算方式 = 结算方式_In And Nvl(卡类别id, 0) = Nvl(卡类别id_In, 0) And 渠道编码 = 渠道编码_In;
  Else
    --调整结算方式
    Update 收费渠道对照
    Set 结算方式 = 结算方式_In
    Where 结算方式 = 原结算方式_In And Nvl(卡类别id, 0) = Nvl(卡类别id_In, 0) And 渠道编码 = 渠道编码_In;
  End If;

Exception
  When Others Then
    zl_ErrorCenter(SQLCode, SQLErrM);
End Zl_收费渠道对照_Update;
/

Create Or Replace Procedure Zl_支付类别对照_Update
(
  操作类型_In   In Number,
...
Begin
  If Nvl(操作类型_In, 0) = 0 Then
    --新增
    Insert Into 支付类别对照 (保险大类id, 大类编码, 大类名称) Values (保险大类id_In, 大类编码_In, 大类名称_In);
  Elsif Nvl(操作类型_In, 0) = 1 Then
    --修改
    Update 支付类别对照 Set 大类编码 = 大类编码_In, 大类名称 = 大类名称_In Where 保险大类id = 保险大类id_In;
  Else
    --删除
    Delete From 支付类别对照 Where 保险大类id = 保险大类id_In;
  End If;

Exception
  When Others Then
    zl_ErrorCenter(SQLCode, SQLErrM);
End Zl_支付类别对照_Update;
/

Create Or Replace Procedure Zl_开票结算对照_Update
(
  操作类型_In     In Number,
  结算方式_In     In 开票结算对照.结算方式%Type,
  开票结算方式_In In 开票结算对照.开票结算方式%Type := Null
) Is
  --说明:
  --     操作类型_In:0-新增或修改;1-删除
Begin
  If Nvl(操作类型_In, 0) = 0 Then
    --新增或修正
    Update 开票结算对照 Set 开票结算方式 = 开票结算方式_In Where 结算方式 = 结算方式_In;
    If Sql%NotFound Then
      Insert Into 开票结算对照 (结算方式, 开票结算方式) Values (结算方式_In, 开票结算方式_In);
    End If;
  Else
    --删除
    Delete From 开票结算对照 Where 结算方式 = 结算方式_In;
  End If;

Exception
  When Others Then
    zl_ErrorCenter(SQLCode, SQLErrM);
End Zl_开票结算对照_Update;
/

--增加公共同义词
Create public Synonym 收据费目对照 For 收据费目对照;
Create public Synonym 收费渠道对照 For 收费渠道对照;
Create public Synonym 开票结算对照 For 开票结算对照;
Create public Synonym 支付类别对照 For 支付类别对照;
Create public Synonym 电子票据核对记录 For 电子票据核对记录;
Create public Synonym 电子票据修正记录 For 电子票据修正记录;
Create public Synonym b_Einvoice_Request_Bs For b_Einvoice_Request_Bs;
Create public Synonym Zl_电子票据核对记录_Update For Zl_电子票据核对记录_Update;
Create public Synonym Zl_电子票据修正记录_Update For Zl_电子票据修正记录_Update;
Create public Synonym Zl_收据费目对照_Update For Zl_收据费目对照_Update;
Create public Synonym Zl_收费渠道对照_Update For Zl_收费渠道对照_Update;
Create public Synonym Zl_支付类别对照_Update For Zl_支付类别对照_Update;
Create public Synonym Zl_开票结算对照_Update For Zl_开票结算对照_Update;
