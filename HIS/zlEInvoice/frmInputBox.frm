...
Public Function InputBox(frmParent As Object, ByVal 标题 As String, ByVal 提示 As String, _
    ...
    txt.Top = lblInfo.Top + lblInfo.Height + 100
...
Private Sub cmdOK_Click()
    If Not mblnNULL And txt.Text = "" Then
        MsgBox "必须输入" & Caption & "！", vbInformation, gstrProductName
        txt.SetFocus: Exit Sub
    End If
    If mintLen > 0 Then
        If zlCommFun.ActualLen(txt.Text) > mintLen Then
            MsgBox Caption & "最多允许输入 " & mintLen & " 个字符或 " & mintLen \ 2 & " 个汉字！", vbInformation, gstrProductName
            txt.SetFocus: Exit Sub
        End If
    End If
