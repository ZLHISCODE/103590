...
Public Function IsMD5Check() As Boolean
'功能:是否使用简易FTP
    Dim strSQL As String, rsTmp As ADODB.Recordset
    On Error GoTo ErrH
    strSQL = "Select Nvl(Max(内容), 0) As MD5不匹配也安装 From Zlreginfo Where 项目 = '安装文件不检查MD5'"
    Set rsTmp = OpenSQLRecord(strSQL, "MD5不匹配也安装")
    IsMD5Check = Val(rsTmp!MD5不匹配也安装 & "") = 0
    Exit Function
ErrH:
    If ErrCenter() = 1 Then
        Resume
