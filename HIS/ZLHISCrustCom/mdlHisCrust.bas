...
Public gblnMD5Check         As Boolean                  '是否MD5不匹配不安装
'命令行解析内容
...
Public gblnHelperMain       As Boolean
...
Public Function InitCommon() As Boolean
    Dim objHelperMainStartJob As New clsMutex
    On Error GoTo ErrH
    gblnAutoLogin = True
    gblnIs64Bits = Is64bit
    gstrSetupPath = GetSetupPath
    gblnHelperMain = objHelperMainStartJob.CheckMutex(M_HELPERMAIN_STARTJOB)
    Set objHelperMainStartJob = Nothing
...
ErrH:
    ...
    If Not gblnHelperMain Then MsgBox "获取命令行信息发生致命错误，请联系管理员！信息：" & vbNewLine & strErrInfo, vbInformation, App.Title
    Err.Clear
End Function
...
'返回：创建的连接
    ...
    If strCommand Like "ZLUPDATE:*" Then
        ...
        For i = LBound(arrCommand) To UBound(arrCommand)
            ...
            ElseIf arrCommand(i) Like "EXEPATH=*" Then
                gstrAppEXE = Mid(arrCommand(i), Len("EXEPATH=*"))
            ElseIf arrCommand(i) Like "WATI=*" Then
                gintWaite = Val(Mid(arrCommand(i), Len("WATI=*")))
            End If
        Next
        gblnAutoLogin = False
        ...
        If strUser = "" Or strPwd = "" Or strServer = "" Then
            ...
            If blnMsg Then
                If Not gblnHelperMain Then MsgBox "命令行信息不完整，请联系管理员！", vbInformation, App.Title
            End If
            Exit Function
        End If
    ...
    If Err.Number <> 0 Then
        ...
        If blnMsg And Not gblnSilence Then
            If Not gblnHelperMain Then MsgBox "打开数据库连接失败，请联系管理员！信息：" & vbNewLine & Err.Description, vbInformation, App.Title
        End If
        Err.Clear
        Exit Function
    End If
...
ErrH:
    ...
    If Not gblnSilence And Not gblnHelperMain Then MsgBox "启动获取连接失败，请联系管理员！信息：" & vbNewLine & Err.Description, vbInformation, App.Title
    Err.Clear
End Function
...
Public Sub CallHISEXE(Optional blnAutoLogin As Boolean)
    ...
    If blnAutoLogin And gblnAutoLogin Then
        ...
        '支持SM4算法，则加密
        If sm_version <> 0 Then
            strCommand = Sm4EncryptEcb(strCommand)
        End If
...
ErrH:
    ...
    If Not gblnHelperMain Then MsgBox "获取基础信息发生错误，请联系管理员！信息：" & vbNewLine & strErrInfo, vbInformation, App.Title
    Err.Clear
    Resume
End Function
...
Private Function InitTerminal(ByRef strAudsid As String) As String
    ...
    Exit Function
ErrH:
    If Not gblnHelperMain Then MsgBox Err.Description
End Function
...
'功能：获取OracleHome路径
    ...
    blnRead = gobjRegistry.GetRegValue("HKEY_LOCAL_MACHINE\SOFTWARE\" & IIf(gblnIs64Bits, "WOW6432Node\", "") & "Oracle\ALL_HOMES", "DEFAULT_HOME", strDefault)
    If blnRead And strDefault <> "" Then
        arrSubKey = gobjRegistry.GetAllSubKey("HKEY_LOCAL_MACHINE\SOFTWARE\" & IIf(gblnIs64Bits, "WOW6432Node\", "") & "Oracle\ALL_HOMES")
        If TypeName(arrSubKey) <> "Empty" Then
            For i = LBound(arrSubKey) To UBound(arrSubKey)
                strHome = ""
                blnRead = gobjRegistry.GetRegValue("HKEY_LOCAL_MACHINE\SOFTWARE\" & IIf(gblnIs64Bits, "WOW6432Node\", "") & "Oracle\ALL_HOMES\" & arrSubKey(i), "NAME", strHome)
                If blnRead And strHome <> "" Then
                    If strHome = strDefault Then
                        blnRead = gobjRegistry.GetRegValue("HKEY_LOCAL_MACHINE\SOFTWARE\" & IIf(gblnIs64Bits, "WOW6432Node\", "") & "Oracle\ALL_HOMES\" & arrSubKey(i), "PATH", strPath)
    ...
    arrSubKey = Empty
    arrSubKey = gobjRegistry.GetAllSubKey("HKEY_LOCAL_MACHINE\SOFTWARE\" & IIf(gblnIs64Bits, "WOW6432Node\", "") & "Oracle")
    If TypeName(arrSubKey) <> "Empty" Then
        For i = LBound(arrSubKey) To UBound(arrSubKey)
            strHome = ""
            blnRead = gobjRegistry.GetRegValue("HKEY_LOCAL_MACHINE\SOFTWARE\" & IIf(gblnIs64Bits, "WOW6432Node\", "") & "Oracle\" & arrSubKey(i), "ORACLE_HOME", strHome)
            If blnRead And strHome <> "" Then
                If Not objPE.Is64bit(strHome & "\Bin\oci.dll") Then
                    If gobjFSO.FileExists(strHome & "\network\ADMIN\tnsnames.ora") Then
                        GetOracleHome = strHome
...
'返回：是否安装成功
    ...
    If Not gclsRegCom.RegCom(strOracleHome & "\Bin\oradc.ocx", strErr, RFT_NormalReg) Then
...
'    If Not gclsRegCom.RegCom(strOracleHome & "\Bin\OIP" & strOracleVer & ".dll", strErr, RFT_NormalReg) Then
'        strErr = strErr & ",OIP" & strOracleVer & ".dll"

'    End If

    If Not gclsRegCom.RegCom(strOracleHome & "\Bin\oo4ocodewiz.dll", strErr, RFT_NormalReg) Then
        strErr = strErr & ",oo4ocodewiz.dll"
    End If

    If Not gclsRegCom.RegCom(strOracleHome & "\Bin\odbtreeview.ocx", strErr, RFT_NormalReg) Then
        strErr = strErr & ",odbtreeview.ocx"
    End If

    If Not gclsRegCom.RegCom(strOracleHome & "\Bin\oo4oaddin.dll", strErr, RFT_NormalReg) Then
    ...
    Call gobjRegistry.CreateRegKey(strOracleReg, "OO4O", strOracleHome & "\OO4O\mesg")
    Call gobjRegistry.CreateRegKey(strOracleReg & "\OO4O", "CacheBlocks", "20")
    Call gobjRegistry.CreateRegKey(strOracleReg & "\OO4O", "FetchLimit", "100")
    Call gobjRegistry.CreateRegKey(strOracleReg & "\OO4O", "FetchSize", "4096")
    Call gobjRegistry.CreateRegKey(strOracleReg & "\OO4O", "PerBlock", "16")
    Call gobjRegistry.CreateRegKey(strOracleReg & "\OO4O", "SliceSize", "256")
    Call gobjRegistry.CreateRegKey(strOracleReg & "\OO4O", "TempFileDirectory", "c:\temp")
    Call gobjRegistry.CreateRegKey(strOracleReg & "\OO4O", "OO4O_HOME", strOracleHome & "\OO4O")
