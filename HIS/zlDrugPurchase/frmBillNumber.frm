...
Begin VB.Form frmBillNumber 
   Caption         =   "配送单号导入"
   ClientHeight    =   8145
   ClientLeft      =   60
   ClientTop       =   405
   ClientWidth     =   15855
   Icon            =   "frmBillNumber.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   ScaleHeight     =   8145
   ScaleWidth      =   15855
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsfList 
      ...
      FormatString    =   $"frmBillNumber.frx":6852
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
   ...
   Begin VB.Image Image 
      Height          =   480
      Left            =   255
      Picture         =   "frmBillNumber.frx":6E13
      Top             =   120
      Width           =   480
   End
   Begin VB.Label lblInfor 
      Caption         =   $"frmBillNumber.frx":711D
      Height          =   405
      Left            =   840
      TabIndex        =   9
      Top             =   240
   ...
   End
End
Attribute VB_Name = "frmBillNumber"
...
Private Sub cmd提取补录发票信息_Click()
    ...
    strSQL = "Select Distinct g.配送单号 , a.No, a.药品id, a.序号, d.编码 As 药品编码,d.名称, d.规格, d.计算单位 as 单位,a.产地 as 生产商, a.原产地, a.批号 as 批号, a.效期 as 有效日期, a.填写数量 As 数量, a.成本价 as 单价, a.成本金额 As 金额," & vbNewLine & _
                    "                Nvl(a.批次, 0) 批次,d.是否变价, Decode(a.扣率, Null, 0, a.扣率) As 扣率, a.零售价 As 零售价, a.零售金额, a.差价, a.批准文号, c.随货单号, c.随货日期, c.发票号," & vbNewLine & _
                    "                c.发票代码, c.发票日期, c.发票金额, a.供药单位id, f.编码 As 供应商编码, f.名称 As 供应商, a.摘要 , a.产品合格证 , a.填制人, a.填制日期, a.修改人, a.修改日期, a.审核人," & vbNewLine & _
                    "                a.审核日期, a.库房id, a.外观, a.验收结论, a.生产日期, a.配药人 As 核查人, a.配药日期 As 核查日期, Nvl(a.用法, 0) As 金额差, a.频次 As 加成率," & vbNewLine & _
                    "                a.对方部门id, a.计划id, a.采购途径 " & vbNewLine & _
...
'填充表格数据
    ...
    If cmdOk.Tag = "新增入库单" Then
        strSQL = "Select Distinct a.Id, a.名称, a.规格, a.编码, a.是否变价, c.现价" & vbNewLine & _
                        ...
                        "      Sysdate Between c.执行日期 And Nvl(c.终止日期, To_Date('3000-01-01', 'YYYY-MM-DD')) And c.价格等级 Is Null And Exists" & vbNewLine & _
        ...
        For i = 1 To rsVal.RecordCount
            ...
            If lng药品ID <> 0 Then
                ...
                ElseIf cmdOk.Tag = "补录发票信息" Then
                    ...
                    .TextMatrix(i, .ColIndex("原产地")) = NVL(rsVal!原产地)
                    .TextMatrix(i, .ColIndex("随货日期")) = NVL(rsVal!随货日期)
                    .TextMatrix(i, .ColIndex("验收结论")) = NVL(rsVal!验收结论)
                    .TextMatrix(i, .ColIndex("采购途径")) = NVL(rsVal!采购途径)
    ...
    With vsfList
        For i = 1 To .Rows - 1
            If .Cell(flexcpForeColor, i, .ColIndex("行号"), i, .ColIndex("发票金额")) = mNoColColor And Trim(.TextMatrix(i, .ColIndex("审核人"))) = "" Then
                ...
                strSQL = strSQL & "," & 0
                '原产地
                strSQL = strSQL & ",'" & .TextMatrix(i, .ColIndex("原产地")) & "'"
                '随货日期
                strSQL = strSQL & "," & "to_date('" & .TextMatrix(i, .ColIndex("随货日期")) & "','yyyy-mm-dd HH24:MI:SS')"
                '验收结论
                ...
                strSQL = strSQL & "," & "to_date('" & strDate & "','yyyy-mm-dd HH24:MI:SS')"
                '采购途径
                strSQL = strSQL & ",'" & .TextMatrix(i, .ColIndex("采购途径")) & "'"
