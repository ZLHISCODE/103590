    ...
    public static class ComLib
    {
        ...
        public static void SaveErrorlog(string errorMessage, bool addDateTime = true)
        {
            ...
            try
            {
                ...
                string fileName = "ZlSoft_HIS_PreTriage_ErrorLog_" + DateTime.Now.ToString("yyyy_MM_dd") + ".log";
        ...
        public static void SaveDatalog(string dataMessage, bool addDateTime = true)
        {
            ...
            try
            {
                ...
                string filePath = Path.GetDirectoryName(GetAssemblyLocation());
                string fileName = "ZlSoft_HIS_PreTriage_DataLog_" + DateTime.Now.ToString("yyyy_MM_dd") + ".log";
        ...
        public static long StrToLng(string intStr)
        {
            try
            {
                long parselng;
                if (long.TryParse(intStr, out parselng))
                    return parselng;
                return 0;
            }
            catch
        ...
        /// 显示气泡提示
        /// </summary>
        public static MyWaitFrom ShowWaitform(string strCaption, string strText, int intCloseTime)
        {
            MyWaitFrom frmWait = new MyWaitFrom();
            frmWait.SetCaption(strCaption);
            frmWait.SetDescription(strText);
            frmWait.setCloseTime(intCloseTime);
            frmWait.Show();
            return frmWait;
        }
        ...
        /// <returns></returns>
        public static bool CReportOpen(string strReport, string objPara,int intType)
        {
            try
            {
                string progId = "zl9Report.clsReport";
                ...
                if (DynamicCall.HasProgId(progId) == true)
                {
                    DynamicCall dcRegister = new DynamicCall(progId, false);
                    dcRegister.Call("ReportOpen", new object[] { BaseData.gcnOracle, ComLib.StrToInt(BaseData.SYS), strReport, null, objPara , intType });
                }
                return true;
            }
            catch (Exception ex)
        ...
        public static bool CReportSet()
        {
            try
            {
                string  strPar = "";
                string progId = "zl9PrintMode.zlPrintMethod";
                if (DynamicCall.HasProgId(progId) == true)
                {
                    DynamicCall dcRegister = new DynamicCall(progId, false);
                    strPar = BaseData.SYS + @"\1244";
                    dcRegister.Call("SwitchPrintSet", new object[] { strPar ,false});
                    dcRegister.Call("zlPrintSet", new object[] { });
                    dcRegister.Call("SwitchPrintSet", new object[] { strPar, true });
                }
                return true;
            }
            catch (Exception ex)
            {
                DevExpress.XtraEditors.XtraMessageBox.Show("打印设置失败：" + ex.Message, "急诊预检分诊工作站", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return false;
            }
        }
    }
